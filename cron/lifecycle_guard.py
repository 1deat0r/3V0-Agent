...
``3v0 gateway restart`` (or ``launchctl kickstart ai.3v0.gateway``
or ``systemctl restart 3v0-gateway``).  When the cron fires, the
...
``3v0 cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``3v0 gateway``, ``launchctl ... 3v0-gateway``,
``systemctl ... 3v0-gateway``, ``pkill`` against the gateway) so it
...
commands and shell scripts they reference when ``_EV0_GATEWAY=1``. It also
rejects ``launchctl submit`` in gateway sessions because launchd treats that
primitive as a persistent KeepAlive job, not a one-shot task. ``3v0 gateway
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    ...
    # (kickstart/etc.): `launchctl submit -l ai.3v0.gateway-<suffix> --
...
# lines (e.g. `launchctl submit \` / `  -l ai.3v0.gateway-... \` / `  -- ...`,
...
# for `systemctl restart 3v0-gateway` in syslog, a SQL LIKE literal over a
...
# to execution (`grep 'systemctl restart 3v0-gateway' f | sh`); never mask
# such a line.
...
def contains_launchctl_submit_command(command: str) -> bool:
    ...
    chosen by whoever writes it, so a neutral name (``ai.3v0.svc-reload-tmp``)
...
def _mask_data_sink_arguments(text: str) -> str:
    ...
    ``systemctl restart 3v0-gateway`` from the same characters appearing
    ...
    (false positives blocking legitimate cron prompts), e.g.::
        ...
        sqlite3 db "SELECT msg FROM log WHERE msg LIKE '%systemctl restart 3v0-gateway%'"
...
def _iter_referenced_shell_scripts(
    ...
    *,
    ...
    for segment in _iter_command_segments(command):
        ...
        # (e.g. `Path.home() / ".3V0"`), not an executable reference.
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    under ``<EV0_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.3V0/scripts/restart.sh``) but is passed as the bare name
...
def check_gateway_lifecycle(
    ...
    if python_script:
        ...
        # `3v0 gateway restart` embedded in a .py script is still
