...
class InProcessCronScheduler(CronScheduler):
    ...
    def start(
        ...
        *,
        ...
        # only the process-global EV0_HOME (the default profile) is ticked.
        # Heartbeats and recovery are also scoped per profile so `3v0 cron
        ...
        # Heartbeat once before the first sleep so `3v0 cron status` sees a
        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                # `3v0 cron status`/`list` (separate processes) can show
    ...
    def _start_multiplex(
        ...
        *,
        ...
        Each profile uses ``set_ev0_home_override()`` + ``use_cron_store()``
            ...
            for entry in profile_homes:
                ...
                try:
                    with use_cron_store(home):
                        ...
                        # so `3v0 cron status` can show WHY ticks fail
                        # (#68483).
                        if ok:
                            clear_ticker_error()
                        elif _tick_error:
