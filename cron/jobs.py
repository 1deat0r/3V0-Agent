...
Jobs are stored in ~/.3V0/cron/jobs.json
Output is saved to ~/.3V0/cron/output/{job_id}/{timestamp}.md
"""
...
# store under its own EV0_HOME, and a profile-scoped gateway runs that
# profile's jobs under that same EV0_HOME — so a job authored in profile
# `coder` lives in `~/.3V0/profiles/coder/cron/jobs.json` and executes with
# `coder`'s `.env`, `config.yaml`, and skills. We deliberately anchor on
# `get_ev0_home()` (the active profile home), NOT `get_default_ev0_root()`
# (the shared root). Anchoring at the root would funnel every profile's jobs
# into one shared `jobs.json` and run them under whatever EV0_HOME the
...
CRON_DIR = EV0_DIR / "cron"
JOBS_FILE = CRON_DIR / "jobs.json"
# Heartbeat file the in-process ticker touches on every loop iteration. The
# gateway process and the (separate) ``3v0 cron status`` process share it
...
# threshold in `3v0 cron status` (ev0_cli/cron.py), so the two never
# drift apart.
...
def _current_cron_store() -> _CronStorePaths:
    ...
    3. the ACTIVE profile home, resolved fresh via get_ev0_home()
       (context-local override, then the EV0_HOME env var) — so a test
       or embedder that re-points EV0_HOME after this module was
...
# the cron inactivity timeout is disabled (EV0_CRON_TIMEOUT=0 → unlimited),
...
# TTL only recovers a claim left by a tick that DIED mid-run. EV0_CRON_TIMEOUT
...
def _oneshot_run_claim_ttl_seconds() -> float:
    ...
    Derived from ``EV0_CRON_TIMEOUT`` (the cron inactivity timeout the
...
def _jobs_lock():
    ...
    and standalone ``3v0`` CLI invocations, which previously shared no lock
...
def _preserve_file_ownership(path: Path, before: Optional[os.stat_result]) -> None:
    ...
    command (``docker exec 3v0 3v0 cron create ...`` — ``docker exec``
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    if 'T' in schedule or re.match(r'^\d{4}-\d{2}-\d{2}', schedule):
        try:
            ...
            #
            # Anchor to the CONFIGURED 3V0 timezone, not the server's local
            # timezone. The due-check (`get_due_jobs`) compares `next_run_at`
            # against `ev0_time.now()`, which uses the configured zone. If a
...
def _ensure_aware(dt: datetime) -> datetime:
    """Return a timezone-aware datetime in 3V0 configured timezone.

    Backward compatibility:
    - Older stored timestamps may be naive.
    - Naive values are interpreted as *system-local wall time* (the timezone
      `datetime.now()` used when they were created), then converted to the
      configured 3V0 timezone.
...
def _stored_wall_clock_is_future(stored: datetime, current: datetime) -> bool:
    ...
    Cron schedules express local wall-clock intent. If 3V0/system local time
    ...
    if kind == "once":
        ...
        if not _ensure_croniter():
            logger.warning(
                ...
                "reinstall 3v0-agent or run 'pip install croniter' in your "
                "runtime env.",
                expr,
            )
            return None
...
# =============================================================================
# Ticker heartbeat (liveness signal for `3v0 cron status`)
# =============================================================================
...
def _atomic_write_epoch(path: Path) -> None:
    ...
    in another process (``3v0 cron status``) never sees a torn/truncated
...
def record_ticker_heartbeat(success: bool = False) -> None:
    ...
    `3v0 cron status` can tell a thread that is merely *alive and looping*
...
def get_ticker_heartbeat_age() -> Optional[float]:
    ...
    ``3v0 cron status`` must report per-profile liveness (#69377).
    """
    store = _current_cron_store()
...
def record_ticker_error(message: str) -> None:
    ...
    The ticker thread lives inside the gateway process; ``3v0 cron
...
def create_job(
    ...
    Args:
        ...
        script: Optional path to a script whose stdout feeds the job. With
                ...
                ~/.3V0/scripts/; ``.sh`` / ``.bash`` files run via bash,
        ...
        monitor_script: Optional path to a cheap monitor source script (same
                ...
                ~/.3V0/scripts/, .sh/.bash via bash, else Python). Each
    ...
    # covered, not just `3v0 cron create`.
...
def _get_due_jobs_locked() -> List[Dict[str, Any]]:
    ...
    # (derived from EV0_CRON_TIMEOUT). See _oneshot_run_claim_ttl_seconds.
    ...
    for job in jobs:
        ...
        try:
            ...
            if next_run_dt <= now:
                ...
                # desktop both run in-process 60s tickers on one EV0_HOME)
                ...
                # (_oneshot_run_claim_ttl_seconds, derived from EV0_CRON_TIMEOUT),
...
# execution. Unlike the quick-snapshot store (`ev0_cli.backup`, capped at 20)
...
def _prune_job_output(job_output_dir: Path, keep: int) -> int:
    ...
    Mirrors the quick-snapshot retention in ``ev0_cli.backup._prune_quick_snapshots``:
...
def _canonical_skill_ref(raw: Any) -> str:
    ...
    A job may store an absolute path under ``EV0_HOME/skills`` or an
