...
# threshold in `3v0 cron status` (threev0_cli/cron.py), so the two never
# drift apart.
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    if 'T' in schedule or re.match(r'^\d{4}-\d{2}-\d{2}', schedule):
        try:
            ...
            # against `threev0_time.now()`, which uses the configured zone. If a
...
# execution. Unlike the quick-snapshot store (`threev0_cli.backup`, capped at 20)
...
def _prune_job_output(job_output_dir: Path, keep: int) -> int:
    ...
    Mirrors the quick-snapshot retention in ``threev0_cli.backup._prune_quick_snapshots``:
