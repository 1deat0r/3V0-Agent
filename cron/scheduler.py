...
Uses a file-based lock (~/.3V0/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `3v0 update` reloads
# the module) fail with ModuleNotFoundError for ev0_time et al.
...
def _fallback_chain_phrase() -> str:
    ...
    if chain:
        return "Fallback chain was exhausted or unavailable."
    return (
        "No fallback chain configured — add one with `3v0 fallback add`, "
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy import: avoid heavy ev0_cli import at cron module load (matches
    # _resolve_cron_enabled_toolsets' fallback) and share one MCP-membership
    # computation with the gateway/CLI platform resolver.
    from ev0_cli.tools_config import enabled_mcp_server_names
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``3v0 tools`` config for the ``cron`` platform.
    ...
    try:
        from ev0_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
def _inflight_min_allowance_minutes() -> float:
    ...
    ``EV0_CRON_INFLIGHT_MAX_MINUTES`` env var is kept as an internal
    escape hatch only.
    """
    try:
    ...
    if raw:
        try:
            ...
            logger.warning(
                "Invalid EV0_CRON_INFLIGHT_MAX_MINUTES=%r; using default %s",
                raw,
...
# (EV0_CRON_TIMEOUT, default 600s): a wedged lock holder stops touching
...
def _cron_inactivity_seconds() -> float:
    """Parse EV0_CRON_TIMEOUT (seconds). 0 = unlimited; bad input = 600.
    ...
    try:
        ...
        logger.warning("Invalid EV0_CRON_TIMEOUT=%r; using default 600s", raw)
        return 600.0
...
def _write_usage_audit(record: dict) -> None:
    """Append a single JSONL line to ~/.3V0/cron/usage_audit.jsonl.
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    ``3v0 update`` / ``3v0 gateway stop`` / systemd restart, or an
...
def _get_ev0_home() -> Path:
    ...
    profile-scoped gateway, so resolving the active EV0_HOME at call time
...
def _get_script_timeout() -> int:
    ...
    if env_value:
        try:
            ...
            logger.warning("Invalid EV0_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(
    ...
    Scripts must reside within EV0_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other 3V0-managed secrets are not inherited
    (SECURITY.md §2.3), matching terminal and MCP child processes.

    Args:
        script_path: Path to the script.  Relative paths are resolved
            against EV0_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    # escape — scripts MUST reside within EV0_HOME/scripts/.
    try:
...
def _preflight_check_provider_key(job: dict, cfg: dict) -> Optional[str]:
    ...
    try:
        ...
        return (
            ...
            "Set the provider API key in .env (or `3v0 setup`), or pin a "
    ...
    for platform_name in platform_parts:
        ...
        if platform_name.lower() not in connected:
            return (
                ...
                "`3v0 setup` or change the job's `deliver` target."
            )
    return None
...
def run_job(
    ...
    *,
    ...
    #
    # Bounded with its own timeout (separate from EV0_CRON_TIMEOUT, which
    ...
    try:
        ...
        if _raw_env_timeout:
            try:
                ...
                logger.warning(
                    "Invalid EV0_CRON_SESSION_DB_TIMEOUT=%r; using config/default",
                    _raw_env_timeout,
                )
    ...
    # gateway message. Do not seed EV0_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads EV0_SESSION_PLATFORM
    #     and EV0_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via EV0_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     EV0_SESSION_PLATFORM == "telegram".
    ...
    #     send_message gate read EV0_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the EV0_CRON_AUTO_DELIVER_* vars set
    # below, so clearing EV0_SESSION_* here does not affect delivery.
    ...
    _ctx_tokens = set_session_vars(
        ...
        # EV0_SESSION_* routing keys just below, so an async delegation's
        ...
        # os.environ EV0_SESSION_KEY, which risks routing a cron subagent's output
    ...
    _cron_delivery_vars = (
        "EV0_CRON_AUTO_DELIVER_PLATFORM",
        "EV0_CRON_AUTO_DELIVER_CHAT_ID",
        "EV0_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    _cron_session_var = _VAR_MAP["EV0_CRON_SESSION"]
    ...
    try:
        ...
        #
        # A kanban worker is a normal `3v0 chat -q` CLI agent whose default
        # toolset includes `cronjob`, running with EV0_KANBAN_TASK
        ...
        # and kanban_complete defaults task_id to $EV0_KANBAN_TASK -- letting
        ...
        # load_ev0_dotenv (not a bare load_dotenv) and reset the secret-
        # source cache first: startup already applied external secrets and
        # recorded this EV0_HOME in _APPLIED_HOMES, so a naive reload would
        ...
        from ev0_cli.env_loader import (
            load_ev0_dotenv,
        ...
        if delivery_target:
            _VAR_MAP["EV0_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["EV0_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["EV0_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
        ...
        # cron-fleet default) > EV0_MODEL env > config.yaml ``model:``
        ...
        if not (isinstance(model, str) and model.strip()):
            raise RuntimeError(
                ...
                "default with `3v0 model <name>`."
            )

        # Apply IPv4 preference if configured.
        try:
            from ev0_constants import apply_ipv4_preference
        ...
        try:
            # Do not inject EV0_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        agent = AIAgent(
            ...
            # EV0_HOME. When a workdir is configured, also inject project
        ...
        # override via EV0_CRON_TIMEOUT env var.  0 = unlimited.
        #
        ...
        if final_response.strip() and turn_exit_reason:
            ...
            _explainer_variants = []
            try:
                from ev0_state import PERSISTENCE_ERROR_CAUSES as _causes
            except Exception:
...
def tick(
    ...
    *,
    ...
    try:
        # Global emergency stop (`3v0 pause`): skip dispatch entirely while
        # the ESTOP sentinel exists. Never touches in-flight runs — due jobs
        # simply wait for the next tick after `3v0 resume`. Logged once per
        ...
        # scheduler STARTUP. A one-shot `3v0 cron run` that claimed a job
        ...
        try:
            ...
            logger.warning("Invalid EV0_CRON_MAX_PARALLEL value; defaulting to unbounded")
