...
Usage:
    python -m threev0_cli.main web          # Start on http://127.0.0.1:9119
    python -m threev0_cli.main web --port 8080
"""
...
from threev0_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
...
def _warm_gateway_module() -> None:
    ...
    ``threev0_cli.gateway``. But the first WS connection and its initial
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from threev0_cli.gateway import _get_restart_drain_timeout
...
def _eager_reconcile_own_session_db() -> None:
    ...
    """
    try:
        from threev0_state import SessionDB, _default_db_path
...
async def _lifespan(app: "FastAPI"):
    ...
    # Import threev0_cli.gateway eagerly *before* the lifespan yield so the
    ...
    if os.getenv("EV0_DESKTOP") == "1":
        ...
        try:
            from threev0_cli.gateway import _reap_unsupervised_gateway_orphans
...
from threev0_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
#
# This list is defined in ``threev0_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from threev0_cli.dashboard_auth.public_paths import (
...
async def _dashboard_auth_gate(request: Request, call_next):
    from threev0_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from threev0_cli.dashboard_auth.token_auth import token_auth_middleware
...
from threev0_cli.web_models import (  # noqa: F401
    ConfigUpdate,
    EnvVarUpdate,
    EnvVarDelete,
    EnvVarReveal,
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    """
    from threev0_cli.config import get_compatible_custom_providers
    from threev0_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from threev0_cli.model_normalize import normalize_model_for_provider
    from threev0_cli.providers import resolve_custom_provider, resolve_user_provider
    ...
    if (
        ...
        try:
            ...
            cur_provider = ""
        from threev0_cli.models import _AGGREGATOR_PROVIDERS
...
# gate + path hardening as /api/fs). Logic lives in ``threev0_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from threev0_cli import web_git as _web_git  # noqa: E402
...
from threev0_cli.web_routers import git as _git_routes  # noqa: E402

app.include_router(_git_routes.router)
from threev0_cli.web_routers.git import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    git_status_route,
    git_worktrees_route,
    git_branches_route,
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    * ``gateway_mode`` — ``"multiplex"`` when the default gateway serves
    ...
    """
    try:
        from threev0_cli.profiles import _check_gateway_running, profiles_to_serve
...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        # Windows install the first import of threev0_cli.gateway blocks the
        ...
        auth_flows: list[str] = []
        try:
            from threev0_cli.dashboard_auth import (
        ...
        try:
            from threev0_cli.auth import get_nous_session_validity
...
def _get_portal_status_sync():
    ...
    auth: Dict[str, Any] = {}
    try:
        from threev0_cli.auth import get_nous_auth_status_local
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from threev0_cli.debug import build_debug_share
...
def _spawn_threev0_action(
    ...
    *,
    ...
    Uses the running interpreter's ``threev0_cli.main`` module so the action
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "threev0_cli.main", *subcommand]
...
async def check_threev0_update(force: bool = False):
    ...
    try:
        from threev0_cli.banner import check_for_updates
...
from threev0_cli.web_routers import sessions as _sessions_routes  # noqa: E402

app.include_router(_sessions_routes.list_router)
from threev0_cli.web_routers.sessions import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_sessions,
)


from threev0_cli.web_routers import profiles as _profiles_routes  # noqa: E402

app.include_router(_profiles_routes.sessions_router)
from threev0_cli.web_routers.profiles import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
...
async def get_egress_status():
    ...
    from threev0_cli.proxy_cli import format_status_text
...
# in threev0_cli/config.py — listed here for deterministic ordering in the UI.
...
async def get_model_options(
    ...
    """
    try:
        from threev0_cli.inventory import build_model_options_payload, load_picker_context
...
def get_recommended_default_model(provider: str = ""):
    ...
    try:
        ...
        from threev0_cli.models import pick_silent_default_model
...
def get_moa_models(profile: Optional[str] = None):
    ...
    try:
        from threev0_cli.moa_config import normalize_moa_config
...
def set_moa_models(body: MoaConfigPayload, profile: Optional[str] = None):
    ...
    try:
        from threev0_cli.moa_config import normalize_moa_config, validate_moa_payload
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (threev0_cli/main.py
        ...
        if provider.strip().lower() == "nous":
            try:
                from threev0_cli.nous_subscription import apply_nous_managed_defaults
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from threev0_cli.main import _auto_provider_name, _save_custom_provider
...
def _catalog_provider_env_metadata() -> dict:
    ...
    """
    try:
        from threev0_cli.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from threev0_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
...
async def set_env_var(body: EnvVarUpdate, profile: Optional[str] = None):
    def _run():
        with _profile_scope(body.profile or profile):
            ...
            from threev0_cli.credential_lifecycle import save_provider_env_credential
...
async def remove_env_var(body: EnvVarDelete, profile: Optional[str] = None):
    def _run():
        with _profile_scope(body.profile or profile):
            ...
            from threev0_cli.credential_lifecycle import remove_provider_env_credential
...
# threev0_cli/setup_hidden_env.py. Shared with the `3v0 setup gateway`
# wizard so the surfaces ask for the same things.
from threev0_cli.setup_hidden_env import (  # noqa: E402
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from threev0_cli import auth as hauth
        if provider_id == "nous":
    ...
    def _run():
        with _profile_scope(profile):
            ...
            if provider_id == "anthropic":
                ...
                try:
                    from threev0_cli.auth import clear_provider_auth
            ...
            try:
                from threev0_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
async def _start_device_code_flow(
    ...
    if provider_id == "xai-oauth":
        from threev0_cli.auth import _xai_oauth_request_device_code
        import httpx
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from threev0_cli.auth import persist_nous_credentials
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from threev0_cli.auth import _save_codex_tokens
...
def _session_db_read_probe_statements() -> tuple:
    ...
    """
    from threev0_state_schema import schema_read_probe_statements
...
def _open_session_db_at_path(db_path: Path, *, read_only: bool):
    ...
    from threev0_state import SessionDB, is_malformed_db_error
...
async def get_logs(
    ...
    from threev0_cli.logs import _read_tail, LOG_FILES
    ...
    try:
        from threev0_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
from threev0_cli.web_routers import cron as _cron_routes  # noqa: E402

app.include_router(_cron_routes.router)
from threev0_cli.web_routers.cron import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_cron_jobs,
    get_cron_job,
    list_cron_job_runs,
    create_cron_job,
...
def _gateway_fire_endpoint(profile: str, home: Path) -> str:
    ...
    try:
        ...
        # yaml.safe_load of config.yaml (tests/threev0_cli/
...
#
# Wraps the same config data layer the CLI uses (threev0_cli.mcp_config), so
...
from threev0_cli.web_routers import mcp as _mcp_routes  # noqa: E402

app.include_router(_mcp_routes.router)
from threev0_cli.web_routers.mcp import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_mcp_servers,
    add_mcp_server,
    replace_mcp_servers,
    remove_mcp_server,
...
def _mcp_oauth_callback_url(request: Request, server_name: str) -> str:
    ...
    from threev0_cli.dashboard_auth.prefix import prefix_from_request, resolve_public_url
...
#
# Wraps the same JSON store the CLI uses (threev0_cli.webhook); the webhook
...
async def list_webhooks():
    import threev0_cli.webhook as wh
...
async def list_credential_pool():
    ...
    from threev0_cli.auth import read_credential_pool
...
async def remove_credential_pool_entry(provider: str, index: int):
    ...
    from threev0_cli.auth import suppress_credential_source
...
async def list_hooks():
    ...
    def _run():
        ...
        try:
            from threev0_cli.plugins import VALID_HOOKS
...
from threev0_cli.web_routers import skills as _skills_routes  # noqa: E402

app.include_router(_skills_routes.hub_router)
from threev0_cli.web_routers.skills import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    install_skill_hub,
    uninstall_skill_hub,
    update_skills_hub,
...
from threev0_cli.web_routers import tools as _tools_routes  # noqa: E402

app.include_router(_tools_routes.router)
from threev0_cli.web_routers.tools import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_toolsets,
    toggle_toolset,
    get_toolset_config,
    get_toolset_models,
...
if sys.platform.startswith("win"):
    try:
        from threev0_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from threev0_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
from threev0_cli.pty_session import PtySessionRegistry, RegistryFull, run_reaper  # noqa: E402
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from threev0_cli.dashboard_auth.audit import AuditEvent, audit_log
        from threev0_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    matching what ``threev0_cli.main._launch_tui`` does for the CLI path.
    ...
    """
    from threev0_cli.main import PROJECT_ROOT, _apply_tui_python_env, _make_tui_argv
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from threev0_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from threev0_cli.console_engine import Ev0ConsoleEngine
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`threev0_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from threev0_cli.dashboard_auth.prefix import normalise_prefix
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as threev0_cli.plugins):
    ...
    # ``threev0_cli/plugins.py`` and the documented user contract.
...
async def get_dashboard_plugins():
    ...
    def _run():
        ...
        try:
            from threev0_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
            enabled_set = _get_enabled_set()
            disabled_set = _get_disabled_set()
        except Exception:
            enabled_set = set()
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from threev0_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from threev0_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from threev0_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from threev0_cli.plugins_cmd import dashboard_remove_user_plugin
...
from threev0_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    try:
        from threev0_cli.nous_auth_keepalive import start_nous_auth_keepalive
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # (#54489).
            try:
                from threev0_cli.config import load_config as _load_cfg
                from threev0_cli.plugins_cmd import _BASIC_AUTH_PLUGIN_KEYS
