...
the existing module-level functions in threev0_cli.gateway and
threev0_cli.gateway_windows directly. This protocol is a thin facade
...
``3v0 gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``3v0 gateway start/stop/restart``).
    """
    ...
    # `3v0 gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged 3v0
    ...
    detection always fails. Since every 3V0 runtime call inside the
    container drops to 3v0 via ``s6-setuidgid``, that silent failure
...
# in ``threev0_cli.gateway`` (systemd/launchd) and ``threev0_cli.gateway_windows``
...
# active (set via the ``3v0 -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in threev0_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from threev0_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from threev0_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from threev0_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from threev0_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in threev0_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from threev0_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from threev0_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from threev0_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from threev0_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``threev0_cli.gateway_windows`` (Scheduled Task /
    ...
    def is_running(self, name: str) -> bool:
        from threev0_cli import gateway_windows
        from threev0_cli.gateway import find_gateway_pids
...
#
# Per-profile gateways are registered dynamically when `3v0 profile create`
# runs inside the container (Phase 4). Static services (main-3v0, dashboard)
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's EV0_HOME blindly;
    derive the shared profile root from the current EV0_HOME and map the
...
# UID/GID of the in-image ``3v0`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the 3v0 user.
    ...
    (the ``3v0`` user — UID 10000 — running every 3V0 runtime
    ...
    with 3v0 ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           3v0:3v0, 0755 (parent must already exist)
    ``svc_dir/event/``                     3v0:3v0, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 3v0:3v0, 0755
    ``svc_dir/supervise/event/``           3v0:3v0, 03730
    ``svc_dir/supervise/control``          3v0:3v0, 0660    (FIFO)
    ...
    slot's supervise/ was 3v0-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _EV0_UID, _EV0_GID)
        except PermissionError:
            # Running as the 3v0 user already — directory is 3v0-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _EV0_UID, _EV0_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is 3v0-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _EV0_UID, _EV0_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``3v0 -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`3v0 profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-3v0, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources EV0_HOME (and any extra env) via with-contenv —
             so e.g. ``-e EV0_HOME=/data/3v0`` is honored at run
          ...
          4. Drops to the 3v0 user and exec's
             ``3v0 -p <profile> gateway run`` (or just ``3v0
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``3v0 gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root EV0_HOME profile" (the implicit profile that exists at
        the top of $EV0_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``threev0_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$EV0_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``EV0_HOME``,
        ...
        allocator in ``threev0_cli.profiles._allocate_gateway_port``).
        ...
        # guard.
        lines.append("export EV0_S6_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its
        # profile's EV0_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``3v0 gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-EV0_HOME PID lock first;
        ...
        # gateway/run.py) so s6 always wins. The EV0_S6_SUPERVISED_CHILD
        # sentinel above prevents the run→start→run redirect recursion.
        # Each profile is scoped to its own EV0_HOME and s6 guarantees a
        # single supervised instance per slot, so there is no legitimate
        # supervised sibling for ``--replace`` to clobber.
        if profile == "default":
            gateway_cmd = "3v0 gateway run --replace"
        else:
            gateway_cmd = f"3v0 -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid 3v0 {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${EV0_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the EV0_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e EV0_HOME=/data/3v0``
        gets its logs under /data/3v0/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``3v0 logs`` reads and what
        ...
        return (
            ...
            f': "${{EV0_HOME:=/opt/data}}"\n'
            f'log_dir="$EV0_HOME/logs/gateways/{prof}"\n'
            # Create the leaf and clear a stale s6-log lock as 3v0 when
            # this script starts as root. Never chown or unlink 3v0-writable
            # volume paths from this restartable root-context script:
            # log/supervise/control is 3v0-owned, so an unprivileged user
            # can race a pathname op through a symlink swap (CWE-59 /
            # CWE-367). Parent logs/gateways is seeded 3v0-owned at stage2
            # boot (#45258; tests/docker/test_log_dir_seed.py).
            f'if [ "$(id -u)" = 0 ]; then\n'
            f'  s6-setuidgid 3v0 mkdir -p "$log_dir"\n'
            f'  s6-setuidgid 3v0 rm -f "$log_dir/lock"\n'
            f'else\n'
            f'  mkdir -p "$log_dir"\n'
            f'  rm -f "$log_dir/lock"\n'
            f'fi\n'
            ...
            f'exec s6-setuidgid 3v0 s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``3v0 gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``3v0 -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with 3v0 ownership
            ...
            # the 3v0 user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is 3v0-owned (see ``_seed_supervise_skeleton``), and on
