...
def remove_wrapper_script():
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains threev0_cli reference)
                content = wrapper.read_text(encoding="utf-8")
                if 'threev0_cli' in content or '3v0-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    try:
        from threev0_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from threev0_cli.gateway import get_launchd_plist_path
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m threev0_cli.uninstall --mode <lite|full>``.

    Exists so the agent can run the uninstall under a Python interpreter
    ...
    The wrapper launches this with the system Python + ``PYTHONPATH=<agentRoot>``
    so ``import threev0_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``threev0_constants`` + ``threev0_cli.colors``,
    so it runs fine under a bare system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m threev0_cli.uninstall")
    parser.add_argument(
        "--mode",
        choices=["lite", "full"],
        required=True,
