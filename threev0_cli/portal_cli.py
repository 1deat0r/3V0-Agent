"""``3v0 portal`` — the human-readable entry point for Nous Portal.

Running ``3v0 portal`` with no subcommand performs the one-shot Portal
...
``3v0 auth add nous --type oauth`` (which still works), is identical to
``3v0 setup --portal``, and runs the same Nous flow as the first-time quick
setup.
...
already in ``3v0 auth`` or ``3v0 tools``. It's the onboarding + discovery
...
DOCS_URL = "https://github.com/1deat0r/3V0-Agent/docs/user-guide/features/tool-gateway"
...
def _cmd_status(args) -> int:
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `3v0 model`)")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `3v0 portal`.", Colors.YELLOW))
        print()
...
def _cmd_login(args) -> int:
    ...
    This is the human-readable front door for `3v0 auth add nous --type
    oauth`. It reuses the exact wiring behind `3v0 setup --portal` (which in
    ...
    """
    from threev0_cli.setup import _run_portal_one_shot
...
def portal_command(args) -> int:
    """Top-level dispatch for `3v0 portal <subcommand>`."""
    sub = getattr(args, "portal_command", None)
    if sub in {None, "", "login"}:
        # Default to the one-shot onboarding — `3v0 portal` is the
        # human-readable alias for `3v0 auth add nous --type oauth` /
        # `3v0 setup --portal`.
    ...
    print("Run `3v0 portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `3v0 portal` on the given argparse subparsers object."""
    portal_parser = subparsers.add_parser(
        "portal",
        help="Set up Nous Portal (login, model pick, Tool Gateway); see also `portal info`",
        description=(
            "Run `3v0 portal` with no subcommand to log in to Nous Portal "
            "and set it up — pick a model, set Nous as your provider, and offer "
            "the Tool Gateway (the human-readable alias for `3v0 auth add "
            "nous --type oauth`, identical to `3v0 setup --portal`). "
