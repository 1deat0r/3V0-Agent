...
from threev0_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
_UPSTREAM_REPO_URL = "https://github.com/1deat0r/3V0-Agent.git"
...
def check_for_updates() -> Optional[int]:
    ...
    try:
        from threev0_cli.config import detect_install_method, get_project_root
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``threev0_cli/build_info.py``) and return it as a frozen
...
def _compute_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    if repo_dir is None:
        ...
        try:
            from threev0_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/1deat0r/3V0-Agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    canonical 1deat0r/3V0-Agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"3V0 Agent v{VERSION} ({RELEASE_DATE})"
...
def _format_update_notice(behind: int) -> str:
    ...
    from threev0_cli.config import get_managed_update_command, recommended_update_command
    if behind > 0:
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    if not _has_native_mcp:
        try:
            from threev0_cli.plugins import get_portable_mcp_server_names_nowait
    ...
    try:
        from threev0_cli.codex_runtime_switch import get_current_runtime
        from threev0_cli.config import load_config as _load_cfg
