...
Toggles `model.openai_runtime` between "auto" (= chat_completions, 3V0'
...
The actual runtime resolution happens in threev0_cli.runtime_provider's
...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    if raw in {"off", "default", "disable", "3v0"}:
        return "auto", []
...
def apply(
    ...
    *,
    ...
    # the world state (managed block in ~/.codex/config.toml, 3v0-tools
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate 3V0' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into 3V0 for
        # browser/web/delegate_task/vision/memory tools (#7 fix).
        # Failures are non-fatal — the runtime change still proceeds.
        try:
            from threev0_cli.codex_runtime_plugin_migration import migrate
            mig_report = migrate(config)
            # Tools/MCP servers (excluding the 3v0-tools callback,
            # which is internal plumbing — surface separately).
            user_servers = [
                s for s in mig_report.migrated if s != "3v0-tools"
            ]
            if user_servers:
                msg_lines.append(
            ...
            # Permissions + 3V0 tool callback are always-on production
            ...
            if "3v0-tools" in mig_report.migrated:
                msg_lines.append(
                    "3V0 tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default 3V0 runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "3V0 tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("OpenAI/Codex turns will use the default 3V0 runtime.")
