...
The provider list shown by ``3v0 model`` (CLI/TUI) and the desktop Settings
→ Providers tabs (Accounts + API keys) **must be the same set**.  Historically
they were not: the CLI picker read :data:`threev0_cli.models.CANONICAL_PROVIDERS`
...
the same universe ``3v0 model`` renders (``CANONICAL_PROVIDERS``), joining:

* ``auth_type`` / ``api_key_env_vars`` / ``base_url_env_var`` from
  :data:`threev0_cli.auth.PROVIDER_REGISTRY` (credential truth), and
...
``CANONICAL_PROVIDERS`` universe, i.e. exactly what ``3v0 model`` shows.
"""
...
# threev0_cli.auth.PROVIDER_REGISTRY and providers.base.ProviderProfile.
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `3v0 model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """Return one descriptor per provider in the ``3v0 model`` universe.
    ...
    # 3V0 overlays carry auth_type for providers that have no registry/profile
    ...
    for order, entry in enumerate(CANONICAL_PROVIDERS):
        ...
        overlay = EV0_OVERLAYS.get(slug)

        # auth_type: registry is authoritative; fall back to profile, then the
        # 3V0 overlay (e.g. moa → "virtual"), then api_key.
        auth_type = (
