"""3V0 update pipeline — extracted from ``threev0_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``threev0_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``threev0_cli.main`` reference — so existing call sites and test monkeypatches
that target ``threev0_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``threev0_cli.main``.
...
Imports are one-way: ``threev0_cli.main`` imports this module, never the reverse
...
def _m():
    """Lazy ``threev0_cli.main`` reference.

    Lets callers keep patching ``threev0_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``threev0_cli.main`` -> ``threev0_cli.update_cmd`` stays one-way
    at import time.
    """
    from threev0_cli import main
...
def _reload_config_modules() -> None:
    ...
    This function force-reloads ``threev0_cli.config_defaults``,
    ``threev0_cli.config``, and ``threev0_cli.config_migrations`` from disk
    so subsequent imports read the UPDATED code.

    It also reloads ``threev0_cli._subprocess_compat`` and
    ``threev0_cli.dashboard_procs`` so that post-update dashboard cleanup
    ...
    importlib.invalidate_caches()
    for mod_name in (
        "threev0_cli.config_defaults",
        "threev0_cli.config",
        "threev0_cli.config_migrations",
        "threev0_cli._subprocess_compat",
...
def _run_config_check_fresh() -> tuple:
    ...
    from threev0_cli.config import check_config_version
...
_UPDATE_CRITICAL_FILES = (
    "threev0_cli/main.py",
    "threev0_cli/config.py",
    "threev0_cli/__init__.py",
    "threev0_cli/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    """
    from threev0_constants import FIRST_PARTY_MODULE_ROOTS
    ...
    probe = (
        ...
        # The root set is injected from threev0_constants so this can't drift
...
def _reload_process_scan_modules() -> None:
    ...
    ``from threev0_cli._subprocess_compat import bounded_probe_run``. If the
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``threev0_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``threev0_constants.py``
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False):
    ...
    from threev0_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        from threev0_cli.model_catalog import seed_cache_from_checkout
...
def _capture_active_tool_dependencies() -> list[str]:
    ...
    try:
        from threev0_cli import tools_config
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    try:
        from threev0_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from threev0_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from threev0_constants import is_wsl
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    """
    from threev0_cli.config import detect_install_method, recommended_update_command_for_method
    ...
    if method == "docker":
        ...
        from threev0_cli.config import format_docker_update_message
    ...
    from threev0_cli.gitlock import clear_stale_git_locks
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            from threev0_cli.banner import _github_compare_behind
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    snapshot_id = None
    try:
        from threev0_cli.backup import (
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    Windows: the Desktop app's backend (``python.exe -m threev0_cli.main
    ...
    for proc in proc_iter:
        ...
        # this venv's path, or a `-m threev0_cli.main ...` invocation tied to
        # this install (install root in the cmdline or as the working dir).
        if not is_holder and venv_prefix in cmdline_low:
            is_holder = True
        if not is_holder and "threev0_cli.main" in cmdline_low:
        ...
        # the `-m threev0_cli.main gateway run` argv, so autostarted gateways
...
# ``cryptography`` is the canonical case: ``threev0_cli.main`` used to import
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from threev0_cli._scan_venv_blockers import _is_pausable_gateway
...
def _orphaned_desktop_backend_pids(
    ...
    - its cmdline is a 3V0 backend (``threev0_cli.main`` + ``serve`` /
      ``dashboard``), and
    ...
    def _is_backend(argv_low: str) -> bool:
        return "threev0_cli.main" in argv_low and (
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m threev0_cli.main
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    when ``threev0_cli.gateway`` itself no longer imports, which is one of the
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    (e.g. importing ``threev0_cli.gateway`` from the freshly pulled checkout)
...
def _rebuild_desktop_after_update(
    ...
    desktop_build_cmd = [sys.executable, "-m", "threev0_cli.main", "desktop", "--build-only"]
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # orphan merge-conflict markers in threev0_cli/config.py bricked
        ...
        # the old bytecode (e.g. get_ev0_home added to threev0_constants).
        removed = _m()._clear_bytecode_cache(_m().PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # symbols in threev0_constants or lazy_deps. The dependency install
        ...
        try:
            from threev0_cli.profiles import backfill_profile_envs
        ...
        # ``sys.modules`` cache holds the OLD ``threev0_cli.config`` and
        # ``threev0_cli.config_migrations`` from before ``git pull`` updated
        # the source files. A function-level ``from threev0_cli.config import
        ...
        try:
            ...
            if (
                ...
                from threev0_cli.tools_config import install_cua_driver
        ...
        try:
            ...
            try:
                from threev0_cli.gateway import _get_restart_exit_wait_budget
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state
      (typically the freshly-pulled ``threev0_cli.gateway`` no longer imports,
