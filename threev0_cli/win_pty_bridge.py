"""Windows ConPTY bridge for the `3v0 dashboard` chat tab.

Drop-in counterpart to ``threev0_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``threev0_cli.web_server`` consumes: ``spawn``, ``read``,
