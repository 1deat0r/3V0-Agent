"""Shell completion script generation for 3v0 CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""# 3V0 Agent bash completion
# Add to ~/.bashrc:
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef 3v0
# 3V0 Agent zsh completion
# Add to ~/.zshrc:
...
_ev0_profiles() {{
    ...
    if [[ -d "$HOME/.3V0/profiles" ]]; then
        profiles+=($HOME/.3V0/profiles/*(N/:t))
    fi
