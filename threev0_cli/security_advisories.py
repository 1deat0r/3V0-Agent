"""
Security advisory checker for 3V0 Agent.
...
- **Acknowledgeable.** Once the user has read and acted on an advisory they
  can dismiss it via ``3v0 doctor --ack <id>``; the ack is persisted to
...
1. ``3v0 doctor`` (and ``3v0 doctor --ack <id>``)
...
run in environments where the rest of 3V0 failed to import.
"""
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        summary=(
            ...
            "imported mistralai 2.4.6 — including 3v0 when configured "
        ...
        remediation=(
            ...
            "Rotate API keys in ~/.3V0/.env (OpenRouter, Anthropic, OpenAI, "
            ...
            "After cleanup: 3v0 doctor --ack shai-hulud-2026-05  to dismiss "
            "this warning.",
        ),
...
def _installed_version(pkg_name: str) -> Optional[str]:
    ...
    except ImportError:  # py<3.8 — 3V0 requires 3.10+ but defensive.
        return None
    try:
...
def short_banner_lines(hits: list[AdvisoryHit]) -> list[str]:
    ...
    lines = [
        ...
        "  Run '3v0 doctor' for remediation steps.",
    ]
    if len(hits) > 1:
...
# ``~/.3V0/cache/advisory_banner_seen`` (a single line per advisory ID:
...
def render_doctor_section(hits: list[AdvisoryHit]) -> tuple[bool, list[str]]:
    """Render the security-advisory section for ``3v0 doctor``.
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active "
            ...
            f"Run `3v0 doctor` on the gateway host for details.")
