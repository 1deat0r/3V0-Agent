...
remains ``3v0 cron`` / the ``cronjob`` tool, which runs in isolated
...
def _get_session_db() -> Optional[Any]:
    # Reuse the goals module's per-EV0_HOME cached SessionDB so both
    # features share one connection instead of thrashing the file.
    try:
        from threev0_cli.goals import _get_session_db as _goals_db
