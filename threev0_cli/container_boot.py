...
``$EV0_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-3v0-setup
(the stage2 hook) has chowned the volume and seeded $EV0_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$EV0_HOME``,
    not under ``profiles/``). The dispatcher in ``threev0_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``3v0 gateway start`` (no ``-p``) targets. Without it,
    bare ``3v0 gateway start`` inside the container would land on
    ...
    ``$EV0_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        ev0_home: The container's EV0_HOME (typically /opt/data).
            Profiles live under ``<ev0_home>/profiles/<name>/``;
            the default profile lives at ``<ev0_home>`` itself.
    ...
    # ``3v0 gateway start`` (no ``-p``) has somewhere to land;
    ...
    default_prior_state = legacy_default_state or _read_desired_state(ev0_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(ev0_home)
    ...
    actions.append(ReconcileAction(
        ...
        prior_exit=_read_prior_exit_label(ev0_home),
    ))

    profiles_root = ev0_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `3v0 profile create` (config.yaml
            # is not — that comes later via `3v0 setup`). Use it as the
            ...
            # via ``3v0 -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(ev0_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    """
    state_file = ev0_home / "gateway_state.json"
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off the container argv, leaving the 3v0 args.

    Two container-command argv shapes are handled:

    * **s6-overlay v2 / tini:** PID 1 argv is
      ``/init /opt/3v0/docker/main-wrapper.sh <subcommand> [args...]``.
    * **s6-overlay v3:** PID 1 is ``s6-svscan`` and the command lives on the
      rc.init-launched process as ``/bin/sh -e
      /run/s6/basedir/scripts/rc.init top /opt/3v0/docker/main-wrapper.sh
    ...
    ``3v0`` invocations carry no wrapper, so fall back to peeling a bare
    ``init`` prefix. The wrapper re-execs ``3v0 <subcommand>``, so an
    explicit leading ``3v0`` is peeled too. Shared by the legacy-gateway
    ...
    # The wrapper re-execs `3v0 <subcommand>`; peel an explicit 3v0.
    if args and Path(args[0]).name == "3v0":
        args = args[1:]
    return args
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``3v0 dashboard ...``) never spawns or
    ...
    the gateway and dashboard containers share a bind-mounted EV0_HOME,
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `3v0 -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit 3v0
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the 3v0 user) won't hit EACCES. See
...
def _write_reconcile_log(
    ev0_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $EV0_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    # bind-mounted EV0_HOME, both race to flock() the same s6-log lock
    ...
    actions = reconcile_profile_gateways(
        ev0_home=ev0_home, scandir=scandir,
    )
    for a in actions:
        print(
