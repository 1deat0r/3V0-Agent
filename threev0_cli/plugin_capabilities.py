...
this. Capabilities govern the *host API surfaces* 3V0 hands out (which
...
until the user re-consents (``3v0 plugins update`` surfaces the diff).
...
def parse_declared_capabilities(raw: Any, plugin_name: str = "?") -> List[str]:
    ...
    for a newer 3V0 may declare ids this build doesn't know; they can
