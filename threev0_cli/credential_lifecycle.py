"""Unified provider-credential lifecycle across every store 3V0 reads.

A provider API key can live in up to THREE stores at once:

    1. ``~/.3V0/.env``                     — the canonical secret store
    2. ``~/.3V0/auth.json`` →
       ``credential_pool.<provider>[*]``      — env-seeded pool entries
       (``source == "env:<VAR>"``) persisted by the pool loader
    3. ``~/.3V0/config.yaml``              — inline mirrors written by the
...
def purge_env_credential_references(
    ...
    # Make the removal sticky the same way `3v0 auth remove` does: a
...
def save_provider_env_credential(env_var: str, value: str) -> Dict[str, Any]:
    ...
    re-add through the UI behaves like ``3v0 auth add``.
    """
    from threev0_cli.config import load_env, save_env_value
    ...
    # that reads this var.
    try:
        from threev0_cli.auth import unsuppress_credential_source
...
def remove_provider_env_credential(env_var: str) -> Dict[str, Any]:
    ...
    """
    from threev0_cli.config import load_env, remove_env_value
