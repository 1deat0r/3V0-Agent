"""Bounded product contract for the first 3V0 shared-metrics slice."""
...
SCHEMA_KEY = "3v0.metrics.schema_version"
SCHEMA_VERSION = "3v0.metrics.event.v2"
MODEL_CALL_SCOPE = "3v0.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "3v0.task_run"
TOOL_CALL_SCOPE = "3v0.tool_call"
CLIENT_ACTIVE_MARK = "3v0.client.active"
TOOL_APPROVAL_MARK = "3v0.tool_approval"
SKILL_LIFECYCLE_MARK = "3v0.skill.lifecycle"
SKILL_LOAD_MARK = "3v0.skill.load"
SUBSCRIBER_NAME = "3v0.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "3v0.client.active"
LEGACY_MODEL_CALL_METRIC = "3v0.model_call.count"
MODEL_ROUTE_METRIC = "3v0.model_route.count"
TASK_STARTED_METRIC = "3v0.task_run.started"
TASK_FINISHED_METRIC = "3v0.task_run.finished"
TOOL_CALL_METRIC = "3v0.tool_call.count"
TOOL_APPROVAL_METRIC = "3v0.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "3v0.skill.lifecycle.count"
SKILL_LOAD_METRIC = "3v0.skill.load.count"
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted 3V0 installation method."""
    normalized = str(value or "").strip().lower()
    if normalized == "nix":
        return "nixos"
...
def client_resource(
    ...
    *,
    ...
    return {
        ...
        "ev0_version": normalized_version,
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("ev0_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its 3V0 logical scope."""
    metadata = getattr(event, "metadata", None)
    if (
    ...
    if relay_metadata - {"otel.status_code"} or metadata.get(
        ...
        return None
    call_role = metadata.get("3v0.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from threev0_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map 3V0 terminal state to bounded task outcome dimensions."""
...
def tool_category(kwargs: dict[str, Any]) -> str:
    """Map 3V0 registry toolset metadata to a low-cardinality category."""
    ...
    if toolset.startswith(
        ("discord", "email", "feishu", "3v0-yuanbao", "slack", "sms")
    ):
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal 3V0 tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to 3V0."""
