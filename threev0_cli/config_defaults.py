"""Default configuration data for 3V0 Agent.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from threev0_cli/config.py. Must not import from threev0_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["3v0-cli"],
    # SQLite journal mode used by every 3V0 database opener. WAL is the
    ...
    # Soft file-descriptor limit for long-running 3V0 server processes.
    ...
    "session": {
        # Per-terminal `3v0 -c`: each CLI session drops a breadcrumb file
        # under $EV0_HOME/terminal-sessions/<terminal-id>, and a bare
    ...
    "agent": {
        ...
        # an interactive `3v0 gateway restart` must never block for hours
        ...
        # the 3V0-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps 3V0
        ...
        # identity slot (SOUL.md). Empty by default. The EV0_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, 3V0 adds a coding operating brief
        ...
        # `3v0 status` / fleet monitoring). Retries never stop — this is a
        ...
        # ``EV0_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        #             EV0_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force EV0_HOME/home when it exists (old strict
        ...
        # skipped. When empty, 3V0 auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), 3V0 sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when 3V0
        ...
        #  "/home/user/.3V0/cache/documents:/output"]
        ...
        # bundled 3V0 image, which drops to the `3v0` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, 3V0 sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and 3V0 should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``3v0 chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.3V0/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: 3v0 sweeps the checkpoint base at startup
        ...
        # ``3v0 checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .3v0.md, or .cursorrules before 3V0 applies
    ...
    # Single-query (``3v0 -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    # single read_file page exceeds these limits, 3V0 truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so 3V0' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); 3v0 =
                                      # 3V0' compression threshold triggers
        ...
        "codex_responses_native": False,  # Opt in to OpenAI's server-side compaction
                                      ...
                                      # route/model is unaffected. 3V0' local
        ...
        "codex_responses_compact_threshold": 200000,  # Server-side compaction trigger
                                      ...
                                      # the server compacts before 3V0 does.
    ...
    "auxiliary": {
        ...
        # Invoked by ``3v0 kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``3v0 kanban decompose`` and the kanban
        ...
        # ``3v0 profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `3v0 model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `3v0` (and `3v0 chat`) launches by default:
        ...
        # REPL and `--tui` (or EV0_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `3v0 --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `3v0 -c` muscle memory.  Default off so existing
        # users aren't surprised.  EV0_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `3v0 --tui` drops a one-time hint
        ...
        # model — see threev0_cli/focus_view.py.
        "focus_view": False,
        ...
        # can set `path`/`paths` to EV0_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/3v0`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `3v0 pets`. Disabled until
        ...
        "pet": {
            ...
            # get_ev0_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``EV0_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``EV0_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``EV0_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``EV0_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``EV0_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/3v0``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; 3V0 appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.3V0/cache/piper-voices/
    ...
    # "Hey 3V0" hands-free wake word. Always-on, on-device hotword
    ...
    "wake_word": {
        ...
        "phrase": "hey 3v0",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
        ...
        "openwakeword": {
            # "hey_3v0" (the bundled, works-out-of-the-box default) OR a
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.3V0/cache/delegation/
    ...
    # response. If not, 3V0 feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before 3V0 auto-pauses the goal and
    ...
    "moa": {
        ...
        # <ev0_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.3V0/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${EV0_SKILL_DIR} and ${EV0_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `3v0 curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `3v0 curator run --consolidate` overrides
        ...
        # normally restored on every `3v0 update`, so pruning them only
        ...
        # skipped), snapshot ~/.3V0/skills/ into
        # ~/.3V0/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `3v0 curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for 3v0-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *3V0 Agent*" header.
    ...
    # dangerous command. -q runs export EV0_INTERACTIVE=1 (for interactive
    ...
    "approvals": {
        ...
        # modal; EV0_TUI_NO_CONFIRM=1 force-skips that modal regardless of
    ...
    # replace 3V0' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.3V0/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or EV0_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # .cursorrules, project-local .3V0 config) always require human
        ...
        # longer trigger the startup banner. Add via `3v0 doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``threev0_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow 3V0 to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $EV0_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via EV0_CRON_MAX_PARALLEL env var.
        ...
        # EV0_CRON_SCRIPT_TIMEOUT. Keep this in sync with
        ...
        # EV0_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
    ...
    # todos to ready, and fires `3v0 -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `3v0` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `3v0 kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     3v0-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core 3V0 tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.3V0/logs/.
    ...
    # update model picker lists without shipping a 3v0-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://github.com/1deat0r/3V0-Agent/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `3v0 model` invocation; network failures
    ...
    # #8731).
    #
    # Provider keys accept the 3V0 provider id (as used elsewhere in
    ...
    "gateway": {
        ...
        # internal EV0_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.3V0/sessions/sessions.json entirely.
        ...
        # the EV0_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.3v0.gateway` that defenses 1-2 don't
        ...
        # `3v0 update` hang because it can never drain the gateway.
        "restart_loop_guard": {
            "max_restarts": 3,
            "window_seconds": 60,
            "max_gap_seconds": 300,
        ...
        # ``EV0_GATEWAY_MAX_STARTS`` / ``EV0_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.3V0/.env,
        ...
        # behavior: files must live under the 3V0 cache, under
        ...
        # user. Bridged to EV0_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the 3V0
        # cache (~/.3V0/cache/{documents,images,audio,video,screenshots})
        ...
        # to EV0_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # EV0_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to EV0_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.3V0/state.db.
    ...
    "sessions": {
        ...
        # the default of ``3v0 sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.3V0/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `3v0 sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `3v0 update` prints a one-line notice with
        ...
        # ~/.3V0/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to EV0_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # EV0_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
        ...
        # (non-streaming) export such as `3v0 sessions export`. Checked
    ...
    # ``3v0 doctor`` behaviour.
    "doctor": {
        # Per-probe timeout (seconds) for the opt-in `3v0 doctor --live`
        # real-call backend probes (Firecrawl/FAL/browser/MCP/TTS/STT).
        "live_probe_timeout": 10,
    },

    # ``3v0 update`` behaviour.
    "updates": {
        ...
        #     DBs) into <EV0_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``3v0 backup``-style zip
        #     of EV0_HOME into <EV0_HOME>/backups/, restorable with
        #     ``3v0 import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `3v0 update` does with uncommitted local changes to the
    ...
    # `3v0 tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.3V0/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.3V0/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.3V0/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, 3V0 may use this encrypted
            ...
            # ~/.3V0/bin/ on first use.  When False you must install
        ...
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.3V0/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. 3V0 disables it for our users unless they
        # explicitly opt in here. When false (default), 3V0 sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; 3V0 also calls
        ...
        "no_overlay": None,
        # cua-driver permission mode for each 3V0 computer-use runtime.
        ...
        # manifest used when permission_mode is "bounded". 3V0 passes the
    ...
    #
    # Configure with `3v0 egress setup`.  Disabled by default — the rest of
    # 3V0 works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.3V0/bin/ on
    ...
    # 3V0 Desktop (Electron app) launch options. These only affect
    # `3v0 desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the EV0_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
        ...
        # Ignored on macOS/Windows. Bridged to the EV0_DESKTOP_PASSWORD_STORE
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials 3V0 mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via '3v0 model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the 3V0 auth store)",
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey 3V0' wake word engine (optional; openWakeWord is the free default)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use 3V0, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @3v0:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. EV0_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #3v0)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: 3v0-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or '3v0-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote 3V0 API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote 3V0 API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote 3V0 API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # EV0_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to EV0_TOOL_PROGRESS_MODE for backward
    ...
    # EV0_TOOL_PROGRESS is fully unsupported since the v12 config support
    # floor retired its only consumer (the v3→4 migration).
    "EV0_PREFILL_MESSAGES_FILE": {
