...
def _current_profile_cli_args() -> list[str]:
    ...
    from threev0_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "threev0_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def get_task_name() -> str:
    ...
    """
    _assert_windows()
    # Local import to avoid circular module initialization during threev0_cli boot.
    from threev0_cli.gateway import _profile_suffix
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - invokes ``python -m threev0_cli.main [--profile X] gateway run``
    ...
    prog_args = [python_exe_path, "-m", "threev0_cli.main"]
    if profile_arg:
...
def windowless_gateway_restart_spec(
    ...
    preserves every argument after the interpreter (``-m threev0_cli.main
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``python.exe -m threev0_cli.main gateway run`` directly — NOT
...
def _prompt_install_choices(
    ...
    from threev0_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    from threev0_cli.gateway import find_gateway_pids, _profile_arg
