"""``3v0 verify`` — detect a project's run recipe and smoke-test it.
...
``.3V0/environment.json``), then runs bootstrap/build/test phases and an
...
(:mod:`agent.verification_evidence`), so a passing ``3v0 verify`` satisfies
