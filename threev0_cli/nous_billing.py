...
billing screens drive. Companion to ``threev0_cli/nous_account.py`` (which owns
...
- **Auth** = the OAuth bearer JWT 3V0 already holds for inference
...
- **Portal base URL** resolves with the same precedence as the device-flow login
  (``auth.py``): ``EV0_PORTAL_BASE_URL`` → ``NOUS_PORTAL_BASE_URL`` → the
...
# threev0_cli.auth.NOUS_BILLING_MANAGE_SCOPE (kept here too so this module has no
...
def _billing_not_logged_in(exc: Optional[BaseException] = None) -> "BillingAuthError":
    ...
    err = BillingAuthError(
        "Not logged into Nous Portal — run `3v0 portal` to log in.",
        status=401,
        error="invalid_token",
    )
    if exc is not None:
