"""3v0 hooks — inspect and manage shell-script hooks.

Usage::

    3v0 hooks list
    3v0 hooks test <event> [--for-tool X] [--payload-file F]
    3v0 hooks revoke <command>
    3v0 hooks doctor

Consent records live under ``~/.3V0/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.3V0/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``3v0 hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: 3v0 hooks {list|test|revoke|doctor}")
        print("Run '3v0 hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs and not outbound:
        print("No shell hooks or outbound webhooks configured in ~/.3V0/config.yaml.")
        print("See `3v0 hooks --help` or")
        print("    website/docs/user-guide/features/hooks.md")
        print("for the config schema and worked examples.")
        return

    if not specs:
        print("No shell hooks configured in ~/.3V0/config.yaml.")
    else:
                    ...
                    if entry and entry.get("approved_at"):
                        ...
                        if mtime_now and mtime_at and mtime_now > mtime_at:
                            print(
                                ...
                                f"run `3v0 hooks doctor` to re-validate"
                            )
            print()
...
# stdin a script sees under `3v0 hooks test` and `3v0 hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (3V0 wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `3v0 hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `3v0 hooks doctor` would execute
