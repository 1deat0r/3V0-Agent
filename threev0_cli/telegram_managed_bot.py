...
manual BotFather token copy-paste. 3V0 talks only to the Nous onboarding
...
DEFAULT_API_URL = "https://setup.github.com/1deat0r/3V0-Agent"
...
DEFAULT_MANAGER_BOT = "Ev0SetupBot"

DEFAULT_BOT_NAME = "3V0 Agent"
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``ev0_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    """Generate a secure suggested bot username like ``ev0_<slug>_bot``.
    ...
    """
    _ = profile_name
    return f"ev0_{generate_username_slug()}_bot"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting 3V0 Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the 3V0 Telegram onboarding service.")
