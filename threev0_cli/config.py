...
from threev0_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``threev0_cli/`` — i.e. the git checkout for source
...
def _running_in_container() -> bool:
    """Thin wrapper around ``threev0_constants.is_container`` (import-safe)."""
    try:
...
# threev0_cli/main.py; lives here so the wording stays consistent and we
...
from threev0_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``threev0_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (threev0_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (threev0_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (threev0_cli/setup.py)
    "known_plugin_toolsets", # written/read by threev0_cli/tools_config.py toolset-save flow
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if floor_refused:
        ...
        # The per-version steps live in threev0_cli.config_migrations as a
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from threev0_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    try:
        ...
        from threev0_cli.toolset_validation import validate_platform_toolsets
...
# Back-compat alias — canonical set lives in threev0_cli.personality.
from threev0_cli.personality import NEUTRAL_PERSONALITY_NAMES as _NEUTRAL_PERSONALITY_NAMES  # noqa: F401


def _prompt_text(value: Any) -> str:
    """Normalize config prompt values from YAML before handing them to AIAgent.

    Delegates to :mod:`threev0_cli.personality` — the single owner of
    personality/overlay semantics. Kept as a re-export for existing importers.
    """
    from threev0_cli.personality import prompt_text
...
def render_personality_prompt(value: Any) -> str:
    ...
    from threev0_cli.personality import render_personality_prompt as _render
...
def resolve_ephemeral_system_prompt_from_config(cfg: Optional[Dict[str, Any]]) -> str:
    ...
    Delegates to :mod:`threev0_cli.personality` (single owner).
    """
    from threev0_cli.personality import resolve_ephemeral_system_prompt
...
def show_config():
    ...
    from threev0_cli.auth import get_anthropic_key
...
_SCHEMA_DEFINED_DICT_KEYS = frozenset({
    ...
    # (threev0_cli/plugins_cmd.py, threev0_cli/plugin_index.py). Absent from
