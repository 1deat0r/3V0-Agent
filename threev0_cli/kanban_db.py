...
``<root>`` is the **shared 3V0 root** (the parent of any active
...
``3v0 -p <profile>`` joins the same board as the dispatcher that
...
the profile's 3V0 home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``EV0_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``EV0_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``3v0 kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.3V0``. In Docker / custom
deployments where ``EV0_HOME`` points outside ``~/.3V0`` (e.g.
``/opt/3v0``), ``<root>`` is ``EV0_HOME``. Legacy env-var
overrides still work:

* ``EV0_KANBAN_DB`` — pin the database file path directly.
* ``EV0_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``EV0_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``EV0_KANBAN_DB``,
``EV0_KANBAN_WORKSPACES_ROOT``, and ``EV0_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/3v0-kanban-v1-spec.pdf`` for the full
...
from threev0_cli.sqlite_util import add_column_if_missing as _add_column_if_missing
...
def normalize_reasoning_effort(effort: Optional[str]) -> Optional[str]:
    ...
    Accepts any level in ``threev0_constants.VALID_REASONING_EFFORTS`` plus
    ...
    """
    from threev0_constants import VALID_REASONING_EFFORTS
...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    ...
    ``profile_name`` is resolved from the active EV0_HOME so dispatcher- and
    worker-side hooks both carry the right profile without the caller plumbing
    it through.
    """
    try:
        from threev0_cli.lifecycle import invoke_hook
...
# ``EV0_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``EV0_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``EV0_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``EV0_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "ev0_kanban_current_board_override",
    default=None,
)
...
# enough that kebab-case names like ``atm10-server`` or ``3v0-agent``
...
def kanban_home() -> Path:
    """Return the shared 3V0 root that anchors the kanban board.

    Resolution order:

    1. ``EV0_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_ev0_root()``, which already returns ``<root>``
       when ``EV0_HOME`` is ``<root>/profiles/<name>``, and returns
       ``EV0_HOME`` directly for Docker / custom deployments.
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``3v0 kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``EV0_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``3v0 kanban boards
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``3v0 kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``EV0_KANBAN_DB`` env var — pins the path directly. Honoured for
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``EV0_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``EV0_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``3v0 kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # ``threev0_constants.VALID_REASONING_EFFORTS``, or ``"none"`` for thinking
    ...
    # within an agent loop that propagated ``EV0_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    workspace_path       TEXT,
    branch_name          TEXT,
    -- Optional link to a first-class Project (threev0_cli/projects_db). When set,
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``EV0_SESSION_ID``. NULL
...
def _sqlite_connect(path: Path) -> sqlite3.Connection:
    ...
    advisory locks on the database (see ``threev0_cli.sqlite_safe_read``).
    The registration is released automatically when the connection closes.
    """
    from threev0_cli.sqlite_safe_read import connect_tracked
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``3v0 gateway run --replace`` /
...
def _validate_sqlite_header(path: Path) -> None:
    ...
    from threev0_cli.sqlite_safe_read import read_header_bytes_preopen
...
def _backup_corrupt_db(path: Path) -> Optional[Path]:
    ...
    # see threev0_cli.sqlite_safe_read), so it must only run once the board has
    ...
    # rescue.
    from threev0_cli.sqlite_safe_read import has_live_connection
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    **Narrow auto-repair:** when the integrity failure consists *only* of
    ...
    all sources 3V0 treats as user-controlled-but-trusted on the
...
def repair_db(
    ...
    *,
    ...
    :class:`RepairResult` so ``3v0 kanban repair`` can report and choose
    its own exit code.
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``EV0_KANBAN_DB`` env → ``EV0_KANBAN_BOARD`` env →
    ...
    # holder (e.g. an external `3v0 kanban list` mid-integrity-probe) block
    ...
    with _cross_process_init_lock(path):
        ...
        from threev0_state import preflight_db_writability
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See threev0_state._WAL_INCOMPAT_MARKERS for detection logic.
                from threev0_state import apply_wal_with_fallback
                apply_wal_with_fallback(conn, db_label=f"kanban.db ({path.name})")
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``3v0 kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``EV0_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _check_file_length_invariant(conn: sqlite3.Connection) -> None:
    ...
    """
    from threev0_cli.sqlite_safe_read import file_length_matches_header
...
def write_txn(conn: sqlite3.Connection, *, allow_nested: bool = False):
    ...
    if getattr(conn, "in_transaction", False):
        ...
        savepoint = f"ev0_nested_{secrets.token_hex(8)}"
...
def create_task(
    ...
    *,
    ...
    each name to ``3v0 --skills ...``. Use this to pin a task to a
    ...
    if project_id:
        from threev0_cli import projects_db as _pdb
    ...
    # `3v0 --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``3v0 kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `3v0 kanban recompute` (issue #22459).
...
def store_attachment_bytes(
    ...
    *,
    ...
    (``3v0 kanban attach``) so name-sanitisation, the size cap, and the
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``3v0 kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``3v0 kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``3v0 kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``3v0 kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``EV0_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold 3V0' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside 3V0-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``3v0 kanban show <id>`` and the dashboard.
...
def _resolve_worktree_workspace(
    ...
    launched from, e.g. the 3V0 checkout). If no anchor is configured
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. If ``workspace_path`` names
      a repo root, 3V0 treats it as an anchor and materializes a linked
      worktree at ``<repo>/.worktrees/<task-id>``. If ``workspace_path`` names
      a concrete target path, 3V0 creates/reuses that linked worktree. With
      no ``workspace_path``, 3V0 anchors on the board's ``default_workdir``
      ...
      dispatcher's CWD. When ``branch_name`` is empty, 3V0 uses
      ``wt/<task-id>``.
...
# without thrashing. Overridable via ``EV0_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a 3V0
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    event so the hold is visible in ``3v0 kanban tail``. The next dispatch
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``3v0 kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real 3V0 profile.
    ...
    if not rows:
        return False
    try:
        from threev0_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def review_dispatch_enabled() -> bool:
    ...
    The default is true because 3V0 ships the ``sdlc-review`` skill and the
...
def _dispatch_once_locked(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from threev0_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real 3V0 profile.
        # `_default_spawn` invokes ``3v0 -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a 3V0
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `3v0 kanban tail` — without
    ...
    # Auto-dispatch is enabled by default because 3V0 bundles the
    ...
    for row in review_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        try:
            from threev0_cli.profiles import profile_exists
        except Exception:
            profile_exists = None  # type: ignore[assignment]
...
def _resolve_ev0_argv() -> list[str]:
    ...
    1. ``$EV0_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("3v0")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\3v0.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m threev0_cli.main`` — fallback for setups where
       3V0 is launched from a venv and the ``3v0`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_ev0_bin`` for the same reason. Kept
    local (not imported from gateway) because ``threev0_cli`` sits below
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``3v0 -p <profile> chat -q ...`` subprocess.
    ...
    ``EV0_KANBAN_DB`` / ``EV0_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject EV0_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `3v0 -p <name>` the
    # _apply_profile_override() runs *before* threev0_constants is imported.
    # If EV0_HOME is absent from the child's env, get_ev0_home() falls
    # back to Path.home() / ".3V0" (the DEFAULT profile root), ignoring the
    ...
    try:
        ...
        # EV0_HOME never had profiles created.
        pass
    if task.tenant:
        env["EV0_TENANT"] = task.tenant
    env["EV0_KANBAN_TASK"] = task.id
    env["EV0_KANBAN_WORKSPACE"] = workspace
    # Tag the worker's session so it lands in state.db as `kanban`, not as an
    # untitled `cli` row. A worker is a dispatcher-owned run whose transcript is
    # read on the board and in `3v0 kanban log` — it is not a conversation
    ...
    if task.branch_name:
        env["EV0_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["EV0_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["EV0_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["EV0_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["EV0_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # that even when the worker activates a profile (`3v0 -p <name>`
    # rewrites EV0_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_ev0_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["EV0_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["EV0_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    _retag_legacy_worker_sessions(env["EV0_KANBAN_WORKSPACES_ROOT"])
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["EV0_KANBAN_BOARD"] = resolved_board
    # EV0_PROFILE is the author the kanban_comment tool defaults to.
    # `3v0 -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["EV0_PROFILE"] = profile_arg

    # A worker must NEVER boot the interactive TUI: an inherited EV0_TUI=1
    ...
    # older 3v0 builds on PATH that predate the flag's precedence.
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("EV0_HOME"))
    if worker_toolsets:
    ...
    # `3v0 kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`3v0` executable not found on PATH. "
            "Install 3V0 Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``3v0 kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled EV0_PROFILE values like "3v0-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default 3V0 root exists

    Reads profile paths directly so this module has no import dependency on
    ``threev0_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from threev0_constants import get_default_ev0_root
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``3v0 kanban assignees`` for the terminal.
