"""Implementation of the ``3v0 bundles`` CLI subcommand.

Mirrors the structure of ``threev0_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `3v0 bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  3v0 bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``3v0 bundles`` argparse tree.

    Called from ``threev0_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `3v0 bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``3v0 bundles <subcommand>`` to the right handler."""
