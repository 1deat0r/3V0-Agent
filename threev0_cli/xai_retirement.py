...
Pure logic: walks a 3V0 config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `3v0 doctor` and a future `3v0 migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a 3V0 config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a 3V0 config and return retirement issues.
...
def apply_migration(
    ...
    from threev0_cli.config import require_readable_config_before_write
    ...
    # None``, so the next `3v0 migrate xai` reports nothing to migrate
