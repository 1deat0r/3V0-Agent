...
# httpx is imported lazily: it costs ~30ms at import time and threev0_cli.auth
...
if TYPE_CHECKING:
    ...
    class _LazyHttpx:
        ...
        # ("threev0_cli.auth.httpx.Client", ...) keeps working in tests.
...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from threev0_cli import __version__ as _EV0_CLI_VERSION
...
def get_anthropic_key() -> str:
    ...
    """
    from threev0_cli.config import get_env_value_prefer_dotenv
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from threev0_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
...
def _format_nous_entitlement_auth_error(error: AuthError) -> str:
    try:
        from threev0_cli.nous_account import (
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    if pconfig is None:
        from threev0_cli.providers import get_provider
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from threev0_cli.config import validate_config_structure
...
def _nous_shared_auth_dir() -> Path:
    ...
    :func:`threev0_constants.get_default_ev0_root` returns — so
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    try:
        from threev0_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
...
def _update_config_for_provider(
    ...
    # model.api_key.
    from threev0_cli.config import clear_model_endpoint_credentials
...
def _confirm_selection_guards(
    ...
    *,
    ...
    :mod:`threev0_cli.model_selection_guards` and shows one [y/N] confirm with
...
def _prompt_model_selection(
    ...
    def _confirmed_selection(mid: str) -> Optional[str]:
        if not mid:
            return None
        # Unified guard registry (threev0_cli.model_selection_guards): the cost
    ...
    try:
        ...
        from threev0_cli.model_search import model_search_text
    ...
    from threev0_cli.curses_ui import format_radio_item_ansi
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from threev0_cli.config import save_config, load_config
