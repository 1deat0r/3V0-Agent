...
def format_nous_portal_entitlement_message(
    ...
    *,
    ...
    if account_info is None:
        return (
            f"3V0 could not verify your Nous Portal entitlement, so {capability} "
            f"is unavailable. Run `3v0 model` to refresh your login, or check "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"Nous inference credentials are configured, but 3V0 cannot verify "
                f"your Nous Portal paid access for {capability}. Log in with "
                f"`3v0 model` to enable Portal-managed features. Billing and "
                f"credits are managed at {billing_url}."
            )
        return (
            f"Log in to Nous Portal to use {capability}: run `3v0 model`. "
            f"Billing and credits are managed at {billing_url}."
        )

    if account_info.paid_service_access is None:
        detail = (
            f"3V0 could not verify your Nous Portal paid access, so {capability} "
            f"is unavailable."
        )
        if account_info.error:
            detail += f" Account lookup failed: {account_info.error}."
        if include_refresh_hint:
            detail += " Run `3v0 model` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            f"3V0 could not find a Nous Portal account or organisation for this "
            f"login, so {capability} is unavailable. Run `3v0 model` to "
    ...
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `3v0 model` to refresh 3V0."
        return message
...
def _no_paid_access_message(
    ...
    if access and access.member_spend_cap_exceeded:
        ...
        return (
            ...
            f" member spend cap at {billing_url}, then run `3v0 model`"
            f" to refresh."
        )
...
def _fresh_account_info(
    *,
    ...
    try:
        from threev0_cli.auth import get_provider_auth_state, resolve_nous_access_token
...
def _info_from_valid_jwt(
    ...
    *,
    ...
    try:
        from threev0_cli.auth import _decode_jwt_claims
    except Exception:
        return None
