...
# several hot modules (cli.py, threev0_cli/main.py, run_agent.py,
...
def _known_threev0_env_keys() -> set[str]:
    ...
    """
    from threev0_cli.config import _EXTRA_ENV_KEYS
    from threev0_cli.config_defaults import OPTIONAL_ENV_VARS
...
def _clear_known_keys_missing_from_dotenv(path: Path) -> None:
    ...
    ``tests/threev0_cli/test_dump_env_visibility.py``), and a startup scrub
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    ``threev0_cli.config._sanitize_env_lines`` normalizes line endings while
    treating content after the first ``=`` as opaque for boundary discovery.
    """
    if not path.exists():
        return
    try:
        from threev0_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_threev0_dotenv(
    *,
    ...
    from threev0_cli import _early_recovery
...
def _reapply_terminal_config_bridge(home_path: Path) -> None:
    ...
    Delegates to ``threev0_cli.config.apply_terminal_config_to_env`` — the
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    time from several hot modules (cli.py, threev0_cli/main.py,
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    # shared cache here lets main.py's early bridge and threev0_logging reuse
