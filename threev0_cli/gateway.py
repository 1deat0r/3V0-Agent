"""
Gateway subcommand for 3v0 CLI.

Handles: 3v0 gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "3v0 CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
# — restart may fail", and `3v0 update` can deadlock behind it.  The loop
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``3v0 gateway status`` from falsely counting the ``3v0`` CLI that
...
def _scan_gateway_pids(
    ...
    # scan (e.g. ``3v0 gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"ev0_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. EV0_HOME
        ...
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "ev0_home=" in command_lc
            and f"ev0_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
    ...
    try:
        if is_windows():
            ...
            # wedges the caller forever. ``3v0 update`` hung exactly there
            ...
            # watchdog probe.
            from threev0_cli._subprocess_compat import bounded_probe_run
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``3v0 update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            3V0 profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to 3V0 profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "threev0_cli.main"]
    if profile != "default":
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m threev0_cli.main gateway
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``3v0 update``.

    A gateway started with ``--external-supervisor`` must exit back to that
    manager. Starting 3V0's detached watcher as well would escape the
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose EV0_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``3v0 update`` users
    # with no running gateway until they re-invoke ``3v0 gateway``
    ...
    from threev0_cli._subprocess_compat import (
    ...
    # interpreter and captures the stable cwd + env overlay (EV0_HOME,
    ...
    if sys.platform == "win32":
        try:
            from threev0_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / EV0_HOME) the windowless
        # base interpreter needs to import threev0_cli.  Empty on POSIX, where
...
def _sync_threev0_home_from_systemd_unit(system: bool) -> None:
    ...
    :func:`get_ev0_home` falls back to ``/root/.3V0`` — the wrong
    profile. The unit file pins ``EV0_HOME`` for the actual gateway
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}3v0 gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}3v0 gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from threev0_cli.service_manager import detect_service_manager, get_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print("  Stop it with: 3v0 gateway stop")
    else:
        print(
            "⚠ Gateway process is running for this profile, but the service is not active"
        )
        print(f"  PID(s): {_format_gateway_pids(snapshot.gateway_pids, limit=None)}")
        print("  This is usually a manual foreground/tmux/nohup run, so `3v0 gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``3v0 gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from threev0_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans(extra_exclude: set | None = None) -> bool:
    ...
    in-process under a ``gateway restart`` argv (threev0_cli/gateway.py restart
    ...
    if is_windows():
        try:
            # The install-time task name is profile-aware (Ev0_Gateway /
            # Ev0_Gateway_<profile>) — never hardcode it, or the guard is
            # dormant on every standard `3v0 gateway install` deployment.
            from threev0_cli.gateway_windows import get_task_name

            _task_name = get_task_name()
        except Exception:
            _task_name = "Ev0_Gateway"
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (EV0_HOME-scoped).
...
from threev0_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our 3V0 Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``3v0 gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``EV0_GATEWAY_DETACHED=1``; older wrappers without the env marker are
...
def _windows_gateway_breakaway_state() -> bool | None:
    ...
    if not is_windows():
        return None
    from threev0_cli._subprocess_compat import _WINDOWS_GATEWAY_BREAKAWAY_ENV
...
_SERVICE_BASE = "3v0-gateway"
SERVICE_DESCRIPTION = "3V0 Agent Gateway - Messaging Platform Integration"

# 3V0 runs its own gateway service under its own name — the generic
# 3v0-gateway-<profile> unit for this profile was retired on migration to
# the native runtime (replaced by 3v0-gateway.service).
_SERVICE_NAME_OVERRIDES = {"3v0": "3v0-gateway"}


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current EV0_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (EV0_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary EV0_HOME paths
...
def _profile_arg(threev0_home: str | None = None, default_root: str | Path | None = None) -> str:
    ...
    Args:
        ev0_home: Optional explicit EV0_HOME path. Defaults to the current
            ``get_ev0_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
        default_root: Optional 3V0 root to compare against. Used when
            generating a system service for another user from a sudo/root
            process, where ``Path.home()`` and ``get_default_ev0_root()``
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this EV0_HOME.

    Default ``~/.3V0`` returns ``3v0-gateway`` (backward compatible).
    Profile ``~/.3V0/profiles/coder`` returns ``3v0-gateway-coder``.
    Any other EV0_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older 3V0 installs that predate the
# 3v0-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (3v0-gateway-*.service) and unrelated third-party
# "3v0" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("3v0.service",)

# ExecStart content markers that identify a unit as running our gateway.
# A legacy unit is only flagged when its file contains one of these.
_LEGACY_UNIT_EXECSTART_MARKERS: tuple[str, ...] = (
    "threev0_cli.main gateway",
    "threev0_cli/main.py gateway",
    "gateway/run.py",
    " 3v0 gateway ",
    "/3v0 gateway ",
)
...
def _find_legacy_threev0_units() -> list[tuple[str, Path, bool]]:
    ...
    different service name (e.g. ``3v0.service`` before the rename to
    ``3v0-gateway.service``). When both a legacy unit and the current
    ``3v0-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``3v0-gateway-coder.service`` and unrelated third-party
      ``3v0-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``3v0.service`` running an unrelated
...
def print_legacy_unit_warning() -> None:
    """Warn about legacy 3V0 gateway unit files if any are installed.
    ...
    print_info("  These run alongside the current 3v0-gateway service and")
...
def remove_legacy_threev0_units(
    ...
    """Stop, disable, and remove legacy 3V0 gateway unit files.

    Iterates over whatever ``_find_legacy_ev0_units()`` returns — which is
    ...
    print()
    print("Legacy 3V0 gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: 3v0 gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo 3v0 gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    3v0 gateway uninstall")
    print_info("    sudo 3v0 gateway uninstall --system")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: 3v0 gateway install"
            )
            return scope, False
...
def ensure_gateway_service(context: str = "setup") -> bool:
    ...
    The zero-decision service path used by ``3v0 setup`` (end of wizard)
    and ``3v0 import``: if this host supports a service manager and no
    ...
    ``3v0 gateway install --system`` explicitly (that path prompts and
    ...
    if is_container():
        ...
        print_info("   3v0 gateway run          # Run as container main process")
        print_info("")
    ...
    if not (supports_systemd or is_macos() or is_windows()):
        ...
        print_info("  Run the gateway in the foreground with: 3v0 gateway")
        return False
    ...
    try:
        ...
        print_info("  You can retry manually: 3v0 gateway install")
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode 3V0 often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.3V0`` → ``ai.3v0.gateway.plist`` (backward compatible).
    Profile ``~/.3V0/profiles/coder`` → ``ai.3v0.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.3v0.gateway-{suffix}" if suffix else "ai.3v0.gateway"
...
def get_python_path() -> str:
    ...
    if venv is not None:
        try:
            ...
            # threev0_constants cached from before this symbol existed. See
            # _reload_threev0_constants() in threev0_cli/managed_uv.py.
            from threev0_cli.managed_uv import _reload_threev0_constants

            venv_python_path = _reload_threev0_constants().venv_python_path
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.3V0/3v0-agent  -> /home/alice/.3V0/3v0-agent
      /opt/3v0                 -> /opt/3v0  (kept as-is)
...
def _threev0_home_for_target_user(target_home_dir: str) -> str:
    ...
    # different EV0_HOME into the generated service unit.
    current_default = Path.home() / ".3V0"
    target_default = Path(target_home_dir) / ".3V0"

    # Default ~/.3V0 → remap to target user's default
    if current_ev0 == current_default:
        return str(target_default)

    # Profile or subdir of ~/.3V0 → preserve the relative structure
    try:
        relative = current_ev0.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.3V0) — keep as-is
        return str(current_ev0)
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m threev0_cli.main``, so module
    ...
    dir, or a clone that ``3v0 update`` later relocates/removes — the path
    ...
    ``EV0_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if EV0_HOME
...
def _append_node_dir_for_service(
    ...
    The 3V0-managed Node under ``$EV0_HOME/node`` goes first when it
    ...
    *ev0_root* is the 3V0 home the unit will run against. System units
    ...
    # target 3V0 home provides managed Node, consulting the invoker's PATH
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if not system:
        ...
        # user's 3V0 home is known — probing here would stat the calling
    ...
    if system:
        ...
        # the remapped ev0_home, not the calling user's. Prepend — the
...
Group={group_name}
ExecStart={python_path} -m threev0_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}ExecStart={python_path} -m threev0_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="EV0_HOME={threev0_home}"
...
def systemd_unit_is_current(system: bool = False) -> bool:
    ...
    # pinned EV0_HOME is adopted before any compare/regenerate" at a single
    # site, so a future callsite cannot regress it by forgetting to pre-sync.
    #
    # Under ``sudo 3v0 gateway … --system``, EV0_HOME is often stripped
    # and falls back to ``/root/.3V0``. Adopting the unit's pinned home
    # first makes TimeoutStopSec / WorkingDirectory / EV0_HOME comparisons
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir EV0_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved EV0_HOME
    ...
    exported a throwaway ``EV0_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``3v0 gateway restart``
    with ``EV0_HOME=/tmp/3v0-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="EV0_HOME=..."`` lines and launchd
    ``<key>EV0_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'EV0_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>EV0_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp EV0_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: EV0_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported EV0_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # single EV0_HOME-sync chokepoint (adopts the unit's pinned home before
    ...
    # sandboxed by the test conftest (only EV0_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp EV0_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../ev0_test`` path into
    # ``Environment="EV0_HOME=..."``. Writing that to the developer's
    ...
    if not system and (
        ...
        or '/ev0_test"' in new_unit
        or "/ev0_test/" in new_unit
    ):
        return False

    # Structural variant of the same belt: refuse to bake ANY temp-dir
    # EV0_HOME into the unit (manual E2E homes like /tmp/3v0-e2e-NNN
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current 3V0 install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo 3v0 gateway uninstall --system")
    print_info("         3v0 gateway install")
    print_info("         3v0 gateway start")
...
def systemd_install(
    ...
    # Offer to remove legacy units (3v0.service from pre-rename installs)
    # before installing the new 3v0-gateway.service. If both remain, they
    ...
    # Existing system units already pin EV0_HOME; adopt it before any
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}3v0 gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}3v0 gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}3v0 gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_start(system: bool = False):
    ...
    # EV0_HOME sync happens inside refresh_systemd_unit_if_needed's
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `3v0 gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `3v0 gateway status` or logs for final state."
            )
            return
        _wait_for_systemd_service_restart(system=system, previous_pid=pid)
        return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}3v0 gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}3v0 gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}3v0 gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}3v0 gateway start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.3v0.gateway-{suffix}" if suffix else "ai.3v0.gateway"


# Cached launchd domain result — probing is cheap but should only run once per
# process invocation (each ``3v0 gateway start/stop/status`` call).
...
#      background process (the `nohup 3v0 gateway run` workaround). See #23387.
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m threev0_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active EV0_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "threev0_cli.main"]
...
def _timestamped_stderr_gateway_command(
    ...
    *,
    ...
    ``3v0 update`` sees the flag on the live grandchild argv and hands
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup 3v0 gateway run --replace`
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(
        f"  Try manually: nohup 3v0 gateway run --replace "
...
def generate_launchd_plist() -> str:
    ...
    # without this block every plist rewrite (e.g. `3v0 gateway start`)
    ...
    nofile_block = ""
    try:
        from threev0_cli.resource_limits import configured_nofile_soft_limit
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current 3V0 install"
    )
    return True
...
def launchd_stop():
    ...
    # `3v0 gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_restart():
    ...
    try:
        ...
        if pid is not None and probe_gateway_loop_liveness(pid) == GATEWAY_LOOP_WEDGED:
            ...
            # full drain budget only stalls the restart (and `3v0 update`
...
def launchd_status(deep: bool = False):
    ...
    # 3V0 PID tracking — may be a detached fallback process spawned when
    ...
    # launchd PID and the 3V0 PID file).
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current 3V0 install")
    else:
        print("⚠ Service definition is stale relative to the current 3V0 install")
        print("  Run: 3v0 gateway start")
    ...
    if service_listed:
        if launchd_pid is not None:
            ...
            if fallback_pid:
                ...
                print("  Cron jobs will fire. Stop with: 3v0 gateway stop")
            else:
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``EV0_S6_SUPERVISED_CHILD``.
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``3v0 gateway run`` is always a misconfiguration, so we
    ...
    try:
        ...
        from threev0_cli.profiles import normalize_profile_name
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``3v0 gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``3v0 gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other EV0_HOME roots.
    """
    ...
    print("  Use '3v0 gateway restart' to replace it,")
    print("  or '3v0 gateway stop' first.")
    print("  Or use '3v0 gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print_error(
        "Refusing to run the 3V0 gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the '3v0' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/3v0/docker/entrypoint.sh before the 3V0 command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$EV0_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set EV0_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `3v0 gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set EV0_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `3v0 gateway restart` which already
    ...
    # `3v0 gateway start/restart` — leaving the gateway vulnerable to
    ...
    # EV0_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    # the env vars ``EV0_GATEWAY_MAX_STARTS`` /
    # ``EV0_GATEWAY_START_WINDOW_S`` override for escape-hatch use.
    ...
    def _hard_exit_after_gateway_teardown(code: int) -> None:
        # ``3v0 gateway run`` enters through this CLI wrapper, not through
...
_PLATFORMS = [
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. 3v0) and copy the bot token",
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Channel ID where 3V0 delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. 3V0 connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: 3v0 pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and 3V0 will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``3v0 setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run 3v0 under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.3V0/plugins/ still require
...
def _setup_standard_platform(platform: dict):
    ...
    from threev0_cli.setup_hidden_env import is_setup_hidden_env as _is_setup_hidden_env
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from threev0_cli.telegram_managed_bot import (
            ...
            if value:
                ...
                if is_email:
                    ...
                    access_choices = [
                        ...
                        "Use DM pairing (unknown users request access, you approve with '3v0 pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: 3v0 pairing approve <platform> <code>"
                    )
                elif is_email:
                    print_success("  Unknown email senders will be ignored.")
                else:
                    print_info(
                        "  Skipped — configure later with '3v0 gateway setup'"
                    )
            continue
...
def _setup_weixin():
    ...
    print()
    print_info("  1. 3V0 will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. 3V0 will store the returned account_id/token in ~/.3V0/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `3v0 gateway setup`.")
        return
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `3v0 pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `3v0 pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "Ev0Agent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``threev0_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from threev0_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.3V0/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.3V0/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: 3v0 gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: 3v0 gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: 3v0 gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo 3v0 gateway install --system"
                        )
                    print_info("  Or run in foreground:  3v0 gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: 3v0 gateway run")
                print_info(
                    "  For persistence:   tmux new -s 3v0 '3v0 gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: 3v0 gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup 3v0 gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run '3v0 gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`threev0_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from threev0_cli.service_manager import (
    ...
    if profile is None:
        ...
        # EV0_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``3v0 gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``3v0 gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``3v0 gateway run`` are unaffected.
      2. ``EV0_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``3v0 gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``EV0_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    # `docker logs` and ${EV0_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if EV0_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or EV0_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    #
    # Prefer `sleep infinity` (matches the static main-3v0 service's
    # pattern in docker/s6-rc.d/main-3v0/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: 3v0 gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: 3v0 gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s 3v0 '3v0 gateway run'"
                )
                print()
            ...
            print()
            print(
                "  3v0 gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s 3v0 '3v0 gateway run'         # persistent via tmux"
            )
            print(
                "  nohup 3v0 gateway run > ~/.3V0/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            from threev0_cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
                print("Per-profile gateways are auto-registered when you create a profile.")
                print()
                print("  3v0 profile create <name>     # creates the s6 service slot")
                print("  3v0 -p <name> gateway start   # bring it up via s6")
                print("  3v0 status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: 3v0 gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: 3v0 gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: 3v0 gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  3v0 profile delete <name>     # tears down the s6 service slot")
                print("  3v0 -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `3v0 -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: 3v0 gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_EV0_GATEWAY") == "1":
            print_error(
                "Refusing to stop the gateway from inside the gateway process.\n"
                "This command was blocked to prevent restart loops.\n"
                "Use `3v0 gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_EV0_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `3v0 gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: 3v0 gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  3v0 gateway install")
                else:
                    print("To install as a service:")
                    print("  3v0 gateway install")
                    print("  sudo 3v0 gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  3v0 gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup 3v0 gateway run > ~/.3V0/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s 3v0 '3v0 gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  3v0 gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  3v0 gateway install  # Install as user service")
                    print(
                        "  sudo 3v0 gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy 3V0 gateway unit files from
        # pre-rename installs (e.g. 3v0.service). Profile units and
        # unrelated third-party services are never touched.
