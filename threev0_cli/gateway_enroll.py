"""``3v0 gateway enroll`` — enroll a self-hosted gateway with a relay connector.
...
``docs/connector-gateway-auth-design.md``:
  ...
  1. Resolve a fresh Nous Portal access token from the existing login
     (``~/.3V0/auth.json``) — the same path ``3v0 dashboard register``
  ...
  4. Persist ``GATEWAY_RELAY_ID`` / ``GATEWAY_RELAY_SECRET`` /
     ...
     ``~/.3V0/.env``. The per-gateway secret authenticates the WS upgrade;
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                ...
                "token could not be verified — try `3v0 auth add nous` and retry."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def cmd_gateway_enroll(args) -> None:
    ...
    from threev0_cli.config import is_managed, save_env_value
    ...
    if is_managed():
        print(
            "✗ `3v0 gateway enroll` is not available in a managed/hosted install.\n"
    ...
    #    save_env_value writes them to ~/.3V0/.env (0600 dir) and never logs.
    to_write = {
