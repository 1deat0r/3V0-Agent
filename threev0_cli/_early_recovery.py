"""Dependency-light venv recovery that runs BEFORE threev0_cli.main's imports.

The ``3v0`` console entry point is ``threev0_cli.main:main``.  Importing
``threev0_cli.main`` pulls in third-party packages at module level (``dotenv``
via ``threev0_cli.env_loader``, ``yaml`` via ``threev0_cli.config``, ...).  In
...
a corrupted venv.  ``threev0_cli.main`` imports and calls
:func:`recover_if_needed` at the very top of its module body, before any
third-party import.

Scope: this early pass only repairs enough for ``threev0_cli.main`` to become
...
def _find_uv_binary() -> str | None:
    ...
    the only sanctioned installer is uv itself, which 3V0 already vendors
    (``~/.3V0/bin/uv.exe``) or the user has on PATH.  Stdlib-only.
    """
    exe = "uv.exe" if sys.platform == "win32" else "uv"
    candidates = [
        Path.home() / ".3V0" / "bin" / exe,
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``3v0 acp`` speaks JSON-RPC on stdout);
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``threev0_cli.main``
...
def recover_if_needed(
    ...
    """Repair wiped core packages so ``threev0_cli.main`` can import at all.

    Fast path (no marker present) is two ``lstat`` calls.  Only acts when a
    recovery marker from a prior ``3v0 update`` exists AND an import probe
    ...
    try:
        ...
        # launch is itself `3v0 update`: CLI and Desktop retries preserve
...
# every `3v0` launch: past this many attempts the early pass hands the
...
def _complete_pending_core_install(root: Path, core_marker: Path) -> bool:
    ...
    a prior ``3v0 update`` (or the self-lock preflight, #83569) left the
    ...
    ``threev0_cli.main``'s third-party imports, maps nothing yet, so the
    ...
    failure does not block every launch (``3v0 acp`` included).
    ...
    """
    try:
        from threev0_cli import _install_repair as ir
        ...
        try:
            ...
            print(
                "  The next launch will retry; 3v0 will keep working from "
