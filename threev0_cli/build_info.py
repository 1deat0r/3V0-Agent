"""
Baked-in build metadata for 3V0 Agent.

Source installs report their git revision live via ``git rev-parse`` (see
``threev0_cli/dump.py`` and ``threev0_cli/banner.py``).  That doesn't work inside
...
To make ``3v0 dump`` and the startup banner identify the exact commit the
image was built from, the Docker build writes the build-time ``$EV0_GIT_SHA``
arg into ``<project_root>/.ev0_build_sha``.  This module is the single
...
- Returns ``None`` when the file is absent.  Source installs and dev images
  built without the ``EV0_GIT_SHA`` build-arg fall through to live-git
...
def get_build_sha(short: int = 8) -> Optional[str]:
    ...
    Reads ``<project_root>/.ev0_build_sha`` if present.  The file is
    written by the Dockerfile's ``EV0_GIT_SHA`` build-arg and contains
