"""
MCP Server Management CLI — ``3v0 mcp`` subcommand.

Implements ``3v0 mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.3V0/config.yaml under the ``mcp_servers`` key.
"""
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from threev0_cli.cli_output import prompt as _shared_prompt
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.3V0/.env`` into ``os.environ`` and recursively interpolate any
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``3v0 mcp login`` to distinguish a genuine authentication
    ...
    try:
        ...
        return Ev0TokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # threev0_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  3v0 mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  3v0 mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  3v0 mcp add myserver --preset mypreset')
        return
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: 3v0 mcp test " + name)
        return
...
def cmd_mcp_remove(args):
    ...
    # earlier `3v0 mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  3v0 mcp add <name> --url <endpoint>')
        _info('  3v0 mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    landed before reporting success. Shared by ``3v0 mcp login`` and
    ``3v0 mcp reauth`` so both behave identically for a single server.
    """
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `3v0 mcp remove` + `3v0 mcp add` to reconfigure auth.")
        return False
    ...
    #
    # force_interactive_oauth: `3v0 mcp login` is *explicitly* user-
    # initiated even when stdin isn't a TTY (3V0 desktop / agent-
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `3v0 mcp login " + name + "`.")
            return False
        if tools:
...
def cmd_mcp_reauth(args):
    ...
    ``3v0 mcp reauth <name>`` re-auths a single server (same as ``login``).
    ``3v0 mcp reauth --all`` discovers every ``auth: oauth`` server in
    ...
    if not name:
        ...
        _info("Usage: 3v0 mcp reauth <name>   |   3v0 mcp reauth --all")
        return
    if name not in servers:
        _error(f"Server '{name}' not found in config.")
        if servers:
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: '3v0 mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``3v0 mcp`` subcommands."""
    ...
    if action == "picker":
        from threev0_cli.mcp_picker import run_picker
        run_picker()
        return
    if action == "catalog":
        from threev0_cli.mcp_picker import show_catalog
        show_catalog()
        return
    if action == "install":
        from threev0_cli.mcp_picker import install_by_name
        import sys as _sys
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `3v0 plugin`.
        from threev0_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("3v0 mcp                                    Open the catalog picker (default)")
        _info("3v0 mcp catalog                            List Nous-approved MCPs")
        _info("3v0 mcp install <name>                     Install a catalog MCP")
        _info("3v0 mcp serve                              Run as MCP server")
        _info("3v0 mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("3v0 mcp add <name> --command <cmd>         Add a stdio server")
        _info("3v0 mcp add <name> --preset <preset>       Add from a known preset")
        _info("3v0 mcp remove <name>                      Remove a server")
        _info("3v0 mcp list                               List configured servers")
        _info("3v0 mcp test <name>                        Test connection")
        _info("3v0 mcp configure <name>                   Toggle tools")
        _info("3v0 mcp login <name>                       Re-authenticate OAuth")
        _info("3v0 mcp reauth <name> | --all              Re-auth one or all OAuth servers")
        print()
