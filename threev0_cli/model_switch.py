...
- ``threev0_cli.providers``        -- canonical provider identity + overlays
- ``threev0_cli.model_normalize``  -- per-provider name formatting
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from threev0_cli.model_switch import
...
async def resolve_display_context_length_async(
    ...
    ``threev0_cli.route_identity.should_clear_context_pin_async``, which
...
def switch_model(
    ...
    # =================================================================
        ...
        try:
            from threev0_cli.config import load_config
            from threev0_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from threev0_cli.providers import resolve_user_provider as _ruser
    ...
    # threev0_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    if target_provider in {"opencode-zen", "opencode-go"} and isinstance(base_url, str):
        from threev0_cli.models import normalize_opencode_base_url
...
def _prefetch_provider_models_parallel(provider_slugs: list[str]) -> None:
    ...
    """
    from threev0_cli.models import cached_provider_model_ids
    ...
    def _fetch_one(slug: str) -> None:
        try:
            ...
            if models:
                from threev0_cli.models import update_provider_cache_entry
...
def _collect_authed_provider_slugs(
    ...
    from threev0_cli.auth import PROVIDER_REGISTRY, _load_auth_store
    from threev0_cli.providers import EV0_OVERLAYS, ALIASES as _PROVIDER_ALIAS_TABLE
    from threev0_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS, CANONICAL_PROVIDERS
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from threev0_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from threev0_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    from threev0_cli.providers import EV0_OVERLAYS
    from threev0_cli.auth import PROVIDER_REGISTRY as _auth_registry
    ...
    for pid, overlay in EV0_OVERLAYS.items():
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                from threev0_cli.auth import get_provider_auth_state as _nous_state
    ...
    # with `3v0 model`).
    try:
        from threev0_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # run_agent.py, threev0_cli/config.py) already consume that dict.
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from threev0_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`threev0_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from threev0_cli.models import fetch_openrouter_models
