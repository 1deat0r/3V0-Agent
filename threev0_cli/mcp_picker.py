"""MCP picker — interactive `3v0 mcp picker` (also the default `3v0 mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``3v0 mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `3v0 plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from threev0_cli.cli_output import prompt_yes_no
from threev0_cli.curses_ui import curses_single_select
from threev0_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new 3V0 session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from threev0_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `3v0 mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: 3v0 mcp install <name>    Picker: 3v0 mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their 3V0 is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer 3V0 — run `3v0 update` "
...
def show_catalog() -> None:
    """`3v0 mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`3v0 mcp picker` (and default `3v0 mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`3v0 mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `3v0 mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
