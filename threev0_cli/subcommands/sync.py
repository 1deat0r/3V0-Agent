"""``3v0 sync`` subcommand parser — Skill Sync.

Cloned from ``threev0_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    3v0 sync status                 show gate/opt-in/head state
    3v0 sync pull                   pull and materialize opted-in skills
    3v0 sync push                   push opted-in skills
    3v0 sync now                    reconcile: pull then push
    3v0 sync enable <skill>         opt a skill into sync
    3v0 sync disable <skill>        opt a skill out of sync
    3v0 sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    3v0 sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  3v0 sync status            what is synced, and from where\n"
            "  3v0 sync enable my-skill   include a skill in your sync\n"
            "  3v0 sync now               pull, then push\n"
            "  3v0 sync propose my-skill  share a skill with your team\n"
        ),
