...
These were module-level helpers in ``threev0_cli/main.py``. They are pulled
into a neutral module so both ``main.py`` and every
``threev0_cli/subcommands/<group>.py`` builder can import them without an
...
def add_accept_hooks_flag(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to EV0_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
