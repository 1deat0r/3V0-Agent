"""``3v0 approvals`` subcommand parser.
...
def build_approvals_parser(subparsers, *, cmd_approvals: Callable) -> None:
    ...
    approvals_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "`3v0 approvals suggest` mines past approval decisions from "
    ...
    suggest_parser.add_argument(
        "--db",
        help="Path to an alternate session database (default: ~/.3V0/state.db)",
    )
    ...
    test_parser = approvals_subparsers.add_parser(
        ...
        description=(
            ...
            "aren't parsed: 3v0 approvals test -- rm -rf /tmp/x"
        ),
    )
