"""``3v0 verify`` subcommand parser.

Follows the pattern of ``threev0_cli/subcommands/doctor.py``: parser built
...
# avoid paying an extra module import on every `3v0` invocation.
...
def build_verify_parser(subparsers, *, cmd_verify: Callable) -> None:
    ...
    verify_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "(or load the saved manifest at .3V0/environment.json), then "
    ...
    verify_parser.add_argument(
        "--save",
        action="store_true",
        help="Save the recipe as .3V0/environment.json in the project",
    )
    verify_parser.add_argument(
        "--skip-start",
        action="store_true",
