"""``3v0 gateway`` and ``3v0 proxy`` subcommand parsers.
...
from threev0_cli.subcommands._shared import add_accept_hooks_flag
...
def build_gateway_parser(
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if EV0_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy 3v0.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy 3V0 gateway unit files "
            "(e.g. 3v0.service) left over from older installs. Profile "
            "units (3v0-gateway-<profile>.service) and unrelated "
    ...
    gateway_enroll = gateway_subparsers.add_parser(
        ...
        description=(
            ...
            "GATEWAY_RELAY_SECRET / GATEWAY_RELAY_DELIVERY_KEY into ~/.3V0/.env. "
            "Requires being logged in (3v0 setup). Not available in managed installs."
        ),
    )
    ...
    gateway_enroll.add_argument(
        ...
        help=(
            ...
            "GATEWAY_RELAY_WAKE_URL in ~/.3V0/.env and forwarded at provision. "
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `3v0 proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
