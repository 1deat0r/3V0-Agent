"""``3v0 dashboard`` / ``3v0 serve`` subcommand parsers.
...
``threev0_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `3v0 dashboard` / `3v0 serve` cmdlines and SIGTERM them directly —
    # the same path `3v0 update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running 3V0 web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running 3V0 web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the 3V0 Agent web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically"
    )
    # Backward-compat shim: older 3V0 desktop app shells (<= 0.15.x) spawn the
    # backend as `3v0 dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "3V0 couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the 3V0 Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the 3V0 backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the 3V0 backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # (cmd_dashboard exports EV0_SERVE_HEADLESS=1). `dashboard` leaves it
    # unset and serves the browser UI as before.
    serve_parser.set_defaults(func=cmd_dashboard, no_open=True, headless_backend=True)

    # `3v0 dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.3V0/.env.
    # Nested subparser so bare `3v0 dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "EV0_DASHBOARD_OAUTH_CLIENT_ID into ~/.3V0/.env, and prints "
            "how to engage the login gate. Requires being logged in (3v0 setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://3v0.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via EV0_DASHBOARD_PORTAL_URL. Mainly for "
