...
on Japanese locales, etc.).  3V0's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``threev0_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. 3V0 is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``EV0_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    # Augment PATH with the 3V0-managed Git install directories so
    ...
    # already-running shells don't see the change — which means 3v0
    ...
    # install without 3V0-managed PortableGit).
    ...
    # stdin is re-configured for completeness; 3V0's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and 3V0's
    ``3v0 config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching 3V0 (User env var in Windows Settings, or
    export in a PowerShell profile) and 3V0 picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known 3V0-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\3v0\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``3v0`` from right after install) retain their old PATH.

    Any subprocess 3V0 spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known 3V0-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "3v0", "git", "cmd"),
        os.path.join(local_appdata, "3v0", "git", "bin"),
        os.path.join(local_appdata, "3v0", "git", "usr", "bin"),
        # 3V0 venv Scripts directory — host of the 3v0.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes 3v0, but harmless to include.
        os.path.join(local_appdata, "3v0", "3v0-agent", "venv", "Scripts"),
