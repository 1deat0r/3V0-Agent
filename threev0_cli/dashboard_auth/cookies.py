...
Three cookies in play:
  ...
  - ev0_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
setter wrote ``__Secure-ev0_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "ev0_session_at"
SESSION_RT_COOKIE = "ev0_session_rt"
...
SESSION_PROVIDER_COOKIE = "ev0_session_provider"
PKCE_COOKIE = "ev0_session_pkce"
...
SSO_ATTEMPT_COOKIE = "ev0_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /3v0`` we want ``Path=/3v0`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    issues a 24h rotating refresh token (3v0 #37247); a provider that
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/3v0``)
