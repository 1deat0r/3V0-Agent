...
def _redirect_uri(request: Request) -> str:
    ...
    Three resolution tiers:
      ...
      1. ``EV0_DASHBOARD_PUBLIC_URL`` env var or
      ...
      2. ``X-Forwarded-Prefix: /3v0`` (Mission Control deploys) — we
    ...
    from threev0_cli.dashboard_auth.prefix import (
        prefix_from_request,
        resolve_public_url,
    )
...
def _prefix(request: Request) -> str:
    ...
    ``threev0_cli.dashboard_auth.prefix`` for the normalisation rules.
    """
    from threev0_cli.dashboard_auth.prefix import prefix_from_request
...
async def auth_login(request: Request, provider: str, next: str = ""):
    ...
    pkce = ls.cookie_payload.get("ev0_session_pkce", "")
...
async def auth_native_authorize(
    ...
    from threev0_cli.dashboard_auth import native_flow
...
async def api_auth_ws_ticket(request: Request):
    ...
    from threev0_cli.dashboard_auth.ws_tickets import TTL_SECONDS, mint_ticket
...
async def auth_native_refresh(request: Request, body: _NativeRefreshBody):
    ...
    """
    from threev0_cli.dashboard_auth import list_session_providers
    from threev0_cli.dashboard_auth.base import RefreshExpiredError
