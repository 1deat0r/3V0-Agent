...
Profile-aware location: ``$EV0_HOME/logs/dashboard-auth.log``.
...
from ``threev0_constants`` or other threev0_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$EV0_HOME/logs/dashboard-auth.log``.

    Uses ``threev0_constants.get_ev0_home()`` (a leaf module — no import
