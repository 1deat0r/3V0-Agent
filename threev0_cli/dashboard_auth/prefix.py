...
prefix (e.g. ``mission-control.tilos.com/3v0/*`` -> dashboard on
:9119), injecting ``X-Forwarded-Prefix: /3v0`` so the backend can
reconstruct prefixed URLs (Location: headers, OAuth redirect_uri,
cookie Path attributes, SPA asset URLs).

This module is also the home of the ``EV0_DASHBOARD_PUBLIC_URL`` /
...
def _warn_if_malformed(source: str, raw: str) -> None:
    ...
    missing scheme (``3v0.example.com`` instead of
    ``https://3v0.example.com``) — the single most common cause of
    "I set EV0_DASHBOARD_PUBLIC_URL but the OAuth callback is still
    ...
    _log.warning(
        ...
        source,
        cleaned,
        cleaned.split("://")[-1] or "3v0.example.com",
    )
...
def normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/3v0"`` (no trailing slash) or ``""``
...
# ---------------------------------------------------------------------------
# EV0_DASHBOARD_PUBLIC_URL / dashboard.public_url
# ---------------------------------------------------------------------------
...
def resolve_public_url() -> str:
    ...
    Precedence (mirrors ``dashboard.oauth.client_id``):
      ...
      1. ``EV0_DASHBOARD_PUBLIC_URL`` env var (when non-empty after
    ...
    if env_clean:
        return env_clean
    _warn_if_malformed("EV0_DASHBOARD_PUBLIC_URL env var", env_raw)
