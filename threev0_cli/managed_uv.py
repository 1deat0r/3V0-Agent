...
class _UvResult(str):
    ...
    the call site from the *old*, already-imported ``threev0_cli.main`` against
...
def _reload_threev0_constants():
    """Re-execute ``threev0_constants`` from disk and return the fresh module.

    ``3v0 update`` imports ``threev0_constants`` from the OLD checkout,
    ...
    return importlib.reload(threev0_constants)
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("threev0_cli.main")
