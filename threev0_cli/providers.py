"""
Single source of truth for provider identity in 3V0 Agent.
...
2. **3V0 overlays** — transport type, auth patterns, aggregator flags,
...
# -- 3V0 overlay ----------------------------------------------------------
# 3V0-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class Ev0Overlay:
    """3V0-specific provider metadata layered on top of models.dev."""
...
EV0_OVERLAYS: Dict[str, Ev0Overlay] = {
    "moa": Ev0Overlay(
        transport="openai_chat",
        auth_type="virtual",
        base_url_override="moa://local",
    ),
    "openrouter": Ev0Overlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "openai-codex": Ev0Overlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": Ev0Overlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": Ev0Overlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": Ev0Overlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="EV0_QWEN_BASE_URL",
    ),
    "lmstudio": Ev0Overlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "github-copilot": Ev0Overlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": Ev0Overlay(
    ...
    "alibaba-coding-plan": Ev0Overlay(
        transport="openai_chat",
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "3v0", "user-config"
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. 3V0 overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + 3V0 overlay
    ...
    overlay = EV0_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + 3v0 extra
    ...
    if overlay is not None:
        # 3V0-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    *model* is optional but required for dual-wire providers (Nous) whose
    ...
    # chat_completions. The 3V0 overlay still advertises openai_chat
    ...
    # Direct provider checks for providers not in EV0_OVERLAYS
    if provider == "bedrock":
        return "bedrock_converse"
...
def resolve_provider_full(
    ...
    # 0.5 Exact 3V0 provider IDs must win over LOSSY alias collapsing.
    ...
    if canonical != raw:
        try:
            from threev0_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
