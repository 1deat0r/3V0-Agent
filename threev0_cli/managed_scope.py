...
A system-level directory (default ``/etc/3v0``, root-owned and not
user-writable) supplies ``config.yaml`` and ``.env`` values that WIN over the
user's ``~/.3V0/config.yaml`` and ``~/.3V0/.env`` on a per-leaf-key basis.

This is DISTINCT from ``threev0_cli.config.is_managed()`` / ``EV0_MANAGED``,
...
_DEFAULT_MANAGED_DIR = Path("/etc/3v0")
...
def _under_pytest() -> bool:
    ...
    Used to ignore the system default ``/etc/3v0`` during tests so a real
...
def get_managed_dir() -> Optional[Path]:
    ...
    Resolution (highest priority first):
      1. ``$EV0_MANAGED_DIR`` — deployment/bootstrap path override (IT-only;
         never persisted to any .env). Honored only when set to a non-empty value
         AND the directory exists.
      2. ``/etc/3v0`` — POSIX default, when it exists. Ignored under pytest so
...
def apply_managed_overlay(config: dict) -> dict:
    ...
    (rather than going through threev0_cli.config.load_config) to honor managed
    scope. Mirrors threev0_cli.config._load_config_impl's managed merge exactly:
    ...
    try:
        ...
        from threev0_cli.config import _deep_merge, _expand_env_vars, _normalize_root_model_keys
