"""CLI handlers for ``3v0 migrate ...``.

Currently exposes only ``3v0 migrate xai`` — diagnoses and (with --apply)
...
def cmd_migrate(args: Any) -> int:
    """Dispatcher for ``3v0 migrate <subtype>``."""
    sub = getattr(args, "migrate_type", None)
    if sub == "xai":
        return cmd_migrate_xai(args)

    print("usage: 3v0 migrate xai [--apply] [--no-backup]", file=sys.stderr)
    return 2
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    if not apply:
        ...
        print(color(
            "Re-run with `3v0 migrate xai --apply` to rewrite "
    ...
    print()
    print(color(
        "Run `3v0 doctor` to confirm no retired xAI models remain.",
        Colors.DIM,
    ))
    return 0
