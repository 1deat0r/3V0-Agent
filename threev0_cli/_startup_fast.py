...
This module is imported by ``threev0_cli/main.py`` BEFORE its heavy import
wall (config, argparse tree, logging, providers). Everything here must stay
**stdlib-only and cheap** (os/sys file probes; no yaml, no threev0_cli.config,
...
``threev0_cli/config.py``'s ``get_container_exec_info()`` reads the same
...
def read_install_method() -> str | None:
    ...
    non-default profile might redirect EV0_HOME.
    """
...
def print_fast_version_info() -> None:
    from threev0_cli import __release_date__, __version__

    print(f"3V0 Agent v{__version__} ({__release_date__})")
    ...
    print("Run '3v0 version' for update status.")


def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``3v0 --version`` before the heavy import wall.

    Termux keeps its historical contract (also accepts the ``version``
    subcommand + the EV0_TERMUX_DISABLE_FAST_CLI escape hatch). Everywhere
