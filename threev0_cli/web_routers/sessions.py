...
the late-binding seam in :mod:`threev0_cli.web_deps` so tests that
...
from threev0_cli.web_deps import late
...
async def search_sessions(
    ...
    try:
        ...
        try:
            ...
            # logs, or another 3V0 surface. FTS can't find those unless the
...
async def import_sessions_endpoint(request: Request):
    ...
    restores a whole 3V0 backup archive, while this endpoint is scoped to
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `3v0 sessions stats`).
