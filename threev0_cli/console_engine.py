"""Safe 3V0 Console command engine.

This module backs ``3v0 console`` and is intentionally narrower than the
full 3V0 CLI. It exposes a curated set of native adapters that can later be
...
class ConsoleCommand:
    ...
    usage: str
    summary: str
    handler: Callable[["Ev0ConsoleEngine", list[str]], str]
    mutating: bool = False
    confirmation: str = ""
...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (
        prev.startswith("Run '3v0 doctor'")
        and last.startswith("Run '3v0 setup'")
    ):
        return text.rstrip()
...
def _split_line(line: str) -> list[str]:
    try:
        ...
        from threev0_cli._subprocess_compat import split_command_line
...
def _parser_root() -> tuple[_ArgumentParser, argparse._SubParsersAction]:
    parser = _ArgumentParser(prog="3v0", add_help=False)
...
# dashboard opens a fresh Ev0ConsoleEngine per /api/console connection, so
...
def _dispatch_extracted_subcommand(
    *,
    ...
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    main_module = importlib.import_module("threev0_cli.main")
    builder = getattr(module, builder_name)
...
def _extracted_handler(
    ...
    main_handler_name: str,
    namespace_update: Callable[[argparse.Namespace], None] | None = None,
) -> Callable[["Ev0ConsoleEngine", list[str]], str]:
    def handler(_engine: Ev0ConsoleEngine, args: list[str]) -> str:
...
def _register_command_family(
    ...
    *,
    ...
    handler_factory: Callable[[Sequence[str]], Callable[["Ev0ConsoleEngine", list[str]], str]],
    ...
    for child_path in paths:
        ...
        command_summary = summary or (summaries or {}).get(full_path) or f"Run `3v0 {usage}`."
        engine.register(
            full_path,
            usage,
            command_summary,
            ...
            confirmation=confirmation or f"Run `3v0 {usage}`?",
        )


class Ev0ConsoleEngine:
    """Curated line-command executor for 3V0 Console."""
        ...
        try:
            ...
            if _contains_shell_syntax(raw_line, tokens):
                raise ConsoleCommandError(
                    "3V0 Console does not run shell syntax. Use one supported "
                    "3V0 command at a time."
                )
    ...
    def _register_defaults(self) -> None:
        self.register(("status",), "status", "Show 3V0 component status.", _status)
        self.register(("doctor",), "doctor", "Run diagnostics without auto-fix.", _doctor)
        self.register(("logs",), "logs [name] [-n N]", "Show recent 3V0 logs.", _logs)
        ...
        self.register(
            ...
            _config_set,
            mutating=True,
            confirmation="Update 3V0 configuration?",
        )
    ...
    def _register_broad_cli_surface(self) -> None:
        """Register non-admin CLI commands that are safe for 3V0 Console."""

        extracted = {
            "version": (
                "threev0_cli.subcommands.version",
            ...
            "prompt-size": (
                "threev0_cli.subcommands.prompt_size",
            ...
            "insights": (
                "threev0_cli.subcommands.insights",
            ...
            "security": (
                "threev0_cli.subcommands.security",
            ...
            "backup": (
                "threev0_cli.subcommands.backup",
                "build_backup_parser",
                "cmd_backup",
                [()],
                {()},
            ...
            "import": (
                "threev0_cli.subcommands.import_cmd",
            ...
            "config": (
                "threev0_cli.subcommands.config",
                "build_config_parser",
                "cmd_config",
                [("env-path",), ("check",)],
                set(),
            ...
            "plugins": (
                "threev0_cli.subcommands.plugins",
            ...
            "skills": (
                "threev0_cli.subcommands.skills",
                "build_skills_parser",
                "cmd_skills",
                [
                    ("browse",),
            ...
            "memory": (
                "threev0_cli.subcommands.memory",
                "build_memory_parser",
                "cmd_memory",
                [("status",), ("off",), ("reset",)],
                {("off",), ("reset",)},
            ...
            "pairing": (
                "threev0_cli.subcommands.pairing",
            ...
            "webhook": (
                "threev0_cli.subcommands.webhook",
            ...
            "profile": (
                "threev0_cli.subcommands.profile",
        ...
        self.register(
            ...
            _config_migrate,
            mutating=True,
            confirmation="Update 3V0 configuration with missing defaults?",
        )
        self.register(
            ("sessions", "export"),
        ...
        self.register(
            ...
            _adder_handler("send", (), "threev0_cli.send_cmd", "register_send_subparser"),
            mutating=True,
        ...
        _register_command_family(
            ...
            root="portal",
            paths=portal_paths,
            summaries=_adder_summaries("threev0_cli.portal_cli", "add_parser"),
            handler_factory=lambda fixed: _adder_handler(
                "portal",
                fixed,
                "threev0_cli.portal_cli",
                "add_parser",
            ),
        )
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("threev0_cli.projects_cmd", "build_parser"),
            mutating=[
                ("create",),
                ("add-folder",),
                ("remove-folder",),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("threev0_cli.kanban", "build_parser"),
            mutating=[
                ("init",),
                ("boards", "create"),
                ("boards", "rm"),
    ...
    def _resolve_command(self, tokens: Sequence[str]) -> tuple[ConsoleCommand, list[str]]:
        ...
        raise ConsoleCommandError(f"Unsupported 3V0 Console command: {probe}.{suffix}")

    def _rejection_for(self, tokens: Sequence[str]) -> str:
        first = tokens[0]
        if first.startswith("-"):
            return f"{first} is not available in 3V0 Console."
        blocked_top = {
            "acp",
            "chat",
            "claw",
        ...
        if first in blocked_top:
            return f"`3v0 {first}` is not available in 3V0 Console."
        blocked_pairs = {
            ("config", "edit"): "`config edit` opens an editor and is not available in 3V0 Console.",
            ("mcp", "serve"): "`mcp serve` starts a server and is not available in 3V0 Console.",
            ("profile", "alias"): "`profile alias` creates shell wrappers and is not available in 3V0 Console.",
            ("skills", "config"): "`skills config` is interactive and is not available in 3V0 Console.",
            ("skills", "publish"): "`skills publish` is not available in 3V0 Console.",
            ("portal", "login"): "`portal login` is interactive and is not available in 3V0 Console.",
            ("portal", "open"): "`portal open` opens a browser and is not available in 3V0 Console.",
            ("kanban", "tail"): "`kanban tail` streams output and is not available in 3V0 Console.",
            ("kanban", "watch"): "`kanban watch` streams output and is not available in 3V0 Console.",
            ("kanban", "daemon"): "`kanban daemon` starts a service and is not available in 3V0 Console.",
            ("kanban", "dispatcher"): "`kanban dispatcher` starts a worker and is not available in 3V0 Console.",
            ("kanban", "swarm"): "`kanban swarm` starts agent work and is not available in 3V0 Console.",
            ("kanban", "decompose"): "`kanban decompose` starts agent work and is not available in 3V0 Console.",
            ("kanban", "specify"): "`kanban specify` starts agent work and is not available in 3V0 Console.",
            ("kanban", "gc"): "`kanban gc` is not available in 3V0 Console.",
        }
        if len(tokens) >= 2:
        ...
        if tuple(tokens[:2]) in {("sessions", "delete"), ("sessions", "prune")}:
            return "`sessions delete` and `sessions prune` are not available in 3V0 Console."
        return ""
...
def _apply_confirmed_defaults(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError("auth add --type api-key requires --api-key in 3V0 Console.")
...
def _status(_engine: Ev0ConsoleEngine, args: list[str]) -> str:
...
def _doctor(_engine: Ev0ConsoleEngine, args: list[str]) -> str:
...
def _logs(_engine: Ev0ConsoleEngine, args: list[str]) -> str:
    if "-f" in args or "--follow" in args:
        raise ConsoleCommandError("`logs -f` is not available in 3V0 Console.")
    ...
    from threev0_cli.logs import list_logs, tail_log
...
def _sessions_list(_engine: Ev0ConsoleEngine, args: list[str]) -> str:
...
def _sessions_stats(_engine: Ev0ConsoleEngine, args: list[str]) -> str:
...
def _config_show(_engine: Ev0ConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config show")
    from threev0_cli.config import show_config

    return _capture_output(show_config)


def _config_path(_engine: Ev0ConsoleEngine, args: list[str]) -> str:
...
def _config_set(_engine: Ev0ConsoleEngine, args: list[str]) -> str:
    if len(args) < 2:
        raise ConsoleCommandError("Usage: config set <key> <value>")
    key = args[0]
    value = " ".join(args[1:])
    from threev0_cli.config import set_config_value

    return _capture_output(lambda: set_config_value(key, value))


def _config_migrate(_engine: Ev0ConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config migrate")

    def _run() -> None:
        from threev0_cli.config import migrate_config
...
def _sessions_export(_engine: Ev0ConsoleEngine, args: list[str]) -> str:
...
def _sessions_rename(_engine: Ev0ConsoleEngine, args: list[str]) -> str:
...
def _sessions_optimize(_engine: Ev0ConsoleEngine, args: list[str]) -> str:
...
def _sessions_repair(_engine: Ev0ConsoleEngine, args: list[str]) -> str:
    ...
    def _run() -> None:
        from threev0_state import DEFAULT_DB_PATH, _db_opens_cleanly, repair_state_db_schema
...
def _profile_status(_engine: Ev0ConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "profile")
    return _dispatch_extracted_subcommand(
        root="profile",
        fixed=(),
        args=(),
        module_name="threev0_cli.subcommands.profile",
        builder_name="build_profile_parser",
        main_handler_name="cmd_profile",
    )


def _cron_list(_engine: Ev0ConsoleEngine, args: list[str]) -> str:
    parser = _ArgumentParser(prog="cron list", add_help=False)
    parser.add_argument("--all", action="store_true")
    ns = parser.parse_args(args)
    from threev0_cli.cron import cron_list

    return _capture_output(lambda: cron_list(show_all=ns.all))


def _cron_status(_engine: Ev0ConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "cron status")
    from threev0_cli.cron import cron_status

    return _capture_output(cron_status)


def _cron_pause(_engine: Ev0ConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
        raise ConsoleCommandError("Usage: cron pause <job>")
    from cron.jobs import AmbiguousJobReference, pause_job

    try:
        job = pause_job(args[0], reason="paused from 3v0 console")
...
def _cron_resume(_engine: Ev0ConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
...
def _cron_run(_engine: Ev0ConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
...
def run_console_repl(
    *,
    ...
    """Run the local ``3v0 console`` REPL."""
    ...
    engine = Ev0ConsoleEngine()
    if interactive:
        print("3V0 Console. Type `help` for commands, `exit` to quit.", file=stdout)

    while True:
        if interactive:
            print("3v0> ", end="", file=stdout, flush=True)
        line = stdin.readline()
        if line == "":
            if interactive:
                print(file=stdout)
