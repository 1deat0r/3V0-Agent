...
def _has_configured_mcp_servers() -> bool:
    ...
    try:
        ...
        if isinstance(mcp_servers, dict) and len(mcp_servers) > 0:
            return True
        from threev0_cli.agent_plugins import has_enabled_agent_plugin_mcp
...
def start_background_mcp_discovery(*, logger, thread_name: str) -> None:
    ...
    with _mcp_discovery_lock:
        ...
        # Capture the caller's context-local EV0_HOME override (profile
...
def _resolve_discovery_timeout(
    ...
    When ``single_query`` is True (``3v0 -z "..."`` / ``-q``), the larger
    ...
    try:
        from threev0_cli.config import load_config, DEFAULT_CONFIG
...
def mcp_discovery_in_flight() -> bool:
    ...
    ``3v0 dashboard``).  Those processes populate THIS module's
...
def ensure_mcp_discovery_before_agent_build(
    *,
    ...
    Non-interactive first turns (``chat -q``, ``3v0 -z``) can construct
