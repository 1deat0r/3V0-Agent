"""
Unified self-relaunch for 3V0 CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``3v0 sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``3v0`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
from threev0_cli._parser import (
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched 3v0."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_threev0_bin() -> Optional[str]:
    ...
    Priority:
      ...
      2. ``shutil.which("3v0")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m threev0_cli.main``.
    ...
    ``3v0.exe`` on PATH or the ``sys.executable -m threev0_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
...
def build_relaunch_argv(
    ...
    *,
    ...
    """Construct an argv list for replacing the current process with 3v0.
    ...
    if bin_path:
        ...
        argv = [sys.executable, "-m", "threev0_cli.main"]
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh 3v0 invocation.
    ...
    relaunch contract wants: "run 3v0 again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    is usually ``3v0.exe`` (a Python console-script shim that wraps
    ``python -m threev0_cli.main``) or a ``.cmd`` batch file, and both
    ...
    That's functionally equivalent — the user sees "3v0 exited, then
    new 3v0 started" — just with two PIDs in play instead of one.
    """
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # cryptic.  Common causes: ``3v0`` not on PATH yet (install
            # hasn't propagated User PATH into this shell) or a stale shim.
            print(
                f"\nEv0 relaunch failed: {exc}\n"
                f"Command: {' '.join(new_argv)}\n"
                f"Fix: open a new terminal so PATH picks up, then re-run 3v0.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
