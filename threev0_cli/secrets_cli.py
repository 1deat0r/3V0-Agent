"""CLI handlers for ``3v0 secrets bitwarden ...``.
...
# that process is ``3v0 update``, the self-lock preflight detects the
# loaded native module and defers (#86781).  Keep the backend import lazy:
# this module is registered parse-time from ``threev0_cli.main`` and must not
...
def __getattr__(name: str):
    ...
    ``threev0_cli.secrets_cli.bw`` directly.  Resolving that attribute at
    ...
    the first actual attribute access, so ``import threev0_cli.secrets_cli``
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        ...
        "Secrets will be pulled at the start of every 3V0 process."
    )
    console.print(
        "  Status:  [cyan]3v0 secrets bitwarden status[/cyan]\n"
        "  Refresh: [cyan]3v0 secrets bitwarden sync[/cyan]\n"
        "  Disable: [cyan]3v0 secrets bitwarden disable[/cyan]"
    )
    return 0
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not enabled:
        console.print("\n  Run [cyan]3v0 secrets bitwarden setup[/cyan] to enable.")
        return 0
    if not token_set:
        console.print(
            f"\n  [yellow]Enabled but {token_env} is not set — 3V0 will skip BSM "
...
def cmd_token(args: argparse.Namespace) -> int:
    ...
    if not args.no_verify:
        ...
        if project_id and projects and project_id not in {p["id"] for p in projects}:
            console.print(
                ...
                "app or re-run `3v0 secrets bitwarden setup` to pick a "
    ...
    console.print(
        ...
        "Takes effect on the next 3V0 invocation."
    )
    if not bw_cfg.get("enabled"):
        console.print(
            "[yellow]Note: the Bitwarden integration is currently disabled — "
            "run `3v0 secrets bitwarden setup` (or set "
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not bw_cfg.get("enabled"):
        console.print(
            ...
            "`3v0 secrets bitwarden setup` first.[/yellow]"
        )
        return 1
    ...
    if not args.apply:
        console.print(
            ...
            "next [cyan]3v0[/cyan] invocation.  Re-run with [cyan]--apply[/cyan] "
    ...
    if res.returncode != 0:
        ...
        if "invalid_client" in lowered or "400 bad request" in lowered:
            console.print(
                ...
                "[cyan]3v0 secrets bitwarden setup[/cyan] and pick EU or "
