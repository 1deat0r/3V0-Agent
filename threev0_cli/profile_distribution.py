"""Profile distributions — shareable, packaged 3V0 profiles via git.

A distribution is a 3V0 profile published as a git repository (or
...
* ``3v0 profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``3v0 skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``3v0 profile``, not a parallel tree):

    3v0 profile install <source> [--name N] [--alias] [--force] [--yes]
    3v0 profile update  <name>  [--force-config] [--yes]
    3v0 profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "threev0_state.db", "response_store.db",
    ...
    "backups", "cache",
    # Infrastructure
    "3v0-agent", ".worktrees", "profiles", "bin", "node_modules",
...
def check_ev0_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires 3V0 {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this 3V0 distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``3v0 profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a 3V0 profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a 3V0 distribution."
        )
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.3V0).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="ev0_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `3v0 profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`3v0 profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="ev0_dist_update_") as tmp:
        plan = plan_install(
