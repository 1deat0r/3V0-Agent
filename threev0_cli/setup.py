"""
Interactive setup wizard for 3V0 Agent.
...
_DOCS_BASE = "https://github.com/1deat0r/3V0-Agent/docs"
...
from threev0_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from threev0_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ 3V0 Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure 3V0 using environment variables or config commands:")
    print_info("  3v0 config set model.provider custom")
    print_info("  3v0 config set model.base_url http://localhost:8080/v1")
    print_info("  3v0 config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run '3v0 setup' in an interactive terminal to use the full wizard.")
    print()
...
def is_noninteractive() -> bool:
    ...
    ``EV0_NONINTERACTIVE=1`` (see ``threev0_cli/web_server.py``). In that
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``EV0_NONINTERACTIVE=1`` or a closed/redirected
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from threev0_cli.curses_ui import curses_checklist
...
def _print_setup_summary(config: dict, ev0_home):
    ...
    try:
        from threev0_cli.auth import resolve_provider
    ...
    if not _provider_ready:
        print()
        print_warning("No inference provider is configured — 3V0 cannot chat yet.")
        print_info("  Finish this one step with either of:")
        print_info("    3v0 model            (pick any provider/model)")
        print_info("    3v0 setup --portal   (Nous Portal OAuth, no API key)")
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run '3v0 setup' to configure"))
    ...
    # Video generation — opt-in via `3v0 tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from threev0_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run '3v0 setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run '3v0 setup tts'"))
    else:
    ...
    if _stt_feature is not None and _stt_feature.managed_by_nous:
        ...
        if fw_ok:
            ...
            tool_status.append(
                ("Speech-to-Text (Local Whisper — not installed)", False, "run '3v0 tools' → Speech-to-Text")
            )
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run '3v0 setup terminal'"))
    ...
    # Spotify (OAuth via 3v0 auth spotify — check auth.json, not env vars)
    try:
    ...
    print()
    print(f"   {color('3v0 setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('3v0 setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('3v0 setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('3v0 setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('3v0 setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('3v0 config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('3v0 config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('3v0 config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('3v0', Colors.GREEN)}              Start chatting")
    print(f"   {color('3v0 gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('3v0 doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `3v0 tools` and `3v0 setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``3v0 model``)
    ...
    provider added to ``3v0 model`` is automatically available here.
    ...
    # Delegate to the shared 3v0 model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from threev0_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: 3v0 model")
    ...
    # on demand via `3v0 auth add`, `3v0 setup` vision, and
    # `3v0 setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``3v0 model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from threev0_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.3V0/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the 3V0 auth store) > existing XAI_API_KEY > prompt the user.
...
def setup_tts(config: dict):
    """Standalone TTS setup (for '3v0 setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where 3V0 runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `3v0 setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `3v0 setup terminal`.
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `3v0 egress setup` then `3v0 egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `3v0 egress setup`."
            )
        ...
        print_info("Requires the optional SDK: pip install '3v0-agent[vercel]'")
        ...
        try:
            ...
            # Managed uv first: $EV0_HOME/bin is never on PATH, so a bare
            # which() misses the uv 3V0 installed. Bootstrapping one is
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install '3v0-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into EV0_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("EV0_MAX_ITERATIONS")
    ...
    print_info("  Run `3v0 setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # EV0_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.3V0/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        from threev0_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where 3V0 delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects 3V0 to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from threev0_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://github.com/1deat0r/3V0-Agent/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://github.com/1deat0r/3V0-Agent/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  3v0 config edit")
    print_info("   Open config in your editor:  3v0 config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from threev0_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with 3V0 from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run '3v0 setup gateway' later to configure.")
    else:
        for idx in selected:
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     3v0 config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
    ...
    # via `3v0 import` or `3v0 setup gateway`). Gating this on
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `3v0 setup tools` and `3v0 tools` use the same flow:
...
def setup_telemetry(config: dict):
    ...
    print_info("Packages stay under this 3V0 profile and are not uploaded.")
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``threev0_cli.auth`` — lists every supported
    ...
    """
    try:
        from threev0_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from threev0_cli.config
    ...
    if section_key == "model":
        ...
        from threev0_cli.gateway import _all_platforms, _platform_status
...
def _load_openclaw_migration_module():
    """Load the openclaw_to_ev0 migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_ev0", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and 3V0.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure 3V0 to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point 3V0 at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point 3V0 at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point 3V0 at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point 3V0 at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to 3V0 equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing 3V0 config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in 3V0.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ 3V0's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(ev0_home: Path) -> bool:
    ...
    print_info("3V0 can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: 3v0 claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: 3v0 claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing 3V0 configs are
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in 3V0 (use 3v0 claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``3v0 setup --portal`` and ``3v0 portal``. This is the
    ...
    3V0 session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``3v0 setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``3v0 model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``3v0 portal`` always offers a model picker,
    ...
    print(color("│     ⚕ 3V0 Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `3v0 portal` == quick setup's Nous step.
    try:
        from threev0_cli.main import _model_flow_nous
        ...
        print_info("  You can retry later with `3v0 portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `3v0 portal`: %s", exc)
        print()
    ...
    print_info("  Run `3v0 portal info` to inspect routing.")
    print_info("  Run `3v0` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      3v0 setup           — full or quick (auto-detected)
      3v0 setup model     — just model/provider
      3v0 setup tts       — just text-to-speech
      3v0 setup terminal  — just terminal backend
      3v0 setup gateway   — just messaging platforms
      3v0 setup tools     — just tool configuration
      3v0 setup telemetry — just local shared metrics
      3v0 setup agent     — just agent settings
    """
    from threev0_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ 3V0 Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ 3V0 Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your 3V0 Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have 3V0 configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with '3v0 setup model|terminal|")
        ...
        setup_mode = prompt_choice(
            "How would you like to set up 3V0?",
            [
    ...
    # Tune later with `3v0 setup agent`.
    if not is_existing:
    ...
    if not (migration_ran and _skip_configured_section(config, "gateway", "Messaging Platforms")):
        ...
        from threev0_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
...
def _run_first_time_quick_setup(config: dict, ev0_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``3v0 setup <section>``
    or switch providers with ``3v0 model``.
    """
    from threev0_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with '3v0 setup gateway'",
        ],
        0,
    )
    ...
    if gateway_choice == 0:
        ...
        # (e.g. via `3v0 import` from another machine).
        from threev0_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
    ...
    print()
    print_info("  Configure all settings:    3v0 setup")
    if gateway_choice != 0:
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``3v0 tools`` (which
    ...
    try:
        ...
        from threev0_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``3v0 setup agent`` /
    ``3v0 config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, ev0_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `3v0 update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        3v0 tools")
        print_info("  Seed skills:         3v0 skills opt-in --sync")
        print_info("  Add MCP servers:     3v0 mcp add")
...
def _blank_slate_walkthrough(config: dict, ev0_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`3v0 update` runs from re-injecting them. Opt back in any")
            print_info("time with `3v0 skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `3v0 tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `3v0 plugins list` / `3v0 plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `3v0 plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `3v0 mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `3v0 mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   3v0 tools")
    print_info("  Seed skills:         3v0 skills opt-in --sync")
    print_info("  Add MCP servers:     3v0 mcp add")
    print_info("  Tune agent settings: 3v0 setup agent")
    print()
...
def _run_quick_setup(config: dict, ev0_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run '3v0 setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect 3V0 to messaging apps to chat from anywhere.")
        print_info("You can configure these later with '3v0 setup gateway'.")
