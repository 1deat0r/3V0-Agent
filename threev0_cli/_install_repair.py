...
- ``threev0_cli._early_recovery.recover_if_needed`` — stdlib-only, runs BEFORE
  ``threev0_cli.main``'s third-party imports, so it can complete a pending
  update while no native extension is mapped yet (#83569).
- ``threev0_cli.main._recover_core_update_marker_locked`` — the historical
...
the corrupted-venv state it exists to repair. ``threev0_cli.main`` imports
``managed_uv``, ``threev0_constants``, and friends only in its late path; the
...
:func:`threev0_cli._early_recovery._find_uv_binary` lookup and falls back to
...
from threev0_cli import _early_recovery as _er
...
def _is_termux_env(env: dict | None = None) -> bool:
    """Stdlib Termux probe (threev0_cli.main's version lives behind imports)."""
...
def _stdout_to_stderr():
    ...
    ``3v0 acp`` speaks JSON-RPC on stdout; an inherited-fd install child
...
def _venv_scripts_dir(root: Path) -> Path | None:
    ...
    if not venv_dir.is_dir():
        return None
    # threev0_constants is stdlib-only, so the canonical layout helper is safe
...
def _run_install_cmd(cmd: list[str], *, env: dict | None, root: Path) -> None:
    ...
    try:
        ...
        # the quarantined shims renamed aside and `3v0` gone from PATH
...
def run_core_install(root: Path) -> None:
    ...
    - quarantine live ``3v0*.exe`` shims on Windows so they can be replaced
