"""`3v0 checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.3V0/checkpoints/``.  Actions:

    3v0 checkpoints               # same as `status`
    3v0 checkpoints status        # total size, project count, breakdown
    3v0 checkpoints list          # per-project checkpoint counts + workdir
    3v0 checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    3v0 checkpoints clear [-f]    # nuke the entire base (asks first)
    3v0 checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    3v0 checkpoints
    3v0 checkpoints prune --retention-days 3 --max-size-mb 200
    3v0 checkpoints clear -f
...
from threev0_cli.sizefmt import format_bytes as _fmt_bytes
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: 3v0 checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``3v0 checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `3v0 checkpoints` → status
