...
from threev0_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE
from threev0_cli.plugin_capabilities import (  # noqa: F401 — re-exported
    CAPABILITY_REGISTRY,
...
VALID_HOOKS: Set[str] = {
    ...
    "pre_transcription",
    # Kanban task lifecycle hooks. Fired by threev0_cli.kanban_db when a task
    ...
    #   result: threev0_cli.kanban_db.DispatchResult (spawned, reclaimed,
...
class PluginManifest:
    ...
    # ``threev0_cli.plugin_capabilities.CAPABILITY_REGISTRY``. Declaration is
...
class PluginContext:
    ...
    def platform_actions(self):
        ...
        if self._platform_actions is None:
            from threev0_cli.platform_actions import PlatformActions
    ...
    def profile_name(self) -> str:
        ...
        :func:`threev0_cli.profiles.get_active_profile_name`, so it works in
    ...
    def register_dashboard_auth_provider(self, provider) -> Optional[PluginRegistration]:
        ...
        ``provider`` must be an instance of
        :class:`threev0_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        from threev0_cli.dashboard_auth.registry import restore_registration
        from threev0_cli.dashboard_auth.registry import snapshot_registration
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        # Lazy import to avoid circular: threev0_cli.main imports plugins indirectly
        from threev0_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
...
class PluginManager:
    ...
    def register_approval_transport(
        ...
        *,
        ...
        from threev0_cli.approval_transport import RegisteredApprovalTransport
    ...
    def has_enabled_portable_mcp(self, raw_config: Mapping[str, Any]) -> bool:
        ...
        for manifest in winners.values():
            ...
            if lookup_key not in enabled and manifest.name not in enabled:
                continue
            try:
                from threev0_cli.agent_plugins import _discover_mcp
        ...
        *,
        ...
        for child in sorted(path.iterdir()):
            ...
            if portable_file.exists() or portable_file.is_symlink():
                try:
                    from threev0_cli.agent_plugins import read_agent_plugin_manifest
    ...
    def _load_portable_plugin(
        ...
        lookup_key = manifest.key or manifest.name
        try:
            from threev0_cli.agent_plugins import load_agent_plugin
...
# threev0_constants.set_ev0_home_override). A process-wide single-slot
...
def _get_pre_tool_call_directive_details(
    ...
    from threev0_cli.lifecycle import invoke_hook as invoke_lifecycle_hook
