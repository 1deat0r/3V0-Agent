"""Slash command definitions and autocomplete for the 3V0 CLI.
...
class CommandDef:
    ...
    # Names a key in ``threev0_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``threev0_cli.slash_exec.run_execute`` and apply only
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of 3V0 config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal 3V0 works on across turns until achieved", "Session",
    ...
    CommandDef("wake", "Toggle the 'Hey 3V0' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("busy", "Control what Enter does while 3V0 is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.3V0/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update 3V0 Agent to the latest version", "Info",
               busy_policy="dispatch"),
    CommandDef("version", "Show 3V0 Agent version", "Info", aliases=("v",),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`threev0_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/3v0`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. 3V0 ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full 3V0
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``3v0 skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``3v0 skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``3v0 skills list`` and
...
# unrelated command landed. These claim their slots right after /3v0,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /3v0 <command>).
...
# native slot, the alias spelling stays reachable via /3v0 reset).
...
# ``/3v0 <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/3v0" decision, not a silent clamp.
#   - topup: the billing/balance surface; reached via /3v0 topup on Slack.
#     (the rehaul folded the old /credits + /billing surfaces into /topup.)
#   - moa: high-cost slash mode, available through /3v0 moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /3v0 debug on Slack.
#   - egress: Docker-only proxy status; reachable as /3v0 egress on Slack.
#   - init: repo-scan AGENTS.md bootstrap — a cwd-centric dev command that is
#     rare from Slack; reachable as /3v0 init. Without this entry, adding
#     /init clamps /version off the native list and breaks Telegram parity.
#   - version: low-frequency info command; reachable as /3v0 version on
...
#   - diff: git working-tree diff; reached via /3v0 diff on Slack so it
...
#     /3v0 update on Slack. Demoted to free the native slot /approvals now
...
#   - heartbeat: session heartbeat management; reached via /3v0 heartbeat
#     on Slack. Added at the 50-cap — a native slot would clamp /insights.
#   - refine: on-demand memory/skill review; reached via /3v0 refine on
#     Slack. Added at the 50-cap — a native slot would clamp an existing
#     native slash.
#   - pause: global emergency stop; reached via /3v0 pause [off] on
#     Slack. Added at the 50-cap — a native slot would clamp /platform.
#   - whoami: one-off identity lookup; reached via /3v0 whoami on Slack.
...
#     /3v0 platform on Slack. Demoted when /save became gateway-available
#     (session export is an interactive surface; platform is a rare
#     informational lookup) — without this entry /save tips the registry
#     past the 50-cap and silently clamps /platform, breaking parity.
_SLACK_VIA_EV0_ONLY = frozenset({"topup", "moa", "debug", "egress", "init", "version", "diff", "update", "heartbeat", "refine", "pause", "whoami", "platform"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/3v0 <verb>`` subcommand.
    ...
    can still reach them via ``/3v0 <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/3v0`` is always reserved as the first entry so the
    legacy ``/3v0 <subcommand>`` form keeps working for anything that
    ...
    # Reserve /3v0 as the catch-all top-level command.
    entries.append(("3v0", "Talk to 3V0 or run a subcommand", "[subcommand] [args]"))
    seen.add("3v0")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_EV0_ONLY:
            # Intentionally Slack-via-/3v0 only (see _SLACK_VIA_EV0_ONLY).
            return
    ...
    # everything except /3v0, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://3v0-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /3v0 handler.

    Maps both canonical names and aliases so /3v0 bg do stuff works
    the same as /3v0 background do stuff.

    Plugin-registered slash commands are included so ``/3v0 <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from threev0_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
    ...
    def _personality_completions(sub_text: str, sub_lower: str):
        """Yield completions for /personality via threev0_cli.personality."""
        try:
