...
3V0 Agent (see website/docs/getting-started/platform-support.md). The
...
build sandbox. ``nix/python.nix`` sets ``EV0_NIX_BUILD=1`` on the
3V0 package derivation, so only that build may create an artifact.
...
_IN_NIX_BUILD = os.environ.get("EV0_NIX_BUILD") == "1"

_BLOCK_MESSAGE = (
    "Building wheels or sdists for 3v0-agent is not supported.\n"
    "3V0 is distributed via the shell installer, Docker image, or Nix.\n"
    "See: https://github.com/1deat0r/3V0-Agent/docs/getting-started/installation\n"
    "\n"
    "If you are developing, use an editable install instead:\n"
    "  uv sync          # or: uv pip install -e .\n"
    "\n"
    "If you are building with Nix (uv2nix), this error should not fire —\n"
    "the 3V0 Nix derivation sets EV0_NIX_BUILD=1. If it does, file a bug."
)
