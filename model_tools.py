...
def _is_dispatcher_owned_worker() -> bool:
    """False when EV0_KANBAN_* is present but this execution does not own it
...
def _run_async(coro):
    ...
    if loop and loop.is_running():
        ...
        # async tools resolve get_ev0_home() under the active profile.
...
#   - cli.py, threev0_cli/*      -> inline on startup (no event loop)
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if (
            ...
            # Dispatcher-spawned workers are scoped by EV0_KANBAN_TASK and
    ...
    # This ensures that even if a composite toolset (like 3v0-cli)
    ...
    if disabled_toolsets:
        for toolset_name in disabled_toolsets:
            if validate_toolset(toolset_name):
                ...
                if toolset_name.startswith("3v0-") or (get_toolset(toolset_name) or {}).get("posture"):
                    # Platform bundles (3v0-*) include _EV0_CORE_TOOLS, and
                    ...
                    if (not quiet_mode and toolset_name.startswith("3v0-")
    ...
    # window). Core 3V0 tools (toolsets._EV0_CORE_TOOLS) are NEVER
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from threev0_cli.config import load_config as _load
        cfg = _load() or {}
...
def _normalize_json_strings_for_schema(value: Any, schema: Any) -> Any:
    ...
    Ported from cline/cline#11803, adapted to 3v0-agent's coercion layer.
    ...
    try:
        ...
        try:
            ...
            if skip_tool_execution_middleware:
                ...
                from threev0_cli.middleware import run_tool_execution_middleware
