...
Component separation:
    ...
    gui.log receives dashboard-side records from ``ev0_cli.web_server``,
    ``ev0_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
from ev0_constants import get_config_path, get_ev0_home
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    def __init__(self, *args, **kwargs):
        from ev0_cli.config import is_managed
...
def _read_logging_config():
    ...
    try:
        ...
        # reuses the parse ev0_cli.main's early bridge already did (one
        # config.yaml parse per process instead of 3-4). Fall back to a
        # direct parse when ev0_cli.config isn't importable (bare
        # ev0_logging consumers).
        try:
            from ev0_cli.config import read_raw_config as _rrc
            cfg = _rrc() or {}
        except Exception:
