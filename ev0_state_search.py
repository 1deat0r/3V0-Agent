...
``ev0_state.SessionDB``. It defines no ``__init__`` and no state of its
...
``SessionDB.__init__``. It must never import ev0_state (cycle) — shared
module-level constants live in ev0_state_common.
"""
...
# Moved methods logged under the "ev0_state" logger before the split;
# keep that logger identity so log filtering/capture behavior is unchanged.
logger = logging.getLogger("ev0_state")
