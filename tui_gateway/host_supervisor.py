...
def _default_registry_path() -> Path:
    return get_threev0_home() / "state" / _REGISTRY_NAME
...
class HostSupervisor:
    ...
    def __init__(
        ...
        *,
        ...
        expected_threev0_home: str | None = None,
        autostart: bool = True,
    ) -> None:
        ...
        self.expected_threev0_home = expected_threev0_home if expected_threev0_home is not None else str(get_threev0_home())
    ...
    def _spawn_locked(self, *, reason: str) -> None:
        ...
        if self.env:
            env.update(self.env)
        env["EV0_COMPUTE_HOST_HEARTBEAT_SECS"] = str(self.heartbeat_secs)
    ...
    def _validate_hello(self) -> None:
        ...
        got_home = str(hello.get("ev0_home") or "")
        if got_home and got_home != self.expected_threev0_home:
            raise RuntimeError(f"compute host EV0_HOME mismatch: {got_home} != {self.expected_threev0_home}")
