...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `EV0_TUI_SIDECAR_URL`, set by the dashboard's
    ...
    """
    url = os.environ.get("EV0_TUI_SIDECAR_URL")
...
# tested; override via ``EV0_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("EV0_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``EV0_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
...
# ``3v0 --tui``) imports cleanly there.  SIGBREAK (Windows' Ctrl+Break)
...
def wait_for_mcp_discovery(timeout: "float | None" = None) -> None:
    ...
    # session profile's EV0_HOME first), so a launch profile with no
...
def main():
    ...
    # Live-apply skins 3V0 activates mid-conversation.
