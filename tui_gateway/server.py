...
# appends every unhandled exception to ~/.3V0/logs/tui_gateway_crash.log
...
try:
    from threev0_cli.banner import prefetch_update_check
...
_current_runtime_session_record: contextvars.ContextVar[dict | None] = (
    contextvars.ContextVar("ev0_gateway_runtime_session_record", default=None)
)
...
def _prepend_tool_paths(env: dict[str, str]) -> dict[str, str]:
    """Prepend 3V0' managed bin, the venv bin dir, and the user-local
    bin dir to PATH so slash_worker child processes can resolve
    3V0-managed CLIs (browser-use, uvx, uv) even when the parent
...
class _SlashWorker:
    """Persistent Ev0CLI subprocess for slash commands."""
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # slash_worker runs the 3V0 agent → needs provider credentials.
        ...
        # gateway's launch EV0_HOME (#40677). The override goes through the
        ...
        # Prepend the 3V0 venv bin dir and the user-local bin dir to PATH so
        # slash_worker child processes can resolve 3V0-managed CLIs
...
def _transfer_active_session_slot(
    ...
    *,
    ...
    if lease is None:
        return True
    try:
        from threev0_cli.active_sessions import transfer_active_session
...
def _close_sessions_for_transport(
    ...
    for sid, session in owned:
        if session.get("close_on_disconnect"):
            ...
            # actually fire; a standalone `3v0 --tui` keeps real _stdio.
...
def _transport_is_dead(transport) -> bool:
    ...
    # for a standalone `3v0 --tui`, so it must NOT count as dead here (doing
...
def _reclaim_orphaned_leases() -> None:
    ...
    try:
        from threev0_cli.active_sessions import release_orphaned_leases
...
def _max_live_sessions() -> int:
    try:
        from threev0_cli.active_sessions import coerce_max_concurrent_sessions
...
# ``profile`` on those calls; we open that profile's db and bind its EV0_HOME
...
def _profile_scoped(handler):
    """Bind ``params['profile']``'s EV0_HOME around a pet RPC handler.

    Pets are per-profile: ``display.pet.*`` lives in the profile's config.yaml and
    sprites install under its ``pets/`` dir (both resolve via ``get_ev0_home``).
...
def _launch_configured_cwd() -> str | None:
    ...
    (wherever ``3v0 dashboard`` was launched) instead of the configured
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `3v0` shows the prompt before constructing AIAgent; the TUI used
    ...
    def _build() -> None:
        ...
        try:
            ...
            # EV0_HOME so config/skills/model resolve to it, and hand the
...
def _ensure_session_db_row(session: dict) -> None:
    ...
    * A terminal session (``3v0`` / ``3v0 --tui`` / CLI) is started from a
...
def _load_dashboard_process_isolation_config(cfg: dict | None = None) -> dict[str, Any]:
    ...
    ``threev0_cli.config.DEFAULT_CONFIG``. Keep
...
def _load_cfg() -> dict:
    ...
    ``threev0_cli.config.load_config_readonly`` — managed-scope overlay and
...
def _apply_managed(cfg: dict) -> dict:
    ...
    threev0_cli.config.load_config, so without this a managed skin / reasoning_effort
...
def _set_session_context(
    ...
    *,
    ...
    try:
        ...
        # can read EV0_SESSION_ID. Without this, set_session_vars leaves the
        ...
        # EV0_SESSION_ID even though agent_init set it via
...
def resolve_skin() -> dict:
    try:
        from threev0_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _broadcast_skin_if_changed() -> None:
    ...
    (``3v0 config set display.skin``) OR edited the active skin's colors in
...
def _ensure_skin_watcher() -> None:
    ...
    3V0 activates, a pet ``/pet`` adopts, a cron the scheduler fires, or a
    ...
    threading.Thread(target=_loop, name="3v0-change-watcher", daemon=True).start()
...
def _resolve_model() -> str:
    ...
    try:
        from threev0_cli.models import get_preferred_silent_default_model
...
def _resolve_session_platform() -> str:
    ...
    Resolution:
      * ``EV0_DESKTOP=1`` and ``EV0_DESKTOP_TERMINAL`` unset → "desktop"
        (the chat-panel backend — a graphical React surface, not a terminal).
      * ``EV0_DESKTOP_TERMINAL=1`` → "tui"
        (``3v0 --tui`` running in the desktop's embedded terminal pane;
...
def _config_model_target() -> tuple[str, str]:
    ...
    Unlike `_resolve_model()`, this never reads EV0_MODEL /
    EV0_INFERENCE_MODEL. Those env vars are a launch-scoped seed
    (`3v0 --tui -m <model>`, hosted-instance provisioning); if they
    ...
    # No fallback to _resolve_model() here: that reads EV0_MODEL /
    # EV0_INFERENCE_MODEL, which `3v0 --tui -m <model>` sets as a
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    try:
        from threev0_cli.models import detect_static_provider_for_model
...
from threev0_state import _BARE_BILLING_PROVIDERS
...
def _persist_live_session_system_prompt(session: dict | None) -> None:
    ...
    # Re-bind EV0_HOME to the session's profile so load_soul_md() and
...
def _load_approval_mode() -> str:
    ...
    canonical ``threev0_cli.config.load_config`` path applies managed-scope
...
def _load_reasoning_config(model: str = "") -> dict | None:
    ...
    :func:`threev0_constants.resolve_reasoning_config` (per-model override >
...
def _gui_surface_toolsets(platform: str) -> set[str]:
    ...
    Both entries are deliberately off ``_EV0_CORE_TOOLS`` — every other
    ...
    paths run with ``EV0_DESKTOP=1``. Keying GUI capability off that env var
    silently stripped every pane/browser tool from URL and cloud gateways while
    the same backend told the model it was "chatting inside the 3V0 desktop
...
def _load_enabled_toolsets(platform: str | None = None) -> list[str] | None:
    ...
    # Coding posture (base 3V0): with no explicit pin, collapse to the
    # coding toolset (+ enabled MCP servers) when sitting in a code workspace.
    # The desktop app and `3v0 --tui` both land here. See
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] EV0_TUI_TOOLSETS=all enables every toolset; "
        ...
        try:
            ...
            from threev0_cli.tools_config import _parse_enabled_flag
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown EV0_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in EV0_TUI_TOOLSETS "
        ...
        fallback_notice = (
            "[tui] no valid EV0_TUI_TOOLSETS entries; using configured CLI toolsets"
        )
    ...
    try:
        ...
        if not enabled:
            return None
        # The client-surface toolsets are off _EV0_CORE_TOOLS (every other
        # platform would carry their schema for nothing), so the platform
        # recovery above — which keys off 3v0-cli's tool universe — can't
        ...
        if fallback_notice is not None:
            print(
                "[tui] no valid EV0_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _apply_model_switch(
    ...
    *,
    ...
    cfg = None
    try:
        from threev0_cli.config import get_compatible_custom_providers, load_config
    ...
    #
    # We deliberately do NOT write process-global env vars (EV0_MODEL /
    # EV0_INFERENCE_MODEL / EV0_TUI_PROVIDER / EV0_INFERENCE_PROVIDER)
...
def _sync_agent_model_with_config(sid: str, session: dict) -> None:
    ...
    try:
        _apply_model_switch(
            ...
            # target the sync computed — the path that leaked `3v0 --tui -m`
...
def _probe_config_health(cfg: dict) -> str:
    ...
    if isinstance(display_cfg, dict):
        ...
        if personality and personality not in {"default", "none", "neutral"}:
            try:
                from threev0_cli.personality import available_personalities
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    if agent is not None and not (session or {}).get("_compute_host_active"):
        try:
            ...
            pass
        try:
            from threev0_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from threev0_cli.banner import get_update_result
...
def _agent_cbs(sid: str) -> dict:
    callbacks = {
        ...
        # directly underneath the 3V0 window, and answers
...
def _available_personalities(cfg: dict | None = None) -> dict:
    """Built-ins + user overrides, via threev0_cli.personality (single owner)."""
    from threev0_cli.personality import available_personalities
...
def _validate_personality(value: str, cfg: dict | None = None) -> tuple[str, str]:
    ...
    Same contract as threev0_cli.personality.resolve_personality — (name,
    ...
    """
    from threev0_cli.personality import normalize_personality_name
...
def _prompt_text(value) -> str:
    ...
    Delegates to threev0_cli.personality (single owner).
    """
...
def _load_fallback_model():
    ...
    stays in parity with ``Ev0CLI.__init__`` and ``gateway/run.py``:
...
def _make_agent(
    ...
    # block. Dashboard /api/ws uses threev0_cli.mcp_startup; TUI stdio keeps
...
def _init_session(
    ...
    with _sessions_lock:
        _sessions[sid] = {
            ...
            "edit_snapshots": {},
            "tool_started_at": {},
            # Profile-scoped EV0_HOME for app-global remote mode; None =
...
def _build_persist_message_with_image_refs(user_text: str, image_paths: list[str]) -> str:
    ...
    format the desktop client (directive-text.tsx / EV0_DIRECTIVE_RE)
...
def _pet_state_rows(spritesheet) -> list[str]:
    ...
    3V0 has to support both the legacy 8-row petdex atlas and the current
...
# (agent/billing_view.py + threev0_cli/nous_billing.py) — same as /topup.
...
#
# Layout:  $EV0_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _maybe_fire_tui_loop_tick(sid: str, session: dict) -> None:
    ...
    """
    try:
        from threev0_cli.loops import LoopManager, goal_blocks_loop_tick
    except Exception:
        return
...
def _collect_kanban_notifications(session: dict) -> list:
    ...
    ``platform="tui"`` and ``chat_id=EV0_SESSION_KEY`` (see
    ...
    # DB when EV0_KANBAN_DB pins the board path (same guard as the gateway
    # notifier).
...
def _wire_desktop_ui() -> None:
    ...
    Idempotent. The tool hands back the turn's ``EV0_UI_SESSION_ID`` as
...
def _run_prompt_submit(
    ...
    *,
    ...
    def run():
        ...
        approval_token = None
        session_tokens = []
        home_token = None  # per-turn EV0_HOME override for a resumed remote profile
        secret_token = None
        ...
        try:
            ...
            if images:
                try:
                    ...
                    from threev0_cli.config import load_config as _tui_load_config
                ...
                try:
                    ...
                    def _thinking_should_play() -> bool:
                        if is_audio_output_active():
                            return False
                        try:
                            from threev0_cli.voice import is_continuous_active
            ...
            if (
                ...
                try:
                    ...
                    logger.warning("voice TTS skipped: threev0_cli.voice unavailable")
                except Exception as e:
            ...
            # Run while any profile-specific EV0_HOME override is still active
...
def _session_images_dir(session: dict) -> Path:
    ...
    run BEFORE ``prompt.submit`` installs the session's profile EV0_HOME
    override, so ``get_ev0_home()`` here would return the gateway's launch
...
def _desktop_attachment_dir(session: dict) -> Path:
    ...
    installs the session's profile EV0_HOME override, while the docker/ssh
...
def _(rid, params: dict) -> dict:
    ...
    if key == "model":
        try:
            ...
            if session:
                from threev0_cli.model_switch import parse_model_switch_args
    ...
    if key == "fast":
        ...
        overrides = None
        if nv == "fast":
            from threev0_cli.models import resolve_fast_mode_overrides
    ...
    if key in {"prompt", "personality", "skin"}:
        try:
            ...
            if key == "prompt":
                ...
                # through threev0_cli.personality (single owner) and never
                # touches the user-owned global system prompt.
                from threev0_cli.personality import persist_personality
...
def _is_repo_junk(root: str) -> bool:
    ...
    :func:`_non_workspace_dirs`) or anything under EV0_HOME (~/.3V0 by
...
def _is_session_cwd_junk(cwd: str) -> bool:
    ...
    EV0_HOME may be an intentional prose/data workspace. The pre-Projects
    desktop surfaced every such cwd, so exclude only the broad defaults that
    would create catch-all projects: EV0_HOME itself and the dirs in
...
def _discover_repos_payload(
    ...
    repos even with zero 3v0 sessions. Session-derived roots cover repos
    outside the scan roots. Both are junk-filtered (3v0 home subtree + bare
    ...
    try:
        ...
        def _read(c) -> None:
            for entry in pdb.list_discovered_repos(c):
                ...
                # i.e. "just now" — so a git checkout with zero 3V0 sessions
...
def _rank_slash_completions(
    ...
    *,
    ...
    of skills someone invokes daily lead the ones that shipped with 3V0
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `3v0` is interactive — use `/3v0 chat -q …` or run `3v0` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`3v0 setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`3v0 gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`3v0 sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`3v0 config edit` needs $EDITOR in a real terminal"
    return None
...
def _model_picker_context(agent):
    ...
    from threev0_cli.inventory import load_picker_context
...
def _format_live_history_output(session: dict) -> str:
    ...
    for idx, message in enumerate(messages, start=1):
        ...
        label = "You" if role == "user" else "3V0" if role == "assistant" else role.title()
...
def _any_session_running() -> bool:
    ...
    Registered as the voice busy-probe (``threev0_cli.voice.set_voice_busy_probe``)
...
def _speak_text_with_barge(text: str) -> None:
    """Speak *text* via threev0_cli.voice.speak_text with spoken barge-in.
    ...
    """
    from threev0_cli.voice import speak_text
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from threev0_cli.voice import start_continuous
            ...
            # are tolerated.
            try:
                from threev0_cli.voice import set_voice_busy_probe
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        import threev0_cli.voice  # noqa: F401
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from threev0_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from threev0_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
