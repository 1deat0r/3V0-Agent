...
def _(rid, params: dict) -> dict:
    ...
    ``threev0_cli.config.reload_env``, matching classic CLI's ``/reload``
...
@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m threev0_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        ...
        r = subprocess.run(
            [sys.executable, "-m", "threev0_cli.main", *argv],
            capture_output=True,
            text=True,
...
def _(rid, params: dict) -> dict:
    ...
    if name == "moa":
        ...
        try:
            from threev0_cli.moa_config import moa_usage, normalize_moa_config
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from threev0_cli.skills_hub import do_install
        ...
        if action == "browse":
            from threev0_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from threev0_cli.skills_hub import inspect_skill
...
# (threev0_cli/web_routers/mcp.py) so a desktop plugin can manage MCP servers for
...
# All persistence reuses threev0_cli/mcp_config.py helpers — no logic is duplicated.
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from threev0_cli.mcp_config import _remove_mcp_server
