...
@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m ev0_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        ...
        r = subprocess.run(
            [sys.executable, "-m", "ev0_cli.main", *argv],
            capture_output=True,
            text=True,
            ...
            cwd=os.getcwd(),
            # cli.exec runs `python -m ev0_cli.main` (can drive the agent) →
...
def _(rid, params: dict) -> dict:
    ...
    if name == "moa":
        ...
        try:
            from ev0_cli.moa_config import moa_usage, normalize_moa_config
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from ev0_cli.skills_hub import do_install
        ...
        if action == "browse":
            from ev0_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from ev0_cli.skills_hub import inspect_skill
