...
# 5. Symlinks the '3v0' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ 3V0 Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/3v0-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink 3v0 into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up 3v0 command..."

EV0_BIN="$SCRIPT_DIR/venv/bin/3v0"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$EV0_BIN" "$COMMAND_LINK_DIR/3v0"
echo -e "${GREEN}✓${NC} Symlinked 3v0 → $COMMAND_LINK_DISPLAY_DIR/3v0"
...
# ============================================================================
# Seed bundled skills into ~/.3V0/skills/
# ============================================================================

EV0_SKILLS_DIR="${EV0_HOME:-$HOME/.3v0}/skills"
mkdir -p "$EV0_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.3V0/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$EV0_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  3v0 status        # Check configuration"
if is_termux; then
    echo "  3v0 gateway       # Run gateway in foreground"
else
    echo "  3v0 gateway install # Install gateway service (messaging + cron)"
fi
echo "  3v0 cron list     # View scheduled jobs"
echo "  3v0 doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m threev0_cli.main setup
fi
