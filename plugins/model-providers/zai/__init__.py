...
request omits ``thinking``.  3V0' ``reasoning_config = {"enabled": False}``
...
:meth:`ZaiProfile.build_api_kwargs_extras` translates the 3V0 reasoning
...
(per Z.AI / BigModel docs).  3V0' richer effort scale is collapsed onto
...
def _glm_5_2_reasoning_effort(reasoning_config: dict | None) -> str | None:
    """Map 3V0 reasoning effort onto GLM-5.2's native ``high``/``max``.
