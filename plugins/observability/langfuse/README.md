...
This plugin ships bundled with 3V0 but is **opt-in** — it only loads when
...
3v0 tools  # → Langfuse Observability

# Manual
pip install langfuse
3v0 plugins enable observability/langfuse
```

## Required credentials

Set these in `~/.3V0/.env` (or via `3v0 tools`):

```bash
EV0_LANGFUSE_PUBLIC_KEY=pk-lf-...
EV0_LANGFUSE_SECRET_KEY=sk-lf-...
EV0_LANGFUSE_BASE_URL=https://cloud.langfuse.com   # or your self-hosted URL
```
...
```bash
3v0 plugins list                 # observability/langfuse should show "enabled"
3v0 chat -q "hello"              # then check Langfuse for a "3V0 turn" trace
```

Generation observations include the 3V0 system prompt when the provider
uses a separate `system` param (Anthropic Messages API). Open an **LLM call**
child span to inspect `role: system` (truncated via `EV0_LANGFUSE_MAX_CHARS`).

## Optional tuning

```bash
EV0_LANGFUSE_ENV=production       # environment tag
EV0_LANGFUSE_RELEASE=v1.0.0       # release tag
EV0_LANGFUSE_SAMPLE_RATE=0.5      # sample 50% of traces
EV0_LANGFUSE_MAX_CHARS=12000      # max chars per field (default: 12000)
EV0_LANGFUSE_CAPTURE=sanitized    # content capture mode (see below)
EV0_LANGFUSE_DEBUG=true           # verbose plugin logging
```

## Capture modes

`EV0_LANGFUSE_CAPTURE` controls how much *content* (prompts, responses,
...
```bash
3v0 plugins disable observability/langfuse
```
