...
Optional 3V0 observability plugin that configures exporters and maps
3V0-specific observer hooks to NeMo Relay marks and ATIF state. 3V0 core
...
not replace 3V0 Agent's planner, tools, memory, model provider routing, or
CLI UX. 3V0 core emits NeMo Relay lifecycle events for provider and tool
...
With this plugin enabled, 3V0 Agent can:

- Export the Relay scopes and LLM/tool lifecycles emitted by 3V0 core.
- Add 3V0 session, turn, approval, and subagent mark events.
...
```bash
3v0 plugins enable observability/nemo_relay
```

The `EV0_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `EV0_HOME` that the
agent run will use:

```bash
env EV0_HOME=/tmp/3v0-nemo-relay-test \
  3v0 plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`EV0_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`EV0_HOME` is the 3V0 profile/config home used by both
`3v0 plugins enable ...` and the later `3v0 chat ...` run. If unset,
3V0 uses the user's default home, usually `~/.3V0`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export EV0_HOME=/tmp/3v0-nemo-relay-test
3v0 plugins enable observability/nemo_relay
3v0 chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `3v0` command you run is built from the
checkout that contains this plugin. A globally installed older CLI will not see
new bundled plugins from your working tree.

```bash
uv sync
uv run 3v0 plugins enable observability/nemo_relay
uv run 3v0 chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
wheel, 3V0 installs its supported NeMo Relay runtime as a normal dependency:

```bash
uv build --wheel
python -m pip install --force-reinstall dist/ev0_agent-*.whl
3v0 plugins enable observability/nemo_relay
```
...
behavior; it does not control 3V0 shared client metrics.
...
`3v0.session.segment` / `3v0.session.segment_reason`
...
The plugin can configure exporters directly from `EV0_NEMO_RELAY_*`
...
```bash
export EV0_NEMO_RELAY_ATOF_ENABLED=1
export EV0_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export EV0_NEMO_RELAY_ATIF_ENABLED=1
export EV0_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `EV0_NEMO_RELAY_ATOF_FILENAME`
- `EV0_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `EV0_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `EV0_NEMO_RELAY_ATIF_AGENT_NAME`
- `EV0_NEMO_RELAY_ATIF_AGENT_VERSION`
- `EV0_NEMO_RELAY_ATIF_MODEL_NAME`
- `EV0_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)
...
```bash
export EV0_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
When `EV0_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`EV0_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`EV0_NEMO_RELAY_ATIF_*` fallback setup is also skipped so 3V0 does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
3V0 keeps the direct env-var fallbacks active for that run.

3V0 core routes provider and tool execution through NeMo Relay managed APIs
...
3V0 uses the dynamic-plugin activation API available in NeMo Relay 0.6 and
later. Configure native or worker plugins with 3V0-owned
...
with this 3V0-owned section. The gateway combines those records with
separate lifecycle state for enablement, trust policy, and worker environments;
the Python binding does not yet expose that resolver. 3V0 rejects
...
3V0 activates these plugins before registering its managed LLM and tool
...
For the full generic 3V0 middleware contract, see
...
with 3V0 and a local Ollama model served through the OpenAI-compatible API.

```bash
export EV0_HOME=/tmp/3v0-nemo-relay-docs/3v0-home
mkdir -p "$EV0_HOME"

cat > "$EV0_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent 3V0 session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export EV0_NEMO_RELAY_ATOF_ENABLED=1
export EV0_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/3v0-nemo-relay-docs/subagent/atof
export EV0_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export EV0_NEMO_RELAY_ATOF_MODE=overwrite
export EV0_NEMO_RELAY_ATIF_ENABLED=1
export EV0_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/3v0-nemo-relay-docs/subagent/atif
export EV0_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export EV0_NEMO_RELAY_ATIF_AGENT_NAME='3V0 Agent E2E'
export EV0_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export EV0_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"3v0.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "3V0 Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. 3V0 dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/3v0-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/3v0-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/3v0-nemo-relay-docs/workdir/beta.txt
cd /tmp/3v0-nemo-relay-docs/workdir

export EV0_NEMO_RELAY_ATOF_ENABLED=1
export EV0_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/3v0-nemo-relay-docs/parallel/atof
export EV0_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export EV0_NEMO_RELAY_ATOF_MODE=overwrite
export EV0_NEMO_RELAY_ATIF_ENABLED=1
export EV0_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/3v0-nemo-relay-docs/parallel/atif
export EV0_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export EV0_NEMO_RELAY_ATIF_AGENT_NAME='3V0 Agent E2E'
export EV0_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- 3V0 sessions map to `agent` scopes.
- 3V0 core managed provider calls map to `llm` scope start/end events.
- 3V0 core managed tool calls map to `tool` scope start/end events.
...
3V0 core owns the LLM and tool boundaries and enters NeMo Relay managed
execution while a 3V0-managed Relay consumer is active. With no shared
metrics subscriber or explicitly configured Relay plugin, 3V0 calls the
...
```bash
export EV0_NEMO_RELAY_PLUGINS_TOML=/tmp/3v0-middleware-test/plugins.toml
```
...
3V0 provider call
  -> nemo_relay.llm.execute(...)
    -> 3V0 provider adapter callback(...)

3V0 tool call
  -> nemo_relay.tools.execute(...)
    -> 3V0 authorization and dispatch callback(...)
```
...
middleware for a local 3V0 run. This path requires a NeMo Relay runtime that
supports `[components.config.tool_parallelism]`, as provided by NeMo Relay 0.6
and later.

```bash
export EV0_HOME=/tmp/3v0-middleware-test/3v0-home
mkdir -p "$EV0_HOME" /tmp/3v0-middleware-test/nemo-relay
...
cat > /tmp/3v0-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/3v0-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/3v0-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "3V0 Middleware E2E"
agent_version = "local"
...
export EV0_NEMO_RELAY_PLUGINS_TOML=/tmp/3v0-middleware-test/nemo-relay/plugins.toml
