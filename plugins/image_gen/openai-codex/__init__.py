...
Output is saved as PNG under ``$EV0_HOME/cache/images/``. Source images for
...
class OpenAICodexImageGenProvider(ImageGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            ...
            "env_vars": [],
            "post_setup_hint": (
                "Sign in with `3v0 auth codex` (or `3v0 setup` → Codex) "
        ...
        if not _read_codex_access_token():
            return error_response(
                error=(
                    ...
                    "`3v0 auth codex` (or `3v0 setup` → Codex) to sign in."
                ),
