...
three virtual model IDs so the ``3v0 tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `3v0 tools` → Image "
                    "Generation → OpenAI to configure, or `3v0 setup` "
                    "to add the key."
                ),
