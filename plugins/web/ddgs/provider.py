...
``3v0 tools`` can prompt the user to install it.
...
GIL — so the whole 3V0 process freezes through Ctrl+C/SIGTERM. Each search
...
def _run_ddgs_search_bounded(query: str, safe_limit: int) -> list[dict[str, Any]]:
    ...
    *its* GIL — it only polls a communicator thread and, on timeout/interrupt,
    ...
    """
    # Imported lazily so plugin import stays light for ``3v0 tools`` probes.
    ...
    if _test_hook:
        env["EV0_DDGS_ALLOW_TEST_HOOKS"] = "1"
...
class DDGSWebSearchProvider(WebSearchProvider):
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        ``primp`` call cannot freeze the 3V0 process (#36776, #68096).
        """
        try:
