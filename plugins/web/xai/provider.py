...
``{title, url, description, position}`` rows every other 3V0 web
provider produces.
...
prefers 3V0-managed xAI Grok OAuth (via ``3v0 auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.3V0/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``3v0 tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `3v0 auth` to sign in with "
