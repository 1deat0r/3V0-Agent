/**
 * 3V0 Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.3V0/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__EV0_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__EV0_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "3v0-kanban-dot-triage",
    todo: "3v0-kanban-dot-todo",
    ready: "3v0-kanban-dot-ready",
    running: "3v0-kanban-dot-running",
    blocked: "3v0-kanban-dot-blocked",
    review: "3v0-kanban-dot-review",
    done: "3v0-kanban-dot-done",
    archived: "3v0-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-3v0-task";
  ...
  const DOCS_URL = "https://github.com/1deat0r/3V0-Agent/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://github.com/1deat0r/3V0-Agent/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "3v0.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="3v0-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])3v0-kanban-md-code\1/i.test(attrs)) {
      return ' class="3v0-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "3v0-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "3v0-kanban-md",
  ...
  // standard `drop` event and our `3v0-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("3v0-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("3v0-kanban-column--drop");
          if (target) target.classList.add("3v0-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("3v0-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("3v0-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("3v0-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__EV0_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "3v0-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "3v0-kanban-attention",
        "3v0-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "3v0-kanban-attention-bar" },
        h("span", { className: "3v0-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "3v0-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "3v0-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "3v0-kanban-attention-row",
                  "3v0-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "3v0-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "3v0-kanban-attention-row-id" }, task.id),
                h("span", { className: "3v0-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "3v0-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "3v0-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "3v0-kanban-diag-action-btn",
      action.suggested ? "3v0-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " 3v0-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".3v0-kanban-drawer-comment-row input, .3v0-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "3v0-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("3v0-kanban-diag", sevClass) },
      h("div", { className: "3v0-kanban-diag-header" },
        h("span", { className: "3v0-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "3v0-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "3v0-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "3v0-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "3v0-kanban-diag-data-row" },
                  h("span", { className: "3v0-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "3v0-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "3v0-kanban-diag-data-row" },
                h("span", { className: "3v0-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "3v0-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "3v0-kanban-diag-reassign-row" },
            h("span", { className: "3v0-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "3v0-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "3v0-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "3v0-kanban-diag-msg",
              msg.ok ? "3v0-kanban-diag-msg--ok" : "3v0-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "3v0-kanban-section" },
      h("div", { className: "3v0-kanban-section-head-row" },
        h("span", { className: "3v0-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "3v0-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "3v0-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "3v0-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "3v0-kanban-docs-link",
      title: "Open 3V0 Kanban docs in a new tab",
      "aria-label": "3V0 Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `3v0 kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "3v0-kanban-msg-ok" : "3v0-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "3v0-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "3v0-kanban-boardswitcher" },
      h("div", { className: "3v0-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "3v0-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "3v0-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "3v0-kanban-dialog-title" },
        ...
        h("div", { className: "3v0-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned 3V0 profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "3v0-kanban-bulk" },
      h("span", { className: "3v0-kanban-bulk-count" },
      ...
      h("div", { className: "3v0-kanban-bulk-priority",
      ...
      h("div", { className: "3v0-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different 3V0 profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "3v0-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("3v0-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("3v0-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "3v0-kanban-trash",
        dragOver ? "3v0-kanban-trash--drop" : "",
        props.draggingTaskId ? "3v0-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "3v0-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "3v0-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".3v0-kanban-card")) return true;
      if (target.closest && target.closest(".3v0-kanban-column-add")) return true;
      if (target.closest && target.closest(".3v0-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("3v0-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("3v0-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".3v0-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ...
      className: cn(
        "3v0-kanban-columns",
        isScrollable ? "3v0-kanban-columns--scrollable" : "",
        isPanning ? "3v0-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("3v0-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("3v0-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "3v0-kanban-column",
        dragOver ? "3v0-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "3v0-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "3v0-kanban-col-check",
        ...
        h("span", { className: cn("3v0-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "3v0-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "3v0-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "3v0-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "3v0-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "3v0-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "3v0-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "3v0-kanban-lane" },
                  h("div", { className: "3v0-kanban-lane-head" },
                    h("span", { className: "3v0-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "3v0-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "3v0-kanban-card--stale-red";
    if (age >= tier.amber) return "3v0-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".3v0-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "3v0-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "3v0-kanban-card",
        props.selected ? "3v0-kanban-card--selected" : "",
        props.failed ? "3v0-kanban-card--failed" : "",
        props.draggingSource ? "3v0-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "3v0-kanban-card-content" },
          h("div", { className: "3v0-kanban-card-row" },
            h("label", {
              className: "3v0-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "3v0-kanban-card-check",
            ...
            h("span", { className: "3v0-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or 3v0 kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "3v0-kanban-warning-badge",
                    "3v0-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            progress
              ? h("span", {
                  className: cn(
                    "3v0-kanban-progress",
                    progress.done === progress.total ? "3v0-kanban-progress--full" : "",
                  ),
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "3v0-kanban-needs-assignee",
          ...
          h("div", { className: "3v0-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "3v0-kanban-card-row 3v0-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "3v0-kanban-assignee",
                            title: `Assigned to 3V0 profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "3v0-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "3v0-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "3v0-kanban-ago",
  ...
  // centered modal reusing the 3v0-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "3v0-kanban-dialog 3v0-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "3V0 profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "3V0 profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "3v0-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "3v0-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "3v0-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "3v0-kanban-drawer-close",
        ...
        data ? h("div", { className: "3v0-kanban-drawer-comment-foot" },
          h("div", {
            className: "3v0-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "3v0-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "3v0-kanban-section" },
      h("div", { className: "3v0-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "3v0-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "3v0-kanban-drawer-body" },
      h("div", { className: "3v0-kanban-drawer-title" },
        h("span", { className: cn("3v0-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "3v0-kanban-section" },
            h("div", { className: "3v0-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "3v0-kanban-section" },
            h("div", { className: "3v0-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "3v0-kanban-done-no-result 3v0-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "3v0-kanban-section" },
            h("div", { className: "3v0-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "3v0-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "3v0-kanban-section" },
        h("div", { className: "3v0-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "3v0-kanban-comment" },
            h("div", { className: "3v0-kanban-comment-head" },
              h("span", { className: "3v0-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "3v0-kanban-diag-action-btn",
      ...
      h("div", { className: "3v0-kanban-section" },
        h("div", { className: "3v0-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "3v0-kanban-comment" },
                h("div", { className: "3v0-kanban-comment-head" },
                  h("span", { className: "3v0-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "3v0-kanban-comment-ago" },
      ...
      h("div", { className: "3v0-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "3v0-kanban-event",
              isDiag ? "3v0-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "3v0-kanban-event-header" },
                  h("span", { className: "3v0-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "3v0-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "3v0-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "3v0-kanban-event-header-plain" },
                  h("span", { className: "3v0-kanban-event-kind" }, e.kind),
                  h("span", { className: "3v0-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "3v0-kanban-event-phantom-row" },
                  h("span", { className: "3v0-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "3v0-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "3v0-kanban-event-payload" },
    ...
    return h("div", { className: "3v0-kanban-section" },
      h("div", { className: "3v0-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "3v0-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `3v0-kanban-run--${r.outcome || r.status || "ended"}`
          : "3v0-kanban-run--active";
        return h("div", { key: r.id, className: cn("3v0-kanban-run", outcomeClass) },
          h("div", { className: "3v0-kanban-run-head" },
            h("span", { className: "3v0-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "3v0-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "3v0-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "3v0-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "3v0-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "3v0-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "3v0-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "3v0-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "3v0-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "3v0-kanban-pre 3v0-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "3v0-kanban-meta-row" },
      h("span", { className: "3v0-kanban-meta-label" }, props.label),
      h("span", { className: "3v0-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "3v0-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "3v0-kanban-meta-row" },
        h("span", { className: "3v0-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "3v0-kanban-meta-value 3v0-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "3v0-kanban-meta-row" },
        h("span", { className: "3v0-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "3v0-kanban-meta-value 3v0-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "3v0-kanban-meta-row" },
        h("span", { className: "3v0-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "3v0-kanban-meta-value 3v0-kanban-editable",
    ...
    return h("div", { className: "3v0-kanban-meta-row" },
      h("span", { className: "3v0-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "3v0-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "3v0-kanban-section" },
      h("div", { className: "3v0-kanban-section-head-row" },
        h("span", { className: "3v0-kanban-section-head" }, tx(t, "description", "Description")),
        editing
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "3v0-kanban-section" },
      h("div", { className: "3v0-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "3v0-kanban-deps-row" },
        h("span", { className: "3v0-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "3v0-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "3v0-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "3v0-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "3v0-kanban-dep-chip-x",
      ...
      h("div", { className: "3v0-kanban-deps-row" },
        h("span", { className: "3v0-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "3v0-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "3v0-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "3v0-kanban-section" },
      ...
      h("div", { className: "3v0-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "3v0-kanban-home-sub 3v0-kanban-home-sub--on"
              : "3v0-kanban-home-sub",
          }, label);
        })
      )
  ...
  if (window.__EV0_PLUGINS__ && typeof window.__EV0_PLUGINS__.register === "function") {
    window.__EV0_PLUGINS__.register("kanban", KanbanPage);
  }
})();
