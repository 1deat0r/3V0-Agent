...
``threev0_cli.kanban_db`` or a direct SQL query. Writes use the same code
...
the in-browser PTY bridge in ``threev0_cli/web_server.py``.
...
from threev0_cli import kanban_db
...
def _ws_upgrade_authorized(ws: "WebSocket") -> bool:
    ...
    WS auth gate (``threev0_cli.web_server._ws_auth_ok``).
    ...
    """
    try:
        from threev0_cli import web_server as _ws
    except Exception:
...
def _compute_task_diagnostics(
    ...
    Uses ``threev0_cli.kanban_diagnostics`` — see that module for the
    rule definitions.
    """
    from threev0_cli import kanban_diagnostics as kd
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from threev0_cli.kanban_diagnostics import SEVERITY_ORDER
...
from threev0_cli.kanban_db import (  # noqa: E402
...
# spawn failures, stuck-blocked). See threev0_cli.kanban_diagnostics for
# the rule engine.
# ---------------------------------------------------------------------------
...
def specify_task_endpoint(
    ...
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        ...
        from threev0_cli import kanban_specify  # noqa: WPS433 (intentional)
...
def model_options():
    ...
    Thin wrapper around ``threev0_cli.inventory.build_models_payload`` — the
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    """
    try:
        from threev0_cli import profile_describer  # noqa: WPS433 (intentional)
...
def decompose_task_endpoint(
    ...
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        from threev0_cli import kanban_decompose  # noqa: WPS433 (intentional)
