...
- OpenViking server running and reachable from 3V0

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
3V0 can identify older servers that expose the legacy status-only health
...
```bash
3v0 memory setup    # select "openviking"
```

The setup can link to an existing `~/.openviking/ovcli.conf`, copy its current
connection values into 3V0, or create a minimal `ovcli.conf` when one does
not exist.

Or manually:

```bash
3v0 config set memory.provider openviking
```

Add the connection settings to the active profile's `.env` file. For the
default profile that is `~/.3V0/.env`; for a named profile use
`~/.3V0/profiles/<profile>/.env`.
...
# OPENVIKING_AGENT=3v0
```

## Config

OpenViking's server config is separate from 3V0:
...
3V0-side provider config is read from environment variables in the active
profile's `.env`:
...
| `OPENVIKING_AGENT` | `3v0` | 3V0 peer ID in OpenViking, used for peer-scoped memories |

When `OPENVIKING_API_KEY` is set, 3V0 lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
3V0 sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
...
3V0 built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| 3V0 action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under the configured peer memory namespace |

Built-in `replace` and `remove` operations are not mirrored because 3V0
...
file URIs, such as
`viking://user/peers/3v0/memories/preferences/mem_abc123.md` or the canonical
`viking://user/default/peers/3v0/memories/preferences/mem_abc123.md`. Files
