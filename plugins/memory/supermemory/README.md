...
- Hosted: API key from [app.supermemory.ai/integrations?connect=3v0](http://app.supermemory.ai/integrations?connect=3v0)
...
```bash
3v0 memory setup    # select "supermemory"
```

Or manually:

```bash
3v0 config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.3V0/.env
```
...
Before running `3v0 memory setup`, add the local endpoint to
`$EV0_HOME/supermemory.json`:
...
Then run `3v0 memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$EV0_HOME/supermemory.json`
...
| `container_tag` | `3v0` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `3v0-{identity}` → `3v0-coder`). |
...
`https://api.supermemory.ai`. 3V0 resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: 3v0`, and document writes stamp
`metadata.sm_source: 3v0`. This is a **functional routing key, not telemetry**:
it groups 3V0-written memories into a dedicated "3V0" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per 3V0 profile:

```json
{
  "container_tag": "3v0-{identity}"
}
```

For a profile named `coder`, this resolves to `3v0-coder`. The default profile resolves to `3v0-default`. Without `{identity}`, all profiles share the same container.
