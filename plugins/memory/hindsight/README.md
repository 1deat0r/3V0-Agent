...
```bash
3v0 memory setup    # select "hindsight"
```
...
```bash
3v0 config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.3V0/.env
```
...
3V0 spins up a local Hindsight daemon with built-in PostgreSQL. Requires an LLM API key for memory extraction and synthesis. The daemon starts automatically in the background on first use and stops after 5 minutes of inactivity.
...
Daemon startup logs: `~/.3V0/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

To open the Hindsight web UI (local embedded mode only):
```bash
hindsight-embed -p 3v0 ui start
```
...
Config file: `~/.3V0/hindsight/config.json`
...
| Key | Default | Description |
|-----|---------|-------------|
| `bank_id` | `3v0` | Memory bank name (static fallback used when `bank_id_template` is unset or resolves empty) |
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `3v0-{profile}` isolates memory per active 3V0 profile. Empty placeholders collapse cleanly (e.g. `3v0-{user}` with no user becomes `3v0`). |
...
| `retain_context` | `conversation between 3V0 Agent and the User` | Context label for retained memories |
| `retain_tags` | — | Default tags applied to retained memories; merged with per-call tool tags |
| `retain_source` | — | Opt-in `metadata.source` attached to retained memories (identifies the storing client, e.g. `3v0`). Empty by default — no attribution tag ships unless you set it. |
...
The LLM API key is stored in `~/.3V0/.env` as `HINDSIGHT_LLM_API_KEY`.
