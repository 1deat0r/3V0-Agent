...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: 3v0)
  ...
  HINDSIGHT_RETAIN_SOURCE          — metadata source value attached to retained memories (default: 3v0)
  HINDSIGHT_RETAIN_USER_PREFIX     — label used before user turns in retained transcripts
  HINDSIGHT_RETAIN_ASSISTANT_PREFIX — label used before assistant turns in retained transcripts

Or via $EV0_HOME/hindsight/config.json (profile-scoped), falling back to
...
# via the ``retain_source`` config key or HINDSIGHT_RETAIN_SOURCE (e.g. "3v0").
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so 3V0 can degrade gracefully instead of repeatedly trying to start
    ...
    healthy and ``3v0 memory status`` would stay green while the daemon
...
def _local_runtime_hint(reason: str | None) -> str:
    ...
    if "no module named" in text and ("hindsight'" in text or 'hindsight"' in text
        ...
        return (
            ...
            f"{sys.executable} hindsight-all — or run '3v0 memory setup'. "
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $EV0_HOME/hindsight/config.json  (profile-scoped)
    ...
    return {
        ...
        "banks": {
            "3v0": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "3v0"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this 3V0 config."""
    profile = config.get("profile", "3v0")
    return str(profile or "3v0")
...
def _load_simple_env(path) -> dict[str, str]:
    ...
    values: dict[str, str] = {}
    # utf-8-sig, not plain utf-8: this is also used on the 3V0 .env during
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active 3V0 profile name (from agent_identity)
      {workspace} — 3V0 workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``3v0-{user}`` with no user becomes ``3v0``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between 3V0 Agent and the User"
    ...
    def post_setup(self, threev0_home: str, config: dict) -> None:
        ...
        # data-volume target instead of writing to /opt/3v0 (NS-605).
        ...
        provider_config.setdefault("bank_id", "3v0")
    ...
    def _offer_starter_template(self, mode: str, provider_config: dict, env_writes: dict) -> None:
        ...
        bank_id = provider_config.get("bank_id", "3v0")
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "3v0"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: 3v0-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between 3V0 Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "3v0"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "3v0"),
    ...
    def _is_retain_op_complete(self, bank_id: str, op_id: str) -> bool:
        ...
        try:
            ...
            # Optional client SDK absent (hindsight extra not installed): we
            # cannot poll server-side op status. Treat as not-complete so the
            # caller's deadline loop drops it (bounded), never crash the
            # prefetch thread with an unhandled import error.
            return False
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            if Version(installed) < Version(_MIN_CLIENT_VERSION):
                ...
                # durable data-volume target instead of /opt/3v0 (NS-605).
                from tools.lazy_deps import install_specs
        ...
        banks = cfg_get(self._config, "banks", "3v0", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "3v0")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between 3V0 Agent and the User")
        ...
        # 3V0 emits a "👁️ Hindsight — recalled N memories" status line so the
        ...
        if self._mode == "local_embedded":
            ...
            if hasattr(os, "geteuid") and os.geteuid() == 0:
                msg = (
                    ...
                    "memory daemon. Run 3V0 as a non-root user, or switch "
                    "to cloud / local_external mode via '3v0 memory setup'."
                )
                logger.warning(msg)
                # Surface to the terminal too — a daemon that never starts
                # would otherwise fail silently and the user would only see
                # 3V0 get sluggish. (issue #13125)
                try:
    ...
    def prefetch(self, query: str, *, session_id: str = "") -> str:
        ...
        # turn's queued recall. See 1deat0r/3V0-Agent#5820.
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When 3V0 created/used that client on the shared async loop,
