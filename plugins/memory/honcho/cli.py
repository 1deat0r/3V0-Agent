...
def _local_config_path() -> Path:
    ...
    """
    return get_ev0_home() / "honcho.json"
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        ev0_host.get("pinUserPeer"),
        ev0_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in ev0_host:
        aliases_src = ev0_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in ev0_host:
        prefix_src = ev0_host.get("runtimePeerPrefix")
...
def _scrub_identity_mapping(ev0_host: dict) -> None:
...
def _gateway_platforms() -> list[str] | None:
    ...
    guarded (matching the idiom ev0_cli already uses for gateway refs).
    """
    try:
...
def _apply_runtime_prefix(
    ev0_host: dict, current_prefix: str, prefix_from_root: bool, label: str
) -> None:
    ...
    if new_prefix and not (prefix_from_root and new_prefix == current_prefix):
        ev0_host["runtimePeerPrefix"] = new_prefix


def _echo_identity_mapping(ev0_host: dict) -> None:
    """Show the resulting keys so the operator can verify what was written."""
    aliases = ev0_host.get("userPeerAliases")
    prefix = ev0_host.get("runtimePeerPrefix")
    print("  resolved →")
    print(f"    pinUserPeer       = {bool(ev0_host.get('pinUserPeer'))}")
...
def _configure_raw_identity_mapping(
    ...
    _scrub_identity_mapping(ev0_host)
    ev0_host["pinUserPeer"] = pin
    if pin:
        return
    aliases = (
    ...
    while True:
        ...
        if "=" in entry:
            ...
            if rid and peer:
                aliases[rid] = peer
    if aliases:
        ev0_host["userPeerAliases"] = aliases
    _apply_runtime_prefix(
        ev0_host, current_prefix, prefix_from_root,
...
def cmd_setup(args) -> None:
    ...
    if is_local:
        ...
        current_host_key = ev0_host.get("apiKey", "")
        masked = (
    ...
    if not is_local:
        ...
        existing_oauth = OAuthCredential.from_host_block(ev0_host)
        ...
        if use_device:
            ...
            try:
                ...
                return
            print(" approved")
            _apply_grant_to_host(ev0_host, cred)
    ...
    # --- 3. Identity ---
    current_peer = ev0_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        ev0_host["peerName"] = new_peer

    current_ai = ev0_host.get("aiPeer") or cfg.get("aiPeer", "3v0")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        ev0_host["aiPeer"] = new_ai

    current_workspace = ev0_host.get("workspace") or cfg.get("workspace", "3v0")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        ev0_host["workspace"] = new_workspace
    ...
    (
        ...
        aliases_from_root,
        prefix_from_root,
    ) = _resolve_effective_identity_mapping(cfg, ev0_host)
    ...
    if run_mapping:
        peer_target = ev0_host.get("peerName") or current_peer or "user"
        ...
        if shape == "single":
            ...
            _scrub_identity_mapping(ev0_host)
            ev0_host["pinUserPeer"] = False
            if prior_aliases:
                ev0_host["userPeerAliases"] = prior_aliases
            _apply_runtime_prefix(
                ev0_host, current_prefix, prefix_from_root,
            ...
            if merged:
                ev0_host["userPeerAliases"] = merged
            _apply_runtime_prefix(
                ev0_host, current_prefix, prefix_from_root,
            ...
            _configure_raw_identity_mapping(
                ev0_host, current_pin, current_aliases, current_prefix,
    ...
    # --- 4. Observation mode ---
    current_obs = ev0_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        ev0_host["observationMode"] = new_obs
    else:
        ev0_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(ev0_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        ev0_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        ev0_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = ev0_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        ev0_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = ev0_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        ev0_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                ev0_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(ev0_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            ev0_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        ev0_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        ev0_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        ev0_host["dialecticReasoningLevel"] = new_reasoning
    else:
        ev0_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = ev0_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        ev0_host["sessionStrategy"] = new_strat

    ev0_host["enabled"] = True
    ev0_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        ev0_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(ev0_config)
...
def _headless() -> tuple[bool, bool]:
    """(is_remote, can_open_browser) — degrades safely if ev0_cli internals move."""
    try:
        from ev0_cli.auth import _can_open_graphical_browser, _is_remote_session
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        from ev0_cli.memory_setup import cmd_setup_provider
