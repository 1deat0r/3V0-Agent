...
def cmd_sync(args) -> None:
    ...
    Scans all 3V0 profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run '3v0 honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run '3v0 honcho setup' first.\n")
        return
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current 3V0 profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $EV0_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_threev0_home() / "honcho.json"
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        threev0_host.get("pinUserPeer"),
        threev0_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in threev0_host:
        aliases_src = threev0_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in threev0_host:
        prefix_src = threev0_host.get("runtimePeerPrefix")
...
def _scrub_identity_mapping(threev0_host: dict) -> None:
...
def _gateway_platforms() -> list[str] | None:
    ...
    guarded (matching the idiom threev0_cli already uses for gateway refs).
    """
    try:
...
def _apply_runtime_prefix(
    threev0_host: dict, current_prefix: str, prefix_from_root: bool, label: str
) -> None:
    ...
    if new_prefix and not (prefix_from_root and new_prefix == current_prefix):
        threev0_host["runtimePeerPrefix"] = new_prefix


def _echo_identity_mapping(threev0_host: dict) -> None:
    """Show the resulting keys so the operator can verify what was written."""
    aliases = threev0_host.get("userPeerAliases")
    prefix = threev0_host.get("runtimePeerPrefix")
    print("  resolved →")
    print(f"    pinUserPeer       = {bool(threev0_host.get('pinUserPeer'))}")
...
def _configure_raw_identity_mapping(
    ...
    _scrub_identity_mapping(threev0_host)
    threev0_host["pinUserPeer"] = pin
    if pin:
        return
    aliases = (
    ...
    while True:
        ...
        if "=" in entry:
            ...
            if rid and peer:
                aliases[rid] = peer
    if aliases:
        threev0_host["userPeerAliases"] = aliases
    _apply_runtime_prefix(
        threev0_host, current_prefix, prefix_from_root,
...
def _ensure_sdk_installed() -> bool:
    ...
    # data-volume target instead of writing to /opt/ev0 (NS-605).
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives 3V0 persistent cross-session memory.")
    ...
    if is_local:
        ...
        current_host_key = threev0_host.get("apiKey", "")
        masked = (
        ...
        if new_local_key:
            threev0_host["apiKey"] = new_local_key
    ...
    if not is_local:
        ...
        existing_oauth = OAuthCredential.from_host_block(threev0_host)
        ...
        if use_device:
            ...
            try:
                ...
                print("\n  Cancelled. Re-run '3v0 honcho setup' to try again.\n")
                return
            except (AuthorizationTimeout, DeviceCodeExpired):
                print("\n  Device code expired before approval.")
                print("  Re-run '3v0 honcho setup' to get a new code.\n")
                return
            except AccessDenied:
                print("\n  Sign-in was denied on the approval page.")
                print("  Re-run '3v0 honcho setup' to retry, or choose an API key instead.\n")
                return
                ...
                return
            print(" approved")
            _apply_grant_to_host(threev0_host, cred)
            ...
            if not cfg.get("apiKey"):
                ...
                print("  Run '3v0 honcho setup' again once you have a key.\n")
                return

    # --- 3. Identity ---
    current_peer = threev0_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        threev0_host["peerName"] = new_peer

    current_ai = threev0_host.get("aiPeer") or cfg.get("aiPeer", "3v0")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        threev0_host["aiPeer"] = new_ai

    current_workspace = threev0_host.get("workspace") or cfg.get("workspace", "3v0")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        threev0_host["workspace"] = new_workspace

    # --- 3b. Gateway identity mapping ---
    # These keys only affect the 3V0 GATEWAY (Telegram/Discord/Slack/...),
    ...
    (
        ...
        aliases_from_root,
        prefix_from_root,
    ) = _resolve_effective_identity_mapping(cfg, threev0_host)
    ...
    if gw_platforms is None:
        ...
        run_mapping = _prompt(
            "Running the 3V0 gateway (Telegram/Discord/etc.)? (y/N)",
            default="n",
    ...
    if run_mapping:
        peer_target = threev0_host.get("peerName") or current_peer or "user"
        ...
        if shape == "single":
            ...
            _scrub_identity_mapping(threev0_host)
            threev0_host["pinUserPeer"] = False
            if prior_aliases:
                threev0_host["userPeerAliases"] = prior_aliases
            _apply_runtime_prefix(
                threev0_host, current_prefix, prefix_from_root,
            ...
            if merged:
                threev0_host["userPeerAliases"] = merged
            _apply_runtime_prefix(
                threev0_host, current_prefix, prefix_from_root,
            ...
            _configure_raw_identity_mapping(
                threev0_host, current_pin, current_aliases, current_prefix,
    ...
    # --- 4. Observation mode ---
    current_obs = threev0_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        threev0_host["observationMode"] = new_obs
    else:
        threev0_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(threev0_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        threev0_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        threev0_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = threev0_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        threev0_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = threev0_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        threev0_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                threev0_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(threev0_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            threev0_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        threev0_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        threev0_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        threev0_host["dialecticReasoningLevel"] = new_reasoning
    else:
        threev0_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = threev0_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        threev0_host["sessionStrategy"] = new_strat

    threev0_host["enabled"] = True
    threev0_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        threev0_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(threev0_config)
...
def _headless() -> tuple[bool, bool]:
    """(is_remote, can_open_browser) — degrades safely if threev0_cli internals move."""
    try:
        from threev0_cli.auth import _can_open_graphical_browser, _is_remote_session
...
def _apply_grant_to_host(threev0_host: dict, cred) -> None:
    """Store an OAuth grant on the host block; the wizard's final save persists it."""
    threev0_host["apiKey"] = cred.access_token
    threev0_host["oauth"] = cred.oauth_block()
    # Default the peer prompt to the name entered at consent.
    if cred.consent_peer_name:
        threev0_host["peerName"] = cred.consent_peer_name


def _active_profile_name() -> str:
    """Return the active 3V0 profile name (respects --target-profile override)."""
    if _profile_override:
...
def _all_profile_host_configs() -> list[tuple[str, str, dict]]:
    ...
    for p in profiles:
        ...
        # ("3v0.work") stay readable per the README's back-compat
    ...
    if not cfg:
        ...
        try:
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run '3v0 honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run '3v0 honcho setup' to configure.\n")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        ...
        print(f"  AI peer:     {ai}")
        print("    3V0' identity in Honcho. Seed with '3v0 honcho identity <file>'.")
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ...
        print("    AI-to-AI inference. 3V0 asks Honcho's AI peer a question")
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run '3v0 honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run '3v0 honcho identity <file>' to seed one.")
        print()
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → 3V0 + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → 3V0\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives 3V0 persistent memory")
        ...
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run '3v0 honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run '3v0'")
        print("  with Honcho configured and no prior session history.")
        print("  (3V0 calls migrate_memory_files() on first session init.)")
        print()
        ...
        if has_key:
            ...
            print("  Run '3v0 honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In 3V0, they are seeded once into Honcho's AI peer through the")
    ...
    print("    3V0:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    3V0:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    3V0:   Honcho observes every message and updates representations")
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running '3v0 memory setup'...\n")
        from threev0_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to 3V0 Honcho",
    )
