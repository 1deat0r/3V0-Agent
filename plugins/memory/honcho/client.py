...
Resolution order for config file:
  1. $EV0_HOME/honcho.json  (instance-local, enables isolated 3V0 instances)
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a 3V0 profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active 3V0 profile.

    Resolution order:
      1. EV0_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``ev0_<profile>``
      3. defaultHost from the active config, but only for the default profile
      4. Fallback: ``"3v0"`` (default profile)
    """
    explicit = os.environ.get("EV0_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
    ...
    # but applying it before profile resolution makes every named 3V0
    # profile share that same host.  Keep named profiles isolated; only the
    # default 3V0 profile may opt into the config's default host.
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $EV0_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.3V0/honcho.json          (default profile — shared host blocks live here)
...
class HonchoClientConfig:
    ...
    # True when Honcho was explicitly configured for this host (hosts.3v0
    ...
    # resolve_config_path()/get_ev0_home() later — those resolvers read a
    ...
    def from_global_config(
        ...
        Resolution: $EV0_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active 3V0 profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.3v0 block or explicit enabled flag means the user
        ...
        # baseUrl / base_url keys below are the 3V0-specific spelling.
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. per-session strategy — 3V0 session_id ({timestamp}_{hex}); authoritative,
             so a generated title never remaps a live conversation
          3. Manual directory override from sessions map
          4. 3V0 session title (from /title command; non-per-session)
...
def spawn_context_thread(
    ...
    *,
    ...
    (set_ev0_home_override); plain threading.Thread targets start with an
    EMPTY context, so any ambient resolution on the thread
    (resolve_config_path, resolve_active_host, get_ev0_home) silently
...
def _credential_fingerprint(config: HonchoClientConfig | None) -> str:
    ...
    makes '3v0 honcho setup' account switches produce a NEW cache identity
...
def _slot_for(key: tuple) -> SingletonSlot:
    ...
    When a (kind, host, config_path/ev0_home) identity reappears with a
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run '3v0 honcho setup' or set HONCHO_API_KEY. "
    ...
    def _build() -> "Honcho":
        ...
        try:
            ...
            raise ImportError(
                ...
                "(or run `3v0 honcho setup` to configure)."
            )
