...
``3v0://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the 3V0 OAuth client. IP-literal so
...
def begin_authorization(
    ...
    *,
    ...
    (``3v0-desktop`` / ``3v0-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to 3V0.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local EV0_HOME override can't reach.
