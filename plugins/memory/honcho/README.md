...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/3v0>
...
```bash
3v0 memory setup honcho   # configure Honcho directly (works on a fresh install)
3v0 memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
3v0 config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.3V0/.env
```

> `3v0 honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `3v0 memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `3v0 model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$EV0_HOME/honcho.json` | Profile-local (isolated 3V0 instances) |
| 2 | `~/.3V0/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active 3V0 profile: `3v0` (default) or `ev0_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `3v0 honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | 3V0 session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`3v0-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`3v0`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-3v0-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening 3v0 in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — 3V0 session ID (timestamp + hex). Every `3v0` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple 3V0 profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`3v0`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active 3V0 profile: `3v0` (default) or `ev0_<profile>` (e.g. `3v0 -p coder` -> host key `ev0_coder`). Older `3v0.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `EV0_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `3v0-agent`) |
...
|---------|-------------|
| `3v0 memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `3v0 honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `3v0 memory setup`) |
| `3v0 honcho status` | Show resolved config for active profile |
| `3v0 honcho enable` / `disable` | Toggle Honcho for active profile |
| `3v0 honcho mode <mode>` | Change recall or observation mode |
| `3v0 honcho peer --user <name>` | Update user peer name |
| `3v0 honcho peer --ai <name>` | Update AI peer name |
| `3v0 honcho tokens --context <N>` | Set context token budget |
| `3v0 honcho tokens --dialectic <N>` | Set dialectic max chars |
| `3v0 honcho map <name>` | Map current directory to a session name |
| `3v0 honcho sync` | Create host blocks for all 3V0 profiles |
