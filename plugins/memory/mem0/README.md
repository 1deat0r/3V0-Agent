...
```bash
3v0 memory setup    # select "mem0"
```

Or manually:
```bash
3v0 config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.3V0/.env
```

## Config

Behavioral settings live in `$EV0_HOME/mem0.json` (set them via `3v0 memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.3V0/.env`.
...
| `user_id` | `3v0-user` | User identifier on Mem0 |
| `agent_id` | `3v0` | Agent identifier |
...
2. Point the plugin at it — via the setup wizard:
   ```bash
   3v0 memory setup    # select "mem0" → "Self-hosted server"
   # Or non-interactive:
   3v0 memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
   ```
   or via env vars:
   ```bash
   echo "MEM0_HOST=http://localhost:8888" >> ~/.3V0/.env
   echo "MEM0_API_KEY=your-admin-api-key" >> ~/.3V0/.env
   ```
   or in `$EV0_HOME/mem0.json`:
   ```json
   {
   ...
   ```
3. Start a fresh 3V0 session and call `mem0_search` — it connects to your server.
...
```bash
3v0 memory setup mem0 --mode oss \
...
```bash
3v0 memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$EV0_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.3V0/mem0_qdrant"}}
  }
}
```
...
```bash
3v0 memory setup mem0 --mode platform --api-key sk-...
```

### Dry Run (preview without writing)

```bash
3v0 memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```
...
- Check `user_id` matches between sessions (`$EV0_HOME/mem0.json`).
