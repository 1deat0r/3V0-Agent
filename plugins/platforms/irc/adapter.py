"""
IRC Platform Adapter for 3V0 Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the 3V0 agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.server = wire_env("IRC_SERVER") or extra.get("server", "")
        try:
            self.port = int(wire_env("IRC_PORT") or extra.get("port", 6697))
        except (ValueError, TypeError):
            self.port = 6697
        self.nickname = wire_env("IRC_NICKNAME") or extra.get("nickname", "3v0-bot")
        self.channel = wire_env("IRC_CHANNEL") or extra.get("channel", "")
        self.use_tls = (
            (wire_env("IRC_USE_TLS") or "").lower() in {"1", "true", "yes"}
            if wire_env("IRC_USE_TLS")
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :3V0 Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :3V0 Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: ev0_, ev0_1, ev0_2...
...
def validate_config(config) -> bool:
    ...
    extra = getattr(config, "extra", {}) or {}
    server = wire_env("IRC_SERVER") or extra.get("server", "")
    channel = wire_env("IRC_CHANNEL") or extra.get("channel", "")
    return bool(server and channel)
...
def interactive_setup() -> None:
    ...
    print_info("Connect 3V0 to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #3v0 — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.3V0/.env")
    print_info("Restart the gateway for changes to take effect: 3v0 gateway restart")
...
def _env_enablement() -> dict | None:
    ...
    """
    server = (wire_env("IRC_SERVER") or "").strip()
    channel = (wire_env("IRC_CHANNEL") or "").strip()
    ...
    port = (wire_env("IRC_PORT") or "").strip()
    if port:
        try:
            seed["port"] = int(port)
        except ValueError:
            pass
    nickname = (wire_env("IRC_NICKNAME") or "").strip()
    if nickname:
        seed["nickname"] = nickname
    use_tls = (wire_env("IRC_USE_TLS") or "").strip().lower()
    if use_tls:
    ...
    home = wire_env("IRC_HOME_CHANNEL") or channel
    if home:
        seed["home_channel"] = {
            "chat_id": home,
            "name": wire_env("IRC_HOME_CHANNEL_NAME", home),
        }
    return seed
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``3v0 cron`` running as a
    separate process from ``3v0 gateway``).  Without this hook,
    ...
    port_value = wire_env("IRC_PORT") or extra.get("port", 6697)
    try:
        port = int(port_value)
    except (TypeError, ValueError):
        return {"error": f"IRC standalone send: invalid port {port_value!r}"}

    nickname = wire_env("IRC_NICKNAME") or extra.get("nickname", "3v0-bot")
    use_tls_env = wire_env("IRC_USE_TLS")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "3v0-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :3V0 Agent (cron)")
