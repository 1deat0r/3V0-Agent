"""ntfy platform adapter (3V0 plugin).
...
This adapter ships as a 3V0 platform plugin under
``plugins/platforms/ntfy/``. The 3V0 plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "3v0-in"              # subscribe topic (incoming)
          publish_topic: "3v0-out"     # optional — defaults to topic
...
_ECHO_TAG = "3v0-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``3v0 cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a 3V0 dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `3v0 gateway status` without
