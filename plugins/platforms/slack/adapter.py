...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `3v0 slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not raw_token:
            logger.error(
                ...
                "error; set SLACK_BOT_TOKEN via `3v0 gateway setup` "
                "or in the active profile's ~/.3V0/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_bot_token",
                "SLACK_BOT_TOKEN not configured. Use `3v0 gateway setup` "
                "or add it to your active profile's ~/.3V0/.env file, "
        ...
        if not app_token:
            logger.error(
                ...
                "error; set SLACK_APP_TOKEN via `3v0 gateway setup` "
                "or in the active profile's ~/.3V0/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_app_token",
                "SLACK_APP_TOKEN not configured. Use `3v0 gateway setup` "
                "or add it to your active profile's ~/.3V0/.env file, "
                "then restart the gateway.",
                retryable=False,
            )
        ...
        try:
            ...
            primary_client = AsyncWebClient(
                token=primary_token,
                user_agent_prefix=_EV0_SLACK_USER_AGENT_PREFIX,
            )
            ...
            # 3V0 has no listener for (e.g. user_change,
            ...
            async def handle_unhandled_event(event, body, logger):
                logger.debug(
                    ...
                    "registered; subscribed events not handled by 3V0 can "
            ...
            # /model work directly without /3v0 prefix). A single regex
            ...
            # manifest (see `3v0 slack manifest`). In Socket Mode, Slack
            ...
            self._app.action("ev0_feedback")(self._handle_feedback_action)
            ...
            self._app.action(
                _re.compile(r"^ev0_clarify_choice_\d+$")
            )(self._handle_clarify_action)
            self._app.action("ev0_clarify_other")(self._handle_clarify_action)
            ...
            if _allow_bots_cfg != "none":
                logger.info(
                    ...
                    "'3v0 slack manifest' if unsure), and (b) the other bot's "
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: 3V0 handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own 3V0 session — matching the per-thread behavior channels already
    ...
    async def _handle_app_context_changed(
        ...
        # 3V0 owns. Do not write it into _channel_team: channel IDs can be
    ...
    async def _handle_slack_message(
        ...
        # the shape produced by peer 3V0 agents in Socket Mode on some
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "ev0_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "ev0_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
    ...
    async def send_clarify(
        ...
        (unique ``ev0_clarify_choice_<idx>`` action_id, ``value`` packs
        ``clarify_id|idx``) plus a final "✏️ Other…" button
        (``ev0_clarify_other``).  A choice click resolves the clarify
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "ev0_confirm_once": "once",
            "ev0_confirm_always": "always",
            "ev0_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "ev0_approve_once": "once",
            "ev0_approve_session": "session",
            "ev0_approve_always": "always",
            "ev0_deny": "deny",
        }
    ...
    async def _handle_clarify_action(self, ack, body, action) -> None:
        ...
        if action_id == "ev0_clarify_other" or token == "other":
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/3v0 <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/3v0
        ...
        if slash_name in {"3v0", ""}:
            # Legacy /3v0 <subcommand> [args] routing + free-form questions.
            ...
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/3v0   ``).
        ...
        # "Running /cmd…" ack shown by handle_ev0_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/3v0 <question>" must produce public replies so
...
def interactive_setup() -> None:
    ...
    def _write_slack_manifest_and_instruct() -> None:
        ...
        try:
            ...
            print_info(
                "   Re-run `3v0 slack manifest --write` anytime to refresh after "
                "3V0 adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `3v0 update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://github.com/1deat0r/3V0-Agent/docs/user-guide/messaging/slack/")
    print()
