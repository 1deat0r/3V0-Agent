...
# ``model_tools`` import time, so ``3v0 status``, ``3v0 chat``, etc.
...
# ``ev0_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "3V0 is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a 3v0 MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "3v0-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${EV0_HOME}/google_chat_thread_counts.json so
        ...
        # they don't sit in the chat forever as "3V0 is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("EV0_HOME", str(_Path.home() / ".3V0"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${EV0_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a 3v0 MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible '3V0 is thinking…' marker message.
        ...
        "3V0 is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "3V0 is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "3V0 is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "3V0 is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``3v0 setup``.

    The setup wizard at ``threev0_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.3V0/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. 3v0-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.3V0/.env")
    print_info("Restart the gateway: 3v0 gateway restart")
...
def register(ctx) -> None:
    """Plugin entry point — called by the 3V0 plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="Run `3v0 setup` to install Google Chat support.",
        ...
        # the "3V0 is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a '3V0 is thinking…' marker message "
