...
and injects content-free wake hints into 3V0' normal gateway session pipeline.
...
class RaftAdapter(BasePlatformAdapter):
    ...
    async def _handle_wake(self, request: "web.Request") -> "web.Response":
        ...
        # 3V0 only verifies that wake hints are content-free.
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        """Accept Raft wake hints without interrupting an active 3V0 turn."""
        if not self._message_handler:
            return
...
def interactive_setup() -> None:
    """Interactive ``3v0 gateway setup`` flow for the Raft platform.
    ...
    print_info("Connect 3V0 to Raft as an external agent.")
