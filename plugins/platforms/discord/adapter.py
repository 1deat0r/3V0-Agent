...
def _needs_server_members_intent(
    ...
    """Return True when 3V0 must request Discord's Server Members intent.
...
def _format_privileged_intents_guidance(*, needs_members: bool) -> str:
    ...
    lines.extend(
        [
            ...
            "Docs: https://github.com/1deat0r/3V0-Agent/docs/user-guide/messaging/discord",
        ]
    )
    return "\n".join(lines)
...
def _discord_ready_timeout_seconds() -> float:
    ...
    if raw:
        try:
            ...
            logger.warning(
                "Ignoring invalid EV0_GATEWAY_PLATFORM_CONNECT_TIMEOUT=%r",
                raw,
            )
    return 30.0
...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.3V0/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = env_float("EV0_DISCORD_TEXT_BATCH_DELAY_SECONDS", 0.6)
        self._text_batch_split_delay_seconds = env_float("EV0_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0)
        ...
        self._liveness_interval_seconds = self._finite_positive_config_float(
            ...
            15.0,
            env_key="EV0_DISCORD_LIVENESS_INTERVAL_SECONDS",
        )
        self._liveness_failure_threshold = self._config_int(
            "websocket_liveness_failure_threshold",
            2,
            env_key="EV0_DISCORD_LIVENESS_FAILURE_THRESHOLD",
        )
    ...
    def _handle_bot_task_done(self, task: asyncio.Task) -> None:
        ...
        marked running, the Discord websocket is dead while the 3V0 gateway
    ...
    def _classify_connect_exception(self, error: Exception) -> tuple:
        ...
        if _is("PrivilegedIntentsRequired"):
            # Name the exact intents 3V0 requested (#79430): Message
    ...
    async def cancel_background_tasks(self) -> None:
        ...
        disconnect budget (``EV0_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT``, default
    ...
    async def _run_missed_message_backfill(self) -> None:
        ...
        that lack a substantive non-outage 3V0 response. Emoji-only acks are
    ...
    async def _should_backfill_discord_message(self, message: Any) -> bool:
        """Return True when a recent Discord message still needs 3V0 work."""
    ...
    def _is_down_notice_content(self, content: str) -> bool:
        """Recognize only explicit 3V0/gateway outage notices."""
        text = (content or "").lower()
        subject = r"(?:3v0|the agent|agent|the gateway|gateway|bmo)"
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields 3V0 manages."""
    ...
    def _is_pairing_approved_user(self, user_id: str) -> bool:
        """True when the Discord user has an explicit 3V0 pairing grant."""
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your 3V0 session")
        ...
        @tree.command(name="status", description="Show 3V0 session status")
        ...
        @tree.command(name="stop", description="Stop the running 3V0 agent")
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.3V0/skills/ for new or removed skills")
        ...
        @tree.command(name="update", description="Update 3V0 Agent to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the 3V0 gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a 3V0 session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to 3V0 in the thread",
        ...
        # threev0_cli/commands.py automatically appear as Discord slash
        ...
        dropped_over_cap = 0
        try:
            from threev0_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
        ...
        try:
            from threev0_cli.commands import _iter_plugin_command_entries
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from threev0_cli.commands import discord_skill_commands_by_category
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new 3V0 session in the thread
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by 3V0: **{name}**"
    ...
    def _derive_auto_thread_name(self, content: str) -> str:
        ...
        3V0 has an LLM-generated session title and can safely rename
        ...
        thread_name = content[:80] if content else "3V0"
        if len(content) > 80:
            ...
            try:
                ...
                try:
                    seed_msg = await message.channel.send(
                        f"\U0001f9f5 Thread created by 3V0: **{thread_name}**"
                    )
                    thread = await seed_msg.create_thread(
                        name=thread_name,
    ...
    async def rename_thread(
        ...
        *,
        ...
        if edit is None:
            return False
        try:
            await edit(name=cleaned, reason="3V0 semantic session title")
            logger.info(
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 3V0 handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            prompt_prefix = (
                ...
                "Do you want 3V0 to run this command?\n\n"
                "**Requested command:**\n```bash\n"
            )
            if smart_denied:
    ...
    async def send_clarify(
        ...
        try:
            ...
            content = self._self_contained_prompt_content(
                "❓ **3V0 needs your input**", str(question or "").strip(),
                tail=clarify_tail,
            )
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``3v0 update --gateway``
                ...
                if thread:
                    ...
                    try:
                        await message.channel.send(
                            "⚠️ 3V0 could not create a Discord thread for "
...
def _component_check_auth(
    ...
    the pairing store (``3v0 pairing approve``).
    ...
    # so users approved via ``3v0 pairing approve`` can interact with
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``3v0 update`` prompts.
...
# is not in this process (e.g. ``3v0 cron`` running standalone) and no live
...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    _websocket_liveness_keys = (
        (
            ...
            "EV0_DISCORD_LIVENESS_INTERVAL_SECONDS",
        ),
        (
            "websocket_liveness_failure_threshold",
            "liveness_failure_threshold",
            "EV0_DISCORD_LIVENESS_FAILURE_THRESHOLD",
        ),
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``threev0_cli.gateway.get_env_value`` at call time (not via
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `3v0 setup` to install Discord support.",
        # Interactive setup wizard — replaces the central
        # threev0_cli/setup.py::_setup_discord function.  Same shape as Teams.
