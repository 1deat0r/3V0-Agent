# A2A — Agent-to-Agent protocol for 3V0
...
peer (another 3V0, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
3v0 gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, 3V0 serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.3V0/a2a_audit.jsonl`.
- Conversations persist to `~/.3V0/a2a_conversations/` — they survive context
