"""
A2A inbound platform adapter — exposes 3V0 as an A2A-discoverable agent.
...
def _default_agent_name() -> str:
    ...
    try:
        import socket
        return f"3v0-{socket.gethostname()}"
    except Exception:
        return "3v0-agent"
...
def _active_profile_name() -> str:
    try:
        ...
        return os.getenv("EV0_PROFILE", "default") or "default"
...
class A2AAdapter(BasePlatformAdapter):
    ...
    def _load_served_agents(self, extra: dict) -> dict[str, dict]:
        ...
        default_desc = os.getenv(
            ...
            "3V0 Agent — a general-purpose agent reachable over A2A.",
        )
        agents[""] = {
            "slug": "",
            "path": "",
        ...
        for key, val in items:
            ...
            agents[slug] = {
                ...
                "local": local,
                "name": str(val.get("name") or f"3V0 {slug}"),
                "description": str(val.get("description") or f"3V0 profile '{profile or slug}' exposed over A2A."),
    ...
    def _build_card(self, public_url: Optional[str] = None, agent: Optional[dict] = None) -> dict:
        ...
        return protocol.build_agent_card(
            ...
            url=url,
            description=agent.get("description") or "3V0 Agent — a general-purpose agent reachable over A2A.",
    ...
    def _forward_to_profile(self, agent: dict, peer: str, context_id: str, framed_text: str) -> tuple[str, str]:
        """Forward a routed A2A task to another local 3V0 profile.
        ...
        with lock:
            ...
            cmd = ["3v0", "chat", "-q", framed_text, "-Q", "--source", "a2a"]
            if session_id:
