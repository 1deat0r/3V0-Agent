"""
Email platform adapter for the 3V0 gateway.

Allows users to interact with 3V0 by sending emails.
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `3v0 config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `3v0 gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "3V0 Agent")
        ...
        msg_id = f"<3v0-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
