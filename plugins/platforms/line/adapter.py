"""
LINE Messaging API platform adapter for 3V0 Agent.
...
to 3V0 Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``EV0_HOME``. PR #8398.
        """
        from aiohttp import web
...
def is_connected(config) -> bool:
    """Surface in ``3v0 status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``3v0 status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``3v0 setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.3V0/.env`` via ``threev0_cli.config``.
    """
    print()
    ...
    try:
        from threev0_cli.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("threev0_cli.config not available; set LINE_* vars manually in ~/.3V0/.env")
        return
