...
# trigger its own recovery path. Refs: NousResearch/3v0-agent#59614
...
# _UPDATER_STOP_TIMEOUT. Refs: NousResearch/3v0-agent#66377
_DRAIN_TIMEOUT = 15.0
...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._media_batch_delay_seconds = env_float("EV0_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS", 0.8)
        ...
        self._text_batch_delay_seconds = self._env_float_clamped(
            "EV0_TELEGRAM_TEXT_BATCH_DELAY_SECONDS",
            0.3,
            min_value=0.08,
            max_value=2.0,
        )
        self._text_batch_split_delay_seconds = self._env_float_clamped(
            "EV0_TELEGRAM_TEXT_BATCH_SPLIT_DELAY_SECONDS",
            1.0,
    ...
    def _thread_kwargs_for_send(
        ...
        metadata. 3V0-created private-chat topic lanes are marked with
        ...
        queued follow-ups after a gateway restart) prefer the 3V0 topic's
        ``message_thread_id`` so they stay in the active topic lane (#87051);
        ``direct_messages_topic_id`` is only used when no topic thread
        resolves, since the native DM-topic id does not match the 3V0
        ...
        if metadata and metadata.get("telegram_dm_topic_reply_fallback"):
            ...
            if reply_to_message_id is None:
                ...
                # active topic lane: prefer the 3V0 topic thread id when it
    ...
    # older PTB/clients. Streaming edits stay on 3V0' existing MarkdownV2
    ...
    def _has_telegram_desktop_details_math_crash_shape(self, content: str) -> bool:
        ...
        3V0 must skip rich delivery up front and use the legacy MarkdownV2
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        # has been killed (e.g. during `3v0 update` or `--replace` handoffs)
        ...
        message = (
            ...
            "To recover: ensure no other 3V0 or OpenClaw instance is running "
            "with this token, then restart the gateway with '3v0 gateway restart'."
    ...
    async def _run_post_connect_housekeeping(self) -> None:
        ...
        try:
            ...
            try:
                ...
                # payload size limit (~4KB total).  3V0 defaults to 60 to
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # readable by the 3V0 process (shared mount, same machine, etc.).
            ...
            request_kwargs = {
                "connection_pool_size": _env_int("EV0_TELEGRAM_HTTP_POOL_SIZE", 512),
                "pool_timeout": _env_float("EV0_TELEGRAM_HTTP_POOL_TIMEOUT", 8.0),
                "connect_timeout": _env_float("EV0_TELEGRAM_HTTP_CONNECT_TIMEOUT", 10.0),
                "read_timeout": _env_float("EV0_TELEGRAM_HTTP_READ_TIMEOUT", 20.0),
                "write_timeout": _env_float("EV0_TELEGRAM_HTTP_WRITE_TIMEOUT", 20.0),
            ...
            disable_fallback = (
                os.getenv("EV0_TELEGRAM_DISABLE_FALLBACK_IPS", "")
                .strip()
                .lower()
                in {"1", "true", "yes", "on"}
            )
            ...
            if not fallback_ips and not disable_fallback:
                discovery_timeout = self._env_float_clamped(
                    "EV0_TELEGRAM_FALLBACK_DISCOVERY_TIMEOUT",
                    5.0,
                    min_value=0.0,
                )
                logger.warning(
            ...
            _max_connect = 8
            _init_timeout = _env_float("EV0_TELEGRAM_INIT_TIMEOUT", 30.0)
                ...
                try:
                    ...
                    if asyncio.get_running_loop().time() >= _total_deadline:
                        raise OSError(
                            ...
                            f"or set EV0_TELEGRAM_HTTP_CONNECT_TIMEOUT / "
                            f"EV0_TELEGRAM_INIT_TIMEOUT to a lower value."
                        )
                    logger.warning(
                    ...
                    if _attempt < _max_connect - 1:
                        ...
                        raise OSError(
                            ...
                            f"or set EV0_TELEGRAM_HTTP_CONNECT_TIMEOUT to a lower value."
                        )
    ...
    def _build_provider_keyboard(self, providers: list, page: int = 0) -> tuple:
        ...
        CLI ``3v0 model`` picker via the shared ``group_providers`` fold,
    ...
    # Scripts live in ~/.3V0/scripts/gmail-triage/. `arg` from the callback
    ...
    async def _handle_gmail_triage_callback(
        ...
        *,
        ...
        script_path = _Path.home() / ".3V0" / "scripts" / "gmail-triage" / script_name
    ...
    def _message_mentions_bot(self, message: Message) -> bool:
        ...
        # raw substring matches like "foo@ev0_bot.example" are not mentions
    ...
    def _explicit_bot_mentions_exclude_self(self, message: Message) -> bool:
        ...
        Telegram groups can contain several 3V0 bot profiles. A message like
    ...
    def _is_own_message(self, message: Message) -> bool:
        ...
        counted as incoming unread messages in the 3V0 inbox.
        """
        if not self._bot:
            return False
...
def _resolve_notifications_mode() -> str:
    ...
    mode = os.getenv("EV0_TELEGRAM_NOTIFICATIONS", "")
    if not mode:
        try:
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `3v0 setup` to install Telegram support.",
