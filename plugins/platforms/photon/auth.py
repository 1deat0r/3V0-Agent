...
Credential storage mirrors every other 3V0 channel:

    * runtime SDK creds  -> ``~/.3V0/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.3V0/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a 3v0 dependency
...
# registers 3V0 as its own client_id.
...
# Default name of the project 3V0 provisions for the operator.
DEFAULT_PROJECT_NAME = "3V0 Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of 3v0-agent.
...
def _save_auth(data: Dict[str, Any]) -> None:
    ...
    # threev0_cli/auth.py:_save_auth_store (#19673, #21148).
    fd = os.open(
        str(tmp),
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.3V0/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.3V0/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.3V0/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: threev0_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `3v0 photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `3v0 photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `3v0 photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `3v0 photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `3v0 photon setup`)"
