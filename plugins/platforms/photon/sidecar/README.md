...
Small Node helper that bridges 3V0 Agent to Photon's Spectrum SDK
(`spectrum-ts`).  3V0 is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-3V0-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and 3V0 can receive inbound messages
...
The 3V0 plugin's `3v0 photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. 3V0 is Python, so replies go through this sidecar
