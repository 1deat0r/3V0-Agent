// 3V0 Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> 3V0): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (3V0 -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-3V0-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `3v0 photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so 3V0 can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "3v0-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply 3V0'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local 3V0 allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of 3V0, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-3v0-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when 3V0 supplied them so a known-good
