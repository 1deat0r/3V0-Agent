"""
``3v0 photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other 3V0
...
# Writable sidecar runtime dir (mirrors to EV0_HOME on immutable
...
# wiring imports this module for `3v0 --help`).
...
# when called later (gateway start, 3v0 status). Cleared on success.
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `3v0 photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: '3V0 Agent')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``3v0 photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "3V0 Agent" project.
    ...
    # 3. Provision Spectrum credentials (runtime -> ~/.3V0/.env,
    ...
    try:
        from ev0_cli.config import write_platform_config_field
    ...
    print("  Start the gateway:  3v0 gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`3v0 photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.3V0/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `3v0 photon telemetry on` / `3v0 photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.3V0/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  3v0 gateway restart")
    return 0
...
#
# `3v0 gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``3v0 photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `3v0 gateway setup` wizard."""
