"""
Photon Spectrum (iMessage) platform adapter for 3V0 Agent.
...
if TYPE_CHECKING:
    ...
    try:
        import httpx
        HTTPX_AVAILABLE = True
    except ImportError:  # pragma: no cover - httpx is already a 3V0 dep
...
#
# Out-of-process senders (cron subprocesses, `3v0 send`, the dashboard)
...
# this module (3v0 status, test collection, plugin discovery).
...
# artifacts so 3V0 sees the link once, not a follow-up "(attachment)" prompt.
...
def sidecar_deps_installed() -> bool:
    ...
    _start_sidecar(), and `3v0 photon status` so all three agree on
...
def check_requirements() -> bool:
    ...
    if not HTTPX_AVAILABLE:
        ...
        return False
    if not shutil.which(wire_env("PHOTON_NODE_BIN") or "node"):
        logger.warning(
            "photon: node binary '%s' not found on PATH",
            wire_env("PHOTON_NODE_BIN") or "node",
        )
        return False
    if not sidecar_deps_installed():
        ...
        # user has no CLI to run `3v0 photon setup`, so the connect path
        # must self-heal). Otherwise keep returning False so
        # `3v0 setup` / status surface the missing-deps state.
        ...
        # (load_gateway_config, 3v0 status, GET /api/status polling) —
        ...
        if npm_error:
            logger.debug(
                ...
                "(last npm error: %s) — run: 3v0 photon setup",
                _sidecar_dir(),
                npm_error,
            )
        else:
            logger.debug(
                "photon: spectrum-ts not installed at %s — run: 3v0 photon setup",
                _sidecar_dir(),
            )
        return False
    return True
...
def _sidecar_deps_stale() -> bool:
    ...
    `3v0 update` rewrites ``package-lock.json`` when the spectrum-ts pin is
...
def _reinstall_sidecar_deps() -> None:
    ...
    Mirrors ``3v0 photon install-sidecar``: ``npm ci`` for an exact,
...
def validate_config(cfg: PlatformConfig) -> bool:
    extra = cfg.extra or {}
    project_id = extra.get("project_id") or wire_env("PHOTON_PROJECT_ID")
...
def _env_enablement() -> Optional[dict]:
    ...
    home = (wire_env("PHOTON_HOME_CHANNEL") or "").strip()
    if home:
        seed["home_channel"] = {
            "chat_id": home,
            "name": wire_env("PHOTON_HOME_CHANNEL_NAME", "Home"),
        }
    return seed
...
def _markdown_enabled() -> bool:
    ...
    """
    return wire_env("PHOTON_MARKDOWN", "true").strip().lower() not in {
        "false", "0", "no",
    }
...
def _richlink_candidate(text: str) -> Optional[str]:
    ...
    markdown/text path so 3V0 does not drop labels or rewrite intent.
    """
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._sidecar_port = _coerce_port(
            extra.get("sidecar_port") or wire_env("PHOTON_SIDECAR_PORT"),
        ...
        self._autostart_sidecar = str(
            wire_env("PHOTON_SIDECAR_AUTOSTART", "true")
        ).lower() not in ("0", "false", "no")
        self._node_bin = wire_env("PHOTON_NODE_BIN") or shutil.which("node") or "node"
        ...
        self._probe_interval = _coerce_float(
            _first_set(
                ...
                wire_env("PHOTON_PROBE_INTERVAL_SECONDS"),
            ),
            600.0,
        )
        self._probe_timeout = _coerce_float(
            _first_set(
                ...
                wire_env("PHOTON_PROBE_TIMEOUT_SECONDS"),
            ),
            10.0,
        )
        self._probe_max_failures = _coerce_int(
            _first_set(
                ...
                wire_env("PHOTON_PROBE_MAX_FAILURES"),
            ),
            3,
        )
        ...
        if _require_mention is None:
            _require_mention = wire_env("PHOTON_REQUIRE_MENTION")
        ...
        self._mention_patterns = self._compile_mention_patterns(
            ...
            else wire_env("PHOTON_MENTION_PATTERNS")
        )
    ...
    def _compile_mention_patterns(raw: Any) -> "list[re.Pattern]":
        ...
        list, or comma/newline-separated), or None (use 3V0 defaults).
    ...
    async def _inbound_loop(self) -> None:
        ...
        headers = {"X-3V0-Sidecar-Token": self._sidecar_token}
        backoff = 1.0
    ...
    def _pid_is_sidecar(pid: int) -> bool:
        ...
        try:
            ...
            return False
        # Checkout-agnostic: any 3V0 checkout's sidecar entry point.
    ...
    async def _start_sidecar(self) -> None:
        if not sidecar_deps_installed():
            ...
            # `3v0 photon setup`, so the connect path must be able to
            ...
            if not sidecar_deps_installed():
                ...
                raise PhotonSidecarStartupError(
                    ...
                    f"Run: cd {_sidecar_dir()} && npm ci   (or `3v0 photon setup`)",
                    code="SIDECAR_DEPS_MISSING",
                    retryable=False,
                )
        # A `3v0 update` that bumps the spectrum-ts pin rewrites
            ...
            while time.time() < deadline:
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        # (cron, `3v0 send`) can reach this sidecar
    ...
    def _reactions_enabled(self) -> bool:
        return wire_env("PHOTON_REACTIONS", "false").strip().lower() in {
            "true", "1", "yes", "on",
        }
...
async def _standalone_send(
    ...
    *,
    ...
    port = _coerce_port(
        (pconfig.extra or {}).get("sidecar_port") or wire_env("PHOTON_SIDECAR_PORT"),
        _DEFAULT_SIDECAR_PORT,
    )
    ...
    if not token:
        ...
        # the gateway process env otherwise, so cron/`3v0 send` would be
        ...
        if not token:
            return {
                "error": (
                    ...
                    "Start the 3V0 gateway (which spawns the sidecar and "
                    "records its address under <3v0-home>/runtime/"
    ...
    headers = {"X-3V0-Sidecar-Token": token}
...
def register(ctx) -> None:
    """Called by the 3V0 plugin loader at startup."""
    # Local import to avoid argparse work at module load; reused for both the
    # gateway-setup hook and the `3v0 photon` CLI command below.
    ...
    ctx.register_platform(
        ...
        install_hint=(
            "Run: 3v0 photon setup  (logs in via device flow, creates a "
            "Spectrum project, links your phone number, installs the "
            "spectrum-ts sidecar)."
        ),
        # Surfaces Photon in `3v0 gateway setup` alongside every other
    ...
    # Register CLI subcommands — `3v0 photon ...`
