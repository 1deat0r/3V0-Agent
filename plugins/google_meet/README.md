...
Let the 3v0 agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where 3v0 runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `3v0 meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `3v0 meet` CLI |
...
| `cli.py` | `3v0 meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$EV0_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `3v0 meet node {run,list,approve,remove,status,ping}` |
...
3v0 meet install                                      # pip + Chromium
3v0 meet setup                                        # preflight
3v0 meet auth                                         # optional
3v0 meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
3v0 meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.3V0/.env
3v0 meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
3v0 meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
3v0 meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.3V0/.env
3v0 meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, 3v0 will **not** switch your system audio input automatically — the
...
3v0 plugins enable google_meet
3v0 meet node run --display-name my-mac --host 0.0.0.0 --port 18789
