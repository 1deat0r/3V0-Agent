"""`3v0 meet node ...` subcommand tree.

Wired into the existing ``3v0 meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``3v0 meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="3v0-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    ...
    if cmd == "run":
        ...
        print(f"             3v0 meet node approve <name> ws://<host>:{args.port} {token}")
        try:
