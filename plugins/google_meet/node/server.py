...
Launched by ``3v0 meet node run``.

Token handling
--------------
On first boot we mint 32 hex chars of entropy and persist them at
``$EV0_HOME/workspace/meetings/node_token.json``. Subsequent boots
...
via ``3v0 meet node approve <name> <url> <token>``.
...
class NodeServer:
    ...
    def __init__(
        ...
        display_name: str = "3v0-meet-node",
    ) -> None:
        self.host = host
        self.port = port
