...
```bash
3v0 plugins enable google_meet
3v0 meet install                 # pip deps + Chromium (transcribe only)
3v0 meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
3v0 meet auth                    # optional; skips guest-lobby wait
3v0 meet setup                   # preflight checks
```

`3v0 meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or EV0_MEET_REALTIME_KEY in ~/.3V0/.env
```
...
pip install playwright websockets && python -m playwright install chromium
3v0 plugins enable google_meet
3v0 meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
3v0 meet node approve my-mac ws://<mac-ip>:18789 <token>
3v0 meet node ping my-mac                   # confirm reachable
```

Run `3v0 meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A 3V0 agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `3v0 meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `EV0_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `ev0_meet_src`). |
...
Local:
```
$EV0_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$EV0_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `3v0 meet node approve`.
