"""disk_cleanup — ephemeral file cleanup for 3V0 Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under EV0_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly EV0_HOME and /tmp/3v0-*
Never touches: ~/.3V0/logs/ or any system directory.
"""
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$EV0_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under EV0_HOME or ``/tmp/3v0-*``.
    ...
    try:
        ...
        pass
    # Allow /tmp/3v0-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("3v0-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "3v0-agent", "backups", "profiles", ".worktrees",
...
# Paths under $EV0_HOME that must NEVER be deleted by quick(),
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside EV0_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under EV0_HOME, but never recurse into known
...
def guess_category(path: Path) -> Optional[str]:
    ...
    try:
        ...
        # Path isn't under EV0_HOME (e.g. /tmp/3v0-*) — fall through.
        pass
