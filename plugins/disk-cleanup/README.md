...
Auto-tracks and cleans up ephemeral files created during 3V0 Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$EV0_HOME` and `/tmp/3v0-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `EV0_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under EV0_HOME | always | Never |
...
- `is_safe_path()` rejects anything outside `EV0_HOME` or `/tmp/3v0-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$EV0_HOME/disk-cleanup/` is itself excluded
- `$EV0_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
