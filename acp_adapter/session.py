"""ACP session manager — maps ACP sessions to 3V0 AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.3V0/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when 3V0 itself is running in WSL.

    Windows ACP clients can launch ``3v0 acp`` inside WSL while still sending
...
def _normalize_cwd_for_compare(cwd: str | None) -> str:
    ...
    from threev0_constants import windows_path_to_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch 3V0 from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["3v0-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed 3V0 agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by 3V0 AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current 3V0 runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.3V0/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``EV0_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_ev0_home``).
        """
    ...
    def _restore(self, session_id: str) -> Optional[SessionState]:
        ...
        # for the rest of the session (see threev0_state.get_messages_as_conversation).
        try:
    ...
    def _make_agent(
        ...
        *,
        ...
        # the automatic late-refresh (see Ev0ACPAgent._schedule_mcp_late_refresh).
        try:
        ...
        # editor/session cwd instead of the 3V0 daemon's process cwd.
