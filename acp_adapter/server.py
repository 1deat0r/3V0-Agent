"""ACP agent server — exposes 3V0 Agent via the Agent Client Protocol."""
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so 3V0 running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; 3V0 cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a 3V0/OpenAI-compatible user content payload."""
...
class Ev0ACPAgent(acp.Agent):
    """ACP Agent implementation wrapping 3V0 AIAgent."""
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so 3V0 maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        The shared 3V0 inventory is also used by ``3v0 model``, the TUI,
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  3V0 estimates ``used`` from the same
    ...
    async def _send_session_info_update(
        ...
        *,
        ...
        When the internal 3V0 head rotated (e.g. compression-driven session
        split during a turn), pass ``previous_ev0_session_id`` so the
        attached ``_meta.3v0.sessionProvenance`` flags the rotation reason.
        """
        if not self._conn:
            return
        try:
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["3v0-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="3v0-agent", version=EV0_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # 3V0' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches 3V0 setup/model selection out-of-band.
    ...
    def _history_summary_meta(message: dict[str, Any], text: str) -> dict[str, Any] | None:
        ...
        Two distinct keys under ``_meta.3v0`` (ACP's extensibility
        ...
        * ``containsCompactionSummary: true`` — a merged-tail message: real
        ...
        if kind is None and message.get(COMPRESSED_SUMMARY_METADATA_KEY):
            ...
            kind = "standalone"
        if kind == "standalone":
            return {"3v0": {"compactionSummary": True}}
        if kind == "merged":
            return {"3v0": {"containsCompactionSummary": True}}
        return None
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes 3V0
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run 3V0 on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive 3V0' local kawaii waiting/status
        ...
        # tools.approval (set_ev0_interactive_context) rather than
        # os.environ["EV0_INTERACTIVE"], so concurrent executor workers can't
        ...
        # callback shape — not the gateway-queue EV0_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind EV0_SESSION_KEY for this session so per-session caches
            ...
            try:
                ...
                # Without it the prompt advertises the global 3V0 workspace
                # while the tools are rooted at the client's project, so the
                # model emits absolute paths under ~/.3V0/workspace and the
                # edit silently lands outside the editor's workspace.
                # cron_session="" explicitly marks this as a non-cron context,
                # masking any leaked process-global EV0_CRON_SESSION (#37968).
            ...
            try:
                ...
                if previous_session_id is None:
                    pop_branded_env("SESSION_ID")
                else:
                    set_branded_env("SESSION_ID", previous_session_id)
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            # Snapshot the internal 3V0 DB session id before the turn so we
        ...
        # 3V0' local "waiting for model response" interrupt status is metadata,
    ...
    def _handle_slash_command(self, text: str, state: SessionState) -> str | None:
        ...
        # an unpinned handler bakes the 3V0 install tree into the session's
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"3V0 Agent v{EV0_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when 3V0 has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
