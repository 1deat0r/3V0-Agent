"""ACP auth helpers — detect and advertise 3V0 authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "3v0-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active 3V0 runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if 3V0 can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for 3V0.
    ...
    not have 3V0 provider credentials configured yet, so 3V0 always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate 3V0 using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure 3V0 provider",
            description=(
                "Open 3V0' interactive model/provider setup in a terminal. "
                "Use this when 3V0 has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
