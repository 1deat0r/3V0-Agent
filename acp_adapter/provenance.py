...
This is an additive 3V0 extension surfaced under ACP ``_meta.3v0`` so
...
compression rotates the internal 3V0 head, ``build_session_provenance`` lets
...
def build_session_provenance(
    ...
    *,
    ...
    """Build ``_meta.3v0.sessionProvenance`` for an ACP session.

    Args:
        db: A ``SessionDB`` (must expose ``get_session``).
        acp_session_id: The stable ACP/editor-facing session handle.
        current_ev0_session_id: The live internal 3V0 DB session id
            (``state.agent.session_id``).
        previous_ev0_session_id: The internal id from before the most recent
            turn, when known. Supplied by ``prompt()`` to flag a rotation.

    Returns:
        A dict suitable for ``{"3v0": {"sessionProvenance": <dict>}}`` under
        ACP ``_meta``, or ``None`` if the session can't be read.
    """
    try:
        row = db.get_session(current_threev0_session_id)
    except Exception:
        return None
    if not row:
        return None
    ...
    root_id = current_threev0_session_id
    compression_depth = 0
    cursor_parent = parent_id
    seen = {current_threev0_session_id}
    ...
    rotated = bool(
        ...
        and previous_threev0_session_id != current_threev0_session_id
    )

    provenance: Dict[str, Any] = {
        "acpSessionId": acp_session_id,
        "currentEv0SessionId": current_threev0_session_id,
        "rootEv0SessionId": root_id,
        "parentEv0SessionId": parent_id,
        "sessionKind": "continuation" if is_continuation else "root",
        "compressionDepth": compression_depth,
    }
    if previous_threev0_session_id:
        provenance["previousEv0SessionId"] = previous_threev0_session_id
    if rotated:
...
def session_provenance_meta(
    ...
    *,
    ...
    """Return a ready ``_meta`` payload: ``{"3v0": {"sessionProvenance": ...}}``."""
    ...
    if prov is None:
        return None
    return {"3v0": {"sessionProvenance": prov}}
