"""CLI entry point for the 3v0-agent ACP adapter.

Loads environment variables from ``~/.3V0/.env``, configures logging
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="3v0-acp",
        description="Run 3V0 Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print 3V0 version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive 3V0 provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.3V0/node/ "
...
def _run_check() -> None:
    ...
    from acp_adapter.server import Ev0ACPAgent  # noqa: F401

    print("3V0 ACP check OK")
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting 3v0-agent ACP adapter")
    ...
    import acp
    from .server import Ev0ACPAgent
