# Contributing to 3V0 Agent

Thank you for contributing to 3V0 Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want 3V0 to work everywhere.
...
Related: #38284 covers the agent-side analog — 3V0 itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every 3V0 install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `3v0 skills browse` (labeled "official") and install it with `3v0 skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `3v0 skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.3V0/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(ev0_home, config)` for setup-wizard integration
...
- Integrate with `3v0 memory setup` via `post_setup()` — no need to touch core code
...
The reason is maintenance load, not quality. Every external product absorbed into the core tree becomes ours to keep working against a fast-moving codebase, for a backend we don't own and can't control. 3V0 ships a lot and the core moves quickly; coupling third-party products into it creates an open-ended burden on the maintainers.

Publish these as a **standalone plugin repo** instead:

- Implement the relevant ABC and use the existing plugin discovery path (`~/.3V0/plugins/`, project `.3V0/plugins/`, or a pip entry point) — see [Build a 3V0 Plugin](https://github.com/1deat0r/3V0-Agent/docs/guides/build-a-3v0-plugin)
...
The installer creates the 3V0 venv, wires the `3v0` command, stamps the
install method for `3v0 update`, and clones the full git project into
`$EV0_HOME/3v0-agent` (usually `~/.3V0/3v0-agent`). That keeps your
...
Use this only if you intentionally do not want 3V0' managed install layout
(for example, a throwaway clone inside a container or CI job). If you install
this way, make sure you run the `3v0` entrypoint from this venv; running the
system `python3 -m threev0_cli.main` can pick up unrelated system Python
packages.
...
export VIRTUAL_ENV="$HOME/.3V0/venvs/3v0-dev"
...
```bash
mkdir -p ~/.3V0/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.3V0/config.yaml
touch ~/.3V0/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.3V0/.env
```

### Run

```bash
# The standard installer already put `3v0` on PATH.
3v0 doctor
3v0 chat -q "Hello"
```

If you used the manual clone fallback, run `./3v0` from the checkout or
symlink this clone's venv explicitly:

```bash
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/3v0" ~/.local/bin/3v0
```
...
├── cli.py                    # Ev0CLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (3v0-cli, 3v0-telegram, etc.)
├── threev0_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── threev0_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.3V0/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (github.com/1deat0r/3V0-Agent)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.3V0/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.3V0/`)

| Path | Purpose |
|------|---------|
| `~/.3V0/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.3V0/.env` | API keys and secrets |
| `~/.3V0/auth.json` | OAuth credentials (Nous Portal) |
| `~/.3V0/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.3V0/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.3V0/state.db` | SQLite session database |
| `~/.3V0/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.3V0/cron/` | Scheduled job data |
| `~/.3V0/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`threev0_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.3V0/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
...
(for example `_EV0_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.3v0`:
...
The user may skip setup and keep loading the skill. 3V0 only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `3v0 setup` or update `~/.3V0/.env` locally.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "3V0 Agent" is the secondary collaborator. If the contributor's commit shows "3V0 Agent" as author because they used 3V0 to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing 3V0 tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `3v0 --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

3V0 uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.3V0/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `threev0_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `threev0_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

3V0 runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the 3v0 wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `threev0_cli/gateway.py::_scan_gateway_pids` for the pattern.
   ```
...
9. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `threev0_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
15. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `threev0_cli/gateway_windows.py::
...
3V0 has terminal access. Security matters.
...
2. **Test manually**: Run `3v0` and exercise the code path you changed
