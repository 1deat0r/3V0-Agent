...
``threev0_state.SessionDB``. It defines no ``__init__`` and no state of its
...
``SessionDB.__init__``. It must never import threev0_state (cycle) — shared
module-level constants live in threev0_state_common.
"""
...
# Moved methods logged under the "threev0_state" logger before the split;
# keep that logger identity so log filtering/capture behavior is unchanged.
logger = logging.getLogger("threev0_state")
...
class SessionSearchMixin:
    ...
    def search_messages(
        ...
        Threshold: EV0_SEARCH_SLOW_MS (default 1000; 0 logs every call).
        """
        started = time.time()
        rows = None
        try:
            ...
            try:
                threshold = float(os.getenv("EV0_SEARCH_SLOW_MS", "1000"))
    ...
    def search_sessions_by_id(
        ...
        a session id from logs, CLI output, or another 3V0 surface and jump
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``EV0_DISABLE_FTS_TRIGRAM`` or not yet created), so
