...
/**
 * Map an uninstall mode to the `python -m ev0_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `ev0_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'ev0_cli.uninstall', '--mode', mode]
}
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import ev0_cli`
 ...
 * modes + `pythonPath`=agentRoot so `import ev0_cli` resolves from source
