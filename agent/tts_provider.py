...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into 3V0 with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.3V0/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of 3V0
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``3v0 tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``3v0 setup`` for
