...
def _ev0_meta(fm: dict[str, Any]) -> dict[str, Any]:
...
def _related(fm: dict[str, Any]) -> list[str]:
    raw = fm.get("related_skills") or _ev0_meta(fm).get("related_skills")
...
def _category(fm: dict[str, Any], skill_md: Path) -> str:
    cat = fm.get("category") or _ev0_meta(fm).get("category")
    if cat:
        return str(cat)
...
def _load_usage() -> dict[str, dict[str, Any]]:
    try:
        ...
        path = get_ev0_home() / "skills" / ".usage.json"
        try:
...
def _skill_roots() -> list[tuple[str, Path]]:
    ...
    home_skills = get_ev0_home() / "skills"
