...
The parent 3V0 process may itself be a Kanban dispatcher worker with
EV0_KANBAN_* variables in process env. delegate_task children run inside the
...
# though the worker's EV0_KANBAN_* vars are legitimately in os.environ (cron
...
DELEGATED_CHILD_ENV_MARKER = "EV0_DELEGATED_CHILD_CONTEXT"
...
def non_dispatcher_owned_context() -> Iterator[None]:
    ...
    own process, where ``EV0_KANBAN_TASK`` is legitimately set.  Without this
    ...
    ``kanban_complete`` defaults ``task_id`` to ``$EV0_KANBAN_TASK`` — letting
...
def is_dispatcher_owned_worker_context() -> bool:
    ...
    The single predicate every ``EV0_KANBAN_*`` identity gate should use
...
def delegated_child_subprocess_env(
    ...
    parent dispatcher ``EV0_KANBAN_*`` vars while losing the ContextVar in
