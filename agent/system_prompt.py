...
upstream prefix cache warm.  See ``3v0-agent-dev``'s
...
_PLUGIN_SECTION_FRAME_RE = re.compile(
    ...
    r"<!-- 3v0-plugin-section-chars:(?P<chars>[0-9]{1,4}) -->\n\n",
    re.MULTILINE,
)
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``EV0_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
...
def _plugin_session_info(agent: Any) -> Dict[str, str]:
    ...
    try:
        ...
        # the EV0_HOME ContextVar (#86313 class; plugin half per @helix4u).
...
def _frozen_plugin_prompt_sections(agent: Any) -> tuple:
    ...
    if isinstance(stored_prompt, str) and stored_prompt:
        ...
        return rendered
    try:
        from ev0_cli.plugins import render_system_prompt_sections
...
def _plugin_section_blocks(sections: tuple, position: str) -> List[str]:
    from ev0_cli.plugins import format_system_prompt_sections
...
def _agent_home(agent: Any) -> Optional[Path]:
    ...
    1. A bound EV0_HOME ContextVar override wins. Surfaces that multiplex
...
def _profile_name_for_home(home: Path) -> str:
    ...
    Uses :func:`get_default_ev0_root` (NOT ``get_ev0_home()``): on a
    correctly bound profile session the ambient home IS the profile dir, so
    ``get_ev0_home()/profiles`` would never contain ``home`` and every
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  3V0 never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want EV0_HOME persona while keeping
    # cwd project instructions disabled.
    _soul_loaded = False
    if agent.load_soul_identity or not agent.skip_context_files:
        # Scope the SOUL.md read to the agent's OWN home (see _agent_home) —
        # ambient resolution on a thread that lost the EV0_HOME ContextVar
    ...
    # Pointer to the 3v0-agent skill + docs for user questions about 3V0 itself.
    stable_parts.append(EV0_AGENT_HELP_GUIDANCE)
    ...
    # EV0_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base 3V0, any interactive coding surface in a code
    ...
    # `3v0 -p <bot> chat --in ~ -c "Bot Chat"` and the desktop pins), on
    # installs where Bot Mode manages profiles (ui_meta['3v0-bots']).
    ...
    # Active-profile hint — names the 3V0 profile the agent is running
    # under so it doesn't conflate ~/.3V0/skills/ (default profile) with
    # ~/.3V0/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    # ambient EV0_HOME: on a build thread that lost the ContextVar this
    ...
    # from get_default_ev0_root(): get_ev0_home() on a bound profile
    ...
    if not agent.skip_context_files:
        ...
        # (developing 3V0). Every other surface (desktop chat panel,
    ...
    from ev0_time import get_timezone as _ev0_tz, now as _ev0_now
    now = _ev0_now()
