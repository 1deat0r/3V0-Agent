...
(``gateway/platforms/webhook.py``): inbound wakes 3V0 when the world
changes; outbound tells the world when 3V0 does something.
...
* Payloads are signed with HMAC-SHA256 (GitHub-style
  ``X-3V0-Signature-256: sha256=<hexdigest>`` over the raw body) when
...
* No consent prompt: unlike shell hooks, an outbound target executes no
  ...
  in config.  ``EV0_SAFE_MODE=1`` still skips registration, matching
  plugins / MCP / shell hooks.
* Registration is idempotent — safe to invoke from both the CLI entry
  point and the gateway entry point.

Config schema (``~/.3V0/config.yaml``)::

    hooks:
      outbound:
        - url: https://ci.example.com/3v0-events
          events: [on_session_end, subagent_stop]
          # secret literal (discouraged) or env var name (preferred):
          secret_env: EV0_OUTBOUND_WEBHOOK_SECRET
          ...
          name: ci-notify   # optional label for logs / `3v0 hooks list`
...
Headers::
    ...
    User-Agent:              3V0-Agent-Outbound-Webhook
    X-3V0-Event:          <hook event name>
    X-3V0-Delivery:       <delivery_id>
    X-3V0-Signature-256:  sha256=<hmac hexdigest>   # only when secret set
"""
...
def register_from_config(cfg: Optional[Dict[str, Any]]) -> List[WebhookTarget]:
    ...
    if env_var_enabled("EV0_SAFE_MODE"):
        logger.info("EV0_SAFE_MODE=1 — outbound webhook registration skipped")
        return []
...
def _serialize_payload(
    ...
    ``delivery_id`` is shared with the ``X-3V0-Delivery`` header so
...
def _build_delivery(
    ...
    if target.secret:
        ...
        headers["X-3V0-Signature-256"] = f"sha256={digest}"
    return {
        "url": target.url,
        "label": target.label,
        "event": event,
