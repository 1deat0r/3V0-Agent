...
``3v0 pause`` writes a sentinel file at ``$EV0_HOME/ESTOP``;
``3v0 resume`` removes it. While the sentinel exists:
...
* new gateway turns get a brief "3V0 is paused" reply instead of an
...
hold even if the file was created by ``touch ~/.3V0/ESTOP``.
...
def is_engaged() -> bool:
    ...
    exists (permission error, transient I/O failure on EV0_HOME), report
...
def paused_reply() -> Optional[str]:
    ...
    if reason:
        return (
            f"⏸️ 3V0 is paused ({reason}). New work is on hold; "
            "run `3v0 resume` to pick things back up."
        )
    return (
        "⏸️ 3V0 is paused. New work is on hold; "
        "run `3v0 resume` to pick things back up."
    )
