...
MoA-enabled; the normal 3V0 agent loop still owns tool calling and turn
...
def _moa_privacy_mode(moa_raw: Any) -> str:
    ...
    from threev0_cli.moa_config import coerce_privacy_filter
...
def _reference_messages(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    conversation flow, but flatten it into clean user/assistant *text* turns:
      ...
      - system prompt: dropped (8K of 3V0 boilerplate, not advisory signal).
...
def _preset_temperature(preset: dict[str, Any], key: str) -> float | None:
    ...
    single-model 3V0 agent (which never sends temperature unless
...
def aggregate_moa_context(
    *,
    ...
    synth_prompt = (
        ...
        "3V0 agent. Focus on next steps, tool-use strategy, risks, and any "
    ...
    return (
        ...
        "normal 3V0 agent loop. You may call tools, continue reasoning, or "
        "finish normally.]\n"
...
class MoAChatCompletions:
    ...
    def create(self, **api_kwargs: Any) -> Any:
        ...
        from threev0_cli.config import get_config_path, load_config
