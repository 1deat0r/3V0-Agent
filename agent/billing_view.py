...
def build_billing_state(*, timeout: float = 15.0) -> BillingState:
    ...
    Dev override: ``EV0_DEV_BILLING_FIXTURE`` short-circuits to a fixture so the
    card-on-file / admin / scope states are testable offline (mirrors
    ``EV0_DEV_CREDITS_FIXTURE`` for the usage model).
    """
...
def _dev_fixture_billing_state() -> Optional[BillingState]:
    """Map ``EV0_DEV_BILLING_FIXTURE`` to a :class:`BillingState` for offline UX.
    ...
    return BillingState(logged_in=False, error=f"unknown EV0_DEV_BILLING_FIXTURE: {name}")
