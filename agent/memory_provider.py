...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `3v0 backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "3v0").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by '3v0 memory setup' to walk the user through configuration.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE EV0_HOME.

        ``3v0 backup`` only walks EV0_HOME, so any provider state kept
        ...
        of the archive, and ``3v0 import`` restores them to their original
