...
def verify_on_stop_enabled(config: dict[str, Any] | None = None) -> bool:
    ...
    Precedence: an explicit ``EV0_VERIFY_ON_STOP`` env var wins, then an
...
def _workspace_has_runnable_recipe(root: Any) -> bool:
    """Whether the workspace has a runtime verify recipe ``3v0 verify`` can run.

    True when a saved ``.3V0/environment.json`` manifest exists, or when
    ...
    if verify_commands:
        ...
        if _workspace_has_runnable_recipe(facts.get("root")):
            command_instruction += (
                ...
                "start + readiness), prefer `3v0 verify --json` — a passing "
        ...
        if _workspace_has_runnable_recipe(facts.get("root")):
            command_instruction = (
                ...
                "project has a runnable verification recipe. Run `3v0 verify "
            ...
            command_instruction = (
                ...
                "`tempfile` path with a `3v0-verify-` filename prefix, run it "
