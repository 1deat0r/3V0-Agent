"""3V0-tools-as-MCP server for the codex_app_server runtime.

When the user runs `openai/*` turns through the codex app-server, codex
owns the loop and builds its own tool list. By default, that means
3V0' richer tool surface — web search, browser automation,
delegate_task subagents, vision analysis, persistent memory, skills,
cross-session search, image generation, TTS — is unreachable.

This module exposes a curated subset of those 3V0 tools to the
spawned codex subprocess via stdio MCP. Codex registers it as a normal
MCP server (per `~/.codex/config.toml [mcp_servers.3v0-tools]`) and
the user gets full 3V0 capability inside a Codex turn.
...
Scope (what we expose):
  ...
  - skill_view, skills_list              — 3V0' skill library
  - text_to_speech                       — TTS
  - kanban_* (complete/block/comment/    — kanban worker + orchestrator
    heartbeat/show/list/create/            handoff (stateless: read env var,
    unblock/link)                          write ~/.3V0/kanban.db)
...
What we DO NOT expose:
  ...
  - delegate_task / memory /             — `_AGENT_LOOP_TOOLS` in 3V0
...
Run with: python -m agent.transports.threev0_tools_mcp_server
...
# Tools we expose. Each name MUST match a registered 3V0 tool that
...
#     `_AGENT_LOOP_TOOLS` in 3V0 (model_tools.py:493). They require
#     the running AIAgent context to dispatch (mid-loop state), so a
#     stateless MCP callback can't drive them. 3V0' default runtime
...
EXPOSED_TOOLS: tuple[str, ...] = (
    ...
    "skills_list",
    "text_to_speech",
    # Kanban worker handoff tools — gated on EV0_KANBAN_TASK env var
    # (set by the kanban dispatcher when spawning a worker). Without these
    # in the callback, a worker spawned with openai_runtime=codex_app_server
    # could do the work but couldn't report completion back to the kernel,
    # making it hang until timeout. Stateless dispatch — they just read
    # the env var and write to ~/.3V0/kanban.db.
    "kanban_complete",
    "kanban_block",
    ...
    # only — the kanban tool gates them on EV0_KANBAN_TASK being unset.
...
def _build_server() -> Any:
    """Create the FastMCP server with 3V0 tools attached. Lazy imports
    ...
    try:
        ...
        raise ImportError(
            f"3v0-tools MCP server requires the 'mcp' package: {exc}"
        ) from exc

    # Discover 3V0 tools so dispatch works.
    ...
    mcp = FastMCP(
        "3v0-tools",
        instructions=(
            "3V0 Agent's tool surface, exposed for use inside a Codex "
    ...
    # Pull authoritative 3V0 tool schemas for the ones we expose, so
    # MCP clients see the same parameter docs 3V0 gives the model.
    all_defs = {
    ...
    for name in EXPOSED_TOOLS:
        ...
        if spec is None:
            logger.debug(
                "skipping %s — not registered in this 3V0 process", name
            )
            continue

        description = spec.get("description") or f"3V0 {name} tool"
    ...
    logger.info(
        "3v0-tools MCP server registered %d/%d tools",
        exposed_count,
        len(EXPOSED_TOOLS),
    )
    return mcp
...
def main(argv: Optional[list[str]] = None) -> int:
    """Entry point for `python -m agent.transports.threev0_tools_mcp_server`."""
    ...
    try:
        ...
        sys.stderr.write(f"3v0-tools MCP server cannot start: {exc}\n")
        return 2
    ...
    try:
        ...
        logger.exception("3v0-tools MCP server crashed")
        sys.stderr.write(f"3v0-tools MCP server error: {exc}\n")
        return 1
    return 0
