"""Projects codex app-server events into 3V0' messages list.

The translator that lets 3V0' memory/skill review keep working under the
...
3V0' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape 3V0
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way 3V0' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but 3V0 stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way 3V0 already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — 3V0' messages store text only.
        text_parts: list[str] = []
