...
projection into 3V0' display, approval bridging, transcript projection into
...
"codex_app_server"`. 3V0' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 3V0 secret — gateway
        ...
        spawn_env = threev0_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("EV0_KANBAN_TASK"):
            kanban_db = spawn_env.get("EV0_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "EV0_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("EV0_HOME", os.path.expanduser("~/.3V0")),
                        "kanban",
                    ),
                )
            )
