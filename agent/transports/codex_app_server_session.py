...
Owns one Codex thread per 3V0 session. Drives `turn/start`, consumes
...
# 3V0' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_EV0_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse 3V0/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per 3V0 session, lifetime owned by AIAgent.
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but 3V0' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into 3V0' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own 3v0-tools callback we
            # auto-accept — the user already approved 3V0' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO 3V0
        approval-mode/timeout logic. The 3V0-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; 3V0 approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map 3V0 approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    3V0 returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the 3V0 approval mode/timeout resolution
    ...
    # "prompt expired"; the 3V0-side messaging already distinguishes them.
    return "decline"
