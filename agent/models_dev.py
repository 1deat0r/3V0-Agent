...
Data resolution order:
  ...
  2. Disk cache (~/.3V0/models_dev_cache.json — any age; stale data is
...
# ---------------------------------------------------------------------------
# Provider ID mapping: 3V0 ↔ models.dev
# ---------------------------------------------------------------------------

# 3V0 provider names → models.dev provider IDs
PROVIDER_TO_MODELS_DEV: Dict[str, str] = {
    ...
    # under the "meta" provider id; 3V0' provider is "meta-ai" (and the
...
# Reverse mapping: models.dev id → 3V0 ids (built lazily; many-to-one,
...
def fetch_models_dev(
    ...
    When ``force_refresh=True`` (used by ``3v0 config refresh``, the
...
def _provider_override_section(provider: str) -> Optional[Dict[str, Any]]:
    ...
    Accepts either the 3V0 provider id or the models.dev provider id as
...
def _get_provider_models(
    ...
    """Resolve a 3V0 provider ID to its models dict from models.dev.
...
def list_provider_models(
    ...
    picker (``3v0 model``), which is an interactive user-facing flow
    where a fresh catalog is worth a short network wait.
    """
    from threev0_cli.models import normalize_provider
...
# Gemma models whose TPM quotas are too small for normal 3V0 agent traffic.
...
def get_provider_info(
    ...
    Accepts either a 3V0 provider ID (e.g. "kilocode") or a models.dev
...
def get_model_info(
    ...
    Accepts 3V0 or models.dev provider ID.  Tries exact match then
