...
#
# When a user invokes a /skill (or /bundle), 3V0 expands the turn into a
...
# tests/openviking_plugin/test_openviking.py::test_skill_markers_match_ev0_scaffolding.
# ---------------------------------------------------------------------------
...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``EV0_PLATFORM`` env var and
    ``EV0_SESSION_PLATFORM`` from the gateway session context. Returns
...
def _resolve_skill_commands_home() -> str:
    """Return the effective 3V0 home the skill scan should be scoped to.

    A gateway session can switch between profiles that each carry their own
    ``skills.external_dirs`` (via ``set_ev0_home_override``), but the
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.3v0.config``
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.3V0/skills/ and return a mapping of /command -> skill info.
        ...
        for scan_dir in dirs_to_scan:
            for skill_md in iter_skill_index_files(scan_dir, "SKILL.md"):
                ...
                try:
                    ...
                    # with a core 3V0 slash command (name or alias). The
                    ...
                    if resolve_command(cmd_name) is not None:
                        logger.warning(
                            ...
                            "collides with a core 3V0 command; skipping "
...
def get_skill_commands() -> Dict[str, Dict[str, Any]]:
    ...
    active profile's 3V0 home changes (e.g. Desktop switching profiles
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.3V0/skills/`` and any ``skills.external_dirs`` so the
...
def build_preloaded_skills_prompt(
    ...
    bundle-invocation gate (#59156). Without this, ``3v0 -s <skill>`` or
    a deployment's ``EV0_TUI_SKILLS`` env var could force-load a skill an
