...
from ev0_constants import get_config_path, get_skills_dir, is_termux
...
# Org mirrors live under ~/.3V0/skills/_org/<org_id>/. Resolution is
...
def _detect_environment(env: str) -> bool:
    ...
    EV0_KANBAN_* vars without owning them), so caching it process-wide would
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``EV0_KANBAN_TASK`` / ``EV0_KANBAN_BOARD`` in the
        ...
        # The 3V0 Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``ev0_cli.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``EV0_PLATFORM`` or
            ``EV0_SESSION_PLATFORM`` env vars.  Returns the global
...
def parse_config_string_list(value) -> List[str]:
    ...
    ``3v0 config set`` and JSON-mode editor saves store lists as quoted
...
# which becomes the dominant cost of ``3v0`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.3V0/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``3v0`` cold-start time
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.3V0/skills/`` first, then external.
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.3V0/skills/`` (including via symlinks) or configured
    ...
    # ~/.3V0/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: 3V0 can discover and view
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes 3V0 metadata, VCS, virtualenv/dependency, cache, and skill
