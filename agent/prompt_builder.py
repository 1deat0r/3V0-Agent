...
def _find_threev0_md(cwd: Path) -> Optional[Path]:
    ...
    # could pick up a .3v0.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _EV0_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are 3V0 Agent, an intelligent AI assistant created by Nous Research. "
...
EV0_AGENT_HELP_GUIDANCE = (
    "You run on 3V0 Agent (by Nous Research). When the user needs help with "
    "3V0 itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://github.com/1deat0r/3V0-Agent/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `3v0-agent` skill with skill_view(name='3v0-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.3V0/kanban.db`. Your task id is in "
    "`$EV0_KANBAN_TASK`; your workspace is `$EV0_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $EV0_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $EV0_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${EV0_KANBAN_BRANCH:-wt/$EV0_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$EV0_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`3v0 profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `3v0 kanban <verb>` for board operations. Use "
...
#
# The 3v0-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to 3v0-agent's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "explicit 3V0 YOLO uses an unrestricted runtime after the user's "
        "launch/session risk acceptance. Permission mode and grants are fixed "
        "when 3V0 launches that runtime.\n\n"
        ...
        "run `3v0 computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that 3V0 "
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    HUD mode is a strip of 3V0 floating over another application, so the
    user is rarely asking about 3V0 — they are asking about the thing behind
    ...
    sentences = [
        "[Note: this message came from HUD mode — a small floating 3V0 "
        ...
        "rather than anything inside 3V0. read_window_below identifies "
        "that app.",
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the 3V0 terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the 3V0 desktop app — a graphical chat "
    ...
    "webui": (
        "You are in the 3V0 WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where 3V0 itself
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where 3V0 itself is running. The host OS, home, and cwd "
                f"of the 3V0 process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where 3V0 "
    ...
    # Embedder-supplied environment description. Lets a host that wraps 3V0
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.3V0/skills/`` directory.  External dirs
    ...
    # EV0_HOME would otherwise fall back to the launch (default) home and
...
def _build_skills_system_prompt_inner(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot 3V0 Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `3v0-agent` skill "
            "first. It has the actual commands (e.g. `3v0 config set …`, `3v0 tools`, "
            "`3v0 setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: 3v0 setup, 3v0 setup tools, 3v0 setup terminal, 3v0 status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(
    ...
    """Load SOUL.md from EV0_HOME and return its content, or None.
    ...
    ambient — which on a thread that lost the EV0_HOME ContextVar falls
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .3v0.md / EV0.md  (walk to git root)
      2. AGENTS.md / agents.md   (merged chain: git root → cwd)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from EV0_HOME is independent and always included when present.
    ...
    # Never let a FALLBACK-picked directory inside the 3V0 install/source
    ...
    # explicitly configured cwd is honored verbatim — the 3V0 tree is a
    ...
    # their launch dir IS the user's shell cwd (developing 3V0 in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the 3V0 install tree (%s) — set terminal.cwd to "
    ...
    # SOUL.md from EV0_HOME only — skip when already loaded as identity
    if not skip_soul:
