...
that instructs the live agent to:
  ...
  1. Gather the sources the user named, using the tools it already has
     ...
     material).
  2. Author a skill via ``skill_manage`` that follows the 3V0
     skill-authoring standards (description <=60 chars, the modern section
     order, 3V0-tool framing, no invented commands). Small sources get one
...
# (HARDLINE)" and the 3v0-agent-dev new-skill salvage reference. Embedded in
# the prompt so the agent authors skills the way a maintainer would by hand.
_AUTHORING_STANDARDS = """\
Follow the 3V0 skill-authoring standards exactly. These are the same
...
- version: 0.1.0
- author: always the literal value `3V0`. NEVER fill it from the host
  ...
  a privacy leak the user never opted into; the skill names itself as 3V0.
- platforms: declare `[macos]`, `[linux]`, and/or `[windows]` IF the skill
  ...
  skills.
- metadata.3v0.tags: a few Capitalized, Relevant, Tags.
...
4. "## How to Run" — the canonical invocation, framed through 3V0 tools.
...
3V0-tool framing (this is what makes it a skill, not shell docs):
- Frame running scripts as "invoke through the `terminal` tool".
- Reference 3V0 tools by name in backticks: `terminal`, `read_file`,
