...
def is_free_tier_model(model: str, base_url: str = "") -> bool:
    ...
    2. A peek into the in-process pricing cache in ``threev0_cli.models``
    ...
    if not base_url:
        return False
    try:
        from threev0_cli.models import _is_model_free, _pricing_cache
    ...
    try:
        ...
        if version_val != 1:
            if version_val > 1 and not _version_warning_emitted:
                ...
                logger.warning(
                    "credits header version %d unsupported, ignoring — update 3V0",
                    version_val,
                )
            return None
...
# ── Dev test fixtures (EV0_DEV_CREDITS_FIXTURE) ───────────────────────────
# Throwaway dev scaffolding: trigger any notice state on demand for testing,
# without real spend or Redis seeding. Set EV0_DEV_CREDITS_FIXTURE to either a
...
def dev_fixture_credits_state() -> Optional[CreditsState]:
    """Return a fixture CreditsState for EV0_DEV_CREDITS_FIXTURE, or None.
    ...
    Hard prod-leak guard: a fixture applies ONLY when the dev flag EV0_DEV_CREDITS
    is also on, so a stray EV0_DEV_CREDITS_FIXTURE (leaked into a shell profile, a
