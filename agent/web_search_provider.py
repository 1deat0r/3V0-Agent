...
``kind: backend``) or ``~/.3V0/plugins/web/<name>/`` (user, opt-in via
``plugins.enabled``).
...
def get_provider_env(name: str) -> str:
    ...
    Resolves *name* via :func:`ev0_cli.config.get_env_value` (checks
    ``os.environ`` first, then ``~/.3V0/.env``) so credentials set
    through 3V0' config layer are visible even when they were never
...
class WebSearchProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``3v0 tools``. Defaults to ``name``."""
        return self.name
    ...
    def is_available(self) -> bool:
        ...
        runs at tool-registration time and on every ``3v0 tools`` paint.
        """
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        Used by ``ev0_cli/tools_config.py`` to inject this provider as a
