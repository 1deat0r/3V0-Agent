...
def _state_file() -> Path:
    return get_threev0_home() / "skills" / ".curator_state"
...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `3v0 update` re-seeds.
...
def get_consolidate() -> bool:
    ...
    The explicit ``3v0 curator run --consolidate`` flag overrides this for
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``3v0 update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``3v0 curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `3v0 curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as 3V0' background skill CURATOR. This is an "
    ...
    "into ~/.3V0/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: 3v0-config-*, 3v0-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.3V0/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.3V0/logs/curator/``)
    ...
    authored skill data in ``~/.3V0/skills/``.

    ``ensure_ev0_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_threev0_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every 3v0 install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: 3v0 curator status
        keep an umbrella stable: 3v0 curator pin document-tools
    ...
    lines.append("full report: 3v0 curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: 3v0 curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.3V0/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.3V0/skills/.archive/` for "
            "safety and can be restored via `3v0 curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.3V0/skills/.archive/`. "
            "Restore any via `3v0 curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `3v0 curator restore <name>`")
    lines.append("- All archives live under `~/.3V0/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    *consolidate* gates the LLM umbrella-building pass. ``None`` (the default)
    ...
    ``3v0 curator run --consolidate`` flag. When consolidation is off, only
    ...
    *consolidate*: when consolidation is off, the preview only reports the
    ...
    # real pass out. We still record a summary so `3v0 curator status`
    ...
    def _llm_pass():
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `3v0 update` only if "
        ...
        # recorded in state so `3v0 curator status` can point at it.
        try:
            after_report = skill_usage.curated_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``3v0 model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `3v0 model` → auxiliary
