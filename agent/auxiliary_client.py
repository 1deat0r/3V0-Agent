...
def _fast_model_from_catalog(provider_id: str) -> str:
    ...
    try:
        ...
        from ev0_cli.models import fetch_models_with_pricing
...
# in lockstep with ev0_cli.__version__ across every Portal call site
...
def _nous_extra_body() -> dict:
    ...
    Computed at call time so a hot-reloaded ``ev0_cli.__version__`` is
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``ev0_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _nous_api_key(provider: dict) -> str:
    ...
    from ev0_cli.auth import _nous_invoke_jwt_is_usable
...
def _resolve_nous_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from ev0_cli.auth import _agent_key_is_usable
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    Falls back to ``ev0_cli.auth``'s singleton runtime resolver for older
...
def _read_codex_access_token() -> Optional[str]:
    ...
    try:
        from ev0_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from ev0_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from ev0_cli.auth import is_provider_explicitly_configured
...
def _aux_openrouter_settings() -> Tuple[bool, str]:
    ...
    """
    try:
        from ev0_cli.config import cfg_get, load_config_readonly
...
def _resolve_moa_aggregator(preset_name: Optional[str]) -> Tuple[Optional[str], Optional[str]]:
    ...
    try:
        from ev0_cli.config import load_config
        from ev0_cli.moa_config import resolve_moa_preset
...
def _try_azure_foundry(
    *,
    ...
    :func:`ev0_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from ev0_cli.runtime_provider import _resolve_azure_foundry_runtime
...
def _fallback_entry_api_key(entry: Dict[str, Any]) -> Optional[str]:
    ...
    gateway multiplexing (see ``ev0_cli.fallback_config.resolve_entry_api_key``).
    """
...
def resolve_provider_client(
    ...
    try:
        ...
        logger.debug("ev0_cli.auth not available for provider %s", provider)
        return None, None
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`ev0_cli.plugins.PluginContext.register_auxiliary_task`) the
