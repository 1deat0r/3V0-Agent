...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.3V0/auth.json active provider)
...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    ``ssl_ca_cert`` / ``ssl_verify`` config and the ``EV0_CA_BUNDLE`` /
    ...
    try:
        ...
        if not _WARNED_KEEPALIVE_IMPORT_SKEW:
            ...
            logger.warning(
                ...
                "back to the SDK default HTTP client. Run `3v0 update` (or "
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # 3V0 owns auxiliary retry + provider/model fallback policy (the
    ...
    # 3V0 sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let 3V0 control the budget; explicit callers can still
    # override via kwargs.
...
def _run_protected_sync_provider_call(
    ...
    threading.Thread(
        ...
        name="3v0-protected-aux-provider",
        daemon=True,
    ).start()
...
# Re-exported from provider_policy (ticket #16) for backward compatibility.
from agent.provider_policy import (
    CODEX_GPT54_GPT55_COMPACTION_THRESHOLD as _CODEX_GPT54_GPT55_COMPACTION_THRESHOLD,
    CODEX_SPARK_COMPACTION_THRESHOLD as _CODEX_SPARK_COMPACTION_THRESHOLD,
    OMIT_TEMPERATURE,
    compression_threshold_for_model as _compression_threshold_for_model,
    fixed_temperature_for_model as _fixed_temperature_for_model,
    is_arcee_trinity_thinking as _is_arcee_trinity_thinking,
    is_codex_gpt54_or_gpt55 as _is_codex_gpt54_or_gpt55,
    is_codex_spark as _is_codex_spark,
    is_kimi_model as _is_kimi_model,
)
...
def _fast_model_from_catalog(provider_id: str) -> str:
    ...
    try:
        ...
        from threev0_cli.models import fetch_models_with_pricing
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``EV0_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``EV0_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "Ev0Agent",
}
...
# in lockstep with threev0_cli.__version__ across every Portal call site
...
def _nous_extra_body() -> dict:
    ...
    Computed at call time so a hot-reloaded ``threev0_cli.__version__`` is
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (3V0 Agent)",
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        #   - ``_``-prefixed keys: private 3V0 plumbing (_reasoning_config
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``threev0_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.3V0/auth.json for an active Nous provider.
...
def _nous_api_key(provider: dict) -> str:
    ...
    from threev0_cli.auth import _nous_invoke_jwt_is_usable
...
def _resolve_nous_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from threev0_cli.auth import _agent_key_is_usable
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``3v0 auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``threev0_cli.auth``'s singleton runtime resolver for older
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from 3V0 auth store.
    ...
    try:
        from threev0_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from threev0_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from threev0_cli.auth import is_provider_explicitly_configured
...
def _aux_openrouter_settings() -> Tuple[bool, str]:
    ...
    """
    try:
        from threev0_cli.config import cfg_get, load_config_readonly
...
def _resolve_moa_aggregator(preset_name: Optional[str]) -> Tuple[Optional[str], Optional[str]]:
    ...
    try:
        from threev0_cli.config import load_config
        from threev0_cli.moa_config import resolve_moa_preset
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `3v0 setup` or `3v0 model` and enter a valid http(s) base URL."
        ) from exc
...
def _try_azure_foundry(
    *,
    ...
    :func:`threev0_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from threev0_cli.runtime_provider import _resolve_azure_foundry_runtime
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second 3v0
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``3v0 config aux reset``)."""
...
def _fallback_entry_api_key(entry: Dict[str, Any]) -> Optional[str]:
    ...
    gateway multiplexing (see ``threev0_cli.fallback_config.resolve_entry_api_key``).
    """
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into 3V0' built-in discovery
...
def _resolve_auto_route(
    ...
    #    scenario where a user switches providers via `3v0 model` but the
    #    old OPENAI_BASE_URL lingers in ~/.3V0/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: 3v0 model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.3V0/.env",
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    # See _create_openai_client: disable SDK-internal retries so 3V0 owns
...
def resolve_provider_client(
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: 3v0 auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: 3v0 model)")
                return None, None
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: 3v0 model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: 3v0 doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    try:
        ...
        logger.debug("threev0_cli.auth not available for provider %s", provider)
        return None, None
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`threev0_cli.plugins.PluginContext.register_auxiliary_task`) the
...
def _build_call_kwargs(
    ...
    # Anthropic Messages adapters translate 3V0 reasoning into native
...
def _call_llm_impl(
    ...
    *,
    ...
    Args:
        ...
        reasoning_config: Optional 3V0 reasoning config for direct model calls
