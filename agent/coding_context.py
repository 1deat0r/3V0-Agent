"""Coding-context awareness — base 3V0, every interactive surface.

When the user runs 3V0 inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), 3V0 shifts into a **coding posture**. This module is the
...
from threev0_cli._subprocess_compat import bounded_git_probe

logger = logging.getLogger("3v0.coding_context")
...
_EDIT_FORMAT_GUIDANCE: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    "replace": (
        ("claude", "sonnet", "opus", "haiku",
         ...
         "3v0", "llama", "mistral", "devstral", "minimax"),
        "- Edit format: author new files with `write_file`; for edits to "
...
# in _EV0_CORE_TOOLS, so they're present on every surface this fires on.
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        selection (``--toolsets``, ``EV0_TUI_TOOLSETS``, …); they never
...
def is_coding_context(
    *,
    ...
    """Whether 3V0 should operate in its coding posture right now."""
