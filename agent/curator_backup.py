...
A pre-run snapshot of ``~/.3V0/skills/`` (excluding ``.curator_backups/``
itself) is taken before any mutating curator pass. Snapshots are tar.gz
files under ``~/.3V0/skills/.curator_backups/<utc-iso>/`` with a
...
``~/.3V0/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
def snapshot_skills(reason: str = "manual", *, protect_ids: Optional[Set[str]] = None) -> Optional[Path]:
    """Create a tar.gz snapshot of ``~/.3V0/skills/`` and prune old ones.
    ...
    if not skills.exists():
        logger.debug("No ~/.3V0/skills/ directory — nothing to back up")
        return None
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    """Restore ``~/.3V0/skills/`` from a snapshot.
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.3V0/skills/``.
    ...
    if target is None:
        return (
            ...
            + " (use `3v0 curator rollback --list` to see available snapshots)",
            None,
        )
