...
_AD_HOC_SCRIPT_NAME_PREFIXES = ("3v0-verify-", "3v0-ad-hoc-")
...
def record_verify_run(
    *,
    ...
    session_id: str | None = None,
    ok: bool,
    command: str = "3v0 verify",
    scope: str = "full",
    output: str = "",
) -> Optional[dict[str, Any]]:
    """Record a completed ``3v0 verify`` run as verification evidence.

    Explicit CLI-side write: unlike :func:`record_terminal_result` there is
    nothing to classify — the caller (the ``3v0 verify`` command) already
