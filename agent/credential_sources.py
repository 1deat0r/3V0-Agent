"""Unified removal contract for every credential source 3V0 reads from.

3V0 seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.3V0/.env
    claude_code   — ~/.claude/.credentials.json
    ev0_pkce   — ~/.3V0/.anthropic_oauth.json
...
unify here is **removal**:
    ...
    ``3v0 auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, nous device_code (partial), ev0_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.3V0/.env  → clear it
    ...
    """
    from threev0_cli.config import get_env_path, remove_env_value
    ...
    try:
        ...
        if env_path.exists():
            ...
            # canonical reader in threev0_cli/config.py. read_text() with no
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to 3V0.",
            f"  The pool entry is now suppressed — 3V0 will ignore "
            f"{env_var} until you run `3v0 auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so 3V0 stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `3v0 auth add anthropic` to re-enable if needed.",
    ])
...
def _remove_threev0_pkce(provider: str, removed) -> RemovalResult:
    ...
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared 3V0 Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    `3v0 auth add nous` (or any other path that writes providers.nous)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `3v0 auth add nous` to re-engage, which is the
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``3v0 auth remove xai-oauth <N>`` silently undoes
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `3v0 model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the 3V0 auth store is not enough — _seed_from_singletons() would
    ...
    via ``3v0 auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `3v0 auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `3v0 auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    3V0 stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `3v0 auth add copilot` to re-enable if needed.",
    ])
