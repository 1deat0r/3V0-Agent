...
configured under ``monitoring.export.otlp``. Lets an operator stream 3V0
...
Notes:
  ...
  * ``opentelemetry-sdk`` + ``opentelemetry-exporter-otlp-proto-http`` are an
    optional extra (``pip install 3v0-agent[otlp]``), imported lazily so the
...
def _span_attrs(ev: Dict[str, Any]) -> Dict[str, Any]:
    ...
    kind = ev.get("event")
    attrs: Dict[str, Any] = {"3v0.event": kind or "unknown"}
    keep_by_kind = {
...
def export_batch(provider, batch: List[Dict[str, Any]]) -> int:
    ...
    tracer = provider.get_tracer("3v0.monitoring")
    n = 0
    for ev in batch:
        try:
            name = f"3v0.{ev.get('event', 'event')}"
    ...
    *,
    ...
    try:
        ...
        logger.warning("monitoring.export.otlp.enabled but the OTel SDK could not "
                       "be installed/imported; install '3v0-agent[otlp]'")
        return None
    from agent.monitoring.emitter import get_emitter
