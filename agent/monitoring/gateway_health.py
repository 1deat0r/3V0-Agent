...
def _base_attrs(*, profile: str, install_id: str, version: str, supervision_mode: str) -> Dict[str, str]:
    ...
    return {
        ...
        "3v0.supervision_mode": mode if mode in _SUPERVISION_MODES else "unknown",
    }
...
def build_gateway_health_snapshot(
    ...
    *,
    ...
    metrics: list[GatewayMetric] = [
        _metric("3v0.gateway.up", 1 if gateway_running else 0, base),
        _metric("3v0.gateway.active_agents", active_agents, base),
        _metric("3v0.gateway.busy", 1 if busy else 0, base),
        _metric("3v0.gateway.drainable", 1 if drainable else 0, base),
        _metric("3v0.gateway.restart_requested", 1 if runtime.get("restart_requested") else 0, base),
    ]
    if gateway_state:
        metrics.append(_metric("3v0.gateway.state", 1, base, **{"3v0.gateway.state": str(gateway_state)}))
    ...
    for platform, pdata in platforms.items():
        ...
        metrics.append(_metric(
            ...
            1 if is_up else 0,
            base,
            **{"3v0.platform": str(platform), "3v0.platform.state": state},
        ))
        metrics.append(_metric(
            "3v0.platform.degraded",
            1 if is_degraded else 0,
            base,
            **{"3v0.platform": str(platform), "3v0.platform.state": state, "3v0.error_code": error_code},
        ))
        if is_degraded:
