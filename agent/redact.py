...
# `export EV0_REDACT_SECRETS=false`) cannot disable redaction
# mid-session.  ON by default — secure default per issue #17691. Users who
# need raw credential values in tool output (e.g. working on the redactor
# itself) can opt out via `security.redact_secrets: false` in config.yaml
# (bridged to this env var in threev0_cli/main.py, gateway/run.py, and
...
def mask_secret(
    ...
    *,
    ...
    Canonical helper for display-time redaction across 3V0 — used by
    ``3v0 config``, ``3v0 status``, ``3v0 dump``, and anywhere
...
def redact_sensitive_text(
    ...
    *,
    ...
    for URLs, ``"eyJ" in text`` for JWTs). On a typical 3v0 log line
...
# (``security.redact_secrets: false`` / EV0_REDACT_SECRETS) applies to
