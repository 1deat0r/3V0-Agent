...
* Python plugins and shell hooks compose naturally: both flow through
  :func:`ev0_cli.plugins.invoke_hook` and its aggregators.  Python
...
* Registration is idempotent — safe to invoke from both the CLI entry
  point (``ev0_cli/main.py``) and the gateway entry point
  (``gateway/run.py``).
...
from ev0_cli._subprocess_compat import IS_WINDOWS, kill_process_tree, windows_hide_flags
...
# ev0_cli.plugins.get_pre_tool_call_block_message / _get_pre_tool_call_
...
def register_from_config(
    ...
    *,
    ...
    ``cfg`` is the full parsed config dict (``ev0_cli.config.load_config``
...
def _parse_hooks_block(hooks_cfg: Any) -> List[ShellHookSpec]:
    ...
    """
    from ev0_cli.plugins import SHELL_UNSUPPORTED_HOOKS, VALID_HOOKS
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    ...
    :func:`ev0_cli.plugins.get_pre_tool_call_block_message`.  This is
...
def run_once(
    ...
    ``kwargs`` is the same dict that :func:`ev0_cli.plugins.invoke_hook`
