...
def _warn_memory_provider_unavailable(name: str, reason: str = "") -> None:
    ...
    not inheriting ``~/.3V0/.env``. See 1deat0r/3V0-Agent#2765.
...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    routes: set[str] = set()
    try:
        from threev0_cli.providers import EV0_OVERLAYS, get_provider
    ...
    try:
        ...
        from threev0_cli.models import normalize_provider as normalize_model_provider
        from threev0_cli.providers import normalize_provider as normalize_registry_provider
...
def init_agent(
    ...
    agent._cache_ttl = "5m"
    try:
        from threev0_cli.config import load_config_readonly as _load_pc_cfg
    ...
    if agent.verbose_logging:
        ...
        # on the fact that threev0_logging.setup_logging() does not
        ...
        # handler level inside threev0_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from threev0_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from threev0_cli.config import load_config_readonly as _load_br_cfg
    ...
    try:
        from threev0_cli.config import load_config_readonly as _load_sess_cfg
    ...
    try:
        from threev0_cli.config import load_config_readonly as _load_agent_config
    ...
    if _config_context_length is not None and isinstance(_model_cfg, dict):
        ...
        if _configured_provider_norm in _runtime_first_provider_ids:
            ...
            try:
                from threev0_cli.auth import resolve_provider as resolve_auth_provider
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from threev0_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from threev0_cli.plugins import get_plugin_context_engine
