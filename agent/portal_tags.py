...
Every 3V0 request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to 3V0 Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=3v0-agent",
        "client=3v0-client-v<__version__>",
    ]

The version is sourced live from ``ev0_cli.__version__`` so it auto-aligns
...
``ev0_cli.__version__`` is the canonical source of truth.
"""
...
def _ev0_version() -> str:
    ...
    Falls back to ``"unknown"`` if ``ev0_cli`` cannot be imported (should
...
def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a 3V0 session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    3V0 conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.3V0/sessions/`` storage, session logs, and lineage.
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    appended so Portal usage can be attributed to a specific 3V0
    ...
    """
    tags = ["product=3v0-agent", ev0_client_tag()]
