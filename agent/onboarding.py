...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time 3V0 starts and finds ``~/.openclaw/``.

    Points users at ``3v0 claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``3v0 claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to 3V0, run "
        "`3v0 claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `3v0 claw cleanup` (renames it to ~/.openclaw.pre-migration — "
