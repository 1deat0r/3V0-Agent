...
Plugins built on 3V0 Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing 3V0
...
:class:`~threev0_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors 3V0' main config
...
handles every provider, fallback chain, and per-task override 3V0
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`threev0_cli.plugins.PluginContext`
