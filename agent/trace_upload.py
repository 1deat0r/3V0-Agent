...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``3v0 trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through 3V0' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "3v0-traces"
_EV0_VERSION = "3v0-agent"
...
# ---------------------------------------------------------------------------
# Conversion: 3V0 OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses 3V0' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render 3V0 conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.3V0/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `3v0 trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/3v0-traces`` dataset. Returns a
