...
import threev0_cli.auth as auth_mod
...
def _write_through_provider_state_to_global_root(
    ...
    ``threev0_cli.auth._write_through_xai_oauth_to_global_root`` (which covers
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith(("ev0_pkce", "threev0_pkce")),
                )
                updated = replace(
                    entry,
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith(("ev0_pkce", "threev0_pkce")),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    try:
        from threev0_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
    ...
    if provider == "anthropic":
        ...
        try:
            from threev0_cli.auth import resolve_qwen_runtime_credentials
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    try:
        from threev0_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from threev0_cli.env_loader import get_secret_source
