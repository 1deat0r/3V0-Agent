...
# un-throttled log fires on *every* selection. On Windows several 3V0
...
# readiness handshake ("Timed out connecting to 3V0 backend after
...
def _write_through_provider_state_to_global_root(
    ...
    ``ev0_cli.auth._write_through_xai_oauth_to_global_root`` (which covers
...
class CredentialPool:
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        (e.g. setup wizard, ``3v0 auth status``) would land on
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sync→POST→write-back sequence below must run atomically across 3V0
    ...
    def _single_use_refresh_lock_timeout(self) -> float:
        ...
        resolves.  Reads the provider's ``EV0_*_REFRESH_TIMEOUT_SECONDS``
        override.
        """
        env_var = (
            "EV0_CODEX_REFRESH_TIMEOUT_SECONDS"
            if self.provider == "openai-codex"
            else "EV0_XAI_REFRESH_TIMEOUT_SECONDS"
        )
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("ev0_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # refresh_token — single-use tokens consumed by another 3V0
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("ev0_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/nous — another 3V0 process
    ...
    def _available_entries(
        ...
        *pending_refresh* tuples so the caller can execute them outside the
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other 3V0 profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `3v0 model` / `3v0 auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `3v0 model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``3v0 auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `3v0 auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    try:
        from ev0_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, 3V0 PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `3v0 setup` ("Claude
        ...
        # Claude Code CLI, ev0_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.3V0/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `3v0 setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"ev0_pkce", "claude_code"}
            ]
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `3v0 auth add nous --label <name>`).  Fall back to the
        ...
        try:
            ...
            # (3v0 auth remove copilot gh_cli) must not pay either on
        ...
        # 3V0 auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from ev0_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.3V0/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `3v0 auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `3v0 auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the 3V0 auth store.  Without this gate the removal is instantly
        ...
        # When the user logs in via ``3v0 model`` -> xAI Grok OAuth,
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            from ev0_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
# Prefer ~/.3V0/.env over os.environ — the user's config file is the
# authoritative source for 3V0 credentials. Stale env vars from parent
...
def get_env_prefer_dotenv(key: str) -> str:
    ...
    # load_ev0_dotenv() -> apply_onepassword_secrets()).  The raw
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Honour user suppression — `3v0 auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.3V0/.env.
    ...
    try:
        from ev0_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from ev0_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.3V0/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `3v0 auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and 3V0
