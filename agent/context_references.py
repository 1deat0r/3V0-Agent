...
_SENSITIVE_EV0_DIRS = (Path("skills") / ".hub",)
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    threev0_home = get_threev0_home().resolve()

    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES}
    blocked_exact.add(threev0_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(threev0_home / rel for rel in _SENSITIVE_EV0_DIRS)
    ...
    # dir — bypassing the exact threev0_home denylist below. Shared
    # classifier: single source of truth for credential-store shapes
    # (agent/file_safety.CREDENTIAL_FILE_NAMES), so this call can never
    # drift from the canonical read-deny list again (#86213).
    ...
    for blocked_dir in blocked_dirs:
        try:
            ...
            continue
        raise ValueError("path is a sensitive credential or internal 3V0 path and cannot be attached")
    ...
    # message text into reference expansion, so `@file:~/.3V0/auth.json` from a
