...
Storage
-------
Bundles live in ``~/.3V0/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``EV0_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``3v0 bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under EV0_HOME.
    ...
    return get_ev0_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `3v0 bundles` CLI subcommand.
# ---------------------------------------------------------------------------
