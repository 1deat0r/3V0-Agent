...
* The ``iron-proxy`` binary is auto-installed into ``<ev0_home>/bin/iron-proxy``
  on first use.  3V0 pins one upstream version (``_IRON_PROXY_VERSION``)
...
* A long-lived CA at ``<ev0_home>/proxy/ca.{crt,key}`` is generated on
  first ``3v0 egress setup``.  Sandboxes trust this CA so iron-proxy can
  terminate TLS and rewrite headers.

* The proxy config lives at ``<ev0_home>/proxy/proxy.yaml``.  It enumerates
...
* The proxy runs as a managed subprocess (``3v0 egress start``), pidfile
  at ``<ev0_home>/proxy/iron-proxy.pid``.  Daemon output (including
  per-request records on v0.39) goes to ``<ev0_home>/proxy/iron-proxy.log``;
...
# request needs the bearer key below.  ``3v0 egress reload`` is the
# client.
#
# The key is minted at setup time, stored at
# ``<ev0_home>/proxy/management.token`` (0600), and injected into the
...
_MGMT_API_KEY_ENV = "EV0_IRON_PROXY_MGMT_KEY"
...
# credential (3V0' auth.py keys Google on both GEMINI_API_KEY and
...
def find_iron_proxy(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<ev0_home>/bin/iron-proxy``  (our managed copy — preferred)
...
def install_iron_proxy(*, force: bool = False) -> Path:
    ...
    these; the user-facing ``3v0 proxy install`` surface lets them
    ...
    with tempfile.TemporaryDirectory(prefix="3v0-iron-proxy-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "3v0-agent"})
    try:
...
def iron_proxy_version(binary: Path) -> str:
    ...
    # Don't cache empty output — that would poison ``3v0 egress
...
def ensure_ca_cert(*, force: bool = False) -> Tuple[Path, Path]:
    ...
    with tempfile.TemporaryDirectory(prefix="3v0-proxy-ca-") as tmpdir:
        tmp = Path(tmpdir)
        tmp_key = tmp / "ca.key"
        tmp_crt = tmp / "ca.crt"
...
def mint_proxy_token(prefix: str = "3v0-proxy") -> str:
...
def ensure_management_token(*, force: bool = False) -> str:
    ...
    Stored at ``<ev0_home>/proxy/management.token`` with 0600 perms.
    The daemon receives it via the ``EV0_IRON_PROXY_MGMT_KEY`` env var
    (named in the generated config's ``management.api_key_env``);
    ``3v0 egress reload`` reads the same file to authenticate.
    """
    ...
    if not force and p.exists():
        try:
            ...
            pass
    token = mint_proxy_token(prefix="3v0-mgmt")
    fd = os.open(
        str(p),
...
def reload_proxy() -> bool:
    ...
    if not pid or not _pid_alive(pid):
        raise RuntimeError(
            ...
            "Run `3v0 egress start`."
        )
    mgmt = _read_management_listen_from_config()
    if mgmt is None:
        raise RuntimeError(
            "The generated proxy.yaml has no management listener (written "
            "before reload support).  Re-run `3v0 egress setup` and use "
            "`3v0 egress restart` this one time."
        )
    token = _read_management_token()
    if not token:
        raise RuntimeError(
            "management.token is missing — re-run `3v0 egress setup`, "
            "then `3v0 egress restart`."
        )
...
def build_proxy_config(
    ...
    * ``secrets`` transform takes ``secrets:`` (plural), each with a
    ...
    return {
        ...
        # in ``ProxyStatus`` / ``3v0 egress status``.
        "metrics": {
            "listen": "127.0.0.1:0",
        },
        ...
        # pipeline — `3v0 egress reload` applies allowlist/token/mapping
...
def write_proxy_config(config: Dict) -> Path:
    """Serialize the config dict to ``<ev0_home>/proxy/proxy.yaml``.

    Uses ``yaml.safe_dump`` so we never emit Python tags.
    """

    try:
        import yaml  # PyYAML is already a 3V0 dep
...
def discover_uncovered_providers(
    *,
    ...
    The wizard and ``3v0 egress status`` use this to print a warning.
...
def merge_mappings(
    *,
    ...
    re-running ``3v0 egress setup`` should not invalidate the tokens
...
# arbitrary env value).
_EV0_IRON_PROXY_NONCE_ENV = "EV0_IRON_PROXY_NONCE"
    ...
    if nonce_candidates:
        try:
            ...
            for nonce in nonce_candidates:
                needle = f"{_EV0_IRON_PROXY_NONCE_ENV}={nonce}".encode()
...
def start_proxy(
    *,
    ...
    if bin_path is None:
        raise RuntimeError(
            "iron-proxy binary not available — run `3v0 egress install`."
        )
    ...
    env[_EV0_IRON_PROXY_NONCE_ENV] = _proxy_nonce
    ...
    # close-on-success path, every restart leaked one fd in the 3V0
    # process.
    #
    ...
    # ``3v0 egress stop`` can clean up the orphan.  Failure paths
    ...
    # hits Ctrl-C while waiting for ``3v0 egress start`` doesn't leak
    ...
    try:
        ...
        if existing_pid and _pid_alive(existing_pid):
            raise RuntimeError(
                ...
                f"Run `3v0 egress stop` if that proxy is stuck."
            )
            ...
            if access_token and project_id:
                ...
                if missing:
                    ...
                    if not (bitwarden_config or {}).get("allow_env_fallback"):
                        raise RuntimeError(
                            ...
                            f"`3v0 egress setup --no-bitwarden`, or set "
                ...
                if warnings:
                    logger.warning(
                        ...
                        "run `3v0 secrets bitwarden status` for detail.",
                        len(warnings),
                    )
            else:
