...
``CommandSecretsProvider`` (3v0-desktop ``src/main/secrets/commandProvider.ts``)
...
* The requested key is passed to the child ONLY via the ``EV0_SECRET_KEY``
...
* The startup/apply path runs the helper exactly ONCE (with an empty
  ``EV0_SECRET_KEY``) — it is never called per-key in a loop, so a
...
def _run_helper(
    ...
    The key is passed as DATA via ``EV0_SECRET_KEY`` — never interpolated
...
def get_command_secret(
    *,
    ...
    ``EV0_SECRET_KEY``.  Returns None on any failure — never raises."""
...
def apply_command_secrets(
    *,
    ...
    # EV0_SECRET_KEY and parse its stdout as a dotenv blob.
    stdout = _run_helper(command, "", timeout_seconds, max_output_bytes)
    if stdout is None:
...
class CommandSource(SecretSource):
    ...
    Bulk shape: the helper enumerates a KEY=VALUE blob in one run.  Config::
        ...
        secrets:
          command:
            enabled: true
            command: "cat /run/user/1000/3v0-secrets.env"
    ...
    def remediation(self, kind, cfg: dict) -> str:
        ...
        if kind == ErrorKind.INTERNAL:
            return (
                ...
                "3V0 discards helper stderr so diagnostics can't leak "
                "secret material."
            )
