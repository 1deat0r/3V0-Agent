...
3V0 pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.3V0/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<ev0_home>/bin/bws`` on
  first use.  3V0 pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.3V0/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``3v0`` invocations don't
  hammer the API.
* Failures NEVER block 3V0 startup.  Missing binary, no network,
...
# In-process cache so repeated load_ev0_dotenv() calls (CLI startup,
# gateway hot-reload, test suites) don't re-fetch from BSM.
_CacheKey = Tuple[str, str, str]  # (access_token_fingerprint, project_id, server_url)
_CACHE: Dict[_CacheKey, _CachedFetch] = {}

# Disk-persisted cache so back-to-back CLI invocations (e.g. `3v0 chat -q ...`
...
# <ev0_home>/cache/bws_cache.json. The file holds only the secret VALUES,
# never the access token. It's plaintext-equivalent to ~/.3V0/.env (which
...
_ENCRYPTED_CACHE_INFO = b"3v0-bws-encrypted-cache-v1"
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<ev0_home>/bin/bws``  (our managed copy — preferred)
...
def install_bws(*, force: bool = False) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="3v0-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _summarize_bws_stderr(raw: str) -> str:
    ...
    Everything from ``Location:`` on is diagnostic noise for a 3V0
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_ev0_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `3v0 secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `3v0 secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `3v0 secrets bitwarden setup` to install."
        )
        return result
...
def clear_caches(home_path: Optional[Path] = None) -> None:
    ...
    Used after a token rotation (`3v0 secrets bitwarden token`) so the
