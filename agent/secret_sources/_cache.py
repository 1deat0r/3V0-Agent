...
cache problem must never block 3V0 startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the 3V0 home used for cache paths.

    ``home_path`` is whatever ``load_ev0_dotenv()`` already resolved;
    falling back to ``$EV0_HOME`` / ``~/.3V0`` keeps direct callers
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<threev0_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if EV0_HOME is ever made traversable.
            try:
