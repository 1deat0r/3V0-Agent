"""Anthropic Messages API adapter for 3V0 Agent.

Translates between 3V0's internal OpenAI-style message format and
...
# 3V0 effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to 3v0's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to 3v0's outer conversation
    ...
    if _is_kimi_coding_endpoint(base_url):
        ...
        # HTTP-Referer + X-Title + Ev0Agent User-Agent.
    ...
    if _is_opencode_endpoint(base_url):
        ...
        headers.setdefault("HTTP-Referer", "https://github.com/1deat0r/3V0-Agent")
        headers.setdefault("X-Title", "3V0 Agent")
        headers.setdefault("User-Agent", f"Ev0Agent/{_EV0_VERSION}")
        kwargs["default_headers"] = headers

    client = _anthropic_sdk.Anthropic(**kwargs)
    # Bearer-only construction leaves ``api_key`` unset, so the SDK fills it
    # from ``ANTHROPIC_API_KEY`` (3V0 loads that into the process env from
    # ``~/.3V0/.env``). The result is dual auth —
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    3V0 notices an expired token, Claude Code may have already rotated it.
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    3V0 historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``3v0 models``) never mutates ``~/.3V0/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by 3V0)
      ...
      5. Anthropic credential_pool OAuth entry (~/.3V0/auth.json)
    ...
    # 1. 3V0-managed OAuth/setup token env var
    ...
    # 5. 3V0 credential_pool OAuth entry.
...
# ── 3V0-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.3V0/.anthropic_oauth.json (our own file).
...
def run_threev0_oauth_login_pure() -> Optional[Dict[str, Any]]:
    ...
    print()
    print("Authorize 3V0 with your Claude Pro/Max subscription.")
    print()
...
def read_threev0_oauth_credentials() -> Optional[Dict[str, Any]]:
    ...
    if oauth_file.exists():
        try:
            ...
            logger.debug("Failed to read 3V0 OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See 3v0-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for i, m in enumerate(result):
        ...
        # replaying it verbatim.  See 3v0-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See 3v0-agent#13848 (Kimi) and
    3v0-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("3V0 Agent", "Claude Code")
                text = text.replace("3V0 agent", "Claude Code")
                text = text.replace("3v0-agent", "claude-code")
        ...
        #      a) bare 3V0-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that 3V0 surfaces in its CLI. We
