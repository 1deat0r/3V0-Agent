"""Lightweight internationalization (i18n) for 3V0 static user-facing messages.

Scope (thin slice, by design): only the highest-impact static strings shown
to the user by 3V0 itself -- approval prompts, a handful of gateway slash
...
Language resolution order:
    ...
    2. ``EV0_LANGUAGE`` environment variable (for tests / quick override)
...
def _locales_dir() -> Path:
    ...
    1. ``EV0_BUNDLED_LOCALES`` env var -- set by the Nix wrapper (or any
    ...
    if override:
        ...
        if candidate.is_dir():
            return candidate
        logger.warning(
            "EV0_BUNDLED_LOCALES points to a non-directory path (%s); "
...
def _load_catalog(lang: str) -> dict[str, str]:
    ...
    try:
        import yaml  # PyYAML is already a 3v0 dependency
