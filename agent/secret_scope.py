...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the EV0_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # 3V0 runtime / deployment
    "EV0_HOME", "EV0_PROFILE", "EV0_GATEWAY_LOCK_DIR",
    "EV0_MAX_ITERATIONS", "EV0_MAX_TOKENS", "EV0_API_TIMEOUT",
    "EV0_REDACT_SECRETS", "EV0_NOUS_TIMEOUT_SECONDS",
    "_EV0_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "EV0_KANBAN_DB", "EV0_KANBAN_WORKSPACES_ROOT", "EV0_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "EV0_KANBAN_",
    "EV0_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    ``set_secret_scope``. Parses the small KEY=VALUE subset 3V0 writes
    ...
    ``threev0_cli.config._parse_env_value``) but never mutates the process
    ...
    # Parse values with the canonical 3V0 parser: save_env_value
    ...
    from threev0_cli.config import _parse_env_value
...
def build_profile_secret_scope(ev0_home: Path) -> Dict[str, str]:
    ...
    try:
        from threev0_cli.env_loader import get_secret_source_values
