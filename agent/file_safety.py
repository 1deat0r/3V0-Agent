...
def _threev0_home_path() -> Path:
    """Resolve the active EV0_HOME (profile-aware) without circular imports."""
    try:
        from threev0_constants import get_threev0_home  # local import to avoid cycles
        return get_threev0_home()
    except Exception:
        return Path(os.path.expanduser("~/.3V0"))


def _threev0_root_path() -> Path:
    """Resolve the 3V0 root dir (always the parent of any profile, never per-profile)."""
    try:
        from threev0_constants import get_default_threev0_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    threev0_home = _threev0_home_path()
    threev0_root = _threev0_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(threev0_home / ".env"),
            # Top-level .env, even when running under a profile — overwriting it
            # leaks credentials across every profile that inherits from root (#15981).
            str(threev0_root / ".env"),
            # Active profile Anthropic PKCE credential store.
            str(threev0_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(threev0_root / ".anthropic_oauth.json"),
            # Bitwarden Secrets Manager encrypted disk cache.
            str(threev0_home / "cache" / "bws_cache.enc.json"),
            str(threev0_root / "cache" / "bws_cache.enc.json"),
...
def _classify_write_denial(path: str) -> Optional[str]:
    ...
    threev0_dirs = []
    for base in (_threev0_home_path(), _threev0_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in threev0_dirs:
                threev0_dirs.append(real)
        except Exception:
            continue

    for base_real in threev0_dirs:
...
def get_write_denied_error(path: str, *, verb: str = "Write") -> Optional[str]:
    ...
    if denial == "safe_root":
        ...
        return (
            f"{verb} denied: '{path}' is outside EV0_WRITE_SAFE_ROOT "
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied 3V0 path.

    Three categories are blocked:

      * Internal 3V0 cache files under ``EV0_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under EV0_HOME and the global 3V0
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.3V0/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active EV0_HOME (profile-aware) AND the global
    # 3V0 root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (EV0_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    threev0_dirs: list[Path] = []
    for base in (_threev0_home_path(), _threev0_root_path()):
        try:
            real = base.resolve()
            if real not in threev0_dirs:
    ...
    for hd in threev0_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal 3V0 cache file "
    ...
    for hd in threev0_dirs:
        for name in CREDENTIAL_FILE_NAMES:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a 3V0 credential store "
    ...
    for hd in threev0_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the 3V0 MCP token directory "
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied 3V0 read (see
...
#
# 3V0 profiles are separate EV0_HOME dirs under
...
#
# Reference: May 2026 incident where a 3v0-security profile session
# edited skills under both ``~/.3V0/profiles/3v0-security/skills/``
# AND ``~/.3V0/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under EV0_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from EV0_HOME.
    ...
    """
    try:
        home_real = _threev0_home_path().resolve()
        root_real = _threev0_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside 3V0 scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    3V0 entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to 3V0 profile {info['target_profile']!r}, but the "
...
#
#   <EV0_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.3V0/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.3V0/…`` segment and warns
# regardless of which 3V0 profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.3V0/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.3V0`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.3V0/…`` and returns the
    index where the inner 3V0-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .3V0 / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".3V0":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative 3V0 state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.3V0``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.3V0`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any 3V0 resolver to
    succeed, so it works correctly even when called from contexts where
    EV0_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host 3V0 process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real EV0_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.3V0/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.3V0/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host EV0_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative 3V0 state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host 3V0 "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real EV0_HOME. Use the "
