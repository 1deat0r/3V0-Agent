...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    across four ``ev0_cli`` modules using three different Windows
    ...
    A few sites outside ``ev0_cli`` (``tools/code_execution_tool.py``,
    ...
    *windows* lets a caller pass its own platform verdict. Several callers
    ...
    Windows paths on Linux CI (``ev0_cli.main._is_windows`` and friends);
