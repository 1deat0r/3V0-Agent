...
- `ev0_cli/runtime_provider.py` — credential resolution, custom-endpoint runtime resolution
- `ev0_cli/auth.py` — provider registry, `resolve_provider()`
- `ev0_cli/model_switch.py` — shared `/model` switch pipeline (CLI + gateway)
...
4. **Config flow**:
   - CLI: reads the fallback chain via `ev0_cli/fallback_config.get_fallback_chain()` → passes to `AIAgent(fallback_model=...)`
...
- `tests/ev0_cli/test_fallback_cmd.py` — the `/fallback` CLI command
