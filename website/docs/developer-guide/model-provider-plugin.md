...
|---|---|---|
| Credential resolution | `ev0_cli/auth.py` | `PROVIDER_REGISTRY["acme-inference"]` populated from profile |
| `--provider` CLI flag | `ev0_cli/main.py` | Accepts `acme-inference` |
| `hermes model` picker | `ev0_cli/models.py` | Appears in `CANONICAL_PROVIDERS`, model list fetched from `{base_url}/models` |
| `hermes doctor` | `ev0_cli/doctor.py` | Health check for `ACME_API_KEY` + `{base_url}/models` probe |
| `hermes setup` | `ev0_cli/config.py` | `ACME_API_KEY` appears in `OPTIONAL_ENV_VARS` and the setup wizard |
...
| Runtime resolution | `ev0_cli/runtime_provider.py` | Returns correct `base_url`, `api_key`, `api_mode` |
