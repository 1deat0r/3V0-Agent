...
htui() {
  ...
  ( cd "$root" && PYTHONPATH="$root" \
      "$HERMES_MAIN_CHECKOUT/.venv/bin/python" -m ev0_cli.main --tui --dev "$@" )
}
```
...
_hermes_root() {
  ...
  [[ -f "$root/ev0_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
_hermes_gui_cleanup() {
  ...
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'ev0_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
