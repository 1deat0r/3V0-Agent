...
1. `ev0_cli/auth.py` decides how credentials are found.
2. `ev0_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `ev0_cli/models.py` and `ev0_cli/main.py` make the provider show up in the CLI. (`ev0_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
:::tip
`ev0_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `hermes setup`.
:::
...
- `PROVIDER_REGISTRY` in `ev0_cli/auth.py`
- `_PROVIDER_LABELS` in `ev0_cli/models.py`
- `_PROVIDER_ALIASES` in both `ev0_cli/auth.py` and `ev0_cli/models.py`
- CLI `--provider` choices in `ev0_cli/main.py`
...
## Step 2: Add auth metadata in `ev0_cli/auth.py`
...
## Step 3: Add model catalog and aliases in `ev0_cli/models.py`
...
## Step 4: Resolve runtime data in `ev0_cli/runtime_provider.py`
...
:::tip
`ev0_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `hermes model` and `hermes setup` automatically.
:::
...
- `tests/ev0_cli/test_runtime_provider_resolution.py`
- `tests/cli/test_cli_provider_resolution.py`
- `tests/ev0_cli/test_model_switch_custom_providers.py` (and adjacent `tests/ev0_cli/test_model_switch_*.py`)
- `tests/ev0_cli/test_setup_model_provider.py`
...
python -m pytest tests/ev0_cli/test_runtime_provider_resolution.py tests/cli/test_cli_provider_resolution.py tests/ev0_cli/test_setup_model_provider.py tests/run_agent/test_provider_parity.py -q
```
...
```bash
source venv/bin/activate
python -m ev0_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
```bash
source venv/bin/activate
python -m ev0_cli.main model
python -m ev0_cli.main setup
```
...
- [ ] `ProviderConfig` added in `ev0_cli/auth.py`
- [ ] aliases added in `ev0_cli/auth.py` and `ev0_cli/models.py`
- [ ] model catalog added in `ev0_cli/models.py`
- [ ] runtime branch added in `ev0_cli/runtime_provider.py`
- [ ] CLI wiring added in `ev0_cli/main.py` (setup.py inherits automatically)
