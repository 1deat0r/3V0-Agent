...
Abridged — see `SCHEMA_SQL` in `ev0_state.py` for the full current column list
...
indexed columns — see `SCHEMA_SQL` in `ev0_state.py` for the exact SQL.
...
This is derived from `ev0_constants.get_hermes_home()` which resolves to
