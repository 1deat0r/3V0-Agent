...
const BACKEND = process.env.EV0_DASHBOARD_URL ?? "http://127.0.0.1:9119";
 ...
 * load, scrapes the `window.__EV0_SESSION_TOKEN__` assignment, and
 * re-injects it into the dev HTML. No-op in production builds.
 */
function ev0DevToken(): Plugin {
  const TOKEN_RE = /window\.__EV0_SESSION_TOKEN__\s*=\s*"([^"]+)"/;
  const EMBEDDED_RE =
    /window\.__EV0_DASHBOARD_EMBEDDED_CHAT__\s*=\s*(true|false)/;

  return {
    name: "3v0:dev-session-token",
    apply: "serve",
    async transformIndexHtml() {
      try {
        ...
        if (!match) {
          console.warn(
            `[3v0] Could not find session token in ${BACKEND} — ` +
              `is \`3v0 dashboard\` running? /api calls will 401.`,
          );
          return;
        }
        ...
        return [
          {
            ...
            injectTo: "head",
            children:
              `window.__EV0_SESSION_TOKEN__="${match[1]}";` +
              `window.__EV0_DASHBOARD_EMBEDDED_CHAT__=${embeddedJs};`,
          },
        ];
      } catch (err) {
        console.warn(
          `[3v0] Dashboard at ${BACKEND} unreachable — ` +
            `start it with \`3v0 dashboard\` or set EV0_DASHBOARD_URL. ` +
...
export default defineConfig({
  plugins: [react(), tailwindcss(), ev0DevToken()],
  resolve: {
    alias: {
      "@": path.resolve(__dirname, "./src"),
      "@3v0/shared": path.resolve(__dirname, "../apps/shared/src"),
    },
  ...
  server: {
    proxy: {
      ...
      // Same host as `3v0 dashboard` must serve these; Vite has no
