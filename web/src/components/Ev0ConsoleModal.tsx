...
interface Ev0ConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function Ev0ConsoleModal({ open, onClose }: Ev0ConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("3v0> ");
  const inputPromptRef = useRef("3v0> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "3v0> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to 3V0 Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="3v0-console-title"
    >
      <div
        className={cn(
          themedBody,
