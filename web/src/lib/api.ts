import { buildEv0WebSocketUrl } from "@3v0/shared";

// The dashboard can be served either at the root of its host (e.g.
// https://kanban.tilos.com/) or under a URL prefix when reverse-proxied
// (e.g. https://mission-control.tilos.com/3v0/). The Python backend
// injects ``window.__EV0_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__EV0_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const EV0_BASE_PATH = readBasePath();
const BASE = EV0_BASE_PATH;
...
declare global {
  interface Window {
    __EV0_SESSION_TOKEN__?: string;
    __EV0_BASE_PATH__?: string;
    ...
    __EV0_AUTH_REQUIRED__?: boolean;
  }
}
const SESSION_HEADER = "X-3V0-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__EV0_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``3v0 update``, ``3v0 gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__EV0_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__EV0_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__EV0_SESSION_TOKEN__`` is valid, so the next
 ...
 *
 * The dashboard's gated-mode WS auth (``threev0_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__EV0_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-3V0-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``ev0_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__EV0_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  updateEv0: () =>
    fetchJSON<ActionResponse>("/api/3v0/update", { method: "POST" }),
  checkEv0Update: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/3v0/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface SkillHubSource {
  ...
  /** 3v0-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``3v0 update``. */
  can_update_ev0?: boolean;
  config_path: string;
  ...
  /** NS-656: disk-usage rollup for the EV0_HOME volume. Absent on
   * older gateways. */
 ...
 * sample of the EV0_HOME volume — no staleness dimension, so no
 * sampled_at. */
