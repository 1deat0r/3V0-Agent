...
import { api, EV0_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "3v0:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${EV0_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${EV0_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?ev0_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-3v0-plugin", manifest.name);
