...
const SIDEBAR_COLLAPSED_KEY = "3v0-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="3v0-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="3v0-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateEv0 = status?.can_update_ev0 === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run '3v0 update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "3v0 update";
    return (
      t.status.updateEv0ConfirmMessage ??
      `This will run '3v0 update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateEv0ConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateEv0) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateEv0,
      runningLabel: t.status.updatingEv0,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the 3V0 gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateEv0ConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateEv0ConfirmTitle ?? `${t.status.updateEv0}?`}
    />
    </>
  );
}
