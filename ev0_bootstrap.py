...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``ev0_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``ev0_bootstrap``, never ``ev0_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
# Apply on import — entry points just need ``import ev0_bootstrap``
# (or ``from ev0_bootstrap import apply_windows_utf8_bootstrap``) at
