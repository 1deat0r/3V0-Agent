...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.3V0",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``EV0_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside EV0_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.

    Containment alone is not sufficient, because EV0_HOME is exactly where
    ...
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to EV0_HOME)",
            relative_path,
        )
        return False
    ...
    # inside EV0_HOME and therefore pass the containment check above.
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="3v0-skills-safe-"))
...
def get_cache_directory_mounts(
    ...
    for new_subpath, old_name in _CACHE_DIRS:
        ...
        if not host_dir.is_dir():
            ...
            # mount costs the feature. get_ev0_dir() already resolved
...
def map_cache_path_to_container(
    ...
    caller decides which ``container_base`` applies (Docker ``/root/.3V0``,
    SSH ``<remote_home>/.3V0``, etc.) and whether translation is wanted.
