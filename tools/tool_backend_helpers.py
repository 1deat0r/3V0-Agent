...
def nous_tool_gateway_unavailable_message(
    ...
    *,
    ...
    try:
        ...
        pass
    return (
        f"{capability} is unavailable. Run `3v0 model` to refresh your "
...
def resolve_provider_secret(
    ...
    Resolution order (fixes #68003 — keys added via ``3v0 auth add
    ...
    2. The environment / ``~/.3V0/.env``. Under a multiplexed gateway turn
       ...
       ``ev0_cli.config.get_env_value`` (os.environ, then ``.env``),
       matching the tools' historical behaviour exactly.
    3. The credential pool / auth store for ``provider_id`` (``3v0 auth
    ...
    when omitted, ``ev0_cli.config.get_env_value`` is used directly.
    """
    ...
    try:
        ...
        # `3v0 auth add <provider>` keys a registry provider by its plain
...
def resolve_openai_audio_api_key() -> str:
    ...
    the credential pool (``3v0 auth add openai-api``) via
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.3V0/.env`` (via
    ``ev0_cli.config.get_env_value`` when available) so tool-side
