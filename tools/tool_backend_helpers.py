...
def resolve_provider_secret(
    ...
    2. The environment / ``~/.3V0/.env``. Under a multiplexed gateway turn
       ...
       ``threev0_cli.config.get_env_value`` (os.environ, then ``.env``),
    ...
    when omitted, ``threev0_cli.config.get_env_value`` is used directly.
    """
...
def fal_key_is_configured() -> bool:
    ...
    ``threev0_cli.config.get_env_value`` when available) so tool-side
