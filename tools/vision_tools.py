...
# ``agent.auxiliary_client`` pulls credential_pool → threev0_cli.auth → httpx
...
def _resolve_download_timeout() -> float:
    env_val = (branded_env("VISION_DOWNLOAD_TIMEOUT") or "").strip()
    if env_val:
        try:
...
def _resolve_vision_cpu_workers() -> int:
    ...
    Resolution order: EV0_VISION_MAX_CONCURRENCY env →
    ...
    """
    env_val = (branded_env("VISION_MAX_CONCURRENCY") or "").strip()
    if env_val:
        try:
            parsed = int(env_val)
            if parsed >= 1:
...
def _normalize_to_supported_image(
    ...
    out_dir = get_threev0_dir("cache/vision", "temp_vision_images")
...
async def _vision_analyze_native(
    ...
    try:
        ...
        temp_dir = get_threev0_dir("cache/vision", "temp_vision_images")
...
async def vision_analyze_tool(
    ...
    Note:
        - For URLs, temporary images are stored under $EV0_HOME/cache/vision/ and cleaned up
...
async def _materialize_video_from_terminal_backend(video_source: str, task_id: Optional[str]) -> Path:
    ...
    temp_dir = get_threev0_dir("cache/video", "temp_video_files")
    temp_dir.mkdir(parents=True, exist_ok=True)
