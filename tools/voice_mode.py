...
def _voice_capture_install_hint() -> str:
    ...
    # If we're running inside a venv (e.g. the bundled 3V0 venv at
    # ~/.3V0/profiles/<name>/3v0-agent/venv/), `pip install` on the
...
_TEMP_DIR = os.path.join(tempfile.gettempdir(), "ev0_voice")
...
def _get_beep_volume() -> float:
    ...
    system can't be imported (e.g. broken ~/.3V0/config.yaml during a
...
class AudioRecorder:
    ...
    def _max_duration_reached(self, elapsed: float) -> bool:
        ...
        recording (see ``Ev0CLI._voice_start_recording``). A value <= 0
...
# playback-phase barge transcript is treated as a self-capture of 3V0'
...
def is_tts_echo(
    ...
    3V0 just spoke using a character-level similarity ratio, which works
    across languages without word-tokenization. A genuine user interjection
    is very unlikely to closely match 3V0' own words, so a high ratio is
...
def _play_audio_file_impl(file_path: str) -> bool:
    ...
    # A unique suffix prevents concurrent 3V0 TTS calls from colliding on
        ...
        if _is_wsl2_env():
            try:
                ...
                if _win_tmp_wsl:
                    ...
                    _wsl_wav = os.path.join(_win_tmp_wsl, f"3v0-tts-{_unique}.wav")
...
def _vad_log(msg: str) -> None:
    ...
    EV0_VOICE_DEBUG=1 so live hardware tuning doesn't need a log tail."""
    logger.debug(msg)
