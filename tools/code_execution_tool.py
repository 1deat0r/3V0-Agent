...
Lets the LLM write a Python script that calls 3V0 tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `ev0_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `ev0_tools.py` with file-based RPC stubs
...
# available on every platform 3V0 itself runs on.
...
# match a safe prefix, the operational EV0_ allowlist, or (on Windows) an
# OS-essential name.  Delegate-task child context is also an exact-name
# operational marker: without it, a sandbox script that spawns/imports 3V0
# code can lose the DB-layer Kanban mutation guard while still inheriting
# EV0_HOME.
#
# NB: the broad "EV0_" prefix was deliberately removed (#27303) — it leaked
# EV0_*-named config that lacks a secret substring (e.g. EV0_BASE_URL,
# EV0_KANBAN_DB, EV0_*_WEBHOOK).  The child only needs the few
# location/profile vars in _EV0_CHILD_ALLOWED below; EV0_RPC_SOCKET /
# EV0_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational EV0_* vars (_EV0_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret EV0_* vars dropped by the tightened allowlist (#27303). The
    # broad "EV0_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like EV0_KANBAN_DB / EV0_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _EV0_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("EV0_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped EV0_* var.
            _dropped_ev0.append(k)
    if _dropped_ev0:
        logger.debug(
            "execute_code: dropped %d non-allowlisted EV0_* var(s) from the "
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-3V0-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-3V0-profile soft guard."""',
...
def _sandbox_failure_hint(stderr_text: str, enabled_tools=None) -> Optional[str]:
    ...
    ev0_tools import misuse (importing tools that aren't in the sandbox,
    ...
    try:
        m = re.search(
            r"cannot import name '(\w+)' from 'ev0_tools'", window
        )
        if m:
            missing = m.group(1)
...
"""Auto-generated 3V0 tools RPC stubs."""
...
def _connect():
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["EV0_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("EV0_RPC_TOKEN", ""),
    }) + "\\n"
    with _call_lock:
        conn = _connect()
...
"""Auto-generated 3V0 tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("EV0_RPC_DIR") or os.path.join(tempfile.gettempdir(), "ev0_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated ev0_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/ev0_exec_{sandbox_id}"
    ...
    try:
        ...
        env_prefix = (
            f"EV0_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
...
def execute_code(
    ...
    # --- Set up temp directory with ev0_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="ev0_sandbox_")
    ...
    # selector from EV0_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"ev0_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated ev0_tools module.
        ...
        #   connect.  EV0_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["EV0_RPC_SOCKET"] = rpc_endpoint
        child_env["EV0_RPC_TOKEN"] = rpc_token
        ...
        # ``ev0_tools.py`` always lives in the staging directory, so that
        # directory must be importable even when project mode changes CWD.
        # 3V0's own package root is useful too, but only when the child
        ...
        if _uses_threev0_python_environment(_child_python):
            ...
            logger.info(
                "execute_code: child interpreter %s is outside the 3V0 "
                "environment; 3v0 root omitted from PYTHONPATH",
                _child_python,
            )
        if _existing_pp:
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.3V0/.env')).
...
def _load_config() -> dict:
    ...
    ``3v0 --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (3v0-agent's python). Reproducible and the
...
# Interpreter paths already reported as outside the 3V0 environment —
...
def _python_environment_prefix(python_path: str) -> str:
    ...
    sticky empty result would silently drop the 3v0 root from every
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``3v0 tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and 3v0-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.3V0/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that calls 3V0 tools programmatically. "
        ...
        f"Available via `from ev0_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from ev0_tools import {import_str}` "
