...
# 3V0 session id (observability identity, distinct from the gateway
...
# os.environ["EV0_INTERACTIVE"] races: one session's restore in `finally`
...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    try:
        ...
        # Forward the 3V0 session id so observer plugins parent approval
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("EV0_SESSION_KEY", default)
...
def _is_cron_approval_context() -> bool:
    ...
    try:
        ...
        return is_truthy_value(get_session_env("EV0_CRON_SESSION", ""))
    except Exception:
        return env_var_enabled("EV0_CRON_SESSION")


def _is_single_query_approval_context() -> bool:
    """True when the current approval decision is from a single-query (-q) session.

    ``3v0 chat -q "..."`` runs one turn and exits with no user waiting to
    answer approval prompts, but it still exports ``EV0_INTERACTIVE=1`` so
    ...
    try:
        ...
        return is_truthy_value(get_session_env("EV0_SINGLE_QUERY_SESSION", ""))
    except Exception:
        return env_var_enabled("EV0_SINGLE_QUERY_SESSION")


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set EV0_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind EV0_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds EV0_SESSION_PLATFORM via
...
def _should_fall_through_to_cli_approval(
    *,
    ...
    ``EV0_EXEC_ASK`` (and sometimes a session platform marker) can leak into
...
# via shell expansions like $HOME or $EV0_HOME, or by the resolved absolute
# active profile home path such as /home/3v0/.3V0/config.yaml. The
# resolved-absolute form is folded into the ~/.3V0/ patterns at detection
...
# go stale when EV0_HOME is set after this module is imported, e.g. under the
...
# ~/.3V0/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _EV0_ENV_PATH; matches the EV0_HOME override form as
# well as ~/.3V0/.
_EV0_CONFIG_PATH = (
    r'(?:~\/\.3v0/|'
    r'(?:\$home|\$\{home\})/\.3v0/|'
...
def _save_blocked_payload(command: str) -> Optional[str]:
    ...
    try:
        ...
        path.write_text(
            "#!/bin/bash\n"
            "# Auto-saved by 3V0: this command exceeded the inline command\n"
...
DANGEROUS_PATTERNS = [
    ...
    # These are native Windows EXEs / cmdlets reachable from ANY 3V0
    ...
    (r'\bappdata[\\/](?:local|roaming)[\\/]3v0[^\n]*\.env\b', "access to 3V0 secrets (Windows path)"),
    # ─────────────────────────────────────────────────────────────────────
    ...
    # that should always require user consent, just like `3v0 gateway restart`
    ...
    # a lifecycle command past the guard — same treatment as the `3v0 ...
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart 3v0-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart 3v0-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(3v0|gateway|cli\.py)\b', "kill 3v0/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill 3v0` but not
    # `kill -9 $(pgrep -f 3v0)` because the substitution is opaque
    ...
    # the `3v0 gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.3v0.gateway`).
    # Catch the operations that stop, restart, or unload it.
    (r'\blaunchctl\s+(stop|kickstart|bootout|unload|kill|disable|remove)\b.*\b(3v0|ai\.3v0)\b', "stop/restart 3v0 launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/3V0 file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.3V0/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a 3V0-managed security file (~/.3V0/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_EV0_CONFIG_PATH}|{_EV0_ENV_PATH})', "in-place edit of 3V0 config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_EV0_CONFIG_PATH}|{_EV0_ENV_PATH})', "in-place edit of 3V0 config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_EV0_CONFIG_PATH}|{_EV0_ENV_PATH})', "in-place edit of 3V0 config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.3V0/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / EV0_HOME even when those are set after this module is
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / EV0_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.3V0``); the matched
...
def _rewrite_resolved_threev0_home(command: str) -> str:
    ...
    ``~/.3V0/`` so the static ``_EV0_CONFIG_PATH`` / ``_EV0_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/3v0/.3V0/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$EV0_HOME``. Matches both POSIX and Windows separators. No-op when the
    ...
    try:
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.3V0")
...
def _mask_quoted_newlines(command: str) -> str:
    ...
    start. That made any multi-line quoted argument (``3v0 send`` message
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one 3V0 ad-hoc temp script."""
    try:
    ...
    if os.path.dirname(target) != temp_dir:
        return False
    return re.fullmatch(r"3v0-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode is derived from 3V0 YOLO.
...
def is_approval_bypass_active_for_session(session_key: str) -> bool:
    """Return whether one exact session bypasses 3V0 approval prompts.

    Collapses the canonical three-source bypass check used across the codebase
    into one place:
      - process-scoped ``--yolo`` / ``EV0_YOLO_MODE`` (frozen at import time
    ...
    *,
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            EV0_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    # Single-query (-q) sessions export EV0_INTERACTIVE=1 but have no user
    ...
    if not is_cli and not is_gateway:
        ...
        if _is_cron_approval_context():
            ...
            logger.warning(
                ...
                "BLOCKED (fail-closed). Set EV0_INTERACTIVE or "
                "EV0_GATEWAY_SESSION to answer the prompt.",
                autoapprove_log_prefix, pattern_key, description,
            )
            return {
                "approved": False,
        ...
        logger.warning(
            "%s (pattern: %s): %s — set EV0_INTERACTIVE or "
            "EV0_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("EV0_EXEC_ASK"):
        ...
        # can see (EV0_EXEC_ASK / platform-marker leaks into CLI).
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``EV0_INTERACTIVE``) fails CLOSED — a plugin-
...
def get_plugin_manager():
    ...
    from threev0_cli.plugins import discover_plugins, get_plugin_manager as _get_manager
...
def _present_with_selected_transport(
    *,
    ...
    try:
        from agent.redact import redact_sensitive_text
        from threev0_cli.approval_transport import ApprovalRequest, invoke_approval_transport
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = _is_interactive_cli()
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("EV0_EXEC_ASK")
    ...
    if _is_single_query_approval_context():
        is_cli = False
        is_gateway = False
        # EV0_EXEC_ASK routes through the gateway decision loop (no human
        ...
        if _is_single_query_approval_context():
            if _get_single_query_approval_mode() == "deny":
                ...
                try:
                    ...
                    try:
                        from threev0_cli.config import load_config_readonly as _load_cfg
...
def check_execute_code_guard(code: str, env_type: str,
    ...
    # Ask-mode (EV0_EXEC_ASK) still takes this path even when INTERACTIVE
