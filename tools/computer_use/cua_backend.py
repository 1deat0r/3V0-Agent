...
etc.) surface as specific blocked checks via `3v0 computer-use doctor`
...
def _action_result_from(
    ...
    *,
    ...
    cua-driver's mcp-tool-notes and NousResearch/3v0-agent#67052.
    """
...
# fetches the latest release, so a `EV0_CUA_DRIVER_VERSION` env var would
# only have *looked* like it pinned. For a reproducible version, point
# `EV0_CUA_DRIVER_CMD` at a specific binary instead.

_CUA_DRIVER_CMD_ENV = "EV0_CUA_DRIVER_CMD"
...
def _cua_no_overlay() -> bool:
    """True when 3V0 should pass ``--no-overlay`` to cua-driver.
...
def _cua_telemetry_disabled() -> bool:
    """True when 3V0 should disable cua-driver telemetry for this user.
...
def _cua_configured_permission_mode() -> str:
    ...
    per-session 3V0 YOLO toggle so a stale config line can never
...
def _cua_grant_existing_profile() -> bool:
    ...
    cua-driver's trusted-launcher grant. 3V0 passes
...
def _standard_runtime_launch_args(
    ...
    *,
    ...
    private_socket = socket_path or os.path.join(
        tempfile.gettempdir(), f"3v0-cua-standard-{uuid.uuid4().hex[:12]}.sock"
    )
...
def _empty_discovery_reason() -> str:
    ...
    if sys.platform == "darwin":
        ...
        return (
            ...
            "`3v0 computer-use doctor`"
        )
    return (
        "window discovery returned no windows; run `3v0 computer-use "
...
def _wsl_windows_path_to_posix(path: str) -> str:
    """Translate a Windows absolute manifest command when 3V0 runs in WSL.

    Windows cua-driver manifests can report ``C:\\Users\\...\\cua-driver.exe``
    even though the 3V0 process uses POSIX subprocess spawning inside WSL.
    ...
    if not re.match(r"^[A-Za-z]:[\\/]", path):
        return path
    try:
        from ev0_constants import is_wsl
...
class _EmbeddedCuaDaemon:
    ...
    machine-wide daemon would therefore let one 3V0 session's YOLO choice
    ...
    * ``unrestricted`` — explicit 3V0 YOLO; launch-time risk
    ...
    * ``bounded`` — a user-reviewed capability manifest
    ...
    def __init__(
        ...
        if sys.platform == "win32":
            self.socket_path = rf"\\.\pipe\3v0-cua-{token}"
        else:
...
def _resolve_mcp_invocation(
    ...
    *,
    ...
    Surface 8 of NousResearch/3v0-agent#47072: instead of hardcoding
    ...
    # A Windows-installed cua-driver can hand a WSL-hosted 3V0 an absolute
...
def _candidate_cua_driver_commands(override: Optional[str] = None) -> List[str]:
    ...
    ``EV0_CUA_DRIVER_CMD`` is authoritative; only when neither is set do we
    ...
    3V0 Desktop/TUI session can otherwise filter out the `computer_use`
    tool even though `3v0 computer-use doctor` succeeds from a login shell.
    """
    ...
    if sys.platform == "win32":
        ...
        candidates.extend([
            ...
            # misses it until every 3V0 process is restarted.
            os.path.join(
...
def resolve_cua_driver_cmd(override: Optional[str] = None) -> Optional[str]:
    ...
    A supplied override (or ``EV0_CUA_DRIVER_CMD``) is never silently
...
def cua_driver_runtime_contract_status(binary: Optional[str] = None) -> Dict[str, Any]:
    """Report whether a local driver can host 3V0' 0.20 integration."""
...
def cua_driver_update_nudge() -> Optional[str]:
    ...
    return (
        ...
        f"update with `3v0 computer-use install --upgrade`."
    )
...
def _maybe_repair_runtime_contract(contract: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ``EV0_CUA_DRIVER_CMD`` override is authoritative even when broken, and
    ...
    try:
        from ev0_cli.tools_config import install_cua_driver
...
def cua_driver_install_hint() -> str:
    ...
    return (
        ...
        "  3v0 computer-use install\n"
        "Or run the upstream installer directly:\n"
        f"{installer}\n"
        "Or run `3v0 tools` and enable the Computer Use toolset to install it automatically."
    )
...
def _parse_elements_from_structured(raw_elements: List[Dict[str, Any]]) -> List[UIElement]:
    """Surface 2 of NousResearch/3v0-agent#47072: read the canonical
...
class _CuaDriverSession:
    ...
    def __init__(
        ...
        self._started = False
        # Surface 4 of NousResearch/3v0-agent#47072: per-tool
    ...
    async def _lifecycle_coro(self) -> None:
        ...
        try:
            ...
            params = StdioServerParameters(
                ...
                # sanitize 3V0-managed secrets out of the child env.
    ...
    def _call_tool_via_cli(self, name: str, args: Dict[str, Any], timeout: float) -> Dict[str, Any]:
        ...
        try:
            for attempt in range(attempts):
                ...
                # 3V0 talks to the direct `cua-driver mcp` runtime
    ...
    def _unknown_transport_outcome(name: str, exc: Exception) -> Dict[str, Any]:
        message = (
            ...
            "unknown, so 3V0 did not replay it. Take fresh state before "
...
def _extract_tool_result(mcp_result: Any) -> Dict[str, Any]:
    ...
    NousResearch/3v0-agent#47072). Each entry corresponds index-for-index
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self, permission_mode: str = "standard") -> None:
        ...
        # Surface 6 of NousResearch/3v0-agent#47072: per-snapshot
        ...
        #   - Gets a distinct agent-cursor color per 3V0 run, with
    ...
    def start(self) -> None:
        ...
        if not contract.get("ready"):
            # An installed-but-incompatible driver (e.g. predating a 3V0
        ...
        if not contract.get("ready"):
            ...
            if os.environ.get(_CUA_DRIVER_CMD_ENV, "").strip():
                repair = (
                    "Update the binary selected by EV0_CUA_DRIVER_CMD or "
                    "remove that override."
                )
            else:
                repair = "Run `3v0 computer-use install` to repair it."
            raise RuntimeError(f"cua-driver is not ready: {reason}. {repair}")
        _maybe_nudge_update()
        # The MCP client SDK (`mcp`) is an optional dependency (the
        # `computer-use` / `mcp` extras), not part of 3V0' minimal core.
    ...
    def capture(
        ...
        Maps 3v0 `capture(mode, app)` → cua-driver `list_windows` +
        `get_window_state` (ax/som) or `screenshot` (vision).
        """
        # Step 1: enumerate on-screen windows to find target pid/window_id.
        # Surface 3 of NousResearch/3v0-agent#47072: read the canonical
        ...
        if mode == "vision":
            ...
            # Surface 2 of NousResearch/3v0-agent#47072: prefer the
    ...
    def _apply_delivery(
        ...
        to proceed. See NousResearch/3v0-agent#67052 phase B.
        """
    ...
    def click(
        ...
        *,
        ...
        # NousResearch/3v0-agent#47072). cua-driver-rs gained an explicit
    ...
    def focus_app(self, app: str, raise_window: bool = False) -> ActionResult:
        ...
        separately approved by the 3V0 adapter, and uses cua-driver's
