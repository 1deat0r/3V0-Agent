...
`health_report` (surfaced by `3v0 computer-use doctor`) reports the
...
# regardless of approval level (e.g. logout kills the session 3V0 runs in).
_BLOCKED_KEY_COMBOS = {
...
# Per-3V0-session cached backends. Each backend owns its own cua-driver
...
# See NousResearch/3v0-agent#67052 gap 4.
...
def _cua_permission_mode(session_id: str) -> str:
    """Map 3V0's explicit approval bypass onto Cua's immutable mode.

    3V0 has TWO session-identity namespaces: the tool-dispatch path passes
        ...
        with _backend_lock:
            ...
            if cached is not None:
                ...
                if backend_name in {"cua", "cua-driver", ""}:
                    ...
                    raise RuntimeError(
                        f"Unknown EV0_COMPUTER_USE_BACKEND={backend_name!r}"
                    )
...
def release_computer_use_session(session_id: str) -> bool:
    ...
    3V0 YOLO selected unrestricted mode. Approval state is cleared even
    ...
    try:
        ...
        # while waiting: unrelated 3V0 sessions remain independent.
...
def _shutdown_backend_atexit() -> None:
    ...
    without this a driver can survive the 3V0 process that spawned it
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    ...
    try:
        ...
        return json.dumps({
            ...
            "hint": "If the cua-driver binary is missing, run `3v0 computer-use install`. "
...
def _request_approval(action: str, args: Dict[str, Any],
    ...
    silently authorize it (NousResearch/3v0-agent#67052).
...
def _dispatch(backend: ComputerUseBackend, action: str, args: Dict[str, Any]) -> Any:
    ...
    # none of those capabilities can be supplied across 3V0 sessions.
...
# "foreground" (it doesn't know 3V0 has a typed page route), so the model
...
def _capture_response(cap: CaptureResult, max_elements: int = _DEFAULT_MAX_ELEMENTS) -> Any:
    ...
    if cap.png_b64 and cap.mode != "ax" and not image_too_small:
        ...
        # parts (Surface 7 of NousResearch/3v0-agent#47072 — trycua/cua#1961
...
def _route_capture_through_aux_vision(
    ...
    *,
    ...
    The captured base64 PNG is materialised to ``$EV0_HOME/cache/vision/``
...
def check_computer_use_requirements() -> bool:
    ...
    `3v0 computer-use doctor` if their session is incomplete (e.g. no
    DISPLAY set).
    """
