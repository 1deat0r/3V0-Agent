...
def _truncate_to_char_budget(content: str, max_chars: int) -> tuple[str, int, bool]:
    ...
    ``read_file``). Where 3v0 previously hard-rejected an oversized read
    ...
    try:
        ...
        if env is not None:
            ...
            if "daytona" in name:
                return "daytona"
        cfg = _get_env_config()
        return str(cfg.get("env_type") or wire_env("TERMINAL_ENV") or "local").lower()
    except Exception:
        return str(wire_env("TERMINAL_ENV") or "local").lower()
...
def _configured_terminal_cwd() -> str | None:
    ...
    """
    return _sentinel_free_abs_cwd(wire_env("TERMINAL_CWD"))
...
def _file_ops_uses_host_paths(file_ops) -> bool:
    """Return True when *file_ops* targets the same host filesystem as 3V0.
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Prevent agents from modifying the 3V0 config file directly.
    ...
    if threev0_config and (resolved == threev0_config or normalized == threev0_config):
        return (
            ...
            "Edit ~/.3V0/config.yaml directly or use '3v0 config' instead."
        )
    return None
...
# .cursorrules (or a project-local .3V0 config tree) outlives the current
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    # The authoritative ~/.3V0 home is governed by its own guards
    ...
    for candidate in (normalized, resolved):
        ...
        for pattern in extra_patterns:
            if fnmatch.fnmatch(base_lower, pattern.lower()):
                return base
        # Project-local .3V0 config dirs (e.g. <repo>/.3V0/config.yaml)
        # are loaded as project context and steer behavior the same way.
        # Scope: the file's IMMEDIATE parent must be ``.3V0`` — matching
        # any ancestor named .3V0 would gate every write inside a
        # checkout that happens to live under ~/.3V0 (e.g. the
        # 3v0-agent repo itself at ~/.3V0/3v0-agent).
        parts = candidate.replace("\\", "/").rstrip("/").split("/")
        if len(parts) >= 2 and parts[-2] == ".3V0":
            return candidate
    return None
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side 3V0 mirror prefix for Docker file tools."""
    try:
        from tools.terminal_tool import (
            _active_environments,
            _env_lock,
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    """Return a soft-guard warning when ``filepath`` lands in another 3V0
    profile's scoped area, a host-side sandbox-mirror of authoritative profile
    state, or the Docker container's sandbox mirror of 3V0 state.
    ...
    * sandbox-mirror (#32049) — writes that hit the
      ``…/sandboxes/<backend>/<task>/home/.3V0/…`` mirror created by a
      non-local terminal backend (Docker, Daytona, etc.), where the host
      3V0 process never reads the mirror and the authoritative file is
      left untouched.
    * container-mirror (#32049 follow-up) — writes from inside a Docker
      ...
      the agent sees a plain ``/root/.3V0/…`` path.

    Returns ``None`` when the write is in-scope or outside 3V0 scope.
    ...
    # in a session that cd'd into ``~/.3V0/profiles/other/`` is
...
def read_file_tool(path: str, offset: int = 1, limit: int = 2000, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── 3V0 internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files,
        # and block credential stores under EV0_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == EV0_HOME (e.g. "auth.json") still hits the
...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    ``cross_profile`` opts out of the soft cross-3V0-profile guard. The
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    ``cross_profile`` opts out of the soft cross-3V0-profile guard for
...
WRITE_FILE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another 3V0 profile's skills/plugins/cron/memories — by default these writes are blocked with a warning because they affect a different profile than the one this session is running under.",
                "default": False,
            },
        },
...
PATCH_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another 3V0 profile's skills/plugins/cron/memories.",
                "default": False,
            },
        },
        "required": ["mode"],
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with ev0_tools.write_file() for very "
            "large files."
        )
