...
def _get_active_profile_name() -> str:
    """Return the active 3V0 profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale 3v0-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=3v0-agent=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling 3V0 process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=3v0-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a 3v0 process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed 3V0
    process leaves its container behind permanently because there's no
    subsequent 3V0 process scheduled to reuse that exact (task, profile)
    pair.
    """
    docker = docker_exe or find_docker() or "docker"
    filters = ["--filter", "label=3v0-agent=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=3v0-profile={_sanitize_label_value(profile_filter)}"])
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``EV0_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
...
#   SETUID/SETGID - the image's init drops from root to the '3v0'
...
def _egress_proxy_args_for_docker() -> tuple[list[str], dict[str, str], list[str]]:
    ...
    * ``env_overrides`` — env vars to set on container creation: ``HTTPS_PROXY``,
      ...
      paths, and one ``EV0_PROXY_TOKEN_<NAME>`` per minted mapping
    * ``host_args`` — extra ``--add-host`` flags so the container can reach the
    ...
    if not status.configured:
        msg = (
            ...
            "Run `3v0 egress setup` to mint tokens and write proxy.yaml."
        )
        if enforce:
    ...
    if not (status.pid and status.listening):
        msg = (
            ...
            "Start it with `3v0 egress start`."
        )
        if enforce:
            raise RuntimeError(msg)
        logger.warning("%s — continuing without proxy (enforce_on_docker=false).", msg)
    ...
    if status.ca_cert_path is None or not status.ca_cert_path.exists():
        ...
        msg = (
            ...
            "Re-run `3v0 egress setup` to regenerate it."
        )
        if enforce:
            raise RuntimeError(msg)
        logger.warning("%s — continuing without proxy (enforce_on_docker=false).", msg)
    ...
    if not mappings:
        msg = (
            ...
            "corrupt.  Re-run `3v0 egress setup` to mint provider "
    ...
    container_ca = "/etc/ssl/certs/3v0-egress-ca.crt"
    ...
    env_overrides: dict[str, str] = {
        ...
        "_EV0_EGRESS_NODE_OPTIONS_APPEND": "--use-openssl-ca",
    }
    ...
    # through the proxy.  Keep the EV0_PROXY_TOKEN_* aliases for
    # diagnostics.
    for m in mappings:
        env_overrides[m.real_env_name] = m.proxy_token
        env_overrides[f"EV0_PROXY_TOKEN_{m.real_env_name}"] = m.proxy_token
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``3v0-agent:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.3V0/sandboxes/). Non-persistent
        ...
        if egress_env_overrides:
            try:
                from threev0_cli.config import load_config as _load_cfg_for_collision
        ...
        try:
            from threev0_cli.config import load_config as _load_cfg_for_precedence
        ...
        # ``_EV0_EGRESS_NODE_OPTIONS_APPEND`` and merge here.
        _egress_node_append = merged_env.pop(
            "_EV0_EGRESS_NODE_OPTIONS_APPEND", None,
        )
        ...
        # s6-overlay images (e.g. 3v0-agent:latest) already use /init as PID 1
        ...
        container_name = f"3v0-{uuid.uuid4().hex[:8]}"
        # Labels make 3v0-created containers identifiable to:
        #   * the orphan reaper (`3v0-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`3v0-task-id`, `3v0-profile`)
        #   * operators running `docker ps --filter label=3v0-agent=1`
        # Values are limited to the safe character set defined by
        # _sanitize_label_value(); the active 3V0 profile is captured at
        ...
        label_args = [
            "--label", "3v0-agent=1",
            "--label", f"3v0-task-id={task_label}",
            "--label", f"3v0-profile={profile_name}",
        ...
        self._labels = {
            "3v0-agent": "1",
            "3v0-task-id": task_label,
            "3v0-profile": profile_name,
            _EGRESS_LABEL_KEY: egress_label,
        }

        # Cross-process container reuse (issue #20561 — docs claim "ONE long-lived
        # container shared across sessions").  If a prior 3V0 process
    ...
    def _resolve_passthrough_env(self) -> tuple[dict[str, str], set[str]]:
        ...
        # win over the generic 3V0 secret blocklist. Only implicit passthrough
        # keys are filtered. Also strip 3V0-internal dynamic secrets
    ...
    def _recreate_container(self) -> bool:
        ...
        task_label = self._labels.get("3v0-task-id", "")
        profile_label = self._labels.get("3v0-profile", "")
        existing = self._find_reusable_container(
        ...
        if not self._container_id:
            ...
            try:
                ...
                new_name = f"3v0-{_uuid.uuid4().hex[:8]}"
    ...
    def _find_reusable_container(
        ...
        matches containers that happened to be named ``3v0-*`` but were
        started by some other tool.
        """
        try:
            filters = [
                "--filter", "label=3v0-agent=1",
                "--filter", f"label=3v0-task-id={task_label}",
                "--filter", f"label=3v0-profile={profile_label}",
            ]
            if egress_label != "off":
                ...
                # 3v0-egress label is present and not "off".  Without
                # this, a container created with egress=on can be silently
                # reused after the operator runs "3v0 egress disable",
        ...
        # container) but can happen if a previous 3V0 process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every 3V0 exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        3V0 process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next 3V0 startup. That covers the
        ...
        # outlives 3V0 processes, processes inside it stay alive, and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"3v0-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``3v0 /quit``
