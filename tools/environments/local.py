...
def _resolve_local_initial_cwd(cwd: str) -> str:
    ...
    directory 3V0 was already launched from (for example ``3v0-agent``
    while the process cwd is ``~/.3V0/3v0-agent``), passing it through
    unchanged makes the wrapper run ``cd 3v0-agent`` *inside* the project
    ...
    # Common recovery for config values like ``3v0-agent`` when 3V0 was
    # launched from that directory already.  ``os.path.abspath`` would point at
    # a nonexistent nested ``./3v0-agent``; use the current directory instead.
...
# 3V0-internal env vars that should NOT leak into terminal subprocesses.
_EV0_PROVIDER_ENV_FORCE_PREFIX = "_EV0_FORCE_"

# 3V0-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a 3V0 inference secret exactly
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    # 3V0-managed inference credential — Claude subscription auth is not a
    # working 3V0 provider path.  Stripping it broke agent-spawned
...
_EV0_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()
...
# project's dependencies into the 3V0 venv path instead of the project's own
# ``.venv`` — silently clobbering the 3V0 environment (e.g. a project pinned
# to a different Python version overwrites it and breaks the gateway). The
# 3V0 venv stays reachable via PATH (its bin dir is first), so stripping
...
def _is_threev0_internal_secret(key: str) -> bool:
    ...
    This is the single source of truth for "3V0-internal dynamic secret"
    across every spawn path — the terminal ``_make_run_env`` /
    ``_sanitize_subprocess_env`` filters, the Docker passthrough filter, and the
    non-terminal :func:`ev0_subprocess_env` helper all call it, so the
...
def _inject_session_context_env(env: dict) -> None:
    ...
    (EV0_SESSION_*) are bridged onto the child env here.
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter 3V0-managed secrets from a subprocess environment."""
    try:
...
# legitimate child 3V0 spawns needs them, and they are the highest-value
...
# narrow subset of _EV0_PROVIDER_ENV_BLOCKLIST; provider keys are handled by
# the conditional Tier-2 strip in ev0_subprocess_env().
_ALWAYS_STRIP_KEYS: frozenset[str] = frozenset({
    ...
    # _is_ev0_internal_secret, but _ID has no secret suffix, so it must be
...
def threev0_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    single source of truth (``_EV0_PROVIDER_ENV_BLOCKLIST``).  The terminal
    ...
    * **Tier 1 (always):** ``_ALWAYS_STRIP_KEYS`` — gateway bot tokens, GitHub
      ...
      ``inherit_credentials``.  No child 3V0 spawns legitimately needs them.
    * **Tier 2 (conditional):** the rest of ``_EV0_PROVIDER_ENV_BLOCKLIST``
    ...
    for key in _ALWAYS_STRIP_KEYS:
        env.pop(key, None)
    # Internal routing hints and 3V0-internal dynamic secrets
    ...
    # legitimate use for them. See :func:`_is_ev0_internal_secret`.
    for key in list(env):
        if key.startswith(_EV0_PROVIDER_ENV_FORCE_PREFIX):
            env.pop(key, None)
        elif _is_threev0_internal_secret(key):
    ...
    if not inherit_credentials:
        ...
        for key in _EV0_PROVIDER_ENV_BLOCKLIST:
            env.pop(key, None)
    ...
    # copies os.environ, whose EV0_SESSION_* mirror is a last-writer-wins
    ...
    # still see the parent's EV0_HOME but lose the DB mutation guard.
...
def build_subprocess_env(
    ...
    *,
    ...
    function (or :func:`ev0_subprocess_env` for the model-driving-CLI
    surface) instead of copying ``os.environ`` directly, so profile-home
    propagation (``EV0_HOME`` / subprocess ``HOME`` contract) and the
    3V0 secret-scrub policy have a single owner.  History: ~11 separate
    ...
    * ``scrub_secrets=True`` (default) — delegate to
      ...
      list (provider blocklist + ``_is_ev0_internal_secret`` dynamic
      patterns + kanban/venv-marker/session-context guards) **and** of
      ``EV0_HOME`` / subprocess-HOME propagation.  On this path profile
    ...
    * ``extra`` — applied **last** on the non-scrub path so explicit caller
      overrides (e.g. a session-scoped ``EV0_HOME``) always win.  On the
      scrub path it is forwarded as ``_sanitize_subprocess_env``'s
      ``extra_env`` (same force-prefix / blocklist handling as today).
    """
    if scrub_secrets:
        # _sanitize_subprocess_env already performs EV0_HOME override
...
def _find_bash() -> str:
    ...
    # system Git (or a stale EV0_GIT_BASH_PATH pointing at one) must not
    ...
    # EV0_GIT_BASH_PATH pointing at a broken Git-for-Windows install
    # (``Directory \\drivers\\etc does not exist``) must not win over a
    # healthy portable Git under %LOCALAPPDATA%\\3v0\\git.
    for candidate in candidates:
        if _bash_starts(candidate):
            if candidate != custom and custom and os.path.isfile(custom):
                logger.warning(
                    "EV0_GIT_BASH_PATH=%s fails to start; using %s instead",
                    custom,
                    candidate,
                )
            return candidate
    ...
    raise RuntimeError(
        "Git Bash not found. 3V0 Agent requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set EV0_GIT_BASH_PATH to your bash.exe location."
    )
...
def _git_bash_aslr_help(bash: str, details: str = "") -> str:
    ...
    return (
        ...
        "Then restart 3V0. If the override is blocked or later re-applied, "
...
# Cached directory containing the ``3v0`` console-script.
...
def _resolve_threev0_bin_dir() -> str | None:
    ...
    3v0 install dir (``~/.local/bin``, the venv ``bin``/``Scripts``, pipx,
    nix) is absent from that PATH, so plugins shelling out to bare ``3v0``
    via the terminal tool hit ``command not found`` (exit 127) even though
    ``3v0`` works fine in the user's own interactive terminal.

    We resolve the install dir once (it never changes within a process) and
    prepend-if-missing it to the subshell PATH so bare ``3v0`` resolves
    regardless of how the gateway was started.

    Resolution order (cheap, no heavy imports):
      1. ``shutil.which("3v0")`` — normal PATH-installed shim.
      2. The directory of ``sys.argv[0]`` when it's an absolute path to a
         real ``3v0`` executable (covers nix-store / venv wrappers).
      3. The directory of ``sys.executable`` — the running interpreter's
         venv ``bin``/``Scripts`` is where its console-scripts live.
    """
    global _EV0_BIN_DIR
    if _EV0_BIN_DIR is not _SENTINEL:
        return _EV0_BIN_DIR  # type: ignore[return-value]

    candidate: str | None = None

    which = shutil.which("3v0")
    if which:
    ...
    if candidate is None:
        ...
        if (
            os.path.isabs(argv0)
            and (base == "3v0" or base.startswith("3v0."))
    ...
    if candidate is None:
        ...
        if exe_dir:
            shim = "3v0.exe" if _IS_WINDOWS else "3v0"
...
def _managed_runtime_path_entries() -> list[str]:
    """Return existing 3V0-managed runtime dirs for the terminal subshell PATH.

    The terminal tool spawns a subshell whose PATH is the agent process's PATH
    plus ``_SANE_PATH``. Neither carries the runtimes 3V0 installs for
    itself, so on a machine where 3V0 provisioned its own toolchain a
    command the agent runs resolves a system copy instead — or nothing at all:

    - ``$EV0_HOME/node`` (+ ``/bin``) — installed to satisfy the desktop and
      browser toolchain. ``tools/browser_tool.py`` already does this for its own
      subprocesses; the agent's shell deserves the same.
    - ``$EV0_HOME/bin`` — the managed ``uv``. ``install.sh`` writes it there
    ...
    ``get_ev0_home()`` is profile-scoped and a managed tree can appear
...
def _append_missing_sane_path_entries(existing_path: str) -> str:
    ...
    3V0-managed runtime dirs are appended alongside the sane entries, not
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    Windows commands such as ``tasklist``, ``schtasks``, and ``wmic``.  3V0
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_EV0_PROVIDER_ENV_FORCE_PREFIX):
            ...
            if k in _EV0_PROVIDER_ENV_BLOCKLIST and not passthrough:
                continue
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, 3V0 trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``EV0_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under EV0_HOME.  Using
