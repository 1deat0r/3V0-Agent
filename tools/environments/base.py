"""Base class for all 3V0 execution environment backends.
...
def _pipe_stdin(proc: subprocess.Popen, data: str) -> None:
    ...
    recorded on ``proc._ev0_stdin_errors`` while stdin is still closed
...
def _cwd_marker(session_id: str) -> str:
    return f"__EV0_CWD_{session_id}__"
...
# the FIRST session's EV0_SESSION_ID into the snapshot makes every LATER
...
# EV0_SESSION_ID leak via the shared snapshot). Stripping them from the
...
# set), not 3V0' per-turn session identity.
#
# Kept in sync with gateway.session_context._VAR_MAP: every bridged name starts
# with one of these prefixes (or is EV0_UI_SESSION_ID). Used by unit tests
# as the Python-side contract for the exclusion set; the dump path unsets by
# name/prefix instead of grepping declare lines (see below / issue #71296).
_SNAPSHOT_EXCLUDED_ENV_REGEX = (
    "^declare -x (EV0_SESSION_|EV0_UI_SESSION_ID|EV0_CRON_AUTO_DELIVER_|EV0_CRON_SESSION)"
)
...
def _export_dump_excluding_session_vars(
    ...
    Matrix room/display name via ``EV0_SESSION_CHAT_NAME``) land in the
    ...
    return (
        "{ ( "
        "unset ${!EV0_SESSION_*} ${!EV0_CRON_AUTO_DELIVER_*} "
        # AI_AGENT / EV0_AGENT are per-command attribution markers
        ...
        "AI_AGENT EV0_AGENT "
        f"EV0_UI_SESSION_ID{extra_unset} 2>/dev/null; "
        "export -p; "
        ") || true; } "
        f"> {tmp_path}"
    )
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all 3V0 backends.
    ...
    def __init__(self, cwd: str, timeout: int, env: dict = None):
        ...
        self._snapshot_path = f"{temp_dir}/3v0-snap-{self._session_id}.sh"
        self._cwd_file = f"{temp_dir}/3v0-cwd-{self._session_id}.txt"
    ...
    def init_session(self):
        ...
        _snap_tmp = '"$__ev0_snap_tmp"'
        snapshot_excluded = self._snapshot_excluded_passthrough_names()
        bootstrap = (
            f"umask 077\n"
            f"__ev0_snap_tmp=$(mktemp {_snap_tmp_template}) || exit 1\n"
            ...
            f"__ev0_fns=$(declare -F | awk '{{print $3}}' | grep -vE '^_[^_]') || true\n"
            f"[ -n \"$__ev0_fns\" ] && declare -f $__ev0_fns "
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        for name in passthrough_names:
            marker = f"_EV0_RUNTIME_PASSTHROUGH_{name}"
        ...
        # under 3V0 via the cross-agent ``AI_AGENT`` standard (read by e.g.
        # huggingface_hub's agent detection) plus the 3V0-specific
        # ``EV0_AGENT`` marker.  The value MUST equal our id in the public
        # agent-harness registry (``3v0-agent`` — see huggingface.js
        ...
        # inherited from the 3V0 process.  ``${VAR:-default}`` semantics:
        # never clobber an outer harness value that arrived via the inherited
        # process env (3V0 running inside another agent's terminal).
        parts.append(
            'export AI_AGENT="${AI_AGENT:-3v0-agent}" '
            'EV0_AGENT="${EV0_AGENT:-true}"'
        )
        ...
        parts.append("__ev0_ec=$?")
        # Restrict 3V0 metadata files without changing the user's command
        ...
        if self._snapshot_ready:
            parts.append(
                f"__ev0_snap_tmp=$(mktemp {_snap_tmp_template}) && "
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"EV0_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _wait_for_process(
        ...
        # --- Debug tracing (opt-in via EV0_DEBUG_INTERRUPT=1) -------------
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the __EV0_CWD_{session}__ marker from stdout output.
