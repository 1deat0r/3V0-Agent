...
def _get_scratch_dir() -> Path:
    custom_scratch = wire_env("TERMINAL_SCRATCH_DIR")
    if custom_scratch:
    ...
    if scratch.exists() and os.access(scratch, os.W_OK):
        user_scratch = scratch / os.getenv("USER", "3v0") / "3v0-agent"
...
class SingularityEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        self.instance_id = f"ev0_{uuid.uuid4().hex[:12]}"
        ...
        if self._persistent:
            overlay_base = _get_scratch_dir() / "3v0-overlays"
