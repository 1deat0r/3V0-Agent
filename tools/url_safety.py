...
Limitations:
  - DNS rebinding (TOCTOU): an attacker-controlled DNS server with TTL=0
    ...
    connection. 3V0-owned direct httpx request paths should use
...
def normalize_url_for_request(url: str) -> str:
    """Return an ASCII-safe HTTP URL for 3V0-owned URL tools.
...
def _global_allow_private_urls() -> bool:
    ...
    1. ``EV0_ALLOW_PRIVATE_URLS`` env var  (``true``/``1``/``yes``)
...
def is_safe_url(url: str) -> bool:
    ...
    ``EV0_ALLOW_PRIVATE_URLS=true``), private-IP blocking is skipped.
...
def ssrf_safe_async_http_transport(**kwargs: Any) -> Any:
    ...
    schemes_by_origin_var = contextvars.ContextVar("ev0_ssrf_async_origin_schemes")
...
def ssrf_safe_http_transport(**kwargs: Any) -> Any:
    ...
    schemes_by_origin_var = contextvars.ContextVar("ev0_ssrf_origin_schemes")
