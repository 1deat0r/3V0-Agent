...
def _is_ev0_provider_credential(name: str) -> bool:
    ...
    Non-3V0 API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    let a skill tunnel a 3V0 credential into the execute_code child.
    """
    try:
        ...
        return True
    # Dynamically-generated 3V0-internal secrets (AUXILIARY_*_API_KEY /
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are 3V0-managed provider credentials (from
    ``_EV0_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a 3V0-managed
    ...
    Non-3V0 third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # 3V0-managed provider credentials must not be passed
                ...
                if _is_ev0_provider_credential(name):
                    logger.warning(
                        ...
                        "by _EV0_PROVIDER_ENV_BLOCKLIST). Operator "
