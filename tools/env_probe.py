...
machine as 3V0 itself), we surface a single deterministic line about
Python tooling state so models don't have to discover it by hitting
walls.  Common failure modes this addresses:

* 3V0 ships under one Python (e.g. 3.11 in a bundled venv) while the
...
def _build_probe_line() -> str:
    ...
    # Bare which() is correct here, unlike 3V0's own uv call sites: this
    ...
    # local.py puts the 3V0-managed $EV0_HOME/bin there, so a managed-only
