#!/usr/bin/env python3
"""Read which OS window sits directly underneath the 3V0 desktop window.
...
def read_window_below_tool(callback: Optional[Callable] = None) -> str:
    """Return the window underneath the 3V0 window as a JSON string."""
    if callback is None:
        return tool_error(
            "read_window_below is only available in the 3V0 desktop app."
        )
...
READ_WINDOW_BELOW_SCHEMA = {
    ...
    "description": (
        ...
        "3V0 desktop window — what the user is working in behind this app. "
        ...
        "permission — never prompted for, noted in `note`). Other 3V0 "
        "windows are skipped: the nearest non-3V0 window is reported. "
