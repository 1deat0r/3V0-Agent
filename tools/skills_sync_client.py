...
409), and is driven by:
  ...
  * the ``3v0 sync status|pull|push|now`` CLI.

It lives beside ``tools/skills_sync.py`` (NOT under ``threev0_cli/``) so the
...
is toggled via ``3v0 sync enable/disable`` (a ``sync`` flag on the skill's
...
user-authored skills under ``~/.3V0/skills/`` are eligible; bundled and
...
# only as the local *intent* the user toggles via ``3v0 sync enable`` — it is
...
# namespace from 3v0-agent's local ``content_hash`` (skills_guard.py:846),
...
# sync.base_url (or the EV0_SYNC_BASE_URL bridge env) overrides it to point
...
def resolve_sync_base_url() -> Optional[str]:
    ...
    Order: EV0_SYNC_BASE_URL env bridge -> config.yaml ``sync.base_url`` ->
...
# ---------------------------------------------------------------------------
# Sync feature configuration — env-first, so a 3V0 Cloud instance can be set
...
# precedence as base_url: the EV0_SYNC_* env var wins, else config.yaml
# ``sync.*``, else a built-in default.
#
#   EV0_SYNC_BASE_URL        -> sync.base_url        (the sync plane URL)
#   EV0_SYNC_ENABLED         -> sync.enabled         (master on/off; default off)
#   EV0_SYNC_DEFAULT_OPT_IN  -> sync.default_opt_in  (personal sync policy; default false
...
def sync_feature_enabled() -> bool:
    ...
    ``EV0_SYNC_ENABLED`` -> ``sync.enabled`` -> False. This is the master
    switch a 3V0 Cloud deployment sets to opt its instances into sync by
    ...
    """
    return _sync_config_bool("EV0_SYNC_ENABLED", "enabled", default=False)


def sync_org_auto_propose() -> bool:
    """Whether an agent/user edit to an org skill is proposed automatically.

    ``EV0_SYNC_ORG_AUTO_PROPOSE`` -> ``sync.org_auto_propose`` -> False.

    False (default): edits to an org-shared skill stay LOCAL until the user
    runs ``3v0 sync propose <skill>``. The skill keeps working with the
    ...
    return _sync_config_bool(
        "EV0_SYNC_ORG_AUTO_PROPOSE", "org_auto_propose", default=False
    )
...
def sync_default_opt_in() -> bool:
    ...
    ``EV0_SYNC_DEFAULT_OPT_IN`` -> ``sync.default_opt_in`` -> False.

    False (default): opt-IN — a skill syncs only after an explicit
    ``3v0 sync enable`` (or a plane manifest that opted it in). True: opt-OUT
    ...
    3V0 Cloud deployment wants. Per the design notes, this default is
    ...
    """
    return _sync_config_bool("EV0_SYNC_DEFAULT_OPT_IN", "default_opt_in", default=False)


# ---------------------------------------------------------------------------
# Local skill eligibility + the personal sync opt-in "sync" flag
#
# Only agent-created + user-authored skills under ~/.3V0/skills/ sync.
...
def is_sync_eligible(skill_name: str) -> bool:
    ...
    Eligible = present locally under ~/.3V0/skills/, NOT bundled, NOT
...
def list_synced_skill_names() -> List[str]:
    ...
    - **opt-out (3V0 Cloud "on by default"):** every *eligible* skill syncs
      UNLESS its usage record explicitly carries ``sync: false``. This is what a
      deployment sets (via ``EV0_SYNC_DEFAULT_OPT_IN``) so a user's skills
...
def _all_local_skill_names() -> List[str]:
    ...
    opt-out policy). A skill is any directory under ~/.3V0/skills/ containing
...
def stable_device_id() -> str:
    ...
    Use ``set_device_name()`` / ``3v0 sync device --name`` to set an explicit
    label."""
    ...
    # 3V0 Cloud (and any templated deployment) can seed the label
    # declaratively via EV0_SYNC_DEVICE_NAME, so a hosted instance shows a
    # recognizable name with no CLI call. Env seeds the FIRST-USE value only; it
    # is then persisted, so a later `3v0 sync device --name` (or editing the
...
def set_device_name(name: str) -> str:
    ...
    Writes the (trimmed) name to ~/.3V0/skills/.sync_device_id, overwriting
...
# path under ~/.3V0/skills/ (the sync contract: "the profile root is a tree
...
def _skill_rel_path(skill_name: str) -> Optional[PurePosixPath]:
    """Return the skill's path relative to ~/.3V0/skills/ (posix), or None."""
    try:
...
def _check_version(caps: Dict[str, Any]) -> None:
    ...
    if major != WIRE_VERSION:
        raise SyncError(
            f"this server speaks sync version {ver!r}, but this 3V0 speaks "
            f"{WIRE_VERSION} — update 3V0 to sync with it"
        )
    ...
    if overlaps:
        ...
        return {
            ...
            "message": (
                ...
                f"{conflict_ref}. Resolve out-of-band (3v0 sync / NAS UI)."
            ),
        }
...
def pull_skills(
    ...
    *,
    ...
    ~/.3V0/skills/. Only paths the user has opted into (``sync: true``) are
...
def maybe_push_skills(*, message: str = "3v0 skill sync") -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if not sync_feature_enabled():
            return None  # feature off for this instance (EV0_SYNC_ENABLED)
...
def sync_status() -> Dict[str, Any]:
    """Return a status snapshot for ``3v0 sync status``. Never raises."""
    status: Dict[str, Any] = {
        "nous_admin": False,
        "logged_in": False,
...
#
# Org skills live under a DISTINCT local namespace, ~/.3V0/skills/_org/
...
#
# `3v0 sync propose` is the org sharing surface; proposal is
...
def pull_org_skills(
    ...
    *,
    ...
    """Pull the org canonical set into ``~/.3V0/skills/_org/<org_id>/``.
