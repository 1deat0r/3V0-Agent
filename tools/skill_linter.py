...
Design contract (matches the 3V0 "no lazy-reading escape hatches / don't
...
def _check_metadata_block(frontmatter: Dict[str, Any]) -> List[LintFinding]:
    ...
    if not isinstance(ev0_meta, dict):
        findings.append(
            LintFinding(
                WARNING,
                "missing-metadata",
                "frontmatter is missing metadata.3v0.{tags, related_skills}.",
            )
        )
    else:
    ...
    if author and author.strip().lower() in ("3v0", "agent", "3v0 agent") and (
        author != "3V0 Agent"
    ):
        findings.append(
            LintFinding(
                WARNING,
                "author-caps",
                f"author '{author}' should be '3V0 Agent' (proper caps) "
