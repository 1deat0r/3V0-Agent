...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or 3V0 exit
...
import json
import logging
from env_compat import branded_env, pop_branded_env, set_branded_env, wire_env
import os
import platform
import re
import shlex
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of 3v0 directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "3v0-*")):
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("EV0_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (EV0_INTERACTIVE=1).
...
def _sudo_nopasswd_works() -> bool:
    ...
    """
    terminal_env = wire_env("TERMINAL_ENV", "local").strip().lower() or "local"
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    (EV0_INTERACTIVE=1 or a registered sudo password callback):
    ...
    # interactive 3V0 password prompt or the sudo -S password-pipe path.
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps long-Exited containers labeled ``3v0-agent=1`` for the current
    profile that match the issue #20561 leak class — containers left behind
    by 3V0 processes that exited without firing ``atexit`` (SIGKILL,
    ...
    * ``terminal.docker_orphan_reaper: false`` disables it entirely (the
      ...
      3V0 processes in the same profile and don't trust the
    ...
    * ``_docker_orphan_reaper_ran`` flag — sweep runs once per Python
    ...
    # 2 × lifetime_seconds gives sibling 3V0 processes a generous grace
    ...
    try:
        lifetime = int(wire_env("TERMINAL_LIFETIME_SECONDS", "300"))
    except (TypeError, ValueError):
        lifetime = 300
...
def _docker_session_isolation_enabled() -> bool:
    ...
    if wire_env("TERMINAL_ENV", "local") != "docker":
        return False
    return wire_env("TERMINAL_CONTAINER_PERSISTENT", "true").lower() not in {"true", "1", "yes"}
...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    Exception: RL / benchmark environments (TerminalBench2, Ev0SweEnv, ...)
...
def _parse_env_var(name: str, default: str, converter: Any = int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.3V0/.env or environment variables."
        )
...
def _safe_getcwd() -> str:
    ...
    try:
        ...
        return wire_env("TERMINAL_CWD") or os.path.expanduser("~")
...
def _ensure_terminal_env_bridged() -> None:
    ...
    vars at startup — but processes that skip all of those paths (``3v0
    ...
    be stale from ``3v0 setup``). Environment values for omitted terminal
    ...
    try:
        from threev0_cli.config import apply_terminal_config_to_env, read_raw_config
...
def _get_env_config() -> Dict[str, Any]:
    ...
    _ensure_terminal_env_bridged()
    env_type = wire_env("TERMINAL_ENV", "local")
    
    mount_docker_cwd = wire_env("TERMINAL_DOCKER_MOUNT_CWD_TO_WORKSPACE", "false").lower() in {"true", "1", "yes"}
    ...
    if docker_backend:
        ...
        docker_shm_size = wire_env("TERMINAL_DOCKER_SHM_SIZE", "1g")
    else:
        docker_forward_env = []
        docker_volumes = []
        docker_env = {}
    ...
    cwd = wire_env("TERMINAL_CWD", default_cwd)
    from threev0_cli.config import _is_ssh_remote_tilde_cwd
    if cwd and not _is_ssh_remote_tilde_cwd(env_type, cwd):
        cwd = os.path.expanduser(cwd)
    host_cwd = None
    if env_type == "docker" and mount_docker_cwd:
        docker_cwd_source = wire_env("TERMINAL_CWD") or _safe_getcwd()
    ...
    return {
        "env_type": env_type,
        "modal_mode": coerce_modal_mode(wire_env("TERMINAL_MODAL_MODE", "auto")),
        "docker_image": wire_env("TERMINAL_DOCKER_IMAGE", default_image),
        "docker_forward_env": docker_forward_env,
        "singularity_image": wire_env("TERMINAL_SINGULARITY_IMAGE", f"docker://{default_image}"),
        "modal_image": wire_env("TERMINAL_MODAL_IMAGE", default_image),
        "daytona_image": wire_env("TERMINAL_DAYTONA_IMAGE", default_image),
        "vercel_runtime": (wire_env("TERMINAL_VERCEL_RUNTIME") or "").strip(),
        "cwd": cwd,
        "host_cwd": host_cwd,
        ...
        "ssh_key": (wire_env("TERMINAL_SSH_KEY") or ""),
        # Persistent shell: SSH defaults to the config-level persistent_shell
        # setting (true by default for non-local backends); local is always opt-in.
        # Per-backend env vars override if explicitly set.
        "ssh_persistent": wire_env(
            "TERMINAL_SSH_PERSISTENT",
            wire_env("TERMINAL_PERSISTENT_SHELL", "true"),
        ).lower() in {"true", "1", "yes"},
        "local_persistent": wire_env("TERMINAL_LOCAL_PERSISTENT", "false").lower() in {"true", "1", "yes"},
        ...
        "container_persistent": wire_env("TERMINAL_CONTAINER_PERSISTENT", "true").lower() in {"true", "1", "yes"},
        "docker_volumes": docker_volumes,
        "docker_env": docker_env,
        "docker_run_as_host_user": wire_env("TERMINAL_DOCKER_RUN_AS_HOST_USER", "false").lower() in {"true", "1", "yes"},
        "docker_network": wire_env("TERMINAL_DOCKER_NETWORK", "true").lower() in {"true", "1", "yes"},
        ...
        # removed on exit).
        "docker_persist_across_processes": wire_env(
            "TERMINAL_DOCKER_PERSIST_ACROSS_PROCESSES", "true"
        ).lower() in {"true", "1", "yes"},
        # Startup orphan reaper for 3v0-tagged containers left behind by
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    if env_type == "local":
        ...
        # prior 3V0 processes that hit SIGKILL / OOM / a closed terminal
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "notify_on_complete=true) so 3V0 tracks the process, then run readiness "
...
def terminal_tool(
    ...
    try:
        ...
        # Hard-block: gateway lifecycle commands (systemctl/launchctl/3v0
        # restart|stop targeting 3v0-gateway) must never run inside the
        ...
        # never restart. This mirrors the `3v0 gateway restart` guard in
        # threev0_cli/gateway.py and the cron-path guard in threev0_cli/cron.py,
        # but applies unconditionally (force=True cannot help here).
        if os.environ.get("_EV0_GATEWAY") == "1":
            ...
            if contains_launchctl_submit_command(command):
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "Use 3V0 cron for one-shot delayed work, or install an "
            ...
            if _sanctioned_reload:
                ...
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "to child processes). Run `3v0 gateway restart` from a "
        ...
        if background:
            ...
            try:
                ...
                # 3v0-agent dev work. May 2026 PRs that surfaced this
                ...
                if background and command:
                    ...
                    if _bad_shape:
                        ...
                        canonical_hint = (
                            ...
                            "repeatedly in 3v0-agent dev work (PRs #31329, "
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    if not _async_ok():
                        ...
                        result_data["notify_unsupported"] = (
                            ...
                            "the turn ends (a one-shot runner such as `3v0 -z`, a "
                        ...
                        _gw_platform = _gse("EV0_SESSION_PLATFORM", "")
                        if _gw_platform:
                            _gw_chat_id = _gse("EV0_SESSION_CHAT_ID", "")
                            _gw_thread_id = _gse("EV0_SESSION_THREAD_ID", "")
                            _gw_user_id = _gse("EV0_SESSION_USER_ID", "")
                            _gw_user_name = _gse("EV0_SESSION_USER_NAME", "")
                            _gw_message_id = _gse("EV0_SESSION_MESSAGE_ID", "")
            ...
            if sudo_cache_cleared:
                has_sudo_prompt_callback = _get_sudo_password_callback() is not None
                if has_sudo_prompt_callback or env_var_enabled("EV0_INTERACTIVE"):
                    output += (
        ...
        degraded_mode = wire_env("TERMINAL_DEGRADED_MODE", "warn").strip().lower()
