...
One ``CDPSupervisor`` runs per 3V0 ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach 3V0 logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "3v0-dialog-bridge.invalid"
...
(() => {
  if (window.__ev0DialogBridgeInstalled) return;
  window.__ev0DialogBridgeInstalled = true;
  const ENDPOINT = "http://3v0-dialog-bridge.invalid/";
