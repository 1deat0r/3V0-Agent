"""Wake-word ("Hey 3V0") detection — hands-free session trigger.
...
wake word, 3V0 opens a fresh session and captures voice via the existing
...
* **openwakeword** (default, free, no API key) — loads an ONNX model. Defaults
  to the bundled "hey 3v0" model (``tools/wakewords/``) so the wake word
...
# Minimum gap between two consecutive wake fires, so one "hey 3v0" can't
...
# Bundled "hey 3v0" model (tools/wakewords/) — the default, so the wake word
# works out of the box. Config names in _ALIASES resolve to it, not a built-in.
_BUNDLED_MODEL_NAME = "hey_3v0"
_BUNDLED_MODEL_ALIASES = frozenset({"", "hey_3v0", "hey 3v0", "3v0"})
...
def wake_phrase(cfg: Optional[Dict[str, Any]] = None) -> str:
    ...
    return str(_get(cfg, "phrase")) or "hey 3v0"
...
def enrolled_profile_phrases() -> Dict[str, str]:
    ...
    try:
        ...
        from threev0_cli.profiles import get_profile_dir, list_profiles
...
def silent_audio_hint(details: Dict[str, Any]) -> str:
    ...
    if sys.platform == "darwin":
        return (
            "Microphone delivers only silence. Grant the 3V0 backend "
...
# EV0_HOME. Keywords are typed phrases tokenized at RUNTIME — no
...
def _sherpa_model_root() -> Path:
    ...
    return get_threev0_home() / "cache" / "wakewords"
...
class _SherpaKwsEngine(_Engine):
    ...
    model's vocabulary, so "hey 3v0", "hey coder", or any other phrase works
    ...
    def __init__(self, cfg: Dict[str, Any]):
        ...
        # wake any profile ("hey 3v0" / "hey coder" / ...). display-name →
        # profile is kept for routing the match back.
        phrase = str(_get(cfg, "phrase") or "hey 3v0").strip()
        ...
        kw = tempfile.NamedTemporaryFile(
            mode="w", suffix=".txt", prefix="3v0-kws-", delete=False, encoding="utf-8"
        )
...
def check_wake_word_requirements(cfg: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    if provider == "porcupine" and not (os.getenv("PORCUPINE_ACCESS_KEY") or "").strip():
        ...
        hint = (f"Wake word needs {missing} configured — run `3v0 tools` "
...
# ---------------------------------------------------------------------------
# Process-wide singleton (mirrors threev0_cli.voice's continuous API)
# ---------------------------------------------------------------------------
...
def _lock_path() -> Path:
    ...
    return get_default_threev0_root() / "runtime" / "wake-word.lock"
