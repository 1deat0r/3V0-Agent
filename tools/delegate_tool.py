...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``3v0-cli`` (which bundles
...
def _blocked_toolsets_for_role(role: str) -> List[str]:
    ...
    mixed platform bundles such as ``3v0-cli`` because those also contain
...
def _build_child_agent(
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. 3v0-cli) so that individual
    ...
    # Blocked tools also live inside mixed platform bundles (3v0-cli,
    # 3v0-telegram, etc.) that _strip_blocked_tools must keep because they
    ...
    # child 3V0/Qwen subagent onto the parent's Claude Messages wire (or the
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.3V0/logs/subagent-<sid>-<ts>.log``
    ...
    if _subagent_id:
        if owner_session_id is None:
            try:
                ...
                owner_session_id = get_session_env("EV0_UI_SESSION_ID", "") or None
            except Exception:
...
def delegate_task(
    ...
    # EV0_SESSION_ID ContextVar and os.environ with the subagent's internal
    # id before the background-dispatch code below would read it. The
    # request-scoped chat_id binding (the raw X-3V0-Session-Id on
    ...
    try:
        ...
        _origin_ui_session_id = get_session_env("EV0_UI_SESSION_ID", "")
    except Exception:
    ...
    if background:
        ...
        if not _async_ok:
            ...
            # with that id in X-3V0-Session-Id once the batch completes.
            ...
            # EV0_SESSION_ID here would return the subagent's internal id.
        ...
        if not _async_ok:
            ...
            if isinstance(_sync_result, dict):
                _sync_result["note"] = (
                    ...
                    "one-shot runner such as `3v0 -z`, a cron job, a Kanban "
        ...
        try:
            ...
            _source = get_session_env("EV0_SESSION_SOURCE", "")
            ...
            _origin_ui_session_id = (
                get_session_env("EV0_UI_SESSION_ID", "") or _origin_ui_session_id
            )
        ...
        if not _session_key:
            ...
            # during gateway/TUI turns and EV0_SESSION_KEY is not in the CLI
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run '3v0 auth'."
        )
...
def _load_config() -> dict:
    """Load delegation config from the active 3V0 config.

    Prefer the shared persistent loader because it follows the active
    EV0_HOME/profile. ``cli.CLI_CONFIG`` is a legacy fallback for entry
    ...
    ``EV0_IGNORE_USER_CONFIG=1`` (``3v0 chat --ignore-user-config``) is
