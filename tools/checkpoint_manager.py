...
-----------------------------------------------------------------------------
    ...
    ~/.3V0/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/3v0/<hash16>        — per-project branch tip
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (3V0 convention — don't recursively snapshot siblings)
    ".worktrees/",
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("EV0_CHECKPOINT_TIMEOUT", 30)))
...
def _git_env(
    ...
    The shared store is internal 3V0 infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``3v0 checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `3v0 checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "3v0@local"], store, cfg_wd)
    _run_git(["config", "user.name", "3V0 Checkpoint"], store, cfg_wd)
...
# look for ``HEAD``/``info/exclude``/``EV0_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at EV0_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "EV0_WORKDIR").write_text(
...
class CheckpointManager:
    ...
    def session_diff(self, working_dir: str) -> Dict:
        ...
        This powers ``/diff session``.  It answers "what has 3V0 changed
        ...
        *retained* checkpoint may predate the current session (or, after
        ...
        3V0 changed", not an exact per-session ledger.
...
# ---------------------------------------------------------------------------
# Public helpers for `3v0 checkpoints` CLI
# ---------------------------------------------------------------------------
