...
``3v0 mcp install <name>`` in the terminal.
"""
...
def setup_mcp_tool(
    ...
    if callback is None:
        return tool_error(
            "setup_mcp is only available in the 3V0 desktop app. Use the "
            "terminal instead: `3v0 mcp install <name>` for catalog entries, "
            "`3v0 mcp login <name>` for OAuth."
        )
...
SETUP_MCP_SCHEMA = {
    ...
    "description": (
        ...
        "3V0 desktop chat. The card lets them install a catalog entry, "
        ...
        "continue without the server. Catalog names: run `3v0 mcp catalog` "
