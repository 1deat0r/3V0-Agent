...
``ui_meta['3v0-bots']`` block in its profile.yaml), a bot's canonical
...
def _is_bot_managed(profile_dir: Path) -> bool:
    """True when profile.yaml carries a ui_meta['3v0-bots'] block.
    ...
    try:
        ...
        if "3v0-bots" not in raw:
            return False
        import yaml

        data = yaml.safe_load(raw)
        ui_meta = data.get("ui_meta") if isinstance(data, dict) else None
        return isinstance(ui_meta, dict) and isinstance(ui_meta.get("3v0-bots"), dict)
    except Exception:
        return False
...
def _handle(name: str) -> str:
    # The mention middleware aliases the default profile as @3v0.
    return "3v0" if name == "default" else name
...
def _build_section(home: Path) -> str:
    ...
    return (
        f"{_PROTOCOL_HEADING}\n"
        "This install runs Bot Mode: each 3V0 profile is an agent teammate with "
        ...
        "```\n"
        f'3v0 -p <agent-name> chat --in ~ -c "Bot Chat" -Q -q "Message from 🤖 {handle} (@{handle}): your message"\n'
        "```\n"
        ...
        "which agent it came from. Run `3v0 profile list` for the LIVE teammate "
