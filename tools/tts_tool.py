...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.3V0/config.yaml``. 3V0
...
Configuration is loaded from ~/.3V0/config.yaml under the 'tts:' key.
...
def _resolve_provider_key(env_var: str, provider_id: str) -> str:
    ...
    credential pool populated by ``3v0 auth add <provider_id>``).
...
def _get_default_output_dir() -> str:
    ...
    return str(get_threev0_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.3V0/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.3V0/config.yaml.
...
def _get_provider(tts_config: Dict[str, Any]) -> str:
    ...
    ``3v0 tools``); otherwise the historical Edge backend remains active.
    """
...
# custom voice-cloning scripts, etc.) into 3V0 without any Python code
...
#           output_format: wav
#
# 3V0 writes the input text to a temp UTF-8 file, runs the command with
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__EV0_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _command_provider_env_passthrough(config: Dict[str, Any]) -> list:
    ...
    template (curl one-liners). The child env is scrubbed of 3V0 secrets by
...
def _run_command_tts(
    ...
    Child env is scrubbed of 3V0 secrets (salvage of #56332) while still
...
# file, so retired models disappear from 3v0 the next time the
...
def _generate_deepinfra_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if not api_key:
        raise ValueError(
            "DEEPINFRA_API_KEY not set. Run `3v0 setup` to configure, "
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if not api_key:
        raise ValueError("No xAI credentials found. Configure xAI OAuth in `3v0 model` or set XAI_API_KEY.")
    ...
    # output_format when 3V0 actually needs a non-default format/override.
...
def _generate_gemini_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if urlparse(base_url).hostname == "generativelanguage.googleapis.com":
        try:
            ...
            _threev0_version = str(_threev0_cli.__version__)
        except Exception:
            _threev0_version = "0.0.0"
        # Include 3V0 client context following Gemini's partner
        # integration guidance:
        # https://ai.google.dev/gemini-api/docs/partner-integration
        headers["X-Goog-Api-Client"] = f"3v0-agent/{_threev0_version}"
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where 3V0 caches Piper voice models.

    Resolves to ``~/.3V0/cache/piper-voices/`` under the active
    EV0_HOME so voice downloads follow profile boundaries.
    """
    from threev0_constants import get_threev0_dir
    root = Path(get_threev0_dir("cache/piper-voices", "piper_voices_cache"))
...
def _text_to_speech_single(
    ...
    *,
    ...
    platform = get_session_env("EV0_SESSION_PLATFORM", "").lower()
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run `3v0 setup` to install Mistral support."
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run 3v0 setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run '3v0 setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run '3v0 tools' and select Piper under TTS, or install manually: "
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_threev0_home()}/audio_cache/<timestamp>.mp3"
            },
            "speed": {
                "type": "number",
