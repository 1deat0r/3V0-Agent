...
def _is_local_terminal_backend() -> bool:
    ...
    """
    return wire_env("TERMINAL_ENV", "local").strip().lower() in ("local", "")


def _media_cache_roots() -> list:
    """Agent-managed media cache directories under EV0_HOME (host side).
...
def _permitted_host_read_target(p: Path, ctx: ResolveContext) -> Optional[Path]:
    ...
    - Non-local backend: permitted only if the path resolves inside a media
      cache root. A container-visible cache path (e.g. ``/root/.3V0/cache/
