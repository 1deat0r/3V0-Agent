...
transport, discovers their tools, and registers them into the 3v0-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.3V0/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.3V0/logs/mcp-stderr.log), tagged with the
...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach 3V0' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    3V0 explicitly tagged during dotenv loading, plus any variables
    ...
    an exception: users configured that backend specifically so 3V0 and
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            candidates = [
                ...
                # image (which the 3V0 Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → 3V0 MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that 3V0 gateways know how to render.
...
def _render_mcp_resource_block(block, server_name: str = "") -> str:
    ...
    - ``EmbeddedResource`` with blob contents → bytes are decoded (size-capped)
      and materialized into the 3V0 document cache; returns a marker with
...
def _unwrap_exception_group(exc: BaseException) -> BaseException:
    ...
    Adapted from :func:`threev0_cli.mcp_config._unwrap_exception_group` with
...
def _resolve_identity_header(server_name: str, config: dict):
    ...
    mode resolves the value to the active 3V0 profile name once at
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through 3V0' existing approval
    ...
    async def __call__(self, context, params):
        ...
        # the agent's contextvars (EV0_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit EV0_SESSION_PLATFORM, so the
    ...
    def _make_logging_callback(self):
        ...
        server into 3V0' logging (agent.log via threev0_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _ensure_mcp_sdk():
            raise ImportError(
                ...
                "it is not installed. Run `3v0 setup` to install MCP support, "
                "then retry."
            )
        ...
        # ungraceful exit of this 3V0 process (kill -9, crash, force-quit)
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through 3V0' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until 3V0 is fully
                ...
                if not self._ready.is_set():
                    if failure_class == "permanent":
                        ...
                        # re-authenticated with ``3v0 mcp login``. Parking
...
def _handle_auth_error_and_retry(
    ...
    return tool_error(
        ...
        f"Run `3v0 mcp login {server_name}` (or delete the tokens "
        f"file under ~/.3V0/mcp-tokens/ and restart). Do NOT retry "
...
# ---------------------------------------------------------------------------
# Advisory file lock that prevents N concurrent 3V0 processes (e.g.
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local EV0_HOME override into ``coro``.
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local EV0_HOME override onto the MCP loop.
    ...
    # vanish here: OAuth token stores and any other get_ev0_home()
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the 3V0 config file.
    ...
    ``os.environ`` (which includes ``~/.3V0/.env`` loaded at startup).
    """
    try:
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through 3V0' MEDIA: tag convention and out to messaging
            ...
            # 3V0' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves 3V0's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. 3V0 uses the ``mcp__<server>__<tool>``
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the 3V0 registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``3v0 tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the 3V0 registry.
...
def _kill_orphaned_mcp_children(
    ...
    survivors, avoiding shared-resource collisions when multiple 3v0
