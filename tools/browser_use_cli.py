...
_PRIVATE_BROWSER_SENTINEL = "_EV0_BU_PRIVATE_BROWSER"
...
def _ev0_ensure_own_tab():
    ...
    _marker = _os.path.join(
        _tf.gettempdir(), "3v0-bu-owntab-%s-%s-%s" % (_uid, _name, _dpid)
    )
...
def _base_subprocess_env() -> dict:
    ...
    # may differ from 3V0's venv Python. PYTHONPATH/PYTHONHOME inherited
    # from the agent process point at 3V0's venv site-packages, and a
...
def default_downgrade_notice() -> Optional[str]:
    ...
    try:
        ...
        return (
            ...
            "Run `3v0 tools` (Browser Automation → Browser Use) to install it, "
...
def install_cli(timeout_s: int = 600) -> Tuple[bool, str]:
    ...
    # prevent provisioning the canonical 3V0-managed copy, or resolution
...
def _resolve_backend_cdp(
    ...
    try:
        ...
        return (
            ...
            "switch backends via `3v0 tools` → Browser Automation."
        )
