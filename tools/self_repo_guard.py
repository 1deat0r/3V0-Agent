...
def _block_message(operation: str, root: Path) -> str:
    ...
    return (
        f"Blocked: `{operation}` would rewrite 3V0's live source checkout "
        ...
        "checkout, stop 3V0, run the command externally, then restart "
        "3V0."
    )
...
def _scratch_dir_hint() -> str:
    ...
    base = Path(ev0_home).expanduser() if ev0_home else Path.home() / ".3V0"
