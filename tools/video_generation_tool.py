...
provider** — turn on a backend by enabling a plugin (``3v0 plugins
enable video_gen/<name>``) and selecting it in ``3v0 tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/3v0-agent-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `3v0 tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"registered that name. Run `3v0 plugins list` to see "
            f"installed video gen backends, or `3v0 tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `3v0 tools` → "
...
# `3v0 tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`3v0 tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if provider is None:
        parts.append(
            ...
            "until the user picks one via `3v0 tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
