...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.3v0.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.3v0.blueprint`` is a mapping containing
    ...
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.3v0.blueprint must be a mapping")
