"""Windows UTF-8 bootstrap for 3V0 entry points.
...
should be imported at the very top of every 3V0 entry point
(``3v0``, ``3v0-agent``, ``3v0-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``threev0_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``threev0_bootstrap``, never ``threev0_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing 3V0 modules.

    3V0 ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real 3V0 source root
    ...
    """
    root = src_root or os.environ.get("EV0_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``EV0_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("EV0_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import threev0_bootstrap``
# (or ``from threev0_bootstrap import apply_windows_utf8_bootstrap``) at
