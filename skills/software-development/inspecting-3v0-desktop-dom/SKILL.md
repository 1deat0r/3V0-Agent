---
name: inspecting-3v0-desktop-dom
description: "Read the live 3V0 desktop DOM/CSS over CDP."
version: 1.0.0
author: 3V0 Agent
license: MIT
...
# Inspecting the live 3V0 desktop DOM
...
`debugging-3v0-desktop`), or anything where the real question is "does this
look right".
...
- **no `EV0_DESKTOP_DEV_SERVER`** — an unpackaged `electron .` against
  `dist/` is how the packaged app gets smoke tested, so it behaves like one.

`EV0_DESKTOP_CDP_PORT` moves the port (`=9333`) or disables it (`=off`).

Check before doing anything else:

```bash
curl -s --max-time 3 http://127.0.0.1:${EV0_DESKTOP_CDP_PORT:-9222}/json/version
```
...
```bash
cd apps/desktop
EV0_HOME=/tmp/cdp-probe-home \
EV0_DESKTOP_DEV_SERVER=http://127.0.0.1:5174 \
EV0_DESKTOP_CDP_PORT=9333 \
  npx electron . --user-data-dir=/tmp/cdp-probe-userdata
```

The separate `--user-data-dir` dodges Electron's single-instance lock, so it
cannot collide with a running `hgui`; the separate `EV0_HOME` keeps it away
from real sessions. Pick a port other than 9222 for the same reason. Run it in
the background and kill it when done.

`npm run perf:serve` does the same with a temp `EV0_HOME` baked in, if you
...
- **A throwaway `EV0_HOME` has no backend.** The app logs `ECONNREFUSED` for
  `3v0:api` and may exit on its own. The renderer still mounts and the DOM is
