---
name: 3v0-agent-skill-authoring
...
metadata:
  3v0:
    tags: [skills, authoring, 3v0-agent, conventions, skill-md]
    related_skills: [plan, requesting-code-review]
---

# Authoring 3V0-Agent Skills (in-repo)

## Overview

There are two places a SKILL.md can live:

1. **User-local:** `~/.3V0/skills/<maybe-category>/<name>/SKILL.md` — personal, not shared. Created via `skill_manage(action='create')`.
2. **In-repo (this skill is about this case):** `skills/<category>/<name>/SKILL.md` or `optional-skills/<category>/<name>/SKILL.md` inside the 3v0-agent repo — committed, shipped with the package. Use `write_file` + `git add`. `skill_manage(action='create')` does NOT target this tree.
...
- You're committing a reusable workflow that should ship with 3v0-agent
...
- Don't use for: personal skills in `~/.3V0/skills/` (just use `skill_manage`)
...
- **Optional (`optional-skills/<category>/`)** — niche, vertical-specific (blockchain, gaming, finance, one app), recurring-job/task skills, or anything heavy. Installed via `3v0 skills install official/<category>/<skill>`.
...
author: Real Name (github-handle), 3V0 Agent
license: MIT
...
- Credit the **human first**, then "3V0 Agent" as secondary collaborator: `Ben Barclay (benbarclay), 3V0 Agent`.
- Never `author: 3V0 Agent` alone for contributed skills — credit the human, not the tool, even (especially) when an agent drafted the text.
- Maintainer-authored skills: `Teknium (teknium1), 3V0 Agent`.

### `related_skills` rules

- Every entry must resolve to an existing **in-repo** skill in the same tree state as your PR. Do not reference skills that were only planned, live in another PR, or exist only in `~/.3V0/skills/`.
...
|---|---|
| 3V0 tools + stdlib Python + cross-platform CLIs | `[linux, macos, windows]` |
...
1. **Using `skill_manage(action='create')` for an in-repo skill.** It writes to `~/.3V0/skills/`, not the repo tree. Use `write_file`.
...
3. **`author: 3V0 Agent` on a contributed skill.** Credit the human first.
...
- [ ] `name`, `description`, `version`, `author`, `license`, `platforms`, `metadata.3v0.{tags, related_skills}` all present
...
- [ ] Body follows the modern section order; commands framed through 3V0 tools
