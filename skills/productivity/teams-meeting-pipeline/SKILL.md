...
Everything operator-facing is a `3v0 teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
Before using the pipeline, verify these are set in `${EV0_HOME:-~/.3v0}/.env`:
...
```bash
3v0 teams-pipeline validate              # config snapshot — run first after any change
3v0 teams-pipeline token-health          # Graph token status
3v0 teams-pipeline token-health --force-refresh   # force a fresh token acquisition
3v0 teams-pipeline list                  # recent meeting jobs
3v0 teams-pipeline list --status failed  # only failed jobs
3v0 teams-pipeline show <job-id>         # full detail of one job
3v0 teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
3v0 teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
3v0 teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
3v0 teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
```

### Subscription management

```bash
3v0 teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

3v0 teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
3v0 teams-pipeline delete-subscription <sub-id>
3v0 teams-pipeline maintain-subscriptions            # renew near-expiry ones
3v0 teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `3v0 teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `3v0 cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
