#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for 3V0 Agent.
...
# Ensure sibling modules (_ev0_home) are importable when run standalone.
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same 3V0 profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the 3V0 Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the 3V0 Docker image's "
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to 3V0 home."""
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for 3V0")
