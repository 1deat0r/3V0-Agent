#!/usr/bin/env python3
"""Google Workspace API CLI for 3V0 Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing 3V0-facing JSON contract and falls back to the Python client
...
def _gws_binary() -> str | None:
    override = os.getenv("EV0_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for 3V0 Agent")
