...
4. Store the token in `${EV0_HOME:-~/.3v0}/.env` (or via `3v0 setup`):
   ```
...
`-s` suppresses curl's progress bar — keep it set for every call so the tool output stays clean for 3V0. Pipe through `python -m json.tool` (always present) or `jq` (if installed) for readable JSON.
...
- **`AIRTABLE_API_KEY` flows from `${EV0_HOME:-~/.3v0}/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
