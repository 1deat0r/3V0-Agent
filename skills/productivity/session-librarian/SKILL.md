...
| List/filter by metadata (age, source, cost, tokens, workspace) | `3v0 sessions list` / `stats` via terminal |
| Rename | `3v0 sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `3v0 sessions archive <filters>` |
| Delete (destructive) | `3v0 sessions delete` / `3v0 sessions prune <filters>` |
| Export before deleting anything valuable | `3v0 sessions export --session-id <id> --format md` |
...
`3v0 sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `3v0 sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `3v0 sessions list`
