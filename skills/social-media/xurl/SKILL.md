...
version: 1.1.3
author: xdevplatform + openclaw + 3V0 Agent
license: MIT
...
- The user must fill `~/.xurl` with secrets manually on their own machine. In Docker, this must be the `~` seen by 3V0 tool subprocesses; see the Docker note below.
...
> **Docker HOME pitfall:** In the official 3V0 Docker layout, `/opt/data` is `EV0_HOME`, but 3V0 tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` for 3V0-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash
...
> ```
> If `HOME=/opt/data xurl auth status` succeeds but `HOME=/opt/data/home xurl auth status` shows no apps or tokens, 3V0 tool calls will not see the credentials.
...
- **Token storage:** `~/.xurl` is YAML. In Docker, use the 3V0 subprocess HOME (`/opt/data/home` in the official image) so tokens land under `/opt/data/home/.xurl`. Never read or send this file to LLM context.
...
- 3V0 adaptation: reformatted for 3V0 skill conventions; safety guardrails preserved verbatim.
