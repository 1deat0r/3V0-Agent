...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _ev0_env="${EV0_HOME:-$HOME/.3v0}/.env"; [ -f "$_ev0_env" ] && grep -q "^GITHUB_TOKEN=" "$_ev0_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_ev0_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
    elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
      GITHUB_TOKEN=$(uv run python "${EV0_HOME:-$HOME/.3v0}/skills/github/github-auth/scripts/git-credential-token.py")
    fi
  fi
fi
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from 3V0 Agent\",
    \"comments\": [
...
```bash
source "${EV0_HOME:-$HOME/.3v0}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by 3V0 Agent. Code looks clean — good test coverage, no security concerns."
