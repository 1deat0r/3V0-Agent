...
- Give it a title like "3v0-agent-<machine-name>"
...
uv run python "${EV0_HOME:-$HOME/.3v0}/skills/github/github-auth/scripts/git-credential-token.py"
```
...
elif _ev0_env="${EV0_HOME:-$HOME/.3v0}/.env"; [ -f "$_ev0_env" ] && grep -q "^GITHUB_TOKEN=" "$_ev0_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_ev0_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
  export GITHUB_TOKEN=$(uv run python "${EV0_HOME:-$HOME/.3v0}/skills/github/github-auth/scripts/git-credential-token.py")
  echo "AUTH_METHOD=curl"
else
