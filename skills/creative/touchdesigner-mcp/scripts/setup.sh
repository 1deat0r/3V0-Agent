...
EV0_HOME_DIR="${EV0_HOME:-$HOME/.3v0}"
EV0_CFG="${EV0_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure 3V0 config has twozero_td MCP entry ──
if [[ ! -f "$EV0_CFG" ]]; then
    echo -e " ${FAIL} 3V0 config not found at ${EV0_CFG}"
    manual_steps+=("Create ${EV0_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$EV0_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in 3V0 config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to 3V0 config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${EV0_CFG} manually"); }
    manual_steps+=("Restart 3V0 session to pick up config change")
fi
