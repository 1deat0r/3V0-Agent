...
```
3V0 Agent -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${EV0_HOME:-$HOME/.3v0}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to 3V0 config (if missing)
...
3. **Restart 3V0 session** to pick up the new MCP server
