...
version: 2.0.0
author: Francesco Bonacci (f-trycua), 3V0 Agent
license: MIT
...
3V0 drives [cua-driver](https://github.com/trycua/cua) under the hood.
This wrapper skill teaches the 3V0 `computer_use` workflow and action
...
skill installed by `cua-driver skills install`. 3V0 autodetection is a
planned cua-driver follow-up, so currently point 3V0 at the resulting
...
1. **Config grant (standard and unrestricted modes).** When
   ...
   3V0 applies the same host-side floor in unrestricted mode. If it is not
...
Explicit 3V0 YOLO (`--yolo`, `/yolo`, or `approvals.mode: off`) launches an
...
|---|---|
| `cua-driver not installed` | Run `3v0 computer-use install`, or `3v0 tools` and enable Computer Use |
| Captures consistently return empty / "no on-screen window" | On Linux: DISPLAY may not be set (X11) or you're on pure Wayland — ask the user to run `3v0 computer-use doctor`. On Windows: you may be in Session 0 (SSH session) instead of the interactive desktop — see the cua-driver `WINDOWS.md` deep-dive |
...
| Type text disappears into a terminal emulator | cua-driver detects terminals (Ghostty, iTerm2, Terminal.app, Windows Terminal, mintty, etc.) and routes through key-event synthesis — should "just work" on a recent cua-driver. If it doesn't, ask the user to run `3v0 computer-use doctor` |
...
| Anything else weird | **First action: ask the user to run `3v0 computer-use doctor`.** It runs the cua-driver `health_report` MCP tool and prints a structured per-check matrix. Their output tells you (and them) exactly what's wrong |
...
3V0 intentionally keeps THIS skill focused on the 3V0-side
...
3V0 autodetection is a planned follow-up in trycua/cua. For now, the command
installs the pack under `~/.cua-driver/skills/cua-driver`; point 3V0 at that
