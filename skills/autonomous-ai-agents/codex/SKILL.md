...
metadata:
  3v0:
    ...
    related_skills: [claude-code, 3v0-agent]
---

# Codex CLI

Delegate coding tasks to [Codex](https://github.com/openai/codex) via the 3V0 terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For 3V0 itself, `model.provider: openai-codex` uses 3V0-managed Codex
OAuth from `~/.3V0/auth.json` after `3v0 auth add openai-codex`. For the
...
When invoking the Codex CLI from a 3V0 gateway/service context (for example,
