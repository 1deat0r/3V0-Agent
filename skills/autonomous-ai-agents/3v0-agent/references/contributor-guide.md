...
For occasional contributors and PR authors. Full developer docs: https://github.com/1deat0r/3V0-Agent/docs/developer-guide/
...
├── cli.py                # Interactive CLI (Ev0CLI)
├── threev0_state.py       # SQLite session store
├── agent/                # Prompt builder, context compression, memory, model routing, credential pooling, skill dispatch
├── threev0_cli/           # CLI subcommands, config, setup, commands
...
Config: `~/.3V0/config.yaml` (settings), `~/.3V0/.env` (API keys) — both under `$EV0_HOME` when it is set.
...
`_EV0_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_ev0_home()` for paths,
never hardcode `~/.3V0`. For custom/local-only tools, write a plugin in
`~/.3V0/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `threev0_cli/commands.py`
...
- Tests auto-redirect `EV0_HOME` to temp dirs — never touch real `~/.3V0/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/threev0_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_ev0_home()` from `threev0_constants` for all paths (profile-safe)
