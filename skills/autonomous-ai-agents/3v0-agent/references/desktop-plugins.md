...
Write plugins for the 3V0 desktop app: statusbar items, layout panes,
...
system (`~/.3V0/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$EV0_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$EV0_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
...
- The 3V0 desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$EV0_HOME/desktop-plugins/` (usually
  `~/.3V0/desktop-plugins/`).

## How to Run

1. Create `$EV0_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.3V0/...` by default, or `~/.3V0/profiles/<profile>/...` under a
...
The ONLY import surface is `@3v0/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  Fires only while the user is away from 3V0 (use `host.notify` for the
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.3V0/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@3v0/plugin-sdk`, `react`, and
