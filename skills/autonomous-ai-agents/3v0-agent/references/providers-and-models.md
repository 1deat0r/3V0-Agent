...
Set via `3v0 model` (picker) or `3v0 setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://github.com/1deat0r/3V0-Agent/docs/integrations/providers
...
| nous | OAuth device code | `3v0 auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `3v0 auth add openai-codex` |
| qwen-oauth | OAuth | `3v0 auth add qwen-oauth` |
| minimax-oauth | OAuth | `3v0 auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`3v0 auth`).
Fallback chain when the primary fails: `3v0 fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`threev0_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   3v0 config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
