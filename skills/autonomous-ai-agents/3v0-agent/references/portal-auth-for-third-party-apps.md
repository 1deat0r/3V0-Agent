...
## Layer 1 — Is this thing a 3V0 plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the 3V0 process**. Its LLM calls go through 3V0's already-configured provider. | Already uses Portal if user's 3V0 is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside 3V0. That LLM call is already authenticated
via 3V0's provider config. The plugin's own server auth (e.g.
...
What 3V0 Agent has that *feels* like OAuth — `3v0 login --provider nous`
opening a browser, user signs in, token lands in `~/.3V0/auth.json` — is a
**3V0-specific browser flow**. Under the hood it produces a credential
3V0 uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read 3V0's existing Portal credential out of `~/.3V0/auth.json`.
...
rides on the credential 3V0 already holds.

Where this could live in 3V0:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for 3V0 itself.
...
1. First decide: 3V0 plugin (runs inside 3V0) or separate app? If plugin,
   it already uses Portal via 3V0's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   3V0 agent itself") is fully supported. 3V0 treats agent
...
pitch in the Tool Gateway announcement is specifically about 3V0 Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when 3V0 is configured to use Portal as its provider. It is
