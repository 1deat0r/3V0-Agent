...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for 3V0 itself.
...
- Write access to the 3V0 home dir — `~/.3V0` by default, or `$EV0_HOME`
  / the active profile's dir. Skins live in `<3v0-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `3v0 config set`).
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `3v0 config set display.skin default` (or they can `/skin default`).
...
```
3v0 skin set <key> <hex>      # e.g. 3v0 skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `3v0 skin set background "#08201f"` changes
only the background; `3v0 skin use <name>` / `3v0 skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.3V0`** when a profile is active — resolve the real home
  from `$EV0_HOME` first, falling back to `~/.3V0`.
...
- **Never hand-edit `config.yaml` to activate.** Use `3v0 config set
...
- **You apply it, not the user.** `3v0 config set display.skin <name>` is
...
- `read_file` the written `<3v0-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `3v0 config get display.skin` and confirm it reports `<name>`.
