...
**three** code paths in 3V0 that cap a batch. If none of them fired, the
cap came from the **model itself** — not from 3V0 — and the user's
narration of "the runtime caps at N" is the model rationalising its own
choice.

## The three real caps in 3V0
...
3. **Cost-warning** — same `_get_max_concurrent_children()`. When the
   ...
   log line** — it does not cap anything. Easy to mis-read as "3V0 is
   refusing my value."
...
3v0 config get delegation.max_concurrent_children

# 2. Did 3V0' truncator or rejector actually fire?
grep -E "Truncated.*delegate_task|Too many tasks" ~/.3V0/logs/agent.log | tail
# If neither line appears, neither cap path executed.

# 3. Confirm the resolver returns what config says (in venv with 3v0 on path)
...
appears, **the cap is the model**, not 3V0.
