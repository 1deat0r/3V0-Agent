...
3V0 runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
`3v0 config edit` writes correctly.

### `execute_code` / Sandbox

**WinError 10106** from the sandbox child process — it can't create an
`AF_INET` socket. Root cause is usually 3V0's env scrubber dropping
...
3V0-installed `venv/Scripts/` has no pip/pytest (stripped for size).
...
every 3V0 tool and most Windows APIs. Prefer forward slashes in code
