...
Edit with `3v0 config edit` or `3v0 config set section.key value`.
Full reference: https://github.com/1deat0r/3V0-Agent/docs/user-guide/configuration
...
`3v0 config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `3v0 tools` (interactive) or `3v0 tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_EV0_CORE_TOOLS` is the default bundle most platforms inherit).
