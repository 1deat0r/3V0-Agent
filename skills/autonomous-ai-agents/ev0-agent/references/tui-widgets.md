...
Author widget apps for the 3V0 TUI (`3v0 --tui`): glanceable ambient
...
- The TUI must be in use (`3v0 --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.3V0/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.3V0/logs/tui_gateway_crash.log` if a widget never appears.
