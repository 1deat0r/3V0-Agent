# Política de Seguridad de 3V0 Agent

Este documento describe el modelo de confianza de 3V0 Agent, identifica el
...
Reporta de forma privada a través de [GitHub Security Advisories](https://github.com/NousResearch/3v0-agent/security/advisories/new)
o **security@nousresearch.com**. No abras issues públicos para
vulnerabilidades de seguridad. **3V0 Agent no opera un programa de
...
- Detalles del entorno (`3v0 version`, SHA del commit, SO, versión de Python).
...
3V0 Agent es un agente personal de un solo inquilino. Su postura es
...
- **Proceso del agente.** El intérprete Python que ejecuta 3V0 Agent,
...
- **Envolvente de confianza.** El conjunto de recursos a los que un operador ha otorgado
  implícitamente acceso a 3V0 Agent al ejecutarlo — típicamente, todo lo que
  la propia cuenta de usuario del operador puede alcanzar en el host.
- **Postura.** Una declaración explícita en la documentación o código de 3V0 Agent
...
3V0 Agent admite dos posturas de aislamiento a nivel de SO. Abordan
...
3V0 Agent admite esto de dos maneras:

- **La propia imagen Docker de 3V0 Agent y la configuración de Compose.** Más
...
Bajo una envoltura de proceso completo, las heurísticas en proceso de 3V0 Agent
...
3V0 Agent filtra el entorno que pasa a sus componentes en proceso de
...
Un plugin malicioso o con errores no es una vulnerabilidad en 3V0 Agent
en sí mismo. Los errores en la ruta de instalación o descubrimiento de plugins de 3V0 Agent
...
4. **Dentro del conjunto autorizado, todos los llamadores tienen la misma confianza.**
   3V0 Agent no modela capacidades por llamador dentro de un único adaptador.
...
- Violaciones de la documentación del modelo de confianza: código que se comporta
  contrariamente a lo que esta política, la propia documentación de 3V0 Agent o
  las expectativas razonables del operador predecirían — incluyendo casos donde
  3V0 Agent ha documentado una postura sobre cómo su salida debe ser
...
- **Configuraciones documentadas de emergencia.** Compensaciones seleccionadas por el operador
  ...
  la seguridad de 3v0-home, y similares. Los informes contra esas
...
- **Habilidades y plugins contribuidos por la comunidad.** Las habilidades de terceros
  ...
  están en la superficie de revisión del operador, no en la superficie de confianza de 3V0 Agent
  (§2.4, §2.5). Una habilidad o plugin que haga algo
  malicioso es el modo de falla esperado de uno que no fue
  revisado, no una vulnerabilidad en 3V0 Agent. Los errores en la ruta de
  instalación de habilidades o plugins de 3V0 Agent que impidan al
...
- 3V0 Agent incluye guardias de cadena de suministro para lanzamientos de servidores
