...
- `generate_video()` split: the Pollinations text-to-image call was removed
  (3V0 uses its own `image_generate` + `pixel_art()` pipeline for base
...
- Pollinations text-to-image generation (`pixelart_image.py`,
  `generate_base()` in `pixelart_video.py`) — 3V0 has `image_generate`
...
- License: MIT (inherits from 3v0-agent repo)
- Original author of the skill shell: dodo-reach
- Expansion with palettes + video: 3V0 Agent contributors
