...
### 1. Start order: editor first, 3V0 second

3V0 probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW 3V0 session.
...
restart the editor, restart the 3V0 session.
...
server fails to bind (Output Log) or 3V0' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.3V0/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for 3V0

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. 3V0' connection
lives in `~/.3V0/config.yaml` via `3v0 mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms 3V0 — just don't
mistake it for the 3V0 setup step.
...
explicitly warns against overlapping calls. 3V0 executes same-turn tool
...
### 9. Timeouts: 3V0 gives up before Unreal does

3V0' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If 3V0 runs elsewhere (SSH backend,
