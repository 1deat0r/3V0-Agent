...
Companion skill for the `unreal-engine` entry in the 3V0 MCP catalog. The
...
Two halves, in this order: the editor side must be up before 3V0 connects.
...
`~/.3V0/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for 3V0 — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. 3V0 connects
...
2. Start the 3V0 session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through 3V0
they appear as:
...
then happens at `3v0 mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — 3V0 runs batched calls
...
- **Start order matters.** Editor + server up first, then the 3V0
...
- **Timeouts on long operations.** 3V0' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.3V0/config.yaml` for
