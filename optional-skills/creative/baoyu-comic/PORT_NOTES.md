...
| Change | Upstream | 3V0 |
|--------|----------|--------|
| Metadata namespace | `openclaw` | `3v0` (with `tags` + `homepage`) |
| Trigger | Slash commands / CLI flags | Natural language skill matching |
| User config | EXTEND.md file (project/user/XDG paths) | Removed — not part of 3V0 infra |
...
| File operations | Generic instructions | 3V0 file tools (`write_file`, `read_file`) |
...
- **`scripts/` directory** (removed entirely): upstream's `merge-to-pdf.ts` depended on `pdf-lib`, which is not declared anywhere in the 3V0 repo. Rather than add a new dependency, the port drops PDF assembly and delivers per-page PNGs.
