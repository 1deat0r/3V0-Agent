#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `3v0 kanban list` and
...
def kanban_list(tenant: str) -> list[dict]:
    ...
    output isn't supported by the installed 3v0 CLI."""
    try:
        out = subprocess.run(
            ["3v0", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `3v0 kanban list`
    out = subprocess.run(
        ["3v0", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, encoding='utf-8', errors='replace', check=False,
    )
    rows = []
