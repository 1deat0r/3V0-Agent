...
the actual `setup.sh` that creates the project workspace, configures 3V0
...
2. **Create profiles** — `3v0 profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.3V0/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `3v0 kanban create` assigned to the director
...
```bash
3v0 profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.3V0/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.3V0/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most 3V0
...
Each profile gets a `SOUL.md` at `~/.3V0/profiles/<name>/SOUL.md` that
...
```bash
3v0 kanban create "Direct production of <video title>" \
    --assignee director \
...
the 3V0 `.env` (`${EV0_HOME:-$HOME/.3v0}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _ev0_env="${EV0_HOME:-$HOME/.3v0}/.env"
    if grep -q "^${var}=" "$_ev0_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_ev0_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_ev0_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY 3v0 ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY 3v0 OPENROUTER_API_KEY || exit 1
# ...
```
