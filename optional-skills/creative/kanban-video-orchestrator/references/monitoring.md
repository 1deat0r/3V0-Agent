...
3v0 kanban watch --tenant <project-slug>

# Snapshot of the board
3v0 kanban list --tenant <project-slug>
3v0 kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
3v0 kanban stats --tenant <project-slug>

# Visual dashboard (browser)
3v0 dashboard

# Inspect a specific task (includes comments + events)
3v0 kanban show <task-id>

# Follow a single task's event stream
3v0 kanban tail <task-id>
```

Verify available subcommands with `3v0 kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `3v0 kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `3v0 kanban block <id>` then `3v0 kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `3v0 kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `3v0 kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `3v0 profile list` (profile exists?), `3v0 status` (gateway/dispatcher up?), and verify tenant. |
...
NEW_TASK_ID=$(3v0 kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
3v0 kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
3v0 kanban block <task-id>
3v0 kanban archive <task-id>

# Diagnose what happened
3v0 kanban show <task-id>      # task body, comments, recent events
3v0 kanban tail <task-id>      # follow the live event stream
3v0 kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `3v0 kanban create` for the director
...
while true; do
    clear
    3v0 kanban list --tenant <slug>
    echo "---"
    3v0 kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `3v0 kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `3v0 profile list` and start
