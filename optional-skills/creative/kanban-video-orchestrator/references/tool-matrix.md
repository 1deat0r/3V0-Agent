...
Maps each role archetype to the 3V0 skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public 3v0-agent
...
## 3V0 skills relevant to video production

### Visual / rendering skills (`3v0-agent/skills/creative/`)
...
### Design / pre-production skills (`3v0-agent/skills/creative/`)
...
### Audio / media skills (`3v0-agent/skills/creative/` + `skills/media/`)
...
These are **not** 3V0 skills but external CLIs / APIs that profiles invoke.
...
## Built-in 3V0 tools for media review

These are native 3V0 tools — not invoked via terminal but through their own
...
key is present in `${EV0_HOME:-~/.3v0}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every 3V0 profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${EV0_HOME:-~/.3v0}/.env` → environment variable.
...
When a new 3V0-public video skill ships:
