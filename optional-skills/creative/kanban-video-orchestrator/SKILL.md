...
short to a music video to an ASCII loop — in a 3V0 Kanban pipeline that
...
3. **Generates** a setup script that creates 3V0 profiles, project workspace, and the initial kanban task
...
For mapping role → which 3V0 skills + toolsets it loads, see
...
3. Creates each 3V0 profile via `3v0 profile create --clone`
...
7. Fires the initial `3v0 kanban create` task assigned to the director
...
```bash
3v0 kanban watch --tenant <project-tenant>     # live events
3v0 kanban list  --tenant <project-tenant>     # board snapshot
3v0 dashboard                                   # visual board UI
```
...
6. **The director never executes.** Even with the full `kanban + terminal +
   ...
   a `3v0 kanban create` call to a specialist profile. The kanban
...
8. **Verify API keys BEFORE firing.** External APIs (TTS, image-gen,
   image-to-video) need keys in `${EV0_HOME:-~/.3v0}/.env` or the user's secret store.
