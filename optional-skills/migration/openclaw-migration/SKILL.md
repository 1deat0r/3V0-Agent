...
description: Import an OpenClaw setup (memories, skills) into 3V0.
version: 1.0.0
...
metadata:
  3v0:
    tags: [Migration, OpenClaw, 3V0, Memory, Persona, Import]
    related_skills: [3v0-agent]
---

# OpenClaw -> 3V0 Migration

Use this skill when a user wants to move their OpenClaw setup into 3V0 Agent with minimal manual cleanup.
...
```bash
3v0 claw migrate              # Full interactive migration
...
3v0 claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `3v0 setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_ev0.py` to:

- import `SOUL.md` into the 3V0 home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into 3V0 memory entries
- merge OpenClaw command approval patterns into 3V0 `command_allowlist`
- migrate 3V0-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to 3V0 working-directory configuration
- copy OpenClaw skills into `~/.3V0/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen 3V0 workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.3V0/tts/`
- archive non-secret docs that do not have a direct 3V0 destination
...
- `scripts/openclaw_to_ev0.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.3V0/skills/migration/openclaw-migration/scripts/openclaw_to_ev0.py`

Do not guess a shorter path like `~/.3V0/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.3V0/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of 3V0-compatible secrets, currently:
...
3V0 CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in 3V0. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a 3V0 workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing 3V0 skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python ~/.3V0/skills/migration/openclaw-migration/scripts/openclaw_to_ev0.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python /home/USER/.3V0/skills/migration/openclaw-migration/scripts/openclaw_to_ev0.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python ~/.3V0/skills/migration/openclaw-migration/scripts/openclaw_to_ev0.py --preset user-data
```

Execute a user-data migration:

```bash
python ~/.3V0/skills/migration/openclaw-migration/scripts/openclaw_to_ev0.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python ~/.3V0/skills/migration/openclaw-migration/scripts/openclaw_to_ev0.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python ~/.3V0/skills/migration/openclaw-migration/scripts/openclaw_to_ev0.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of 3V0 unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty 3V0 targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean 3V0 destination. Unsupported auth blobs must still be reported as skipped.
...
- 3V0 persona state imported
- 3V0 memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.3V0/skills/openclaw-imports/`
