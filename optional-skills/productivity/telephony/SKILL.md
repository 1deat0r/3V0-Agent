...
This optional skill gives 3V0 practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `${EV0_HOME:-~/.3v0}/.env`
...
It does **not** turn 3V0 into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to 3V0 memory
...
### 1) "I want 3V0 to own a real phone number"
Use **Twilio**.
...
- pairs well with 3V0 `text_to_speech` plus a public file host or tunnel
...
```bash
SCRIPT="$(find ~/.3V0/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
3v0 skills search telephony
3v0 skills install official/productivity/telephony
```
...
Then save credentials into 3V0:
...
3. Buy it and save it into `${EV0_HOME:-~/.3v0}/.env` + state:
```bash
python "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is 3V0 calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing 3V0's existing `text_to_speech` support.

Use this when:
- you want the call to use 3V0's configured TTS voice rather than Twilio `<Say>`
...
Recommended 3V0 TTS -> Twilio Play workflow:

1. Generate the audio with 3V0 `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask 3V0 to create the message audio with `text_to_speech`
...
Important note:
- 3V0 TTS is great for prerecorded outbound messages
...
- 3V0 STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to 3V0 memory.
...
- Do not store arbitrary third-party phone numbers in 3V0 memory.
...
3. persist that number to `${EV0_HOME:-~/.3v0}/.env`
