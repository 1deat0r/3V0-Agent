...
- Skill helper paths:
  - `${EV0_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${EV0_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${EV0_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client 3v0
```
...
```bash
PUBLISH="${EV0_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client 3v0
```
...
```bash
DRIVE="${EV0_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `3v0`)    |
