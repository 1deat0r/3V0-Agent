...
Invoke through the `terminal` tool from the target repo's root, then use `read_file` / `search_files` / `write_file` to produce the wiki. Default output location is `~/.3V0/wikis/<repo-name>/`. Only write into the repo (`docs/wiki/`) when the user explicitly requests it.
...
```bash
OUTPUT_DIR="$HOME/.3V0/wikis/$REPO_NAME"
...
1. **User input** — [`cli.py:Ev0CLI.run_session`](<link>)
...
cat > "$OUTPUT_DIR/.codewiki-state.json" <<EOF
{
  ...
  "generator": "3v0-agent code-wiki skill v0.1.0",
  "modules_documented": []
}
EOF
```
...
```
Generated wiki at ~/.3V0/wikis/<repo-name>/:
...
- **In-repo output without asking.** Default is `~/.3V0/wikis/`. Only write into the repo when the user explicitly requests it.
