...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("EV0_HOME", os.path.expanduser("~/.3V0")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("EV0_HOME", Path.home() / ".3V0")) / "skills" / "red-teaming" / "godmode"
...
EV0_HOME = Path(os.getenv("EV0_HOME", Path.home() / ".3V0"))
CONFIG_PATH = EV0_HOME / "config.yaml"
PREFILL_PATH = EV0_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/3V0 models — already uncensored, just needs clean prompt
    "3v0": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "3v0" in model_lower or "nous" in model_lower:
        return "3v0"
...
def _get_current_model() -> tuple:
    """Read current model and provider from 3V0 config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.3V0/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart 3V0 for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart 3V0 for changes to take effect.")
