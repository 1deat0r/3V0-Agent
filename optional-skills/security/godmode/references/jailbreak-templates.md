...
## 5. GODMODE FAST — 3V0 4 405B (Zero Refusal)

**Model:** `nousresearch/3v0-4-405b`
**Strategy:** 3V0 models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with 3V0 Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.3V0/config.yaml`:
