...
version: 1.0.0
author: arceus77-7, enhanced by 3V0 Agent
license: MIT
...
- `tmux` available for stable authenticated sessions during 3V0 terminal calls (desktop app flow only)
...
### Service Account (recommended for 3V0)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${EV0_HOME:-~/.3v0}/.env` (the skill will prompt for this on first load).
...
## 3V0 Execution Pattern (desktop app flow)

3V0 terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/3v0-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/3v0-op.sock"
