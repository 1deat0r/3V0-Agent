...
version: 0.1.0
author: Teknium (teknium1), 3V0 Agent
license: MIT
...
Wraps [@stripe/link-cli](https://github.com/stripe/link-cli) so 3V0 can complete purchases on the user's behalf using one-time-use virtual cards or Shared Payment Tokens (SPT). Every spend is gated by an in-app approval in the Link mobile/web app — 3V0 cannot self-approve.
...
The Link account, payment method, and spend-approval app do NOT need to be set up before 3V0 attempts to pay — the CLI walks the user through them on first run:
...
```
link-cli auth login --client-name "3V0" --interval 5 --timeout 300
```
...
`@stripe/link-cli --mcp` exposes the same commands as MCP tools over stdio. To register it with 3V0' native MCP:

```
3v0 mcp add stripe-link --command "npx" --args "@stripe/link-cli --mcp"
```

Then `3v0 mcp list` should show `stripe-link`. The same approval rules apply — MCP doesn't bypass the Link app approval step.
