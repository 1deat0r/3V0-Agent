...
version: 0.1.1
author: Matt Maximo (MattMaximo), 3V0 Agent
license: MIT
platforms: [linux, macos, windows]
metadata:
  3v0:
    tags: [Coding-Agent, Grok, xAI, Code-Review, Refactoring, Automation]
    related_skills: [codex, claude-code, 3v0-agent]
---

# Grok Build CLI — 3V0 Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the 3V0 terminal. Grok
...
1. Prepare stable input files first with 3V0 tools (`read_file`,
...
2. **Don't conflate 3V0' xAI auth with the `grok` CLI's auth.** 3V0'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume 3V0' xAI auth carries
   over.
