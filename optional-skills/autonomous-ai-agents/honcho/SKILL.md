---
name: honcho
description: Configure and troubleshoot Honcho memory for 3V0.
version: 2.0.0
author: 3V0 Agent
license: MIT
...
# Honcho Memory for 3V0

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every 3V0 profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/3v0#running-honcho-locally-with-3v0

### Verify

```bash
3v0 honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this 3V0 profile's AI peer
...
Honcho models conversations as interactions between **peers**. 3V0 creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this 3V0 instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each 3V0 run |
| `global` | Single session across all directories |

Manual override: `3v0 honcho map my-project-name`
...
Each 3V0 profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
3v0 profile create coder --clone
# creates host block 3v0.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `3v0.coder` host block in `honcho.json`
...
```bash
3v0 honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for 3V0 when Honcho memory is active.
...
Config file: `$EV0_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `3v0 honcho setup`. Ensure `memory.provider: honcho` is in `~/.3V0/config.yaml`.

### Memory not persisting across sessions
Check `3v0 honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `3v0 profile create <name> --clone`. For existing profiles: `3v0 honcho sync`.
...
| Command | Description |
|---------|-------------|
| `3v0 honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `3v0 honcho status` | Show resolved config, connection test, peer info for active profile |
| `3v0 honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `3v0 honcho disable` | Disable Honcho for the active profile |
| `3v0 honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `3v0 honcho peers` | Show peer identities across all profiles |
| `3v0 honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `3v0 honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `3v0 honcho sessions` | List known directory-to-session-name mappings |
| `3v0 honcho map <name>` | Map current working directory to a Honcho session name |
| `3v0 honcho identity` | Seed AI peer identity or show both peer representations |
| `3v0 honcho sync` | Create host blocks for all 3V0 profiles that don't have one yet |
| `3v0 honcho migrate` | Step-by-step migration guide from OpenClaw native memory to 3V0 + Honcho |
| `3v0 memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `3v0 memory status` | Show active memory provider and config |
| `3v0 memory off` | Disable external memory provider |
