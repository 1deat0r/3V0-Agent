...
version: 0.1.0
author: Tim Koepsel (xzessmedia), 3V0 Agent
license: MIT
platforms: [linux, macos]
metadata:
  3v0:
    tags: [Coding-Agent, OpenHands, Model-Agnostic, LiteLLM]
    related_skills: [claude-code, codex, opencode, 3v0-agent]
---
...
This skill is the headless-mode wrapper for batch / one-shot delegation. The interactive textual UI is not used from 3V0.
...
For Claude-native, prefer `claude-code`. For OpenAI-native, prefer `codex`. For 3V0-native subagents, use `delegate_task`.
...
- Sibling skills: `claude-code` (Anthropic-only), `codex` (OpenAI-only), `opencode` (multi-provider via OpenCode), `3v0-agent` (3V0 subagents via `delegate_task`).
