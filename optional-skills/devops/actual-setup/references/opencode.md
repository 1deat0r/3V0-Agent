...
is typically only in `~/.3V0/.env`, not a shell profile, so the env form
...
```
OpenCode DOES use the `provider/model` slash form on the CLI (unlike 3V0,
...
`3v0-custom-providers` skill, pitfall 2). 3V0 hits this because it forwards
