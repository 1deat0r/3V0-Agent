---
name: actual-setup
description: Set up Actual Computer (actual.inc) inference in 3V0.
version: 2.0.0
author: shl0ms + 3V0 Agent
license: MIT
...
Sets up [actual.inc](https://actual.inc) (Actual Computer) as a 3V0 inference
...
- User has an `ac_` key and wants 3V0 routed through their Actual cluster.
...
- 3V0 has **first-class `actual` provider support** (provider id `actual`,
  aliases `actual-computer`, `actualcomputer`, `aci`). Do NOT configure Actual
  as a `custom_providers` / `providers.actual.*` entry on current 3V0 — the
...
1. Put the key in `.env` (secrets only — never config.yaml):
   append `ACTUAL_API_KEY=ac_...` to `~/.3V0/.env`.
...
3. Select provider + model:
   ```bash
   3v0 config set model.provider actual
   3v0 config set model.default "MODEL_ID_FROM_DISCOVERY"
   ```
4. Verify end-to-end:
   ```bash
   3v0 chat -Q -q "Reply with exactly: ACTUAL_OK" --provider actual -m MODEL_ID
   ```
...
2. Download and load a model (scriptable once authorized):
   ...
   ```
3. Point 3V0 at the daemon. `ACTUAL_BASE_URL` with a loopback host flips the
   built-in provider into local no-auth mode automatically — no key needed:
   append `ACTUAL_BASE_URL=http://127.0.0.1:8080` to `~/.3V0/.env`, then:
   ```bash
   3v0 config set model.provider actual
   3v0 config set model.default "INSTALLED_MODEL_NAME"
   ```
4. Verify (reduced toolset — see context-window pitfall below):
   ```bash
   3v0 chat -Q -q "Reply with exactly: LOCAL_OK" --provider actual -m INSTALLED_NAME -t file,web
   ```
...
1. **reasoning_effort trap (handled by 3V0 since the first-class provider).**
   ...
   request still 400s this way on an old 3V0, set a per-model cap:
   `agent.reasoning_overrides.<model>: high` in config.yaml.
2. **Context-window overflow on small local models.** 3V0' default toolset
   ...
   servers emit a bare `data: [DONE]` — 3V0 reports
...
5. **Do not create a custom provider named `actual`.** Older setup guides
   ...
   current 3V0 the built-in provider wins the name; stale custom blocks
...
```bash
# Relay:
3v0 chat -Q -q "Reply with exactly: ACTUAL_OK" --provider actual -m MODEL
# Local (small model — reduced toolset):
3v0 chat -Q -q "Reply with exactly: LOCAL_OK" --provider actual -m MODEL -t file,web
