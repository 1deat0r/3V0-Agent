...
Usage (via cron with --no-agent):
    ...
    3v0 cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$EV0_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "3V0-Watcher/1.0"})
