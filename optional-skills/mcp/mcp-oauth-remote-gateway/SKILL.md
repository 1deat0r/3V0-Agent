...
metadata:
  3v0:
    ...
    related_skills: [3v0-agent, mcporter, fastmcp]
---

# MCP OAuth on a Remote 3V0 Gateway

## Overview

3V0' built-in MCP OAuth client runs a one-shot HTTP listener on `127.0.0.1:<port>`
inside the 3V0 process and registers that loopback address as the OAuth
`redirect_uri`. That works perfectly for a local CLI on the user's own machine.
It breaks completely when 3V0 runs as a remote gateway (container, VPS,
messaging bot), because the user's browser resolves `127.0.0.1` to the user's own
laptop, not the remote container — so the authorization code never reaches 3V0.

This skill does the OAuth dance by hand and writes the resulting tokens into the
exact files 3V0' token storage expects, so a subsequent `/reload-mcp` finds
...
2. 3V0 is running as a **remote gateway** (container, VPS, Docker, managed service) — NOT a local CLI on the user's laptop.
...
Do NOT use this for:
- **Local CLI 3V0** — just set `auth: oauth` in `mcp_servers.<name>` and `/reload-mcp`. The built-in flow opens a browser and captures the callback on localhost. Works perfectly.
...
laptop and fails to connect. The callback never reaches the 3V0 process, the
flow times out, and `/reload-mcp` returns "No MCP tools available" with no detail.

Symptoms to recognize: `[xdg-open] <defunct>` processes under the 3v0 user, an
empty or missing tokens directory (`$EV0_HOME/mcp-tokens/`), and a reload that
...
already cover the deployment. When 3V0 detects a remote session it prints two
...
Both require an interactive terminal to the 3V0 host. The rest of this skill
is for when there is NO interactive TTY — 3V0 running purely as a messaging
gateway/bot where `/reload-mcp` triggers the flow with nobody at a prompt.

## Preferred Front Door: the 3V0 Dashboard (try this BEFORE manual token surgery)

A remote 3V0 gateway often also runs the **dashboard** web UI as a SEPARATE
process (e.g. `3v0 dashboard --host 0.0.0.0 --port <port>`; check with
`ps aux | grep '3v0 dashboard'`). It exposes a connector/MCP console —
...
```bash
env | grep -iE "EV0_DASHBOARD_PUBLIC_URL|RAILWAY_PUBLIC_DOMAIN|RAILWAY_STATIC_URL|RAILWAY_SERVICE_.*_URL|PUBLIC_URL|BASE_URL|DOMAIN" \
  | sed -E 's/(TOKEN|SECRET|KEY|PASSWORD)=.*/\1=***REDACTED***/I'
```

`EV0_DASHBOARD_PUBLIC_URL` is authoritative when present. On Railway also check
...
across restarts) and anything reading credentials from `$EV0_HOME/.env`. Those
...
3V0' `Ev0TokenStorage` would have written, so on `/reload-mcp` 3V0 finds
cached tokens and skips the browser flow entirely.
...
```json
{
  "client_name": "3V0 Agent (manual OAuth)",
...
### 8. Write tokens in 3V0' exact schema

`tools/mcp_oauth.py::Ev0TokenStorage` expects two files under
`$EV0_HOME/mcp-tokens/` (create dir with `0o700`, files with `0o600`):
...
body = json.dumps({
    ...
    "params": {
        ...
        "capabilities": {},
        "clientInfo": {"name": "3v0-debug", "version": "1.0"},
    },
}).encode()
...
UA** — Cloudflare will 403 you even though 3V0 (which uses httpx) will succeed.
`scripts/diagnose-oauth-mcp.py` automates this smoke test.

### 11. Tell the user to run `/reload-mcp`

On reload, 3V0 sees `auth: oauth`, calls `Ev0TokenStorage.get_tokens()`,
...
1. **Do not assume "headless" means "OAuth impossible."** The built-in flow works fine for local CLI; the issue is strictly remote deployments where the user's browser and the 3V0 process are on different machines. Check the execution environment before claiming OAuth isn't an option.
...
11. **Client info file is NOT optional.** 3V0 needs `<server>.client.json` to know the `client_id` for refresh grants. Skipping it means the first refresh fails and the user has to re-auth — writing both files is the whole point of this skill.
...
- **Don't use `mcp-remote` as a fallback.** It runs an npx subprocess whose OAuth callback server ALSO sits on the remote container's localhost — same problem. `mcp-remote` only helps when the MCP client doesn't speak remote HTTP at all (3V0 does natively).
...
- **Don't claim 3V0 doesn't support a feature without reading the source.** Grep the source tree before making capability claims.
...
- `native-mcp` — general guide to configuring MCP in 3V0. Authoritative config reference lives there.
- `mcporter` — the external CLI bridge, for ad-hoc MCP calls outside of 3V0' config.
