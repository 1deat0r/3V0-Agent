...
metadata:
  3v0:
    ...
    related_skills: [3v0-agent, mcporter]
prerequisites:
  commands: [python]
---
...
- smoke-test a server with the FastMCP CLI before wiring it into 3V0 or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to 3V0. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.3V0/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.3V0/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is 3V0 integration, either:

- configure the server in `~/.3V0/config.yaml` using the `native-mcp` skill, or
...
### 3V0 cannot see the deployed server

The server-building part may be correct while the 3V0 config is not. Load the `native-mcp` skill and configure the server in `~/.3V0/config.yaml`, then restart 3V0.
