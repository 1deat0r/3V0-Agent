...
This is an optional third-party workflow, not a 3V0 core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with 3V0 native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer 3V0 native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer 3V0 native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
## Recommended 3V0 usage patterns
...
- Do not silently choose Parallel when 3V0 native tools are already sufficient.
