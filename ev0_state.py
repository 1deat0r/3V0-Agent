...
from ev0_state_common import (  # noqa: F401  (re-exported for back-compat)
    _BRANCH_CHILD_SQL,
...
from ev0_state_portability import SessionPortabilityMixin
...
# ev0_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def _apply_wal_size_limit(conn: sqlite3.Connection) -> None:
    ...
    ``ev0_cli/kanban_db.py`` already bounds its WAL growth with
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``ev0_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see ev0_cli/kanban_db.py) would
...
def apply_database_pragmas(
    ...
    * ``journal_size_limit`` — max journal/WAL size in bytes
    ...
    """
    try:
        # Local import avoids a circular import with ev0_cli.config.
...
def _backup_db_file(db_path: Path) -> "Tuple[Optional[Path], Optional[str]]":
    ...
    connection's POSIX advisory locks (see ``ev0_cli.sqlite_safe_read``).
...
def preflight_db_writability(
    ...
    *,
    ...
    to check). Shared by :class:`SessionDB` and ``ev0_cli.kanban_db``.
    """
    raw = str(db_path)
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    (scaffold/embed installs that ship ev0_state without ev0_cli). A
    ...
    try:
        ...
        logger.debug(
            "ev0_cli.sqlite_safe_read unavailable; opening %s untracked "
    ...
    # patch ev0_state.sqlite3.connect keep control of connection creation;
...
def is_zeroed_state_db(
    ...
    Prefer ``ev0_cli.backup.is_zeroed_sqlite_file`` when available; this
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            # ``ev0_state._set_last_init_error(None)`` explicitly.
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        if repair_alternation and messages:
            # Lazy import: ev0_state already depends on agent.* (see
