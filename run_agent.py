...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    ...
    launched from is meaningful for ``3v0 -c`` / ``--resume`` (relaunch
...
def _safe_session_filename_component(session_id: str) -> str:
    ...
    ``X-3V0-Session-Id`` API header) and are otherwise interpolated raw
    into on-disk artifact filenames under ``~/.3V0/sessions/``.  Without
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[3v0-agent: tool call arguments were corrupted in this session and "
    ...
    def _ensure_db_session(self) -> None:
        ...
        try:
            ...
            # `3v0 --resume`. set_session_yolo() no-ops on a missing row, so
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``3v0 chat -q``. In that mode,
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``EV0_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``EV0_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return env_float("EV0_API_TIMEOUT", 1800.0)
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``EV0_API_CALL_STALE_TIMEOUT`` env var
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        3v0-agent #21444 for the symptom history.  The upstream backend
        ...
        return (
            ...
            "See 3v0-agent#21444 for symptom history."
        )
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        ...
        SuperGrok subscribers can use 3V0 against xai-oauth.  Lots of
    ...
    def _summarize_api_error(error: Exception) -> str:
        ...
        while current is not None and id(current) not in seen:
            ...
            if any(
                ...
                return (
                    "3V0 can't reach the model provider. You may be offline. "
    ...
    def _redact_message_content(content):
        ...
        Respects ``EV0_REDACT_SECRETS`` via ``redact_sensitive_text`` —
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.3V0/sessions/session_{sid}.json``
        ...
        # untrusted input (X-3V0-Session-Id header) and must not escape
        ...
        try:
            ...
            for msg in messages:
                ...
                # output, or user paste. Respects EV0_REDACT_SECRETS via
    ...
    def interrupt(self, message: Optional[str] = None, *, hard_cancel: bool = False) -> None:
        ...
        # interrupt event rather than 3V0' per-thread flag.
    ...
    def redirect(self, text: str) -> bool:
        ...
        During a normal 3V0 model request this cancels only that request;
    ...
    def _file_mutation_verifier_enabled(self) -> bool:
        ...
        ``EV0_FILE_MUTATION_VERIFIER`` env var overrides config.  Exposed
        ...
        try:
            import os as _os
            env = _os.environ.get("EV0_FILE_MUTATION_VERIFIER")
            if env is not None:
    ...
    def _format_file_mutation_failure_footer(cls, failed: Dict[str, Dict[str, Any]]) -> str:
        ...
        (e.g. ``~/.3V0/config.yaml``) to a messaging channel (#35584).
        """
        if not failed:
            return ""
        lines = [
    ...
    def _turn_completion_explainer_enabled(self) -> bool:
        ...
        True).  ``EV0_TURN_COMPLETION_EXPLAINER`` env var overrides
        ...
        try:
            import os as _os
            env = _os.environ.get("EV0_TURN_COMPLETION_EXPLAINER")
            if env is not None:
                return env.strip().lower() not in {"0", "false", "no", "off"}
    ...
    def _format_turn_completion_explanation(
        ...
        if reason == "session_persistence_failed":
            ...
            if cause == "turn_lease":
                return (
                    prefix
                    + "the turn was stopped because another 3V0 process "
            ...
            if cause == "locked":
                return (
                    ...
                    "(another 3V0 process was writing to the state "
            ...
            if cause == "corrupt":
                return (
                    ...
                    "not help — run `3v0 doctor` to repair the state "
            ...
            return (
                ...
                "Check the state database health (`3v0 doctor`), then "
    ...
    def _touch_activity(
        ...
        *,
        ...
        process is a dispatcher-spawned worker (EV0_KANBAN_TASK set),
    ...
    def _capture_credits(self, http_response: Any) -> None:
        ...
        """
        # Dev test fixture (EV0_DEV_CREDITS_FIXTURE): inject a chosen notice state
        ...
        if _fixture is not None:
            ...
            logger.info(
                ...
                "(real headers bypassed — `echo clear` / unset EV0_DEV_CREDITS_FIXTURE to restore)",
        ...
        if _dev:
            # EV0_DEV_CREDITS: stream each capture to agent.log — watch live with
            # `3v0 logs -f` (grep 'credits ▸'). Dev-only; silent for normal users.
    ...
    def close(self) -> None:
        ...
        # a private embedded daemon when 3V0 YOLO selected unrestricted
    ...
    def _build_keepalive_http_client(base_url: str = "", *, verify: Any = True) -> Any:
        ...
        ``EV0_CA_BUNDLE`` settings.  It is passed on the client AND on
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``3v0 auth add xai-oauth``) whose tokens belong to
    ...
    def _try_refresh_env_client_credentials(self) -> bool:
        """Adopt ~/.3V0/.env credential/base-url edits at the turn boundary.

        A Settings save (desktop ``PUT /api/env``, ``3v0 setup``) updates
    ...
    def _compress_context(
        ...
        *,
        ...
        try:
            ...
            # EV0_SESSION_ID to the child id after an out-of-place
    ...
    def run_conversation(
        ...
        try:
            # Serialize the full load -> run -> flush region across 3V0
            ...
            if (
                ...
                def _on_session_turn_lease_wait(elapsed: float) -> None:
                    ...
                    if elapsed < 1.0:
                        self._emit_status(
                            "⏳ Another 3V0 process is using this session; "
                            "waiting for it to finish before starting your turn..."
                        )
                    else:
                        self._emit_status(
                            "⏳ Still waiting for the other 3V0 process on "
                ...
                if not _turn_db.acquire_session_turn_lease(
                    ...
                    if getattr(self, "_interrupt_requested", False):
                        ...
                        interrupt_msg = (
                            "Stopped waiting for another 3V0 process on "
                    ...
                    timeout_msg = (
                        "⏳ Another 3V0 process kept this session busy too "
