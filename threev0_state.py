...
from threev0_state_common import (  # noqa: F401  (re-exported for back-compat)
    _BRANCH_CHILD_SQL,
...
from threev0_state_portability import SessionPortabilityMixin
...
def _real_platform_state_root() -> Optional[Path]:
    ...
    Deliberately avoids ``Path.home()`` / ``threev0_constants``: tests
    routinely monkeypatch ``Path.home`` to a tempdir, and ``threev0_state``
...
# threev0_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def _apply_wal_size_limit(conn: sqlite3.Connection) -> None:
    ...
    ``threev0_cli/kanban_db.py`` already bounds its WAL growth with
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``threev0_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see threev0_cli/kanban_db.py) would
...
def apply_database_pragmas(
    ...
    * ``journal_size_limit`` — max journal/WAL size in bytes
    ...
    """
    try:
        # Local import avoids a circular import with threev0_cli.config.
...
def _backup_db_file(db_path: Path) -> "Tuple[Optional[Path], Optional[str]]":
    ...
    connection's POSIX advisory locks (see ``threev0_cli.sqlite_safe_read``).
...
def preflight_db_writability(
    ...
    *,
    ...
    to check). Shared by :class:`SessionDB` and ``threev0_cli.kanban_db``.
    """
    raw = str(db_path)
    ...
    try:
        ...
        for fts_table in ("messages_fts", "messages_fts_trigram", "messages_fts_cjk"):
            try:
                ...
                # (threev0_state.py:645-723). The supported degraded-runtime
                # path (SessionDB._is_fts5_unavailable_error + the
                # regression suite in tests/test_threev0_state.py:600-632)
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    (scaffold/embed installs that ship threev0_state without threev0_cli). A
    ...
    try:
        ...
        logger.debug(
            "threev0_cli.sqlite_safe_read unavailable; opening %s untracked "
    ...
    # patch threev0_state.sqlite3.connect keep control of connection creation;
...
def is_zeroed_state_db(
    ...
    Prefer ``threev0_cli.backup.is_zeroed_sqlite_file`` when available; this
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            # ``threev0_state._set_last_init_error(None)`` explicitly.
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        if repair_alternation and messages:
            # Lazy import: threev0_state already depends on agent.* (see
