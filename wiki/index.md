...
- `manifest.tsv` + `areas/*.md` — **generated** by `scripts/build_wiki.py --rebuild` (auto rows from docstrings, markdown headings, path rules + sibling relations; stale boilerplate is refreshed automatically on rebuild, manual rows never touched).
