...
has to be thrown away or summarized. 3V0 has always done this in one batch:
...
3V0 folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. 3V0's normal session flush is
...
With `micro_compact` unset or `false` 3V0 behaves exactly as it always has:
...
Defaults to `$EV0_HOME/logs/agent.log`. It reports passes, outcome mix, net
