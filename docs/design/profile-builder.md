...
Supersedes: PR #31781 (prompt_toolkit `3v0 profile wizard`)

## Why this, not the CLI wizard

PR #31781 added a keyboard-driven `3v0 profile wizard` in the terminal.
...
needs, and a profile is just a EV0_HOME directory — so the dashboard is the
right home for a full-featured builder, and it can reuse everything that
already exists.

A profile = a full `~/.3V0/profiles/<name>/` directory with its own:
...
| Model + Provider | ModelsPage | `_write_profile_model(profile_dir, …)` | yes — EV0_HOME override, already wired into create endpoint |
| MCPs | McpPage | `mcp_config._save_mcp_server` + `/api/mcp/catalog` | yes — wrap with EV0_HOME override |
...
### Seam #1 — hub-skill install cannot use the EV0_HOME override

`tools/skills_hub.py` binds `SKILLS_DIR = EV0_HOME / "skills"` at **module
import time**. The context-local `set_ev0_home_override()` swap (which makes
...
```python
_spawn_ev0_action(["-p", profile, "skills", "install", identifier], "skills-install")
```

A fresh subprocess re-imports `skills_hub` with the profile's EV0_HOME bound
...
spawned detached (`_spawn_ev0_action` returns a PID immediately). So the
create flow is:

1. `create_profile()` — make the dir (synchronous)
2. write model (synchronous, EV0_HOME override)
3. write selected MCP servers (synchronous, EV0_HOME override)
4. seed/enable selected built-in + optional skills (synchronous)
5. spawn `3v0 -p <profile> skills install <id>` per hub skill (async, returns PIDs)
...
class ProfileCreate(BaseModel):
    ...
    mcp_servers: List[MCPServerCreate] = []      # synchronous, EV0_HOME override
...
page afterward. Mirror `_write_profile_model`'s EV0_HOME-override helper for
...
- Backend E2E with isolated EV0_HOME: POST a full create body
