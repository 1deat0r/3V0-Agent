...
3V0 supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.3V0/config.yaml`:
...
Or set the env var: `EV0_KANBAN_DISPATCH_IN_GATEWAY=false`
