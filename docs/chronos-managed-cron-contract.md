...
Chronos lets a hosted 3V0 gateway **scale to zero** while idle and still
...
|---|---|---|---|
| 1 | agent → NAS (`provision`/`cancel`/`list`) | the agent's existing **Nous Portal access token** (Bearer) — for a hosted agent this is the **bootstrap-session token** NAS planted in `auth.json` (client `3v0-cli-vps`), NOT an `agent:*` client token | NAS (its normal agent-token path) |
...
> minted under the bootstrap-only client `3v0-cli-vps` and seeded into the
...
1. **Dashboard app** (`threev0_cli/web_server.py`) — the agent's only public
...
- **Multi-replica agents:** the store CAS makes the fire at-most-once across N
  gateway replicas sharing one `EV0_HOME` — exactly one replica runs each
  fire.
