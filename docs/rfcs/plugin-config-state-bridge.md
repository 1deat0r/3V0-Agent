...
or change arbitrary 3V0 configuration.
...
small host fact requested by the RFC. Settings writes use 3V0'
...
`EV0_HOME`, so one globally loaded plugin context follows multiplexed
...
```text
<EV0_HOME>/plugin-data/<plugin-data-namespace>/state.json
```
...
The implementation is covered with real temporary-3V0-home tests for:
