...
`3v0 config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.3V0/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `EV0_HOME`, secret scope, and
