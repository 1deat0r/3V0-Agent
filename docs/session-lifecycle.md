...
The canonical transcript store is SQLite via `SessionDB` (from `threev0_state`). The
...
Counts consecutive restarts via a JSON file (`{EV0_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `3v0 update`, `3v0 gateway restart`,
or `/restart`.
