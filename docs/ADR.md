...
`get_ev0_home() / "plugins"`, and context-engine plugins (e.g.
`3v0-lcm`) capture profile-scoped state — such as the LCM database
path — at registration time. A single-slot cache meant:

1. Switching homes via `set_ev0_home_override()` was invisible to a
   naive "did `EV0_HOME` change" check, so the singleton silently kept
...
2. Even when a fresh `PluginManager` *was* created for a new home, plugin
   modules are imported into `sys.modules` as `ev0_plugins.<slug>` by
   ...
   `ev0_plugins.<slug>.<submodule>`, and Python's import machinery
...
- Replace the single-slot singleton with a cache keyed on the *resolved*
  3V0 home path (`_plugin_managers_by_home: Dict[Path, PluginManager]`).
  `get_plugin_manager()` resolves the current home via `get_ev0_home()`
  (which itself already consults `get_ev0_home_override()` before
...
- Per-profile LCM instances (and any other context-engine plugin) use
  ...
  through `EV0_HOME` or `set_ev0_home_override()`.
...
- Regression coverage exercises the real production path
  (`set_ev0_home_override()`) rather than only the env-var path, and
