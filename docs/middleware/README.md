...
3V0 middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing 3V0'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before 3V0 calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving 3V0 retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving 3V0 guardrails,
...
- `telemetry_schema_version`: currently `3v0.observer.v1`
- `middleware_schema_version`: currently `3v0.middleware.v1`
...
3V0 stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, 3V0 runs
them as a nested chain in registration order. Middleware failures are fail-open:
3V0 logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, 3V0 applies middleware in this order:
...
For each tool call, 3V0 applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal 3V0 pre-execution path against the effective arguments:
...
```bash
3v0 plugins enable <plugin-name>
```

For isolated local testing, use one `EV0_HOME` for plugin enablement and the
agent run:

```bash
export EV0_HOME=/tmp/3v0-middleware-test
mkdir -p "$EV0_HOME"
3v0 plugins enable <plugin-name>
3v0 chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run 3v0 plugins enable <plugin-name>
uv run 3v0 chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["ev0_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/3v0-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape 3V0 expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, 3V0 treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, 3V0 preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. 3V0 does not convert downstream
