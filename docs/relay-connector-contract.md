...
This document is the formal interface between the **3V0 gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `3v0 gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`3v0 gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `3v0` CLI.
