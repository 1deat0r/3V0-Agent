...
When running 3V0 inside Docker, the default `network_mode: host` gives the
...
The 3V0 [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   EV0_UID=$(id -u) EV0_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=3v0,3v0-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://3v0-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — 3V0 trust model and vulnerability reporting
