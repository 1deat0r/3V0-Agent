...
3V0 observer hooks are the read-only telemetry contract for plugins that
...
They do not replace 3V0' planner, model providers, memory, tool registry,
...
3V0 also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "3v0.observer.v1"
```

Hook callbacks are fail-open. 3V0 catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so 3V0 only builds
