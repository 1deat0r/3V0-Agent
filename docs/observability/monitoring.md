...
3V0 gateway daemon, exported over OTLP/HTTP to an operator-configured
...
(`plugins/observability/nemo_relay/`) and its 3V0-owned subscribers.

## What gets exported

| Signal | OTLP route | Content |
| --- | --- | --- |
| Gateway gauges | `/v1/metrics` | `3v0.gateway.up/state/busy/drainable/active_agents/background_work/background_delegations/restart_requested`, `3v0.platform.up/degraded` with bounded `error_code` attributes |
...
`3v0.gateway.active_agents`, `3v0.gateway.background_work`, and
`3v0.gateway.background_delegations` are complementary. `active_agents`
...
```bash
3v0 monitoring status
```

The OpenTelemetry SDK is an optional extra (`pip install '3v0-agent[otlp]'`),
...
a plain `3v0 gateway run`: the exporter lives in the gateway process, so
...
`3v0.gateway.up`, `3v0.platform.up`, and `3v0.platform.degraded`.
...
absent_over_time(ev0_gateway_up[5m])

# Locally owned bridge is explicitly down.
ev0_platform_up == 0

# Scheduler thread is stale even though the gateway may still be alive.
ev0_cron_scheduler_heartbeat_age_seconds > 180

# Ticker loops but has not completed a successful tick recently.
ev0_cron_scheduler_last_success_age_seconds > 300

# One or more jobs are beyond their existing scheduler grace window.
ev0_cron_jobs_overdue > 0

# Catch-up counter increased, proving at least one stale occurrence was
# collapsed and run once after a delay.
increase(ev0_cron_scheduler_catch_up_occurrences[15m]) > 0
```

Cron execution lifecycle records arrive as `3v0.cron_execution` spans.
Alert or derive events from bounded attributes such as:

```text
3v0.status = failed|unknown
3v0.delivery_outcome = failed|not_configured
3v0.error_class = auth_failed|rate_limited|timeout|network_error|
...
3. a cron lifecycle feed keyed only by opaque `3v0.job_key`;
...
3V0 Agent-owned Relay transport health remains in scope. A separate gateway
...
python scripts/observability/otel_capture_collector.py \
  --host 127.0.0.1 --port 4318 --log /tmp/ev0_otel_capture.jsonl
...
python scripts/observability/gateway_health_export_probe.py \
  ...
  --log /tmp/ev0_otel_capture.jsonl --wait 8
...
`3v0.error_class = ...` list in this file's alert section and the enum's unit
...
The `3v0 monitoring` CLI intentionally exposes `status` only. This first
release covers only 3V0 Agent-owned service-health and operational-diagnostic
signals, including 3V0 Agent-owned Relay transport health. Team Gateway's
