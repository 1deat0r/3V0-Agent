...
3V0 includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into 3V0 and does not require `3v0 plugins enable
observability/nemo_relay`. 3V0 remains importable without Relay on other
native targets. Those targets use an explicit reduced-capability no-op host:
3V0 execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `3v0-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

3V0 requires NeMo Relay 0.6.0 or later within the 0.6 release line. That
...
3V0 installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the 3V0 process so configured interceptors can
...
Collection remains off unless 3V0 policy enables it:
...
```text
3V0 turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> 3V0 shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

3V0 sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that 3V0 used for the logical call, such as
...
New calls use `3v0.model_route.count`. The previous
`3v0.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `3v0.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; 3V0
version, OS family, architecture, and install method remain bounded package
resources. Concurrent 3V0 processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `3v0.task_run`, parented to
the owning 3V0 session. The start counter contains only bounded execution
...
provider attempts for the same 3V0 API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their 3V0 tool-call
...
cancellations. Active task ownership follows the task ID if 3V0 rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`3v0.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. 3V0
...
tool or plugin names. 3V0 does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `3v0.tool_approval` marks and
...
Successful skill mutations emit `3v0.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `3v0.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. 3V0 derives reuse and patch-generation
...
```text
$EV0_HOME/telemetry/shared_metrics/metrics.sqlite3
$EV0_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the 3V0 version,
...
schema field name, its current scope is one `EV0_HOME`, so it is more
...
Deleting `$EV0_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `EV0_HOME`. To reset it, stop 3V0
processes and remove `$EV0_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real 3V0 CLI turn against the deterministic local model server:
