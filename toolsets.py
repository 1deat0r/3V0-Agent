...
_EV0_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (EV0_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "or XAI_API_KEY). Off by default; enable in `3v0 tools` → "
            "X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (EV0_KANBAN_TASK env "
    ...
    # Coding posture (base 3V0 — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in threev0_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full 3V0 toolsets (CLI + messaging platforms)
    #
    ...
    # the `3v0 send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "3v0-cli": {
        ...
        "tools": _EV0_CORE_TOOLS,
        "includes": []
    },

    "3v0-cron": {
        # Mirrors 3v0-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `3v0 tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as 3v0-cli; gated by `3v0 tools`",
        "tools": _EV0_CORE_TOOLS,
        "includes": []
    },
    ...
    "3v0-email": {
        "description": "Email bot toolset - interact with 3V0 via email (IMAP/SMTP)",
        "tools": _EV0_CORE_TOOLS,
        "includes": []
    },
    ...
    "3v0-sms": {
        "description": "SMS bot toolset - interact with 3V0 via SMS (Twilio)",
        "tools": _EV0_CORE_TOOLS,
        "includes": []
    },
    ...
    "3v0-gateway": {
        ...
        "tools": [],
        "includes": ["3v0-telegram", "3v0-discord", "3v0-whatsapp", "3v0-slack", "3v0-signal", "3v0-bluebubbles", "3v0-homeassistant", "3v0-email", "3v0-sms", "3v0-mattermost", "3v0-matrix", "3v0-dingtalk", "3v0-feishu", "3v0-wecom", "3v0-wecom-callback", "3v0-weixin", "3v0-qqbot", "3v0-webhook", "3v0-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``3v0-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_EV0_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``3v0-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (3v0-<name>).
        # Gives them _EV0_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("3v0-"):
            platform_name = name[len("3v0-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_EV0_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
