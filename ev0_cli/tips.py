...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or 3v0 -c.",
    ...
    "Ctrl+Z suspends 3V0 to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "3v0 -c resumes your most recent CLI session. 3v0 -c \"project name\" resumes by title.",
    "3v0 -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "3v0 -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "3v0 chat -t web,terminal enables only specific toolsets for a focused session.",
    "3v0 chat -s github-pr-workflow preloads a skill at launch.",
    "3v0 chat -q \"query\" runs a single non-interactive query and exits.",
    "3v0 chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "3v0 chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "3v0 --yolo bypasses all dangerous command approval prompts for the entire session.",
    "3v0 chat --source telegram tags the session for filtering in 3v0 sessions list.",
    "3v0 -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "3v0 doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "3v0 dump outputs a compact setup summary — great for bug reports.",
    "3v0 config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "3v0 config edit opens config.yaml in your default editor.",
    "3v0 config check scans for missing or stale configuration options.",
    "3v0 sessions browse opens an interactive session picker with search.",
    "3v0 sessions stats shows session counts by platform and database size.",
    "3v0 sessions prune --older-than 30 cleans up old sessions.",
    "3v0 skills search react --source skills-sh searches the skills.sh public directory.",
    "3v0 skills check scans installed hub skills for upstream updates.",
    "3v0 skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "3v0 skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "3v0 mcp add github --command npx adds MCP servers from the command line.",
    "3v0 mcp serve runs 3V0 itself as an MCP server for other agents.",
    "3v0 auth add lets you add multiple API keys for credential pool rotation.",
    "3v0 completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "3v0 logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "3v0 backup creates a zip backup of your entire 3V0 home directory.",
    "3v0 profile create coder creates an isolated profile that becomes its own command.",
    "3v0 profile create work --clone copies your current config and keys to a new profile.",
    "3v0 update syncs new bundled skills to ALL profiles automatically.",
    "3v0 gateway install sets up 3V0 as a system service (systemd/launchd).",
    "3v0 memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "3v0 webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: 3v0 tools disables unused tools, 3v0 skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "3v0 models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as 3v0 -w).",
    ...
    "EV0_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call 3V0 tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — '3v0 profile create coder' creates the 'coder' command.",
    "3v0 profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, 3V0 prints a resume command with session ID and stats.",
    "3v0 sessions export backup.jsonl exports all sessions for backup or analysis.",
    "3v0 -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "3v0 skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .3V0/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: 3v0 cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "3V0 runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "3v0 gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to 3V0 via a local macOS server.",
    ...
    "Context files (.3v0.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "3V0 loads project context from .3v0.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "3v0 mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "3v0 mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "3v0 mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — 3v0 tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.3V0/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "3v0 chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "3v0 plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "3v0 acp runs 3V0 as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "EV0_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "3v0 auth add nous or 3v0 auth add openai-codex sets up OAuth-based providers.",
    ...
    "3v0 status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.3V0/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make 3V0 your own.",
    ...
    "Before a gateway session reset, 3V0 auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.3V0/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope 3V0 state via EV0_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "EV0_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in EV0_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.3V0/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — '3v0 profile create ls' would be rejected.",
    "3v0 profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "3v0 claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "EV0_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via 3v0 tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.3V0/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.3V0/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — 3V0 auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of 3V0 config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while 3V0 is working.',
    ...
    '/reload reloads ~/.3V0/.env into the running session — pick up new API keys without restarting.',
    ...
    "EV0_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.3V0/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.3V0/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    '3v0 curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "3v0 curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    '3v0 curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    '3v0 auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'EV0_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "EV0_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    '3v0 dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    '3v0 dashboard embeds the full 3V0 TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.3V0/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.3V0/dashboard-plugins/ — no npm build required.',
    ...
    'EV0_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs concise/all/error/off).',
    'EV0_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'EV0_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'EV0_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.3V0/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.3V0/skills/ so drop-in skills appear without restarting the session.',
    ...
    '3v0 -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    '3v0 chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    '3v0 chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    '3v0 chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "3v0 chat --source tool tags programmatic chats so they don't clutter 3v0 sessions list.",
    '3v0 dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    '3v0 sessions rename <ID> "new title" renames any past session; 3v0 sessions delete <ID> removes one.',
    '3v0 import restores a full 3V0 backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    '3v0 fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    '3v0 pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    '3v0 setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    '3v0 status --deep runs the full health sweep across every component; plain 3v0 status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'EV0_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'EV0_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.3V0/plugins/ — trust-gated by design.',
    "EV0_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'EV0_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'EV0_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'EV0_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'EV0_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'EV0_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'EV0_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'EV0_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'EV0_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'EV0_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'EV0_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes 3V0 block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from 3v0 sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.3V0/dashboard-plugins/.',
]
