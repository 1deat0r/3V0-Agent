...
``ev0_cli.config.migrate_config``. Each step is a function
...
``ev0_cli.config`` imports :func:`run_migrations` lazily (inside
``migrate_config``), and every step function here resolves its helpers
(``read_raw_config``, ``_persist_migration``, ``get_env_value``, …) lazily
through the live ``ev0_cli.config`` module object at call time via
:func:`_cfg`. There is deliberately NO module-level import of
``ev0_cli.config`` here, so no circular import can form — and, just as
importantly, tests that monkeypatch helpers on ``ev0_cli.config`` (e.g.
``patch("ev0_cli.config.read_raw_config", ...)``) keep working, because
...
def _cfg():
    """Return the live ``ev0_cli.config`` module (lazy, cycle-free)."""
    from ev0_cli import config
