...
def _split_line(line: str) -> list[str]:
    try:
        ...
        from ev0_cli._subprocess_compat import split_command_line
...
def _dispatch_extracted_subcommand(
    *,
    ...
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    main_module = importlib.import_module("ev0_cli.main")
    builder = getattr(module, builder_name)
...
class Ev0ConsoleEngine:
    ...
    def _register_broad_cli_surface(self) -> None:
        ...
        extracted = {
            ...
            "prompt-size": (
                "ev0_cli.subcommands.prompt_size",
            ...
            "import": (
                "ev0_cli.subcommands.import_cmd",
        ...
        self.register(
            ...
            _adder_handler("send", (), "ev0_cli.send_cmd", "register_send_subparser"),
            mutating=True,
        ...
        _register_command_family(
            ...
            root="portal",
            paths=portal_paths,
            summaries=_adder_summaries("ev0_cli.portal_cli", "add_parser"),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("ev0_cli.projects_cmd", "build_parser"),
            mutating=[
                ("create",),
                ("add-folder",),
                ("remove-folder",),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("ev0_cli.kanban", "build_parser"),
            mutating=[
                ("init",),
                ("boards", "create"),
                ("boards", "rm"),
...
def _logs(_engine: Ev0ConsoleEngine, args: list[str]) -> str:
    ...
    from ev0_cli.logs import list_logs, tail_log
...
def _config_show(_engine: Ev0ConsoleEngine, args: list[str]) -> str:
    ...
    from ev0_cli.config import show_config
...
def _config_set(_engine: Ev0ConsoleEngine, args: list[str]) -> str:
    ...
    key = args[0]
    value = " ".join(args[1:])
    from ev0_cli.config import set_config_value
...
def _config_migrate(_engine: Ev0ConsoleEngine, args: list[str]) -> str:
    ...
    def _run() -> None:
        from ev0_cli.config import migrate_config
...
def _sessions_repair(_engine: Ev0ConsoleEngine, args: list[str]) -> str:
    ...
    def _run() -> None:
        from ev0_state import DEFAULT_DB_PATH, _db_opens_cleanly, repair_state_db_schema
...
def _profile_status(_engine: Ev0ConsoleEngine, args: list[str]) -> str:
    ...
    return _dispatch_extracted_subcommand(
        ...
        fixed=(),
        args=(),
        module_name="ev0_cli.subcommands.profile",
...
def _cron_list(_engine: Ev0ConsoleEngine, args: list[str]) -> str:
    ...
    ns = parser.parse_args(args)
    from ev0_cli.cron import cron_list
...
def _cron_status(_engine: Ev0ConsoleEngine, args: list[str]) -> str:
    ...
    from ev0_cli.cron import cron_status
