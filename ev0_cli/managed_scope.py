...
This is DISTINCT from ``ev0_cli.config.is_managed()`` / ``EV0_MANAGED``,
...
def apply_managed_overlay(config: dict) -> dict:
    ...
    (rather than going through ev0_cli.config.load_config) to honor managed
    scope. Mirrors ev0_cli.config._load_config_impl's managed merge exactly:
    ...
    try:
        ...
        from ev0_cli.config import _deep_merge, _expand_env_vars, _normalize_root_model_keys
