...
The 3V0 docs site hosts a JSON manifest of curated models for providers
...
1. ``get_catalog()`` — returns a parsed manifest dict.
   ...
   - Reads disk cache at ``~/.3V0/cache/model_catalog.json``.
...
DEFAULT_CATALOG_URL = (
    "https://github.com/1deat0r/3V0-Agent/docs/api/model-catalog.json"
)
...
DEFAULT_CATALOG_FALLBACK_URLS: tuple[str, ...] = (
    "https://raw.githubusercontent.com/NousResearch/3v0-agent/main/website/static/api/model-catalog.json",
)
DEFAULT_TTL_HOURS = 1
...
def get_default_model_from_cache(provider: str) -> str | None:
    ...
    ``"default": true``; that entry is the model 3V0 silently lands on when
    ...
    network-free. The cache is kept fresh by the picker/`3v0 update` paths;
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    ``3v0 update`` pulls the latest repo, so the freshly-pulled
    ...
    to ``~/.3V0/cache/model_catalog.json`` via the same atomic writer the
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``3v0 model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
