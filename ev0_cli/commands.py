...
class CommandDef:
    ...
    # Names a key in ``ev0_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``ev0_cli.slash_exec.run_execute`` and apply only
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`ev0_cli.plugins.PluginContext.register_command`. They behave
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from ev0_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
    ...
    def _personality_completions(sub_text: str, sub_lower: str):
        """Yield completions for /personality via ev0_cli.personality."""
        try:
