...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    if sys.platform == "darwin":
        ...
        # ``uninstall_gui``.
        from ev0_cli.linux_desktop_entry import desktop_entry_path
...
def uninstall_gui(hermes_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``hermes-agent/ev0_cli`` (agent source), ``venv/``, or any
    ...
    if sys.platform.startswith("linux"):
        ...
        try:
            from ev0_cli.linux_desktop_entry import (
                desktop_entry_path,
