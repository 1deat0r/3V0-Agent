"""Dependency-light venv recovery that runs BEFORE ev0_cli.main's imports.

The ``3v0`` console entry point is ``ev0_cli.main:main``.  Importing
``ev0_cli.main`` pulls in third-party packages at module level (``dotenv``
via ``ev0_cli.env_loader``, ``yaml`` via ``ev0_cli.config``, ...).  In
...
a corrupted venv.  ``ev0_cli.main`` imports and calls
:func:`recover_if_needed` at the very top of its module body, before any
third-party import.

Scope: this early pass only repairs enough for ``ev0_cli.main`` to become
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``ev0_cli.main``
...
def recover_if_needed(
    ...
    """Repair wiped core packages so ``ev0_cli.main`` can import at all.
...
def _complete_pending_core_install(root: Path, core_marker: Path) -> bool:
    ...
    ``ev0_cli.main``'s third-party imports, maps nothing yet, so the
    ...
    """
    try:
        from ev0_cli import _install_repair as ir
