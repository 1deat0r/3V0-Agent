...
the existing module-level functions in ev0_cli.gateway and
ev0_cli.gateway_windows directly. This protocol is a thin facade
...
# in ``ev0_cli.gateway`` (systemd/launchd) and ``ev0_cli.gateway_windows``
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in ev0_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from ev0_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from ev0_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from ev0_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from ev0_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in ev0_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from ev0_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from ev0_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from ev0_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from ev0_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``ev0_cli.gateway_windows`` (Scheduled Task /
    ...
    def is_running(self, name: str) -> bool:
        from ev0_cli import gateway_windows
        from ev0_cli.gateway import find_gateway_pids
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        ``ev0_cli.gateway`` maps that empty string to the
        ...
        allocator in ``ev0_cli.profiles._allocate_gateway_port``).
