"""Migrate 3V0' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads 3V0' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  3V0 mcp_servers.<n>.command/args/env  → codex stdio transport
  3V0 mcp_servers.<n>.url/headers       → codex streamable_http transport
  3V0 mcp_servers.<n>.timeout           → codex tool_timeout_sec
  3V0 mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  3V0-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. 3V0' own
...
MIGRATION_MARKER = (
    "# managed by 3v0-agent — `3v0 codex-runtime migrate` regenerates this section"
)
MIGRATION_END_MARKER = (
    "# end 3v0-agent managed section"
)
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in 3V0 config.")
...
# 3V0 keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # 3V0' sampling subsection — codex MCP has no equivalent
    "sampling",
}
...
def _translate_one_server(
    ...
    for key in ev0_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            ...
            skipped.append(f"{key} (unknown 3V0 key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (EV0_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by 3V0)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert 3V0' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before 3V0' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior 3V0
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="3v0-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a EV0_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed 3v0-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.3V0`` symlink that happens to live under ``/private/var/folders``
...
def _build_ev0_tools_mcp_entry() -> dict:
    ...
    so a 3v0 installed under /opt/, /usr/local/, or a venv all work.
    EV0_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # EV0_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_ev0_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever EV0_HOME its launcher sets
    ...
    # — that would override the launcher's EV0_HOME and pin the subprocess
    ...
    env["EV0_QUIET"] = "1"
    env["EV0_REDACT_SECRETS"] = env.get("EV0_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    Args:
        ev0_config: full ~/.3V0/config.yaml dict
    ...
    # Inject 3V0' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into 3V0 for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/ev0_tools_mcp_server.py
