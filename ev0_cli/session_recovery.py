...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    locks -- the failure class ``ev0_cli.sqlite_safe_read`` exists to
    ...
    keeps this path consistent with ``ev0_state._backup_db_file``.
    """
    from ev0_cli.sqlite_safe_read import LiveConnectionError, offline_file_access
