...
``CommandDef.execute`` (ev0_cli/commands.py) names a key in
...
for a fixed context (enforced by tests/ev0_cli/test_commands_execute.py).

Import discipline: this module imports nothing heavy at module level and
``ev0_cli.commands`` does NOT import this module (the ``execute`` field is
...
def _exec_version(ctx: CommandContext) -> CommandReply:
    ...
    from ev0_cli.banner import format_banner_version_label
