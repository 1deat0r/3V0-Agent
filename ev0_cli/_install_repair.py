...
- ``ev0_cli._early_recovery.recover_if_needed`` — stdlib-only, runs BEFORE
  ``ev0_cli.main``'s third-party imports, so it can complete a pending
  update while no native extension is mapped yet (#83569).
- ``ev0_cli.main._recover_core_update_marker_locked`` — the historical
...
the corrupted-venv state it exists to repair. ``ev0_cli.main`` imports
``managed_uv``, ``ev0_constants``, and friends only in its late path; the
...
:func:`ev0_cli._early_recovery._find_uv_binary` lookup and falls back to
...
from ev0_cli import _early_recovery as _er
...
def _is_termux_env(env: dict | None = None) -> bool:
    """Stdlib Termux probe (ev0_cli.main's version lives behind imports)."""
...
def _venv_scripts_dir(root: Path) -> Path | None:
    ...
    if not venv_dir.is_dir():
        return None
    # ev0_constants is stdlib-only, so the canonical layout helper is safe
