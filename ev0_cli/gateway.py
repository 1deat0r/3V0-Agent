...
# when ev0_constants is cached from a pre-update version during `3v0 update`.
...
def _scan_gateway_pids(
    ...
    try:
        if is_windows():
            ...
            # watchdog probe.
            from ev0_cli._subprocess_compat import bounded_probe_run
...
def find_profile_gateway_processes(
    ...
    try:
        from gateway.status import get_running_pid
        from ev0_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "ev0_cli.main"]
    if profile != "default":
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m ev0_cli.main gateway
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    from ev0_cli._subprocess_compat import (
    ...
    if sys.platform == "win32":
        try:
            from ev0_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # base interpreter needs to import ev0_cli.  Empty on POSIX, where
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from ev0_cli.service_manager import detect_service_manager, get_service_manager
...
def _gateway_list() -> None:
    ...
    """
    try:
        from ev0_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans(extra_exclude: set | None = None) -> bool:
    ...
    in-process under a ``gateway restart`` argv (ev0_cli/gateway.py restart
    ...
    if is_windows():
        try:
            ...
            from ev0_cli.gateway_windows import get_task_name
...
from ev0_constants import is_container, is_termux, is_wsl
...
def _windows_gateway_breakaway_state() -> bool | None:
    ...
    if not is_windows():
        return None
    from ev0_cli._subprocess_compat import _WINDOWS_GATEWAY_BREAKAWAY_ENV
...
_LEGACY_UNIT_EXECSTART_MARKERS: tuple[str, ...] = (
    "ev0_cli.main gateway",
    "ev0_cli/main.py gateway",
    "gateway/run.py",
    " 3v0 gateway ",
    "/3v0 gateway ",
)
    ...
    try:
        ...
        if not _is_service_installed():
            ...
            if supports_systemd:
                ...
                from ev0_cli import gateway_windows
...
def get_python_path() -> str:
    ...
    if venv is not None:
        try:
            ...
            # ev0_constants cached from before this symbol existed. See
            # _reload_ev0_constants() in ev0_cli/managed_uv.py.
            from ev0_cli.managed_uv import _reload_ev0_constants
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m ev0_cli.main``, so module
...
Group={group_name}
ExecStart={python_path} -m ev0_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}ExecStart={python_path} -m ev0_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="EV0_HOME={ev0_home}"
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m ev0_cli.main [--profile X] gateway run --replace` argv.
    ...
    """
    cmd = [get_python_path(), "-m", "ev0_cli.main"]
...
def generate_launchd_plist() -> str:
    ...
    nofile_block = ""
    try:
        from ev0_cli.resource_limits import configured_nofile_soft_limit
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    try:
        ...
        from ev0_cli.profiles import normalize_profile_name
...
def _setup_standard_platform(platform: dict):
    ...
    from ev0_cli.setup_hidden_env import is_setup_hidden_env as _is_setup_hidden_env
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from ev0_cli.telegram_managed_bot import (
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``ev0_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from ev0_cli import setup as _s
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`ev0_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from ev0_cli.service_manager import (
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            ...
            from ev0_cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
