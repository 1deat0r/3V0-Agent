...
def resolve_ev0_bin() -> Optional[str]:
    ...
    Priority:
      ...
      3. ``None`` → caller should fall back to ``python -m ev0_cli.main``.
    ...
    ``3v0.exe`` on PATH or the ``sys.executable -m ev0_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
...
def build_relaunch_argv(
    ...
    *,
    ...
    if bin_path:
        ...
        argv = [sys.executable, "-m", "ev0_cli.main"]
...
def relaunch(
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    ``python -m ev0_cli.main``) or a ``.cmd`` batch file, and both
