...
from ev0_cli.sqlite_util import add_column_if_missing as _add_column_if_missing
...
def normalize_reasoning_effort(effort: Optional[str]) -> Optional[str]:
    ...
    Accepts any level in ``ev0_constants.VALID_REASONING_EFFORTS`` plus
    ...
    """
    from ev0_constants import VALID_REASONING_EFFORTS
...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    ...
    """
    try:
        from ev0_cli.lifecycle import invoke_hook
...
class Task:
    ...
    # ``ev0_constants.VALID_REASONING_EFFORTS``, or ``"none"`` for thinking
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    workspace_path       TEXT,
    branch_name          TEXT,
    -- Optional link to a first-class Project (ev0_cli/projects_db). When set,
...
def _sqlite_connect(path: Path) -> sqlite3.Connection:
    ...
    advisory locks on the database (see ``ev0_cli.sqlite_safe_read``).
    The registration is released automatically when the connection closes.
    """
    from ev0_cli.sqlite_safe_read import connect_tracked
...
def _validate_sqlite_header(path: Path) -> None:
    ...
    from ev0_cli.sqlite_safe_read import read_header_bytes_preopen
...
def _backup_corrupt_db(path: Path) -> Optional[Path]:
    ...
    # see ev0_cli.sqlite_safe_read), so it must only run once the board has
    ...
    # rescue.
    from ev0_cli.sqlite_safe_read import has_live_connection
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
    ...
    with _cross_process_init_lock(path):
        ...
        from ev0_state import preflight_db_writability
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See ev0_state._WAL_INCOMPAT_MARKERS for detection logic.
                from ev0_state import apply_wal_with_fallback
                apply_wal_with_fallback(conn, db_label=f"kanban.db ({path.name})")
...
def _check_file_length_invariant(conn: sqlite3.Connection) -> None:
    ...
    """
    from ev0_cli.sqlite_safe_read import file_length_matches_header
...
def create_task(
    ...
    *,
    ...
    if project_id:
        from ev0_cli import projects_db as _pdb
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    if not rows:
        return False
    try:
        from ev0_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def _dispatch_once_locked(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from ev0_cli.profiles import profile_exists as _pe
    ...
    for row in review_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        try:
            from ev0_cli.profiles import profile_exists
        except Exception:
            profile_exists = None  # type: ignore[assignment]
...
def _module_ev0_argv() -> list[str]:
    ...
    # ``ev0_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``3v0`` package — there is no
    # ``3v0`` package to import.
    return [sys.executable, "-m", "ev0_cli.main"]
...
def _resolve_ev0_argv() -> list[str]:
    ...
    3. ``sys.executable -m ev0_cli.main`` — fallback for setups where
    ...
    local (not imported from gateway) because ``ev0_cli`` sits below
...
def _default_spawn(
    ...
    *,
    ...
    # _apply_profile_override() runs *before* ev0_constants is imported.
...
def list_profiles_on_disk() -> list[str]:
    ...
    ``ev0_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from ev0_constants import get_default_ev0_root
