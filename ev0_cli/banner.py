...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``ev0_cli/build_info.py``) and return it as a frozen
...
def _compute_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    if repo_dir is None:
        ...
        try:
            from ev0_cli.build_info import get_build_sha
...
def _format_update_notice(behind: int) -> str:
    ...
    from ev0_cli.config import get_managed_update_command, recommended_update_command
    if behind > 0:
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    if not _has_native_mcp:
        try:
            from ev0_cli.plugins import get_portable_mcp_server_names_nowait
    ...
    try:
        from ev0_cli.codex_runtime_switch import get_current_runtime
        from ev0_cli.config import load_config as _load_cfg
