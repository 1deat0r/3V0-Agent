"""``3v0 debug`` debug tools for 3V0 Agent.

Currently supports:
    3v0 debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.3V0/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[3v0 debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.3V0/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``3v0 debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``3v0 debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `3v0 debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "3v0-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``3v0 debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.3V0/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``3v0 debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "3v0-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from ev0_cli.logs import LOG_FILES
...
# desktop app talks to a remote/docker/SSH backend, `3v0 debug share` runs
...
_CLIENT_SIDE_LOGS = {
    "desktop": (
        "written by 3V0 Desktop on the machine running the app, not by this "
...
def _capture_dump() -> str:
    """Run ``3v0 dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``3v0 dump`` is run
        internally.
...
_NOUS_BUNDLE_FORMAT = "3v0-debug-share/1"
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``3v0 debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "3v0 debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `3v0 debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print("To delete now:  3v0 debug delete <url>")

    print("\nShare these links with the 3V0 team for support.")
...
⚠️  --nous: This uploads your debug bundle to Nous-INTERNAL storage (AWS S3),
  ...
  • System info (OS, Python/3V0 version, provider, which API keys are
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    """Handle ``3v0 debug share --nous``: upload the bundle to Nous-S3.
    ...
    suggests ``--local``.
    """
    from ev0_cli.diagnostics_upload import share_to_nous
    ...
    if redact:
        logger.info(
            "3v0 debug share --nous: applied force-mode redaction before upload"
        )
    ...
    try:
        ...
        print(
            ...
            "provisioned.\n"
            "Run `3v0 debug share --local` to print the report instead, "
            "or `3v0 debug share` to upload to a public paste service.\n",
            file=sys.stderr,
        )
        sys.exit(1)
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: 3v0 debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by '3v0 debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``3v0 debug`` call.
    ...
    # best-effort — any failure is swallowed so ``3v0 debug`` stays
