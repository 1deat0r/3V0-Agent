...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for 3V0 Agent. Use '3v0 debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    3v0 debug share              Upload debug report (asks for confirmation)
    3v0 debug share --yes        Skip confirmation (for scripts/CI)
    3v0 debug share --lines 500  Include more log lines
    3v0 debug share --expire 30  Keep paste for 30 days
    3v0 debug share --local      Print report locally (no upload)
    3v0 debug share --no-redact  Disable upload-time secret redaction
    3v0 debug share --nous       Upload to Nous-internal storage (private)
    3v0 debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by '3v0 debug share'",
    )
