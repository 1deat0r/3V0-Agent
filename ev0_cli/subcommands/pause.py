"""``3v0 pause`` / ``3v0 resume`` — the global emergency stop.

``3v0 pause`` writes the ESTOP sentinel at ``$EV0_HOME/ESTOP``, which
halts cron dispatch, kanban dispatch, and new gateway turns on their next
check. In-flight work is never killed. ``3v0 resume`` removes the
...
def cmd_pause(args: argparse.Namespace) -> int:
    ...
    state = get_state() or {}
    verb = "Still paused" if already else "3V0 paused"
    ...
    print(
        ...
        "    In-flight work keeps running. Run `3v0 resume` to lift the pause."
    )
    return 0
...
def cmd_resume(args: argparse.Namespace) -> int:
    ...
    if disengage():
        print("▶️  3V0 resumed — dispatch picks up on the next tick.")
    else:
        print(f"3V0 is not paused (no sentinel at {sentinel_path()}).")
    return 0
...
def build_pause_parser(subparsers) -> None:
    ...
    pause_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "`3v0 resume`. In-flight work is never killed."
        ),
    )
    ...
    resume_parser = subparsers.add_parser(
        "resume",
        help="Lift the emergency stop set by `3v0 pause`",
