...
from ev0_constants import get_optional_skills_dir
...
from ev0_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from ev0_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def is_noninteractive() -> bool:
    ...
    ``EV0_NONINTERACTIVE=1`` (see ``ev0_cli/web_server.py``). In that
...
def _print_setup_summary(config: dict, ev0_home):
    ...
    try:
        from ev0_cli.auth import resolve_provider
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from ev0_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    from ev0_cli.main import select_provider_and_model
    try:
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    """
    try:
        from ev0_cli.auth import get_xai_oauth_auth_status
...
def _setup_telegram_auto_result():
    ...
    try:
        from ev0_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None
...
def _setup_qqbot():
    ...
    from ev0_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def setup_gateway(config: dict):
    ...
    from ev0_cli.gateway import _all_platforms, _platform_status, _configure_platform
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``ev0_cli.auth`` — lists every supported
    ...
    """
    try:
        from ev0_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from ev0_cli.config
    ...
    if section_key == "model":
        ...
        from ev0_cli.gateway import _all_platforms, _platform_status
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    try:
        from ev0_cli.main import _model_flow_nous
...
def run_setup_wizard(args):
    ...
    """
    from ev0_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if not (migration_ran and _skip_configured_section(config, "gateway", "Messaging Platforms")):
        ...
        from ev0_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    try:
        ...
        from ev0_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
