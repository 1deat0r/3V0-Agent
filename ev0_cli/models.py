...
def _custom_provider_ssl_context(base_url: str):
    ...
    if not base_url:
        return None
    try:
        from ev0_cli.config import get_custom_provider_tls_settings
...
def _codex_curated_models() -> list[str]:
    ...
    """
    from ev0_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # endpoint expects (see ev0_cli/model_setup_flows.py).
...
def check_nous_free_tier(*, force_fresh: bool = False) -> bool:
    ...
    try:
        from ev0_cli.nous_account import get_nous_portal_account_info
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from ev0_cli.model_catalog import get_default_model_from_cache
...
# ``get_recommended_default_model`` in ev0_cli/web_server.py and
...
# ev0_constants.VALID_REASONING_EFFORTS ("none" included so an explicit
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from ev0_cli.model_catalog import get_curated_openrouter_models
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from ev0_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from ev0_constants import AI_GATEWAY_BASE_URL
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from ev0_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from ev0_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``ev0_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    """
    try:
        from ev0_cli.auth import resolve_api_key_provider_credentials
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from ev0_cli.model_search import model_alias_canonical
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from ev0_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # ``3v0 model`` shows.
            from ev0_cli.providers import is_official_openai_host
...
def probe_api_models(
    ...
    if isinstance(request_headers, dict):
        ...
        from ev0_cli.config import normalize_extra_headers
