...
the tool registry or launching 3V0 (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``3v0-cli`` to the non-existent ``3v0``. ``resolve_toolset('3v0')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``3v0-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``3v0`` instead of ``3v0-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `3v0 tools` to reconfigure."
        )
    return warnings
