...
This module is imported by ``ev0_cli/main.py`` BEFORE its heavy import
wall (config, argparse tree, logging, providers). Everything here must stay
**stdlib-only and cheap** (os/sys file probes; no yaml, no ev0_cli.config,
...
``ev0_cli/config.py``'s ``get_container_exec_info()`` reads the same
...
def print_fast_version_info() -> None:
    from ev0_cli import __release_date__, __version__
