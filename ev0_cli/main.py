...
# IMPORTANT: ev0_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``ev0_bootstrap`` is a
...
# new code references ``ev0_bootstrap`` but the editable install's
...
from ev0_cli._subprocess_compat import suppress_platform_ver_console
...
# Two lines of inline path math so ``python ev0_cli/main.py`` (script
# mode — sys.path[0] is ev0_cli/, not the repo root) can import the
# canonical helpers; everything else lives in ev0_cli._startup_fast.
_bootstrap_root = os.path.realpath(os.path.join(os.path.dirname(__file__), os.pardir))
if _bootstrap_root not in sys.path:
    sys.path.insert(0, _bootstrap_root)
from ev0_cli import _startup_fast  # noqa: E402
...
# ``from ev0_cli.env_loader import ...`` / ``from ev0_cli.config import
...
# package directory, so if IT can't import, nothing else in ev0_cli can
# either. It is also the canonical home of the probe/repair tables reused by
# the full recovery path below.
from ev0_cli import _early_recovery as _early_recovery_mod
...
def _run_and_exit_oneshot(
    ...
    *,
    ...
    try:
        from ev0_cli.oneshot import run_oneshot
...
# that run *before* ev0_cli.config is importable. Mirrors the explicit
...
from ev0_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from ev0_cli.subcommands.cron import build_cron_parser
from ev0_cli.subcommands.sync import build_sync_parser
from ev0_cli.subcommands.gateway import build_gateway_parser
from ev0_cli.subcommands.profile import build_profile_parser
from ev0_cli.subcommands.model import build_model_parser
from ev0_cli.subcommands.setup import build_setup_parser

from ev0_cli.subcommands.whatsapp import build_whatsapp_parser
from ev0_cli.subcommands.slack import build_slack_parser
from ev0_cli.subcommands.login import build_login_parser
from ev0_cli.subcommands.logout import build_logout_parser
from ev0_cli.subcommands.auth import build_auth_parser
from ev0_cli.subcommands.status import build_status_parser
from ev0_cli.subcommands.pause import build_pause_parser
from ev0_cli.subcommands.webhook import build_webhook_parser
from ev0_cli.subcommands.hooks import build_hooks_parser
from ev0_cli.subcommands.doctor import build_doctor_parser
from ev0_cli.subcommands.verify import build_verify_parser
from ev0_cli.subcommands.security import build_security_parser
from ev0_cli.subcommands.approvals import build_approvals_parser
from ev0_cli.subcommands.dump import build_dump_parser
from ev0_cli.subcommands.debug import build_debug_parser
from ev0_cli.subcommands.backup import build_backup_parser
from ev0_cli.subcommands.import_cmd import build_import_cmd_parser
from ev0_cli.subcommands.import_agent import build_import_agent_parser
from ev0_cli.subcommands.config import build_config_parser
from ev0_cli.subcommands.skin import build_skin_parser
from ev0_cli.subcommands.console import build_console_parser
from ev0_cli.subcommands.version import build_version_parser
from ev0_cli.subcommands.update import build_update_parser
from ev0_cli.subcommands.uninstall import build_uninstall_parser
from ev0_cli.subcommands.dashboard import build_dashboard_parser
from ev0_cli.subcommands.logs import build_logs_parser
from ev0_cli.subcommands.prompt_size import build_prompt_size_parser
from ev0_cli.subcommands.memory import build_memory_parser
from ev0_cli.subcommands.acp import build_acp_parser
from ev0_cli.subcommands.tools import build_tools_parser
from ev0_cli.subcommands.insights import build_insights_parser
from ev0_cli.subcommands.monitoring import build_monitoring_parser
from ev0_cli.subcommands.skills import build_skills_parser
from ev0_cli.subcommands.pairing import build_pairing_parser
from ev0_cli.subcommands.plugins import build_plugins_parser
from ev0_cli.subcommands.mcp import build_mcp_parser
from ev0_cli.subcommands.claw import build_claw_parser
...
def _apply_profile_override() -> None:
    ...
    # Mirrors ev0_cli.profiles._PROFILE_ID_RE so we never call
...
# var BEFORE ev0_logging imports agent.redact (which snapshots the flag at
...
try:
    from ev0_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from ev0_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if ev0_constants not importable yet
...
from ev0_cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to ev0_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (ev0_cli.main._model_flow_*) keep resolving unchanged.
from ev0_cli.model_setup_flows import (
...
def _relative_time(ts) -> str:
    ...
    in :mod:`ev0_cli.timefmt` so lightweight consumers don't have to
    import the whole CLI surface.
    """
    from ev0_cli.timefmt import relative_time
...
def _has_any_provider_configured() -> bool:
    ...
    from ev0_cli.config import get_env_path, get_ev0_home, load_config
    from ev0_cli.auth import get_auth_status
...
def _confirm_startup_expensive_model_override(args) -> None:
    ...
    try:
        ...
        from ev0_cli.model_selection_guards import (
            combined_message,
            selection_warnings,
        )
    except Exception as exc:
...
def _resolve_continue_arg(args, *, use_tui: bool) -> None:
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            try:
                from ev0_cli.terminal_breadcrumbs import resolve_breadcrumb_session
...
def _find_bundled_tui(ev0_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if ev0_cli_dir is None:
        ev0_cli_dir = Path(__file__).parent
    bundled = ev0_cli_dir / "tui_dist" / "entry.js"
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    # ev0_cli/tui_dist/entry.js but never ships ui-tui/ at all (that
    ...
    if (
        ...
        if result.returncode != 0:
            ...
            from ev0_cli.npm_engine import maybe_repair_npm_engine
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from ev0_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    try:
        from ev0_cli.config import apply_terminal_config_to_env
...
def _pin_kanban_board_env() -> None:
    ...
    if os.environ.get("EV0_KANBAN_BOARD"):
        return
    try:
        from ev0_cli.kanban_db import get_current_board
...
def cmd_chat(args):
    ...
    if isinstance(_resume_foreign, str) and _resume_foreign.strip().lower() in (
        ...
        from ev0_cli.foreign_sessions import (
    ...
    try:
        from ev0_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from ev0_cli.config import load_config as _load_config_for_xai_check
    ...
    if _termux_should_prefetch_update_check():
        try:
            from ev0_cli.banner import prefetch_banner_data, prefetch_update_check
...
def cmd_gateway(args):
    ...
    from ev0_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from ev0_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from ev0_constants import find_node_executable, with_ev0_node_path
...
def cmd_whatsapp_cloud(args):
    ...
    ``ev0_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from ev0_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from ev0_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    # PROVIDER_GROUPS in ev0_cli/models.py). A multi-member group shows one
...
def _clear_stale_openai_base_url():
    ...
    """
    from ev0_cli.config import get_env_value, save_env_value, load_config
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`ev0_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from ev0_cli.plugins import get_plugin_auxiliary_tasks
...
def _aux_select_for_task(task: str) -> None:
    ...
    """
    from ev0_cli.config import load_config
    from ev0_cli.inventory import build_aux_picker_rows, format_aux_picker_entries
...
def _aux_flow_provider_model(
    ...
    from ev0_cli.auth import _prompt_model_selection
    from ev0_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from ev0_cli.setup import _curses_prompt_choice
...
def _remove_custom_provider(config):
    ...
    try:
        from ev0_cli.curses_ui import curses_radiolist
...
# downstream call sites read `ev0_cli.main._PROVIDER_MODELS` directly,
...
# surface so argparse wiring and test monkeypatches on ev0_cli.main.<name>
...
_LAZY_COMMAND_EXPORTS = {
    "ev0_cli.sessions_cmd": (
        "cmd_sessions",
    ),
    "ev0_cli.dashboard_procs": (
...
def _self():
    ...
    keeps monkeypatches on ev0_cli.main.<name> working, exactly like a
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from ev0_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(
    ...
    """
    from ev0_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    from ev0_cli.env_loader import format_secret_source_suffix
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from ev0_constants import display_ev0_home as _dhh_fn
...
def cmd_login(args):
    ...
    from ev0_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from ev0_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from ev0_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from ev0_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from ev0_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from ev0_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    if sub == "manifest":
        from ev0_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from ev0_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from ev0_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from ev0_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from ev0_cli.doctor import run_doctor
...
def cmd_security(args):
    ...
    if sub in ("audit", None):
        from ev0_cli.security_audit import cmd_security_audit
...
def cmd_approvals(args):
    ...
    from ev0_cli.approvals_suggest import approvals_command
...
def cmd_dump(args):
    ...
    from ev0_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from ev0_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from ev0_cli.config import config_command
...
def cmd_skin(args):
    ...
    from ev0_cli.skin_cmd import skin_command
...
def cmd_backup(args):
    ...
    if getattr(args, "quick", False):
        from ev0_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from ev0_cli.backup import run_backup
...
def cmd_import(args):
    ...
    from ev0_cli.backup import run_import
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    try:
        from ev0_cli.banner import UPDATE_AVAILABLE_NO_COUNT, check_for_updates
        from ev0_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    ...
    from ev0_cli.uninstall import run_uninstall
...
# Update pipeline lives in ev0_cli/update_cmd.py (main.py decomposition,
...
# monkeypatches on ev0_cli.main.<name> keep resolving unchanged without
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    ``ev0_cli/web_dist/`` (per vite.config.ts outDir), NOT ``web/dist/``,
    ...
    dist_dir = project_root / "ev0_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``3v0`` launcher present but ``ev0_cli`` not importable.
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    project_root = web_dir.parent.parent if web_dir.parent.name == "apps" else web_dir.parent
    dist_index = project_root / "ev0_cli" / "web_dist" / "index.html"
    try:
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    # build path usable on a stock `py -m ev0_cli.main web` invocation.
...
# Dashboard process-hygiene helpers live in ev0_cli/dashboard_procs.py
...
# ev0_cli.main.<name> keep resolving unchanged.
...
def _parse_dashboard_runtime(command: str) -> tuple[str, str, int] | None:
    ...
    if any(
        ...
        for pattern in (
            "3v0 dashboard",
            "ev0_cli.main dashboard",
            "ev0_cli/main.py dashboard",
        )
    ):
        mode = "dashboard"
    elif any(
...
def _recover_core_update_marker_locked() -> None:
    ...
    try:
        from ev0_cli import _install_repair as _ir
...
def _venv_scripts_dir() -> Path | None:
    ...
    if not venv_dir.is_dir():
        return None
    from ev0_constants import venv_bin_dir
    ...
    *,
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m ev0_cli.main <command>"
        )
    else:
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    try:
        ...
        # ev0_cli.config.get_ev0_home to simulate setup failure.
        from ev0_cli.config import get_ev0_home as _get_ev0_home
...
def cmd_update(args):
    ...
    from ev0_cli.update_lock import (
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        from ev0_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from ev0_constants import get_ev0_home as _hh
        ...
        from ev0_cli import profile_describer as _pd
        ...
        from ev0_cli.profiles import profile_exists, validate_alias_name
        ...
        from ev0_cli.profiles import rename_profile
        ...
        from ev0_cli.profiles import export_profile
        ...
        from ev0_cli.profiles import import_profile
        ...
        import tempfile
        from ev0_cli.profile_distribution import (
            plan_install,
        ...
        from ev0_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        from ev0_cli.profile_distribution import describe_distribution, DistributionError
...
def _render_distribution_plan(plan) -> None:
    ...
    from ev0_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    try:
        from ev0_cli.web_server import should_require_auth
    ...
    try:
        from ev0_cli.dashboard_auth import list_providers
        if list_providers():
    ...
    try:
        from ev0_cli.config import load_config, save_config
        from ev0_cli.plugins_cmd import ensure_basic_auth_plugin_enabled_in_config
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    if sys.platform == "win32":
        from ev0_cli.windows_ssh_runtime import read_token
...
def cmd_dashboard(args):
    ...
    from ev0_cli.resource_limits import apply_nofile_soft_limit
    ...
    try:
        from ev0_logging import setup_logging as _setup_logging_gui
    ...
    if _headless_backend:
        ...
        _dist_root = (
            ...
            else PROJECT_ROOT / "ev0_cli" / "web_dist"
        )
    ...
    from ev0_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from ev0_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from ev0_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from ev0_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from ev0_cli.prompt_size import cmd_prompt_size as _impl
...
def cmd_logs(args):
    ...
    from ev0_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    ...
    from ev0_cli.console_engine import run_console_repl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from ev0_cli.models import CANONICAL_PROVIDERS as _cp
...
# the top-level flags declared in ``ev0_cli/_parser.py``.
#
...
def _prepare_agent_startup(args) -> None:
    ...
    if not _is_tui_chat_launch(args):
        ...
        try:
            from ev0_cli.plugins import start_background_plugin_discovery
...
def _try_fast_chat_launch() -> bool:
    ...
    try:
        from ev0_cli.config import get_container_exec_info
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        from ev0_cli.memory_setup import memory_command
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from ev0_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from ev0_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from ev0_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from ev0_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from ev0_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from ev0_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from ev0_cli.plugins_cmd import plugins_command
...
def cmd_claw(args):
    from ev0_cli.claw import claw_command
...
def main():
    ...
    # =========================================================================
    # model command  (parser built in ev0_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from ev0_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from ev0_cli.fallback_cmd import cmd_fallback
    ...
    from ev0_cli import secrets_cli as _secrets_cli
    from ev0_cli import onepassword_secrets_cli as _op_secrets_cli
    ...
    from ev0_cli import proxy_cli as _proxy_cli
    ...
    # migrate command
    # =========================================================================
    from ev0_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in ev0_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in ev0_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)


    # =========================================================================
    # whatsapp command  (parser built in ev0_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    # =========================================================================
    # slack command  (parser built in ev0_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from ev0_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in ev0_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in ev0_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in ev0_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in ev0_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # pause / resume commands  (parser built in ev0_cli/subcommands/pause.py)
    # =========================================================================
    build_pause_parser(subparsers)

    # =========================================================================
    # cron command  (parser built in ev0_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)
    build_sync_parser(subparsers, cmd_sync=cmd_sync)

    # =========================================================================
    # webhook command  (parser built in ev0_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from ev0_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from ev0_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from ev0_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in ev0_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in ev0_cli/subcommands/doctor.py)
    # =========================================================================
    build_doctor_parser(subparsers, cmd_doctor=cmd_doctor)

    # =========================================================================
    # verify command  (parser built in ev0_cli/subcommands/verify.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in ev0_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # approvals command  (parser built in ev0_cli/subcommands/approvals.py)
    # =========================================================================
    build_approvals_parser(subparsers, cmd_approvals=cmd_approvals)

    # =========================================================================
    # dump command  (parser built in ev0_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in ev0_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in ev0_cli/subcommands/backup.py)
    # =========================================================================
    ...
    from ev0_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in ev0_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # import-agent command  (parser: ev0_cli/subcommands/import_agent.py)
    # =========================================================================
    def cmd_import_agent(args):
        from ev0_cli.agent_import import import_agent_command
        import_agent_command(args)

    build_import_agent_parser(subparsers, cmd_import_agent=cmd_import_agent)

    # =========================================================================
    # config command  (parser built in ev0_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # skin command  (parser built in ev0_cli/subcommands/skin.py)
    # =========================================================================
    build_skin_parser(subparsers, cmd_skin=cmd_skin)

    # =========================================================================
    # console command  (parser built in ev0_cli/subcommands/console.py)
    # =========================================================================
    build_console_parser(subparsers, cmd_console=cmd_console)

    # =========================================================================
    # pairing command  (parser built in ev0_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in ev0_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from ev0_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in ev0_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from ev0_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from ev0_cli.curator import register_cli as _register_curator_cli
    ...
    try:
        from ev0_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from ev0_cli.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # memory command  (parser built in ev0_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in ev0_cli/subcommands/tools.py)
    # =========================================================================
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            import os as _os
            import subprocess
            from ev0_cli.tools_config import _cua_driver_contract_status
            ...
            if path:
                version = ""
                try:
                    from ev0_cli.tools_config import _cua_driver_env
                    ...
                    pass
                from ev0_cli.tools_config import _cua_version_summary
    ...
    # =========================================================================
    # mcp command  (parser built in ev0_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    # cmd_sessions lives in ev0_cli/sessions_cmd.py (main.py decomposition).
    ...
    # and lets monkeypatches on ev0_cli.main.cmd_sessions keep working.
    ...
    # =========================================================================
    # insights command  (parser built in ev0_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)
    build_monitoring_parser(subparsers, cmd_monitoring=cmd_monitoring)

    # =========================================================================
    # claw command  (parser built in ev0_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in ev0_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in ev0_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in ev0_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in ev0_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in ev0_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in ev0_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # =========================================================================
    # logs command  (parser built in ev0_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in ev0_cli/subcommands/prompt_size.py)
    # =========================================================================
