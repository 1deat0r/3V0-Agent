...
Usage:
    3v0 pairing list              # Show all pending + approved users
    3v0 pairing approve <platform> <request-id|code>  # Approve a pairing request
    3v0 pairing revoke <platform> <user_id> # Revoke user access
    3v0 pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle 3v0 pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: 3v0 pairing {list|approve|revoke|clear-pending}")
        print("Run '3v0 pairing --help' for details.")
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: 3v0 pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.3V0/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run '3v0 pairing list' to see pending requests.\n")
