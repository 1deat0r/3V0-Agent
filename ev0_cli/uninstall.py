...
def uninstall_gateway_service():
    ...
    try:
        from ev0_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from ev0_cli.gateway import get_launchd_plist_path
...
def run_gui_uninstall(args):
    ...
    """
    from ev0_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
...
def _perform_uninstall(
    *,
    ...
    try:
        from ev0_cli.gui_uninstall import uninstall_gui
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m ev0_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import ev0_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``ev0_constants`` + ``ev0_cli.colors``
    (and lazily ``ev0_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m ev0_cli.uninstall")
    parser.add_argument(
        "--mode",
