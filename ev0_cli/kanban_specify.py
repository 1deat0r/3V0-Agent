...
Used by ``3v0 kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``ev0_cli/goals.py`` — same aux
...
EV0_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    env_int("EV0_KANBAN_SPECIFY_MAX_TOKENS", 6000),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the 3V0 Agent board.
...
def _profile_author() -> str:
    """Mirror of ``ev0_cli.kanban._profile_author``. Kept local to
    ...
    *,
    ...
    try:
        ...
        resp = call_llm(
            ...
            temperature=0.3,
            max_tokens=EV0_KANBAN_SPECIFY_MAX_TOKENS,
