"""
Backup and import commands for 3v0 CLI.

`3v0 backup` creates a zip archive of the entire ~/.3V0/ directory
(excluding the 3v0-agent repo and transient files).

`3v0 import` restores from a backup zip, overlaying onto the current
EV0_HOME root.
"""
...
from ev0_constants import get_default_ev0_root, get_ev0_home, display_ev0_home
from utils import (
    _preserve_file_mode,
...
from ev0_cli.sizefmt import format_bytes as _format_size
...
# ``3v0-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/3v0-agent/``
...
# EV0_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "3v0-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under EV0_HOME) —
...
# File names that ``3v0 import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE EV0_HOME
...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the 3V0 backup slot."""
            ...
            while True:
                try:
                    ...
                    if time.monotonic() >= deadline:
                        raise BackupInProgressError("another 3V0 backup is already running")
                    time.sleep(0.05)
        else:
            import fcntl
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside EV0_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to 3v0 root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``3v0-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/3v0-agent/`` — must be preserved.
        if part == "3v0-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside EV0_HOME.
...
# to the user as a hung `3v0 update` (#70553 follow-up). Sessions databases
...
def verify_sqlite_integrity(
    ...
    *,
    ...
    if check_header:
        ...
        # ev0_cli.sqlite_safe_read). Verification targets snapshots and
...
def _run_backup_locked(args, ev0_root: Path) -> None:
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"3v0-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"3v0-backup-{stamp}.zip"
    ...
    for dirpath, dirnames, filenames in os.walk(ev0_root, followlinks=False):
        ...
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "3v0-agent" and not is_root)
        ]
    ...
    # outside EV0_HOME, so the walk above never sees it. Ask the active
    ...
    if not errors:
        print(f"\nRestore with: 3v0 import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a 3V0 backup.
    ...
    # Look for telltale files that a 3v0 home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a 3V0 backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.3V0/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a 3v0 dir name
        if prefix in {".3V0", "3v0"}:
            return prefix + "/"
...
def _extract_member_atomically(
    ...
    with nothing behind it — during ``3v0 import``, which is the
    ...
    so without the chown a ``sudo 3v0 import`` would silently re-own every
    ...
    if mode is None:
        ...
        # take over an existing setuid/setgid file, so ``3v0 import`` would
        # hand whoever produced the zip the identity that file runs as.  Nothing
        # constrains that to 3V0' own state either: the ``_external/`` branch
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has 3V0 configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under EV0_HOME.
        ...
        if profiles_dir.is_dir():
            try:
                ...
                # ev0_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: 3v0 profile list  (after installing 3v0)")
        ...
        if restored_profiles:
            ...
            for pname in gw_profiles:
                print(f"  3v0 -p {pname} gateway install")
        ...
        try:
            from ev0_cli.gateway import ensure_gateway_service, _is_service_running
        ...
        print("Done. Your 3V0 configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and 3v0 backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to EV0_HOME).
...
# — `3v0 update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $EV0_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside EV0_HOME and the entry is silently
...
def _create_quick_snapshot_locked(
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: 3v0 snapshot list"
        )
        logger.error(
...
# Relative path of the cron job database inside EV0_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    Args:
        ...
        ev0_home: Override for the 3V0 home directory (tests).
...
def run_quick_backup(args) -> None:
    """CLI entry point for 3v0 backup --quick."""
...
def create_pre_update_backup(
    ...
    but writes to ``<EV0_HOME>/backups/pre-update-<timestamp>.zip`` and
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `3v0 claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    ``3v0 claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``3v0 import <archive>``.  Writes to
    ``<EV0_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
