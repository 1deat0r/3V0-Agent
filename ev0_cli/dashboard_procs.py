"""Dashboard process-hygiene helpers — extracted from ``ev0_cli/main.py``.
...
helpers that STAY in ``ev0_cli.main`` (``_find_stale_dashboard_pids``,
``_respawn_dashboard_processes``, ``_is_windows``, ...) are routed through a
lazy ``_m()`` main reference so existing test monkeypatches on
``ev0_cli.main.<name>`` keep reaching this code path, and imports stay
...
patches on ``ev0_cli.main`` resolve unchanged.
"""
...
def _m():
    """Lazy ``ev0_cli.main`` reference (call-time; keeps patches working)."""
    from ev0_cli import main
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    ...
    try:
        if sys.platform == "win32":
            ...
            # with `ev0_cli.gateway._scan_gateway_pids` and avoids the
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    ...
    if "serve" not in cmd:
        return False
    if "3v0" not in cmd and "ev0_cli" not in cmd:
        return False
