"""
3v0 fallback — manage the fallback provider chain.
...
Subcommands:
  3v0 fallback [list]   Show the current fallback chain (default when no subcommand)
  3v0 fallback add      Pick provider + model via the same picker as `3v0 model`,
                           then append the selection to the chain
  3v0 fallback remove   Pick an entry to delete from the chain
  3v0 fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.3V0/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from ev0_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `3v0 model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  3v0 fallback add")
        print()
        return
    ...
    print("  Docs: https://github.com/1deat0r/3V0-Agent/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `3v0 model`, then append the selection to the chain."""
    from ev0_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `3v0 model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `3v0 fallback list` to view, or `3v0 fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``3v0 fallback [subcommand]``."""
