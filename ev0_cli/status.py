...
from ev0_cli.auth import AuthError, resolve_provider
from ev0_cli.colors import Colors, color
from ev0_cli.config import get_env_path, get_env_value, get_ev0_home, load_config
from ev0_cli.models import provider_label
from ev0_cli.nous_account import (
    format_nous_portal_entitlement_message,
    get_nous_portal_account_info,
)
from ev0_cli.nous_subscription import get_nous_subscription_features
from ev0_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``3v0 config``'s
...
def _estop_status_line():
    """One-line pause banner for `3v0 status`, or None when not paused.

    Cheap: a single stat on $EV0_HOME/ESTOP via agent.estop.
    """
    try:
    ...
    return f"⏸️  PAUSED (global emergency stop{suffix}; `3v0 resume` to lift)"


def show_status(args):
    """Show status of all 3V0 Agent components."""
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ 3V0 Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        # Read-only display: use the refresh-free snapshot so `3v0 status`
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: 3v0 portal)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: 3v0 model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: 3v0 auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: 3v0 auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: 3v0 model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from ev0_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install '3v0-agent[vercel]')"
    ...
    try:
        from ev0_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    jobs_file = get_ev0_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    print(color("  Run '3v0 doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run '3v0 setup' to configure", Colors.DIM))
    print()
