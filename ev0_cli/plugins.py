...
from ev0_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE
from ev0_cli.plugin_capabilities import (  # noqa: F401 — re-exported
    CAPABILITY_REGISTRY,
    VALID_CAPABILITY_IDS,
    plugin_capability_granted,
)
from ev0_cli.plugin_capabilities import (
...
VALID_HOOKS: Set[str] = {
    ...
    "pre_transcription",
    # Kanban task lifecycle hooks. Fired by ev0_cli.kanban_db when a task
    ...
    #   result: ev0_cli.kanban_db.DispatchResult (spawned, reclaimed,
...
class PluginManifest:
    ...
    # ``ev0_cli.plugin_capabilities.CAPABILITY_REGISTRY``. Declaration is
...
class PluginContext:
    ...
    def platform_actions(self):
        ...
        if self._platform_actions is None:
            from ev0_cli.platform_actions import PlatformActions
    ...
    def register_dashboard_auth_provider(self, provider) -> Optional[PluginRegistration]:
        ...
        ``provider`` must be an instance of
        :class:`ev0_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        from ev0_cli.dashboard_auth.registry import restore_registration
        from ev0_cli.dashboard_auth.registry import snapshot_registration
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        # Lazy import to avoid circular: ev0_cli.main imports plugins indirectly
        from ev0_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
...
class PluginManager:
    ...
    def register_approval_transport(
        ...
        *,
        ...
        from ev0_cli.approval_transport import RegisteredApprovalTransport
    ...
    def has_enabled_portable_mcp(self, raw_config: Mapping[str, Any]) -> bool:
        ...
        for manifest in winners.values():
            ...
            if lookup_key not in enabled and manifest.name not in enabled:
                continue
            try:
                from ev0_cli.agent_plugins import _discover_mcp
        ...
        *,
        ...
        for child in sorted(path.iterdir()):
            ...
            if portable_file.exists() or portable_file.is_symlink():
                try:
                    from ev0_cli.agent_plugins import read_agent_plugin_manifest
    ...
    def _load_portable_plugin(
        ...
        lookup_key = manifest.key or manifest.name
        try:
            from ev0_cli.agent_plugins import load_agent_plugin
...
def _get_pre_tool_call_directive_details(
    ...
    from ev0_cli.lifecycle import invoke_hook as invoke_lifecycle_hook
