"""
Profile management for multiple isolated 3V0 instances.

Each profile is a fully independent EV0_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.3V0/profiles/<name>/`` by default.

The "default" profile is ``~/.3V0`` itself — backward compatible,
zero migration needed.

Usage::

    3v0 profile create coder          # fresh profile + bundled skills
    3v0 profile create coder --clone  # also copy config, .env, SOUL.md, skills
    3v0 profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    3v0 -p coder chat                 # or via flag
    3v0 profile use coder             # set as sticky default
    3v0 profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See ev0_constants.get_subprocess_home().
    "home",
]
...
# Marker file written by `3v0 profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `3v0
...
# `3v0 skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         3V0 infrastructure directories that only the default profile
         (``~/.3V0``) ever contains.  Gated on ``source_dir`` actually
...
# Directories/files to exclude when exporting the default (~/.3V0) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "3v0-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when EV0_HOME equals the
...
# into the export. The set below identifies the *known 3V0 profile
# artifacts* at the root of EV0_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``ev0_constants``.
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the 3v0 root, NOT to the current EV0_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where EV0_HOME points outside
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the 3V0 installation itself or a common system binary.  "
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, 3v0 subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _EV0_SUBCOMMANDS:
        return f"'{canon}' conflicts with a 3v0 subcommand"
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running 3V0. If we
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``3v0 -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``3v0 profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``3v0 -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``3v0 profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``EV0_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing 3V0 config
...
def profiles_to_serve(
    ...
    """Return the ``(profile_name, ev0_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own EV0_HOME. When
    ...
    The returned ``ev0_home`` is the path to pass to
    ``set_ev0_home_override`` when scoping a turn to that profile.
    """
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``3v0 update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.3V0)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.3V0/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `3v0 -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this 3V0 profile.\n"
    ...
    if not soul_path.exists():
        try:
            from ev0_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `3v0 update`'s
    ...
    # Cloned configs can be older than the running 3V0 (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `3v0 profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `3v0 -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches EV0_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``3v0 profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "EV0_HOME": str(profile_dir)},
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running 3V0 *backends* bound to this profile.
    ...
    names it, so find it by inspection: a 3V0 backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``EV0_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `3v0 -p <canon> profile
    ...
    ev0_markers = ("ev0_cli.main", "3v0-gateway", "tui_gateway")
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a 3V0 process: either an entrypoint marker in argv, or
            # a resolved executable named `3v0`.
            joined = " ".join(argv)
            ...
            # ...or by EV0_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("EV0_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.3V0).\n"
            "To remove everything, use: 3v0 uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_ev0_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``EV0_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from ev0_cli.service_manager import get_service_manager
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.3V0/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: 3v0 profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from EV0_HOME.

    Returns ``"default"`` if EV0_HOME is not set or points to ``~/.3V0``.
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      EV0_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-3V0 file the
      user may have lying alongside EV0_HOME (#58394).
...
def _scrub_export_secrets(staged: Path) -> None:
    ...
    ``3v0 sessions export --redact``. Runs on the *staged copy only* so the
    live profile is never rewritten. ``force=True`` ignores
    ``security.redact_secrets`` / ``EV0_REDACT_SECRETS`` — share archives
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.3V0 itself — its parent is ~/ and its
        # directory name is ".3V0", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: 3v0 profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.3V0 itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.3V0). "
            "Specify a different name: 3v0 profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="ev0_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"ev0_{old_name}"
    legacy_old_host = f"3v0.{old_name}"
    new_host = f"ev0_{new_name}"
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("ev0_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a EV0_HOME path string.

    Called early in the CLI entry point, before any 3v0 modules
    are imported, to set the EV0_HOME environment variable.
    """
