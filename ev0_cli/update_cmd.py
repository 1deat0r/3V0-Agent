...
change is that references to helpers/constants that STAY in ``ev0_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``ev0_cli.main`` reference — so existing call sites and test monkeypatches
that target ``ev0_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``ev0_cli.main``.
...
Imports are one-way: ``ev0_cli.main`` imports this module, never the reverse
...
def _m():
    """Lazy ``ev0_cli.main`` reference.

    Lets callers keep patching ``ev0_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``ev0_cli.main`` -> ``ev0_cli.update_cmd`` stays one-way
    at import time.
    """
...
def _reload_config_modules() -> None:
    ...
    This function force-reloads ``ev0_cli.config_defaults``,
    ``ev0_cli.config``, and ``ev0_cli.config_migrations`` from disk
    so subsequent imports read the UPDATED code.

    It also reloads ``ev0_cli._subprocess_compat`` and
    ``ev0_cli.dashboard_procs`` so that post-update dashboard cleanup
    ...
    importlib.invalidate_caches()
    for mod_name in (
        "ev0_cli.config_defaults",
        "ev0_cli.config",
        "ev0_cli.config_migrations",
        "ev0_cli._subprocess_compat",
...
def _run_config_check_fresh() -> tuple:
    ...
    from ev0_cli.config import check_config_version
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    """
    from ev0_constants import FIRST_PARTY_MODULE_ROOTS
...
def _reload_process_scan_modules() -> None:
    ...
    ``from ev0_cli._subprocess_compat import bounded_probe_run``. If the
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``ev0_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``ev0_constants.py``
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False):
    ...
    from ev0_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        from ev0_cli.model_catalog import seed_cache_from_checkout
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    try:
        from ev0_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from ev0_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    """
    from ev0_cli.config import detect_install_method, recommended_update_command_for_method
    ...
    from ev0_cli.gitlock import clear_stale_git_locks
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            from ev0_cli.banner import _github_compare_behind
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    for proc in proc_iter:
        ...
        # the `-m ev0_cli.main gateway run` argv, so autostarted gateways
...
# ``cryptography`` is the canonical case: ``ev0_cli.main`` used to import
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from ev0_cli._scan_venv_blockers import _is_pausable_gateway
...
def _orphaned_desktop_backend_pids(
    ...
    def _is_backend(argv_low: str) -> bool:
        return "ev0_cli.main" in argv_low and (
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m ev0_cli.main
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    when ``ev0_cli.gateway`` itself no longer imports, which is one of the
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    (e.g. importing ``ev0_cli.gateway`` from the freshly pulled checkout)
...
def _rebuild_desktop_after_update(
    ...
    desktop_build_cmd = [sys.executable, "-m", "ev0_cli.main", "desktop", "--build-only"]
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # symbols in ev0_constants or lazy_deps. The dependency install
        ...
        try:
            from ev0_cli.profiles import backfill_profile_envs
        ...
        try:
            ...
            try:
                from ev0_cli.gateway import _get_restart_exit_wait_budget
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state
      (typically the freshly-pulled ``ev0_cli.gateway`` no longer imports,
