...
Extracted verbatim from threev0_state.py so the SessionDB mixin modules
(threev0_state_search / threev0_state_schema / threev0_state_portability) can
reference them without importing threev0_state (which would be a cycle).
threev0_state re-imports every name here for backward compatibility.
"""
...
# optimized via ``3v0 sessions optimize-storage``. A legacy DB sits at
...
# `3v0 sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `3v0 db optimize`. This is
