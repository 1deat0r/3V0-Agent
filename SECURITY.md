# 3V0 Agent Security Policy

This document describes 3V0 Agent's trust model, names the one
...
- Environment details (`3v0 version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted 3V0 Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in 3V0 Agent's documentation
...
3V0 Agent supports two OS-level isolation postures. They address
...
3V0 Agent supports this in two ways:

- **3V0 Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, 3V0 Agent's in-process heuristics
...
3V0 Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in 3V0 Agent
itself. Bugs in 3V0 Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   3V0 Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, 3V0 Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  3V0 Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  3v0-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not 3V0 Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in 3V0 Agent. Bugs in 3V0
...
- 3V0 Agent includes supply-chain guards for MCP server
