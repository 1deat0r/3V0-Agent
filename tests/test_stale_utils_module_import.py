...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.3V0/3v0-agent/utils.py)
...
2. ``3v0 update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``threev0_cli/main.py`` (the ``3v0 update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``threev0_constants`` after the
