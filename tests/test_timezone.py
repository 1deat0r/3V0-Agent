...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When EV0_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) EV0_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("EV0_TIMEZONE=" + os.environ.get("EV0_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "EV0_TIMEZONE=NOT_SET" in result["output"], (
            "EV0_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When EV0_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("EV0_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=ev0_tz) which shifted
        absolute time when system-local tz != 3V0 tz.  The fix interprets
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when 3V0 tz
        ...
        # Use a 3V0 timezone far behind UTC so that the numeric wall time
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of 3V0 timezone"
        )
