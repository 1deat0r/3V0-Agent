...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - the child process resolves threev0_cli.kanban_db on its own
  ...
  - worker logs are captured to EV0_HOME/kanban/logs/<task>.log
    ...
    def _spawn(task, workspace):
        ...
        env = {
            **os.environ,
            ...
            "HOME": home,
            "PYTHONPATH": WT,
            "EV0_KANBAN_TASK": task.id,
            "EV0_KANBAN_WORKSPACE": workspace,
...
def main():
    home = tempfile.mkdtemp(prefix="ev0_e2e_")
    os.environ["EV0_HOME"] = home
    os.environ["HOME"] = home
    sys.path.insert(0, WT)
    from threev0_cli import kanban_db as kb

    # Point the `3v0` CLI child processes will run at the worktree
    # threev0_cli.main. We do this by putting a shim on PATH.
    shim_dir = os.path.join(home, "bin")
    os.makedirs(shim_dir, exist_ok=True)
    shim_path = os.path.join(shim_dir, "3v0")
    with open(shim_path, "w") as f:
        f.write(f"""#!/bin/sh
exec {PY} -m threev0_cli.main "$@"
""")
    ...
    print("=" * 60)
    # Scenario A workers wrote to /tmp/ev0_e2e_*/worker_*.log
    import glob
