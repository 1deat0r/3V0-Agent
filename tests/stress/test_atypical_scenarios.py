...
normal tests assume away:
  ...
  - Filesystem: EV0_HOME with spaces / unicode / symlinks.
...
def scenario(name):
    """Decorator: run `fn` in its own EV0_HOME, collect failures.
    ...
    def wrap(fn):
        def run():
            home = tempfile.mkdtemp(prefix=f"ev0_atyp_{name}_")
...
def _(home, kb):
    ...
    env = {**os.environ, "PYTHONPATH": str(WT), "EV0_HOME": home, "HOME": home}
    bad_metas = [
        "not-json",
...
def _(home, kb):
    ...
    try:
        ...
        try:
            ...
            home_abs = str(Path(os.environ["EV0_HOME"]).resolve())
            if not resolved_abs.startswith(home_abs) and resolved_abs.startswith("/tmp"):
                ...
                print(f"  ⚠ workspace resolved OUTSIDE ev0_home: {resolved}")
...
@scenario("ev0_home_with_spaces")
def _(home, kb):
    """EV0_HOME at a path with spaces — should work but catches
    ...
    weird = tempfile.mkdtemp(prefix="3v0 with spaces ")
    os.environ["EV0_HOME"] = weird
    ...
    try:
        ...
        print(f"  EV0_HOME with spaces: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)


@scenario("ev0_home_with_unicode")
def _(home, kb):
    """EV0_HOME with non-ASCII chars."""
    # Pre-create directly since tempfile doesn't love unicode prefixes
    weird = f"/tmp/ev0_héllo_émöji_{os.getpid()}"
    ...
    try:
        ...
        print(f"  EV0_HOME with unicode path: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)


@scenario("ev0_home_via_symlink")
def _(home, kb):
    """EV0_HOME is a symlink to the real dir. _INITIALIZED_PATHS
    uses Path.resolve() — two different symlink names pointing at the
    same dir should NOT double-init."""
    real = tempfile.mkdtemp(prefix="ev0_real_")
    link1 = real + "_link1"
    link2 = real + "_link2"
    os.symlink(real, link1)
    os.symlink(real, link2)
    try:
        os.environ["EV0_HOME"] = link1
        ...
        conn2.close()
        print("  symlinks to same EV0_HOME share DB correctly")
    finally:
