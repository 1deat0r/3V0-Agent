"""`setup_path()` must also install a `3v0-acp` launcher.

ACP hosts (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving a command
name against the login-shell PATH. The `3v0-acp` console script generated by
the packaging metadata lives inside the install's venv, which is not on that
PATH, so those hosts report 3V0 as not installed. `setup_path()` therefore
writes a `3v0-acp` launcher next to the `3v0` one.
...
ACP_BLOCK = re.compile(
    r'(    rm -f "\$command_link_dir/3v0-acp".*?'
    r'log_success "Installed 3v0-acp launcher[^\n]*\n)',
    re.S,
)
...
def _extract_acp_shim_block() -> str:
    ...
    assert match, (
        "could not locate the 3v0-acp launcher block in scripts/install.sh — "
...
def _run_block(tmp_path: Path, use_venv: str) -> Path:
    ...
    return command_link_dir / "3v0-acp"
...
def test_acp_launcher_does_not_follow_a_symlink_into_the_venv(tmp_path):
    ...
    An older install could leave `3v0-acp` as a symlink to the venv console
    ...
    shim_path = command_link_dir / "3v0-acp"
    ...
    assert console_script.read_text(encoding="utf-8") == marker, (
        "venv/bin/3v0-acp was overwritten — symlink-stomp regression (#21454)"
    )
    assert not shim_path.is_symlink(), (
        "command_link_dir/3v0-acp must be replaced with a regular file"
    )


# ---------------------------------------------------------------------------
# 3v0-agent launcher regression (#74819)
# ---------------------------------------------------------------------------

EV0_AGENT_BLOCK = re.compile(
    r'(rm -f "\$command_link_dir/3v0-agent".*?'
    r'log_success "Installed 3v0-agent launcher[^\n]*\n)',
    re.S,
)
...
def test_threev0_agent_launcher_cleanup_on_uninstall(tmp_path):
    ...
    assert local_shim in removed, "local 3v0-agent wrapper must be removed"
