"""Tests for pip entry-point provider discovery (ev0_agent.plugins group).

Verifies that ``providers/__init__.py`` imports provider plugins exposed via a
distribution's ``ev0_agent.plugins`` entry point, supporting both a
...
def _enable(monkeypatch, *names, disabled=()):
    ...
    """
    import threev0_cli.plugins as hp
