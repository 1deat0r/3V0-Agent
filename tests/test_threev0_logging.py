"""Tests for threev0_logging — centralized logging setup."""
import io
import logging
import os
import stat
...
import threev0_logging
# Use whatever RotatingFileHandler class threev0_logging actually resolved so
# the autouse fixture's isinstance checks (which strip rotating handlers
# between tests) match the real handlers on every platform. threev0_logging
...
from threev0_logging import RotatingFileHandler
...
def _reset_logging_state():
    ...
    """
    threev0_logging._logging_initialized = False
    ...
    threev0_logging._reset_queued_handlers()
    ...
    threev0_logging._install_session_record_factory()
    yield
    ...
    threev0_logging._logging_initialized = False
    threev0_logging.clear_session_context()
...
class TestSetupLogging:
    ...
    def test_creates_agent_log_handler(self, threev0_home):
        ...
        agent_handlers = [
            h for h in threev0_logging.rotating_file_handlers()
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = threev0_logging._ComponentFilter(("gateway",))
...
class TestAddRotatingHandler:
    ...
    def test_no_duplicate_for_same_path(self, tmp_path):
        ...
        threev0_logging._add_rotating_handler(
            logger, log_path,
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        handlers = [h for h in threev0_logging.rotating_file_handlers() if isinstance(h, RotatingFileHandler)]
        ...
        threev0_logging.set_session_context("factory_test")
...
class TestWindowsConcurrentLogLockTimeout:
    ...
    def _make_logger_and_handler(self, log_path: Path):
        ...
        handler = threev0_logging._ManagedRotatingFileHandler(
    ...
    def test_helper_only_matches_windows_concurrent_lock_timeout(self):
        ...
        assert threev0_logging._is_windows_concurrent_log_lock_timeout(
            RuntimeError("Cannot acquire lock after 20 attempts")
        )
        assert not threev0_logging._is_windows_concurrent_log_lock_timeout(
...
class TestExternalRotationRecovery:
    ...
    def _make_handler(self, log_path: Path) -> threev0_logging._ManagedRotatingFileHandler:
        handler = threev0_logging._ManagedRotatingFileHandler(
    ...
    def _emit(self, handler: logging.Handler, msg: str) -> None:
        ...
        # Match the record factory that threev0_logging installs at import time.
        record.session_tag = ""
        handler.emit(record)
        threev0_logging.flush_log_queue()
...
class TestSafeStderr:
    ...
    def test_wraps_non_utf8_stderr(self, monkeypatch):
        ...
        result = threev0_logging._safe_stderr()
...
class TestAsyncQueueLogging:
    ...
    def test_file_handlers_not_on_root(self, threev0_home):
        ...
        assert any(
            ...
            for h in threev0_logging.rotating_file_handlers()
        )
