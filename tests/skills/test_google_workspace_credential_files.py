...
class TestGoogleWorkspaceCredentialFiles:
    ...
    def test_entries_are_registered_when_files_exist(self, tmp_path):
        ...
        threev0_home.mkdir()
        (threev0_home / "google_token.json").write_text("{}")
        (threev0_home / "google_client_secret.json").write_text("{}")
        ...
        try:
            ...
            assert "/root/.3V0/google_token.json" in container_paths
            assert "/root/.3V0/google_client_secret.json" in container_paths
        finally:
            clear_credential_files()
