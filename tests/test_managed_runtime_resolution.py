"""Guard: 3V0-owned subprocesses must not resolve managed runtimes by bare PATH.

3V0 installs runtimes for itself — ``uv`` at ``$EV0_HOME/bin/uv``, Node at
``$EV0_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in 3V0's own
...
* when a system copy also exists, the one 3V0 does not own wins — which is
...
``iter_ev0_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes 3V0 provisions into EV0_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not 3V0-owned subprocess code: plugins ship their own
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a 3V0-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$EV0_HOME/bin (uv) and $EV0_HOME/node are not on an "
        ...
        "  node/npm -> threev0_constants.find_node_executable()\n"
        "  PATH env -> threev0_constants.iter_ev0_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(threev0_constants, helper))


def test_managed_uv_helpers_exist():
    from threev0_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
