...
class TestGeneratedSystemdUnits:
    ...
    def test_launchd_plist_persists_configured_nofile_soft_limit(self, monkeypatch):
        ...
        import ev0_cli.resource_limits as resource_limits
...
class TestSystemUnitEv0Home:
    ...
    def test_node_path_lookup_remains_fallback_without_managed_node(
        ...
        monkeypatch.setattr(
            "ev0_constants.iter_ev0_node_dirs", lambda root=None: []
        )
        monkeypatch.setattr(
            "ev0_constants.ev0_managed_node_tree_present",
...
class TestLegacyEv0UnitDetection:
    ...
    _OUR_UNIT_TEXT = (
        ...
        "ExecStart=/usr/bin/python -m ev0_cli.main gateway run --replace\n"
    )
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m ev0_cli.main gateway run
          - python path/to/ev0_cli/main.py gateway run
          - 3v0 gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m ev0_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/3v0/ev0_cli/main.py gateway run",
...
class TestMigrateLegacyCommand:
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        import ev0_cli.main as cli_main
        ...
        if project_root is None:
            import ev0_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "ev0_cli.main", "gateway", "--help"],
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "ev0_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemdUnitPathQuoting:
    ...
    def test_whitespace_python_path_is_double_quoted(self, monkeypatch):
        ...
        assert 'ExecStart="/tmp/Some Dir/.venv/bin/python" -m ev0_cli.main' in unit
    ...
    def test_plain_python_path_stays_unquoted(self, monkeypatch):
        ...
        unit = gateway_cli.generate_systemd_unit()
        assert "ExecStart=/usr/bin/python -m ev0_cli.main" in unit
