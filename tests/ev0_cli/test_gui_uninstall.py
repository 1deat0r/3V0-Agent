"""Tests for ev0_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import ev0_cli.gui_uninstall as gu


def _make_agent(hermes_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = hermes_home / "hermes-agent"
    (agent_root / "ev0_cli").mkdir(parents=True)
    (agent_root / "ev0_cli" / "__init__.py").write_text("")
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    assert (hermes_home / "hermes-agent" / "ev0_cli").is_dir()
