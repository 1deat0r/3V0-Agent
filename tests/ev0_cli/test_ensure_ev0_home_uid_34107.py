"""Regression tests for #34107 — Docker UID/GID handling in ensure_ev0_home.

When 3V0 runs in Docker with ``EV0_UID=1000`` / ``EV0_GID=911``,
the entrypoint chowns the top-level ``EV0_HOME`` once at startup. But
...
The fix is a ``_chown_to_ev0_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_ev0_uid_gid
# ---------------------------------------------------------------------------


class TestResolveEv0UidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        ...
        from ev0_cli.config import _resolve_ev0_uid_gid
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("EV0_UID", raising=False)
        monkeypatch.delenv("EV0_GID", raising=False)
        from ev0_cli import config as cfg
