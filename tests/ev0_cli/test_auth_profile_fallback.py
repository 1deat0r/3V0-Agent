...
When ``EV0_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.3V0/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.3V0            (has its own auth.json fixture)
    * Profile     -> tmp_path/.3V0/profiles/coder   (active, EV0_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"3V0 is not
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (EV0_HOME != ~/.3V0, no profiles)."""
