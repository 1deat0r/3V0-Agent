...
from ev0_cli.main import cmd_update, PROJECT_ROOT
...
def _patch_managed_uv(request):
    ...
    with patch("ev0_cli.managed_uv.resolve_uv", side_effect=_fake_resolve_uv), \
         patch("ev0_cli.managed_uv.ensure_uv", side_effect=_fake_ensure_uv), \
         patch("ev0_cli.managed_uv.update_managed_uv", side_effect=_fake_update_managed_uv):
        yield
...
def _patch_gateway_discovery():
    ...
    """
    with patch("ev0_cli.gateway.find_gateway_pids", return_value=[]), \
         patch("ev0_cli.gateway.supports_systemd_services", return_value=False), \
         patch("ev0_cli.gateway.find_profile_gateway_processes", return_value=[]):
        yield
...
class TestCmdUpdateNpmLockfileCache:
    ...
    def test_record_npm_lockfile_hash(self, tmp_path, monkeypatch):
        from ev0_cli import main as hm
    ...
    def test_package_json_only_edit_defeats_skip(self, tmp_path, monkeypatch):
        ...
        npm — lockfile unchanged. `3v0 update` must still install (the
    ...
    def test_update_uses_one_shared_npm_cache_across_profiles(
        ...
        shared_root = tmp_path / ".3V0"
        named_profile = shared_root / "profiles" / "work"
        named_profile.mkdir(parents=True)

        monkeypatch.setattr(hm, "PROJECT_ROOT", checkout)
        monkeypatch.setattr(ev0_constants.Path, "home", lambda: tmp_path)
        monkeypatch.setattr(
            ev0_constants, "find_node_executable", lambda _name: "/usr/bin/npm"
        )
        ...
        with patch.object(
            ...
            monkeypatch.setenv("EV0_HOME", str(shared_root))
            hm._update_node_dependencies()

            monkeypatch.setenv("EV0_HOME", str(named_profile))
            hm._update_node_dependencies()
...
class TestCmdUpdateTermuxUvBootstrap:
    ...
    def test_termux_reuses_existing_path_uv_without_pip(self, mock_run, monkeypatch):
        ...
        monkeypatch.setattr(hm, "_is_termux_env", lambda env=None: True)
        # Production resolve_uv only checks $EV0_HOME/bin/uv; model an empty
        # managed dir so the PATH probe is what surfaces the packaged uv.
        monkeypatch.setattr("ev0_cli.managed_uv.resolve_uv", lambda: None)
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_on_fork_checks_upstream_when_origin_up_to_date(
        ...
        origin but behind NousResearch/3v0-agent silently misses updates.
        """
        from ev0_cli import main as hm
        ...
        with patch.object(
            hm,
            "_get_origin_url",
            return_value="https://github.com/example/3v0-agent.git",
    ...
    def test_update_non_interactive_runs_safe_config_migrations(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "ev0_cli.config.get_missing_env_vars", return_value=["MISSING_KEY"]
        ), patch(
            "ev0_cli.config.get_missing_config_fields",
            return_value=[{"key": "new.option", "default": True}],
        ), patch(
            "ev0_cli.update_cmd._reload_config_modules"
        ), patch(
            "ev0_cli.update_cmd._run_config_check_fresh", return_value=(1, 2)
        ), patch(
            "ev0_cli.update_cmd._run_migrate_config_fresh",
            return_value={"env_added": [], "config_added": ["new.option"]},
        ) as migrate_config, patch("ev0_cli.main.sys") as mock_sys:
...
class TestCmdUpdateMigrationPrompt:
    ...
    def test_version_bump_only_applies_silently_without_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "ev0_cli.config.get_missing_env_vars", return_value=[]
        ), patch(
            "ev0_cli.config.get_missing_config_fields", return_value=[]
        ), patch(
            "ev0_cli.update_cmd._reload_config_modules"
        ), patch(
            "ev0_cli.update_cmd._run_config_check_fresh", return_value=(5, 24)
        ), patch(
            "ev0_cli.update_cmd._run_migrate_config_fresh",
    ...
    def test_version_bump_only_surfaces_migration_resets(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "ev0_cli.update_cmd._run_config_check_fresh", return_value=(33, 34)
        ), patch(
            "ev0_cli.update_cmd._run_migrate_config_fresh",
            return_value={
                "env_added": [],
    ...
    def test_new_options_are_listed_by_name_before_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "ev0_cli.config.get_missing_env_vars", return_value=env_items
        ), patch(
            "ev0_cli.config.get_missing_config_fields", return_value=cfg_items
        ), patch(
            "ev0_cli.update_cmd._reload_config_modules"
        ), patch(
            "ev0_cli.update_cmd._run_config_check_fresh", return_value=(1, 24)
        ), patch(
            "ev0_cli.update_cmd._run_migrate_config_fresh",
            return_value={"env_added": [], "config_added": [], "warnings": []},
        ), patch("ev0_cli.main.sys") as mock_sys:
...
class TestConfigVersionCheckUsesFreshModules:
    ...
    Before the fix, ``3v0 update`` ran in the PRE-pull Python process.
    After ``git pull`` updated the source on disk, function-level imports
    returned the OLD cached ``ev0_cli.config`` module — so
    ...
    def test_run_config_check_fresh_reloads_modules(self):
        ...
        sys.modules held the OLD ev0_cli.config with the OLD
        DEFAULT_CONFIG["_config_version"] after git pull.
        """
        from unittest.mock import patch

        import ev0_cli.update_cmd as update_cmd
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        default_p = SimpleNamespace(name="default", path=Path("/fake/.3V0"))
        active_p = SimpleNamespace(name="bit", path=Path("/fake/.3V0/profiles/bit"))
        other_p = SimpleNamespace(name="work", path=Path("/fake/.3V0/profiles/work"))
        ...
        with (
            patch("ev0_cli.profiles.list_profiles", return_value=all_profiles),
            patch("ev0_cli.profiles.seed_profile_skills", side_effect=fake_seed),
    ...
    def test_single_profile_default_is_synced(
        ...
        with (
            patch("ev0_cli.profiles.list_profiles", return_value=[default_p]),
            patch("ev0_cli.profiles.seed_profile_skills", side_effect=fake_seed),
            patch("tools.skills_sync.sync_skills", return_value=empty_sync),
        ):
            cmd_update(mock_args)
...
class TestCmdUpdateBranchFlag:
    """``3v0 update --branch <name>`` targets the requested branch.
...
class TestCmdUpdateCheckBranchFlag:
    """``3v0 update --check --branch <name>`` honors the branch override.
    ...
    @patch("ev0_cli.config.detect_install_method", return_value="git")
    @patch("subprocess.run")
...
class TestCmdUpdateZipBranchRefusal:
    """``3v0 update --branch=<non-main>`` must refuse on the ZIP fallback path.
    ...
    def test_zip_fallback_refuses_non_main_branch(self, capsys):
        from ev0_cli.main import _update_via_zip
...
class TestNodeRuntimeNpmResolution:
    ...
    def test_wsl_update_skips_windows_npm_build_paths(self, mock_args, monkeypatch):
        ...
        monkeypatch.setattr(ev0_constants, "is_wsl", lambda: True)
        monkeypatch.setattr(
            ev0_constants,
