...
ev0_cli.config.load_config, so the managed overlay has to be wired into each.
...
def test_timezone_honors_managed(homes, monkeypatch):
    home, managed = homes
    # ev0_time checks an env override first; ensure it's unset so config wins.
    ...
    assert ev0_time._resolve_timezone_name() == "Asia/Tokyo"
