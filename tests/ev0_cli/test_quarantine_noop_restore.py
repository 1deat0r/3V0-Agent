...
``3v0*.exe`` shims aside (``3v0.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``3v0`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("3v0", "3v0-agent", "3v0-acp", "3v0-gateway"):
...
# ---------------------------------------------------------------------------
# ev0_cli.main._run_quarantined_install
# ---------------------------------------------------------------------------
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "3v0.exe" in names, "3v0.exe must be restored after a no-op install"
    assert "3v0-acp.exe" in names
    assert "3v0-gateway.exe" in names
    assert (scripts / "3v0.exe").read_bytes() == b"MZ-old-3v0"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "3v0.exe").read_bytes() == b"MZ-new-3v0"
...
# ---------------------------------------------------------------------------
# ev0_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "3v0.exe" in names, "3v0.exe must be restored after a no-op recovery install"
    assert (scripts / "3v0.exe").read_bytes() == b"MZ-old-3v0"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "3v0.exe").write_bytes(b"MZ-new-3v0")
