"""Tests for ev0_cli.managed_uv — one path, no guessing."""
...
def _runtime_info(
    ...
    from ev0_cli.sqlite_runtime import SQLiteRuntimeInfo
...
def _RRR(status):
    ...
    """
    from ev0_cli.managed_uv import RuntimeRepairResult
...
class TestEnsureUv:
    ...
    def test_install_reports_runtime_repair_to_observer(self, tmp_path):
        from ev0_cli.managed_uv import (
            RuntimeRepairResult,
            ensure_uv,
        )
...
class TestUpdateManagedUv:
    ...
    def test_fresh_stamp_skips_network_self_update_but_not_repair(self, tmp_path, monkeypatch):
        ...
        from ev0_cli.managed_uv import RuntimeRepairResult, update_managed_uv
    ...
    def test_stale_stamp_runs_self_update_and_refreshes_stamp(self, tmp_path):
        ...
        from ev0_cli.managed_uv import UV_SELF_UPDATE_INTERVAL_SECONDS, update_managed_uv
...
class TestManagedPythonStore:
    ...
    def test_environment_is_private_and_sanitized(self, tmp_path):
        from ev0_cli.managed_uv import managed_python_env
...
class TestRuntimeRepair:
    def test_safe_runtime_is_a_noop(self, tmp_path):
        from ev0_cli.managed_uv import repair_vulnerable_runtime

        root, live, sentinel = _make_runtime_install(tmp_path)
        current = _runtime_info(live / "bin" / "python", (3, 53, 1))
        with patch(
                 "ev0_cli.managed_uv.probe_sqlite_runtime",
                 return_value=current,
             ), \
             patch(
                 "ev0_cli.managed_uv._install_safe_python_generation"
             ) as mock_install:
    ...
    def test_stage_candidate_sync_keeps_uv_project_config(self, tmp_path):
        from ev0_cli.managed_uv import _stage_candidate_venv
        ...
        with patch("ev0_cli.managed_uv.subprocess.run", side_effect=fake_run), \
             patch(
                 "ev0_cli.managed_uv._smoke_candidate_venv",
    ...
    def test_failed_candidate_preserves_live_venv(self, tmp_path):
        ...
        with patch(
             ...
             patch(
                 "ev0_cli.managed_uv._stage_candidate_venv",
                 return_value=None,
             ):
            result = repair_vulnerable_runtime("uv", project_root=root)
...
class TestInstallUvInternals:
    def test_posix_sets_uv_unmanaged_install(self, tmp_path):
        ...
        with patch("ev0_cli.managed_uv._install_uv_posix") as mock_posix:
            from ev0_cli.managed_uv import _install_uv
            _install_uv(target)
...
class TestRuntimeRequestMinorLine:
    ...
    def test_requests_minor_line(self):
        from ev0_cli.managed_uv import _runtime_request
...
class TestRepairRetriesAfterUvRefresh:
    def _run_repair(self, tmp_path, *, refresh_result, second_attempt):
        ...
        with patch(
             ...
             patch(
                 "ev0_cli.managed_uv._refresh_managed_uv_catalog",
...
class TestDefaultLiveVenv:
    ...
    def test_dot_venv_only_is_targeted(self, tmp_path):
        from ev0_cli.managed_uv import _default_live_venv
...
class TestVenvPythonUpdateBoundary:
    ...
    def test_recovers_when_the_cached_module_predates_the_symbol(self, monkeypatch):
        ...
        from ev0_cli.managed_uv import _venv_python
        ...
        monkeypatch.delattr(ev0_constants, "venv_python_path", raising=False)
