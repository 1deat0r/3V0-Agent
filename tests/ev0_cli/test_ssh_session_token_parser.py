...
from ev0_cli.main import _read_ssh_session_token_file, cmd_dashboard
...
def test_token_anchor_is_os_home_not_active_profile(tmp_path, monkeypatch):
    ...
    ``$HOME/.3V0/desktop-ssh`` (a literal ``~/.3V0/desktop-ssh`` in
    apps/desktop/electron/remote-lifecycle.ts, expanded against the account's
    $HOME). A non-default sticky profile re-homes ``get_ev0_home()`` to
    ``<root>/profiles/<name>``, and a Docker-style ``EV0_HOME`` can point
    elsewhere entirely — neither must move the validator off
    ``$HOME/.3V0/desktop-ssh``, or the token is wrongly rejected."""
    home = tmp_path / "home"
    monkeypatch.setenv("HOME", str(home))
    token_dir = home / ".3V0" / "desktop-ssh" / ("a" * 32)
    token_dir.mkdir(parents=True, mode=0o700)
    token_path = token_dir / "0123456789abcdef.token"

    # A sticky profile and a custom (Docker) root both point get_ev0_home()
    # away from $HOME/.3V0; the anchor must ignore both.
...
def test_token_under_profile_desktop_ssh_is_rejected(tmp_path, monkeypatch):
    ...
    home = tmp_path / "home"
    monkeypatch.setenv("HOME", str(home))
    profile_home = home / ".3V0" / "profiles" / "coder"
...
def test_token_file_rejects_parent_escape(tmp_path, monkeypatch):
    home = tmp_path / "home"
    monkeypatch.setenv("HOME", str(home))
    token_root = home / ".3V0" / "desktop-ssh"
...
def test_windows_runtime_root_stays_at_machine_root_for_named_profile(tmp_path, monkeypatch):
    from ev0_cli import windows_ssh_runtime

    machine_root = tmp_path / "custom-3v0-root"
    monkeypatch.setenv("EV0_HOME", str(machine_root / "profiles" / "writer_2"))
