...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        """
        import ev0_cli.oneshot as oneshot
        from ev0_cli.main import _exit_after_oneshot
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "ev0_state", mod("ev0_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "ev0_cli.config",
        mod("ev0_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "ev0_cli.models",
        mod("ev0_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    monkeypatch.setitem(
        sys.modules,
        "ev0_cli.tools_config",
        mod("ev0_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
