"""Tests for ev0_cli.container_boot — the cont-init.d-time
reconciliation that recreates per-profile gateway s6 service slots
from the persistent profiles directory.

These tests run against a fake $EV0_HOME under tmp_path; no real
...
from ev0_cli.container_boot import (
    ReconcileAction,
...
def _hermetic_container_argv(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    fallback). On a host that is *also* running 3v0 containers, those
    ...
    """
    monkeypatch.setattr(
        "ev0_cli.container_boot._read_container_argv",
        lambda: (),
    )
...
def _make_profile(
    ...
    *,
    ...
    if config:
        ...
        # `3v0 profile create`. See container_boot._render_run_script.
...
def test_main_skips_reconcile_in_dashboard_container_s6v3(
    ...
    """
    from ev0_cli import container_boot
    ...
    monkeypatch.setattr(
        ...
        lambda: (
            ...
            "top",
            "/opt/3v0/docker/main-wrapper.sh",
            "dashboard",
            "--host",
            "0.0.0.0",
            "--port",
