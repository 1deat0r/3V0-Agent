...
Complements tests/ev0_cli/test_kanban_db.py (schema + CAS atomicity)
and tests/ev0_cli/test_kanban_cli.py (end-to-end run_slash).  The
...
def kanban_home(tmp_path, monkeypatch):
    ...
    # tests in tests/ev0_cli/test_kanban_db.py.
    monkeypatch.setenv("EV0_KANBAN_CRASH_GRACE_SECONDS", "0")
    kb.init_db()
    return home
...
def test_cli_daemon_help_marks_deprecated():
    ...
    from ev0_cli import kanban as kb_cli
...
def _drive_worker_exit(conn, tid, fake_pid, raw_status):
    ...
    Deliberately resolves ``ev0_cli.kanban_db`` fresh and uses that single
