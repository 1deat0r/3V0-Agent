"""Tests for the Kanban DB layer (ev0_cli.kanban_db)."""
...
def test_stale_claim_reclaim_event_records_diagnostic_payload(
    ...
    timing context)."""
    import json
    import ev0_cli.kanban_db as _kb
...
def test_rate_limit_exit_requeues_without_counting_failure(
    ...
    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.setenv("EV0_KANBAN_CRASH_GRACE_SECONDS", "0")
...
def test_respawn_guard_defers_rate_limited_within_cooldown(
    ...
    monkeypatch.setenv("EV0_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "300")
    now = 5_000_000
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """3V0' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` 3V0' DB/metadata/logs on task completion.
    """
...
# spawned with `3v0 -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's EV0_HOME.
# ---------------------------------------------------------------------------
...
class TestSharedBoardPaths:
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.3V0/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.3V0/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.3V0/kanban.db.
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # EV0_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile EV0_HOME and reads.
    ...
    def test_dispatcher_spawn_injects_kanban_paths_without_stale_session(
        ...
        # EV0_SESSION_* identity inherited from the long-lived gateway.
        # The one exception is EV0_SESSION_SOURCE, which the dispatcher
        ...
        env = captured["env"]
        assert env["EV0_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["EV0_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["EV0_KANBAN_TASK"] == "t_dispatch_env"
        assert env["EV0_KANBAN_BRANCH"] == "wt/t_dispatch_env"
        for key in sc._VAR_MAP:
            if key == "EV0_SESSION_SOURCE":
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see ev0_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    import ev0_state as _ev0_state
    monkeypatch.setattr(
        _ev0_state, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
    _ev0_state._wal_fallback_warned_paths.clear()

    # Clear module cache so a fresh connect() is attempted
    kb._INITIALIZED_PATHS.clear()
    ev0_state._wal_fallback_warned_paths.clear()
    ...
    with _patch("ev0_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("ERROR", logger="ev0_state"):
            conn = kb.connect()
...
def test_connect_works_when_wal_is_silently_refused(tmp_path, monkeypatch, caplog):
    ...
    with _patch(
        "ev0_cli.kanban_db.sqlite3.connect",
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual '3v0 kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_ev0_argv()
#
# Workers spawned by the dispatcher must use a `3v0` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["3v0", ...]`
...
def test_resolve_ev0_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m ev0_cli.main`.

    Pins the correct module name (NOT `3v0` — there is no top-level
    `3v0` package). Regression for #23198: the original PR shipped
    `python -m 3v0` which fails with `No module named 3v0` on every
    invocation.
    """
    import shutil
    import sys
    import ev0_cli.kanban_db as kb

    monkeypatch.delenv("EV0_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kb._resolve_ev0_argv()
    assert argv == [sys.executable, "-m", "ev0_cli.main"]
...
def test_resolve_ev0_argv_module_actually_runs():
    ...
    sufficient — if `ev0_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m ev0_cli.main --version`
    ...
    assert "3V0 Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="ev0_cli.kanban_db"):
        with kb.connect() as conn:
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    """
    import struct
    from ev0_cli.kanban_db import connect
