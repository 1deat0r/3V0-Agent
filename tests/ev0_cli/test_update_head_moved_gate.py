...
def _patch_update_deps(monkeypatch, tmp_path, run_side_effect):
    """Patch the ev0_cli.main helpers ``_cmd_update_impl`` touches.

    ``_m()`` in update_cmd.py lazily returns ev0_cli.main, so patching
    attributes on that module is the canonical test surface (matches
    tests/ev0_cli/test_cmd_update.py).
    """
    ...
    import ev0_cli.gateway as ev0_gateway
