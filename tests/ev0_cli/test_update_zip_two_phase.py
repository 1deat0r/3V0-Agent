...
from ev0_constants import venv_bin_dir, venv_python_path
...
def test_commit_swaps_every_entry(tmp_path):
    ...
    assert not [p for p in os.listdir(live) if "3v0-update" in p]
...
def test_staging_clears_leftovers_from_an_interrupted_run(tmp_path):
    ...
    stale = Path(f"{live / 'agent'}.3v0-update-staging")
    stale.mkdir()
...
def test_no_open_coded_venv_layout_remains_in_ev0_cli():
    ...
    exempt = {"stdio.py"}
    pkg = Path(ev0_cli.__file__).parent
    offenders = []
    ...
    assert not offenders, (
        "open-coded venv layout found (use ev0_constants.venv_bin_dir):\n"
        + "\n".join(offenders)
    )
...
def test_top_level_files_are_swapped_atomically(tmp_path):
    ...
    ev0_constants.py, ...). Covering only directories would leave exactly
...
def test_failed_staging_leaves_no_orphaned_copies(tmp_path, monkeypatch):
    ...
    leftovers = [p for p in os.listdir(live) if "3v0-update" in p]
...
def test_venv_helpers_honour_an_explicit_platform_verdict():
    ...
    `ev0_cli.main._is_windows`. A helper that reads `sys.platform`
...
def test_staging_restores_backup_when_dst_is_missing(tmp_path, monkeypatch):
    ...
    backup = live / "agent.3v0-update-old"
    backup.mkdir()
...
def test_commit_failure_plus_discard_leaves_no_staging_litter(tmp_path, monkeypatch):
    ...
    litter = [p for p in os.listdir(live) if "3v0-update" in p]
