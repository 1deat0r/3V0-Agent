"""Tests for ev0_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("ev0_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from ev0_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    ...
    @patch("ev0_cli.config.load_config")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from ev0_cli.skills_config import _get_categories
        skills = [
