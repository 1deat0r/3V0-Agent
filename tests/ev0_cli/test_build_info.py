"""Tests for ev0_cli.build_info — baked-in build SHA resolution.

The build SHA is written by the Dockerfile's ``EV0_GIT_SHA`` build-arg
into ``<project_root>/.ev0_build_sha``.  These tests cover the read-side
...
def test_get_build_sha_returns_none_when_file_absent(tmp_path):
    ...
    from ev0_cli import build_info
