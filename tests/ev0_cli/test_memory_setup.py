...
import ev0_cli.memory_setup as memory_setup
...
def test_cmd_setup_generic_choice_cancel_writes_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(memory_setup, "get_ev0_home", lambda: tmp_path)
    monkeypatch.setattr("ev0_cli.config.load_config", lambda: {"memory": {}})
...
def test_install_dependencies_force_reinstalls_versioned_specs(tmp_path, monkeypatch):
    ...
    so a downgraded/stripped bridge package is restored on 3v0 update."""
