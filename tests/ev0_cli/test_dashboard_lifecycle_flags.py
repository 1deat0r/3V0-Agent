"""Tests for ``3v0 dashboard --stop`` / ``--status`` flags.

These flags share the detection + kill path with the post-``3v0 update``
...
from ev0_cli.main import cmd_dashboard
...
class TestDashboardStatus:
    def test_status_no_processes(self, capsys):
        with patch("ev0_cli.main._scan_dashboard_processes", return_value=[]), \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(status=True))
        assert exc.value.code == 0
        out = capsys.readouterr().out
        assert "No 3v0 dashboard processes running" in out

    def test_status_with_processes(self, capsys):
        processes = [
            (12345, "3v0 dashboard --port 9119"),
            (12346, "python -m ev0_cli.main dashboard --host 0.0.0.0 --port 9120"),
        ]
        with patch("ev0_cli.main._scan_dashboard_processes", return_value=processes), \
             patch("gateway.status._pid_exists", return_value=True), \
             patch("ev0_cli.main._dashboard_listening", return_value=True), \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(status=True))
        # Status is informational — always exits 0.
        assert exc.value.code == 0
        out = capsys.readouterr().out
        assert "2 3v0 dashboard process(es) running" in out
...
class TestDashboardStop:
    ...
    def test_stop_exits_nonzero_if_kill_leaves_survivors(self):
        ...
        with patch("ev0_cli.main._find_stale_dashboard_pids",
                   side_effect=lambda: next(scans)), \
             patch("ev0_cli.main._kill_stale_dashboard_processes"), \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(stop=True))
...
class TestLifecycleFlagsTakePrecedence:
    ...
    who typed ``3v0 dashboard --stop`` must not end up ALSO starting
    a new server."""
    ...
    def test_stop_does_not_fall_through_to_server_start(self):
        ...
        with patch("ev0_cli.main._find_stale_dashboard_pids",
                   return_value=[]), \
             patch.dict(sys.modules, {"ev0_cli.web_server": fake_ws}), \
...
class TestArgparseWiring:
    ...
    ``3v0 dashboard --stop`` / ``--status`` actually parse."""

    def test_flags_are_registered(self):
        from ev0_cli.main import main as _cli_main  # noqa: F401
        ...
        import importlib
        mod = importlib.import_module("ev0_cli.main")
