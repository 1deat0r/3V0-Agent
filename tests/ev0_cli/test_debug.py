...
def ev0_home(tmp_path, monkeypatch):
    ...
    (logs_dir / "gui.log").write_text(
        "2026-04-12 17:00:12 INFO ev0_cli.web_server: dashboard request\n"
    )
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from ev0_cli.debug import _upload_paste_rs
        ...
        with patch("ev0_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "ev0_cli.debug.urllib.request.urlopen",
...
class TestUploadToPastebin:
    ...
    def test_falls_back_to_dpaste_com(self):
        from ev0_cli.debug import upload_to_pastebin

        with patch("ev0_cli.debug._upload_paste_rs",
                    side_effect=Exception("down")), \
             patch("ev0_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_keeps_first_line_when_truncation_on_boundary(self, ev0_home):
        ...
        from ev0_cli.debug import _capture_log_snapshot
...
class TestMissingLogNote:
    ...
    def test_report_carries_the_note_for_a_remote_backend(self, ev0_home):
        ...
        from ev0_cli.debug import collect_debug_report
...
class TestCaptureLogSnapshotRedaction:
    ...
    def test_capture_default_log_snapshots_threads_redact(
        ...
        from ev0_cli.debug import _capture_default_log_snapshots
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, ev0_home):
        ...
        with patch("ev0_cli.dump.run_dump") as mock_dump:
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, ev0_home, capsys):
        ...
        from ev0_cli.debug import run_debug_share
        ...
        with patch("ev0_cli.dump.run_dump"), \
             patch("ev0_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
             patch("ev0_cli.debug.upload_to_pastebin",
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        ...
        with patch("ev0_cli.dump.run_dump"), \
             patch("ev0_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("ev0_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from ev0_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from ev0_cli.debug import delete_paste
        ...
        with patch("ev0_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    def test_records_pending_to_json(self, ev0_home):
        ...
        from ev0_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_dedupes_same_url(self, ev0_home):
        ...
        from ev0_cli.debug import _schedule_auto_delete, _load_pending
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_deletes_expired_entries(self, ev0_home):
        ...
        with patch("ev0_cli.debug.delete_paste", side_effect=fake_delete):
    ...
    def test_sweep_leaves_future_entries_alone(self, ev0_home):
        from ev0_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("ev0_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, ev0_home):
        ...
        with patch("ev0_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)
...
class TestRunDebugDelete:
    ...
    def test_handles_delete_failure(self, capsys):
        from ev0_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("ev0_cli.debug.delete_paste",
...
class TestBuildDebugShare:
    ...
    def test_redaction_keeps_secrets_out_of_payload(self, ev0_home):
        ...
        with patch("ev0_cli.dump.run_dump"), patch(
            "ev0_cli.debug.upload_to_pastebin", side_effect=_upload
        ), patch("ev0_cli.debug._schedule_auto_delete"):
...
class TestCollectShareBundle:
    ...
    def test_no_redact_omits_banner(self, ev0_home):
        from ev0_cli.debug import collect_share_bundle
...
class TestBuildNousBundle:
    def test_envelope_shape_and_gzip(self, ev0_home):
        ...
        from ev0_cli.debug import build_nous_bundle
...
class TestRunDebugShareNous:
    ...
    def test_nous_success_prints_view_url(self, ev0_home, capsys):
        ...
        with patch("ev0_cli.dump.run_dump"), patch(
            "ev0_cli.diagnostics_upload.share_to_nous", return_value=res
        ) as share:
    ...
    def test_nous_failure_suggests_local(self, ev0_home, capsys):
        ...
        with patch("ev0_cli.dump.run_dump"), patch(
            "ev0_cli.diagnostics_upload.share_to_nous",
    ...
    def test_nous_does_not_touch_pastebin(self, ev0_home):
        ...
        with patch("ev0_cli.dump.run_dump"), patch(
            "ev0_cli.diagnostics_upload.share_to_nous", return_value=res
        ), patch("ev0_cli.debug.upload_to_pastebin") as paste:
            run_debug_share(self._args())
...
class TestDebugSlashCommand:
    ...
    def _captured(self, cmd_original):
        ...
        with patch("ev0_cli.debug.run_debug_share", _fake_run):
...
class TestShareConsentGate:
    ...
    def test_non_interactive_requires_yes(self, ev0_home, capsys, monkeypatch):
        ...
        with patch("ev0_cli.dump.run_dump"), \
             patch("ev0_cli.debug.upload_to_pastebin") as mock_upload:
            with pytest.raises(SystemExit) as exc:
                run_debug_share(self._args())
