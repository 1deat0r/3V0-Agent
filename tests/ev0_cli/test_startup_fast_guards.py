"""Guards for ev0_cli._startup_fast — the pre-import version fast path.
...
1. IMPORT WEIGHT: _startup_fast must stay stdlib-only. The whole point of
   ...
   ``from ev0_cli.config import ...`` silently makes `3v0 --version`
...
def test_startup_fast_import_weight():
    ...
    assert not offenders, (
        f"ev0_cli._startup_fast imported heavy modules: {offenders} — "
...
def _run_version(env_overrides: dict) -> subprocess.CompletedProcess:
    ...
    return subprocess.run(
        [sys.executable, "-m", "ev0_cli.main", "--version"],
        capture_output=True,
        text=True,
        timeout=60,
        cwd=REPO_ROOT,
