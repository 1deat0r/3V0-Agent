...
from ev0_cli import nous_subscription as ns
...
def _capture_checklist(monkeypatch, *, selected_idx):
    ...
    monkeypatch.setattr(
        "ev0_cli.config.save_config", lambda cfg: None, raising=False
    )
    return captured
...
def _block_legacy_agent_browser_checks(monkeypatch):
    ...
    monkeypatch.setattr("ev0_constants.agent_browser_runnable", lambda path: False)
