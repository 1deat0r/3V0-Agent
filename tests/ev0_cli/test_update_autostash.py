...
from ev0_cli import config as ev0_config
...
def test_refresh_active_memory_provider_dependencies_reinstalls_active_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        "ev0_cli.memory_setup._install_dependencies",
...
def test_reload_updated_runtime_modules_restores_new_ev0_constants_symbol(monkeypatch):
    """A pre-pull module object missing a new helper is repaired by reload."""
    import ev0_constants

    monkeypatch.delattr(ev0_constants, "apply_subprocess_home_env", raising=False)
    assert not hasattr(ev0_constants, "apply_subprocess_home_env")

    ev0_main._reload_updated_runtime_modules()

    assert callable(ev0_constants.apply_subprocess_home_env)
...
def test_bootstrap_marker_not_autostashed_by_update(tmp_path):
    ...
    ``3v0 update``'s ``git stash push --include-untracked`` does not sweep it
    ...
    marker = tmp_path / ".3v0-bootstrap-complete"
    marker.write_text("")

    # Exact flags used by 3v0 update (ev0_cli/main.py).
    git("stash", "push", "--include-untracked", "-m", "3v0-update-autostash")

    assert marker.exists(), (
        ".3v0-bootstrap-complete was swept into the update autostash — it must "
    ...
    assert ".3v0-bootstrap-complete" not in status
...
def test_update_autostash_survives_undeletable_untracked_dir(tmp_path):
    ...
    try:
        ...
        assert (pkg / "3v0-agent.rb").read_text() == "formula\n"
    finally:
        os.chmod(pkg, 0o755)
