"""Tests for SIGHUP protection and stdout mirroring in ``3v0 update``.

Covers ``_UpdateOutputStream``, ``_install_hangup_protection``, and
``_finalize_update_output`` in ``ev0_cli/main.py``.  These exist so
that ``3v0 update`` survives a terminal disconnect mid-install
...
def test_update_completion_includes_bounded_action_identity(monkeypatch, capsys):
    monkeypatch.setenv("EV0_ACTION_ID", "a" * 32)

    _print_update_completion("✓ Update complete!")

    assert capsys.readouterr().out.splitlines() == [
        "✓ Update complete!",
        f"=== 3v0-update completed {'a' * 32} ===",
    ]


def test_update_completion_rejects_untrusted_action_identity(monkeypatch, capsys):
    monkeypatch.setenv("EV0_ACTION_ID", "not-safe\nforged")
...
class TestInstallHangupProtection:
    ...
    def test_wraps_stdout_and_stderr_with_mirror(self, tmp_path, monkeypatch):
        ...
        import ev0_cli.config as _cfg
        if hasattr(_cfg, "_EV0_HOME_CACHE"):
            _cfg._EV0_HOME_CACHE = None  # type: ignore[attr-defined]
        ...
        try:
            ...
            assert "3v0 update started" in contents
        finally:
    ...
    def test_non_fatal_if_log_setup_fails(self, monkeypatch):
        """If get_ev0_home() raises, stdio must be left untouched but SIGHUP still handled."""
        prev_out, prev_err = sys.stdout, sys.stderr
        ...
        monkeypatch.setattr(
            "ev0_cli.config.get_ev0_home", _boom, raising=True
        )
