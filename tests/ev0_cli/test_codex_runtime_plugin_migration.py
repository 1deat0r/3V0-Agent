...
from ev0_cli.codex_runtime_plugin_migration import (
    MIGRATION_MARKER,
...
# ---- end-to-end migrate(, expose_ev0_tools=False) ----
...
class TestMigrate:
    ...
    def test_plugin_discovery_writes_plugin_blocks(self, tmp_path, monkeypatch):
        ...
        from ev0_cli import codex_runtime_plugin_migration as crpm
    ...
    def test_preserves_user_mcp_server_outside_managed_block(self, tmp_path):
        ...
        to ~/.codex/config.toml outside 3V0' managed block, re-running
        ...
        assert "[mcp_servers.3v0-mcp]" in final
...
class TestStripUnmanagedPluginTables:
    ...
    ``3v0 codex-runtime migrate`` would re-emit them inside the managed
    ...
    def test_migrate_dedups_codex_owned_plugin_tables(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "ev0_cli.codex_runtime_plugin_migration._query_codex_plugins",
            fake_query,
        )
...
# ---- Bug C: EV0_HOME tempdir leak into ~/.codex/config.toml ----


class TestEv0HomeLeakGuard:
    """Regression tests for issue #26250 Bug C.

    Previously ``_build_ev0_tools_mcp_entry()`` read ``EV0_HOME``
    ...
    once codex spawned the 3v0-tools MCP subprocess.
    """
