"""Tests for ev0_cli.service_manager — the abstract ServiceManager
...
def test_windows_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # against the `ev0_cli` package succeeds — gateway_windows is
    # imported lazily inside the wrapper and may not yet be loaded.
    import ev0_cli.gateway_windows  # noqa: F401
    ...
    monkeypatch.setattr("ev0_cli.gateway_windows", _FakeWindowsModule)
    monkeypatch.setattr(
        "ev0_cli.gateway.find_gateway_pids",
        lambda **kw: [12345],
    )
...
def test_seed_supervise_skeleton_creates_expected_layout(tmp_path) -> None:
    ...
    from ev0_cli.service_manager import _seed_supervise_skeleton
