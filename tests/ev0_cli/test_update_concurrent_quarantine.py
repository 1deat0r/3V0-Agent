"""Tests for issue #26670 — concurrent 3v0.exe detection and improved
quarantine retry / reboot-deferred fallback during `3v0 update` on Windows.
...
# Tests in this module either exercise the REAL _detect_concurrent_ev0_instances
# helper (and need the autouse stub in tests/ev0_cli/conftest.py disabled),
...
def _make_proc(pid: int, exe: str, name: str = "3v0.exe"):
...
def test_detect_concurrent_parents_call_robust_to_one_bad_hop(_winp, tmp_path):
    ...
    rows = [
        ...
        _make_proc(launcher_pid, str(shim), "3v0.exe"),
    ]
...
# ---------------------------------------------------------------------------
# _quarantine_running_ev0_exe — retry + reboot-deferred fallback
# ---------------------------------------------------------------------------
...
def test_quarantine_succeeds_first_attempt(_winp, tmp_path):
    ...
    assert orig == shim
    assert quarantine.name.startswith("3v0.exe.old.")
...
def test_pause_windows_gateways_for_update_stops_profile_and_unmapped_pids(
    ...
    monkeypatch.setattr(
        ...
        lambda pid: ["pythonw.exe", "-m", "ev0_cli.main", "gateway", "run"]
        if pid == 202
        else None,
    )
    ...
    assert token == {
        ...
        "unmapped": [
            {
                "pid": 202,
                "argv": ["pythonw.exe", "-m", "ev0_cli.main", "gateway", "run"],
            }
        ],
    }
...
def test_unreadable_argv_falls_back_to_the_captured_prefix(monkeypatch):
    ...
    gateway_prefix = r"venv\Scripts\python.exe -m ev0_cli.main gateway run"
