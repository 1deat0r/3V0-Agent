...
`EV0_REDACT_SECRETS` at module-import time. `ev0_cli/main.py` at
...
in .env), the toggle was silently ignored in both `3v0 chat` and
`3v0 gateway run`.

Fix: bridge `security.redact_secrets` from config.yaml → `EV0_REDACT_SECRETS`
env var in `ev0_cli/main.py` BEFORE the `setup_logging()` call.
"""
import os
import subprocess
import sys
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    # Spawn a fresh Python process that imports ev0_cli.main and checks
    # _REDACT_ENABLED. Must be a subprocess — we need a clean module state.
    probe = textwrap.dedent(
        """\
        import sys, os
        # Make absolutely sure the env var is not pre-set
        os.environ.pop("EV0_REDACT_SECRETS", None)
        sys.path.insert(0, %r)
        import ev0_cli.main  # triggers the bridge + setup_logging
        import agent.redact
        print(f"REDACT_ENABLED={agent.redact._REDACT_ENABLED}")
        print(f"ENV_VAR={os.environ.get('EV0_REDACT_SECRETS', '<unset>')}")
        """
    ) % str(REPO_ROOT)
