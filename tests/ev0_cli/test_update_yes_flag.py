"""Tests for `3v0 update --yes / -y` — assume yes for interactive prompts.
...
from ev0_cli.main import cmd_update
...
class TestUpdateYesConfigMigration:
    ...
    @patch("ev0_cli.update_cmd._reload_config_modules")
    @patch("ev0_cli.update_cmd._run_migrate_config_fresh")
    @patch("ev0_cli.update_cmd._run_config_check_fresh", return_value=(1, 2))
    @patch("ev0_cli.config.get_missing_config_fields", return_value=[])
    @patch("ev0_cli.config.get_missing_env_vars", return_value=["NEW_KEY"])
    ...
    def test_no_yes_flag_still_prompts_in_tty(
        ...
        # real ``sys`` module instead of replacing ``ev0_cli.main.sys`` with
        # a MagicMock. The MagicMock approach was flaky under ``pytest-xdist``
        # — a sibling test that imported ``ev0_cli.main`` first could leave
...
class TestUnicodeDecodeErrorInUpdatePrompts:
    ...
    the exception escape and crash `3v0 update` mid-flight.
    """
    ...
    def test_upstream_remote_prompt_unicode_decode_error_falls_through_to_skip(
        ...
        from ev0_cli.update_cmd import _sync_with_upstream_if_needed

        with patch(
            "ev0_cli.update_cmd._has_upstream_remote", return_value=False
        ), patch(
            "ev0_cli.update_cmd._should_skip_upstream_prompt", return_value=False
        ), patch(
            "ev0_cli.update_cmd._add_upstream_remote"
        ) as mock_add, patch(
            "builtins.input",
            side_effect=UnicodeDecodeError("utf-8", b"\xff", 0, 1, "invalid byte"),
        ):
