...
(e.g. orphan merge-conflict markers in ``ev0_cli/config.py``), the CLI
...
markers in ``ev0_cli/config.py``; users who ran ``hermes update`` in
...
def test_validate_critical_files_syntax_tolerates_missing_files(tmp_path):
    ...
    # Populate everything except ev0_constants.py
    for relpath in hermes_main._UPDATE_CRITICAL_FILES:
        if relpath == "ev0_constants.py":
            continue
