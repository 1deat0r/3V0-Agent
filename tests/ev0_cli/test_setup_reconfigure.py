...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("ev0_cli.setup.ensure_ev0_home", {}),
        ("ev0_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("ev0_cli.config.is_managed", {"return_value": False}),
        ("ev0_cli.setup.load_config", {"return_value": {}}),
        ("ev0_cli.setup.save_config", {}),
        ("ev0_cli.setup.get_env_value", {"return_value": None}),
        ("ev0_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("ev0_cli.setup._print_setup_summary", {}),
        ("ev0_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("ev0_cli.setup.load_config", {"return_value": {}}),
        ("ev0_cli.setup.save_config", {}),
        ("ev0_cli.auth.get_active_provider", {"return_value": None}),
        ("ev0_cli.setup.get_env_value", {"return_value": None}),
        ("ev0_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="ev0_cli.setup.prompt_choice",
                quick="ev0_cli.setup._run_quick_setup",
                model="ev0_cli.setup.setup_model_provider",
                terminal="ev0_cli.setup.setup_terminal_backend",
                agent="ev0_cli.setup.setup_agent_settings",
                gateway="ev0_cli.setup.setup_gateway",
                tools="ev0_cli.setup.setup_tools",
            )
    ...
    def test_reconfigure_on_fresh_install_falls_through(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("ev0_cli.setup.prompt_choice", {"return_value": 0}),
                first="ev0_cli.setup._run_first_time_quick_setup",
            )
            from ev0_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "ev0_cli.setup.run_setup_wizard",
