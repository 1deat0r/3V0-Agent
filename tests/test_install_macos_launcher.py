"""Regression coverage for the user-facing macOS 3V0 launcher."""
...
def test_venv_launcher_bypasses_uv_console_script_that_requires_realpath(tmp_path: Path) -> None:
    """Stock macOS must start 3V0 even when its uv console script needs realpath."""
    ...
    (install_dir / "3v0").write_text("# source entrypoint\n", encoding="utf-8")
    _make_executable(
        venv_bin / "3v0",
        "#!/bin/sh\n"
        f'PATH="{minimal_path}"\n'
