...
``EV0_HOME`` with named profile dirs — no mocks of the config/mcp layer — and
...
def ev0_root(tmp_path, monkeypatch):
    ...
    (get_default_ev0_root's Docker/custom branch), so named profiles live at
...
def test_default_profile_add_when_profile_omitted(ev0_root):
    ...
    # Omitted profile → launch/default profile == EV0_HOME root config.yaml.
    default_cfg = _read_yaml(root / "config.yaml")
