...
before ``prompt.submit`` installs the session's profile EV0_HOME override, so
...
def test_profile_home_session_writes_under_profile(tmp_path):
    ...
    profile_home = tmp_path / ".3V0" / "profiles" / "coder"
...
def test_two_profiles_are_isolated(tmp_path):
    ...
    home_a = tmp_path / ".3V0" / "profiles" / "a"
    home_b = tmp_path / ".3V0" / "profiles" / "b"
