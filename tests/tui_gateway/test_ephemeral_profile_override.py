"""Regression tests: profile EV0_HOME override in ephemeral agent threads (#50233).

Why: normal prompt turns bind ``session['profile_home']`` via
``set_ev0_home_override`` before ``run_conversation`` so the turn runs against
...
``EV0_HOME`` ContextVar set on the session-create thread does NOT propagate to
...
PROFILE_HOME = "/home/user/.3V0/profiles/work"
