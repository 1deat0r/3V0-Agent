...
Driving the update as `python.exe -m ev0_cli.main update` puts the inherited
...
def test_update_invocation_uses_module_entrypoint() -> None:
    ...
    assert '@("-m", "ev0_cli.main", "update"' in source, (
        "The update step must invoke `python.exe -m ev0_cli.main update ...` "
        "so the inherited image handle lands on python.exe, which uv never has "
        "to replace."
    )
    assert (
        '@("-m", "ev0_cli.main", "desktop", "--force-build", "--build-only")'
        in source
    ), (
        "The desktop rebuild step must also go through "
        "`python.exe -m ev0_cli.main desktop ...` for the same reason."
    )
