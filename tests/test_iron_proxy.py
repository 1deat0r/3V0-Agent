...
def test_load_mappings_handles_corrupt_json(ev0_home):
...
def test_verify_checksums_signature_skips_without_gpg(ev0_home, monkeypatch, tmp_path):
...
def test_start_proxy_idempotent_when_already_running(ev0_home, monkeypatch):
    state = ip._proxy_state_dir()
...
def test_subprocess_env_strips_unrelated_secrets(ev0_home, monkeypatch):
...
def test_ca_key_created_with_0o600(ev0_home, monkeypatch):
...
def test_ensure_audit_log_creates_with_0o600(ev0_home, tmp_path):
...
def test_ensure_audit_log_tightens_existing_perms(ev0_home, tmp_path):
    audit = tmp_path / "audit.log"
...
def test_proxy_state_dir_is_0o700(ev0_home):
    state = ip._proxy_state_dir()
...
def test_mappings_roundtrip_preserves_headers_and_aliases(ev0_home):
    m = ip.TokenMapping(
...
def test_ensure_management_token_persists_and_is_stable(ev0_home):
...
def test_reload_proxy_refuses_when_not_running(ev0_home, monkeypatch):
...
def test_reload_proxy_posts_bearer_to_management_endpoint(ev0_home, monkeypatch):
...
def test_start_proxy_injects_management_key_env(ev0_home, monkeypatch):
    ...
    cfg = ip.build_proxy_config(
        mappings=[_sample_mapping()],
        ca_cert=ev0_home / "ca.crt",
        ca_key=ev0_home / "ca.key",
        http_listen=["127.0.0.1:9090"],
    )
    ip.write_proxy_config(cfg)
    (ev0_home / "bin").mkdir(parents=True, exist_ok=True)
    fake_bin = ev0_home / "bin" / "iron-proxy"
...
def test_docker_egress_node_options_uses_sentinel(ev0_home, monkeypatch):
...
def test_persisted_nonce_roundtrip(ev0_home, monkeypatch):
...
def test_get_status_probes_configured_bind_host(ev0_home, monkeypatch):
