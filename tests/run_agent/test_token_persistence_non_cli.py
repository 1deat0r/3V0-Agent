...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    threev0_state = ModuleType("threev0_state")
    threev0_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "threev0_state", threev0_state)
    ...
    result = json.loads(agent._invoke_tool(
        "session_search",
        {"query": "3V0", "detail": "full"},
        "task-id",
    ))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "3V0"
...
def test_sequential_session_search_forwards_detail(monkeypatch):
    ...
    tool_call = SimpleNamespace(
        ...
        function=SimpleNamespace(
            name="session_search",
            arguments=json.dumps({"query": "3V0", "detail": "full"}),
        ),
    )
