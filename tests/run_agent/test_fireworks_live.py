"""Live Fireworks smoke test — exercises the 3V0 runtime, not a raw SDK client.

Opt-in only:
    EV0_LIVE_TESTS=1 FIREWORKS_API_KEY=fw_... \\
        pytest tests/run_agent/test_fireworks_live.py -q

Unlike a bare OpenAI() client pointed at the endpoint, this drives 3V0'
...
LIVE = os.environ.get("EV0_LIVE_TESTS") == "1"
FIREWORKS_KEY = os.environ.get("FIREWORKS_API_KEY", "")

pytestmark = [
    pytest.mark.skipif(not LIVE, reason="live-only: set EV0_LIVE_TESTS=1"),
...
def _resolve_runtime_client(provider="fireworks"):
    """Build the Fireworks client the way the 3V0 runtime does."""
    from agent.auxiliary_client import resolve_provider_client

    client, model = resolve_provider_client(provider)
    assert client is not None, "3V0 failed to build a Fireworks client"
    return client, model


def test_threev0_wires_fireworks_client():
...
def test_fireworks_basic_chat_through_runtime():
    """A single-turn completion via the 3V0-resolved client returns text."""
    client, model = _resolve_runtime_client()
