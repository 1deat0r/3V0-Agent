...
class TestHandleFunctionCall:
    ...
    def test_post_tool_call_receives_non_negative_integer_duration_ms(self):
        ...
        with (
            ...
            patch("ev0_cli.plugins.has_hook", return_value=True),
            patch("ev0_cli.plugins.invoke_hook") as mock_invoke_hook,
        ):
    ...
    def test_no_listener_skips_post_and_transform_emit(self):
        ...
        with (
            patch("model_tools.registry.dispatch", return_value='{"ok":true}'),
            patch("ev0_cli.plugins.has_hook", return_value=False),
            patch("ev0_cli.plugins.invoke_hook") as mock_invoke_hook,
        ):
    ...
    def test_tool_request_and_execution_middleware_wrap_registry_dispatch(self, monkeypatch):
        ...
        monkeypatch.setattr("ev0_cli.plugins.invoke_middleware", fake_invoke_middleware)
        monkeypatch.setattr("ev0_cli.plugins.get_plugin_manager", lambda: manager)
        hook_calls = []
        monkeypatch.setattr(
            "ev0_cli.plugins.invoke_hook",
            lambda hook_name, **kwargs: hook_calls.append((hook_name, kwargs)) or [],
        )
        monkeypatch.setattr("ev0_cli.plugins.has_hook", lambda name: True)
    ...
    def test_registry_exception_emits_terminal_tool_hook(self, monkeypatch):
        from ev0_cli import lifecycle

        hook_calls = []
        monkeypatch.setattr("ev0_cli.plugins.invoke_hook", lambda *_args, **_kwargs: [])
...
class TestPreToolCallBlocking:
    ...
    def test_blocked_tool_returns_error_and_skips_dispatch(self, monkeypatch):
        ...
        monkeypatch.setattr("ev0_cli.plugins.invoke_hook", fake_invoke_hook)
        monkeypatch.setattr("ev0_cli.plugins.has_hook", lambda name: True)
        monkeypatch.setattr("model_tools.registry.dispatch", fake_dispatch)
    ...
    def test_relay_rewrite_is_visible_to_pre_tool_authorization(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "ev0_cli.observability.relay_runtime.apply_tool_request_intercepts",
            rewrite,
        )
        monkeypatch.setattr("ev0_cli.plugins.invoke_hook", fake_invoke_hook)
        monkeypatch.setattr("ev0_cli.plugins.has_hook", lambda name: True)
