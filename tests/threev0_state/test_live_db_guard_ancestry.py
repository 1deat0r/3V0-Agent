...
root = threev0_state._real_platform_state_root()
if root is None:
    print("NO-ROOT")
else:
    try:
        threev0_state._ensure_test_isolation(root / "state.db")
    except RuntimeError:
        print("REFUSED")
    else:
        print("ALLOWED")
...
class TestScrubbedChildEnvironment:
    ...
    def test_env_bypass_lets_a_deliberate_child_through(self):
        ...
        """
        env = _scrubbed_env(**{threev0_state._STATE_DB_GUARD_BYPASS_ENV: "1"})
...
class TestPytestProcessRecognition:
    ...
    def test_recognises_pytest_invocations(self, cmdline):
        assert threev0_state._process_looks_like_pytest(self._FakeProc(cmdline))

    @pytest.mark.parametrize(
        "cmdline",
        [
            ["3v0", "gateway", "start"],
            ["/usr/bin/python", "-m", "threev0_cli.main", "sessions", "list"],
    ...
    def test_ignores_non_pytest_invocations(self, cmdline):
        assert not threev0_state._process_looks_like_pytest(self._FakeProc(cmdline))
    ...
    def test_unreadable_process_is_not_pytest(self):
        ...
        assert not threev0_state._process_looks_like_pytest(_Denied())
