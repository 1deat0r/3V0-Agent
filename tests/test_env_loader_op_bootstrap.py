...
1. ``load_ev0_dotenv()`` auto-loads ``~/.3V0/.op.env`` so the
...
def test_op_env_autoloads_bootstrap_token_in_cron_context(tmp_path, monkeypatch):
    ...
    env_loader.load_threev0_dotenv(threev0_home=home)
...
def _seed_openrouter_token(monkeypatch, dotenv_value, environ_value):
    ...
    monkeypatch.setattr(
        "threev0_cli.auth.is_source_suppressed", lambda _p, _s: False
    )
