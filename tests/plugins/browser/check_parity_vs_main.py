...
Run from the PR worktree:
    ...
    cd ~/.3V0/3v0-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.3V0/3v0-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.3V0/3v0-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a 3v0-agent checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a 3v0-agent checkout"
)
...
# Isolated EV0_HOME for the config write.
