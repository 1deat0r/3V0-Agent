...
gateway process memory + sidecar child env — so cron/`3v0 send`
standalone sends structurally could not authenticate. The adapter now
persists ``<3v0-home>/runtime/photon-sidecar.json`` after the sidecar
...
async def test_standalone_send_consumes_record_when_env_missing(
    ...
    assert ":9111/" in url
    assert headers["X-3V0-Sidecar-Token"] == "record-token"
