...
_CATALOG = {
    "templates": [
        {"id": "conversation", "name": "Conversation", "integrations": ["litellm", "3v0"],
        ...
        {"id": "3v0-gateway-bot", "name": "Gateway Bot", "integrations": ["3v0"],
         "manifest_file": "templates/3v0-gateway-bot.json"},
    ]
}
...
def test_fetch_manifest_resolves_relative_url(monkeypatch):
    ...
    assert seen["url"] == "https://raw.example/data/templates/3v0-gateway-bot.json"
...
def test_apply_template_posts_to_import_endpoint(monkeypatch):
    ...
    tpl.apply_template("https://api.hindsight.vectorize.io/", "3v0", "hsk_abc", {"version": "1"})

    assert captured["url"] == "https://api.hindsight.vectorize.io/v1/default/banks/3v0/import"
...
def test_apply_template_omits_auth_when_no_key(monkeypatch):
    ...
    monkeypatch.setattr(tpl, "open_credentialed_url", _fake_open)
    tpl.apply_template("http://localhost:8888", "3v0", None, {"version": "1"})
...
def test_run_template_step_applies_selected(monkeypatch):
    ...
    assert result == "3v0-gateway-bot"
    assert applied["bank"] == "3v0"
...
def test_run_template_step_swallows_apply_errors(monkeypatch):
    ...
    logs = []
    result = tpl.run_template_step(
        api_url="https://api", bank_id="3v0", api_key=None,
...
def test_probe_existing_customization_true_when_bank_has_config(monkeypatch):
    ...
    assert tpl.probe_existing_customization("https://api", "3v0", "k") is True


def test_probe_existing_customization_false_when_empty(monkeypatch):
    monkeypatch.setattr(tpl, "open_credentialed_url",
                        _fake_open({"version": "1"}))
    assert tpl.probe_existing_customization("https://api", "3v0", "k") is False
