...
from threev0_cli.memory_setup import _CANCELLED
...
class TestPostSetup:
    def test_setup_cancel_at_mode_picker_writes_nothing(self, tmp_path, monkeypatch):
        ...
        run = MagicMock()
        monkeypatch.setattr("threev0_cli.memory_setup._curses_select", lambda *args, **kwargs: _CANCELLED)
    ...
    def test_local_embedded_setup_materializes_profile_env(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("threev0_cli.memory_setup._curses_select", lambda *args, **kwargs: next(selections))
...
class TestPostSetupEnvEncoding:
    def _run_cloud_post_setup(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("threev0_cli.memory_setup._curses_select",
                            lambda *a, **kw: 0)  # cloud mode
        monkeypatch.setattr("threev0_cli.config.save_config", lambda c: None)
