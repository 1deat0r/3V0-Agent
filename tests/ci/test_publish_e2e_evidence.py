...
def test_publish_marks_evidence_upload_failure_in_pr_comment(tmp_path, monkeypatch):
    comment = {
        "id": 123,
        "body": "before\n<!-- 3v0-e2e-evidence:start -->\npending\n<!-- 3v0-e2e-evidence:end -->\nafter",
    }
    updates = []
    ...
    assert updates == [
        (
            "https://api.github.com/repos/NousResearch/3v0-agent/issues/comments/123",
            "github-token",
            "PATCH",
            {
...
def test_find_review_comment_requires_the_evidence_marker():
    pending = "<!-- 3v0-ci-review-bot -->\n<!-- 3v0-e2e-evidence:start -->\npending\n<!-- 3v0-e2e-evidence:end -->"

    assert _mod._find_review_comment([{"body": "<!-- 3v0-ci-review-bot --> no evidence"}]) is None
