...
#
# Installs a real, earlier 3V0 the way a user does, applies ONE update route,
# and requires the checkout to land on this commit with a working `3v0`.
#
...
# Keep sandbox state out of the default .3v0-sandbox so a run never clobbers
...
SANDBOX_DIR_NAME=".3v0-sandbox-e2e-$ROUTE"
export EV0_DEV_SANDBOX_DIR="$SANDBOX_DIR_NAME"

SANDBOX_ROOT="$REPO_ROOT/$SANDBOX_DIR_NAME"
INSTALL_DIR="/home/3v0/.3V0/3v0-agent"   # user-level layout (sandbox default)
FAKE_REMOTE="/work/repos/3v0-agent.git"
...
LOG_DIR="${EV0_E2E_LOG_DIR:-$(mktemp -d -t 3v0-install-e2e-logs.XXXXXX)}"
...
# ── helpers ────────────────────────────────────────────────────────────────
# Does the INSTALLED 3v0 accept FLAG on `3v0 update`?
#
...
# that silently rots. `3v0 update --help` is the same surface a user meets,
# and argparse prints every option it accepts.
update_supports() {
  local flag="$1"
  in_sandbox "3v0 update --help 2>&1" | grep -qF -- "$flag"
}
...
require_ev0_works() {
  local when="$1" out
  out="$(in_sandbox "3v0 --version" 2>&1)" \
    || { printf '%s\n' "$out" >&2; fail "3v0 --version failed $when"; }
  printf '%s\n' "$out" | sed 's/^/    /'
  ok "3v0 runs $when"
}

# ── install the earlier 3V0 ─────────────────────────────────────────────
...
require_ev0_works 'after install'

# ── apply exactly one update route ─────────────────────────────────────────
case "$ROUTE" in
  update)
    step 'ROUTE: 3v0 update'
    ...
    # 3v0 which it accepts; older ones read the prompt from stdin, so close it.
    if update_supports --yes; then
      update_cmd="3v0 update --yes"
    else
      update_cmd="3v0 update </dev/null"
    fi
    if ! in_sandbox "cd $INSTALL_DIR && $update_cmd"; then
      collect_sandbox_logs update
      fail "3v0 update failed ($update_cmd)"
    fi
    require_landed_on_target '3v0 update'
    require_ev0_works 'after 3v0 update'
    ;;
  installer)
    ...
    require_ev0_works 'after installer re-run'
    ;;
esac
