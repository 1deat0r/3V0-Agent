...
Under a gateway (Telegram, Discord, Slack, ...) 3V0 passes the
...
class TestPinPeerNameConfigParsing:
    ...
    def test_root_level_true(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("EV0_HOME", str(tmp_path / "isolated"))
...
class TestCrossPlatformMemoryUnification:
    """The same physical user talking to 3V0 via Telegram AND Discord
...
class TestProfilePeerUniqueness:
    """Each 3V0 profile can pin to its own unique peerName.

    Profile cloning copies host blocks, but operators routinely diverge them
    afterwards (e.g. `3v0 -p partner` pinned to a different person's peer).
