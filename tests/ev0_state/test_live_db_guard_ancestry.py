...
root = ev0_state._real_platform_state_root()
if root is None:
    print("NO-ROOT")
else:
    try:
        ev0_state._ensure_test_isolation(root / "state.db")
    except RuntimeError:
        print("REFUSED")
    else:
        print("ALLOWED")
...
class TestScrubbedChildEnvironment:
    ...
    def test_env_bypass_lets_a_deliberate_child_through(self):
        ...
        """
        env = _scrubbed_env(**{ev0_state._STATE_DB_GUARD_BYPASS_ENV: "1"})
...
class TestPytestProcessRecognition:
    ...
    def test_recognises_pytest_invocations(self, cmdline):
        assert ev0_state._process_looks_like_pytest(self._FakeProc(cmdline))
    ...
    def test_ignores_non_pytest_invocations(self, cmdline):
        assert not ev0_state._process_looks_like_pytest(self._FakeProc(cmdline))
    ...
    def test_unreadable_process_is_not_pytest(self):
        ...
        assert not ev0_state._process_looks_like_pytest(_Denied())
