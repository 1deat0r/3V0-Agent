"""Global emergency stop (`3v0 pause` / `3v0 resume`) — agent/estop.py.
...
already in flight is never touched. Removing the sentinel (`3v0 resume`)
...
async def test_gateway_slash_commands_bypass_estop(ev0_home):
    ...
    try:
        ...
        return
    assert reply is None or "3v0 is paused" not in (reply or "").lower()
