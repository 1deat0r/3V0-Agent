"""Tests for ev0_bootstrap — Windows UTF-8 stdio shim.
...
Key invariants covered by these tests:
  ...
  5. Load order: every 3V0 entry point imports ev0_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported ev0_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("ev0_bootstrap", None)
    import ev0_bootstrap  # noqa: WPS433
    return ev0_bootstrap
...
class TestEntryPointsImportBootstrap:
    """Every 3V0 entry point must import ev0_bootstrap as its
    ...
    # import ev0_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "ev0_cli/main.py",   # 3v0 CLI (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import ev0_bootstrap' and that
        ...
        ``ev0_bootstrap`` as a top-level module) leaves 3v0
        ...
        # at tests/test_ev0_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # 3v0 start even when ev0_bootstrap hasn't been
        ...
        assert first_import_name == "ev0_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'ev0_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import ev0_bootstrap' line to be the first import."
        )
