...
class TestPipeStdinSurrogates:
    ...
    def test_unencodable_surrogate_captures_error_and_closes_stdin(self, tmp_path):
        ...
        assert proc._ev0_stdin_errors  # the encode failure was captured
        assert isinstance(proc._ev0_stdin_errors[0], UnicodeEncodeError)
...
class TestPipeStdinRemainingBranches:
    ...
    def test_stdin_none_records_runtime_error(self, tmp_path):
        ...
        assert proc.returncode == 0
        assert proc._ev0_stdin_errors
        assert isinstance(proc._ev0_stdin_errors[0], RuntimeError)
