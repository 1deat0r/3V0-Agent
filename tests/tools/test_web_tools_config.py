...
class TestBackendSelection:
    ...
    ``3v0 tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
...
class TestFirecrawlEnvResolution:
    """Verify Firecrawl reads env values from threev0_cli.config.get_env_value,
    not just os.getenv.  This catches the regression reported in #40190 where
    values stored in ~/.3V0/.env were invisible to the provider."""
...
class TestSiblingProvidersEnvResolution:
    ...
    ~/.3V0/.env), not bare os.getenv. Parametrized over the four
