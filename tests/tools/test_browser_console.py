...
class TestBrowserConsole:
    ...
    def test_restrict_evaluate_reads_browser_config(self):
        ...
        with patch("threev0_cli.config.read_raw_config", return_value={"browser": {"restrict_evaluate": "true"}}):
            assert _restrict_browser_evaluate() is True
        with patch("threev0_cli.config.read_raw_config", return_value={"browser": {"restrict_evaluate": False}}):
            assert _restrict_browser_evaluate() is False
        # Default (key absent) is off — the denylist is opt-in.
        with patch("threev0_cli.config.read_raw_config", return_value={}):
            assert _restrict_browser_evaluate() is False
...
class TestBrowserVisionConfig:
    ...
    def test_browser_vision_uses_configured_temperature_and_timeout(self, tmp_path):
        ...
        with (
            patch("threev0_constants.get_threev0_dir", return_value=shots_dir),
            ...
            patch("tools.browser_tool._get_vision_model", return_value="test-model"),
            patch("threev0_cli.config.load_config", return_value={"auxiliary": {"vision": {"temperature": 1, "timeout": 45}}}),
