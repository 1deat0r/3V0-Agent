"""Tests for tools.wake_word — the "Hey 3V0" hotword detector.
...
def test_config_defaults_and_clamping():
    ...
    assert ww.wake_phrase({"phrase": "hey 3v0"}) == "hey 3v0"
    assert ww.wake_phrase({}) == "hey 3v0"
...
def test_load_wake_word_config_guards_non_dict(monkeypatch):
    monkeypatch.setattr(
        "threev0_cli.config.load_config", lambda: {"wake_word": "oops"}
    )
...
def test_requirements_openwakeword_available(monkeypatch):
    ...
    r = ww.check_wake_word_requirements(
        {"provider": "openwakeword", "phrase": "hey 3v0"}
    )
    assert r["available"] is True
    assert r["provider"] == "openwakeword"
    assert r["phrase"] == "hey 3v0"
...
def test_bundled_hey_3v0_model_ships_on_disk():
    # The "hey 3v0" wake word works out of the box only if the model is
