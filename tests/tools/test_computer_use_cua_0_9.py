...
def _make_backend(session: _FakeSession):
    ...
    backend._session_id = "3v0-session"
...
def _browser_route(driver: _BrowserDriver, session_id: str = "3v0-a"):
...
def test_exact_browser_binding_injects_ev0_session_capability():
    ...
    assert driver.calls == [
        (
            "get_browser_state",
            {"pid": 101, "window_id": 202, "session": "3v0-owned-session"},
        )
    ]
...
def test_typed_browser_refs_do_not_cross_route_sessions():
    driver = _BrowserDriver()
    first = _browser_route(driver, session_id="3v0-a")
    second = _browser_route(driver, session_id="3v0-b")
