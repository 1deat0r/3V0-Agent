...
class TestRunBrowserCommandPathConstruction:
 ...
 patch("tools.browser_tool._chromium_installed", return_value=True), \
             ...
             patch("ev0_constants.Path.home", return_value=tmp_path), \
    ...
    def test_npx_sentinel_resolves_via_resolve_npx_bin_with_pinned_spec(self, tmp_path):
        ...
        could let a broken system npx shadow a healthy 3V0-managed one) and
        ...
        assert captured_cmd[:5] == [
            "/opt/3v0/node/bin/npx", "--ignore-scripts", "--prefer-offline", "-y",
            AGENT_BROWSER_NPX_SPEC,
        ]
...
class TestResolveNpxBinPriority:
    ...
    3V0-managed one — and each candidate must be validated (actually
    ...
    def test_prefers_managed_extended_path_over_bare_path(self, monkeypatch):
        ...
        monkeypatch.setattr(bt, "_merge_browser_path", lambda _p: "/3v0/node/bin")
        monkeypatch.setattr(
            bt.shutil, "which",
            lambda cmd, path=None: (
                "/3v0/node/bin/npx" if path == "/3v0/node/bin"
                else "/usr/local/bin/npx"
            ),
        )
        monkeypatch.setattr(bt, "node_tool_runnable", lambda p: True)

        assert bt._resolve_npx_bin() == "/3v0/node/bin/npx"
    ...
    def test_falls_back_to_bare_path_when_extended_dir_has_no_npx(self, monkeypatch):
        ...
        monkeypatch.setattr(
            bt.shutil, "which",
            lambda cmd, path=None: None if path == "/3v0/node/bin" else "/usr/bin/npx",
        )
        monkeypatch.setattr(bt, "node_tool_runnable", lambda p: True)
