...
Older images made /opt/3v0/ui-tui and /opt/3v0/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV EV0_TUI_DIR=/opt/3v0/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R 3v0:3v0 /opt/3v0/ui-tui" not in text
    assert "chown -R 3v0:3v0 /opt/3v0/node_modules" not in text
