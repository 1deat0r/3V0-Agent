...
UDS socket lifecycle, ev0_tools generation, timeout enforcement,
...
class TestExecuteCodeRemoteTempDir(unittest.TestCase):
    def test_execute_remote_uses_backend_temp_dir_for_sandbox(self):
        ...
        self.assertIn("mkdir -p /data/data/com.termux/files/usr/tmp/ev0_exec_", mkdir_cmd)
        self.assertIn("EV0_RPC_DIR=/data/data/com.termux/files/usr/tmp/ev0_exec_", run_cmd)
        self.assertIn("rm -rf /data/data/com.termux/files/usr/tmp/ev0_exec_", cleanup_cmd)
        self.assertNotIn("mkdir -p /tmp/ev0_exec_", mkdir_cmd)

    def test_timezone_shell_quoted_in_remote_execution(self):
        """EV0_TIMEZONE must be shell-quoted in remote env_prefix to prevent injection."""
        class FakeEnv:
            def __init__(self):
                self.commands = []
...
class TestExecuteCode(unittest.TestCase):
    ...
    def test_repo_root_modules_are_importable(self):
        ...
        result = self._run('import ev0_constants; print(ev0_constants.__file__)')
        self.assertEqual(result["status"], "success")
        self.assertIn("ev0_constants.py", result["output"])
...
"""
    ...
    def test_shell_quote_helper(self):
        ...
        code = """
from ev0_tools import shell_quote
...
"""
    ...
    def test_json_parse_helper_bom(self):
        ...
        code = """
from ev0_tools import json_parse
...
class TestRpcTokenAuthorization(unittest.TestCase):
    ...
    request without the matching EV0_RPC_TOKEN must be rejected before
