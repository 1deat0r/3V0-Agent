...
def fake_ev0(tmp_path, monkeypatch):
    """Build a two-profile 3V0 layout and point EV0_HOME at
    the 3v0-security profile (matching the original-incident shape).
    """
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_ev0):
        from tools.file_tools import write_file_tool
        target = fake_ev0["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_ev0):
        """The May 2026 incident — security-profile session edits default
        profile's skill. Must be blocked."""
        from tools.file_tools import write_file_tool
        target = fake_ev0["root"] / "skills" / "shared-skill" / "SKILL.md"
        ...
        assert "3v0-security" in result["error"]
        # File untouched.
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_blocked(self, fake_ev0):
    ...
    def test_cross_profile_patch_bypass(self, fake_ev0):
        from tools.file_tools import patch_tool
        target = fake_ev0["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_extracts_path_for_guard(self, fake_ev0):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        self, fake_ev0, monkeypatch
    ):
        """The original incident shape — model expects 'foo' in active
        profile, but 'foo' lives in default. Error must point at default."""
        self._make_skill_in_profile(fake_ev0["root"], "default-only-skill")

        # Re-import the module so SKILLS_DIR picks up EV0_HOME (set in
        ...
        assert "not found in active profile '3v0-security'" in err
...
class TestSystemPromptActiveProfile:
    ...
    def test_named_profile_line_in_prompt_text(self, fake_ev0):
        """When active profile is '3v0-security', the prompt warns
        ...
        assert "Active 3V0 profile" in src
        assert "cross_profile=True" in src
        assert "~/.3V0/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active 3V0 profile: default" in src
        assert "Active 3V0 profile: {active_profile}" in src
