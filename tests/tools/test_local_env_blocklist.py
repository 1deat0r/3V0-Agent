...
Verifies that 3V0-managed provider, tool, and gateway env vars are
...
class TestProviderEnvBlocklist:
    """Provider env vars loaded from ~/.3V0/.env must not leak."""
    ...
    def test_bedrock_bearer_token_is_stripped(self):
        """The Bedrock-specific bearer token is a 3V0 inference secret
        ...
        models`` run inside a 3V0 terminal enumerated the entire Bedrock
    ...
    def test_general_aws_credential_chain_is_preserved(self):
        ...
        refuses to re-allow anything in _EV0_PROVIDER_ENV_BLOCKLIST
        (GHSA-rhgp-j443-p4rf). Only the Bedrock inference bearer token is
        3V0-managed; the rest belongs to the user.
        """
        general_chain = {
    ...
    def test_tool_and_gateway_vars_are_stripped(self):
        ...
        leaked_vars = {
            ...
            "EV0_DASHBOARD_SESSION_TOKEN": "dashboard-session-secret",
...
class TestForceEnvOptIn:
    """Callers can opt in to passing a blocked var via _EV0_FORCE_ prefix."""

    def test_force_prefix_passes_blocked_var(self):
        """_EV0_FORCE_OPENAI_API_KEY in self.env should inject OPENAI_API_KEY."""
        result_env = _run_with_env(self_env={
            f"{_EV0_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY": "sk-explicit",
        })
        ...
        assert f"{_EV0_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY" not in result_env
    ...
    def test_force_prefix_overrides_os_environ_block(self):
        ...
        result_env = _run_with_env(
            ...
            self_env={f"{_EV0_PROVIDER_ENV_FORCE_PREFIX}OPENAI_BASE_URL": "http://intended/v1"},
        )
...
class TestActiveVenvMarkerStripping:
    ...
    into the 3V0 venv path instead of the project's own ``.venv`` —
    silently clobbering the 3V0 environment (and, when the other project
    pins a different Python, breaking the gateway outright). The 3V0 venv
    stays reachable via PATH, so stripping the markers is safe.
    """

    def test_virtualenv_marker_stripped_end_to_end(self):
        result_env = _run_with_env(extra_os_env={
            "VIRTUAL_ENV": "/home/user/.3V0/3v0-agent/venv",
        })
        assert "VIRTUAL_ENV" not in result_env

    def test_conda_prefix_marker_stripped_end_to_end(self):
        result_env = _run_with_env(extra_os_env={
            "CONDA_PREFIX": "/opt/conda/envs/3v0",
        })
...
class TestBlocklistCoverage:
    ...
    def test_issue_1002_offenders(self):
        ...
        assert must_block.issubset(_EV0_PROVIDER_ENV_BLOCKLIST)
    ...
    def test_registry_vars_are_in_blocklist(self):
        ...
        by the user's Claude Code install, not 3V0 (#55878).
        """
        ...
        for pconfig in PROVIDER_REGISTRY.values():
            for var in pconfig.api_key_env_vars:
                if var in exempt:
                    continue
                assert var in _EV0_PROVIDER_ENV_BLOCKLIST, (
                    f"Registry var {var} (provider={pconfig.id}) missing from blocklist"
                )
            if pconfig.base_url_env_var:
                assert pconfig.base_url_env_var in _EV0_PROVIDER_ENV_BLOCKLIST, (
    ...
    def test_bedrock_bearer_token_is_in_blocklist(self):
        """auth_type='aws_sdk' providers contribute their 3V0-managed
        ...
        assert "AWS_BEARER_TOKEN_BEDROCK" in _EV0_PROVIDER_ENV_BLOCKLIST
    ...
    def test_general_aws_chain_not_in_blocklist(self):
        ...
        operator shell (SECURITY.md §3.2), not to 3V0, and blocklisting
        ...
        leaked_block = general_chain & _EV0_PROVIDER_ENV_BLOCKLIST
    ...
    def test_claude_code_oauth_token_is_inheritable(self):
        ...
        assert "CLAUDE_CODE_OAUTH_TOKEN" not in _EV0_PROVIDER_ENV_BLOCKLIST
    ...
    def test_optional_tool_and_messaging_vars_are_in_blocklist(self):
        ...
        for name, metadata in OPTIONAL_ENV_VARS.items():
            ...
            if category in {"tool", "messaging"}:
                assert name in _EV0_PROVIDER_ENV_BLOCKLIST, (
...
class TestSanePathIncludesHomebrew:
    ...
    def _disable_ev0_bin_injection(self):
        ...
        saved = local_mod._EV0_BIN_DIR
        local_mod._EV0_BIN_DIR = None  # resolved -> no dir to inject
        yield
        local_mod._EV0_BIN_DIR = saved
...
class TestEv0BinDirOnPath:
    """The 3v0 install dir is reachable in the terminal subshell PATH.

    Plugins shelling out to bare ``3v0`` via the terminal tool must work
    even when the gateway was launched without the 3v0 install dir on
    PATH (systemd, service managers, cron). See the discussion that motivated
    _resolve_ev0_bin_dir / _prepend_ev0_bin_dir.
    """

    def _reset_cache(self):
        from tools.environments import local as local_mod
        local_mod._EV0_BIN_DIR = local_mod._SENTINEL
    ...
    def test_make_run_env_injects_ev0_bin_dir(self):
        ...
        from tools.environments.local import _make_run_env
        self._reset_cache()
        local_mod._EV0_BIN_DIR = "/opt/3v0/bin"
        with patch.dict(os.environ, {"PATH": "/usr/bin:/bin"}, clear=True):
            result = _make_run_env({})
        entries = result["PATH"].split(os.pathsep)
        assert entries[0] == "/opt/3v0/bin"
        assert "/usr/bin" in entries


class TestEv0InternalDynamicSecrets:
    """Dynamically-named 3V0 secrets injected at gateway/CLI startup must
    not leak into terminal subprocesses.

    The static ``_EV0_PROVIDER_ENV_BLOCKLIST`` is name-based and derived
    ...
    ``_is_ev0_internal_secret`` is the single source of truth; every spawn
    path (``_sanitize_subprocess_env``, ``_make_run_env``,
    ``ev0_subprocess_env``, Docker forward filter, ``env_passthrough``)
    ...
    def test_gateway_relay_static_names_in_blocklist(self):
        ...
        assert "GATEWAY_RELAY_SECRET" in _EV0_PROVIDER_ENV_BLOCKLIST
        assert "GATEWAY_RELAY_DELIVERY_KEY" in _EV0_PROVIDER_ENV_BLOCKLIST
        assert "GATEWAY_RELAY_ID" in _EV0_PROVIDER_ENV_BLOCKLIST
