...
def test_resolve_stdio_command_falls_back_to_usr_local_bin():
    """When ``npx`` isn't on the filtered PATH and isn't under ``$EV0_HOME/node/bin``
    ...
    the upstream ``node:bookworm-slim`` image (which the 3V0 Docker image
    ...
    # the other candidates ($EV0_HOME/node/bin/npx and ~/.local/bin/npx)
