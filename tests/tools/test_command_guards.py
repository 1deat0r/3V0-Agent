...
def _mode_manual(monkeypatch):
    ...
    The test conftest redirects EV0_HOME to an empty tempdir, so the
...
def _clean_state():
    ...
    saved = {}
    for k in ("EV0_INTERACTIVE", "EV0_GATEWAY_SESSION", "EV0_EXEC_ASK", "EV0_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["EV0_INTERACTIVE"] = "1"
        session_key = os.getenv("EV0_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No EV0_INTERACTIVE or EV0_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
