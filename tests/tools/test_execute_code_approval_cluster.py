...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad EV0_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: EV0_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_threev0_allowlist_and_secret_blocks():
    ...
    env = {
        ...
        "EV0_HOME": "/h", "EV0_PROFILE": "p",
        "EV0_CONFIG": "/c.yaml", "EV0_ENV": "/e",
        "EV0_DELEGATED_CHILD_CONTEXT": "1",
        # other EV0_* → dropped (broad prefix removed)
        "EV0_BASE_URL": "https://x", "EV0_INTERACTIVE": "1",
        "EV0_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in (
        ...
        "EV0_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "EV0_BASE_URL", "EV0_INTERACTIVE", "EV0_KANBAN_DB",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped EV0_* vars."""
    import logging
