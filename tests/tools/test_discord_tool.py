...
class TestCheckRequirements:
    ...
    def test_gateway_tool_prompt_gate_uses_active_profile_token(self, monkeypatch):
        ...
        scope_token = secret_scope.set_secret_scope({"UNRELATED_SECRET": "value"})
        try:
            with patch("threev0_cli.config.load_config") as load_config:
...
# ---------------------------------------------------------------------------
# Toolset: discord / discord_admin only in 3v0-discord
# ---------------------------------------------------------------------------
...
class TestToolsetInclusion:
    def test_discord_tools_only_in_ev0_discord_toolset(self):
        ...
        assert "discord" not in _EV0_CORE_TOOLS
        assert "discord_admin" not in _EV0_CORE_TOOLS

    def test_discord_tools_not_in_other_toolsets(self):
        from toolsets import TOOLSETS
        for name, ts in TOOLSETS.items():
            if name in {"3v0-discord", "3v0-gateway", "discord", "discord_admin"}:
                continue
            tools = ts.get("tools", [])
...
class TestConfigAllowlist:
    ...
    def test_config_load_failure_is_permissive(self, monkeypatch):
        ...
        monkeypatch.setattr("threev0_cli.config.load_config", bad_load)
        assert _load_allowed_actions_config() is None
...
class TestModelToolsIntegration:
    ...
    def test_discord_admin_schema_rebuilt_by_get_tool_definitions(
        ...
        tools = get_tool_definitions(enabled_toolsets=["3v0-discord"], quiet_mode=True,
