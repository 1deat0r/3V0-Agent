...
``ev0_tools.py`` and ``script.py`` into a temp dir, and those files
...
def _legacy_posix_scrubber(source_env, is_passthrough):
    ...
    Deliberately updated for #27303 (the broad ``EV0_`` prefix was dropped
...
class TestPosixEquivalence:
    ...
    _POSIX_SYNTHETIC_ENV = {
        ...
        # EV0_* handling (#27303): only the operational allowlist passes;
        # every other EV0_* is dropped (the broad prefix was removed).
        "EV0_HOME": "/home/alice/.3V0",        # allowlisted → kept
        "EV0_PROFILE": "default",                 # allowlisted → kept
        "EV0_INTERACTIVE": "1",                   # not allowlisted → dropped
        "EV0_BASE_URL": "https://api.internal",   # not allowlisted → dropped
        "EV0_KANBAN_DB": "postgres://u:p@h/db",   # not allowlisted → dropped
...
# ``ev0_tools.py`` stub, and the LLM's ``script.py``.  Both contain
...
class TestSandboxWritesUtf8:
    ...
    def test_stub_and_script_writes_specify_utf8(self):
        """Both ``ev0_tools.py`` and ``script.py`` writes in
...
# let the child import ev0_tools, a user script that printed non-ASCII
