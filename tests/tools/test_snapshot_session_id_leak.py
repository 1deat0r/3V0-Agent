"""Cross-session EV0_SESSION_ID leak via the shared bash snapshot.
...
``EV0_SESSION_ID`` into the snapshot, so every LATER session ``source``d that
stale value and its ``echo $EV0_SESSION_ID`` reported a FOREIGN session's id
— overriding the correct per-command Popen env injected by
``_inject_session_context_env``.

The fix strips the per-session bridged vars (EV0_SESSION_* / UI /
...
def test_export_snippet_shape():
    snippet = _export_dump_excluding_session_vars('"$__ev0_snap_tmp"')
    ...
    assert "${!EV0_SESSION_*}" in snippet
    assert "${!EV0_CRON_AUTO_DELIVER_*}" in snippet
    assert "EV0_UI_SESSION_ID" in snippet
    assert "grep -vE" not in snippet
    assert '"$__ev0_snap_tmp"' in snippet
    ...
    assert snippet.rstrip().endswith('> "$__ev0_snap_tmp"')
    ...
    try:
        def run_as(sid):
            ...
            def worker():
                ...
                out["r"] = env.execute('echo "[$EV0_SESSION_ID]"')
        ...
        if os.path.exists(snap):
            with open(snap) as f:
                assert "EV0_SESSION_ID" not in f.read()
    finally:
        env.cleanup()
