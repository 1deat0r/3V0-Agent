...
class GetSpillConfigTests(unittest.TestCase):
    ...
    def test_load_config_exception_is_swallowed(self):
        with patch("threev0_cli.config.load_config", side_effect=RuntimeError("bad")):
            cfg = hos.get_spill_config()
        self.assertEqual(cfg["max_chars"], hos.DEFAULT_MAX_CHARS)
        self.assertTrue(cfg["enabled"])
...
class SpillIfOversizedTests(unittest.TestCase):
    def setUp(self):
        self.tmpdir = tempfile.mkdtemp(prefix="3v0-spill-test-")
    ...
    def test_default_directory_uses_threev0_home(self):
        ...
        try:
            with patch.dict(os.environ, {"EV0_HOME": test_home}):
                # Also patch get_ev0_home to the env var to mirror production.
            ...
            # ~/.3V0/hook_outputs depending on get_ev0_home behaviour.
            candidates = [
                Path(test_home) / "hook_outputs" / "sess",
                Path(os.path.expanduser("~/.3V0/hook_outputs/sess")),
            ]
