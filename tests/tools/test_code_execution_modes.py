...
class TestExecuteCodeModeIntegration(unittest.TestCase):
    ...
    def test_strict_mode_runs_in_tmpdir(self):
        ...
        self.assertIn("ev0_sandbox_", result["output"])
    ...
    def test_project_mode_interpreter_is_venv_python(self):
        ...
        # The 3v0-agent venv is always active during tests, so this also
    ...
    def test_project_mode_can_still_import_threev0_tools(self):
        ...
        breaks `from ev0_tools import terminal`.
        """
        import tempfile
...
# ---------------------------------------------------------------------------
# _python_environment_prefix / _uses_ev0_python_environment
# ---------------------------------------------------------------------------
...
class TestPythonEnvironmentPrefix(unittest.TestCase):
    ...
    def test_failure_is_not_cached(self):
        ...
        A sticky cached failure would silently drop the 3v0 root from
...
class TestUsesEv0PythonEnvironment(unittest.TestCase):
    """Unit tests for _uses_ev0_python_environment."""
    ...
    def test_true_for_current_interpreter_without_probe(self):
        ...
        must never drop the 3v0 root for the interpreter 3V0 itself runs.
        """
        with patch("subprocess.run",
                   side_effect=subprocess.TimeoutExpired(cmd=[], timeout=5)) as mock_run:
            self.assertTrue(_uses_threev0_python_environment(sys.executable))
...
# ---------------------------------------------------------------------------
# PYTHONPATH composition — 3v0 root included only for same-env interpreters
# ---------------------------------------------------------------------------

class TestPythonPathComposition(unittest.TestCase):
    """Verify 3v0 root inclusion in PYTHONPATH depends on env match.

    Patches ``_uses_ev0_python_environment`` directly so these tests are
