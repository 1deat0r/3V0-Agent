...
- Signal forwarding runs through the init so ``docker stop`` triggers
  3v0's own graceful-shutdown path.
...
def test_dockerfile_installs_an_init_for_zombie_reaping(dockerfile_text):
    ...
    Without a PID-1 init that handles SIGCHLD, 3v0 accumulates zombie
    ...
    assert installed, (
        ...
        "reap orphaned subprocesses, 3v0 accumulates zombies in Docker "
...
def test_dockerfile_entrypoint_routes_through_the_init(dockerfile_text):
    ...
    PID 1 and 3v0 would run without zombie reaping.
    """
    ...
    assert "/opt/3v0/docker/entrypoint-dispatch.sh" in entrypoint_line, (
    ...
    assert "exec /init /opt/3v0/docker/main-wrapper.sh" in dispatcher, (
