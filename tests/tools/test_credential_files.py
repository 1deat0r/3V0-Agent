...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.3V0/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.3V0/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.3V0/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape EV0_HOME."""
        monkeypatch.setenv("EV0_HOME", str(tmp_path / ".3V0"))
        (tmp_path / ".3V0").mkdir()

        # Create a sensitive file one level above ev0_home
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # The resolved path escapes EV0_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        assert "/root/.3V0/cache/documents" in paths
        assert "/root/.3V0/cache/audio" in paths
        assert "/root/.3V0/cache/videos" in paths
    ...
    def test_empty_threev0_home(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.3V0/attachments" in container_paths
        assert "/root/.3V0/images" in container_paths
        assert "/root/.3V0/cache/images" in container_paths
        for mount in mounts:
    ...
    def test_images_upload_dir_is_mounted(self, tmp_path, monkeypatch):
        ...
        Desktop / clipboard / PDF uploads land in ``EV0_HOME/images``, not
    ...
    def test_images_upload_file_maps_into_container(self, tmp_path, monkeypatch):
        ...
        assert (
            ...
            == "/root/.3V0/images/upload_20260722_181019_1.png"
        )
...
class TestToAgentVisiblePathPerBackend:
    ...
    3V0 cache — not just docker — and skips the ones where the host path
    ...
    def test_docker_maps_to_root_ev0(self, tmp_path, monkeypatch):
        ...
        assert to_agent_visible_cache_path(staged) == "/root/.3V0/attachments/drop.zip"

    def test_ssh_maps_to_tilde_ev0(self, tmp_path, monkeypatch):
        staged = self._staged(tmp_path, monkeypatch)
        monkeypatch.setenv("TERMINAL_ENV", "ssh")
        from tools.credential_files import to_agent_visible_cache_path
        assert to_agent_visible_cache_path(staged) == "~/.3V0/attachments/drop.zip"
...
class TestMasterCredentialStoresAreNeverMountable:
    """Containment is not enough — EV0_HOME *is* where the keys live.
    ...
    ``../../.ssh/id_rsa`` from escaping EV0_HOME, but every master
    ...
    def test_refused_entry_does_not_block_the_rest_of_the_batch(self, tmp_path):
        ...
        paths = [m["container_path"] for m in mounts]
        assert "/root/.3V0/google_token.json" in paths
        assert "/root/.3V0/.env" not in paths
