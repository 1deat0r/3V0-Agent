...
code paths 3V0 takes on native Windows.
...
class TestConfigureWindowsStdio:
    """``threev0_cli.stdio.configure_windows_stdio`` wiring.
    ...
    - respect EV0_DISABLE_WINDOWS_UTF8 opt-out
    """
    ...
    def _reset_configured(self, monkeypatch):
        ...
        sys.modules.pop("threev0_cli.stdio", None)
        # Fresh import now; tests import from threev0_cli.stdio themselves,
        # but this guarantees the module they get is a brand-new copy.
        import threev0_cli.stdio as _s
        _s._CONFIGURED = False
        yield
        sys.modules.pop("threev0_cli.stdio", None)

    def test_no_op_on_posix(self, monkeypatch):
        from threev0_cli import stdio
...
class TestSigkillFallback:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("threev0_cli.kanban_db", 'getattr(signal, "SIGKILL", signal.SIGTERM)'),
        ],
    )
...
class TestWebServerPtyBridgeGuard:
    ...
    def test_import_guard_present_in_source(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "threev0_cli" / "web_server.py").read_text(encoding="utf-8")
...
class TestEntryPointsConfigureStdio:
    """cli.py, threev0_cli/main.py, gateway/run.py must call configure_windows_stdio."""

    @pytest.mark.parametrize(
        "relpath",
        ["cli.py", "threev0_cli/main.py", "gateway/run.py"],
    )
    def test_entry_point_calls_configure_stdio(self, relpath):
        root = Path(__file__).resolve().parents[2]
        source = (root / relpath).read_text(encoding="utf-8")
        assert "configure_windows_stdio" in source, (
            f"{relpath} must call threev0_cli.stdio.configure_windows_stdio() "
...
class TestSubprocessCompatHelpers:
    """threev0_cli/_subprocess_compat.py POSIX + Windows behaviour."""

    def test_is_windows_matches_sys_platform(self):
        from threev0_cli import _subprocess_compat as sc
    ...
    def test_resolve_node_command_returns_absolute_on_posix(self):
        ...
        from threev0_cli._subprocess_compat import resolve_node_command
    ...
    def test_windows_detach_flags_includes_breakaway_from_job(self):
        ...
        Without it, the gateway-respawn watcher spawned by ``3v0 update``
        (which runs under 3v0-setup.exe, itself a grandchild of the
...
# ---------------------------------------------------------------------------
# threev0_cli/kanban_db.py waitpid guard
# ---------------------------------------------------------------------------
...
class TestKanbanWaitpidWindowsGuard:
    ...
    def test_source_gates_waitpid_loop(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "threev0_cli" / "kanban_db.py").read_text(encoding="utf-8")
...
class TestCodeExecutionTransportTcpFallback:
    ...
    AF_INET endpoints based on the EV0_RPC_SOCKET format.
    """
...
class TestNpmBareSpawnsResolved:
    ...
    shutil.which / threev0_cli._subprocess_compat.resolve_node_command
    so Windows can execute the .cmd batch shims."""

    @pytest.mark.parametrize(
        "relpath",
        [
            "threev0_cli/tools_config.py",
            "threev0_cli/doctor.py",
...
class TestGatewayDetachedWatcherWindowsFlags:
    ...
    def test_threev0_cli_gateway_uses_compat_kwargs(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "threev0_cli" / "gateway.py").read_text(encoding="utf-8")
        assert "windows_detach_popen_kwargs" in source, (
            "threev0_cli/gateway.py must use the platform-aware detach helper"
        )
    ...
    def test_launch_detached_profile_gateway_restart_inlined_watcher_uses_breakaway(self):
        ...
        threev0_cli._subprocess_compat; the watcher must call that helper from
        ...
        """
        root = Path(__file__).resolve().parents[2]
        text = (root / "threev0_cli" / "gateway.py").read_text(encoding="utf-8")
        ...
        block = text[idx:end]
        assert "from threev0_cli._subprocess_compat import" in block
    ...
    def test_watcher_threads_hidden_console_spec_into_respawn(self):
        ...
        The spec supplies the stable cwd + env overlay (EV0_HOME,
        ...
        assert '_popen_kwargs["env"]' in block, (
            ...
            "EV0_HOME) from the restart spec."
        )
...
class TestWindowlessGatewayRestartSpec:
    ...
    def test_noop_on_non_windows(self):
        import threev0_cli.gateway_windows as gw

        argv = ["/path/venv/bin/python", "-m", "threev0_cli.main", "gateway", "run"]
    ...
    def test_windows_keeps_console_python_and_preserves_tail(self):
        ...
        it up — a pre-import so the lazy ``threev0_cli.gateway`` import didn't
        re-run ``gateway/status``'s ``import msvcrt`` branch, and a mock of
        ``get_ev0_home`` because the real one's ``Path.resolve()`` consults
...
class TestGatewayRunRestartWatcherOuterPopenFallback:
    ...
    def test_outer_watcher_retries_without_breakaway_on_oserror(self, monkeypatch):
        ...
        assert "_EV0_GATEWAY" not in kw1["env"]
    ...
    def test_outer_watcher_dual_failure_warns_without_leaking_secrets(
        ...
        monkeypatch.setenv("EV0_TEST_SECRET", secret)
        ...
        assert secret in (kwargs_used.get("env") or {}).get("EV0_TEST_SECRET", "")
