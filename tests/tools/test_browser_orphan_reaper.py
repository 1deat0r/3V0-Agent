...
def _make_socket_dir(tmpdir, session_name, pid=None, owner_pid=None):
    ...
    Args:
        ...
        owner_pid: owning 3v0 PID to write to <session>.owner_pid
...
class TestOwnerPidCrossProcess:
    ...
    The owner_pid file records which 3v0 process owns a daemon so that
    concurrent 3v0 processes don't reap each other's active browser
...
class TestLeakedDaemonWithLiveOwner:
    ...
    long-lived 3v0 process, pinning ~5 CPU cores and driving load to 100+.
...
class TestPeriodicOrphanReap:
    ...
    boot — which is exactly what happens in a 3v0 process that stays up
    for days.
    """
