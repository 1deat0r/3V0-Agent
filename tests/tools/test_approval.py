...
class TestApprovalModeParsing:
    ...
    def test_config_bool_false_maps_to_off(self):
        with mock_patch("threev0_cli.config.load_config_readonly", return_value={"approvals": {"mode": False}}):
...
class TestSmartApproval:
    ...
    def test_smart_approval_does_not_allowlist_the_pattern_for_session(self, monkeypatch):
        ...
        monkeypatch.setenv("EV0_SESSION_KEY", session_key)
        monkeypatch.setenv("EV0_EXEC_ASK", "1")
        monkeypatch.delenv("EV0_CRON_SESSION", raising=False)
        monkeypatch.setattr(
            approval_module,
...
class TestDetectDangerousRm:
    ...
    def test_nonrecursive_verification_artifact_cleanup_is_not_dangerous(self):
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
            for prefix in ("3v0-verify-", "3v0-ad-hoc-"):
    ...
    def test_symlinked_temp_dir_only_exempts_canonical_target(self, tmp_path):
        ...
        basename = "3v0-verify-example.py"
    ...
    def test_verification_cleanup_exemption_rejects_broader_deletions(self):
        commands = (
            "rm -rf /tmp/3v0-verify-example.py",
            "rm -f /tmp/3v0-verify-example.py /tmp/other.py",
            "rm -f /tmp/nested/../3v0-verify-example.py",
            "rm -f /tmp/a/../../tmp/3v0-verify-example.py",
            "rm -f /var/tmp/3v0-verify-example.py",
            "rm -f /tmp/3v0-verify-*",
            "rm -f /tmp/3v0-verify-$(touch>/tmp/pwned).py",
            "rm -f /tmp/3v0-ad-hoc-`touch>/tmp/pwned`.py",
            "rm -f /tmp/3v0-verify-example.py; touch /tmp/pwned",
        )
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
...
class TestWindowsShellDestructiveCommands:
    def test_windows_destructive_requires_approval(self):
        cases = [
            (r"cmd /c del /f /q C:\tmp\3v0-victim\file.txt", "Windows cmd destructive delete"),
            (r"cmd.exe /k rmdir /s /q C:\tmp\3v0-victim", "Windows cmd destructive delete"),
            ...
            (r"powershell Remove-Item -Recurse -Force C:\tmp\3v0-victim",
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"EV0_SESSION_KEY": "bob"}, clear=False):
...
class TestEv0ConfigWriteProtection:
    """Terminal-side pairing for the file_tools write_file/patch deny on
    ~/.3V0/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_write_idioms_against_config(self):
        for command in (
            "echo 'approvals:' > ~/.3V0/config.yaml",
            "echo '  mode: off' >> ~/.3V0/config.yaml",
            "echo x | tee ~/.3V0/config.yaml",
            "echo x | tee $EV0_HOME/config.yaml",
            "cp /tmp/evil.yaml ~/.3V0/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
            assert key is not None, command
    ...
    def test_reads_and_unrelated_writes_are_safe(self):
        # Reading config is not a write; a non-3V0 absolute config.yaml is
        # handled by the project patterns, not the 3V0-home rule.
        for cmd in (
            "cat ~/.3V0/config.yaml",
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.3V0/config.yaml/.env must require approval — the
...
class TestWindowsAbsolutePathFolding:
    """Windows absolute home / 3V0-home prefixes must fold to ~/ and
    ~/.3V0/ in dangerous-command detection.
    ...
    SSH, and 3V0 config/env files returned "safe" without an approval prompt.
    The OS-specific ``Path.home()`` / ``get_ev0_home()`` tests above only
    exercise this branch on a Windows host; these monkeypatch a Windows-style
    HOME/EV0_HOME so the fold is verified on the POSIX CI runner too."""
...
class TestSmartDeniedPrompt:
    ...
    def test_smart_deny_uses_locale_specific_once_deny_choices(self, monkeypatch, capsys):
        monkeypatch.setenv("EV0_LANGUAGE", "tr")
...
class TestGatewayProtection:
    ...
    def test_gateway_run_backgrounded_detected(self):
        cmd = "kill 1605 && cd ~/.3V0/3v0-agent && source venv/bin/activate && python -m threev0_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc
        for variant in (
            "python -m threev0_cli.main gateway run --replace &",
            "nohup python -m threev0_cli.main gateway run --replace",
        ):
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart 3v0-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestIFSWhitespaceBypass:
    ...
    def test_ifs_forms_still_flagged_dangerous(self):
        for cmd in (
            ...
            # In-place edit of the 3V0 security config via IFS.
            "sed${IFS}-i ~/.3V0/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(cmd)
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep 3v0) bypasses the pkill/killall name-matching
    ...
    def test_kill_pidof_expansion_detected(self):
        """`kill $(pidof 3v0)` is the BSD/Linux equivalent of the
        pgrep expansion and bypasses the pkill/killall name pattern
        in the same way. See issue #33071."""
        dangerous, _, desc = detect_dangerous_command("kill -TERM $(pidof threev0_cli.main)")
        assert dangerous is True
        assert "pidof" in desc.lower() or "pgrep" in desc.lower()
        assert detect_dangerous_command("kill -9 `pidof 3v0`")[0] is True
...
class TestLaunchctlGatewayLifecycle:
    """launchctl stop/kickstart/bootout/unload against the 3V0 service
    label achieves the same effect as `3v0 gateway stop|restart` and
    ...
    def test_launchctl_against_threev0_label_detected(self):
        for cmd in (
            ...
            "launchctl bootout system/ai.3v0.gateway",
            "launchctl unload ~/Library/LaunchAgents/ai.3v0.gateway.plist",
        ):
            dangerous, _, desc = detect_dangerous_command(cmd)
            assert dangerous is True, cmd

    def test_unrelated_labels_not_flagged(self):
        """Read-only inspection, and lifecycle ops on non-3V0 labels, are
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("EV0_GATEWAY_SESSION", "EV0_CRON_SESSION",
                      "EV0_YOLO_MODE",
                      "EV0_SESSION_KEY", "EV0_INTERACTIVE")
        }
        os.environ.pop("EV0_YOLO_MODE", None)
        os.environ.pop("EV0_INTERACTIVE", None)
        # EV0_CRON_SESSION takes priority over EV0_GATEWAY_SESSION in
        ...
        os.environ.pop("EV0_CRON_SESSION", None)
        os.environ["EV0_GATEWAY_SESSION"] = "1"
        os.environ["EV0_SESSION_KEY"] = self.SESSION_KEY
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_import_error_allows_when_fail_open_or_disabled(self, enabled, fail_open):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("threev0_cli.config.load_config_readonly", return_value=cfg):
                with _patch("tools.approval.detect_dangerous_command", return_value=(False, None, None)):
                    with mock_patch.dict("os.environ", {"EV0_INTERACTIVE": "1"}, clear=False):
