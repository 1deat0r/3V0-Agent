...
1. `3v0 secrets bitwarden setup --help` works (dispatch path)
2. `3v0 update --check` works (update path)
3. `3v0 secrets bitwarden disable` works (handler execution)
4. `3v0 secrets onepassword status` works (lazy backend loads on demand)
...
def _run_ev0(args: list[str], timeout: int = 30) -> subprocess.CompletedProcess[str]:
...
class TestSecretsDispatchE2E:
    ...
    def test_bitwarden_status(self) -> None:
        """`3v0 secrets bitwarden status` must exit 0 (runs lazy backend)."""
        result = _run_ev0(["secrets", "bitwarden", "status"])
    ...
    def test_bitwarden_disable(self) -> None:
        """`3v0 secrets bitwarden disable` must exit 0."""
        result = _run_ev0(["secrets", "bitwarden", "disable"])
        assert result.returncode == 0, (
    ...
    def test_onepassword_status(self) -> None:
        """`3v0 secrets onepassword status` must exit 0 (1Password lazy backend)."""
        result = _run_ev0(["secrets", "onepassword", "status"])
        assert result.returncode in (0, 1), (
    ...
    def test_onepassword_setup_help(self) -> None:
        """`3v0 secrets onepassword setup --help` must exit 0."""
        result = _run_ev0(["secrets", "onepassword", "setup", "--help"])
...
class TestUpdatePathE2E:
    ...
    These tests invoke the real `3v0 update --check` path as a subprocess.
    ...
    def test_update_check_clean(self) -> None:
        """`3v0 update --check` must not load cryptography._rust."""
        result = _run_ev0(["update", "--check"])
    ...
    def test_main_update_check_crypto_absent_in_sys_modules(self) -> None:
        """Decisive invariant: invoking main() with argv=['3v0','update','--check']
