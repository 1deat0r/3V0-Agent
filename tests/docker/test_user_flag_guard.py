...
Build the real image and verify the actual runtime behavior:
  ...
  3. --user <3v0-uid> (10000) is allowed (supported non-root start)
"""
...
def test_arbitrary_user_uid_rejected(
    ...
    assert "EV0_UID" in combined or "PUID" in combined, (
...
def test_user_pinned_to_threev0_uid_works(
    ...
    assert r.returncode == 0, (
        f"--user 10000:10000 (3v0 UID) was rejected: {r.stderr[-500:]}"
    )
    assert "OK" in r.stdout