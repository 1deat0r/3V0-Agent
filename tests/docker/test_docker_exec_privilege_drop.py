...
The shim (docker/3v0-exec-shim.sh, installed at /opt/3v0/bin/3v0)
exists to prevent the auth.json ownership-mismatch bug where
`docker exec <c> 3v0 login` would write /opt/data/auth.json as
...
failed: 3V0 is not logged into Nous Portal" on every message.

These tests verify:

1. ``docker exec <c> 3v0 …`` (defaulting to root) gets dropped to the
   3v0 user before the real binary runs.
2. ``docker exec --user 3v0 <c> 3v0 …`` (already non-root) short-
   circuits and doesn't try to drop again.
3. Files written under $EV0_HOME from a ``docker exec`` session land
   as 3v0:3v0 — the actual user-visible invariant.
4. The EV0_DOCKER_EXEC_AS_ROOT opt-out lets diagnostic sessions keep
...
def _wait_for_cont_init(container: str) -> None:
    ...
    ``01-3v0-setup`` cont-init hook (docker/stage2-hook.sh) has
    finished seeding + ``chown 3v0:3v0`` config.yaml and running the
    ...
    clobbered the root-written file's ownership back to 3v0:3v0,
    failing ``test_shim_opt_out_keeps_root`` non-deterministically.

    The reliable "cont-init is done" signal is
    ``$EV0_HOME/logs/container-boot.log``: it is written by
    ``02-reconcile-profiles`` (threev0_cli.container_boot), which s6 runs
    *strictly after* ``01-3v0-setup`` in lexicographic order. The
...
def test_shim_drops_root_to_threev0_uid(sleep_container: str) -> None:
    ...
    pure-3v0 "print my uid" command, so we use the venv's python directly
    ...
    fact that the venv's `3v0` is a console_scripts entry — under the
    hood it's a tiny Python wrapper. We can't easily inject "print my uid"
    into it without forking subcommands. Simplest approach: have `3v0`
    do anything that writes to disk, then check the file's owner.

    Use `3v0 config set` which writes config.yaml under EV0_HOME.
    ...
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "3v0", "config", "set", "_test.shim_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: stdout={r.stdout!r} stderr={r.stderr!r}"

    # The written file must be owned by 3v0, not root.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    assert r.stdout.strip() == "3v0:3v0", (
        f"config.yaml owned by {r.stdout.strip()!r}, expected 3v0:3v0. "
        "The shim did not drop privileges before invoking 3v0."
    )
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    The shim sits at /opt/3v0/bin earliest on PATH; main-wrapper.sh
    invokes `s6-setuidgid 3v0 3v0 <args>` which resolves `3v0`
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    Pre-shim: ``docker exec <c> 3v0 login`` (root) wrote
    ...
    "3V0 is not logged into Nous Portal" on every message.

    We can't do a real OAuth login in a unit test, but we can stand in
    for it by writing the same file shape via `3v0 config set`-style
    writes — what matters is the *file ownership invariant* downstream
    of `_save_auth_store`. If the shim works, every file the
    `docker exec` path produces is 3v0-readable.

    Specifically: pretend the operator ran `3v0 login` (writes
    auth.json) and verify (a) the file exists and (b) it's readable by
    the 3v0 UID. We use `3v0 auth list` since that touches the
    ...
    # `3v0 auth list` is read-only but still exercises _load_auth_store
    # under the shim's UID. We invoke `3v0 config set` first to
    # provoke a write into EV0_HOME so we have something concrete to
    # owner-check.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "3v0", "config", "set", "_test.e2e_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: {r.stderr}"

    # The supervised UID (10000) must be able to read everything under
    # EV0_HOME that docker exec just wrote.
    r = subprocess.run(
        ["docker", "exec", "--user", "3v0", sleep_container,
    ...
    assert not unreadable, (
        "Files written by `docker exec` are unreadable to the 3v0 user "
