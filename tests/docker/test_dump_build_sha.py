"""Regression test: ``3v0 dump`` reports a real git SHA inside the container.
...
is absent and ``3v0 dump`` correctly falls back to ``(unknown)``.

This test handles both cases:

* If ``/opt/3v0/.ev0_build_sha`` exists in the image, assert that
  ``3v0 dump`` surfaces its content as the version SHA (not
  ``(unknown)``).
...
def _run_dump(image: str) -> str:
    ...
    Dockerfile) so the container's 3v0 user (UID 10000) can bootstrap
    ...
    assert r.returncode == 0, (
        f"3v0 dump exited {r.returncode}: "
...
def _read_baked_sha_from_image(image: str) -> str | None:
    """Return the ``/opt/3v0/.ev0_build_sha`` content, or None if absent."""
    r = subprocess.run(
        [
            "docker", "run", "--rm", "--entrypoint", "cat", image,
            "/opt/3v0/.ev0_build_sha",
        ],
...
def test_dump_reports_baked_sha_when_present(built_image: str) -> None:
    """When the image was built with ``EV0_GIT_SHA``, dump must surface it.
    ...
    # CI path: build-arg was set, baked file exists.  ``3v0 dump``
