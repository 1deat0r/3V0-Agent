...
Override the image with ``EV0_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("EV0_TEST_IMAGE", "3v0-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``EV0_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("EV0_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged 3v0 user
# ---------------------------------------------------------------------------
#
# Background: every 3V0 runtime path inside the container drops to UID
# 10000 (the ``3v0`` user) via ``s6-setuidgid 3v0``. ``docker exec``
...
# fails (PermissionError swallowed) for 3v0, so a test that ran as root
...
# helpers below run every probe under ``-u 3v0`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: 3v0).
