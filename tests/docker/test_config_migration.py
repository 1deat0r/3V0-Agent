...
Build the real image and verify: a config.yaml present in $EV0_HOME
is migrated by docker_config_migrate.py on boot, running as the 3v0
user.
"""
...
def test_config_migration_runs_on_boot(
    ...
    """A config.yaml in $EV0_HOME must be migrated on boot by
    docker_config_migrate.py, running as the 3v0 user."""
    # Start container
    ...
    assert "EXISTS" in r.stdout, (
        f"config.yaml not found in $EV0_HOME: {r.stdout}"
    )

    # Verify the migration script exists in the image
    r = docker_exec_sh(
        container_name,
        "test -f /opt/3v0/scripts/docker_config_migrate.py && "
    ...
    # Verify config.yaml is owned by 3v0 (migration ran as 3v0)
    r = docker_exec_sh(
        container_name,
        'stat -c "%U" /opt/data/config.yaml',
        timeout=10,
    )
    assert r.stdout.strip() == "3v0", (
        f"config.yaml not owned by 3v0 (migration may have run as root): "
        f"{r.stdout.strip()}"
    )
