...
def test_dashboard_service_resets_home(
    ...
    container with EV0_DASHBOARD=1 and verify the dashboard process
...
def test_dashboard_does_not_auto_insecure_from_host(
    ...
    EV0_DASHBOARD_HOST. The auth gate is the authority now.
    ...
    """
    start_container(built_image, container_name, "EV0_DASHBOARD=1", "EV0_DASHBOARD_HOST=0.0.0.0")
...
def test_stage2_repairs_profiles_and_cron_ownership(
    ...
    The stage2 hook chowns these dirs to 3v0:3v0 on every boot.
    ...
    # Verify files are now owned by 3v0.
    r = docker_exec_sh(
        container_name,
        'stat -c "%U" /opt/data/profiles/testprof/marker '
        '/opt/data/cron/root_owned.json',
    ...
    assert "3v0" in r.stdout, (
        f"expected 3v0-owned files after restart, got: {r.stdout!r} — "
