"""Runtime smoke test for Docker $EV0_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the 3v0 user after container boot.
...
registered later runs its log service as the dropped 3v0 user and
...
def test_logs_gateways_seeded_and_ev0_owned(
    ...
    assert "logs=3v0" in r.stdout, (
        f"logs/ not owned by 3v0: {r.stdout}"
    )
    assert "gateways=3v0" in r.stdout, (
        f"logs/gateways/ not owned by 3v0: {r.stdout}"
    )
