...
package-lock.json. Without EV0_TUI_DIR set, ``_make_tui_argv`` falls
...
The fix is ``ENV EV0_TUI_DIR=/opt/3v0/ui-tui`` in the Dockerfile, which
...
def _exec_py(image: str, py: str) -> str:
    """Run a Python snippet inside the image as the 3v0 user, return stdout."""
    inner = (
        "source /opt/3v0/.venv/bin/activate && "
        "cd /opt/3v0 && "
        f"python3 -c {shlex.quote(py)}"
    )
    # Drop to the 3v0 user (UID 10000) so we exercise the same path the
    # dashboard PTY child runs as — not root.
    cmd = [
        "docker", "run", "--rm", "--entrypoint", "su", image,
        "3v0", "-s", "/bin/bash", "-c", inner,
    ]
...
def test_threev0_tui_dir_env_is_set(built_image: str) -> None:
    ...
    r = subprocess.run(
        ["docker", "run", "--rm", "--entrypoint", "sh", built_image,
         "-c", 'printf "%s" "$EV0_TUI_DIR"'],
        capture_output=True, text=True, timeout=60,
    )
    assert r.returncode == 0, r.stderr[-2000:]
    assert r.stdout.strip() == "/opt/3v0/ui-tui", (
        f"EV0_TUI_DIR={r.stdout.strip()!r} (expected /opt/3v0/ui-tui)"
    )
...
def test_prebuilt_bundle_present_and_no_runtime_install(built_image: str) -> None:
    ...
    py = (
        ...
        "from threev0_cli.main import _tui_need_npm_install, _find_bundled_tui, _make_tui_argv\n"
        "ui = Path('/opt/3v0/ui-tui')\n"
    ...
    # With EV0_TUI_DIR set, _make_tui_argv returns the prebuilt path BEFORE
