...
# `threev0_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
...
def _threev0_home_points_at_production(value: str) -> bool:
    ...
    imports (logging handlers, ``threev0_state.DEFAULT_DB_PATH``) froze paths
...
# like a real CLI") — which used to disarm threev0_state's live-DB guard in
...
# threev0_state._running_under_pytest() as a test-context signal. A child
...
_EV0_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``threev0_cli.voice.speak_text`` — real synthesis,
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    # 3b. threev0_state computes ``DEFAULT_DB_PATH = get_ev0_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from threev0_state import ...``) that
    ...
    try:
        import threev0_cli.plugins as _plugins_mod
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    Only patches when ``threev0_cli.kanban_db`` is *already imported* — a
    ...
    """
    _kdb = sys.modules.get("threev0_cli.kanban_db")
    if _kdb is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import threev0_cli.kanban_db`
...
# ``threev0_state._ensure_test_isolation`` (the single choke point every
...
# so subprocess children that import threev0_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("threev0_state")
...
# correlated with a ``tests/threev0_cli/`` pytest run starting up.
#
...
def _wal_is_usable() -> bool:
    ...
    IMPORTANT: this must NOT import ``threev0_state``. That module computes
    ...
    duplicated from ``threev0_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      ``threev0_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``threev0_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``threev0_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``threev0_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
def _live_system_guard(request, monkeypatch):
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                ...
                # whose cmdline contains "python -m threev0_cli.main".
                if (
                    "3v0" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        ...
        # equivalent `python -m threev0_cli.main update`).  These commands
        ...
        if "update" in low and (
            ...
            or
            # python -m threev0_cli.main update --gateway
            ("threev0_cli" in low and "update" in low.split())
            or
...
def _audio_playback_guard(request, monkeypatch):
    ...
    try:
        import threev0_cli.voice as _voice
    except Exception:
