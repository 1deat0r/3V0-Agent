...
an isolated daemon separately, then point this script at its socket:
    ...
    cua-driver serve --embedded --socket /tmp/3v0-cua-0-9-live.sock \
        --no-permissions-gate --no-overlay
    CUA_DRIVER_LIVE_SOCKET=/tmp/3v0-cua-0-9-live.sock \
...
async def run_smoke(socket_path: str) -> dict[str, dict[str, Any]]:
    session_id = f"3v0-cua-live-{uuid.uuid4().hex[:8]}"
    ...
    file_descriptor, temporary_name = tempfile.mkstemp(
        prefix="3v0-cua-live-", suffix=".txt"
    )
