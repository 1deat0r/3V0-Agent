...
Uses a throwaway EV0_HOME so it never touches ~/.3v0. No real names.
"""
...
def _fresh_home():
    """Point EV0_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_ev0_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["EV0_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER EV0_HOME is set.
...
# a different get_ev0_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"EV0_HOME (throwaway): {HOME}")
