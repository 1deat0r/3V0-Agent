...
Precedence verified here:
    EV0_MAX_TOKENS env  >  model.max_tokens  >  per-provider
...
def isolated_home(tmp_path, monkeypatch):
    """Isolated EV0_HOME with a writable config.yaml and a clean module cache.

    These tests deliberately re-import ``threev0_cli`` / ``gateway`` so each
    ...
    _saved = {
        ...
        if k.startswith(("threev0_cli", "gateway"))
    }
    ...
    def fresh_gateway():
        for mod in list(sys.modules.keys()):
            if mod.startswith(("threev0_cli", "gateway")):
                del sys.modules[mod]
