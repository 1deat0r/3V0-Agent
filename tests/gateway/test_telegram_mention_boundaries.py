...
substrings like 'foo@ev0_bot.example'.
...
def _make_adapter():
    ...
    adapter._bot = SimpleNamespace(id=999, username="ev0_bot")
    return adapter
...
def _telegram_mention_entity(text, mention="@ev0_bot", entity_type="mention"):
...
class TestRealMentionsAreDetected:
    ...
    def test_mention_after_non_bmp_characters_uses_telegram_offsets(self):
        adapter = _make_adapter()
        text = "\U0001f680\U0001f680 @ev0_bot hello"
...
class TestSubstringFalsePositivesAreRejected:
    ...
    def test_email_like_substring(self):
        """bug #12545 exact repro: 'foo@ev0_bot.example'."""
        adapter = _make_adapter()
        msg = _message(text="email me at foo@ev0_bot.example")
    ...
    def test_substring_inside_url_without_entity(self):
        ...
        adapter = _make_adapter()
        msg = _message(text="see https://example.com/@ev0_bot for details")
        assert adapter._message_mentions_bot(msg) is False


    def test_email_substring_in_caption(self):
        adapter = _make_adapter()
        msg = _message(caption="foo@ev0_bot.example")
        assert adapter._message_mentions_bot(msg) is False
...
class TestCaseInsensitivity:
    ...
    def test_uppercase_mention(self):
        adapter = _make_adapter()
        text = "hi @EV0_BOT"
        msg = _message(text=text, entities=[_mention_entity(text, mention="@EV0_BOT")])
        assert adapter._message_mentions_bot(msg) is True
...
class TestTelegramUtf16EntityOffsets:
    def test_extracts_bot_mention_username_after_non_bmp_characters(self):
        text = "\U0001f9ea\U0001f9ea @ev0_bot please"
        msg = _message(text=text, entities=[_telegram_mention_entity(text)])
        assert TelegramAdapter._extract_bot_mention_usernames(msg) == {"ev0_bot"}

    def test_bot_command_suffix_after_non_bmp_characters_mentions_bot(self):
        adapter = _make_adapter()
        text = "\U0001f9ea\U0001f9ea /new@ev0_bot"
        entity = _telegram_mention_entity(text, mention="/new@ev0_bot", entity_type="bot_command")
