...
class TestSend:
    ...
    async def test_send_posts_to_webhook(self):
        ...
        assert payload["markdown"]["title"] == "3V0"
...
class TestMentionPatterns:
    ...
    def test_pattern_matches_text(self, monkeypatch):
        adapter = _make_gating_adapter(
            monkeypatch, extra={"mention_patterns": ["^3v0"]}
        )
        assert adapter._message_matches_mention_patterns("3v0 please help") is True
        assert adapter._message_matches_mention_patterns("please 3v0 help") is False
