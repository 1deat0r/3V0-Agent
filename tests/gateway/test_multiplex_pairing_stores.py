...
def test_secondary_profile_pairing_stores_created(tmp_path, monkeypatch):
    ...
    with patch("ev0_cli.profiles.profiles_to_serve", return_value=[
        ("coder", tmp_path / ".3V0" / "profiles" / "coder"),
    ]), patch("ev0_cli.profiles.get_active_profile_name", return_value="default"):
