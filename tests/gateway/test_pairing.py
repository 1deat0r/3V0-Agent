...
class TestProfileScopedDiscovery:
    def test_list_approved_scopes_platform_discovery_to_profile_dir(self, tmp_path):
        ...
        # A profile's store anchors to the 3v0 ROOT, not the current
        # EV0_HOME — the current home may itself be a profile, and nesting
        ...
        with patch("gateway.pairing.PAIRING_DIR", global_dir), patch(
            ...
            # consolidated layout a standalone `3v0 -p alice` resolves —
...
class TestProfileScopedStorage:
    ...
    under each profile's own 3V0 home so a multiplexing gateway can keep
    ...
    def test_profile_store_uses_profiles_subdir(self, tmp_path, monkeypatch):
        """Explicit profile stores use that profile's normal 3V0 layout."""
        monkeypatch.setenv("EV0_HOME", str(tmp_path))
