...
class TestAsyncDeliverySupported:
    ...
    def test_set_false_is_unsupported(self):
        ...
        try:
            ...
            assert get_session_env("EV0_SESSION_PLATFORM") == "api_server"
        finally:
...
class TestDeclareStatelessChannel:
    """``3v0 -z`` and cron cannot receive a completion after their turn ends.

    Cron clears the ``EV0_SESSION_*`` routing keys, so an async delegation's
