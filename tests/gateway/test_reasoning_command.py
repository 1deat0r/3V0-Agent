...
class TestReasoningCommand:
    ...
    def test_resolve_session_reasoning_prefers_session_override(self, tmp_path, monkeypatch):
        threev0_home = tmp_path / "3v0"
        threev0_home.mkdir()
        (threev0_home / "config.yaml").write_text("agent:\n  reasoning_effort: low\n", encoding="utf-8")
    ...
    def test_run_agent_includes_enabled_mcp_servers_in_gateway_toolsets(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_run, "_threev0_home", threev0_home)
        monkeypatch.setattr(gateway_run, "_env_path", threev0_home / ".env")
...
class TestLoadShowReasoningCoercion:
    ...
    def _load_with_config(self, tmp_path, monkeypatch, yaml_body: str) -> bool:
        threev0_home = tmp_path / "3v0"
        threev0_home.mkdir()
        (threev0_home / "config.yaml").write_text(yaml_body, encoding="utf-8")
        monkeypatch.setattr(gateway_run, "_threev0_home", threev0_home)
