"""Unit tests for ``threev0_cli.proxy_cli`` command handlers.
...
from threev0_cli import proxy_cli
...
# ---------------------------------------------------------------------------
# _load_env_file_into_environ — setup discovers keys kept only in ~/.3V0/.env
# ---------------------------------------------------------------------------
...
def test_cmd_disable_uses_public_status_pid_not_private_read_pid(
    ...
    # mutation only.
    from threev0_cli.config import load_config as _lc
    cfg2 = _lc()
...
def test_register_cli_uses_egress_command_dest():
    ...
    the inbound OAuth '3v0 proxy' subparser (dest='proxy_command').
    A future grep-and-refactor on proxy_command should not hit this
    subparser by accident."""

    parser = argparse.ArgumentParser(prog="3v0 egress")
