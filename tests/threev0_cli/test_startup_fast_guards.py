"""Guards for threev0_cli._startup_fast — the pre-import version fast path.
...
1. IMPORT WEIGHT: _startup_fast must stay stdlib-only. The whole point of
   ...
   ``from threev0_cli.config import ...`` silently makes `3v0 --version`
...
def test_startup_fast_import_weight():
    ...
    probe = (
        "import sys, json\n"
        "import threev0_cli._startup_fast\n"
    ...
    assert not offenders, (
        f"threev0_cli._startup_fast imported heavy modules: {offenders} — "
...
def _run_version(env_overrides: dict) -> subprocess.CompletedProcess:
    ...
    env.pop("EV0_DEV", None)
    return subprocess.run(
        [sys.executable, "-m", "threev0_cli.main", "--version"],
        capture_output=True,
        text=True,
        timeout=60,
        cwd=REPO_ROOT,
...
def test_fast_version_parity_off_termux(tmp_path):
    home = tmp_path / ".3V0"
    home.mkdir()
    result = _run_version({"EV0_HOME": str(home), "TERMUX_VERSION": ""})
    assert result.returncode == 0, result.stderr
    out = result.stdout
    for field in ("3V0 Agent v", "Install directory:", "Python:", "OpenAI SDK:"):
...
def test_fast_version_parity_on_termux(tmp_path):
    ...
    home.mkdir()
    result = _run_version(
        {"EV0_HOME": str(home), "TERMUX_VERSION": "0.118"}
    )
    assert result.returncode == 0, result.stderr
    assert "3V0 Agent v" in result.stdout
