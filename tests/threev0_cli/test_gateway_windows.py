"""Tests for threev0_cli.gateway_windows."""
...
import threev0_cli.gateway as gateway
import threev0_cli.gateway_windows as gateway_windows
import threev0_cli.setup as setup
...
def test_spawn_detached_marks_primary_breakaway_success(monkeypatch, tmp_path, caplog):
    ...
    argv = ["python.exe", "-m", "threev0_cli.main", "gateway", "run"]
    cwd = str(tmp_path)
    calls = []
...
def test_gateway_vbs_script_is_console_less(monkeypatch):
    ...
    assert "threev0_cli.main" in content
