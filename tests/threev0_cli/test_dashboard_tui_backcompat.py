"""Regression test: `3v0 dashboard --tui` must not hard-crash.

Older 3V0 desktop app shells (<= 0.15.x) spawn the backend as::

    3v0 dashboard --no-open --tui --host 127.0.0.1 --port <PORT>
...
died before it became ready and the desktop GUI showed only "3V0 couldn't
...
def _run_cli(args, timeout=60):
    """Invoke the real threev0_cli.main parser in a subprocess.
