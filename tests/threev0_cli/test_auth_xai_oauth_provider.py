...
def test_refresh_xai_oauth_tokens_preserves_active_provider(tmp_path, monkeypatch):
    ...
    _setup_ev0_auth(ev0_home, access_token=near, refresh_token="rt-old")
    monkeypatch.setenv("EV0_HOME", str(ev0_home))
...
def _seed_xai_oauth_state(
    ev0_home: Path, state: dict, *, active_provider: str = "xai-oauth"
) -> None:
    ev0_home.mkdir(parents=True, exist_ok=True)
    auth_store = {
        "version": 1,
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure(
    ...
    """
    ev0_home = tmp_path / "3v0"
    _seed_xai_oauth_state(ev0_home, dict(_STALE_XAI_OAUTH_STATE), active_provider="nous")
    monkeypatch.setenv("EV0_HOME", str(ev0_home))
    ...
    raw = json.loads((ev0_home / "auth.json").read_text())
...
def test_credential_pool_seeds_xai_oauth_from_singleton(tmp_path, monkeypatch):
    ...
    _setup_ev0_auth(ev0_home, access_token=fresh, refresh_token="rt-1")
    monkeypatch.setenv("EV0_HOME", str(ev0_home))
...
def test_auth_remove_xai_oauth_clears_singleton_and_sticks(tmp_path, monkeypatch):
    ...
    # nothing to import.
    raw_after = json.loads((ev0_home / "auth.json").read_text())
...
def test_pool_sync_back_writes_to_singleton(tmp_path, monkeypatch):
    ...
    _setup_ev0_auth(ev0_home, access_token=expired, refresh_token="rt-old")
    monkeypatch.setenv("EV0_HOME", str(ev0_home))
...
def test_runtime_provider_uses_pool_entry_for_xai_oauth(tmp_path, monkeypatch):
    ...
    ev0_home = tmp_path / "3v0"
    fresh = _jwt_with_exp(int(time.time()) + 2 * 60 * 60)
    _setup_ev0_auth(ev0_home, access_token=fresh)
    monkeypatch.setenv("EV0_HOME", str(ev0_home))
    monkeypatch.delenv("EV0_XAI_BASE_URL", raising=False)
    monkeypatch.delenv("XAI_BASE_URL", raising=False)
...
def test_pool_refresh_recovers_when_other_process_already_refreshed(tmp_path, monkeypatch):
    ...
    _setup_ev0_auth(ev0_home, access_token=in_memory_at, refresh_token="rt-shared")
    monkeypatch.setenv("EV0_HOME", str(ev0_home))
    ...
    def _fake_refresh(access_token, refresh_token, **kwargs):
        ...
        (ev0_home / "auth.json").write_text(json.dumps(raw))
        raise AuthError(
...
def test_pool_manual_entry_does_not_sync_back_to_singleton(tmp_path, monkeypatch):
    ...
    _setup_ev0_auth(ev0_home, access_token=singleton_at, refresh_token="rt-singleton")
    monkeypatch.setenv("EV0_HOME", str(ev0_home))
...
def test_pool_sync_back_preserves_active_provider(tmp_path, monkeypatch):
    ...
    _setup_ev0_auth(ev0_home, access_token=near_expiry, refresh_token="rt-xai")
    monkeypatch.setenv("EV0_HOME", str(ev0_home))
