...
from threev0_cli.main import cmd_update, PROJECT_ROOT
...
class TestCmdUpdateNpmLockfileCache:
    ...
    def test_record_npm_lockfile_hash(self, tmp_path, monkeypatch):
        from threev0_cli import main as hm
    ...
    def test_update_uses_one_shared_npm_cache_across_profiles(
        ...
        monkeypatch.setattr(threev0_constants.Path, "home", lambda: tmp_path)
        monkeypatch.setattr(
            threev0_constants, "find_node_executable", lambda _name: "/usr/bin/npm"
        )
...
class TestCmdUpdateTermuxUvBootstrap:
    ...
    def test_termux_reuses_existing_path_uv_without_pip(self, mock_run, monkeypatch):
        ...
        monkeypatch.setattr("threev0_cli.managed_uv.resolve_uv", lambda: None)
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_non_interactive_runs_safe_config_migrations(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "threev0_cli.config.get_missing_env_vars", return_value=["MISSING_KEY"]
        ), patch(
            "threev0_cli.config.get_missing_config_fields",
            return_value=[{"key": "new.option", "default": True}],
        ), patch(
            "threev0_cli.update_cmd._reload_config_modules"
        ), patch(
            "threev0_cli.update_cmd._run_config_check_fresh", return_value=(1, 2)
        ), patch(
            "threev0_cli.update_cmd._run_migrate_config_fresh",
            return_value={"env_added": [], "config_added": ["new.option"]},
        ) as migrate_config, patch("threev0_cli.main.sys") as mock_sys:
...
class TestCmdUpdateMigrationPrompt:
    ...
    def test_version_bump_only_applies_silently_without_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "threev0_cli.config.get_missing_env_vars", return_value=[]
        ), patch(
            "threev0_cli.config.get_missing_config_fields", return_value=[]
        ), patch(
            "threev0_cli.update_cmd._reload_config_modules"
        ), patch(
            "threev0_cli.update_cmd._run_config_check_fresh", return_value=(5, 24)
        ), patch(
            "threev0_cli.update_cmd._run_migrate_config_fresh",
    ...
    def test_version_bump_only_surfaces_migration_resets(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "threev0_cli.update_cmd._run_config_check_fresh", return_value=(33, 34)
        ), patch(
            "threev0_cli.update_cmd._run_migrate_config_fresh",
            return_value={
                "env_added": [],
    ...
    def test_new_options_are_listed_by_name_before_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "threev0_cli.config.get_missing_env_vars", return_value=env_items
        ), patch(
            "threev0_cli.config.get_missing_config_fields", return_value=cfg_items
        ), patch(
            "threev0_cli.update_cmd._reload_config_modules"
        ), patch(
            "threev0_cli.update_cmd._run_config_check_fresh", return_value=(1, 24)
        ), patch(
            "threev0_cli.update_cmd._run_migrate_config_fresh",
            return_value={"env_added": [], "config_added": [], "warnings": []},
        ), patch("threev0_cli.main.sys") as mock_sys:
...
class TestConfigVersionCheckUsesFreshModules:
    ...
    returned the OLD cached ``threev0_cli.config`` module — so
    ...
    def test_run_config_check_fresh_reloads_modules(self):
        ...
        sys.modules held the OLD threev0_cli.config with the OLD
        DEFAULT_CONFIG["_config_version"] after git pull.
        """
        from unittest.mock import patch

        import threev0_cli.update_cmd as update_cmd
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        with (
            patch("threev0_cli.profiles.list_profiles", return_value=all_profiles),
            patch("threev0_cli.profiles.seed_profile_skills", side_effect=fake_seed),
    ...
    def test_single_profile_default_is_synced(
        ...
        with (
            patch("threev0_cli.profiles.list_profiles", return_value=[default_p]),
            patch("threev0_cli.profiles.seed_profile_skills", side_effect=fake_seed),
            patch("tools.skills_sync.sync_skills", return_value=empty_sync),
        ):
            cmd_update(mock_args)
...
class TestCmdUpdateZipBranchRefusal:
    ...
    def test_zip_fallback_refuses_non_main_branch(self, capsys):
        from threev0_cli.main import _update_via_zip
...
class TestNodeRuntimeNpmResolution:
    ...
    def test_wsl_update_skips_windows_npm_build_paths(self, mock_args, monkeypatch):
        ...
        monkeypatch.setattr(threev0_constants, "is_wsl", lambda: True)
        monkeypatch.setattr(
            threev0_constants,
