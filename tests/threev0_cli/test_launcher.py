"""Tests for the top-level `./3v0` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./3v0` should use `threev0_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "3v0-cli"
    called = []

    fake_main_module = types.ModuleType("threev0_cli.main")

    def fake_main():
        called.append("threev0_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "threev0_cli.main", fake_main_module)
    ...
    assert called == ["threev0_cli.main"]
