"""Regression test: ``3v0 mcp add --command`` must not clobber the
top-level ``args.command`` subparser dest.

The top-level argparse parser uses ``dest="command"`` for its subparsers
(``threev0_cli/_parser.py``).  The dispatcher in ``threev0_cli/main.py``
...
result, ``3v0 mcp add foo --url ...`` silently launches chat instead
...
def _build_parser():
    """Minimal replica of the slice of the 3v0 parser that exhibits
...
class TestMcpAddCommandDest:
    def test_url_invocation_preserves_top_level_command(self):
        """`3v0 mcp add foo --url ...` must keep args.command == "mcp".
