...
Covers:
 - Normal path (tokens already in 3V0 auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — 3V0
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `3v0 auth openai-codex`. The old
...
@pytest.fixture()
def threev0_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in 3V0 auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(threev0_auth_only_env):
    """openai-codex appears when tokens are already in 3V0 auth store."""
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or 3V0
    auth store."""
    threev0_home = tmp_path / ".3V0"
    threev0_home.mkdir()
