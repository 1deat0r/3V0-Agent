...
def test_seed_from_singletons_respects_ev0_pkce_suppression(tmp_path, monkeypatch):
    ...
    import yaml
    (ev0_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    ...
    monkeypatch.setattr(aa, "read_ev0_oauth_credentials", lambda: {
