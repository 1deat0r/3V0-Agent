...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    from threev0_cli.auth_commands import auth_add_command
    ...
    payload = json.loads((tmp_path / "3v0" / "auth.json").read_text())
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "threev0_cli.auth._nous_device_code_login",
        lambda **kwargs: {
    ...
    # `3v0 auth add nous` must also populate providers.nous so the
    ...
    # raises "3V0 is not logged into Nous Portal" and the agent dies.
...
def test_auth_add_nous_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`3v0 auth add nous --type oauth --label <name>` must preserve the
...
def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``3v0 auth add openai-codex`` runs for different ChatGPT
    ...
    second independent one. ``3v0 auth list`` showed two labels sharing
    ...
    monkeypatch.setattr("threev0_cli.auth._codex_device_code_login", lambda: next(logins))
...
def test_codex_auth_status_reports_pool_only_credential(tmp_path, monkeypatch):
    ...
    from threev0_cli.auth import get_codex_auth_status
...
def test_codex_runtime_pool_only_rate_limit_is_not_missing_auth(tmp_path, monkeypatch):
    ...
    from threev0_cli.auth import AuthError, CODEX_RATE_LIMITED_CODE, resolve_codex_runtime_credentials
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    """3v0 auth add xai-oauth must set active_provider and write a pool entry.
...
def test_auth_add_xai_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``3v0 auth add xai-oauth`` runs must produce independent pool entries.
...
def test_auth_remove_codex_migrates_legacy_dict_suppression(tmp_path, monkeypatch):
    ...
    payload = json.loads((tmp_path / "3v0" / "auth.json").read_text(encoding="utf-8"))
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`3v0 logout --provider openai-codex` must still clear model.provider.
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from threev0_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
# =============================================================================
# Unified credential-source stickiness — every source 3V0 reads from has a
...
def test_seed_from_singletons_respects_threev0_pkce_suppression(tmp_path, monkeypatch):
    """anthropic ev0_pkce must not re-seed from ~/.3V0/.anthropic_oauth.json when suppressed."""
    threev0_home = tmp_path / "3v0"
    threev0_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("EV0_HOME", str(threev0_home))

    import yaml
    (threev0_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (threev0_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["ev0_pkce"]},
    }))

    # Stub the readers so only ev0_pkce is "available"; claude_code returns None
    import agent.anthropic_adapter as aa
    monkeypatch.setattr(aa, "read_threev0_oauth_credentials", lambda: {
    ...
    # ev0_pkce suppressed, claude_code returns None → nothing should be seeded
    assert entries == []
    assert "ev0_pkce" not in active
...
def test_auth_remove_copilot_suppresses_all_variants(tmp_path, monkeypatch):
    ...
    from types import SimpleNamespace
    from threev0_cli.auth import is_source_suppressed
    from threev0_cli.auth_commands import auth_remove_command

    with patch(
        "threev0_cli.copilot_auth.resolve_copilot_token",
        return_value=("ghp_fake", "gh"),
    ), patch(
        "threev0_cli.copilot_auth.get_copilot_api_token",
