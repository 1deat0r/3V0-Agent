...
`3v0 --tui --resume latest --in ./dir` (and the classic-CLI equivalents)
...
def test_top_level_in_value_not_mistaken_for_subcommand(monkeypatch):
    # `3v0 --in chat` — "chat" is the flag's value, not the subcommand.
    import sys

    import threev0_cli.main as mod

    monkeypatch.setattr(sys, "argv", ["3v0", "--in", "chat", "--resume", "latest"])
