...
``3v0 update`` killed mid-install (Ctrl-C, terminal close, WSL OOM) gets
...
def _stub_install_env(monkeypatch, m, seen):
    ...
    monkeypatch.setattr("threev0_cli.managed_uv.ensure_uv", lambda: None)
    # The install executor moved to threev0_cli._install_repair (shared between
    ...
    import threev0_cli._install_repair as ir
