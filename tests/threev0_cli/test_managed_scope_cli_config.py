...
threev0_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `3v0 config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_ev0_home = get_ev0_home()`` at import time (module
    singleton), so monkeypatching EV0_HOME after import doesn't move it.
    ...
    this only matters for tests that swap EV0_HOME.)
    """
    import cli
