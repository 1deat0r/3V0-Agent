...
hand-rolled parsers — ``threev0_cli.config.load_env``,
``threev0_cli.main._has_any_provider_configured``, and
...
def test_config_load_env_strips_export_prefix(tmp_path):
    from threev0_cli.config import invalidate_env_cache, load_env
...
def test_skills_tool_load_env_strips_export_prefix(tmp_path, monkeypatch):
    ...
    # skills_tool.load_env reads get_ev0_home()/.env directly.
    import importlib
...
def test_has_any_provider_configured_with_export_prefix(tmp_path, monkeypatch):
    ...
    import threev0_cli.main as hmain

    importlib.reload(hmain)
    # get_env_path() derives from EV0_HOME (set above) → tmp_path/.env, so
    # no patching is needed. Re-clear os.environ provider keys that
    # load_ev0_dotenv may have populated at import/reload time, forcing the
