"""
Tests for --yes / --force flag separation in `3v0 skills install`.
...
def test_cli_skills_install_yes_sets_skip_confirm(monkeypatch):
    ...
    monkeypatch.setattr("threev0_cli.skills_hub.skills_command", fake_skills_command)
    monkeypatch.setattr(
        sys,
        "argv",
        ["3v0", "skills", "install", "official/email/agentmail", "--yes"],
    )
