...
which broke ``3v0 --tui`` because ``ui-tui/`` had vanished.
...
from threev0_cli.main import _atomic_replace_dir
...
def test_atomic_replace_swaps_content_on_success(tmp_path: Path) -> None:
    ...
    assert not (dst.parent / "ui-tui.3v0-update-staging").exists()
    assert not (dst.parent / "ui-tui.3v0-update-old").exists()
