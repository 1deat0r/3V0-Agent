"""Tests for threev0_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("threev0_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from threev0_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    ...
    @patch("threev0_cli.config.load_config")
    ...
    @patch("threev0_cli.config.load_config")
    @patch.dict("os.environ", {"EV0_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """EV0_SESSION_PLATFORM should be used when EV0_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from threev0_cli.skills_config import _get_categories
        skills = [
