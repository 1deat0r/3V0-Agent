"""Tests for threev0_cli.managed_uv — one path, no guessing."""
...
def _runtime_info(
    ...
    from threev0_cli.sqlite_runtime import SQLiteRuntimeInfo
...
def _RRR(status):
    ...
    """
    from threev0_cli.managed_uv import RuntimeRepairResult
...
class TestEnsureUv:
    ...
    def test_install_reports_runtime_repair_to_observer(self, tmp_path):
        from threev0_cli.managed_uv import (
            RuntimeRepairResult,
            ensure_uv,
        )
...
class TestEnsureUvUpdateBoundary:
    ...
    ``threev0_cli.main`` against the freshly pulled ``managed_uv``. A release
...
class TestEnsureUvWindowsSafe:
    ...
    def test_uvresult_would_break_windows_list2cmdline(self):
        ...
        import subprocess
        from threev0_cli.managed_uv import _UvResult
...
class TestUpdateManagedUv:
    ...
    def test_fresh_stamp_skips_network_self_update_but_not_repair(self, tmp_path, monkeypatch):
        ...
        from threev0_cli.managed_uv import RuntimeRepairResult, update_managed_uv
    ...
    def test_stale_stamp_runs_self_update_and_refreshes_stamp(self, tmp_path):
        ...
        from threev0_cli.managed_uv import UV_SELF_UPDATE_INTERVAL_SECONDS, update_managed_uv
...
class TestManagedPythonStore:
    def test_store_is_checkout_scoped_across_profiles(self, tmp_path, monkeypatch):
        from threev0_cli.managed_uv import managed_python_install_dir
    ...
    def test_environment_is_private_and_sanitized(self, tmp_path):
        from threev0_cli.managed_uv import managed_python_env
...
class TestRuntimeRepair:
    def test_safe_runtime_is_a_noop(self, tmp_path):
        from threev0_cli.managed_uv import repair_vulnerable_runtime
    ...
    def test_stage_candidate_sync_keeps_uv_project_config(self, tmp_path):
        from threev0_cli.managed_uv import _stage_candidate_venv
...
class TestRuntimeCutover:
    def test_os_lock_blocks_concurrent_repair_and_releases(self, tmp_path):
        from threev0_cli.managed_uv import _acquire_repair_lock, _release_repair_lock
    ...
    def test_post_swap_smoke_failure_rolls_back_live_venv(self, tmp_path):
        from threev0_cli.managed_uv import _cut_over_candidate
...
class TestInstallUvInternals:
    def test_posix_sets_uv_unmanaged_install(self, tmp_path):
        ...
        with patch("threev0_cli.managed_uv._install_uv_posix") as mock_posix:
            from threev0_cli.managed_uv import _install_uv
            _install_uv(target)
...
class TestRuntimeRequestMinorLine:
    ...
    def test_requests_minor_line(self):
        from threev0_cli.managed_uv import _runtime_request
...
class TestDefaultLiveVenv:
    ...
    def test_dot_venv_only_is_targeted(self, tmp_path):
        from threev0_cli.managed_uv import _default_live_venv
...
class TestVenvPythonUpdateBoundary:
    """``_venv_python`` must survive a threev0_constants predating its symbol.

    ``3v0 update`` imports threev0_constants from the OLD checkout, ``git
    pull`` replaces that file, and the freshly-pulled managed_uv then runs its
    lazy ``from threev0_constants import venv_python_path`` against the module
    ...
    plainly contains it, which is what made the error so confusing:
        ...
        cannot import name 'venv_python_path' from 'threev0_constants'
        (~/.3V0/3v0-agent/threev0_constants.py)
    ...
    def test_recovers_when_the_cached_module_predates_the_symbol(self, monkeypatch):
        ...
        from threev0_cli.managed_uv import _venv_python
        ...
        monkeypatch.delattr(threev0_constants, "venv_python_path", raising=False)
    ...
    def test_recovery_uses_the_shared_helper_not_a_second_copy(self, monkeypatch):
        """The reload must resolve through threev0_constants, not open-code it.

        Hand-rolling `Scripts`/`bin` here is what #76105 deduped away and what
        `test_no_open_coded_venv_layout_remains_in_threev0_cli` bans.
        """
        import threev0_constants
