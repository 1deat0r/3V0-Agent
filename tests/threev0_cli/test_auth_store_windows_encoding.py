...
def threev0_home(tmp_path, monkeypatch):
    """Point EV0_HOME at a tmp dir so we never touch the real auth store.

    Required because ``_auth_file_path()`` has a seat belt that refuses to
    resolve to the real user's ~/.3V0/auth.json under pytest.
    """
...
class TestAuthStoreEncodingRoundTrip:
    def test_load_reads_utf8_with_non_ascii_label(self, threev0_home, windows_default_encoding):
    ...
    def test_load_does_not_corrupt_store_on_non_ascii(self, threev0_home, windows_default_encoding):
        ...
        assert not (threev0_home / "auth.json.corrupt").exists()
    ...
    def test_load_handles_utf8_with_bom(self, threev0_home):
...
class TestExplicitEncodingPassed:
    ...
    def test_load_auth_store_passes_utf8_encoding(self, threev0_home):
        auth_path = threev0_home / "auth.json"
...
# --- sibling readers of the same ~/.3V0/auth.json in other modules -------
#
# _load_auth_store lives in threev0_cli/auth.py, but several other modules read
# the same ~/.3V0/auth.json directly. They had the same UTF-8-vs-cp1252
# asymmetry on Windows — these tests pin the sibling reads too.

class TestAuthJsonSiblingReaders:
    def test_has_xai_credentials_reads_non_ascii_store(self, threev0_home, windows_default_encoding, monkeypatch):
        ...
        _write_utf8(threev0_home / "auth.json", store)

        from tools.xai_http import has_xai_credentials

        assert has_xai_credentials() is True

    def test_auxiliary_nous_provider_reads_non_ascii_store(self, threev0_home, windows_default_encoding, monkeypatch):
        ...
        # EV0_HOME env from the fixture doesn't reach it — point it at
        # the tmp store explicitly.
        monkeypatch.setattr(aux, "_AUTH_JSON_PATH", threev0_home / "auth.json")
    ...
    def test_read_shared_nous_state_reads_non_ascii_store(
        ...
        """threev0_cli.auth._read_shared_nous_state must read a non-ASCII store.
    ...
    def test_has_any_provider_configured_reads_non_ascii_auth_store(
        self, threev0_home, monkeypatch, windows_default_encoding
    ):
        """threev0_cli.main._has_any_provider_configured reads auth.json.
        ...
        # threev0_cli.auth; patch it on the source module so the local import
        # sees the fake.
    ...
    def test_managed_tool_gateway_reads_non_ascii_nous_state(
        self, threev0_home, windows_default_encoding
    ):
