...
class TestAuthStoreEncodingRoundTrip:
    def test_load_reads_utf8_with_non_ascii_label(self, ev0_home, windows_default_encoding):
    ...
    def test_load_does_not_corrupt_store_on_non_ascii(self, ev0_home, windows_default_encoding):
        ...
        assert not (ev0_home / "auth.json.corrupt").exists()
    ...
    def test_load_handles_utf8_with_bom(self, ev0_home):
...
class TestExplicitEncodingPassed:
    ...
    def test_load_auth_store_passes_utf8_encoding(self, ev0_home):
        auth_path = ev0_home / "auth.json"
...
class TestAuthJsonSiblingReaders:
    def test_has_xai_credentials_reads_non_ascii_store(self, ev0_home, windows_default_encoding, monkeypatch):
        ...
        _write_utf8(ev0_home / "auth.json", store)

        from tools.xai_http import has_xai_credentials

        assert has_xai_credentials() is True

    def test_auxiliary_nous_provider_reads_non_ascii_store(self, ev0_home, windows_default_encoding, monkeypatch):
        ...
        monkeypatch.setattr(aux, "_AUTH_JSON_PATH", ev0_home / "auth.json")
    ...
    def test_has_any_provider_configured_reads_non_ascii_auth_store(
        self, ev0_home, monkeypatch, windows_default_encoding
    ):
    ...
    def test_managed_tool_gateway_reads_non_ascii_nous_state(
        self, ev0_home, windows_default_encoding
    ):
