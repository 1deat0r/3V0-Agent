...
def main_mod(monkeypatch):
    import threev0_cli.main as mod
...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        """
        import threev0_cli.oneshot as oneshot
        from threev0_cli.main import _exit_after_oneshot
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    ...
    from threev0_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "threev0_state", mod("threev0_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "threev0_cli.config",
        mod("threev0_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "threev0_cli.models",
        mod("threev0_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    monkeypatch.setitem(
        sys.modules,
        "threev0_cli.tools_config",
        mod("threev0_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
