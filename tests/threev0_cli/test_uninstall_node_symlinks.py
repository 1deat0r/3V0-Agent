"""Tests for threev0_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $EV0_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the 3V0-managed node dir.
"""
...
import threev0_cli.uninstall as uninstall
...
def _make_threev0_node(threev0_home: Path) -> Path:
    """Create a fake $EV0_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = threev0_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    threev0_home = fake_home / ".3V0"
    _make_threev0_node(threev0_home)
    ...
    removed = uninstall.remove_node_symlinks(threev0_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    threev0_home = fake_home / ".3V0"
    node_bin = _make_threev0_node(threev0_home)
