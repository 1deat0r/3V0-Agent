"""Tests for threev0_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate EV0_HOME + reset any module-level catalog cache per test."""
    home = tmp_path / ".3V0"
    home.mkdir()
    ...
    import importlib
    from threev0_cli import model_catalog
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from threev0_cli.model_catalog import _validate_manifest
...
class TestDefaultModelFromCache:
    ...
    def test_shipped_manifest_labels_glm52_default(self, isolated_home):
        ...
        import threev0_cli.model_catalog as model_catalog
        from threev0_cli.models import PREFERRED_SILENT_DEFAULT_MODEL
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in threev0_cli.models."""
    ...
    def test_picker_nous_row_uses_curated_list(self, tmp_path, monkeypatch):
        ...
        list in curated order — matching the ``3v0 model`` CLI — not the live
        ...
        # ``EV0_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` EV0_HOME directly instead.
        import importlib
        from threev0_cli import model_catalog
        from threev0_cli.models import get_curated_nous_model_ids
        importlib.reload(model_catalog)
        try:
            from threev0_cli.model_switch import list_picker_providers

            active_home = Path(os.environ["EV0_HOME"])
            ...
            with patch.object(
                model_catalog, "_fetch_manifest", return_value=_valid_manifest()
            ), patch("threev0_cli.models.check_nous_free_tier", return_value=False), patch(
                "threev0_cli.models.union_with_portal_free_recommendations",
