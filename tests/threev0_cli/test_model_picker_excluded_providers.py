...
interactive ``3v0 model`` CLI picker.

The CLI picker (``threev0_cli.main.select_provider_and_model``) builds its
...
def test_cli_picker_hides_excluded_provider(config_home):
    ...
    from the ``3v0 model`` provider menu."""
...
def test_cli_picker_hides_excluded_provider_by_alias(config_home):
    ...
    ev0_id / alias names."""
    # 'openai' is an alias-style 3v0 id; ensure excluding it hides the
    # canonical openai provider row if present. Use the canonical slug's
    # alias from _PROVIDER_ALIASES to stay robust to renames.
    from threev0_cli.models import _PROVIDER_ALIASES, CANONICAL_PROVIDERS
