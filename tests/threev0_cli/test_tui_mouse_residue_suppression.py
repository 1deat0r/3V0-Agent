...
The Python launcher (`3v0 --tui …`) has a ~100–300ms cold-start window
where stdin is still in cooked + echo mode. If a previous 3V0 session
...
from threev0_cli.main import _suppress_mouse_residue_early
...
class TestEarlyMouseDisable:
    ...
    def test_respects_diagnostic_escape_hatch(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["3v0", "--tui"])
        monkeypatch.delenv("EV0_TUI", raising=False)
        monkeypatch.setenv("EV0_TUI_NO_EARLY_DISABLE", "1")
    ...
    def test_oserror_is_swallowed(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["3v0", "--tui"])
        monkeypatch.delenv("EV0_TUI", raising=False)
        monkeypatch.delenv("EV0_TUI_NO_EARLY_DISABLE", raising=False)
