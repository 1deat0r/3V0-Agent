"""Tests for threev0_cli/loops.py — /loop recurring in-session wakeups."""
...
class TestParseIntervalToken:
    def test_minutes(self):
        from threev0_cli.loops import parse_interval_token
...
class TestParseLoopArgs:
    def test_fixed_interval(self):
        from threev0_cli.loops import parse_loop_args
...
class TestFormatInterval:
    def test_render(self):
        from threev0_cli.loops import format_interval
...
class TestResponseSignalsComplete:
    def test_marker_on_own_line(self):
        from threev0_cli.loops import response_signals_complete
...
class TestLoopStateSerde:
    def test_round_trip(self):
        from threev0_cli.loops import LoopState
...
class TestTickLifecycle:
    ...
    def test_until_judge_done_stops(self, ev0_home):
        ...
        state.next_due_at = time.time() - 1
        mgr.fire_tick()
        with patch("threev0_cli.goals.judge_goal", return_value=("done", "suite green", False, None, False)):
    ...
    def test_until_judge_continue_loops(self, ev0_home):
        ...
        state.next_due_at = time.time() - 1
        mgr.fire_tick()
        with patch("threev0_cli.goals.judge_goal", return_value=("continue", "3 failures", False, None, False)):
    ...
    def test_until_judge_error_fails_open(self, ev0_home):
        ...
        state.next_due_at = time.time() - 1
        mgr.fire_tick()
        with patch("threev0_cli.goals.judge_goal", side_effect=RuntimeError("api down")):
