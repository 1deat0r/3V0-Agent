...
- Bare `3v0 setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `3v0 setup --quick` runs the narrower "fill in missing items" flow.
- `3v0 setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("threev0_cli.setup.ensure_threev0_home", {}),
        ("threev0_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("threev0_cli.config.is_managed", {"return_value": False}),
        ("threev0_cli.setup.load_config", {"return_value": {}}),
        ("threev0_cli.setup.save_config", {}),
        ("threev0_cli.setup.get_env_value", {"return_value": None}),
        ("threev0_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("threev0_cli.setup._print_setup_summary", {}),
        ("threev0_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("threev0_cli.setup.load_config", {"return_value": {}}),
        ("threev0_cli.setup.save_config", {}),
        ("threev0_cli.auth.get_active_provider", {"return_value": None}),
        ("threev0_cli.setup.get_env_value", {"return_value": None}),
        ("threev0_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `3v0 setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="threev0_cli.setup.prompt_choice",
                quick="threev0_cli.setup._run_quick_setup",
                model="threev0_cli.setup.setup_model_provider",
                terminal="threev0_cli.setup.setup_terminal_backend",
                agent="threev0_cli.setup.setup_agent_settings",
                gateway="threev0_cli.setup.setup_gateway",
                tools="threev0_cli.setup.setup_tools",
            )
    ...
    def test_reconfigure_on_fresh_install_falls_through(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("threev0_cli.setup.prompt_choice", {"return_value": 0}),
                first="threev0_cli.setup._run_first_time_quick_setup",
            )
            from threev0_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        import sys
        from threev0_cli.main import main

        captured = {}
        monkeypatch.setattr(
            "threev0_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["3v0", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
