...
def _refresh_bindings_against_live_module():
    """Rebind module-level names to the *current* ``threev0_cli.main``.

    Other tests in the suite (notably ``test_env_loader.py`` and
    ``test_skills_subparser.py``) reload or delete ``threev0_cli.main`` from
    ``sys.modules``.  When that happens on the same xdist worker before we
    run, our top-of-file ``from threev0_cli.main import ...`` bindings end
    up pointing at the *old* module object.  ``patch(\"threev0_cli.main.X\")``
    ...
    live = sys.modules.get("threev0_cli.main")
    if live is None:
        live = importlib.import_module("threev0_cli.main")
    ...
    def test_self_pid_excluded(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(os.getpid(), "python3 -m threev0_cli.main dashboard"),
...
class TestDashboardUpdateCleanup:
    ...
    def test_all_failed_stops_do_not_claim_the_dashboard_was_stopped(self, capsys):
        with patch(
            "threev0_cli.main._kill_stale_dashboard_processes",
    ...
    def test_wmic_routed_through_bounded_probe_run_with_ignore_errors(self):
        ...
        with patch("sys.platform", "win32"),\
            ...
            mock_probe.return_value = subprocess.CompletedProcess(
                ...
                returncode=0,
                stdout=(
                    "CommandLine=python -m threev0_cli.main dashboard\n"
                    "ProcessId=12345\n"
                ),
                stderr="",
            )
...
class TestSupervisedBackendRestart:
    ...
    def _live(self):
        return sys.modules["threev0_cli.main"]
...
class TestFilterDashboardRespawnCandidates:
    ...
    def test_dedupes_identical_normalized_cmdlines(self):
        ...
        a = ["/usr/bin/python3", "-m", "threev0_cli.main", "dashboard", "--port", "8300"]
        b = ["/other/python", "-m", "threev0_cli.main", "dashboard", "--port", "8300"]
...
class TestCmdlineCapture:
    ...
    def test_reads_proc_cmdline_when_available(self, tmp_path, monkeypatch):
        ...
        proc_file.write_bytes(b"/usr/bin/python3\x00-m\x00threev0_cli.main\x00serve\x00")
        ...
        assert argv == ["/usr/bin/python3", "-m", "threev0_cli.main", "serve"]
...
class TestPostUpdateStaleModuleReload:
    ...
    adds a new symbol to ``threev0_cli._subprocess_compat`` (as #87134 added
    ``bounded_probe_run``), the post-update dashboard cleanup's lazy
    ``from threev0_cli._subprocess_compat import bounded_probe_run`` hits the
    ...
    def test_reload_restores_missing_symbol(self):
        ...
        import threev0_cli._subprocess_compat as compat
        from threev0_cli import update_cmd
        ...
        try:
            ...
            stale = sys.modules["threev0_cli._subprocess_compat"]
            assert hasattr(stale, "bounded_probe_run")
        finally:
            importlib.reload(sys.modules["threev0_cli._subprocess_compat"])
            importlib.reload(sys.modules["threev0_cli.dashboard_procs"])
    ...
    def test_config_reload_list_includes_process_scan_modules(self):
        ...
        assert "threev0_cli._subprocess_compat" in reloaded
        assert "threev0_cli.dashboard_procs" in reloaded
