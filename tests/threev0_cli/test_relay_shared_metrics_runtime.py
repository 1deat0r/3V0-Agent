...
from threev0_cli import lifecycle, plugins
...
def test_real_binding_drives_lifecycle_aggregation_export_and_snapshot(
    ...
    from threev0_cli.observability.shared_metrics import SharedMetricsStore

    root = tmp_path / "3v0-home" / "telemetry" / "shared_metrics"
    store = SharedMetricsStore(root / "metrics.sqlite3", root / "outbox")
    tomorrow = datetime.now(timezone.utc) + timedelta(days=1)
    monkeypatch.setattr(
        "threev0_cli.observability.shared_metrics._utc_now",
        lambda: tomorrow,
    )
