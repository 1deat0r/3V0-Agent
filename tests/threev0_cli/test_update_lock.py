"""Cross-process update mutual exclusion (``threev0_cli.update_lock``).

Three surfaces can start an update of one install tree: a terminal ``3v0
...
dashboard-spawned ``3v0 update`` was mid-``npm install``, which then failed
...
def test_marker_path_follows_process_ev0_home(tmp_path, monkeypatch):
    ...
    assert update_marker_path() == tmp_path / ".3v0-update-in-progress"
...
class TestHandoffFromOrchestratingUpdater:
    """The Tauri updater holds the marker, then spawns ``3v0 update``.

    The regression: the child saw its own parent's live marker and exited 2,
    so every GUI update failed with "3V0 is still running" and retrying
...
class TestAncestryHandoff:
    ...
    ``3v0-setup`` under ``~/.3V0`` is only refreshed by a full installer
    ...
    def _liveness_pinned_true(self, monkeypatch):
        monkeypatch.setattr("threev0_cli.update_lock._pid_alive", lambda pid: True)
