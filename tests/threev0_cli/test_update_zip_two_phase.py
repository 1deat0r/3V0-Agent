...
from threev0_constants import venv_bin_dir, venv_python_path
...
def test_no_open_coded_venv_layout_remains_in_threev0_cli():
    ...
    exempt = {"stdio.py"}
    pkg = Path(threev0_cli.__file__).parent
    offenders = []
    ...
    assert not offenders, (
        "open-coded venv layout found (use threev0_constants.venv_bin_dir):\n"
        + "\n".join(offenders)
    )
...
def test_top_level_files_are_swapped_atomically(tmp_path):
    ...
    threev0_constants.py, ...). Covering only directories would leave exactly
...
def test_venv_helpers_honour_an_explicit_platform_verdict():
    ...
    `threev0_cli.main._is_windows`. A helper that reads `sys.platform`
