...
Covers:
- Defense 1: gateway stop/restart refuse when _EV0_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        "3v0 gateway restart",
        "3v0 gateway stop",
        "3v0  gateway  restart",         # double spaces
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.3v0.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.3V0/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l 3v0-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.3v0.gateway.restart-once.plist",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.3v0.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.3V0/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated 3v0 note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        "3v0 cron list",
        "3v0 update",
        "3v0 config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `3v0 gateway start` is benign — starting a gateway from inside a
        ...
        "3v0 gateway start",
        "3v0 gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway 3v0
        # services must not be falsely blocked (the old `.*3v0` matched any
        # 3v0 token).
        "launchctl unload ai.3v0.update-checker.plist",
        "launchctl restart ai.3v0.daemon",
        ...
        "systemctl restart 3v0-meta.service",
        "systemctl restart 3v0-cron-helper",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_threev0_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade 3v0 then run 3v0 gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_ev0_gateway_once.sh). The script must live under
        # EV0_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("EV0_HOME", str(tmp_path / ".3V0"))
        scripts_dir = tmp_path / ".3V0" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    """Verify 3v0 gateway stop/restart refuse when _EV0_GATEWAY=1."""
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_EV0_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _EV0_GATEWAY=1.

    Issue #37453: systemctl --user restart 3v0-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart 3v0-gateway",
        "systemctl --user restart 3v0-gateway",
        "systemctl stop 3v0-gateway.service",
        "3v0 gateway restart",
        "launchctl kickstart gui/501/ai.3v0.gateway",
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart 3v0-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.3v0.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-3v0 label: label-independent detection is the point
        # (#62891 second reproduction used `ai.3v0.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.3v0.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.3v0.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.3v0.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-3v0 systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run 3v0 gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("3v0 gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        ...
        (scripts_dir / "restart.sh").write_text(
            "launchctl kickstart -k gui/501/ai.3v0.gateway\n"
        )
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/3v0\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 3v0 gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_ev0_home()` falls back to Path.home(), which raises
        RuntimeError when neither EV0_HOME nor HOME resolves
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    threev0_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run 3v0 gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("EV0_HOME", str(tmp_path / ".3V0"))
        (tmp_path / ".3V0").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run 3v0 gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop 3v0-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart 3v0-gateway' /var/log/syslog",
        "rg '3v0 gateway restart' /home/user/.3V0/logs/",
        "journalctl -u 3v0-gateway --grep 'systemctl restart 3v0-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'ev0_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell 3v0 gateway restart"',
        'psql -c "\\! systemctl restart 3v0-gateway"',
        "grep 'systemctl restart 3v0-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart 3v0-gateway",
        'grep "$(systemctl restart 3v0-gateway)" f',
        "grep 'restart' log; systemctl restart 3v0-gateway",
        'sqlite3 db "SELECT 1"; 3v0 gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "3v0 gateway restart",
        "sudo systemctl stop 3v0-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart 3v0-gateway%'\""
        )
