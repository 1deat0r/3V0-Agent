"""Tests for threev0_cli configuration management."""
...
class TestEnsureEv0Home:
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from threev0_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
...
class TestLoadConfigParseFailure:
    ...
    def test_corrupt_config_is_backed_up(self, tmp_path, capsys):
        ...
        """
        from threev0_cli import config as cfg_mod
...
class TestEmptyConfigSections:
    ...
    def test_null_override_of_non_dict_default_still_applies(self, tmp_path):
        ...
        from threev0_cli.config import _deep_merge
...
class TestSanitizeEnvLines:
    ...
    def test_migrate_reports_normalized_line_formatting(self, capsys):
        ...
        with (
            patch("threev0_cli.config.sanitize_env_file", return_value=2),
            patch(
                "threev0_cli.config.check_config_version",
                return_value=(latest_version, latest_version),
            ),
            patch("threev0_cli.config.read_raw_config", return_value={}),
            patch("threev0_cli.config.get_missing_env_vars", return_value=[]),
            patch("threev0_cli.config.get_missing_config_fields", return_value=[]),
            patch("threev0_cli.config.get_missing_skill_config_vars", return_value=[]),
        ):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from threev0_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
...
class TestCustomProviderCompatibility:
    ...
    def _run_ladder(current_ver: int):
        from threev0_cli.config_migrations import run_migrations
...
"""
    ...
    def test_persist_migration_writes_full_read_raw_config(self, tmp_path):
        from threev0_cli.config import _persist_migration, read_raw_config
