...
from threev0_cli.codex_models import DEFAULT_CODEX_MODELS, get_codex_model_ids
...
def test_setup_wizard_codex_import_resolves():
    ...
    # This mirrors the exact import used in threev0_cli/setup.py line 873.
    # A prior bug had 'get_codex_models' (wrong) instead of 'get_codex_model_ids'.
    from threev0_cli.codex_models import get_codex_model_ids as setup_import
...
def test_fetch_from_api_keeps_supported_in_api_false_models(monkeypatch):
    ...
    """
    import sys
    from threev0_cli import codex_models
...
def test_model_command_prompts_to_reuse_or_reauthenticate_codex_session(monkeypatch, capsys):
    from threev0_cli.main import _model_flow_openai_codex
    ...
    monkeypatch.setattr(
        "threev0_cli.auth.get_codex_auth_status",
        lambda: {"logged_in": True, "source": "3v0-auth-store"},
    )
    monkeypatch.setattr(
        "threev0_cli.auth.resolve_codex_runtime_credentials",
    ...
    monkeypatch.setattr("threev0_cli.auth._login_openai_codex", _fake_login)
    monkeypatch.setattr(
