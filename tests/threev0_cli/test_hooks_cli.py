"""Tests for the ``3v0 hooks`` CLI subcommand."""
...
from threev0_cli import hooks as hooks_cli


@pytest.fixture(autouse=True)
def _isolated_home(tmp_path, monkeypatch):
    monkeypatch.setenv("EV0_HOME", str(tmp_path / "home"))
    monkeypatch.delenv("EV0_ACCEPT_HOOKS", raising=False)
...
class TestHooksList:
    def test_empty_config(self, tmp_path):
        with patch("threev0_cli.config.load_config", return_value={}):
...
class TestHooksDoctor:
    ...
    def test_unallowlisted_script_is_not_executed(self, tmp_path):
        """Regression for M4: `3v0 hooks doctor` used to run every
