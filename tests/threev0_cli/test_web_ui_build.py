...
Critical invariant: the dashboard Vite build outputs to threev0_cli/web_dist/
(vite.config.ts: outDir: "../../threev0_cli/web_dist"), NOT web/dist/.
...
def _make_web_dir(tmp_path: Path) -> tuple[Path, Path]:
    ...
    dist_dir = tmp_path / "threev0_cli" / "web_dist"
...
class TestBuildWebUIFlock:
    ...
    def test_contended_lock_without_dist_waits_then_skips_fresh_build(self, tmp_path):
        ...
        import fcntl
        import threading
        from threev0_cli.main import _build_web_ui as build
