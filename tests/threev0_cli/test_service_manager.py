"""Tests for threev0_cli.service_manager — the abstract ServiceManager
...
def test_windows_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # against the `threev0_cli` package succeeds — gateway_windows is
    # imported lazily inside the wrapper and may not yet be loaded.
    import threev0_cli.gateway_windows  # noqa: F401
    ...
    monkeypatch.setattr("threev0_cli.gateway_windows", _FakeWindowsModule)
    monkeypatch.setattr(
        "threev0_cli.gateway.find_gateway_pids",
        lambda **kw: [12345],
    )
...
# unprivileged 3v0 user from every lifecycle op. These tests run
...
def test_seed_supervise_skeleton_creates_expected_layout(tmp_path) -> None:
    ...
    from threev0_cli.service_manager import _seed_supervise_skeleton
...
def test_render_run_script_uses_replace_to_take_over_stale_holder() -> None:
    ...
    ``3v0 gateway run``, an agent action, the Open WebUI helper) holds
    the per-EV0_HOME PID lock; the supervised slot then execs a bare
    ...
    Covers both the default (root EV0_HOME, no ``-p``) and named-profile
    render paths.
    """
    default_text = S6ServiceManager._render_run_script("default", {})
    # Root profile: bare `3v0 gateway run --replace` (no -p flag).
    assert "3v0 gateway run --replace" in default_text
    assert "3v0 -p default" not in default_text
...
#
# `3v0 gateway stop` inside a container dispatches through
...
def test_s6_log_run_creates_leaf_as_ev0_without_chown(
    ...
    3v0). Restartable log/run must not pathname-chown or pathname-rm a
    3v0-writable tree from root — that is a symlink TOCTOU hole.
    """
    ...
    assert not any(line.lstrip().startswith("chown ") for line in log_text.splitlines()), (
        "restartable log/run must not invoke chown on 3v0-writable paths; "
        f"saw: {log_text!r}"
    )
    assert 's6-setuidgid 3v0 mkdir -p "$log_dir"' in log_text
    assert 's6-setuidgid 3v0 rm -f "$log_dir/lock"' in log_text
    assert 'else\n  mkdir -p "$log_dir"\n  rm -f "$log_dir/lock"\nfi\n' in log_text
...
def test_s6_log_run_never_invokes_chown_with_symlinked_log_dir(tmp_path) -> None:
    ...
    # Mark the drop so fake rm can refuse unlink outside EV0_HOME the way
    # a real 3v0 uid cannot delete a foreign root-owned lock.
    ...
    (bin_dir / "s6-setuidgid").write_text(
        "#!/bin/sh\n"
        "shift\n"
        'EV0_TEST_DROPPED=1 exec "$@"\n',
        encoding="utf-8",
    )
    real_rm = "/bin/rm"
    (bin_dir / "rm").write_text(
        "#!/bin/sh\n"
        # Privilege-dropped: no-op. Models that 3v0 cannot unlink a foreign
