"""Tests for the AI_AGENT / EV0_AGENT harness-attribution env vars.
...
3V0-specific marker, without clobbering an outer harness.

The AI_AGENT value must equal 3V0' id in the public agent-harness
registry (``3v0-agent`` in huggingface.js ``agent-harnesses.ts``) —
...
environment does not inherit the 3V0 process env, and survives the
cross-session leak guard that strips ``EV0_SESSION_*`` from subprocess
...
from threev0_cli.main import _advertise_agent_env

# Registry id — must stay in sync with huggingface.js agent-harnesses.ts.
HARNESS_ID = "3v0-agent"
...
class TestWrapCommandAdvertisesHarness:
    ...
    def _wrap(self, command: str) -> str:
        ...
        env._cwd_marker = "__EV0_CWD_testsession0__"
        env._snapshot_path = "/tmp/3v0-snap-testsession0.sh"
    ...
    def test_wrap_command_contains_export(self):
        ...
        assert 'EV0_AGENT="${EV0_AGENT:-true}"' in wrapped
    ...
    def test_shell_sets_default_and_preserves_outer(self):
        ...
        wrapped = self._wrap('echo "AI=$AI_AGENT EV0=$EV0_AGENT"')

        clean_env = {k: v for k, v in os.environ.items()
                     if k not in ("AI_AGENT", "EV0_AGENT")}
        out = subprocess.run(
            ["bash", "-c", wrapped], capture_output=True, text=True,
            env=clean_env, timeout=30,
        )
        assert f"AI={HARNESS_ID} EV0=true" in out.stdout

        outer_env = dict(clean_env, AI_AGENT="pi", EV0_AGENT="false")
        out = subprocess.run(
            ["bash", "-c", wrapped], capture_output=True, text=True,
            env=outer_env, timeout=30,
        )
        assert "AI=pi EV0=false" in out.stdout
