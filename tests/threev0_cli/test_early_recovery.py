"""Tests for threev0_cli._early_recovery — the dependency-light bootstrap
repair that runs BEFORE threev0_cli.main's third-party imports (#57828 / #58004).

Covers:
- entry-point lifecycle: a broken core import (dotenv) crashes the import of
  threev0_cli.main WITHOUT early recovery, and imports fine when recovery runs
...
from threev0_cli import _early_recovery as er
...
def _run_lifecycle_subprocess(tmp_path: Path, *, repair: bool) -> subprocess.CompletedProcess:
    ...
    script.write_text(
        textwrap.dedent(
            ...
            import threev0_cli._early_recovery as er
...
def test_broken_dotenv_crashes_main_import_without_repair(tmp_path):
    """Negative control: the shadow really breaks importing threev0_cli.main,
...
def test_early_recovery_module_is_stdlib_only(tmp_path):
    ...
    script.write_text(
        textwrap.dedent(
            ...
            STDLIB = set(sys.stdlib_module_names) | {"threev0_cli"}
            ...
            import threev0_cli._early_recovery  # noqa: F401
...
def test_core_marker_triggers_install_before_any_native_import(
    ...
    import threev0_cli._install_repair  # noqa: F401  (pre-import for patch)
...
def test_core_marker_marks_attempts_and_keeps_marker_on_install_failure(
    ...
    import threev0_cli._install_repair  # noqa: F401
