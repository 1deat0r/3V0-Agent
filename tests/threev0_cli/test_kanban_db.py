"""Tests for the Kanban DB layer (threev0_cli.kanban_db)."""
...
def test_stale_claim_reclaim_event_records_diagnostic_payload(
    ...
    timing context)."""
    import json
    import threev0_cli.kanban_db as _kb
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see threev0_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    import threev0_state as _threev0_state
    monkeypatch.setattr(
        _threev0_state, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
    _threev0_state._wal_fallback_warned_paths.clear()

    # Clear module cache so a fresh connect() is attempted
    kb._INITIALIZED_PATHS.clear()
    threev0_state._wal_fallback_warned_paths.clear()
    ...
    with _patch("threev0_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("ERROR", logger="threev0_state"):
            conn = kb.connect()
...
def test_connect_works_when_wal_is_silently_refused(tmp_path, monkeypatch, caplog):
    ...
    with _patch(
        "threev0_cli.kanban_db.sqlite3.connect",
...
def test_resolve_ev0_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    ...
    import shutil
    import sys
    import threev0_cli.kanban_db as kb
...
def test_resolve_ev0_argv_module_actually_runs():
    ...
    sufficient — if `threev0_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m threev0_cli.main --version`
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="threev0_cli.kanban_db"):
        with kb.connect() as conn:
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    """
    import struct
    from threev0_cli.kanban_db import connect
