...
EV0_HOME (tests/conftest.py isolation) and assert every store agrees
afterwards.
...
def threev0_home(monkeypatch, tmp_path):
    """Fresh EV0_HOME with .env + auth.json + config.yaml fixtures."""
    home = tmp_path / "cred_home"
    home.mkdir()
    monkeypatch.setenv("EV0_HOME", str(home))
    from threev0_cli.config import invalidate_env_cache
...
def test_delete_clears_provider_models_cache(threev0_home):
    _write_env(threev0_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    _write_auth(threev0_home, {"zai": [_zai_pool_fixture()["zai"][0]]})
    cache_path = threev0_home / "provider_models_cache.json"
...
def test_update_rotates_config_yaml_model_mirror(threev0_home):
    old = "sk-oe-" + "f" * 24
    new = "sk-oe-" + "g" * 24
    _write_env(threev0_home, OPENAI_API_KEY=old)
    _write_config(
        threev0_home,
        "model:\n"
        "  provider: custom\n"
    ...
    cfg_text = threev0_home.joinpath("config.yaml").read_text(encoding="utf-8")
