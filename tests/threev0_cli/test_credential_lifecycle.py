...
def test_delete_clears_provider_models_cache(ev0_home):
    _write_env(ev0_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    _write_auth(ev0_home, {"zai": [_zai_pool_fixture()["zai"][0]]})
    cache_path = ev0_home / "provider_models_cache.json"
...
def test_update_rotates_config_yaml_model_mirror(ev0_home):
    old = "sk-oe-" + "f" * 24
    new = "sk-oe-" + "g" * 24
    _write_env(ev0_home, OPENAI_API_KEY=old)
    _write_config(
        ev0_home,
        "model:\n"
        "  provider: custom\n"
    ...
    cfg_text = ev0_home.joinpath("config.yaml").read_text(encoding="utf-8")
