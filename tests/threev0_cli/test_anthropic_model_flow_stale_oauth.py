"""Tests for Bug #12905 fix — stale OAuth token detection in 3v0 model flow.

Bug 3: `3v0 model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.3V0/.env but no valid
...
class TestStaleOAuthTokenDetection:
    ...
    def test_stale_oauth_token_triggers_reauth(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr("threev0_cli.secret_prompt.masked_secret_prompt", lambda _: "")

        from threev0_cli.main import _model_flow_anthropic
        cfg = {}
