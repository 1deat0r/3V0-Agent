...
def _run_native_windows_gateway_start_diag(
    ...
    from threev0_cli._subprocess_compat import windows_detach_flags_without_breakaway
...
def test_s6_runtime_snapshot_reports_supervised_service(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("threev0_cli.service_manager.detect_service_manager", lambda: "s6")
    monkeypatch.setattr("threev0_cli.service_manager.get_service_manager", lambda: FakeS6Manager())
...
def test_module_has_logger():
    ...
    assert gateway.logger.name == "threev0_cli.gateway"
