"""Tests for Codex auth — tokens stored in 3V0 auth store (~/.3V0/auth.json)."""
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``3v0 auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``3v0 auth add openai-codex``.
    ...
    (threev0_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # Independent account from `3v0 auth add openai-codex` —
...
def test_save_codex_tokens_does_not_overwrite_independent_manual_entries(tmp_path, monkeypatch):
    ...
    Regression for #39236: ``3v0 auth add openai-codex`` for accounts B and C
    ...
    (threev0_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # ``3v0 auth add openai-codex`` (account B and account C).
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    ...
    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches 3V0 store
    assert not (codex_home / "auth.json").exists()

    # 3V0 auth store should have the tokens
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run 3v0 auth".
    """
    from threev0_cli.auth import (
    ...
    assert "3v0 auth" not in rendered
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from threev0_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from threev0_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def _patch_httpx_post(monkeypatch, responses):
    """Patch threev0_cli.auth.httpx.Client so .post() returns queued responses."""
    seq = iter(responses)
    ...
    monkeypatch.setattr("threev0_cli.auth.httpx.Client", lambda *a, **k: _FakeClient())
