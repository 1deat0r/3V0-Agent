"""Tests for ``3v0 doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from threev0_cli import doctor_live
from threev0_cli.doctor_live import (
    ProbeResult,
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `3v0 doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("threev0_cli.doctor.EV0_HOME", tmp_path / "home")
        monkeypatch.setattr("threev0_cli.doctor.PROJECT_ROOT", tmp_path / "root")
