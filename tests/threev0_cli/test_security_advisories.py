"""Tests for threev0_cli.security_advisories.
...
import threev0_cli.security_advisories as adv
...
def isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect EV0_HOME so banner cache and config writes are sandboxed."""
    home = tmp_path / ".3V0"
    home.mkdir()
    (home / "cache").mkdir()
...
class TestAck:
    ...
    def test_ack_advisory_persists_id(self, isolated_home, monkeypatch):
        ...
        # don't depend on the full threev0_cli.config bootstrap.
        store: dict = {"security": {}}
        monkeypatch.setattr(
            "threev0_cli.config.load_config", lambda: store
        )
        monkeypatch.setattr(
            "threev0_cli.config.save_config",
