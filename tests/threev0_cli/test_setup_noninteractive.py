...
import pytest
from threev0_cli.config import DEFAULT_CONFIG, load_config, save_config
...
class TestNonInteractiveSetup:
    ...
    def test_chat_first_run_headless_skips_setup_prompt(self, capsys):
        """Bare `3v0` should not prompt for input when no provider exists and stdin is headless."""
        from threev0_cli.main import cmd_chat

        args = _make_chat_args()

        with (
            patch("threev0_cli.main._has_any_provider_configured", return_value=False),
            patch("threev0_cli.main.cmd_setup") as mock_setup,
        ...
        out = capsys.readouterr().out
        assert "3v0 config set model.provider custom" in out
