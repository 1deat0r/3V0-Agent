...
The ``3v0 update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    # The 3v0.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM 3v0.exe' in text
