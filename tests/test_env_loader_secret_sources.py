"""Tests for the secret-source tracking in ``threev0_cli.env_loader``.

These cover the small public surface that lets `3v0 model` / `3v0 setup`
...
def test_cold_profile_hydration_seeds_op_env_bootstrap(tmp_path, monkeypatch):
    ...
    load_ev0_dotenv() reads <home>/.op.env for OP_SERVICE_ACCOUNT_TOKEN
...
def test_apply_external_secret_sources_dedupes_within_process(tmp_path, monkeypatch):
    """``load_ev0_dotenv()`` is called at module-import time from several
    hot modules (cli.py, threev0_cli/main.py, run_agent.py, ...).  The
    ...
    runs exactly once per EV0_HOME per process.
    """
    ...
    # cli.py, threev0_cli/main.py, run_agent.py, trajectory_compressor.py,
