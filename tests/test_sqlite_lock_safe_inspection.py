"""POSIX advisory locks must survive 3V0' own database inspection.
...
3V0 used to byte-probe live databases in several places (kanban's
post-commit page-count check, the zeroed-state.db detector run on every
SessionDB construction, backup header verification). Under `3v0 sessions
...
out across 3V0' inspection calls.
"""
...
from threev0_cli.sqlite_safe_read import (
...
def test_probe_and_connect_do_not_race(tmp_path, clean_registry, monkeypatch):
    ...
    """
    import threev0_cli.sqlite_safe_read as ssr
