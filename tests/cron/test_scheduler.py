...
class TestPerJobToolsetMcpMerge:
    ...
    def test_resolver_empty_per_job_falls_through_to_platform(self):
        ...
        with patch("ev0_cli.tools_config._get_platform_tools",
...
class TestRunJobSessionPersistence:
    ...
    def test_run_job_sets_auto_delivery_env_from_dotenv_home_channel(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("EV0_CRON_AUTO_DELIVER_PLATFORM", raising=False)
        monkeypatch.delenv("EV0_CRON_AUTO_DELIVER_CHAT_ID", raising=False)
        monkeypatch.delenv("EV0_CRON_AUTO_DELIVER_THREAD_ID", raising=False)
        ...
        class FakeAgent:
            ...
            def run_conversation(self, *args, **kwargs):
                ...
                seen["platform"] = get_session_env("EV0_CRON_AUTO_DELIVER_PLATFORM") or None
                seen["chat_id"] = get_session_env("EV0_CRON_AUTO_DELIVER_CHAT_ID") or None
        ...
        assert os.getenv("EV0_CRON_AUTO_DELIVER_PLATFORM") is None
        assert os.getenv("EV0_CRON_AUTO_DELIVER_CHAT_ID") is None
        assert os.getenv("EV0_CRON_AUTO_DELIVER_THREAD_ID") is None
        fake_db.close.assert_called_once()
    ...
    def test_run_job_resets_secret_source_cache_before_reload(self, tmp_path, monkeypatch):
        ...
        this EV0_HOME in ``_APPLIED_HOMES``, so the external-secret pull
        ...
        ``load_ev0_dotenv`` (which then re-applies external secret sources).
        """
        ...
        with patch("cron.scheduler._ev0_home", tmp_path), \
             patch("cron.scheduler._resolve_origin", return_value=None), \
             patch("ev0_cli.env_loader.reset_secret_source_cache", _record_reset), \
             patch("ev0_cli.env_loader.load_ev0_dotenv", _record_load), \
             patch("ev0_state.SessionDB", return_value=fake_db), \
             patch(
                 "ev0_cli.runtime_provider.resolve_runtime_provider",
                 return_value={
    ...
    def test_run_job_clears_stale_auto_delivery_thread_id_between_jobs(self, tmp_path, monkeypatch):
        ...
        class FakeAgent:
            ...
            def run_conversation(self, *args, **kwargs):
                ...
                seen.append(
                    {
                        "platform": get_session_env("EV0_CRON_AUTO_DELIVER_PLATFORM") or None,
                        "chat_id": get_session_env("EV0_CRON_AUTO_DELIVER_CHAT_ID") or None,
                        "thread_id": get_session_env("EV0_CRON_AUTO_DELIVER_THREAD_ID") or None,
                    }
                )
                return {"final_response": "ok"}
...
class TestRunJobConfigEnvVarExpansion:
    ...
    def test_model_env_ref_in_config_yaml_is_expanded(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text("model: ${_EV0_TEST_CRON_MODEL}\n")
        monkeypatch.setenv("_EV0_TEST_CRON_MODEL", "gpt-4o-mini-cron-test")
    ...
    def test_unexpanded_ref_passthrough_when_var_unset(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text("model: ${_EV0_TEST_CRON_UNSET_VAR}\n")
        monkeypatch.delenv("_EV0_TEST_CRON_UNSET_VAR", raising=False)
        ...
        assert kwargs["model"] == "${_EV0_TEST_CRON_UNSET_VAR}"
...
class TestRunJobModelResolution:
    ...
    ``model: null``. At fire time the scheduler must:
      1. fall back to ``EV0_MODEL`` env if set,
    ...
    def test_null_job_model_falls_back_to_env(self, tmp_path, monkeypatch):
        """``model: null`` on the job uses EV0_MODEL when set."""
        (tmp_path / "config.yaml").write_text("")
        monkeypatch.setenv("EV0_MODEL", "env-model")
    ...
    def test_config_model_alias_key_resolves(self, tmp_path, monkeypatch):
        ...
        ``ev0_cli/oneshot.py``, ``fallback_cmd.py`` and ``prompt_size.py``
...
class TestParallelTick:
    ...
    def test_max_parallel_env_var(self, monkeypatch):
        """EV0_CRON_MAX_PARALLEL=1 should restore serial behaviour."""
        monkeypatch.setenv("EV0_CRON_MAX_PARALLEL", "1")
        call_times = []
...
class TestHomeTargetEnvVarRegistry:
    ...
    entry means ``3v0 cron create --deliver=<platform>`` silently
