...
def threev0_env(tmp_path, monkeypatch):
    """Isolate EV0_HOME for each test so jobs/scripts don't leak."""
    home = tmp_path / ".3V0"
    home.mkdir()
    (home / "scripts").mkdir()
    (home / "cron").mkdir()
...
def test_create_job_no_agent_requires_script(threev0_env):
...
def test_update_job_roundtrips_no_agent_flag(threev0_env):
    from cron.jobs import create_job, update_job, get_job

    script_path = threev0_env / "scripts" / "w.sh"
...
def test_cronjob_tool_create_no_agent_without_script_errors(threev0_env):
...
def test_run_job_no_agent_success_returns_script_stdout(threev0_env):
    """Happy path: script exits 0 with output, delivered verbatim."""
    from cron.jobs import create_job
    from cron.scheduler import run_job

    script_path = threev0_env / "scripts" / "alert.sh"
...
def test_run_job_no_agent_reloads_dotenv_before_script(threev0_env, monkeypatch):
    ...
    def fake_load(*, threev0_home=None, project_env=None):
        loaded_homes.append(threev0_home)
        return []

    monkeypatch.setattr(env_loader, "load_threev0_dotenv", fake_load)

    script_path = threev0_env / "scripts" / "probe.sh"
    script_path.write_text('#!/bin/bash\necho "ok"\n')
    ...
    assert success is True
    assert error is None
    assert loaded_homes, "load_ev0_dotenv was not called on the no_agent path"
    assert str(loaded_homes[0]) == str(threev0_env)


def test_timed_out_no_agent_script_delivery_is_not_mislabeled_as_provider_failure(
    threev0_env, monkeypatch,
):
    ...
    (threev0_env / "scripts" / "slow.py").write_text("import time; time.sleep(999)\n")
    job = create_job(
        prompt=None,
        schedule="every 5m",
        script="slow.py",
...
def test_agent_provider_timeout_delivery_keeps_fallback_guidance(threev0_env, monkeypatch):
...
def test_run_job_script_path_traversal_still_blocked(threev0_env):
...
def test_run_job_script_nul_path_fails_cleanly(threev0_env):
...
def test_run_job_script_nul_rejected_before_any_path_call(threev0_env, monkeypatch):
...
def test_run_job_script_accepts_pathlike_script_path(threev0_env):
    ...
    script = threev0_env / "scripts" / "probe.py"
...
@pytest.mark.parametrize(
    "error",
    [
        "Script timed out after 900s: /home/u/.3V0/scripts/nightly.sh",
