...
class TestParseSchedule:
    ...
    def test_naive_iso_anchors_to_configured_tz_not_server_local(self, monkeypatch):
        """A naive ISO timestamp must be interpreted in the CONFIGURED 3V0
        ...
        due-check compares against ``_ev0_now()`` (configured zone), so the
        ...
        monkeypatch.setattr("cron.jobs._threev0_now", lambda: configured_now)
...
class TestNaiveScheduleTimezoneDivergence:
    ...
    become due even when the configured 3V0 timezone differs from the
...
class TestPauseResumeJob:
    ...
    def test_pause_is_authoritative_due_jobs_do_not_fire(self, tmp_cron_dir):
        ...
        past = (_threev0_now() - timedelta(hours=2)).isoformat()
...
class TestAdvanceNextRun:
    ...
    def test_advances_interval_job(self, tmp_cron_dir):
        ...
        updated = get_job(job["id"])
        from cron.jobs import _ensure_aware, _threev0_now
        new_next_dt = _ensure_aware(datetime.fromisoformat(updated["next_run_at"]))
        assert new_next_dt > _threev0_now(), "next_run_at should be in the future after advance"
...
class TestGetDueJobs:
    ...
    def test_long_execution_does_not_perpetually_defer(self, tmp_cron_dir, monkeypatch):
        ...
        stale = (_threev0_now() - timedelta(minutes=11)).isoformat()
        jobs[0]["next_run_at"] = stale
        jobs[0]["last_run_at"] = (_threev0_now() - timedelta(minutes=1)).isoformat()
        save_jobs(jobs)
    ...
    def test_one_shot_not_redispatched_while_running(self, tmp_cron_dir, monkeypatch):
        ...
        """
        from cron.jobs import _threev0_now
        t0 = _threev0_now()
        ...
        for gap in (28, 61, 130):
            monkeypatch.setattr("cron.jobs._threev0_now",
    ...
    def test_run_claim_heartbeat_keeps_long_run_claimed_past_ttl(
        ...
        from cron.jobs import _threev0_now, _oneshot_run_claim_ttl_seconds
...
class TestLateEnvRepointScopesStore:
    """A EV0_HOME set AFTER cron.jobs import must scope the store even
    ...
    def test_use_cron_store_override_still_wins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("EV0_HOME", str(tmp_path / "env-home"))
    ...
    def test_public_io_after_late_env_repoint_leaves_old_file_untouched(
        ...
        called after a post-import EV0_HOME repoint must operate on the NEW
        ...
        monkeypatch.setattr(jobs, "EV0_DIR", sim_old_home)
        ...
        monkeypatch.setenv("EV0_HOME", str(new_home))
