...
These exercise the real store against a temp EV0_HOME (no mocks) per the
...
def temp_home(tmp_path, monkeypatch):
    """Isolated EV0_HOME so jobs.json doesn't touch the real store."""
    monkeypatch.setenv("EV0_HOME", str(tmp_path))
    # cron.jobs caches no home at import; get_ev0_home() reads the env live.
    yield tmp_path
