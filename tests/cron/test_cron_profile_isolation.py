...
profile's EV0_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.3V0/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active EV0_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever EV0_HOME the ticker happens to have —
...
def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    ...
    assert threev0_constants.get_threev0_home().resolve() == profile_home.resolve()
    assert threev0_constants.get_default_threev0_root().resolve() == root.resolve()
    ...
    try:
        assert jobs.EV0_DIR.resolve() == profile_home.resolve()
        assert (
