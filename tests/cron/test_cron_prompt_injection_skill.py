...
def cron_env(tmp_path, monkeypatch):
    """Isolated EV0_HOME with an empty skills tree.

    `tools.skills_tool` snapshots `SKILLS_DIR` at module-import time, so
    setting `EV0_HOME` alone doesn't reach it. We also patch the
...
class TestBuildJobPromptScansSkillContent:
    ...
    def test_skill_with_env_exfil_command_in_prose_is_allowed(self, cron_env):
        ...
        ``cat ~/.3V0/.env``") must NOT be blocked. This was a real
        false positive in the bundled `3v0-agent-dev` skill that
...
class TestScriptOutputNotStrictScanned:
    ...
    Live incident: the ``3v0-triage`` cron was blocked every 5 minutes
