"""Tests for ev0_logging — centralized logging setup."""
import io
import logging
import os
import stat
...
import ev0_logging
# Use whatever RotatingFileHandler class ev0_logging actually resolved so
# the autouse fixture's isinstance checks (which strip rotating handlers
# between tests) match the real handlers on every platform. ev0_logging
...
from ev0_logging import RotatingFileHandler
...
def _reset_logging_state():
    ...
    """
    ev0_logging._logging_initialized = False
    ...
    ev0_logging._reset_queued_handlers()
    ...
    ev0_logging._install_session_record_factory()
    yield
    ...
    ev0_logging._logging_initialized = False
    ev0_logging.clear_session_context()
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, ev0_home):
        log_dir = ev0_logging.setup_logging(ev0_home=ev0_home)
        assert log_dir == ev0_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, ev0_home):
        ev0_logging.setup_logging(ev0_home=ev0_home)
        root = logging.getLogger()

        agent_handlers = [
            h for h in ev0_logging.rotating_file_handlers()
    ...
    def test_idempotent_no_duplicate_handlers(self, ev0_home):
        ev0_logging.setup_logging(ev0_home=ev0_home)
        ev0_logging.setup_logging(ev0_home=ev0_home)  # second call — should be no-op
    ...
    def test_writes_to_agent_log(self, ev0_home):
        ...
        test_logger = logging.getLogger("test_ev0_logging.write_test")
        test_logger.info("test message for agent.log")

        # Flush handlers
        ev0_logging.flush_log_queue()
    ...
    def test_explicit_params_override_config(self, ev0_home):
        ...
        ev0_logging.setup_logging(ev0_home=ev0_home, log_level="WARNING")
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, ev0_home):
        ev0_logging.setup_logging(ev0_home=ev0_home, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, ev0_home):
        ev0_logging.setup_logging(ev0_home=ev0_home, mode="cli")
        root = logging.getLogger()
...
class TestGuiMode:
    ...
    def test_gui_log_created(self, ev0_home):
        ev0_logging.setup_logging(ev0_home=ev0_home, mode="gui")
        root = logging.getLogger()
    ...
    def test_gui_log_receives_only_gui_components(self, ev0_home):
        ...
        logging.getLogger("ev0_cli.web_server").info("dashboard online")
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, ev0_home):
        ...
        ev0_logging.setup_logging(ev0_home=ev0_home)
        ev0_logging.set_session_context("abc123")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = ev0_logging._ComponentFilter(("gateway",))
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, ev0_home):
        ev0_logging.setup_logging(ev0_home=ev0_home)
        ev0_logging.setup_verbose_logging()
...
class TestAddRotatingHandler:
    ...
    def test_no_duplicate_for_same_path(self, tmp_path):
        ...
        ev0_logging._add_rotating_handler(
            logger, log_path,
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        handlers = [h for h in ev0_logging.rotating_file_handlers() if isinstance(h, RotatingFileHandler)]
        ...
        ev0_logging.set_session_context("factory_test")
...
class TestWindowsConcurrentLogLockTimeout:
    ...
    def _make_logger_and_handler(self, log_path: Path):
        ...
        handler = ev0_logging._ManagedRotatingFileHandler(
    ...
    def test_helper_only_matches_windows_concurrent_lock_timeout(self):
        ...
        assert ev0_logging._is_windows_concurrent_log_lock_timeout(
            RuntimeError("Cannot acquire lock after 20 attempts")
        )
        assert not ev0_logging._is_windows_concurrent_log_lock_timeout(
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, ev0_home):
        level, max_size, backup = ev0_logging._read_logging_config()
...
class TestExternalRotationRecovery:
    ...
    def _make_handler(self, log_path: Path) -> ev0_logging._ManagedRotatingFileHandler:
        handler = ev0_logging._ManagedRotatingFileHandler(
    ...
    def _emit(self, handler: logging.Handler, msg: str) -> None:
        ...
        # Match the record factory that ev0_logging installs at import time.
        record.session_tag = ""
        handler.emit(record)
        ev0_logging.flush_log_queue()
...
class TestSafeStderr:
    ...
    def test_wraps_non_utf8_stderr(self, monkeypatch):
        ...
        result = ev0_logging._safe_stderr()
...
class TestAsyncQueueLogging:
    ...
    def test_file_handlers_not_on_root(self, ev0_home):
        ...
        assert any(
            ...
            for h in ev0_logging.rotating_file_handlers()
        )
