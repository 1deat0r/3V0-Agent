...
def _isolate(tmp_path, monkeypatch):
    """Isolate EV0_HOME so tests don't touch real config."""
    home = tmp_path / ".3V0"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal Ev0CLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = Ev0CLI.__new__(Ev0CLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["3v0-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below 3V0' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly 3V0' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above 3V0' minimum context length."""
