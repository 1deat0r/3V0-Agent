...
def _make_cli(**kwargs):
    """Create a Ev0CLI with prompt_toolkit stubbed out."""
    _clean_config = {
        "model": {
...
class TestStashStateInit:
    ...
    def test_stash_starts_empty(self, cli):
        ...
        # module, which re-imports threev0_cli.prompt_toolkit stubs and can
...
class TestKeybindingRegistration:
    ...
    ``Ev0CLI.run()``'s local ``KeyBindings`` instance, so they are not
