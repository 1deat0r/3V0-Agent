...
Pre-fix bug (issue #33925): ``cli.Ev0CLI._toggle_yolo`` mutated only
``os.environ["EV0_YOLO_MODE"]``. That env var is captured once at
...
prompt. Only ``3v0 --yolo`` (process-start env), ``EV0_YOLO_MODE=1``,
and ``3v0 config set approvals.mode off`` actually bypassed.
...
read (``session_id``). This avoids the heavy ``Ev0CLI`` construction
path used in ``test_cli_init.py``, which is incompatible with this test
file's path layout — ``Ev0CLI.__init__`` imports a lot of optional
...
def _clear_approval_state(monkeypatch):
    ...
    # 3V0-driven test runs may inherit EV0_YOLO_MODE=1 from the parent
...
def _make_stand_in(session_id: str = SESSION_KEY) -> SimpleNamespace:
    ...
    to invoking them on a fully-constructed ``Ev0CLI`` for the
    behaviour under test, and avoids the brittle prompt_toolkit / config
    stubbing required to instantiate ``Ev0CLI`` from this test file.
    """
...
class TestToggleYoloIsSessionScoped:
    ...
    def test_toggle_yolo_disables_session_bypass_on_second_call(self):
        stand_in = _make_stand_in()
        with patch("cli._cprint"):
            Ev0CLI._toggle_yolo(stand_in)  # ON
            assert approval_module.is_session_yolo_enabled(SESSION_KEY) is True
            Ev0CLI._toggle_yolo(stand_in)  # OFF
            assert approval_module.is_session_yolo_enabled(SESSION_KEY) is False
...
class TestIsSessionYoloActiveHelper:
    ...
    def test_helper_reflects_toggle(self):
        ...
        assert Ev0CLI._is_session_yolo_active(stand_in) is False

        with patch("cli._cprint"):
            Ev0CLI._toggle_yolo(stand_in)

        assert Ev0CLI._is_session_yolo_active(stand_in) is True
    ...
    def test_helper_honors_frozen_yolo_mode(self):
        """``3v0 --yolo`` sets ``EV0_YOLO_MODE`` before tool imports, so
...
class TestToggleYoloEndToEnd:
    ...
    def test_toggle_yolo_bypasses_dangerous_command_check(self):
        ...
        try:
            with patch("cli._cprint"):
                Ev0CLI._toggle_yolo(stand_in)  # YOLO ON
...
class TestSessionRotationTransfersYolo:
    ...
    def test_transfer_moves_yolo_to_new_session(self):
        ...
        try:
            ...
            Ev0CLI._transfer_session_yolo(stand_in, "old-id", "new-id")
    ...
    def test_transfer_handles_empty_inputs_safely(self):
        ...
        # to transfer from "" / to "".
        Ev0CLI._transfer_session_yolo(stand_in, "", "new")
        Ev0CLI._transfer_session_yolo(stand_in, "old", "")
