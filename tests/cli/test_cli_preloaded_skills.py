...
def _real_finalize(cli_obj):
    """Call the real Ev0CLI.finalize_preloaded_skills on a dummy object."""
    return _REAL_FINALIZE(cli_obj)


def _capture_real_finalize():
    import cli as cli_mod
    return cli_mod.Ev0CLI.__dict__["finalize_preloaded_skills"]
...
def test_main_applies_preloaded_skills_to_system_prompt(monkeypatch):
    ...
    monkeypatch.setattr(cli_mod, "Ev0CLI", fake_cli)
    monkeypatch.setattr(
        cli_mod,
        "build_preloaded_skills_prompt",
        lambda skills, task_id=None: ("skill prompt", ["3v0-agent-dev", "github-auth"], []),
    )

    with pytest.raises(SystemExit):
        cli_mod.main(skills="3v0-agent-dev,github-auth", list_tools=True)
    ...
    assert cli_obj.preloaded_skills == ["3v0-agent-dev", "github-auth"]
...
def test_show_banner_does_not_print_skills():
    ...
    cli_obj.preloaded_skills = ["3v0-agent-dev", "github-auth"]
