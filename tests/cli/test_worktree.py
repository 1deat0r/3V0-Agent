...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("3v0/3v0-")
...
class TestStaleWorktreePruning:
    ...
    def test_prunes_old_clean_worktree(self, git_repo):
        ...
        for entry in worktrees_dir.iterdir():
            if not entry.is_dir() or not entry.name.startswith("3v0-"):
                continue
            try:
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned 3v0/* and pr-* branches."""

    def test_prunes_orphaned_threev0_branch(self, git_repo):
        """3v0/3v0-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "3v0/3v0-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "3v0/3v0-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "3v0/3v0-deadbeef" in result.stdout
        ...
        assert "3v0/3v0-deadbeef" in orphaned
        ...
        assert "3v0/3v0-deadbeef" not in result.stdout
...
class TestWorktreeLockReaping:
    ...
    def _mk(cli, repo, name, pid=None, dirty=False, unpushed=False, age_h=100):
        ...
        subprocess.run(
            ["git", "worktree", "add", str(p), "-b", f"3v0/{name}", "HEAD"],
            cwd=repo, capture_output=True,
        )
        if pid is not None:
            subprocess.run(
                ["git", "worktree", "lock", "--reason", f"3v0 pid={pid}", str(p)],
                cwd=repo, capture_output=True,
            )
        if unpushed:
    ...
    def test_live_locked_survives_at_any_age(self, git_repo):
        import cli
        wt = self._mk(cli, git_repo, "3v0-live", pid=os.getpid())
    ...
    def test_dead_locked_clean_is_reaped(self, git_repo):
        import cli
        wt = self._mk(cli, git_repo, "3v0-dead", pid=999999)
    ...
    def test_dead_locked_dirty_survives(self, git_repo):
        import cli
        wt = self._mk(cli, git_repo, "3v0-deaddirty", pid=999999, dirty=True)
        cli._prune_stale_worktrees(str(git_repo))
    ...
    def test_dead_locked_unpushed_survives(self, git_repo):
        import cli
        wt = self._mk(cli, git_repo, "3v0-deadunp", pid=999999, unpushed=True)
        cli._prune_stale_worktrees(str(git_repo))
...
class TestWorktreeLockPredicate:
    ...
    def test_unlocked_returns_none(self, git_repo):
        import cli
        p = git_repo / ".worktrees" / "3v0-x"
        (git_repo / ".worktrees").mkdir(exist_ok=True)
        subprocess.run(
            ["git", "worktree", "add", str(p), "-b", "3v0/3v0-x", "HEAD"],
    ...
    def test_foreign_lock_reason_returns_dead(self, git_repo):
        import cli
        p = self._mk_locked(git_repo, "3v0-foreign", "some other tool")
...
class TestWidenedPruner:
    ...
    Previously only ``3v0-*`` directories were considered, so salvage/
    ...
    # -- named (non 3v0-*) directories are now covered ------------------
    ...
    def test_squash_merged_tree_is_reaped(self, git_repo):
        import cli
        wt, sha = self._mk(git_repo, "3v0-merged", commit=True, age_h=100)
...
class TestMergeVerdictCache:
    ...
    dominates ``3v0 -w`` startup (~0.2-1.0s per worktree, re-run on every
    ...
    def test_cache_hit_matches_uncached_verdict(self, git_repo):
        ...
        import cli
        wt, sha = self._mk(git_repo, "3v0-cachehit", commit=True)
        self._merge_upstream(git_repo, sha)
    ...
    def test_new_commit_invalidates_cached_verdict(self, git_repo):
        ...
        """
        import cli
        wt, sha = self._mk(git_repo, "3v0-moves", commit=True)
        self._merge_upstream(git_repo, sha)
...
class TestPruneParallelEquivalence:
    ...
    def _board(self, git_repo, tag=""):
        ...
        names = {}
        for i in range(3):
            n = f"3v0-merged{tag}{i}"
            wt, sha = self._mk(git_repo, n, commit=True)
            self._merge_upstream(git_repo, sha)
            names[n] = wt
        for i in range(3):
            n = f"3v0-unpushed{tag}{i}"
    ...
    def test_pool_failure_falls_back_to_serial(self, git_repo, monkeypatch):
        ...
        wt, sha = self._mk(git_repo, "3v0-poolfail", commit=True)
        self._merge_upstream(git_repo, sha)
...
class TestShallowCloneDeepening:
    ...
    (real incident: 21 of 25 3v0-* trees stuck on a default install).
    ...
    def _stuck_worktree(self, tmp_path):
        ...
        wt = clone / ".worktrees" / "3v0-shallowstuck"
        (clone / ".worktrees").mkdir()
        self._run(
            ["git", "worktree", "add", str(wt), "-b", "3v0/3v0-shallowstuck", "HEAD"],
            clone,
        )
