...
minimal ``Ev0CLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def ev0_home(tmp_path, monkeypatch):
    ...
    # Bust the goal module's DB cache so it re-resolves EV0_HOME each test.
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal Ev0CLI stub with an active goal wired in."""
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, ev0_home):
        ...
        src = inspect.getsource(Ev0CLI.chat)
