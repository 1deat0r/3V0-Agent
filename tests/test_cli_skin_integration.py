...
from cli import Ev0CLI, _build_compact_banner, _rich_text_from_ansi
...
class TestCliSkinPromptIntegration:
    ...
    def test_ares_prompt_fragments_use_skin_symbol(self):
        ...
        assert cli._get_tui_prompt_fragments() == [("class:prompt", "❯ ")]  # Tide/rebrand prompt
    ...
    def test_secret_prompt_fragments_preserve_secret_state(self):
        ...
        set_active_skin("ares")
        assert cli._get_tui_prompt_fragments() == [("class:sudo-prompt", "🔑 ❯ ")]  # Tide/rebrand sudo prompt
...
class TestCompactBannerSkinIntegration:
    ...
    def test_poseidon_compact_banner_uses_skin_branding_instead_of_nous_ev0(self):
        set_active_skin("poseidon")

        with patch("cli.shutil.get_terminal_size", return_value=SimpleNamespace(columns=90)), \
             patch.dict(_build_compact_banner.__globals__, {"format_banner_version_label": lambda: "3V0 Agent v0.1.0 (test)"}):
            banner = _build_compact_banner()

        assert "Poseidon Agent" in banner
        assert "NOUS EV0" not in banner
