"""Windows installer regression for 3V0 children outside the venv.

The venv sweep deliberately selects process roots by executable path so it
does not kill unrelated Python processes. A selected 3V0 process can spawn
...
def test_venv_sweep_stops_managed_runtime_children_but_not_unrelated_processes(
    ...
    install_dir = threev0_home / "3v0-agent"
    venv_scripts = install_dir / "venv" / "Scripts"
    runtime_dir = threev0_home / ".3v0-runtime" / "python" / "generation-test"
    ...
    # Keep the test away from real gateway tasks and real 3V0 launchers
