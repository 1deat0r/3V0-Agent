...
1. MULTIPLEX INVERSION (@kshitijk4poor): the messaging gateway hands every
   ...
   via the EV0_HOME ContextVar (copy_context into the worker). A bound
...
def test_full_prompt_scoped_to_bot_on_bare_thread(tmp_path, monkeypatch):
    ...
    assert "Active 3V0 profile: mybot" in prompt
    assert "Active 3V0 profile: default" not in prompt
