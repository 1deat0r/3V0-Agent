...
class TestBuildLearnPrompt:
    ...
    def test_teaches_the_full_hardline_standards(self):
        ...
        # author is always the literal 3V0, never the host/OS identity (#52368).
        assert "author: always the literal value `3v0`" in std
        assert "never fill it from the host" in std
        # #2 3V0-tool framing names the wrapped tools, not shell utilities.
