"""Tests for the core Relay-managed 3V0 tool adapter."""
...
def test_request_rewrite_reaches_authorized_callback_once(relay_turn):
    ...
    relay.intercepts.register_tool_request(
        "3v0-test-tool-request", 1, False, rewrite_request
    )
    relay.intercepts.register_tool_execution(
        "3v0-test-tool-execution", 1, wrap_execution
    )
    try:
        ...
        relay.intercepts.deregister_tool_execution("3v0-test-tool-execution")
        relay.intercepts.deregister_tool_request("3v0-test-tool-request")
