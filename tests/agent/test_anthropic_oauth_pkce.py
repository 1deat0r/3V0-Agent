...
History:
  - PR #1775 first fixed this on ``run_ev0_oauth_login()``.
  - PR #2647 (b17e5c10) added ``run_ev0_oauth_login_pure()`` and silently
...
def _patch_oauth_flow(
    ...
    *,
    ...
    """Wire up monkeypatches that let ``run_ev0_oauth_login_pure()`` run
    ...
    monkeypatch.setattr(
        "threev0_cli.auth._can_open_graphical_browser", lambda: True
    )
