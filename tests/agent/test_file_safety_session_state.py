...
manipulation. 3V0 keeps canonical conversation history in state.db and may
...
def fake_homes(tmp_path, monkeypatch):
    """Point EV0_HOME at a temp profile dir.

    Uses the real env-var resolution chain (get_ev0_home /
    get_default_ev0_root) instead of monkeypatching private helpers —
    ...
    EV0_HOME=<root>/profiles/<name> makes get_default_ev0_root()
