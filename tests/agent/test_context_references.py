...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "3V0 Tests")
...
def test_binary_reference_block_maps_host_attachment_to_container_path(tmp_path: Path, monkeypatch):
    ...
    assert result.expanded
    # Default container base for the docker backend is /root/.3v0.
    assert "/root/.3V0/attachments/archive.zip" in result.message
...
def test_binary_reference_block_keeps_host_path_on_local_backend(tmp_path: Path, monkeypatch):
    ...
    assert "/root/.3V0/attachments/" not in result.message
...
async def test_blocks_canonical_read_denylist_credential_stores(tmp_path: Path, monkeypatch):
    ...
    a chat peer could otherwise attach `@file:~/.3V0/auth.json` and read the
    ...
    result = await preprocess_context_references_async(
        "inspect @file:.3v0/auth.json and @file:.3v0/.anthropic_oauth.json "
        "and @file:.3v0/mcp-tokens/github.json and @file:project/.env",
        cwd=tmp_path,
...
@pytest.mark.parametrize(
    ...
    [
        "/tmp/plain.png",
        "/Users/me/Library/Application Support/3V0/composer-images/a.png",
