...
for the credential stores while leaving arbitrary ``EV0_HOME`` files
...
def fake_home(tmp_path, monkeypatch):
    """Point ``_ev0_home_path()`` at a tmp dir for isolated checks."""
...
def test_identically_named_ev0_files_outside_home_not_blocked(
    ...
    outside EV0_HOME must remain readable — the gate is per-location for
    ...
    project.mkdir()
    # auth.json outside EV0_HOME — readable (per-location gate).
    p = project / "auth.json"
    p.write_text("not secret here", encoding="utf-8")
    assert get_read_block_error(str(p)) is None, (
        "auth.json outside EV0_HOME should NOT be blocked"
    )
...
def test_profile_mode_blocks_root_credentials(tmp_path, monkeypatch):
    """Under a profile, EV0_HOME = <root>/profiles/<name>, but
