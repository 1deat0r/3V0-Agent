...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. 3V0 passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("EV0_HOME", "/users/alice/.3V0/profiles/backend-worker")
        monkeypatch.setenv("EV0_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "EV0_KANBAN_DB",
            "/users/alice/.3V0/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.3V0/kanban/boards/smoke"]'
            in cmd
        )
...
class TestSpawnEnvSecretStripping:
    """codex app-server routes its spawn env through ev0_subprocess_env(
    ...
    credentials to authenticate, but it must NOT inherit Tier-1 3V0 secrets
    ...
    def test_tier1_and_internal_secrets_stripped_from_spawn_env(self, monkeypatch):
        for var, val in {
            ...
            "EV0_DASHBOARD_SESSION_TOKEN": "dash-secret",
        ...
        for var in (
            ...
            "EV0_DASHBOARD_SESSION_TOKEN", "AUXILIARY_VISION_API_KEY",
