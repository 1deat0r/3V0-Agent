...
def clear_verify_env(monkeypatch):
    ...
    set EV0_SESSION_SOURCE and the gateway sets EV0_SESSION_PLATFORM.
    """
    for var in (
        "EV0_VERIFY_ON_STOP",
        "EV0_PLATFORM",
...
def test_verify_on_stop_env_can_enable(clear_verify_env):
    ...
    clear_verify_env.setenv("EV0_VERIFY_ON_STOP", "1")
    clear_verify_env.setenv("EV0_SESSION_PLATFORM", "telegram")
...
def test_verify_on_stop_auto_on_for_interactive_surfaces(clear_verify_env, source):
    ...
    clear_verify_env.setenv("EV0_SESSION_SOURCE", source)
...
def test_verify_on_stop_default_path_through_load_config(tmp_path, clear_verify_env):
    ...
    clear_verify_env.setenv("EV0_HOME", str(tmp_path / ".3V0"))
    ...
    clear_verify_env.setenv("EV0_SESSION_SOURCE", "cli")
...
def test_ad_hoc_pass_satisfies_no_suite_stop_loop(tmp_path, monkeypatch):
    ...
    script = Path(tempfile.gettempdir()) / f"3v0-ad-hoc-stop-{tmp_path.name}.py"
