...
def test_pyproject_pins_match_lazy_deps_pins():
    ...
    places. When they drift, `3v0 update` resolves the pyproject extra
    ...
    assert not drift, (
        ...
        "for every shared package — otherwise `3v0 update` downgrades the "
...
def test_dingtalk_extra_includes_qrcode_for_qr_auth():
    """DingTalk's QR-code device-flow auth (threev0_cli/dingtalk_auth.py)
...
def test_every_lazy_deps_exact_pin_matches_uv_lock():
    ...
    install ships the locked version, and the ``3v0 update`` lazy-refresh
    ...
    assert not drift, (
        ...
        "every package the core lock also ships — otherwise `3v0 update` "
...
def test_huggingface_hub_lazy_pin_matches_uv_lock():
    ...
    a feature from mere package presence, the `3v0 update` lazy-refresh
    ...
    assert lazy_pin == locked, (
        ...
        "huggingface-hub`), or `3v0 update` will churn/downgrade the "
