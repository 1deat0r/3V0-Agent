...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"3v0-agent[termux]"' in text
    assert '"3v0-agent[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"3v0-agent[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
