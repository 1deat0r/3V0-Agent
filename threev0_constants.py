...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    across four ``threev0_cli`` modules using three different Windows
    ...
    A few sites outside ``threev0_cli`` (``tools/code_execution_tool.py``,
    ...
    *windows* lets a caller pass its own platform verdict. Several callers
    ...
    Windows paths on Linux CI (``threev0_cli.main._is_windows`` and friends);
...
# `threev0_cli.update_cmd`'s post-update probe consumes this same set so the
...
def is_first_party_module(name: str | None) -> bool:
    ...
    if not root:
        return False
    return root in FIRST_PARTY_MODULE_ROOTS or root.startswith(("ev0_", "threev0_"))
