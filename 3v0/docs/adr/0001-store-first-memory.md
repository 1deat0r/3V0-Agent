...
the append-only, provenance-bearing source of truth; the Hermes profile
(MEMORY.md / USER.md / SKILL.md) is re-exported from it as a projection.
This inverts Hermes's default (profile-as-origin) so nothing is ever silently
...
- *Profile-as-origin (Hermes default):* simpler, but edits are destructive — a
