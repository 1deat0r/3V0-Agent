...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with 3v0-agent)
2. User plugins: ``$EV0_HOME/plugins/model-providers/<name>/``
3. Pip-installed plugins: distributions exposing a ``ev0_agent.plugins``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$EV0_HOME/plugins/model-providers/`` can override
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple EV0_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_ev0_user_provider_{safe_name}"
...
def _discover_entry_point_providers() -> None:
    """Import pip-installed provider plugins via the ``ev0_agent.plugins``
    entry-point group so they self-register.

    A distribution ships::

        [project.entry-points."ev0_agent.plugins"]
        acme-inference = "acme_ev0_plugin:register"
    ...
    * **Provider targets only.** The ``ev0_agent.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$EV0_HOME``) keep their
    ...
    try:
        from threev0_cli.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$EV0_HOME/plugins/model-providers/<name>/``
    ...
    # 0. Pip-installed plugins — entry points in the ``ev0_agent.plugins``
    ...
    #    filesystem steps means a bundled or ``$EV0_HOME`` profile of the
    ...
    # 1. Bundled plugins — shipped with 3v0-agent.
    ...
    # 2. User plugins — under $EV0_HOME/plugins/model-providers/<name>/.
