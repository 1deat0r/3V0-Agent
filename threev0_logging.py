"""Centralized logging setup for 3V0 Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.3V0/logs/`` (profile-aware via ``get_ev0_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``threev0_cli.web_server``,
    ``threev0_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in 3V0 (TUI, gateway, ``hy_memory`` server, MCP
...
# Used by _ComponentFilter and exposed for ``3v0 logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``3v0 logs --component gateway``.
    "gateway": ("gateway", "ev0_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("threev0_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "threev0_cli.web_server",
        "threev0_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ...
    """Configure the 3V0 logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    ev0_home
        Override for the 3V0 home directory.  Falls back to
        ``get_ev0_home()`` (profile-aware).
    log_level
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the 3v0 group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from threev0_cli.config import is_managed
...
# the module header): when several 3V0 processes log to the same file, an
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="3v0-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _read_logging_config():
    ...
    try:
        ...
        # reuses the parse threev0_cli.main's early bridge already did (one
        # config.yaml parse per process instead of 3-4). Fall back to a
        # direct parse when threev0_cli.config isn't importable (bare
        # threev0_logging consumers).
        try:
            from threev0_cli.config import read_raw_config as _rrc
            cfg = _rrc() or {}
        except Exception:
