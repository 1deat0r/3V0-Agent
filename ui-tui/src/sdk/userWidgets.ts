...
/**
 * User widget apps — 3V0 authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$EV0_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.3V0/plugins/`: files under EV0_HOME execute
...
const widgetsDir = () => join(process.env.EV0_HOME?.trim() || join(homedir(), '.3V0'), 'tui-widgets')
 ...
 *  every change, so a widget 3V0 writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or 3V0) ever writes must hot-load too — a 10s poll here read
