import type { SkinBranding, SkinColors } from '@3v0/shared/skin'
...
// Mirrors @3v0/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @3v0/ink is bundled separately from packages/.
 ...
 *  answer when known (cached in EV0_TUI_BACKGROUND), else the mode's
 *  assumed pole. */
function referenceBackground(isLight: boolean, env: NodeJS.ProcessEnv = process.env): string {
  const cached = (env.EV0_TUI_BACKGROUND ?? '').trim()
...
// Terminal. Explicit EV0_TUI_THEME / COLORFGBG signals above still win,
...
// `EV0_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `EV0_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `EV0_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `EV0_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default 3V0 palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.EV0_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.EV0_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.EV0_TUI_BACKGROUND ?? '')
 ...
 * the OSC-11 background answer cached into EV0_TUI_BACKGROUND after
...
export function fromSkin(
  ...
  // itself into EV0_TUI_BACKGROUND. See #applySkin / syncThemeToTerminalBackground).
  ...
  //    base seeds for this polarity. The base's IDENTITY FILLS (3V0 navy
  ...
  // the derived value already carries the identity seeds (e.g. 3V0 navy).
  const activeRow =
