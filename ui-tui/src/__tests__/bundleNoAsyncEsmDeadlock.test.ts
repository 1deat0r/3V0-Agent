 ...
 *
 * Root cause: re-exporting `ink-text-input` from `@ev0/ink`'s
 * entry-exports drags the upstream `ink` package into the bundle. That
 * `ink` graph and our in-tree `@ev0/ink` graph reference each other
...
function bundleIsFresh(): boolean {
  ...
  try {
    ...
    const sourceMtime = statSync(resolve(uiTuiRoot, 'packages/ev0-ink/src/entry-exports.ts')).mtimeMs
...
describe('TUI bundle (issue #31227)', () => {
  ...
  it('does not bundle the upstream ink package or ink-text-input', () => {
    ...
    // that #31227 was about. The in-tree fork at @ev0/ink replaces
    // all of `ink`; nothing in ui-tui imports `TextInput` from
    // `@ev0/ink` so the re-export is unused dead weight.
  ...
  it('has the @ev0/ink entry-exports module compiled to sync init', () => {
    // Sanity check that the alias swap to packages/ev0-ink/src/entry-exports.ts
    // is still active and producing the expected synchronous init shape.
    expect(bundleSrc).toMatch(/var init_entry_exports = __esm\(\{\s*"packages\/ev0-ink\/src\/entry-exports\.ts"\(\)/)
  })
})
