 ...
 * `3v0-boot-background` / `3v0-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports EV0_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.3V0 matches get_ev0_home()'s default.
const bootFilePath = () => join(process.env.EV0_HOME ?? join(homedir(), '.3V0'), 'tui-theme-boot.json')

// Never touch the user's real ~/.3V0 from test runs (the TS suite has no
// EV0_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into EV0_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.EV0_TUI_THEME || env.EV0_TUI_LIGHT) {
    return result
  }
  ...
  if (
    ...
    !env.EV0_TUI_BACKGROUND
  ) {
    env.EV0_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.EV0_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.EV0_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into EV0_TUI_THEME.
