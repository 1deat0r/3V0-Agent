...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.EV0_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep 3V0 on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via EV0_TUI_TRUECOLOR=1.
