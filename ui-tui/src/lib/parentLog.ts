...
const logDir = join(process.env.EV0_HOME?.trim() || join(homedir(), '.3V0'), 'logs')
...
// a real ~/.3V0 (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('3v0-tui: parent lifecycle log unavailable\n')
    }
  }
}
