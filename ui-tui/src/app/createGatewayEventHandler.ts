...
import { forceRedraw, onTerminalBackground, onTerminalForeground } from '@3v0/ink'
...
const commitTheme = (theme: Theme) => {
  ...
  const pin = configPinnedTheme ? process.env.EV0_TUI_THEME : undefined

  writeBootTheme(theme, process.env.EV0_TUI_BACKGROUND, pin === 'light' || pin === 'dark' ? pin : undefined)
 ...
 * to EV0_TUI_THEME — the priority-2 signal `detectLightMode` already
 * honors (only an explicit EV0_TUI_LIGHT env var outranks it); 'auto'
 ...
 */
// True once CONFIG (via light/dark) owns the EV0_TUI_THEME env pin, so an
...
export function applyConfiguredTuiTheme(raw: unknown): void {
  ...
  const current = process.env.EV0_TUI_THEME ?? ''
  ...
  if (mode === 'light' || mode === 'dark') {
    ...
    process.env.EV0_TUI_THEME = mode
  } else {
    // 'auto' clears only a pin CONFIG set — never a EV0_TUI_THEME the user
    ...
    delete process.env.EV0_TUI_THEME
  }
 ...
 * is cached into EV0_TUI_BACKGROUND — the slot `detectLightMode` already
 ...
 * EV0_TUI_LIGHT / EV0_TUI_THEME overrides still win inside
...
export function syncThemeToTerminalBackground(): void {
  ...
  onTerminalBackground(hex => {
    ...
    resolved = true
    process.env.EV0_TUI_BACKGROUND = hex
    reapplyTheme()
  })
  ...
  onTerminalForeground(hex => {
    if (resolved || process.env.EV0_TUI_THEME || process.env.EV0_TUI_LIGHT) {
      return
    }
    ...
    resolved = true
    process.env.EV0_TUI_BACKGROUND = inferred
    reapplyTheme()
  })
  ...
  setTimeout(() => {
    if (
      ...
      process.env.EV0_TUI_BACKGROUND ||
      process.env.EV0_TUI_THEME ||
      process.env.EV0_TUI_LIGHT ||
    ...
    execFile('defaults', ['read', '-g', 'AppleInterfaceStyle'], (error, stdout) => {
      if (resolved || process.env.EV0_TUI_BACKGROUND || process.env.EV0_TUI_THEME) {
        return
      }
      ...
      // inference).
      resolved = true
      process.env.EV0_TUI_BACKGROUND = dark ? '#1e1e1e' : '#ffffff'
      reapplyTheme()
    })
  }, 1500).unref?.()
}
...
export function createGatewayEventHandler(ctx: GatewayEventHandlerContext): (ev: GatewayEvent) => void {
  ...
  const handleReady = (skin?: GatewaySkin) => {
    ...
    // Arm "Hey 3V0" if this surface owns it (server gates on config).
    ...
    // forging a brand-new one.  Mirrors classic CLI's `3v0 -c` /
    // `3v0 --tui` muscle memory and addresses the audit's "session
  ...
  return (ev: GatewayEvent) => {
    ...
    switch (ev.type) {
      ...
      case 'wake.detected': {
        // "Hey 3V0": optionally open a fresh session (start_new_session),
        ...
        void (async () => {
          ...
          if (wakeProfile && wakeProfile !== ownProfile) {
            sys(`wake phrase for profile '${wakeProfile}' — run: 3v0 -p ${wakeProfile} --tui`)
