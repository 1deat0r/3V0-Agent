...
import { terminalBackgroundHex } from '@ev0/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $EV0_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see EV0_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['EV0_TUI_BACKGROUND', process.env.EV0_TUI_BACKGROUND ?? '(unset)'],
            ['EV0_TUI_THEME', process.env.EV0_TUI_THEME ?? '(unset)'],
