...
const dumpNotice = (snap: MemorySnapshot, dump: HeapDumpResult | null) =>
  `3v0-tui: ${snap.level} memory (${formatBytes(snap.heapUsed)}) — auto heap dump → ${dump?.heapPath ?? dump?.diagPath ?? '(failed)'}\n`
...
setupGracefulExit({
  ...
  onError: (scope, err) => {
    ...
    try {
      process.stderr.write(`3v0-tui lifecycle ${scope}: ${message.slice(0, 2000)}\n`)
    } catch {
  ...
  onSignal: signal => {
    ...
    resetTerminalModes()
    process.stderr.write(`3v0-tui lifecycle: received ${signal}\n`)
  },
...
const stopMemoryMonitor = startMemoryMonitor({
  onCritical: (snap, dump) => {
    ...
    process.stderr.write(
      `3v0-tui lifecycle: memory critical exit heap=${formatBytes(snap.heapUsed)} rss=${formatBytes(snap.rss)}\n`
    )
    process.stderr.write(dumpNotice(snap, dump))
    process.stderr.write('3v0-tui: exiting to avoid OOM; restart to recover\n')
    process.exit(137)
  },
  ...
  onWarn: snap => {
    ...
    process.stderr.write(
      `3v0-tui: heap climbing fast (${formatBytes(snap.heapUsed)}) — a large tool output or long session may be straining memory\n`
    )
  }
})

if (process.env.EV0_HEAPDUMP_ON_START === '1') {
