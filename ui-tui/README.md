...
React + Ink terminal UI for 3V0. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `EV0_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `3v0 setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.3V0/.ev0_history` or under `EV0_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `3v0 setup` wizard, suspends Ink while it runs
...
ui-tui/
  ...
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external 3v0 setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.3V0/tui-parent.log
...
tui_gateway/
  ...
  slash_worker.py        persistent Ev0CLI subprocess for slash commands
```
