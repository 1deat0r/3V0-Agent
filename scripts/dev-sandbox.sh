...
unprivileged `3v0` user, giving the layout most people have —
$EV0_HOME/3v0-agent plus a ~/.local/bin launcher. Pass --root for the FHS
...
Environment:
  EV0_DEV_SANDBOX_DIR    Sandbox directory name, relative to the repo root
                            (default: .3v0-sandbox).

Examples:
  # create a sandbox, install this branch as `main`, and then drop to a shell,
  # skipping `3v0 setup` & the browser tools for speed.
...
GIT_ROOT="${EV0_SANDBOX_SOURCE_ROOT:-$(git rev-parse --show-toplevel)}"
...
SANDBOX_DIR_NAME="${EV0_DEV_SANDBOX_DIR:-.3v0-sandbox}"
...
if [ "$PERSISTENT" = true ]; then
  ...
  SANDBOX_ROOT="$(mktemp -d -t 3v0-sandbox.XXXXXX)"
...
if [ -n "$INSTALL_REF" ]; then
  ...
  UPSTREAM_REPO="$(mktemp -d -t 3v0-sandbox-upstream.XXXXXX)"
...
if [ "$INSTALL_SHORTCUT" = true ]; then
  mkdir -p "$SANDBOX_ROOT/root/http/github.com/1deat0r/3V0-Agent"
  if [ -n "$INSTALL_REF" ]; then
    git -C "$UPSTREAM_REPO" show "$UPSTREAM_COMMIT:scripts/install.sh" \
      > "$SANDBOX_ROOT/root/http/github.com/1deat0r/3V0-Agent/install.sh"
  else
    cp -a "$INSTALLER_PATH" "$SANDBOX_ROOT/root/http/github.com/1deat0r/3V0-Agent/install.sh"
  fi
  set -- bash -c '
    set +e
    curl -fsSL https://github.com/1deat0r/3V0-Agent/install.sh | bash -s -- "$@"
    install_status=$?
    if [ "$install_status" -eq 0 ] && [ -f /work/promote-main ]; then
      next_main=$(cat /work/promote-main)
      if git --git-dir=/work/repos/3v0-agent.git update-ref refs/heads/main "$next_main"; then
...
# A user-level install writes the `3v0` launcher to ~/.local/bin and the
# checkout to $EV0_HOME; both live under the sandbox HOME, which is bound
...
SOURCE_REF="$COMMIT"
SNAPSHOT_REPO=""
FAKE_REPO="$SANDBOX_ROOT/root/repos/3v0-agent.git"
git -C "$SANDBOX_ROOT/root/repos" init --bare -q 3v0-agent.git
if [ -n "$INSTALL_REF" ]; then
...
if [ -n "$(git -C "$GIT_ROOT" status --porcelain)" ]; then
  ...
  SNAPSHOT_REPO="$(mktemp -d -t 3v0-sandbox-snapshot.XXXXXX)"
  git -C "$SNAPSHOT_REPO" init -q
  git -C "$SNAPSHOT_REPO" fetch -q "$GIT_ROOT" "$COMMIT"
  git -C "$SNAPSHOT_REPO" config user.name '3V0 sandbox'
...
if [ -n "$INSTALL_REF" ]; then
  git --git-dir="$FAKE_REPO" fetch -q --force "$SOURCE_REPO" \
    "$SOURCE_REF:refs/3v0-sandbox/next"
