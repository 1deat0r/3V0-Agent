...
# Python-driven tool session (e.g. 3V0 terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/3v0-agent.git"
REPO_URL_HTTPS="https://github.com/NousResearch/3v0-agent.git"
EV0_HOME="${EV0_HOME:-$HOME/.3v0}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${EV0_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$EV0_INSTALL_DIR"
...
#   code at /usr/local/lib/3v0-agent, command at /usr/local/bin/3v0,
#   data still at /root/.3V0 (EV0_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$EV0_HOME/.no-bundled-skills so future"
            echo "                   '3v0 update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> 3V0.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.3V0/3v0-agent"
            echo "                   default (root, Linux): /usr/local/lib/3v0-agent"
            echo "  --3v0-home PATH  Data directory (default: ~/.3V0, or \$EV0_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, 3V0 installs the code under"
            echo "  /usr/local/lib/3v0-agent and links the command into"
            echo "  /usr/local/bin/3v0 (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$EV0_HOME"
            echo "  (default /root/.3V0).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$EV0_HOME/3v0-agent are preserved in-place."
...
# intentional, but they leave the checkout dirty — which forces `3v0 update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (3V0-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download 3V0 Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install 3v0 command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `3v0` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $EV0_HOME/3v0-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $EV0_HOME/3v0-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/3v0-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $EV0_HOME/3v0-agent — then preserve it)
#
# Always no-op when the user set --dir or $EV0_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in EV0_HOME.
    if is_termux; then
        INSTALL_DIR="$EV0_HOME/3v0-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$EV0_HOME/3v0-agent/.git" ]; then
            INSTALL_DIR="$EV0_HOME/3v0-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/3v0-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/3v0-agent"
        ...
        # /usr/local/bin/3v0 wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/3v0"
        log_info "  Data:    $EV0_HOME (unchanged)"
...
# Point a 3V0-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $EV0_HOME/node/bin — which is NOT on
...
# 3V0's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no 3V0-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$EV0_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$EV0_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$EV0_HOME/node/etc/npmrc"
}

get_ev0_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/3v0" ]; then
        echo "$link_dir/3v0"
    else
        echo "3v0"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://github.com/1deat0r/3V0-Agent/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    log_info "Installing managed uv into $EV0_HOME/bin ..."
    mkdir -p "$EV0_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/3v0-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/3v0-uv-installer.$$.sh")"
    ...
    # directly into $EV0_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$EV0_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# with the 3V0-managed Node $NODE_VERSION.
...
check_node() {
    ...
    # Repair pre-existing 3V0-managed installs where `npm install -g` lands
    ...
    if command -v node &> /dev/null && command -v npm &> /dev/null \
        ...
        log_warn "min-release-age-exclude) — installing 3V0-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi

    # Prefer a 3V0-managed Node from a previous run over a too-old system one.
    if [ -x "$EV0_HOME/node/bin/node" ] && [ -x "$EV0_HOME/node/bin/npm" ] \
        && node_satisfies_build "$("$EV0_HOME/node/bin/node" --version)"; then
        export PATH="$EV0_HOME/node/bin:$PATH"
        log_success "Node.js $("$EV0_HOME/node/bin/node" --version) found (3V0-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        log_warn "node found but npm is not on PATH (stray node symlink?) — installing 3V0-managed Node $NODE_VERSION LTS..."
    elif command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old (3V0 requires Node >=26) — installing 3V0-managed Node $NODE_VERSION..."
...
install_node() {
    ...
    log_info "Extracting to ~/.3V0/node/..."
    ...
    # Place into ~/.3V0/node/ and symlink binaries into the same bin dir
    # the 3v0 command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$EV0_HOME/node"
    mkdir -p "$EV0_HOME"
    mv "$extracted_dir" "$EV0_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$EV0_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$EV0_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$EV0_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$EV0_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.3V0/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. 3V0 install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "3V0 Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `3v0 update` path
                # (#4735).
                ...
                local stash_name
                stash_name="3v0-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``3v0 update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if 3V0 behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up 3v0 command..."

    if [ "$USE_VENV" = true ]; then
        EV0_BIN="$INSTALL_DIR/venv/bin/python"
        EV0_ENTRYPOINT="$INSTALL_DIR/3v0"
    else
        EV0_BIN="$(which 3v0 2>/dev/null || echo "")"
        if [ -z "$EV0_BIN" ]; then
            log_warn "3v0 not found on PATH after install"
            return 0
        fi
    fi

    # Verify the interpreter and the checked-in entrypoint needed by the launcher.
    if [ ! -x "$EV0_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$EV0_ENTRYPOINT" ]; }; then
        log_warn "3V0 launcher prerequisites not found"
    ...
    # Create a user-facing shim for the 3v0 command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $EV0_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$EV0_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/3v0"
    if [ "$USE_VENV" = true ]; then
        ...
        cat > "$command_link_dir/3v0" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$EV0_BIN" "$EV0_ENTRYPOINT" "\$@"
EOF
    else
        cat > "$command_link_dir/3v0" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/3v0"
    log_success "Installed 3v0 launcher → $command_link_display_dir/3v0"

    # Also expose `3v0-agent`. The `3v0-agent` console script declared in
    ...
    rm -f "$command_link_dir/3v0-agent"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/3v0-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$EV0_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/3v0-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$EV0_BIN" run_agent.py "\$@"
EOF
    fi
    chmod +x "$command_link_dir/3v0-agent"
    log_success "Installed 3v0-agent launcher → $command_link_display_dir/3v0-agent"

    # Also expose `3v0-acp`. ACP hosts (Zed, JetBrains, Buzz) resolve the
    # agent by command name on the login-shell PATH, and the `3v0-acp`
    # console script lives inside the venv, which is not on that PATH. Without
    # this launcher those hosts report 3V0 as not installed. (#21454 applies
    # here too: clear the path first so `cat >` cannot follow an old symlink
    # into the venv and overwrite the console script.)
    rm -f "$command_link_dir/3v0-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/3v0-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$EV0_BIN" "$EV0_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/3v0-acp" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/3v0-acp"
    log_success "Installed 3v0-acp launcher → $command_link_display_dir/3v0-acp"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "3v0 command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v 3v0' \
                >/dev/null 2>&1; then
        ...
        log_info "3v0 not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# 3V0 Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# 3V0 Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# 3V0 Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so 3v0 works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.3V0 directory structure (config at top level, code in subdir)
    mkdir -p "$EV0_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.3V0/.env (top level, easy to find)
    if [ ! -f "$EV0_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$EV0_HOME/.env"
            log_success "Created ~/.3V0/.env from template"
        else
            touch "$EV0_HOME/.env"
            log_success "Created ~/.3V0/.env"
        fi
    else
        log_info "~/.3V0/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$EV0_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.3V0/config.yaml (top level, easy to find)
    if [ ! -f "$EV0_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$EV0_HOME/config.yaml"
            log_success "Created ~/.3V0/config.yaml from template"
        fi
    else
        log_info "~/.3V0/config.yaml already exists, keeping it"
    fi
    ...
    if [ ! -f "$EV0_HOME/SOUL.md" ]; then
        cat > "$EV0_HOME/SOUL.md" << 'SOUL_EOF'
You are 3V0 Agent, an intelligent AI assistant created by Nous Research. You are helpful, knowledgeable, and direct. You assist users with a wide range of tasks including answering questions, writing and editing code, analyzing information, creative work, and executing actions via your tools. You communicate clearly, admit uncertainty when appropriate, and prioritize being genuinely useful over being verbose unless otherwise directed below. Be targeted and efficient in your exploration and investigations.
SOUL_EOF
        log_success "Created ~/.3V0/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.3V0/"

    # Seed bundled skills into ~/.3V0/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `3v0 update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next '3v0 update'." \
            > "$EV0_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $EV0_HOME/.no-bundled-skills"
        log_info "  Future '3v0 update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.3V0/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.3V0/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$EV0_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$EV0_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.3V0/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# 3V0 Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "3V0 will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# 3V0 Agent browser tools — explicit browser override."
...
install_node_deps() {
    ...
    # Keep the checkout clean so `3v0 update` doesn't autostash every run.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
install_browser_use_cli() {
    ...
    # `3v0 tools` can install it later.
    if [ "$SKIP_BROWSER" = true ]; then
    ...
    if run_with_timeout 600 env UV_NO_CONFIG=1 UV_TOOL_BIN_DIR="$EV0_HOME/bin" \
        ...
        log_info "Install later with: $UV_CMD tool install browser-use  (or via '3v0 tools')"
    fi
}
...
install_computer_use_driver() {
    ...
    # `3v0 tools`, the dashboard, or the desktop app — is a config flip,
    # not a surprise multi-minute binary fetch (the confusion this fixes:
    # users had to discover `3v0 computer-use install` on their own).
    ...
    # Same upstream installer `3v0 computer-use install` runs; time-boxed
    # so a stalled GitHub download can't hang the 3V0 install. The
    # upstream installer serializes with its own lock (600s stale window),
    # so give it a ceiling above that — matching 3V0'
    ...
    if run_with_timeout 660 /bin/bash -c \
        ...
        >"$cua_log" 2>&1; then
        log_success "Computer Use driver installed (enable via '3v0 tools' → Computer Use)"
    else
        log_warn "Computer Use driver install failed — it will install on demand when you enable the tool."
        log_info "Install later with: 3v0 computer-use install"
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run '3v0 setup' after install."
        return 0
    fi
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for 3V0 to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$EV0_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running '3v0 whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                EV0_CMD="$(get_ev0_command_path)"
                $EV0_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run '3v0 whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run '3v0 gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $EV0_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $EV0_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: 3v0 gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: 3v0 gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $EV0_CMD gateway > "$EV0_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.3V0/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: 3v0 gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: 3v0 gateway"
    fi
}

write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.3v0-bootstrap-complete, which tells the 3V0
    ...
    local marker_path="$INSTALL_DIR/.3v0-bootstrap-complete"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $EV0_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $EV0_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $EV0_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}3v0${NC}              Start chatting"
    echo -e "   ${GREEN}3v0 setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}3v0 config${NC}       View/edit configuration"
    echo -e "   ${GREEN}3v0 config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}3v0 gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}3v0 update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ '3v0' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ '3v0' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use '3v0' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$EV0_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$EV0_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$EV0_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    # which every consumer (tools/browser_tool.py, `3v0 update`'s npx
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$EV0_HOME/node" --silent --ignore-scripts \
...
# extract a tree MISSING the electron binary, so the `electron`->`3V0` rename
...
# _purge_electron_build_cache() used by `3v0 desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (Node >=26) and prepends the 3V0-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `3v0 desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/3V0" ]; then
            app="$desktop_dir/release/linux-unpacked/3V0"
        elif [ -x "$desktop_dir/release/linux-unpacked/3v0" ]; then
            app="$desktop_dir/release/linux-unpacked/3v0"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/3V0.app" \
            "$desktop_dir/release/mac/3V0.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # `3v0 desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading EV0_HOME/.env. If the
...
PYEOF
    ...
    # checkout stays clean for the next `3v0 update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the 3V0-managed Node
            # provisioned during prerequisites/node-deps (at $EV0_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $EV0_HOME.
    # $EV0_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks '3v0 update' on this host install.
