#!/usr/bin/env python3
"""Live test harness for 3V0 Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any 3v0 imports.
ORIGINAL_HOME = os.environ.get("EV0_HOME")
ORIGINAL_AUTH = Path.home() / ".3V0" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.3V0/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.3V0/.env`` so
