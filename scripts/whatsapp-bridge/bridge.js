 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.3V0/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.3V0', 'whatsapp', 'session'));
// Cache directories: the Python gateway passes the profile-aware paths via
// env (EV0_HOME-aware, new cache/ layout).  Fall back to the legacy
// hardcoded locations for bridges launched outside the gateway.
const IMAGE_CACHE_DIR = process.env.EV0_IMAGE_CACHE_DIR
  || path.join(process.env.HOME || '~', '.3V0', 'image_cache');
const DOCUMENT_CACHE_DIR = process.env.EV0_DOCUMENT_CACHE_DIR
  || path.join(process.env.HOME || '~', '.3V0', 'document_cache');
const AUDIO_CACHE_DIR = process.env.EV0_AUDIO_CACHE_DIR
  || path.join(process.env.HOME || '~', '.3V0', 'audio_cache');
...
// `3v0 update` updates bridge.js on disk but a long-lived bridge process
...
const DEFAULT_REPLY_PREFIX = '⚕ *3V0 Agent*\n────────────\n';
...
function enqueuePollUpdateEvent({ key, update, selectedOptions, aggregation }) {
  ...
  const pollId = key?.id
    ...
    || '';
  // Only surface votes on polls 3V0 itself created (tracked when
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['3V0 Agent', 'Chrome', '120.0'],
    ...
    for (const msg of messages) {
      ...
      if (msg.key.fromMe) {
        ...
        if (WHATSAPP_MODE === 'bot') {
          ...
          // the owner replied to would leak into 3V0 and trigger
      ...
      // Ignore 3V0' own reply messages in self-chat mode to avoid loops.
    ...
    switch (type) {
      case 'image':
        if (ext === 'gif') {
          ...
          let tmpGifMp4 = null;
          try {
            tmpGifMp4 = path.join(tmpdir(), `ev0_gif_${randomBytes(6).toString('hex')}.mp4`);
            execFileSync(
              'ffmpeg',
      ...
      case 'audio': {
        ...
        let tmpPath = null;
        if (needsConversion) {
          tmpPath = path.join(tmpdir(), `ev0_voice_${randomBytes(6).toString('hex')}.ogg`);
          try {
            execFileSync(
              'ffmpeg',
