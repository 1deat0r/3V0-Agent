#!/usr/bin/env python3
"""Drive the 3V0 TUI under EV0_DEV_PERF and summarize the pipeline.
...
~30 Hz (matching xterm key-repeat), summarizes ~/.3V0/perf.log on exit.
...
Environment overrides:
  EV0_PERF_LOG     (default ~/.3V0/perf.log)
  EV0_PERF_NODE    (default node from $PATH)
  EV0_TUI_DIR      (default: <repo>/ui-tui relative to this script)

Exit code is 0 if the harness ran and parsed results, 2 if the TUI crashed
or produced no perf data (suggests EV0_DEV_PERF wiring is broken).
"""
...
def format_report(data: dict[str, Any]) -> str:
    ...
    if not react:
        out.append("  (no react events — EV0_DEV_PERF wired? threshold too high?)")
    else:
...
def run_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    env = os.environ.copy()
    env["EV0_DEV_PERF"] = "1"
    env["EV0_DEV_PERF_MS"] = str(args.threshold_ms)
    env["EV0_DEV_PERF_LOG"] = str(log)
    env["EV0_TUI_RESUME"] = sid
    ...
    node = os.environ.get("EV0_PERF_NODE", "node")
...
def main() -> int:
    ...
    p.add_argument("--threshold-ms", type=float, default=0.0, help="EV0_DEV_PERF_MS (0 = capture all)")
