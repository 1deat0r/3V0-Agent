#!/usr/bin/env bash
# Canonical test runner for 3v0-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.3V0/...)
#
# Usage:
...
# (EV0_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
#
# A candidate must have pytest INSTALLED, not merely exist. The release venv
# at ~/.3V0/3v0-agent/venv has bin/activate but no pytest, so an
...
VENV_PYTHON=""
SKIPPED_VENVS=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.3V0/3v0-agent/venv"; do
...
if [ -n "$VENV" ]; then
  PYTHON="$VENV_PYTHON"
elif [ -n "${EV0_PYTHON:-}" ] && [ -x "$EV0_PYTHON" ] \
    && "$EV0_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: EV0_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `3v0` binary rather
  # than the devShell hook.
  PYTHON="$EV0_PYTHON"
  echo "▶ no local venv — using Nix dev venv via EV0_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and EV0_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
if [ -f "$HOME/.3V0/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.3V0"
...
#
#   * EV0_TEST_WORKERS / PATHS / FILE_TIMEOUT / FILE_RETRIES / SLICE are
#     read by run_tests_parallel.py at argparse-default time — inside the
#     stripped environment.
#   * EV0_TEST_IMAGE is read by tests/docker/conftest.py to skip its
...
#     EV0_GIT_SHA build-arg the workflow bakes in).
#
# These are test-infrastructure knobs, not credentials — same class as the
# EV0_RUN_SLOW_PET_TESTS / EV0_E2E_BROWSER opt-ins already forwarded.
# Keep this an explicit allowlist (no EV0_TEST_* glob) so the "no
# credential can leak" property stays auditable at a glance.
TEST_ENV=()
for _test_var in EV0_TEST_IMAGE EV0_TEST_WORKERS EV0_TEST_PATHS \
  EV0_TEST_FILE_TIMEOUT EV0_TEST_FILE_RETRIES EV0_TEST_SLICE; do
  if [ -n "${!_test_var:-}" ]; then
...
exec env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONUTF8=1 \
  ${EV0_RUN_SLOW_PET_TESTS:+EV0_RUN_SLOW_PET_TESTS="$EV0_RUN_SLOW_PET_TESTS"} \
  ${EV0_E2E_BROWSER:+EV0_E2E_BROWSER="$EV0_E2E_BROWSER"} \
