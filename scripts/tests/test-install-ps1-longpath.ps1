...
function Invoke-Normalization {
    ...
    try {
        ...
        # its diagnostics are all `[3v0] `-prefixed, so the two separate
        ...
        $stderr = ($raw | Where-Object { $_ -like '`[3v0`]*' }) -join "`n"
        $stdout = ($raw | Where-Object { $_ -notlike '`[3v0`]*' }) -join "`n"
    } finally {
    ...
    return @{
        ...
        Ev0Home = $(if ($paths) { $paths.ev0_home } else { $null })
...
$expectedInstallDir = "$($longRoot)${sep}AppData${sep}Local" + '\3v0\3v0-agent'
...
$result = Invoke-Normalization -Environment @{ TEMP = $shortTemp; TMP = $shortTemp } `
    -ExtraArgs @('-InstallDir', (Join-Path $shortProfile 'custom-3v0'))
Assert-Equal -Expected (Join-Path $longRoot 'custom-3v0') -Actual $result.InstallDir -Label "explicit -InstallDir keeps the caller's directory, on the long root"
