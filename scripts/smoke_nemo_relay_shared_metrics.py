"""Run a real 3V0 CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_threev0_executable(threev0_repo: Path) -> Path:
    ...
    raise SystemExit(
        "3V0 executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated EV0_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["3v0.client.active"] != [
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['3v0.client.active']}"
        )
    [model] = by_name["3v0.model_route.count"]
    expected_model = {
        "name": "3v0.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['3v0.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["3v0.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['3v0.task_run.started']}"
        )
    [terminal] = by_name["3v0.task_run.finished"]
    ...
    [tool] = by_name["3v0.tool_call.count"]
    ...
    lifecycle = by_name["3v0.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["3v0.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The 3V0 development environment requires jsonschema"
        ) from exc
    ...
    if metrics["3v0.client.active"] != [
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['3v0.client.active']}"
        )
    [model] = metrics["3v0.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['3v0.model_route.count']}"
        )
    [terminal] = metrics["3v0.task_run.finished"]
    ...
    [tool] = metrics["3v0.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["3v0.skill.lifecycle.count"]
    ...
    loads = metrics["3v0.skill.load.count"]
    if {
        (
...
def main() -> int:
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="3v0-relay-shared-metrics-"))
    home = root / "3v0-home"
    ...
    (root / "3v0.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "3v0.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"3V0 exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("3V0 model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("3V0 did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("3V0 did not print the mock model response")
    ...
    print("3V0 -> NeMo Relay shared-metrics smoke test passed")
