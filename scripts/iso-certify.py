...
What it does
------------
1. Spawns a SCRATCH dashboard (``3v0 dashboard``) bound to loopback on a
   free port, with an ISOLATED ``EV0_HOME`` (temp dir, minimal seeded state).
   It NEVER touches the live :9119 dashboard / ai.3v0.dashboard / live
   state.db. Loopback bind ⇒ no auth gate (web_server.should_require_auth).
2. Arms the synthetic GIL-heavy turn seam (``EV0_ISO_CERTIFY_SYNTH_TURN=1``,
...
the scratch EV0_HOME; ``--isolation on|off`` sets it. Run BOTH:
...
_READY_RE = re.compile(r"EV0_(?:DASHBOARD|BACKEND)_READY port=(\d+)")
...
def run_certify(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        with ScratchDashboard(
            ...
            env_extra={"EV0_DASHBOARD_SESSION_TOKEN": token},
        ) as dash:
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--isolation", choices=["on", "off"], default="on",
                   help="set dashboard.turn_isolation in the scratch EV0_HOME")
    ...
    p.add_argument("--keep-home", action="store_true", help="do not delete the scratch EV0_HOME on exit")
