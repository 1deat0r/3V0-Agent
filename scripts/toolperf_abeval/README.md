...
batch (tracker: [#77056](https://github.com/NousResearch/3v0-agent/issues/77056)).
...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  3V0 home, same model, same tasks, same reps.
...
1. Create a dedicated 3V0 home with credentials for the models under test:
   ...
   EV0_HOME=$ABEVAL_HOME 3v0 plugins enable observability/nemo_relay
   ```
