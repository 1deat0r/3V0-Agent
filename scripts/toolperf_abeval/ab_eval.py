...
measured in the production session DB) through `3v0 chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    EV0_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured 3V0 home with credentials for the
    ...
    for rep in range(reps):
        for name in TASKS:
            ...
            env.update({
                ...
                "EV0_HOME": str(HOME),
                "EV0_NEMO_RELAY_ATOF_ENABLED": "1",
                "EV0_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY": str(atof.parent),
                "EV0_NEMO_RELAY_ATOF_FILENAME": atof.name,
                "EV0_NEMO_RELAY_ATOF_MODE": "overwrite",
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "threev0_cli.main", "chat", "--query", q,
