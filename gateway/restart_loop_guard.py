...
Defenses 1 and 2 (the ``_EV0_GATEWAY`` guard on ``3v0 gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.3v0.gateway")``,
...
State lives in ``<EV0_HOME>/gateway/restart_loop.json`` so it is
