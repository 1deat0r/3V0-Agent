...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-3V0-Session-Id header; opt-in long-term memory scoping via X-3V0-Session-Key header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id; X-3V0-Session-Key supported)
- GET  /v1/responses/{response_id} — Retrieve a stored response
- DELETE /v1/responses/{response_id} — Delete a stored response
- GET  /v1/models                  — lists 3v0-agent and any configured model_routes aliases
- GET  /v1/capabilities            — machine-readable API capabilities for external UIs
- GET  /api/sessions               — list client-visible 3V0 sessions
- POST /api/sessions               — create an empty 3V0 session
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to 3v0-agent
...
# Canonical shared copy (ticket #23) — was hand-duplicated per-adapter
...
def _threev0_version() -> str:
    ...
    ``ev0_agent-*.dist-info`` after a source update until the environment is
...
def _resolve_request_runtime_agent_kwargs(provider: str, target_model: Optional[str] = None) -> Dict[str, Any]:
    ...
    """
    from threev0_cli.runtime_provider import resolve_runtime_provider, format_runtime_provider_error, _get_model_config
...
def _request_agent_overrides(
    ...
    *,
    ...
    ``/v1/models`` advertises a stable virtual model (usually ``3v0-agent``)
    ...
    ``direct_model_requests`` config value here, while 3V0-native
    endpoints (session chat, /v1/runs) always allow it.  A request that
    sends an explicit ``provider`` is unambiguously 3V0-aware and is
    always honored.
    """
...
class ResponseStore:
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        # gracefully on NFS/SMB/FUSE-mounted EV0_HOME (same filesystem
        # issue addressed for state.db/kanban.db — see
        # threev0_state._WAL_INCOMPAT_MARKERS).
...
def _derive_chat_session_id(
    ...
    the same 3V0 session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through 3v0-agent's AIAgent.
    """
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send an explicit ``provider`` — and the 3V0-native session-chat
    ...
    def _resolve_model_name(explicit: str) -> str:
        ...
        3. Fallback: "3v0-agent"

        Delegates the tiered fallthrough to
        :func:`threev0_cli.model_switch.resolve_effective_model` (the shared
        override > mid-tier > default precedence owner).
        """
        from threev0_cli.model_switch import resolve_effective_model
        ...
        try:
            ...
            pass
        return resolve_effective_model(explicit, profile_name, "3v0-agent")
    ...
    def _parse_session_key_header(
        ...
        """Extract and validate the ``X-3V0-Session-Key`` header.

        The session key is a stable per-channel identifier that scopes
        long-term memory (e.g. Honcho sessions) across transcripts.  It
        is independent of ``X-3V0-Session-Id``: callers may send
        ...
        """
        raw = request.headers.get("X-3V0-Session-Key", "").strip()
        if not raw:
            return None, None

        if not self._api_key:
            logger.warning(
                "X-3V0-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping."
            )
            return None, web.json_response(
                _openai_error(
                    "X-3V0-Session-Key requires API key authentication. "
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``3v0 sessions list``
        shows API-server conversations alongside CLI and gateway ones.

        Under multiplex ``/p/<profile>/`` requests the profile runtime scope
        redirects ``get_ev0_home()``, so each profile gets its own DB —
    ...
    def _normalize_session_source(value: Any) -> str:
        ...
        allowed = {"api_server", "ev0_browser", "browser", "cli", "telegram", "discord", "slack", "desktop", "dashboard"}
        if text in allowed:
            return "ev0_browser" if text == "browser" else text
        return "api_server"
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the 3v0-api-server default.

        ``gateway_session_key`` is a stable per-channel identifier supplied
        by the client (via ``X-3V0-Session-Key``).  Unlike ``session_id``
        ...
        # threev0_cli.model_switch.resolve_effective_model (session /model
        ...
        # (e.g. user ran `3v0 auth add openai-codex` without `3v0 model`),
        ...
        if not model and runtime_kwargs.get("provider"):
            try:
                from threev0_cli.models import get_default_model_for_provider
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — list 3v0-agent and any configured model_routes aliases.
    ...
    async def _handle_model_options(self, request: "web.Request") -> "web.Response":
        """GET /api/model/options — return 3V0 provider/model inventory.
        ...
        3V0 provider catalog instead of scraping the single OpenAI-compatible
        `/v1/models` alias.
        """
        auth_err = self._check_auth(request)
        if auth_err:
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        every 3V0 version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({
            "object": "3v0.api_server.capabilities",
            "platform": "3v0-agent",
            ...
            "runtime": {
                ...
                "split_runtime": False,
                "description": (
                    "The API server creates a server-side 3V0 AIAgent; "
            ...
            "features": {
                ...
                "session_continuity_header": "X-3V0-Session-Id",
                "session_key_header": "X-3V0-Session-Key",
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions — list persisted 3V0 sessions."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions -- create an empty 3V0 session row.
        ...
        return web.json_response({"object": "3v0.session", "session": self._session_response(session)}, status=201)
    ...
    async def _handle_get_session(self, request: "web.Request") -> "web.Response":
        ...
        if err:
            return err
        return web.json_response({"object": "3v0.session", "session": self._session_response(session)})
    ...
    async def _handle_delete_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "3v0.session.deleted", "id": session_id, "deleted": bool(deleted)})
    ...
    async def _handle_fork_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "3v0.session", "session": self._session_response(fork)}, status=201)
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        headers = {"X-3V0-Session-Id": effective_session_id or session_id}
        if gateway_session_key:
            headers["X-3V0-Session-Key"] = gateway_session_key
        runtime = {}
        ...
        return web.json_response(
            {
                "object": "3v0.session.chat.completion",
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # stable per-channel identifier via X-3V0-Session-Key.  This
        # is independent of X-3V0-Session-Id: the key persists across
        ...
        # Allow caller to continue an existing session by passing X-3V0-Session-Id.
        ...
        provided_session_id = request.headers.get("X-3V0-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-3V0-Session-Id rejected: "
            ...
            # conversation map to the same 3V0 session.  The first user
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``3v0.tool.progress`` with ``status: running``.
        ...
        response_headers = {
            "X-3V0-Session-Id": result.get("session_id", session_id),
        }
        if gateway_session_key:
            response_headers["X-3V0-Session-Key"] = gateway_session_key
        ...
        if not final_response and (is_failed or is_partial):
            ...
            response_headers["X-3V0-Completed"] = "false"
            response_headers["X-3V0-Partial"] = "true" if is_partial else "false"
            return web.json_response(err_body, status=502, headers=response_headers)

        # Soft-partial path: we have *some* text but the run did not complete
        # (e.g. truncation with partial buffered output). Still 200 but signal
        # truncation via finish_reason="length" + 3V0-specific extras.
        response_data = {
            "id": completion_id,
        ...
        if is_partial or is_failed or not completed:
            ...
            response_headers["X-3V0-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-3V0-Error"] = _redact_api_error_text(err_msg, limit=200)
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-3V0-Session-Id"] = session_id
        if gateway_session_key:
            sse_headers["X-3V0-Session-Key"] = gateway_session_key
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: 3v0.tool.progress`` SSE event so
                frontends can display them without storing the markers in
                conversation history.  See #6972 for the original event,
                #16588 for the ``toolCallId``/``status`` lifecycle fields.
                """
                if isinstance(item, tuple) and len(item) == 2 and item[0] == "__tool_progress__":
                    await response.write(_sse_frame(item[1], event="3v0.tool.progress"))
                else:
                    content_chunk = {
                        "id": completion_id, "object": "chat.completion.chunk",
                        "created": created, "model": model,
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        # response and the X-3V0-Session-Id header.  Without this,
        ...
        response_headers = {"X-3V0-Session-Id": _effective_session_id}
        if gateway_session_key:
            response_headers["X-3V0-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
            ...
            if role == "assistant" and msg.get("tool_calls"):
                for tc in msg["tool_calls"]:
                    ...
                    items.append({
                        ...
                        # 3V0 agent; they are replayed for structured tool
        ...
        def _run():
            ...
            with self._profile_scope(request_profile):
                ...
                try:
                    ...
                    # (e.g. X-3V0-Session-Id header) can track compression-
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = (
            {"X-3V0-Session-Key": gateway_session_key} if gateway_session_key else {}
        )
        return web.json_response(
    ...
    async def _handle_run_approval(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "3v0.run.approval_response",
            "run_id": run_id,
            "choice": choice,
            "resolved": resolved,
        })
    ...
    async def _handle_steer_run(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "3v0.run.steer", "run_id": run_id, "accepted": True})
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Store the adapter after native routes are registered. Local 3V0-Relay
            ...
            # that is the exact surface the 3v0-0day campaign abused to write
            # ~/.3V0/config.yaml and plant persistence. Sandboxing (Docker /
