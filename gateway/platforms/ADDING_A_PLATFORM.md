...
There are two ways to add a platform to the 3V0 gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.3V0/plugins/` (or under `plugins/platforms/`
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `3v0 gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `threev0_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the 3V0 core.
...
And add it to the `3v0-gateway` composite:

```python
"3v0-gateway": {
    "includes": [..., "3v0-your-platform"]
}
```
...
## 12. Status Display (`threev0_cli/status.py`)
...
## 13. Gateway Setup Wizard (`threev0_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ threev0_cli/ toolsets.py \
  --include="*.py" -l | sort -u
