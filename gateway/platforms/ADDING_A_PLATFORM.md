...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `ev0_cli/config.py` so the setup
...
## 12. Status Display (`ev0_cli/status.py`)
...
## 13. Gateway Setup Wizard (`ev0_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ ev0_cli/ toolsets.py \
  --include="*.py" -l | sort -u
