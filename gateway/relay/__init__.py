"""Relay/connector support package for the 3V0 gateway.
...
def relay_connection_auth() -> tuple[Optional[str], Optional[str]]:
    ...
    Both come from enrollment (``3v0 gateway enroll`` writes them to
    ``~/.3V0/.env``): ``GATEWAY_RELAY_ID`` identifies the enrolled instance,
...
def relay_endpoint() -> Optional[str]:
    ...
    ``EV0_DASHBOARD_PUBLIC_URL``); a hosted/NAS container has the same var
...
def relay_wake_url() -> Optional[str]:
    ...
    sets it explicitly (or passes ``--wake-url`` to ``3v0 gateway enroll``).
...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        ...
        # multi-agent scope it would prefix every reply "**3V0 Agent:**" —
...
def _resolve_relay_identity_token() -> str:
    ...
    ``3v0 gateway enroll`` CLI. Three modes, in precedence order:
...
def self_provision_relay() -> bool:
    ...
    up. The creds live ONLY in process memory — never written to ``~/.3V0/.env``.
    ...
    sets neither ``EV0_MANAGED`` nor a ``.managed`` marker), so gating on it
    ...
    self-guarding:
      ...
      - A self-hosted operator who ran ``3v0 gateway enroll``: has a PINNED
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or '3v0'}"
