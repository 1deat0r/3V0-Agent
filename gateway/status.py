...
The PID file lives at ``{EV0_HOME}/gateway.pid``.  EV0_HOME defaults to
``~/.3V0`` but can be overridden via the environment variable.  This means
separate EV0_HOME directories naturally get separate PID files — a property
...
from ev0_constants import get_ev0_home, _get_platform_default_ev0_home
...
def _get_process_ev0_home() -> Path:
    ...
    ``get_ev0_home()`` honors ``_EV0_HOME_OVERRIDE`` contextvar used for
...
def _get_lock_dir() -> Path:
    ...
    return state_home / "3v0" / _LOCKS_DIRNAME
...
# Reject epoch values before 2000-01-01T00:00:00Z: nothing in 3V0' lifetime
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the 3V0 gateway lifecycle subcommand from a command line.
    ...
    test also matched ``ev0_cli.main gateway status`` and even unrelated
    ...
    (``"C:\\Program Files\\...\\3v0-gateway.exe"``) survive, and strips
    ``--profile``/``-p`` selectors from anywhere in argv -- 3V0's
    ...
    for token in tokens:
        ...
        if basename in ("3v0-gateway", "3v0-gateway.exe"):
            return "run"

    joined = " ".join(tokens)
    has_gateway_entry = (
        "ev0_cli.main" in joined
        or "ev0_cli/main.py" in joined
        or any(t.rsplit("/", 1)[-1] in ("3v0", "3v0.exe") for t in tokens)
    )
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `3v0 gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating 3V0-owned runtime records
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the 3V0 gateway."""
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a EV0_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.3V0`` or ``$EV0_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    Mirrors ``ev0_cli.gateway._matches_current_profile`` so the dashboard's
    ...
    explicit ``EV0_HOME=<path>``) on its argv; the default/root gateway runs
    bare with no profile flag.
    """
    # Normalize separators before the substring match: on Windows,
    # str(Path) renders backslashes while a EV0_HOME= value on the argv
    ...
    if profile_name is not None and profile_name != "default":
        ...
        return (
            ...
            or f"ev0_home={home_lc}" in command_lc
        )
    ...
    # a non-matching explicit EV0_HOME= on the argv. EV0_HOME is usually
    ...
    if "--profile " in command_lc or " -p " in command_lc:
        return False
    if "ev0_home=" in command_lc and f"ev0_home={home_lc}" not in command_lc:
        return False
    return True
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `3v0 status` show per-profile
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``EV0_HOME`` in-process.  Defaults to
...
def resolve_gateway_liveness(
    *,
    ...
    passes its ``ev0_cli.web_server`` bindings so the long-standing
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    EV0_HOME.  Pass it when validating *another* profile's state file (the
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different EV0_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``3v0.service`` + ``3v0-
# gateway.service``).
#
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # TARGET home.  That permits a deliberate cross-EV0_HOME --replace while
    ...
    # ``3v0 gateway stop`` on Windows would be misclassified as an
...
def write_takeover_marker(
    ...
    *,
    ...
    gateway's EV0_HOME rather than the replacer's.  Same-home callers omit
...
def _validated_scoped_lock_gateway_owner(
    ...
    the lock record, the target EV0_HOME's gateway PID record, and the live
