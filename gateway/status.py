...
def _gateway_command_subcommand(command: str | None) -> str | None:
    ...
    test also matched ``threev0_cli.main gateway status`` and even unrelated
    ...
    joined = " ".join(tokens)
    has_gateway_entry = (
        "threev0_cli.main" in joined
        or "threev0_cli/main.py" in joined
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    Mirrors ``threev0_cli.gateway._matches_current_profile`` so the dashboard's
...
def resolve_gateway_liveness(
    *,
    ...
    passes its ``threev0_cli.web_server`` bindings so the long-standing
