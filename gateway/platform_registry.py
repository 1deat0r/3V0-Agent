...
class PlatformEntry:
    ...
    # status displays (``3v0 setup``, ``3v0 status``, the dashboard
    ...
    # Env vars this platform needs (for ``3v0 setup`` display).
    ...
    # built-ins).  Used by ``3v0 gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved EV0_HOME and overlay the
        ...
        # plugin-discovery time added several seconds to *every* `3v0`
        # invocation -- including plain `3v0 chat`, which never touches any
        ...
        # `3v0 setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``3v0 setup``/``gateway status``, channel
