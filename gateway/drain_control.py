...
Contract (presence-based, mirroring ``.restart_notify.json``):
  ...
  * begin-drain  → write ``{EV0_HOME}/.drain_request.json`` with
...
Why the epoch (NS-570). ``EV0_HOME`` is a **durable** store — on 3V0
...
def current_instantiation_epoch() -> str:
    ...
    Together they discriminate every restart mode that matters:
      ...
      | host ``3v0 gateway restart``| same    | same(init) | SAME   | honour |
...
def write_drain_request(
    *,
    ...
    later survives a machine restart on the durable EV0_HOME volume can be
...
def drain_requested(*, home: Optional[Path] = None) -> bool:
    ...
    treated as absent: it survived a container/VM restart (EV0_HOME is a
    durable Fly volume on 3V0 Cloud) and the lifecycle action that triggered
...
def drain_notification_suppressed(*, home: Optional[Path] = None) -> bool:
    ...
    EV0_HOME volume, or an expired same-epoch orphan (#85433), is
